...
void S3CrtClient::init(const ClientConfiguration& config, const Aws::Auth::AWSCredentials* credentials)
{
  ...
  static const uint64_t DEFAULT_PART_SIZE = 5 * 1024 * 1024; // 5MB
    ...
    int response_status, void *user_data)
{
  ...
  for (size_t i = 0; i < headersCount; i++)
  {
    ...
    userData->response->AddHeader(StringUtils::AwsStringFromByteCursor(header.name), StringUtils::AwsStringFromByteCursor(header.value));
  }
...
void S3CrtClient::GetObjectAsync(const GetObjectRequest& request, const GetObjectResponseReceivedHandler& handler, const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context) const
{
  ...
  std::shared_ptr<Aws::Crt::Http::HttpRequest> crtHttpRequest = Aws::Http::HttpRequestToCrtHttpRequest(*(userData->request));
...
ComputeEndpointOutcome S3CrtClient::ComputeEndpointString(const Aws::String& bucketOrArn) const
{
    ...
    if (arn)
    {
        ...
        S3CrtARNOutcome s3ArnOutcome = m_useArnRegion ? arn.Validate() : arn.Validate(m_region.c_str());
        ...
        {
            ss << S3CrtEndpoint::ForAccessPointArn(arn, m_useArnRegion ? "" : m_region, m_useDualStack);
        ...
        else if (arn.GetResourceType() == ARNResourceType::OUTPOST)
        {
            ...
            ss << S3CrtEndpoint::ForOutpostsArn(arn, m_useArnRegion ? "" : m_region);
