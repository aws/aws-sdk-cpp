...
namespace Model
{
  ...
  public:
     ...
     * of correctly classified items to the total number of (correctly and incorrectly)
     * classified items. It is used for binary and multiclass classification. It
     * measures how close the predicted class values are to the actual values. Accuracy
     * values vary between zero and one: one indicates perfect accuracy and zero
     * indicates perfect inaccuracy.</p> </li> <li> <p> <code>F1</code>: The F1 score
     * is the harmonic mean of the precision and recall. It is used for binary
     * classification into classes traditionally referred to as positive and negative.
     * Predictions are said to be true when they match their actual (correct) class and
     * false when they do not. Precision is the ratio of the true positive predictions
     * to all positive predictions (including the false positives) in a data set and
     ...
     * actual positive instances and measures how completely a model predicts the
     * actual class members in a data set. The standard F1 score weighs precision and
     ...
     * best possible performance and zero the worst.</p> </li> <li> <p>
     * <code>AUC</code>: The area under the curve (AUC) metric is used to compare and
     * evaluate binary classification by algorithms such as logistic regression that
     * return probabilities. A threshold is needed to map the probabilities into
     ...
     * negative example. AUC scores vary between zero and one: a score of one indicates
     * perfect accuracy and a score of one half indicates that the prediction is not
     * better than a random classifier. Values under one half predict less accurately
     * than a random predictor. But such consistently bad predictors can simply be
     * inverted to obtain better than random predictors.</p> </li> <li> <p>
     ...
     * class in binary classification. Then, use these values to calculate the F1 score
     * for each class and average them to obtain the F1macro score. F1macro scores vary
     * between zero and one: one indicates the best possible performance and zero the
