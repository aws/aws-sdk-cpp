    ...
    public:
      typedef Aws::Client::AWSJsonClient BASECLASS;
        ...
        */
         ...
         * Kinesis Firehose delivery stream can be configured to receive records directly
         * from providers using <a>PutRecord</a> or <a>PutRecordBatch</a>, or it can be
         * configured to use an existing Kinesis stream as its source. To specify a Kinesis
         * stream as input, set the <code>DeliveryStreamType</code> parameter to
         * <code>KinesisStreamAsSource</code>, and provide the Kinesis stream ARN and role
         * ARN in the <code>KinesisStreamSourceConfiguration</code> parameter.</p> <p>A
         * delivery stream is configured with a single destination: Amazon S3, Amazon ES,
         * or Amazon Redshift. You must specify only one of the following destination
         ...
         * or <b>RedshiftDestinationConfiguration</b>.</p> <p>When you specify
         ...
         * provided, Kinesis Firehose buffers data up to 5 MB or for 5 minutes, whichever
         * condition is satisfied first. Note that <b>BufferingHints</b> is a hint, so
         * there are some cases where the service cannot adhere to these conditions
         * strictly; for example, record boundaries are such that the size is a little over
         * or under the configured buffering size. By default, no encryption is performed.
         * We strongly recommend that you enable encryption to ensure secure data storage
         * in Amazon S3.</p> <p>A few notes about Amazon Redshift as a destination:</p>
         * <ul> <li> <p>An Amazon Redshift destination requires an S3 bucket as
         * intermediate location, as Kinesis Firehose first delivers data to S3 and then
         * uses <code>COPY</code> syntax to load data into an Amazon Redshift table. This
         * is specified in the <b>RedshiftDestinationConfiguration.S3Configuration</b>
         ...
         * you use the user name and password you provide exclusively with Kinesis
         * Firehose, and that the permissions for the account are restricted for Amazon
         * Redshift <code>INSERT</code> permissions.</p> </li> </ul> <p>Kinesis Firehose
         * assumes the IAM role that is configured as part of the destination. The role
         * should allow the Kinesis Firehose principal to assume the role, and the role
         ...
         * and so on.</p> <p>Kinesis Firehose buffers records before delivering them to the
         ...
         * destination.</p> <p>The <a>PutRecord</a> operation returns a <b>RecordId</b>,
         * which is a unique string assigned to each record. Producer applications can use
         * this ID for purposes such as auditability and investigation.</p> <p>If the
         * <a>PutRecord</a> operation throws a <b>ServiceUnavailableException</b>, back off
         * and retry. If the exception persists, it is possible that the throughput limits
         * have been exceeded for the delivery stream. </p> <p>Data records sent to Kinesis
         ...
         * could be a segment from a log file, geographic location data, web site
         * clickstream data, and so on.</p> <p>Kinesis Firehose buffers records before
         ...
         * unsuccessfully processed records. Kinesis Firehose attempts to process all
         ...
         * been exceeded for the delivery stream.</p> <p>Data records sent to Kinesis
         * Firehose are stored for 24 hours from the time they are added to a delivery
         * stream as it attempts to send the records to the destination. If the destination
         * is unreachable for more than 24 hours, the data is no longer
         * available.</p><p><h3>See Also:</h3>   <a
         ...
         * the destination type is the same, Kinesis Firehose merges the configuration
         ...
         * Amazon S3 to Amazon Redshift, Kinesis Firehose does not merge any parameters. In
         * this case, all parameters must be specified.</p> <p>Kinesis Firehose uses
