...
namespace Model
{
  ...
  public:
    ...
    /**
     * <p>The Region ID of the AWS Region for this replica key. </p> <p>Enter the
     * Region ID, such as <code>us-east-1</code> or <code>ap-southeast-2</code>. For a
     * list of AWS Regions in which AWS KMS is supported, see <a
     * href="https://docs.aws.amazon.com/general/latest/gr/kms.html#kms_region">AWS KMS
     * service endpoints</a> in the <i>Amazon Web Services General Reference</i>.</p>
     * <p>The replica must be in a different AWS Region than its primary key and other
     * replicas of that primary key, but in the same AWS partition. AWS KMS must be
     * available in the replica Region. If the Region is not enabled by default, the
     * AWS account must be enabled in the Region. </p> <p>For information about AWS
     * partitions, see <a
    ...
    /**
     * <p>The key policy to attach to the CMK. This parameter is optional. If you do
     ...
     * key policy</a> to the CMK.</p> <p>The key policy is not a shared property of
     * multi-Region keys. You can specify the same key policy or a different key policy
     * for each key in a set of related multi-Region keys. AWS KMS does not synchronize
     ...
     * the caller <code>kms:PutKeyPolicy</code> permission on the replica CMK. This
     * reduces the risk that the CMK becomes unmanageable. For more information, refer
     * to the scenario in the <a
     ...
     * Access Management User Guide</i>.</p> </li> <li> <p>The key policy size quota is
     * 32 kilobytes (32768 bytes).</p> </li> </ul>
     */
    ...
    /**
     * <p>A description of the CMK. Use a description that helps you decide whether the
     ...
     * key in a set of related multi-Region keys. AWS KMS does not synchronize this
     * property.</p>
     */
     ...
     * operation.</p>  <p>Tagging or untagging a CMK can allow or deny permission
     * to the CMK. For details, see <a
     ...
     * Guide</i>.</p>  <p>To use this parameter, you must have <a
     ...
     * in a set of related multi-Region keys. AWS KMS does not synchronize this
     ...
     * string. You cannot have more than one tag on a CMK with the same tag key. If you
     * specify an existing tag key with a different tag value, AWS KMS replaces the
     * current tag value with the specified one.</p> <p>When you assign tags to an AWS
     * resource, AWS generates a cost allocation report with usage and costs aggregated
