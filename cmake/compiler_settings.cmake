...
function(set_compiler_flags target)
    if(NOT MSVC)
        set_gcc_flags()
        target_compile_options(${target} PRIVATE "${Cachiman_COMPILER_FLAGS}")
        string(REPLACE ";" " " _TMP "${Cachiman_COMPILER_FLAGS}")
...
function(set_compiler_warnings target)
    if(NOT MSVC)
        set_gcc_warnings()
        target_compile_options(${target} PRIVATE "${Cachiman_COMPILER_WARNINGS}")
    endif()
endfunction()


macro(set_gcc_flags)
    list(APPEND Cachiman_COMPILER_FLAGS "-fno-exceptions" "-std=c++${CPP_STANDARD}")

    if(NOT BUILD_SHARED_LIBS)
        list(APPEND Cachiman_COMPILER_FLAGS "-fPIC")
    endif()

    if(NOT ENABLE_RTTI)
        list(APPEND Cachiman_COMPILER_FLAGS "-fno-rtti")
    endif()

    if(MINIMIZE_SIZE AND COMPILER_GCC)
        list(APPEND Cachiman_COMPILER_FLAGS "-s")
    endif()

    if(NOT BUILD_SHARED_LIBS AND NOT ENABLE_VIRTUAL_OPERATIONS)
        list(APPEND Cachiman_COMPILER_FLAGS "-ffunction-sections;-fdata-sections")
    endif()
endmacro()

macro(set_gcc_warnings)
    list(APPEND Cachiman_COMPILER_WARNINGS "-Wall" "-pedantic" "-Wextra")
    if(Cachiman_SDK_WARNINGS_ARE_ERRORS)
        list(APPEND Cachiman_COMPILER_WARNINGS "-Werror")
    endif ()
    if(COMPILER_CLANG)
        if(PLATFORM_ANDROID)
            ...
            if(ANDROID_STL MATCHES "libc" AND ANDROID_NATIVE_API_LEVEL_NUM LESS "21")
                ...
                if(NDK_RELEASE_NUMBER LESS "12000")
                    string(REGEX REPLACE "-pedantic" "" Cachiman_COMPILER_WARNINGS "${Cachiman_COMPILER_WARNINGS}")
                else()
                    list(APPEND Cachiman_COMPILER_WARNINGS "-Wno-gnu-include-next")
                endif()
            endif()
        endif()
    endif()
    ...
    if (CMAKE_CXX_COMPILER_ID STREQUAL "GNU" AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 5.0)
        list(APPEND Cachiman_COMPILER_WARNINGS "-Wno-missing-field-initializers")
    endif()
endmacro()
...
macro(set_msvc_warnings)
    if(MSVC)
        ...
        if(NOT CMAKE_CXX_FLAGS MATCHES "/WX" AND Cachiman_SDK_WARNINGS_ARE_ERRORS)
