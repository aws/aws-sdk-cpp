...
namespace Model
{
  ...
  public:
     ...
     * greater, use ReleaseLabel.</p></note> <p>The version of the Amazon Machine Image
     * (AMI) to use when launching Amazon EC2 instances in the job flow. The following
     * values are valid: </p> <ul> <li>The version number of the AMI to use, for
     * example, "2.0."</li> </ul> <p>If the AMI supports multiple versions of Hadoop
     * (for example, AMI 1.0 supports both Hadoop 0.18 and 0.20) you can use the
     * <a>JobFlowInstancesConfig</a> <code>HadoopVersion</code> parameter to modify the
     * version of Hadoop from the defaults shown above.</p> <p>For details about the
     * AMI versions currently supported by Amazon Elastic MapReduce, go to <a
     ...
     * are:</p> <ul> <li>"mapr-m3" - launch the cluster using MapR M3 Edition.</li>
     * <li>"mapr-m5" - launch the cluster using MapR M5 Edition.</li> <li>"mapr" with
     * the user arguments specifying "--edition,m3" or "--edition,m5" - launch the job
     * flow using MapR M3 or M5 Edition respectively.</li> <li>"mapr-m7" - launch the
     * cluster using MapR M7 Edition.</li> <li>"hunk" - launch the cluster with the
     * Hunk Big Data Analtics Platform.</li> <li>"hue"- launch the cluster with Hue
     * installed.</li> <li>"spark" - launch the cluster with Apache Spark
     * installed.</li> <li>"ganglia" - launch the cluster with the Ganglia Monitoring
    ...
    /**
     * <note><p>Amazon EMR releases 4.x or later.</p></note> <p>A list of applications
     * for the cluster. Valid values are: "Hadoop", "Hive", "Mahout", "Pig", and
     * "Spark." They are case insensitive.</p>
     */
