...
namespace Model
{
  ...
  public:
     ...
     * action.</p> <p>If the action type is <code>forward</code>, you can specify a
     * single target group. The protocol of the target group must be HTTP or HTTPS for
     * an Application Load Balancer or TCP for a Network Load Balancer.</p> <p>If the
     * action type is <code>authenticate-oidc</code>, you can use an identity provider
     * that is OpenID Connect (OIDC) compliant to authenticate users as they access
     * your application.</p> <p>If the action type is
