...
HttpRequestMethodConditionConfig& HttpRequestMethodConditionConfig::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    if(!valuesNode.IsNull())
    {
      ...
      {
        m_values.push_back(HttpRequestMethodEnumMapper::GetHttpRequestMethodEnumForName(StringUtils::Trim(valuesMember.GetText().c_str())));
...
void HttpRequestMethodConditionConfig::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const
{
  if(m_valuesHasBeenSet)
  {
      ...
      {
        oStream << location << index << locationValue << ".Values.member." << valuesIdx++ << "=" << HttpRequestMethodEnumMapper::GetNameForHttpRequestMethodEnum(item) << "&";
      }
  }
...
void HttpRequestMethodConditionConfig::OutputToStream(Aws::OStream& oStream, const char* location) const
{
  if(m_valuesHasBeenSet)
  {
      ...
      for(auto& item : m_values)
      {
        oStream << location << ".Values.member." << valuesIdx++ << "=" << HttpRequestMethodEnumMapper::GetNameForHttpRequestMethodEnum(item) << "&";
      }
  }
}
