...
ModifyTargetGroupAttributesResult& ModifyTargetGroupAttributesResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "ModifyTargetGroupAttributesResult"))
  {
    resultNode = rootNode.FirstChild("ModifyTargetGroupAttributesResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::ElasticLoadBalancingv2::Model::ModifyTargetGroupAttributesResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
