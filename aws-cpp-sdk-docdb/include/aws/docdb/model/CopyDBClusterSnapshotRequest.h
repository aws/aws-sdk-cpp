...
namespace Model
{
  ...
  public:
     ...
     * is in the same Region as the copy, specify a valid snapshot identifier.</p>
     ...
     * <code>KmsKeyId</code>, then the copy of the cluster snapshot is encrypted with
     * the same KMS key as the source cluster snapshot.</p> <p>If you copy an encrypted
     * cluster snapshot that is shared from another account, then you must specify a
     * value for <code>KmsKeyId</code>.</p> <p>To copy an encrypted cluster snapshot to
     * another Region, set <code>KmsKeyId</code> to the KMS key ID that you want to use
     ...
     * can't use encryption keys from one Region in another Region.</p> <p>If you copy
     * an unencrypted cluster snapshot and specify a value for the
     ...
     * the<code>CopyDBClusterSnapshot</code> API action in the Region that contains the
     * source cluster snapshot to copy. You must use the <code>PreSignedUrl</code>
     * parameter when copying a cluster snapshot from another Region.</p> <p>If you are
     * using an Amazon Web Services SDK tool or the CLI, you can specify
     ...
     * for the operation that can be executed in the source Region.</p> <p>The
     * presigned URL must be a valid request for the <code>CopyDBClusterSnapshot</code>
     * API action that can be executed in the source Region that contains the cluster
     * snapshot to be copied. The presigned URL request must contain the following
     * parameter values:</p> <ul> <li> <p> <code>SourceRegion</code> - The ID of the
     * region that contains the snapshot to be copied.</p> </li> <li> <p>
     ...
     * copying an encrypted cluster snapshot from the us-east-1 Region, then your
