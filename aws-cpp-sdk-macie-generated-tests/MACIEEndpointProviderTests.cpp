...
static const char* ALLOCATION_TAG = "AWSMacieEndpointProviderTests";
using MacieEndpointProvider = Aws::Macie::Endpoint::MacieEndpointProvider;
using EndpointParameters = Aws::Vector<Aws::Endpoint::EndpointParameter>;
using MacieResolveEndpointOutcome = Aws::Macie::Endpoint::MacieEndpointProvider::MacieResolveEndpointOutcome;
...
class MacieEndpointProviderTests :public ::testing::TestWithParam<size_t>
{
public:
...
struct MacieEndpointProviderEndpointTestCase
{
...
static const Aws::Vector<MacieEndpointProviderEndpointTestCase> TEST_CASES = {
  /*TEST CASE 0*/
...
void ValidateOutcome(const MacieResolveEndpointOutcome& outcome, const MacieEndpointProviderEndpointTestCase::Expect& expect)
{
...
TEST_P(MacieEndpointProviderTests, EndpointProviderTest)
{
    ...
    const MacieEndpointProviderEndpointTestCase& TEST_CASE = TEST_CASES.at(TEST_CASE_IDX);
    SCOPED_TRACE(Aws::String("\nTEST CASE # ") + Aws::Utils::StringUtils::to_string(TEST_CASE_IDX) + ": " + TEST_CASE.documentation);

    std::shared_ptr<MacieEndpointProvider> endpointProvider = Aws::MakeShared<MacieEndpointProvider>(ALLOCATION_TAG);
    ASSERT_TRUE(endpointProvider) << "Failed to allocate/initialize MacieEndpointProvider";
    ...
    for(const auto& operation : TEST_CASE.operationInput)
    {
         ...
         */
        const Aws::Vector<std::reference_wrapper<const MacieEndpointProviderEndpointTestCase::OperationParamsFromTest>>
