...
PolicyTypeDescription& PolicyTypeDescription::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    XmlNode policyAttributeTypeDescriptionsNodeParent = resultNode.FirstChild("PolicyAttributeTypeDescriptions");
    XmlNode policyAttributeTypeDescriptionsNode = policyAttributeTypeDescriptionsNodeParent.FirstChild("member");
    if(!policyAttributeTypeDescriptionsNode.IsNull())
    {
      while(!policyAttributeTypeDescriptionsNode.IsNull())
      {
        m_policyAttributeTypeDescriptions.push_back(policyAttributeTypeDescriptionsNode);
        policyAttributeTypeDescriptionsNode = policyAttributeTypeDescriptionsNode.NextNode("member");
      }
...
void PolicyTypeDescription::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const
{
  ...
  if(m_policyAttributeTypeDescriptionsHasBeenSet)
  {
      for(auto& item : m_policyAttributeTypeDescriptions)
      {
        ...
        policyAttributeTypeDescriptionsSs << location << index << locationValue << ".PolicyAttributeTypeDescriptions";
...
void PolicyTypeDescription::OutputToStream(Aws::OStream& oStream, const char* location) const
{
  ...
  if(m_policyAttributeTypeDescriptionsHasBeenSet)
  {
      for(auto& item : m_policyAttributeTypeDescriptions)
      {
        ...
        locationAndListMember += ".PolicyAttributeTypeDescriptions";
