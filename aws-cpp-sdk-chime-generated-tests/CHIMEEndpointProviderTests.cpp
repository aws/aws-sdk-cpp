...
static const char* ALLOCATION_TAG = "AWSChimeEndpointProviderTests";
using ChimeEndpointProvider = Aws::Chime::Endpoint::ChimeEndpointProvider;
using EndpointParameters = Aws::Vector<Aws::Endpoint::EndpointParameter>;
using ChimeResolveEndpointOutcome = Aws::Chime::Endpoint::ChimeEndpointProvider::ChimeResolveEndpointOutcome;
...
class ChimeEndpointProviderTests :public ::testing::TestWithParam<size_t>
{
public:
...
struct ChimeEndpointProviderEndpointTestCase
{
...
static const Aws::Vector<ChimeEndpointProviderEndpointTestCase> TEST_CASES = {
  /*TEST CASE 0*/
  {"For region aws-global with FIPS disabled and DualStack disabled", // documentation
    ...
    {}, // tags
    {{/*epUrl*/"https://chime.us-east-1.amazonaws.com",
       {/*properties*/{"authSchemes", {EpProp("signingRegion", "us-east-1"), EpProp("name", "sigv4"), EpProp("signingName", "chime")}}},
...
void ValidateOutcome(const ChimeResolveEndpointOutcome& outcome, const ChimeEndpointProviderEndpointTestCase::Expect& expect)
{
...
TEST_P(ChimeEndpointProviderTests, EndpointProviderTest)
{
    ...
    const ChimeEndpointProviderEndpointTestCase& TEST_CASE = TEST_CASES.at(TEST_CASE_IDX);
    SCOPED_TRACE(Aws::String("\nTEST CASE # ") + Aws::Utils::StringUtils::to_string(TEST_CASE_IDX) + ": " + TEST_CASE.documentation);

    std::shared_ptr<ChimeEndpointProvider> endpointProvider = Aws::MakeShared<ChimeEndpointProvider>(ALLOCATION_TAG);
    ASSERT_TRUE(endpointProvider) << "Failed to allocate/initialize ChimeEndpointProvider";
    ...
    for(const auto& operation : TEST_CASE.operationInput)
    {
         ...
         */
        const Aws::Vector<std::reference_wrapper<const ChimeEndpointProviderEndpointTestCase::OperationParamsFromTest>>
