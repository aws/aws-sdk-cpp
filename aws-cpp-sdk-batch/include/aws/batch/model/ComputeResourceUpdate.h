...
namespace Model
{
  ...
  public:
     ...
     * providing an empty list is handled as if this parameter wasn't specified and no
     * change is made. For EC2 compute resources, providing an empty list removes the
     * security groups from the compute resource.</p> <p>When updating a compute
     * environment, changing the EC2 security groups requires an infrastructure update
     * of the compute environment. For more information, see <a
     * href="https://docs.aws.amazon.com/batch/latest/userguide/updating-compute-environments.html">Updating
     * compute environments</a> in the <i>Batch User Guide</i>.</p>
     */
     ...
     * parameter isn't applicable to jobs that are running on Fargate resources, and
     * shouldn't be specified.</p>  <dl> <dt>BEST_FIT_PROGRESSIVE</dt> <dd>
     * <p>Batch will select additional instance types that are large enough to meet the
     * requirements of the jobs in the queue, with a preference for instance types with
     * a lower cost per unit vCPU. If additional instances of the previously selected
     * instance types aren't available, Batch will select new instance types.</p> </dd>
     * <dt>SPOT_CAPACITY_OPTIMIZED</dt> <dd> <p>Batch will select one or more instance
     ...
     * <code>SPOT_CAPACITY_OPTIMIZED</code> strategies, Batch might need to go above
     ...
     * parameter isn't applicable to jobs that are running on Fargate resources, and
     * shouldn't be specified.</p>   <p>When you create a compute
     ...
     * the same compute environment.</p>   <p>Currently,
     * <code>optimal</code> uses instance types from the C4, M4, and R4 instance
     ...
     * using the Batch <code>ListTagsForResource</code> API operation.</p> <p>When
     * updating a compute environment, changing this setting requires an infrastructure
     ...
     * <p>When updating a compute environment, changing the bid percentage requires an
     * infrastructure update of the compute environment. For more information, see <a
     * href="https://docs.aws.amazon.com/batch/latest/userguide/updating-compute-environments.html">Updating
     * compute environments</a> in the <i>Batch User Guide</i>.</p>  <p>This
     * parameter isn't applicable to jobs that are running on Fargate resources, and
     ...
     * value is <code>false</code>.</p>  <p>If an AMI ID is specified in the
     * <code>imageId</code> or <code>imageIdOverride</code> parameters or by the launch
     * template specified in the <code>launchTemplate</code> parameter, this parameter
     ...
     * compute environments</a> in the <i>Batch User Guide</i>.</p>  <p>This
     * parameter isn't applicable to jobs that are running on Fargate resources, and
     * shouldn't be specified.</p>   <p>The AMI that you choose for a
