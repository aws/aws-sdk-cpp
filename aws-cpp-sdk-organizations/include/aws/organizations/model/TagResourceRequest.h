...
namespace Model
{
  ...
  public:
    ...
    /**
     * <p>A list of tags to add to the specified resource.</p> <p>You can specify any
     * of the following taggable resources.</p> <ul> <li> <p>AWS account – specify the
     ...
     * </li> </ul> <p>For each tag in the list, you must specify both a tag key and a
     * value. You can set the value to an empty string, but you can't set it to
     * <code>null</code>.</p>  <p>If any one of the tags is invalid or if you
     * exceed the allowed number of tags for an account user, then the entire request
     * fails and the account is not created.</p> 
     */
