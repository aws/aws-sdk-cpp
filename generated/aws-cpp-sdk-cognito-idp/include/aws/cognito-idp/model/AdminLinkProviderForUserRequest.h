...
namespace Model
{
  ...
  public:
    ...
    /**
     * <p>An external identity provider account for a user who doesn't exist yet in the
     * user pool. This user must be a federated user (for example, a SAML or Facebook
     * user), not another native user.</p> <p>If the <code>SourceUser</code> is using a
     * federated social identity provider, such as Facebook, Google, or Login with
     ...
     * <code>LoginWithAmazon</code>, and Amazon Cognito will automatically parse the
     ...
     * social identity provider token.</p> <p/> <p>For SAML, the
     * <code>ProviderAttributeName</code> can be any value that matches a claim in the
     * SAML assertion. If you want to link SAML users based on the subject of the SAML
     * assertion, you should map the subject to a claim through the SAML identity
     * provider and submit that claim name as the <code>ProviderAttributeName</code>.
     * If you set <code>ProviderAttributeName</code> to <code>Cognito_Subject</code>,
     * Amazon Cognito will automatically parse the default unique identifier found in
     * the subject from the SAML token.</p>
     */
