...
class ListStudiosResult {
 public:
  ...
  void SetStudios(StudiosT&& value) {
    m_studiosHasBeenSet = true;
    m_studios = std::forward<StudiosT>(value);
  }
  template <typename StudiosT = Aws::Vector<StudioSummary>>
  ListStudiosResult& WithStudios(StudiosT&& value) {
    SetStudios(std::forward<StudiosT>(value));
    return *this;
  }
  template <typename StudiosT = StudioSummary>
  ListStudiosResult& AddStudios(StudiosT&& value) {
    m_studiosHasBeenSet = true;
    m_studios.emplace_back(std::forward<StudiosT>(value));
    return *this;
  }
  ///@}
