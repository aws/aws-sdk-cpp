...
class ListStepsRequest : public EMRRequest {
 public:
  ...
  void SetStepStates(StepStatesT&& value) {
    ...
    m_stepStates = std::forward<StepStatesT>(value);
  }
  template <typename StepStatesT = Aws::Vector<StepState>>
  ListStepsRequest& WithStepStates(StepStatesT&& value) {
    SetStepStates(std::forward<StepStatesT>(value));
    return *this;
  }
  inline ListStepsRequest& AddStepStates(StepState value) {
    m_stepStatesHasBeenSet = true;
    m_stepStates.push_back(value);
    return *this;
  }
  ///@}
