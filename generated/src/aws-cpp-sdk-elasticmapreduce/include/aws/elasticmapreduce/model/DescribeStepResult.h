...
class DescribeStepResult {
 public:
  ...
  void SetStep(StepT&& value) {
    m_stepHasBeenSet = true;
    m_step = std::forward<StepT>(value);
  }
  template <typename StepT = Step>
  DescribeStepResult& WithStep(StepT&& value) {
    SetStep(std::forward<StepT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  DescribeStepResult& WithRequestId(RequestIdT&& value) {
