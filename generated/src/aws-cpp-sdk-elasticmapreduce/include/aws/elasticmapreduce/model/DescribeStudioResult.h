...
class DescribeStudioResult {
 public:
  ...
  void SetStudio(StudioT&& value) {
    m_studioHasBeenSet = true;
    m_studio = std::forward<StudioT>(value);
  }
  template <typename StudioT = Studio>
  DescribeStudioResult& WithStudio(StudioT&& value) {
    SetStudio(std::forward<StudioT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  DescribeStudioResult& WithRequestId(RequestIdT&& value) {
