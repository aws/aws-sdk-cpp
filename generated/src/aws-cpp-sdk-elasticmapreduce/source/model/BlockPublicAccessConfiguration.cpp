...
BlockPublicAccessConfiguration::BlockPublicAccessConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
BlockPublicAccessConfiguration& BlockPublicAccessConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("PermittedPublicSecurityGroupRuleRanges")) {
    Aws::Utils::Array<JsonView> permittedPublicSecurityGroupRuleRangesJsonList =
        jsonValue.GetArray("PermittedPublicSecurityGroupRuleRanges");
    for (unsigned permittedPublicSecurityGroupRuleRangesIndex = 0;
         permittedPublicSecurityGroupRuleRangesIndex < permittedPublicSecurityGroupRuleRangesJsonList.GetLength();
         ++permittedPublicSecurityGroupRuleRangesIndex) {
      m_permittedPublicSecurityGroupRuleRanges.push_back(
          permittedPublicSecurityGroupRuleRangesJsonList[permittedPublicSecurityGroupRuleRangesIndex].AsObject());
    }
...
JsonValue BlockPublicAccessConfiguration::Jsonize() const {
  ...
  if (m_permittedPublicSecurityGroupRuleRangesHasBeenSet) {
    ...
    for (unsigned permittedPublicSecurityGroupRuleRangesIndex = 0;
         ...
         ++permittedPublicSecurityGroupRuleRangesIndex) {
      permittedPublicSecurityGroupRuleRangesJsonList[permittedPublicSecurityGroupRuleRangesIndex].AsObject(
          m_permittedPublicSecurityGroupRuleRanges[permittedPublicSecurityGroupRuleRangesIndex].Jsonize());
    }
