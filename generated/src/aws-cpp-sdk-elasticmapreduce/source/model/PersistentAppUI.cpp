...
PersistentAppUI::PersistentAppUI(JsonView jsonValue) { *this = jsonValue; }
...
PersistentAppUI& PersistentAppUI::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("PersistentAppUITypeList")) {
    ...
    for (unsigned persistentAppUITypeListIndex = 0; persistentAppUITypeListIndex < persistentAppUITypeListJsonList.GetLength();
         ++persistentAppUITypeListIndex) {
      m_persistentAppUITypeList.push_back(PersistentAppUITypeMapper::GetPersistentAppUITypeForName(
          persistentAppUITypeListJsonList[persistentAppUITypeListIndex].AsString()));
    }
...
JsonValue PersistentAppUI::Jsonize() const {
  ...
  if (m_persistentAppUITypeListHasBeenSet) {
    ...
    for (unsigned persistentAppUITypeListIndex = 0; persistentAppUITypeListIndex < persistentAppUITypeListJsonList.GetLength();
         ++persistentAppUITypeListIndex) {
      persistentAppUITypeListJsonList[persistentAppUITypeListIndex].AsString(
          PersistentAppUITypeMapper::GetNameForPersistentAppUIType(m_persistentAppUITypeList[persistentAppUITypeListIndex]));
    }
