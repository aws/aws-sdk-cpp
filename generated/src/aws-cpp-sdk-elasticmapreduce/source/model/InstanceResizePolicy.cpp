...
InstanceResizePolicy::InstanceResizePolicy(JsonView jsonValue) { *this = jsonValue; }
...
InstanceResizePolicy& InstanceResizePolicy::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("InstancesToTerminate")) {
    ...
    for (unsigned instancesToTerminateIndex = 0; instancesToTerminateIndex < instancesToTerminateJsonList.GetLength();
         ++instancesToTerminateIndex) {
  ...
  if (jsonValue.ValueExists("InstancesToProtect")) {
    ...
    for (unsigned instancesToProtectIndex = 0; instancesToProtectIndex < instancesToProtectJsonList.GetLength();
         ++instancesToProtectIndex) {
