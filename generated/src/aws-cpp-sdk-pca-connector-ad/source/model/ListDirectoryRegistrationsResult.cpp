...
ListDirectoryRegistrationsResult::ListDirectoryRegistrationsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListDirectoryRegistrationsResult& ListDirectoryRegistrationsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("DirectoryRegistrations")) {
    ...
    for (unsigned directoryRegistrationsIndex = 0; directoryRegistrationsIndex < directoryRegistrationsJsonList.GetLength();
         ++directoryRegistrationsIndex) {
      m_directoryRegistrations.push_back(directoryRegistrationsJsonList[directoryRegistrationsIndex].AsObject());
    }
    m_directoryRegistrationsHasBeenSet = true;
  }
