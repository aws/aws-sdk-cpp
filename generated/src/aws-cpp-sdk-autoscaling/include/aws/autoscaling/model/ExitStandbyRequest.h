...
/**
 */
class ExitStandbyRequest : public AutoScalingRequest {
 public:
  AWS_AUTOSCALING_API ExitStandbyRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ExitStandby"; }
 ...
 public:
  ...
  template <typename InstanceIdsT = Aws::Vector<Aws::String>>
  ExitStandbyRequest& WithInstanceIds(InstanceIdsT&& value) {
    SetInstanceIds(std::forward<InstanceIdsT>(value));
    return *this;
  }
  template <typename InstanceIdsT = Aws::String>
  ExitStandbyRequest& AddInstanceIds(InstanceIdsT&& value) {
    m_instanceIdsHasBeenSet = true;
  ...
  template <typename AutoScalingGroupNameT = Aws::String>
  ExitStandbyRequest& WithAutoScalingGroupName(AutoScalingGroupNameT&& value) {
