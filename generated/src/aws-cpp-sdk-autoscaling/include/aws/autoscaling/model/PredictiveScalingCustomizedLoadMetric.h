...
class PredictiveScalingCustomizedLoadMetric {
 public:
  ...
  template <typename MetricDataQueriesT = Aws::Vector<MetricDataQuery>>
  PredictiveScalingCustomizedLoadMetric& WithMetricDataQueries(MetricDataQueriesT&& value) {
    SetMetricDataQueries(std::forward<MetricDataQueriesT>(value));
    return *this;
  }
  template <typename MetricDataQueriesT = MetricDataQuery>
  PredictiveScalingCustomizedLoadMetric& AddMetricDataQueries(MetricDataQueriesT&& value) {
    m_metricDataQueriesHasBeenSet = true;
