...
class AutoScalingInstanceDetails {
 public:
   ...
   * <code>Standby</code> | <code>ReplacingRootVolume</code> |
   * <code>ReplacingRootVolume:Wait</code> | <code>ReplacingRootVolume:Proceed</code>
   * | <code>RootVolumeReplaced</code> | <code>Warmed:Pending</code> |
  ...
  void SetLifecycleState(LifecycleStateT&& value) {
    ...
    m_lifecycleState = std::forward<LifecycleStateT>(value);
  }
  template <typename LifecycleStateT = Aws::String>
  AutoScalingInstanceDetails& WithLifecycleState(LifecycleStateT&& value) {
    SetLifecycleState(std::forward<LifecycleStateT>(value));
    return *this;
  }
  ///@}
  ...
  ///@{
  /**
   * <p> The ID of the Amazon Machine Image (AMI) associated with the instance. This
   * field shows the current AMI ID of the instance's root volume. It may differ from
   * the original AMI used when the instance was first launched. </p> <p> This field
   * appears for: </p> <ul> <li> <p>Instances with root volume replacements through
   * Instance Refresh</p> </li> <li> <p>Instances launched with AMI overrides </p>
   * </li> </ul> <p>This field won't appear for:</p> <ul> <li> <p>Existing instances
   * launched from Launch Templates without overrides</p> </li> <li> <p>Existing
   * instances that didn’t have their root volume replaced through Instance
   * Refresh</p> </li> </ul>
   */
  ...
  inline void SetProtectedFromScaleIn(bool value) {
    m_protectedFromScaleInHasBeenSet = true;
    m_protectedFromScaleIn = value;
  }
  inline AutoScalingInstanceDetails& WithProtectedFromScaleIn(bool value) {
    SetProtectedFromScaleIn(value);
    return *this;
  }
  ///@}
