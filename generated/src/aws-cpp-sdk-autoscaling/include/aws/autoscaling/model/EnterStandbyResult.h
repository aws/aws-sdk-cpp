...
class EnterStandbyResult {
 public:
  ...
  template <typename ActivitiesT = Aws::Vector<Activity>>
  EnterStandbyResult& WithActivities(ActivitiesT&& value) {
    SetActivities(std::forward<ActivitiesT>(value));
    return *this;
  }
  template <typename ActivitiesT = Activity>
  EnterStandbyResult& AddActivities(ActivitiesT&& value) {
    m_activitiesHasBeenSet = true;
  ...
  template <typename ResponseMetadataT = ResponseMetadata>
  EnterStandbyResult& WithResponseMetadata(ResponseMetadataT&& value) {
