...
class PredictiveScalingCustomizedCapacityMetric {
 public:
  ...
  template <typename MetricDataQueriesT = Aws::Vector<MetricDataQuery>>
  PredictiveScalingCustomizedCapacityMetric& WithMetricDataQueries(MetricDataQueriesT&& value) {
    SetMetricDataQueries(std::forward<MetricDataQueriesT>(value));
    return *this;
  }
  template <typename MetricDataQueriesT = MetricDataQuery>
  PredictiveScalingCustomizedCapacityMetric& AddMetricDataQueries(MetricDataQueriesT&& value) {
    m_metricDataQueriesHasBeenSet = true;
