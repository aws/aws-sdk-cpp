...
class DetachInstancesResult {
 public:
  ...
  template <typename ActivitiesT = Aws::Vector<Activity>>
  DetachInstancesResult& WithActivities(ActivitiesT&& value) {
    SetActivities(std::forward<ActivitiesT>(value));
    return *this;
  }
  template <typename ActivitiesT = Activity>
  DetachInstancesResult& AddActivities(ActivitiesT&& value) {
    m_activitiesHasBeenSet = true;
  ...
  template <typename ResponseMetadataT = ResponseMetadata>
  DetachInstancesResult& WithResponseMetadata(ResponseMetadataT&& value) {
