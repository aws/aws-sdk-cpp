  ...
  if (m_policyTypesHasBeenSet) {
    if (m_policyTypes.empty()) {
      ...
      for (auto& item : m_policyTypes) {
        ss << "PolicyTypes.member." << policyTypesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        policyTypesCount++;
      }
    }
  }
...
void DescribePoliciesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
