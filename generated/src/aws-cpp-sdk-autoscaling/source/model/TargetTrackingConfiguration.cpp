...
TargetTrackingConfiguration::TargetTrackingConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
...
TargetTrackingConfiguration& TargetTrackingConfiguration::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!disableScaleInNode.IsNull()) {
      m_disableScaleIn = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(disableScaleInNode.GetText()).c_str()).c_str());
...
void TargetTrackingConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
