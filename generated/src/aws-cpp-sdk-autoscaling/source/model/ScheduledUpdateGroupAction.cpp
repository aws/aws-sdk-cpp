...
ScheduledUpdateGroupAction::ScheduledUpdateGroupAction(const XmlNode& xmlNode) { *this = xmlNode; }
...
ScheduledUpdateGroupAction& ScheduledUpdateGroupAction::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!timeNode.IsNull()) {
      m_time = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(timeNode.GetText()).c_str()).c_str(),
...
void ScheduledUpdateGroupAction::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_timeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Time=" << StringUtils::URLEncode(m_time.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
