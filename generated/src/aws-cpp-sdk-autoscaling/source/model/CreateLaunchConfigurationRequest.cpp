  ...
  if (m_classicLinkVPCSecurityGroupsHasBeenSet) {
    if (m_classicLinkVPCSecurityGroups.empty()) {
      ...
      for (auto& item : m_classicLinkVPCSecurityGroups) {
        ss << "ClassicLinkVPCSecurityGroups.member." << classicLinkVPCSecurityGroupsCount << "=" << StringUtils::URLEncode(item.c_str())
           << "&";
...
void CreateLaunchConfigurationRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
