...
InstanceMetadataOptions::InstanceMetadataOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceMetadataOptions& InstanceMetadataOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!httpTokensNode.IsNull()) {
      m_httpTokens = InstanceMetadataHttpTokensStateMapper::GetInstanceMetadataHttpTokensStateForName(
...
void InstanceMetadataOptions::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_httpTokensHasBeenSet) {
    oStream << location << index << locationValue << ".HttpTokens="
            << StringUtils::URLEncode(InstanceMetadataHttpTokensStateMapper::GetNameForInstanceMetadataHttpTokensState(m_httpTokens))
            << "&";
  }
