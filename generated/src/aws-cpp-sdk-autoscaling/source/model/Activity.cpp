...
Activity::Activity(const XmlNode& xmlNode) { *this = xmlNode; }
...
Activity& Activity::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusCodeNode.IsNull()) {
      m_statusCode = ScalingActivityStatusCodeMapper::GetScalingActivityStatusCodeForName(
    ...
    if (!progressNode.IsNull()) {
      m_progress =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(progressNode.GetText()).c_str()).c_str());
...
void Activity::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusCodeHasBeenSet) {
    oStream << location << index << locationValue
            << ".StatusCode=" << StringUtils::URLEncode(ScalingActivityStatusCodeMapper::GetNameForScalingActivityStatusCode(m_statusCode))
            << "&";
  }
