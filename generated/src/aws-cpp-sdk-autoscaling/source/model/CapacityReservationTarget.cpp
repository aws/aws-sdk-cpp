...
void CapacityReservationTarget::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_capacityReservationIdsHasBeenSet) {
    ...
    for (auto& item : m_capacityReservationIds) {
      oStream << location << index << locationValue << ".CapacityReservationIds.member." << capacityReservationIdsIdx++ << "="
  ...
  if (m_capacityReservationResourceGroupArnsHasBeenSet) {
    ...
    for (auto& item : m_capacityReservationResourceGroupArns) {
      oStream << location << index << locationValue << ".CapacityReservationResourceGroupArns.member."
              << capacityReservationResourceGroupArnsIdx++ << "=" << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
}
...
void CapacityReservationTarget::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_capacityReservationResourceGroupArnsHasBeenSet) {
    unsigned capacityReservationResourceGroupArnsIdx = 1;
    for (auto& item : m_capacityReservationResourceGroupArns) {
      oStream << location << ".CapacityReservationResourceGroupArns.member." << capacityReservationResourceGroupArnsIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
}
