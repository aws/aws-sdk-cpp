...
InstanceReusePolicy::InstanceReusePolicy(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceReusePolicy& InstanceReusePolicy::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!reuseOnScaleInNode.IsNull()) {
      m_reuseOnScaleIn = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(reuseOnScaleInNode.GetText()).c_str()).c_str());
