...
RollbackDetails::RollbackDetails(const XmlNode& xmlNode) { *this = xmlNode; }
...
RollbackDetails& RollbackDetails::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!rollbackStartTimeNode.IsNull()) {
      m_rollbackStartTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(rollbackStartTimeNode.GetText()).c_str()).c_str(),
    ...
    if (!percentageCompleteOnRollbackNode.IsNull()) {
      m_percentageCompleteOnRollback = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(percentageCompleteOnRollbackNode.GetText()).c_str()).c_str());
    ...
    if (!instancesToUpdateOnRollbackNode.IsNull()) {
      m_instancesToUpdateOnRollback = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(instancesToUpdateOnRollbackNode.GetText()).c_str()).c_str());
...
void RollbackDetails::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_rollbackStartTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".RollbackStartTime=" << StringUtils::URLEncode(m_rollbackStartTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
