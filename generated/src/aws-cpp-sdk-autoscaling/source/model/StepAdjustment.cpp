...
StepAdjustment::StepAdjustment(const XmlNode& xmlNode) { *this = xmlNode; }
...
StepAdjustment& StepAdjustment::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!metricIntervalLowerBoundNode.IsNull()) {
      m_metricIntervalLowerBound = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(metricIntervalLowerBoundNode.GetText()).c_str()).c_str());
    ...
    if (!metricIntervalUpperBoundNode.IsNull()) {
      m_metricIntervalUpperBound = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(metricIntervalUpperBoundNode.GetText()).c_str()).c_str());
