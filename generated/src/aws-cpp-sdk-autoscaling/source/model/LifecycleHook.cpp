...
LifecycleHook::LifecycleHook(const XmlNode& xmlNode) { *this = xmlNode; }
...
LifecycleHook& LifecycleHook::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!globalTimeoutNode.IsNull()) {
      m_globalTimeout = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(globalTimeoutNode.GetText()).c_str()).c_str());
