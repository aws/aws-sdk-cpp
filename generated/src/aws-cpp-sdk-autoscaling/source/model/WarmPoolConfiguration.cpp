...
WarmPoolConfiguration::WarmPoolConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
...
WarmPoolConfiguration& WarmPoolConfiguration::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!maxGroupPreparedCapacityNode.IsNull()) {
      m_maxGroupPreparedCapacity = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maxGroupPreparedCapacityNode.GetText()).c_str()).c_str());
    ...
    if (!poolStateNode.IsNull()) {
      m_poolState = WarmPoolStateMapper::GetWarmPoolStateForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(poolStateNode.GetText()).c_str()));
      m_poolStateHasBeenSet = true;
    }
    XmlNode statusNode = resultNode.FirstChild("Status");
    if (!statusNode.IsNull()) {
      m_status = WarmPoolStatusMapper::GetWarmPoolStatusForName(
...
void WarmPoolConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_poolStateHasBeenSet) {
    oStream << location << index << locationValue
            << ".PoolState=" << StringUtils::URLEncode(WarmPoolStateMapper::GetNameForWarmPoolState(m_poolState)) << "&";
  }

  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(WarmPoolStatusMapper::GetNameForWarmPoolStatus(m_status)) << "&";
  }
