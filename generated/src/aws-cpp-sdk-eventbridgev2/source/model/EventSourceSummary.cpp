...
EventSourceSummary::EventSourceSummary(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

EventSourceSummary& EventSourceSummary::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "EventSourceArn") {
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_type = EventSourceTypeMapper::GetEventSourceTypeForName(
              ...
              else if (initialKeyStr == "EventSourceAccountId") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_eventSourceAccountId = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("EventSourceSummary", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void EventSourceSummary::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_eventSourceAccountIdHasBeenSet) {
    mapSize++;
  }
  ...
  if (m_typeHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(EventSourceTypeMapper::GetNameForEventSourceType(m_type).c_str()));
  }

  if (m_stateHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("State"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(EventSourceStateMapper::GetNameForEventSourceState(m_state).c_str()));
  }
  ...
  if (m_eventSourceAccountIdHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("EventSourceAccountId"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_eventSourceAccountId.c_str()));
  }
}
