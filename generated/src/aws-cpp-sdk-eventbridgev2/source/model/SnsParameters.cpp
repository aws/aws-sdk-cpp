...
SnsParameters::SnsParameters(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

SnsParameters& SnsParameters::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "Subject") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_subject = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
              ...
              else if (initialKeyStr == "MessageStructure") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_messageStructure = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
                    ...
                    if (mapSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < mapSize_0.value(); j_0++) {
                        ...
                        if (key_1.has_value()) {
                          ...
                          m_messageAttributes[keyStr_1] = SnsMessageAttributeValue(decoder);
                        }
                      }
                    }
                  } else  // IndefMapStart
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("SnsParameters", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void SnsParameters::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_subjectHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("Subject"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_subject.c_str()));
  }

  if (m_messageStructureHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("MessageStructure"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_messageStructure.c_str()));
  }
