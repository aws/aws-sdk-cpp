...
CreateSubscriberResult::CreateSubscriberResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result) { *this = result; }

CreateSubscriberResult& CreateSubscriberResult::operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "StartingPosition") {
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_startingPosition = StartingPositionMapper::GetStartingPositionForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
                m_startingPositionHasBeenSet = true;
              }

              else if (initialKeyStr == "PointInTimeConfiguration") {
                m_pointInTimeConfiguration = PointInTimeConfiguration(decoder);
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("CreateSubscriberResult", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
