...
PutRawEventsRequestEntry::PutRawEventsRequestEntry(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

PutRawEventsRequestEntry& PutRawEventsRequestEntry::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Data") {
                ...
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Bytes) {
                    ...
                    m_data = Aws::Utils::ByteBuffer(ss.str());
                    ss.clear();
                  }
                }
                ...
                m_systemMetadata = PutRawEventsSystemMetadata(decoder);
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("PutRawEventsRequestEntry", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void PutRawEventsRequestEntry::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_dataHasBeenSet) {
    mapSize++;
  ...
  if (m_dataHasBeenSet) {
    ...
    encoder.WriteBytes(Aws::Crt::ByteCursorFromCString(reinterpret_cast<const char*>(m_data.GetUnderlyingData())));
  }
