...
class InputCaptions {
 public:
  ...
  void SetCaptionSources(CaptionSourcesT&& value) {
    ...
    m_captionSources = std::forward<CaptionSourcesT>(value);
  }
  template <typename CaptionSourcesT = Aws::Vector<CaptionSource>>
  InputCaptions& WithCaptionSources(CaptionSourcesT&& value) {
    SetCaptionSources(std::forward<CaptionSourcesT>(value));
    return *this;
  }
  template <typename CaptionSourcesT = CaptionSource>
  InputCaptions& AddCaptionSources(CaptionSourcesT&& value) {
    m_captionSourcesHasBeenSet = true;
    m_captionSources.emplace_back(std::forward<CaptionSourcesT>(value));
    return *this;
  }
  ///@}
 private:
