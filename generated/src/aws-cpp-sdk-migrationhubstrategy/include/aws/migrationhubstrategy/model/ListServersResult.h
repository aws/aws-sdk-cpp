...
class ListServersResult {
 public:
  ...
  AWS_MIGRATIONHUBSTRATEGYRECOMMENDATIONS_API ListServersResult& operator=(
  ...
  void SetServerInfos(ServerInfosT&& value) {
    m_serverInfosHasBeenSet = true;
    m_serverInfos = std::forward<ServerInfosT>(value);
  }
  template <typename ServerInfosT = Aws::Vector<ServerDetail>>
  ListServersResult& WithServerInfos(ServerInfosT&& value) {
    SetServerInfos(std::forward<ServerInfosT>(value));
    return *this;
  }
  template <typename ServerInfosT = ServerDetail>
  ListServersResult& AddServerInfos(ServerInfosT&& value) {
    m_serverInfosHasBeenSet = true;
    m_serverInfos.emplace_back(std::forward<ServerInfosT>(value));
    return *this;
  }
  ///@}
