...
Heterogeneous::Heterogeneous(JsonView jsonValue) { *this = jsonValue; }
...
Heterogeneous& Heterogeneous::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("targetDatabaseEngine")) {
    ...
    for (unsigned targetDatabaseEngineIndex = 0; targetDatabaseEngineIndex < targetDatabaseEngineJsonList.GetLength();
         ++targetDatabaseEngineIndex) {
      m_targetDatabaseEngine.push_back(HeterogeneousTargetDatabaseEngineMapper::GetHeterogeneousTargetDatabaseEngineForName(
          targetDatabaseEngineJsonList[targetDatabaseEngineIndex].AsString()));
    }
...
JsonValue Heterogeneous::Jsonize() const {
  ...
  if (m_targetDatabaseEngineHasBeenSet) {
    ...
    for (unsigned targetDatabaseEngineIndex = 0; targetDatabaseEngineIndex < targetDatabaseEngineJsonList.GetLength();
         ++targetDatabaseEngineIndex) {
      targetDatabaseEngineJsonList[targetDatabaseEngineIndex].AsString(
          HeterogeneousTargetDatabaseEngineMapper::GetNameForHeterogeneousTargetDatabaseEngine(
              m_targetDatabaseEngine[targetDatabaseEngineIndex]));
    }
