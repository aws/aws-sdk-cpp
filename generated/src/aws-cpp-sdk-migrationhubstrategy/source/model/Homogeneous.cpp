...
Homogeneous::Homogeneous(JsonView jsonValue) { *this = jsonValue; }
...
Homogeneous& Homogeneous::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("targetDatabaseEngine")) {
    ...
    for (unsigned targetDatabaseEngineIndex = 0; targetDatabaseEngineIndex < targetDatabaseEngineJsonList.GetLength();
      ...
      m_targetDatabaseEngine.push_back(HomogeneousTargetDatabaseEngineMapper::GetHomogeneousTargetDatabaseEngineForName(
...
JsonValue Homogeneous::Jsonize() const {
  ...
  if (m_targetDatabaseEngineHasBeenSet) {
    ...
    for (unsigned targetDatabaseEngineIndex = 0; targetDatabaseEngineIndex < targetDatabaseEngineJsonList.GetLength();
         ++targetDatabaseEngineIndex) {
      targetDatabaseEngineJsonList[targetDatabaseEngineIndex].AsString(
          HomogeneousTargetDatabaseEngineMapper::GetNameForHomogeneousTargetDatabaseEngine(
