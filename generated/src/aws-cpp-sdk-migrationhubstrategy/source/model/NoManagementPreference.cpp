...
NoManagementPreference::NoManagementPreference(JsonView jsonValue) { *this = jsonValue; }
...
NoManagementPreference& NoManagementPreference::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("targetDestination")) {
    ...
    for (unsigned targetDestinationIndex = 0; targetDestinationIndex < targetDestinationJsonList.GetLength(); ++targetDestinationIndex) {
      m_targetDestination.push_back(NoPreferenceTargetDestinationMapper::GetNoPreferenceTargetDestinationForName(
...
JsonValue NoManagementPreference::Jsonize() const {
  ...
  if (m_targetDestinationHasBeenSet) {
    ...
    for (unsigned targetDestinationIndex = 0; targetDestinationIndex < targetDestinationJsonList.GetLength(); ++targetDestinationIndex) {
      targetDestinationJsonList[targetDestinationIndex].AsString(
          NoPreferenceTargetDestinationMapper::GetNameForNoPreferenceTargetDestination(m_targetDestination[targetDestinationIndex]));
    }
