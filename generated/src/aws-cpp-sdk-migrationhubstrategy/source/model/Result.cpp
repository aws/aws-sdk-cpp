...
Result& Result::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("antipatternReportResultList")) {
    ...
    for (unsigned antipatternReportResultListIndex = 0; antipatternReportResultListIndex < antipatternReportResultListJsonList.GetLength();
         ++antipatternReportResultListIndex) {
...
JsonValue Result::Jsonize() const {
  ...
  if (m_antipatternReportResultListHasBeenSet) {
    ...
    for (unsigned antipatternReportResultListIndex = 0; antipatternReportResultListIndex < antipatternReportResultListJsonList.GetLength();
         ++antipatternReportResultListIndex) {
      antipatternReportResultListJsonList[antipatternReportResultListIndex].AsObject(
          m_antipatternReportResultList[antipatternReportResultListIndex].Jsonize());
    }
