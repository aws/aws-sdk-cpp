...
/**
 */
class ListSecurityGroupsRequest : public WickrRequest {
 public:
  AWS_WICKR_API ListSecurityGroupsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListSecurityGroups"; }
  ...
  template <typename NetworkIdT = Aws::String>
  ListSecurityGroupsRequest& WithNetworkId(NetworkIdT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListSecurityGroupsRequest& WithNextToken(NextTokenT&& value) {
  ...
  ///@{
  /**
   * <p>The maximum number of security groups to return in a single page. Valid range
   * is 1-100. Default is 10.</p>
   */
  ...
  inline ListSecurityGroupsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  ///@{
  /**
   * <p>The field to sort security groups by. Accepted values include 'id' and
   * 'name'.</p>
   */
  ...
  template <typename SortFieldsT = Aws::String>
  ListSecurityGroupsRequest& WithSortFields(SortFieldsT&& value) {
  ...
  inline ListSecurityGroupsRequest& WithSortDirection(SortDirection value) {
