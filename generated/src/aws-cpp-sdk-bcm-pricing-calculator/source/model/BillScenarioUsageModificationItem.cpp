...
BillScenarioUsageModificationItem::BillScenarioUsageModificationItem(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  *this = decoder;
}

BillScenarioUsageModificationItem& BillScenarioUsageModificationItem::operator=(
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "quantities") {
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_quantities.push_back(UsageQuantity(decoder));
                      }
                    }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("BillScenarioUsageModificationItem", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void BillScenarioUsageModificationItem::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_quantitiesHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("quantities"));
    encoder.WriteArrayStart(m_quantities.size());
    for (const auto& item_0 : m_quantities) {
