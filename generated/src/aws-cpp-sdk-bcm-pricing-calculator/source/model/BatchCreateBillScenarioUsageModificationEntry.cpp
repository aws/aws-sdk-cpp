...
BatchCreateBillScenarioUsageModificationEntry& BatchCreateBillScenarioUsageModificationEntry::operator=(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_amounts.push_back(UsageAmount(decoder));
                      }
                    }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("BatchCreateBillScenarioUsageModificationEntry", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void BatchCreateBillScenarioUsageModificationEntry::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_amountsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("amounts"));
    encoder.WriteArrayStart(m_amounts.size());
    for (const auto& item_0 : m_amounts) {
