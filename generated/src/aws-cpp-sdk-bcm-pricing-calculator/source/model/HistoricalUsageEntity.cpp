...
HistoricalUsageEntity::HistoricalUsageEntity(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

HistoricalUsageEntity& HistoricalUsageEntity::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "filterExpression") {
                m_filterExpression = Expression(decoder);
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("HistoricalUsageEntity", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void HistoricalUsageEntity::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_filterExpressionHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("filterExpression"));
    m_filterExpression.CborEncode(encoder);
  }
}
