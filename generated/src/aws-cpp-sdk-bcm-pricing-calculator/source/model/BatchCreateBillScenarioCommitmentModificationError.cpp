...
BatchCreateBillScenarioCommitmentModificationError::BatchCreateBillScenarioCommitmentModificationError(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  *this = decoder;
}

BatchCreateBillScenarioCommitmentModificationError& BatchCreateBillScenarioCommitmentModificationError::operator=(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "key") {
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_errorCode = BatchCreateBillScenarioCommitmentModificationErrorCodeMapper::
                      GetBatchCreateBillScenarioCommitmentModificationErrorCodeForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("BatchCreateBillScenarioCommitmentModificationError", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void BatchCreateBillScenarioCommitmentModificationError::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_keyHasBeenSet) {
    mapSize++;
  ...
  if (m_errorCodeHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(
        BatchCreateBillScenarioCommitmentModificationErrorCodeMapper::GetNameForBatchCreateBillScenarioCommitmentModificationErrorCode(
            m_errorCode)
            .c_str()));
  }
}
