...
ExpressionFilter::ExpressionFilter(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

ExpressionFilter& ExpressionFilter::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "matchOptions") {
                      ...
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        ...
                        if (peekType_1.has_value()) {
                          if (peekType_1.value() == Aws::Crt::Cbor::CborType::Text) {
                            auto val = decoder->PopNextTextVal();
                            if (val.has_value()) {
                              m_matchOptions.push_back(Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                            }
                          } else {
                            decoder->ConsumeNextSingleElement();
                            ...
                            m_matchOptions.push_back(ss_1.str());
                            ss_1.clear();
                          }
                        }
                      }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("ExpressionFilter", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void ExpressionFilter::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_keyHasBeenSet) {
    mapSize++;
  ...
  if (m_matchOptionsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("matchOptions"));
    encoder.WriteArrayStart(m_matchOptions.size());
    for (const auto& item_0 : m_matchOptions) {
