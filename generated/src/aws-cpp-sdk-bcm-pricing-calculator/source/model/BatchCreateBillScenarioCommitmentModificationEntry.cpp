...
BatchCreateBillScenarioCommitmentModificationEntry::BatchCreateBillScenarioCommitmentModificationEntry(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  *this = decoder;
}

BatchCreateBillScenarioCommitmentModificationEntry& BatchCreateBillScenarioCommitmentModificationEntry::operator=(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "commitmentAction") {
                m_commitmentAction = BillScenarioCommitmentModificationAction(decoder);
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("BatchCreateBillScenarioCommitmentModificationEntry", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void BatchCreateBillScenarioCommitmentModificationEntry::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_keyHasBeenSet) {
    mapSize++;
  ...
  if (m_commitmentActionHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("commitmentAction"));
    m_commitmentAction.CborEncode(encoder);
  }
}
