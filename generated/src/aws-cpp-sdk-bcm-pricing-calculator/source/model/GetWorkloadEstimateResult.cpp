...
GetWorkloadEstimateResult::GetWorkloadEstimateResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result) {
  *this = result;
}

GetWorkloadEstimateResult& GetWorkloadEstimateResult::operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "id") {
              ...
              else if (initialKeyStr == "rateType") {
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_rateType = WorkloadEstimateRateTypeMapper::GetWorkloadEstimateRateTypeForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
                m_rateTypeHasBeenSet = true;
              }

              else if (initialKeyStr == "rateTimestamp") {
                auto tag = decoder->PopNextTagVal();
                if (tag.has_value() &&
                    tag.value() == 1)  // 1 represents Epoch-based date/time. See https://www.rfc-editor.org/rfc/rfc8949.html#tags
                {
                  ...
                  if (dateType.has_value()) {
                    if (dateType.value() == Aws::Crt::Cbor::CborType::Float) {
                      auto val = decoder->PopNextFloatVal();
                      if (val.has_value()) {
                        m_rateTimestamp = Aws::Utils::DateTime(val.value());
                      }
                    } else {
                      auto val = decoder->PopNextUnsignedIntVal();
                      if (val.has_value()) {
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_status = WorkloadEstimateStatusMapper::GetWorkloadEstimateStatusForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
                m_statusHasBeenSet = true;
              }

              else if (initialKeyStr == "totalCost") {
                auto val = decoder->PopNextFloatVal();
                if (val.has_value()) {
                  m_totalCost = val.value();
                }
                m_totalCostHasBeenSet = true;
              }

              else if (initialKeyStr == "costCurrency") {
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_costCurrency = CurrencyCodeMapper::GetCurrencyCodeForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("GetWorkloadEstimateResult", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
