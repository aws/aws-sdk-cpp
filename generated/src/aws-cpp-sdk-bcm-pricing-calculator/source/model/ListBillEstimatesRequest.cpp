...
Aws::String ListBillEstimatesRequest::SerializePayload() const {
  ...
  if (m_createdAtFilterHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("createdAtFilter"));
    m_createdAtFilter.CborEncode(encoder);
  }

  if (m_expiresAtFilterHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("expiresAtFilter"));
    m_expiresAtFilter.CborEncode(encoder);
  }
