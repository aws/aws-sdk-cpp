...
BillEstimateSummary::BillEstimateSummary(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

BillEstimateSummary& BillEstimateSummary::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "id") {
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_status = BillEstimateStatusMapper::GetBillEstimateStatusForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("BillEstimateSummary", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void BillEstimateSummary::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_idHasBeenSet) {
    mapSize++;
  ...
  if (m_statusHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(BillEstimateStatusMapper::GetNameForBillEstimateStatus(m_status).c_str()));
  }
