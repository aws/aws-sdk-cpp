...
class WorkloadEstimateUsageQuantity {
 public:
  ...
  AWS_BCMPRICINGCALCULATOR_API WorkloadEstimateUsageQuantity(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_BCMPRICINGCALCULATOR_API WorkloadEstimateUsageQuantity& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  template <typename UnitT = Aws::String>
  WorkloadEstimateUsageQuantity& WithUnit(UnitT&& value) {
  ...
  inline WorkloadEstimateUsageQuantity& WithAmount(double value) {
    SetAmount(value);
    return *this;
  }
  ///@}
