...
class BillInterval {
 public:
  ...
  AWS_BCMPRICINGCALCULATOR_API BillInterval(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_BCMPRICINGCALCULATOR_API BillInterval& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  template <typename StartT = Aws::Utils::DateTime>
  BillInterval& WithStart(StartT&& value) {
  ...
  template <typename EndT = Aws::Utils::DateTime>
  BillInterval& WithEnd(EndT&& value) {
