...
class GetSourceRepositoryCloneUrlsResult {
 public:
  ...
  AWS_CODECATALYST_API GetSourceRepositoryCloneUrlsResult& operator=(
  ...
  void SetHttps(HttpsT&& value) {
    m_httpsHasBeenSet = true;
    m_https = std::forward<HttpsT>(value);
  }
  template <typename HttpsT = Aws::String>
  GetSourceRepositoryCloneUrlsResult& WithHttps(HttpsT&& value) {
    SetHttps(std::forward<HttpsT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetSourceRepositoryCloneUrlsResult& WithRequestId(RequestIdT&& value) {
