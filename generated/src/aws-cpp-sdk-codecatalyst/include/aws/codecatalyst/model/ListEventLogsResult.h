...
class ListEventLogsResult {
 public:
  ...
  template <typename NextTokenT = Aws::String>
  ListEventLogsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename ItemsT = Aws::Vector<EventLogEntry>>
  ListEventLogsResult& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = EventLogEntry>
  ListEventLogsResult& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListEventLogsResult& WithRequestId(RequestIdT&& value) {
