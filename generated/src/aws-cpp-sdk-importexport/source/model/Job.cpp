...
Job::Job(const XmlNode& xmlNode) { *this = xmlNode; }
...
Job& Job::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!isCanceledNode.IsNull()) {
      m_isCanceled =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isCanceledNode.GetText()).c_str()).c_str());
