  ...
  if (m_jobIdsHasBeenSet) {
    if (m_jobIds.empty()) {
      ...
      for (auto& item : m_jobIds) {
        ss << "jobIds.member." << jobIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        jobIdsCount++;
      }
    }
  }
...
void GetShippingLabelRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
