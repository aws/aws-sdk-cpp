...
class OS {
 public:
  ...
  void SetFullString(FullStringT&& value) {
    m_fullStringHasBeenSet = true;
    m_fullString = std::forward<FullStringT>(value);
  }
  template <typename FullStringT = Aws::String>
  OS& WithFullString(FullStringT&& value) {
    SetFullString(std::forward<FullStringT>(value));
    return *this;
  }
  ///@}
 private:
