...
namespace drs {
template <>
AWS_DRS_API ConflictException DrsError::GetModeledError() {
  assert(this->GetErrorType() == DrsErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_DRS_API ServiceQuotaExceededException DrsError::GetModeledError() {
  assert(this->GetErrorType() == DrsErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_DRS_API ThrottlingException DrsError::GetModeledError() {
  assert(this->GetErrorType() == DrsErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_DRS_API ResourceNotFoundException DrsError::GetModeledError() {
  assert(this->GetErrorType() == DrsErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_DRS_API InternalServerException DrsError::GetModeledError() {
  assert(this->GetErrorType() == DrsErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_DRS_API ValidationException DrsError::GetModeledError() {
  assert(this->GetErrorType() == DrsErrors::VALIDATION);
  return ValidationException(this->GetJsonPayload().View());
}

template <>
AWS_DRS_API UninitializedAccountException DrsError::GetModeledError() {
  assert(this->GetErrorType() == DrsErrors::UNINITIALIZED_ACCOUNT);
  return UninitializedAccountException(this->GetJsonPayload().View());
}

template <>
AWS_DRS_API AccessDeniedException DrsError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(DrsErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
