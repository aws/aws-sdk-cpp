...
class AmiProductTitleFilter {
 public:
  ...
  template <typename ValueListT = Aws::Vector<Aws::String>>
  AmiProductTitleFilter& WithValueList(ValueListT&& value) {
    SetValueList(std::forward<ValueListT>(value));
    return *this;
  }
  template <typename ValueListT = Aws::String>
  AmiProductTitleFilter& AddValueList(ValueListT&& value) {
    m_valueListHasBeenSet = true;
  ...
  template <typename WildCardValueT = Aws::String>
  AmiProductTitleFilter& WithWildCardValue(WildCardValueT&& value) {
