...
class SaaSProductTitleFilter {
 public:
  ...
  template <typename ValueListT = Aws::Vector<Aws::String>>
  SaaSProductTitleFilter& WithValueList(ValueListT&& value) {
    SetValueList(std::forward<ValueListT>(value));
    return *this;
  }
  template <typename ValueListT = Aws::String>
  SaaSProductTitleFilter& AddValueList(ValueListT&& value) {
    m_valueListHasBeenSet = true;
  ...
  template <typename WildCardValueT = Aws::String>
  SaaSProductTitleFilter& WithWildCardValue(WildCardValueT&& value) {
