...
class DataProductTitleFilter {
 public:
  ...
  template <typename ValueListT = Aws::Vector<Aws::String>>
  DataProductTitleFilter& WithValueList(ValueListT&& value) {
    SetValueList(std::forward<ValueListT>(value));
    return *this;
  }
  template <typename ValueListT = Aws::String>
  DataProductTitleFilter& AddValueList(ValueListT&& value) {
    m_valueListHasBeenSet = true;
  ...
  template <typename WildCardValueT = Aws::String>
  DataProductTitleFilter& WithWildCardValue(WildCardValueT&& value) {
