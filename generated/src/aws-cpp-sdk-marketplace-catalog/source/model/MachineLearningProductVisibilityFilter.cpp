...
MachineLearningProductVisibilityFilter::MachineLearningProductVisibilityFilter(JsonView jsonValue) { *this = jsonValue; }
...
MachineLearningProductVisibilityFilter& MachineLearningProductVisibilityFilter::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("ValueList")) {
    ...
    for (unsigned valueListIndex = 0; valueListIndex < valueListJsonList.GetLength(); ++valueListIndex) {
      m_valueList.push_back(MachineLearningProductVisibilityStringMapper::GetMachineLearningProductVisibilityStringForName(
...
JsonValue MachineLearningProductVisibilityFilter::Jsonize() const {
  ...
  if (m_valueListHasBeenSet) {
    ...
    for (unsigned valueListIndex = 0; valueListIndex < valueListJsonList.GetLength(); ++valueListIndex) {
      valueListJsonList[valueListIndex].AsString(
          MachineLearningProductVisibilityStringMapper::GetNameForMachineLearningProductVisibilityString(m_valueList[valueListIndex]));
    }
