...
AmiProductVisibilityFilter::AmiProductVisibilityFilter() : 
...
AmiProductVisibilityFilter& AmiProductVisibilityFilter::operator =(JsonView jsonValue)
{
  if(jsonValue.ValueExists("ValueList"))
  {
    ...
    {
      m_valueList.push_back(AmiProductVisibilityStringMapper::GetAmiProductVisibilityStringForName(valueListJsonList[valueListIndex].AsString()));
    }
...
JsonValue AmiProductVisibilityFilter::Jsonize() const
{
  JsonValue payload;
  ...
  if(m_valueListHasBeenSet)
  {
   ...
   for(unsigned valueListIndex = 0; valueListIndex < valueListJsonList.GetLength(); ++valueListIndex)
   {
     valueListJsonList[valueListIndex].AsString(AmiProductVisibilityStringMapper::GetNameForAmiProductVisibilityString(m_valueList[valueListIndex]));
   }
