...
SaaSProductVisibilityFilter::SaaSProductVisibilityFilter() : 
...
SaaSProductVisibilityFilter& SaaSProductVisibilityFilter::operator =(JsonView jsonValue)
{
  if(jsonValue.ValueExists("ValueList"))
  {
    ...
    {
      m_valueList.push_back(SaaSProductVisibilityStringMapper::GetSaaSProductVisibilityStringForName(valueListJsonList[valueListIndex].AsString()));
    }
...
JsonValue SaaSProductVisibilityFilter::Jsonize() const
{
  ...
  if(m_valueListHasBeenSet)
  {
   ...
   for(unsigned valueListIndex = 0; valueListIndex < valueListJsonList.GetLength(); ++valueListIndex)
   {
     valueListJsonList[valueListIndex].AsString(SaaSProductVisibilityStringMapper::GetNameForSaaSProductVisibilityString(m_valueList[valueListIndex]));
   }
