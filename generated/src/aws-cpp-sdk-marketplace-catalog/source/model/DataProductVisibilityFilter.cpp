...
DataProductVisibilityFilter::DataProductVisibilityFilter() : 
...
DataProductVisibilityFilter& DataProductVisibilityFilter::operator =(JsonView jsonValue)
{
  if(jsonValue.ValueExists("ValueList"))
  {
    ...
    {
      m_valueList.push_back(DataProductVisibilityStringMapper::GetDataProductVisibilityStringForName(valueListJsonList[valueListIndex].AsString()));
    }
...
JsonValue DataProductVisibilityFilter::Jsonize() const
{
  JsonValue payload;
  ...
  if(m_valueListHasBeenSet)
  {
   ...
   for(unsigned valueListIndex = 0; valueListIndex < valueListJsonList.GetLength(); ++valueListIndex)
   {
     valueListJsonList[valueListIndex].AsString(DataProductVisibilityStringMapper::GetNameForDataProductVisibilityString(m_valueList[valueListIndex]));
   }
