...
ContainerProductVisibilityFilter::ContainerProductVisibilityFilter() : 
...
ContainerProductVisibilityFilter& ContainerProductVisibilityFilter::operator =(JsonView jsonValue)
{
  if(jsonValue.ValueExists("ValueList"))
  {
    ...
    {
      m_valueList.push_back(ContainerProductVisibilityStringMapper::GetContainerProductVisibilityStringForName(valueListJsonList[valueListIndex].AsString()));
    }
...
JsonValue ContainerProductVisibilityFilter::Jsonize() const
{
  JsonValue payload;
  ...
  if(m_valueListHasBeenSet)
  {
   ...
   for(unsigned valueListIndex = 0; valueListIndex < valueListJsonList.GetLength(); ++valueListIndex)
   {
     valueListJsonList[valueListIndex].AsString(ContainerProductVisibilityStringMapper::GetNameForContainerProductVisibilityString(m_valueList[valueListIndex]));
   }
