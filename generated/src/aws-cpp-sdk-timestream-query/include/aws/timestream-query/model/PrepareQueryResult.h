...
class PrepareQueryResult {
 public:
  ...
  template <typename QueryStringT = Aws::String>
  PrepareQueryResult& WithQueryString(QueryStringT&& value) {
  ...
  template <typename ColumnsT = Aws::Vector<SelectColumn>>
  PrepareQueryResult& WithColumns(ColumnsT&& value) {
    SetColumns(std::forward<ColumnsT>(value));
    return *this;
  }
  template <typename ColumnsT = SelectColumn>
  PrepareQueryResult& AddColumns(ColumnsT&& value) {
    m_columnsHasBeenSet = true;
  ...
  template <typename ParametersT = Aws::Vector<ParameterMapping>>
  PrepareQueryResult& WithParameters(ParametersT&& value) {
    SetParameters(std::forward<ParametersT>(value));
    return *this;
  }
  template <typename ParametersT = ParameterMapping>
  PrepareQueryResult& AddParameters(ParametersT&& value) {
    m_parametersHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  PrepareQueryResult& WithRequestId(RequestIdT&& value) {
