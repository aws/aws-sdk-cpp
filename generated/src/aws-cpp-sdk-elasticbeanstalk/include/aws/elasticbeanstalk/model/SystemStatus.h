...
class SystemStatus {
 public:
  ...
  void SetCPUUtilization(CPUUtilizationT&& value) {
    ...
    m_cPUUtilization = std::forward<CPUUtilizationT>(value);
  }
  template <typename CPUUtilizationT = CPUUtilization>
  SystemStatus& WithCPUUtilization(CPUUtilizationT&& value) {
    SetCPUUtilization(std::forward<CPUUtilizationT>(value));
    return *this;
  }
  ///@}
  ...
  void SetLoadAverage(LoadAverageT&& value) {
    ...
    m_loadAverage = std::forward<LoadAverageT>(value);
  }
  template <typename LoadAverageT = Aws::Vector<double>>
  SystemStatus& WithLoadAverage(LoadAverageT&& value) {
    SetLoadAverage(std::forward<LoadAverageT>(value));
    return *this;
  }
  inline SystemStatus& AddLoadAverage(double value) {
    m_loadAverageHasBeenSet = true;
    m_loadAverage.push_back(value);
    return *this;
  }
  ///@}
 private:
