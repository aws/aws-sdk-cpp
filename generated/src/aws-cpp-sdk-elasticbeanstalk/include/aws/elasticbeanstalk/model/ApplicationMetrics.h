...
/**
 * <p>Application request metrics for an Elastic Beanstalk
...
class ApplicationMetrics {
 public:
  ...
  inline ApplicationMetrics& WithDuration(int value) {
    SetDuration(value);
    return *this;
  }
  ///@}
  ...
  inline ApplicationMetrics& WithRequestCount(int value) {
  ...
  void SetLatency(LatencyT&& value) {
    m_latencyHasBeenSet = true;
    m_latency = std::forward<LatencyT>(value);
  }
  template <typename LatencyT = Latency>
  ApplicationMetrics& WithLatency(LatencyT&& value) {
    SetLatency(std::forward<LatencyT>(value));
    return *this;
  }
  ///@}
 private:
