...
ManagedAction::ManagedAction(const XmlNode& xmlNode) { *this = xmlNode; }
...
ManagedAction& ManagedAction::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = ActionStatusMapper::GetActionStatusForName(
    ...
    if (!windowStartTimeNode.IsNull()) {
      m_windowStartTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(windowStartTimeNode.GetText()).c_str()).c_str(),
...
void ManagedAction::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(ActionStatusMapper::GetNameForActionStatus(m_status)) << "&";
  }

  if (m_windowStartTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".WindowStartTime=" << StringUtils::URLEncode(m_windowStartTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
}
