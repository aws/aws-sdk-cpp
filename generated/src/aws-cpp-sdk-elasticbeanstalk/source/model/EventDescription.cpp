...
EventDescription::EventDescription(const XmlNode& xmlNode) { *this = xmlNode; }
...
EventDescription& EventDescription::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!eventDateNode.IsNull()) {
      m_eventDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(eventDateNode.GetText()).c_str()).c_str(),
    ...
    if (!severityNode.IsNull()) {
      m_severity = EventSeverityMapper::GetEventSeverityForName(
...
void EventDescription::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_eventDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".EventDate=" << StringUtils::URLEncode(m_eventDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  ...
  if (m_severityHasBeenSet) {
    oStream << location << index << locationValue
            << ".Severity=" << StringUtils::URLEncode(EventSeverityMapper::GetNameForEventSeverity(m_severity)) << "&";
  }
}

void EventDescription::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_eventDateHasBeenSet) {
    oStream << location << ".EventDate=" << StringUtils::URLEncode(m_eventDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
  if (m_messageHasBeenSet) {
