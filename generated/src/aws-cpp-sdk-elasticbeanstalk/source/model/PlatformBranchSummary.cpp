...
PlatformBranchSummary::PlatformBranchSummary(const XmlNode& xmlNode) { *this = xmlNode; }
...
PlatformBranchSummary& PlatformBranchSummary::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!branchOrderNode.IsNull()) {
      m_branchOrder =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(branchOrderNode.GetText()).c_str()).c_str());
