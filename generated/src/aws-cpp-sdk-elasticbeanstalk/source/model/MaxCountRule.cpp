...
MaxCountRule::MaxCountRule(const XmlNode& xmlNode) { *this = xmlNode; }
...
MaxCountRule& MaxCountRule::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!maxCountNode.IsNull()) {
      m_maxCount =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maxCountNode.GetText()).c_str()).c_str());
