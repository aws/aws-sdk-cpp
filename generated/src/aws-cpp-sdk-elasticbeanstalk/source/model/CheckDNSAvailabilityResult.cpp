...
CheckDNSAvailabilityResult::CheckDNSAvailabilityResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
CheckDNSAvailabilityResult& CheckDNSAvailabilityResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!availableNode.IsNull()) {
      m_available =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(availableNode.GetText()).c_str()).c_str());
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::ElasticBeanstalk::Model::CheckDNSAvailabilityResult",
