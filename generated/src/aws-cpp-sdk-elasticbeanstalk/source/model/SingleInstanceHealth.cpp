...
SingleInstanceHealth::SingleInstanceHealth(const XmlNode& xmlNode) { *this = xmlNode; }
...
SingleInstanceHealth& SingleInstanceHealth::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!launchedAtNode.IsNull()) {
      m_launchedAt = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(launchedAtNode.GetText()).c_str()).c_str(),
...
void SingleInstanceHealth::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_launchedAtHasBeenSet) {
    oStream << location << index << locationValue
            << ".LaunchedAt=" << StringUtils::URLEncode(m_launchedAt.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
