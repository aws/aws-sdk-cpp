...
SolutionStackDescription::SolutionStackDescription(const XmlNode& xmlNode) { *this = xmlNode; }
...
void SolutionStackDescription::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_permittedFileTypesHasBeenSet) {
    ...
    for (auto& item : m_permittedFileTypes) {
      oStream << location << index << locationValue << ".PermittedFileTypes.member." << permittedFileTypesIdx++ << "="
