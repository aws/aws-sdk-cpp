  ...
  if (m_environmentIdsHasBeenSet) {
    if (m_environmentIds.empty()) {
      ...
      for (auto& item : m_environmentIds) {
        ss << "EnvironmentIds.member." << environmentIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        environmentIdsCount++;
      }
    }
  }
  ...
  if (m_environmentNamesHasBeenSet) {
    if (m_environmentNames.empty()) {
      ...
      for (auto& item : m_environmentNames) {
        ss << "EnvironmentNames.member." << environmentNamesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeEnvironmentsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
