...
Deployment::Deployment(const XmlNode& xmlNode) { *this = xmlNode; }
...
Deployment& Deployment::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!deploymentIdNode.IsNull()) {
      m_deploymentId =
          StringUtils::ConvertToInt64(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deploymentIdNode.GetText()).c_str()).c_str());
    ...
    if (!deploymentTimeNode.IsNull()) {
      m_deploymentTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deploymentTimeNode.GetText()).c_str()).c_str(),
...
void Deployment::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_deploymentTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".DeploymentTime=" << StringUtils::URLEncode(m_deploymentTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
