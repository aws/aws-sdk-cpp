...
SourceBuildInformation::SourceBuildInformation(const XmlNode& xmlNode) { *this = xmlNode; }
...
SourceBuildInformation& SourceBuildInformation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!sourceRepositoryNode.IsNull()) {
      m_sourceRepository = SourceRepositoryMapper::GetSourceRepositoryForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sourceRepositoryNode.GetText()).c_str()));
...
void SourceBuildInformation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_sourceRepositoryHasBeenSet) {
    oStream << location << index << locationValue
            << ".SourceRepository=" << StringUtils::URLEncode(SourceRepositoryMapper::GetNameForSourceRepository(m_sourceRepository))
            << "&";
  }
