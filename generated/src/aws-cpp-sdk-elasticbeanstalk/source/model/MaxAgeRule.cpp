...
MaxAgeRule::MaxAgeRule(const XmlNode& xmlNode) { *this = xmlNode; }
...
MaxAgeRule& MaxAgeRule::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!maxAgeInDaysNode.IsNull()) {
      m_maxAgeInDays =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maxAgeInDaysNode.GetText()).c_str()).c_str());
    ...
    if (!deleteSourceFromS3Node.IsNull()) {
      m_deleteSourceFromS3 = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deleteSourceFromS3Node.GetText()).c_str()).c_str());
