...
ConfigurationOptionDescription::ConfigurationOptionDescription(const XmlNode& xmlNode) { *this = xmlNode; }
...
ConfigurationOptionDescription& ConfigurationOptionDescription::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!userDefinedNode.IsNull()) {
      m_userDefined =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(userDefinedNode.GetText()).c_str()).c_str());
      m_userDefinedHasBeenSet = true;
    }
    XmlNode valueTypeNode = resultNode.FirstChild("ValueType");
    if (!valueTypeNode.IsNull()) {
      m_valueType = ConfigurationOptionValueTypeMapper::GetConfigurationOptionValueTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(valueTypeNode.GetText()).c_str()));
    ...
    if (!minValueNode.IsNull()) {
      m_minValue =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(minValueNode.GetText()).c_str()).c_str());
      m_minValueHasBeenSet = true;
    }
    XmlNode maxValueNode = resultNode.FirstChild("MaxValue");
    if (!maxValueNode.IsNull()) {
      m_maxValue =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maxValueNode.GetText()).c_str()).c_str());
...
void ConfigurationOptionDescription::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_valueTypeHasBeenSet) {
    oStream << location << index << locationValue << ".ValueType="
            << StringUtils::URLEncode(ConfigurationOptionValueTypeMapper::GetNameForConfigurationOptionValueType(m_valueType)) << "&";
  }

  if (m_valueOptionsHasBeenSet) {
    unsigned valueOptionsIdx = 1;
    for (auto& item : m_valueOptions) {
      oStream << location << index << locationValue << ".ValueOptions.member." << valueOptionsIdx++ << "="
