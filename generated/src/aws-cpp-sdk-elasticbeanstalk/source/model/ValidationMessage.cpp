...
ValidationMessage::ValidationMessage(const XmlNode& xmlNode) { *this = xmlNode; }
...
ValidationMessage& ValidationMessage::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!severityNode.IsNull()) {
      m_severity = ValidationSeverityMapper::GetValidationSeverityForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(severityNode.GetText()).c_str()));
...
void ValidationMessage::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_severityHasBeenSet) {
    oStream << location << index << locationValue
            << ".Severity=" << StringUtils::URLEncode(ValidationSeverityMapper::GetNameForValidationSeverity(m_severity)) << "&";
  }
