...
EnvironmentInfoDescription::EnvironmentInfoDescription(const XmlNode& xmlNode) { *this = xmlNode; }
...
EnvironmentInfoDescription& EnvironmentInfoDescription::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!infoTypeNode.IsNull()) {
      m_infoType = EnvironmentInfoTypeMapper::GetEnvironmentInfoTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(infoTypeNode.GetText()).c_str()));
    ...
    if (!sampleTimestampNode.IsNull()) {
      m_sampleTimestamp = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sampleTimestampNode.GetText()).c_str()).c_str(),
...
void EnvironmentInfoDescription::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                const char* locationValue) const {
  if (m_infoTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".InfoType=" << StringUtils::URLEncode(EnvironmentInfoTypeMapper::GetNameForEnvironmentInfoType(m_infoType)) << "&";
  }
  ...
  if (m_sampleTimestampHasBeenSet) {
    oStream << location << index << locationValue
            << ".SampleTimestamp=" << StringUtils::URLEncode(m_sampleTimestamp.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
