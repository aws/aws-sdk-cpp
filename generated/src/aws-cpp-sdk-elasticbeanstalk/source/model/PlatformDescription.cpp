...
PlatformDescription::PlatformDescription(const XmlNode& xmlNode) { *this = xmlNode; }
...
PlatformDescription& PlatformDescription::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!platformStatusNode.IsNull()) {
      m_platformStatus = PlatformStatusMapper::GetPlatformStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(platformStatusNode.GetText()).c_str()));
...
void PlatformDescription::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_platformStatusHasBeenSet) {
    oStream << location << index << locationValue
            << ".PlatformStatus=" << StringUtils::URLEncode(PlatformStatusMapper::GetNameForPlatformStatus(m_platformStatus)) << "&";
  }
  ...
  if (m_supportedTierListHasBeenSet) {
    ...
    for (auto& item : m_supportedTierList) {
      oStream << location << index << locationValue << ".SupportedTierList.member." << supportedTierListIdx++ << "="
  ...
  if (m_supportedAddonListHasBeenSet) {
    ...
    for (auto& item : m_supportedAddonList) {
      oStream << location << index << locationValue << ".SupportedAddonList.member." << supportedAddonListIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
  ...
  if (m_platformBranchLifecycleStateHasBeenSet) {
    oStream << location << index << locationValue
            << ".PlatformBranchLifecycleState=" << StringUtils::URLEncode(m_platformBranchLifecycleState.c_str()) << "&";
  }
}
