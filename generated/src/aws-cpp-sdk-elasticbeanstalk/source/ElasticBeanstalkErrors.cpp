...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == ELASTIC_BEANSTALK_SERVICE_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ElasticBeanstalkErrors::S3_LOCATION_NOT_IN_SERVICE_REGION),
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ElasticBeanstalkErrors::CODE_BUILD_NOT_IN_SERVICE_REGION),
                                RetryableType::NOT_RETRYABLE);
  } else if (hashCode == PLATFORM_VERSION_STILL_REFERENCED_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ElasticBeanstalkErrors::PLATFORM_VERSION_STILL_REFERENCED),
                                RetryableType::NOT_RETRYABLE);
  } else if (hashCode == TOO_MANY_CONFIGURATION_TEMPLATES_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ElasticBeanstalkErrors::TOO_MANY_CONFIGURATION_TEMPLATES),
                                RetryableType::NOT_RETRYABLE);
  } else if (hashCode == MANAGED_ACTION_INVALID_STATE_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ElasticBeanstalkErrors::MANAGED_ACTION_INVALID_STATE),
                                RetryableType::NOT_RETRYABLE);
  } else if (hashCode == TOO_MANY_APPLICATION_VERSIONS_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ElasticBeanstalkErrors::TOO_MANY_APPLICATION_VERSIONS),
                                RetryableType::NOT_RETRYABLE);
