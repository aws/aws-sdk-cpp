...
PermissionGroup::PermissionGroup(JsonView jsonValue) { *this = jsonValue; }
...
PermissionGroup& PermissionGroup::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("applicationPermissions")) {
    ...
    for (unsigned applicationPermissionsIndex = 0; applicationPermissionsIndex < applicationPermissionsJsonList.GetLength();
      ...
      m_applicationPermissions.push_back(ApplicationPermissionMapper::GetApplicationPermissionForName(
          applicationPermissionsJsonList[applicationPermissionsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("membershipStatus")) {
    m_membershipStatus =
        PermissionGroupMembershipStatusMapper::GetPermissionGroupMembershipStatusForName(jsonValue.GetString("membershipStatus"));
...
JsonValue PermissionGroup::Jsonize() const {
  ...
  if (m_applicationPermissionsHasBeenSet) {
    ...
    for (unsigned applicationPermissionsIndex = 0; applicationPermissionsIndex < applicationPermissionsJsonList.GetLength();
         ++applicationPermissionsIndex) {
      applicationPermissionsJsonList[applicationPermissionsIndex].AsString(
          ApplicationPermissionMapper::GetNameForApplicationPermission(m_applicationPermissions[applicationPermissionsIndex]));
    }
  ...
  if (m_membershipStatusHasBeenSet) {
    payload.WithString("membershipStatus",
                       PermissionGroupMembershipStatusMapper::GetNameForPermissionGroupMembershipStatus(m_membershipStatus));
  }
