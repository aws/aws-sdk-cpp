...
class S3ExportSpecification {
 public:
  ...
  void SetDestinationBucket(DestinationBucketT&& value) {
    ...
    m_destinationBucket = std::forward<DestinationBucketT>(value);
  }
  template <typename DestinationBucketT = Aws::String>
  S3ExportSpecification& WithDestinationBucket(DestinationBucketT&& value) {
    SetDestinationBucket(std::forward<DestinationBucketT>(value));
    return *this;
  }
  ///@}
  ...
  void SetDestinationPrefix(DestinationPrefixT&& value) {
    ...
    m_destinationPrefix = std::forward<DestinationPrefixT>(value);
  }
  template <typename DestinationPrefixT = Aws::String>
  S3ExportSpecification& WithDestinationPrefix(DestinationPrefixT&& value) {
    SetDestinationPrefix(std::forward<DestinationPrefixT>(value));
    return *this;
  }
  ///@}
 private:
