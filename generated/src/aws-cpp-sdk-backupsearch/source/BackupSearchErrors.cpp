...
template <>
AWS_BACKUPSEARCH_API ConflictException BackupSearchError::GetModeledError() {
  assert(this->GetErrorType() == BackupSearchErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_BACKUPSEARCH_API ThrottlingException BackupSearchError::GetModeledError() {
  assert(this->GetErrorType() == BackupSearchErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_BACKUPSEARCH_API ServiceQuotaExceededException BackupSearchError::GetModeledError() {
  assert(this->GetErrorType() == BackupSearchErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_BACKUPSEARCH_API ResourceNotFoundException BackupSearchError::GetModeledError() {
  assert(this->GetErrorType() == BackupSearchErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_BACKUPSEARCH_API InternalServerException BackupSearchError::GetModeledError() {
