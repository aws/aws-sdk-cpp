...
EBSItemFilter::EBSItemFilter(JsonView jsonValue) { *this = jsonValue; }
...
EBSItemFilter& EBSItemFilter::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("LastModificationTimes")) {
    ...
    for (unsigned lastModificationTimesIndex = 0; lastModificationTimesIndex < lastModificationTimesJsonList.GetLength();
         ++lastModificationTimesIndex) {
