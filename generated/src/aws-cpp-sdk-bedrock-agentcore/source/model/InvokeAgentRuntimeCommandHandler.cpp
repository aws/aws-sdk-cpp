...
static const char INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG[] = "InvokeAgentRuntimeCommandHandler";

InvokeAgentRuntimeCommandHandler::InvokeAgentRuntimeCommandHandler() : EventStreamHandler() {
  m_onInitialResponse = [&](const InvokeAgentRuntimeCommandInitialResponse&, const Utils::Event::InitialResponseType eventType) {
    AWS_LOGSTREAM_TRACE(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG,
                        "InvokeAgentRuntimeCommand initial response received from "
                            << (eventType == Utils::Event::InitialResponseType::ON_EVENT ? "event" : "http headers"));
  };

  m_onResponseChunk = [&](const ResponseChunk&) {
    AWS_LOGSTREAM_TRACE(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG, "ResponseChunk received.");
  };

  m_onError = [&](const AWSError<BedrockAgentCoreErrors>& error) {
    AWS_LOGSTREAM_TRACE(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG, "BedrockAgentCore Errors received, " << error);
  };
}

void InvokeAgentRuntimeCommandHandler::OnEvent() {
  ...
  if (messageTypeHeaderIter == headers.end()) {
    AWS_LOGSTREAM_WARN(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG, "Header: " << MESSAGE_TYPE_HEADER << " not found in the message.");
    return;
  }
  ...
  switch (Aws::Utils::Event::Message::GetMessageTypeForName(messageTypeHeaderIter->second.GetEventHeaderValueAsString())) {
    ...
    default:
      AWS_LOGSTREAM_WARN(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG,
                         "Unexpected message type: " << messageTypeHeaderIter->second.GetEventHeaderValueAsString());
      break;
  }
}

void InvokeAgentRuntimeCommandHandler::HandleEventInMessage() {
  const auto& headers = GetEventHeaders();
  auto eventTypeHeaderIter = headers.find(EVENT_TYPE_HEADER);
  if (eventTypeHeaderIter == headers.end()) {
    AWS_LOGSTREAM_WARN(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG, "Header: " << EVENT_TYPE_HEADER << " not found in the message.");
    return;
  }
  switch (InvokeAgentRuntimeCommandEventMapper::GetInvokeAgentRuntimeCommandEventTypeForName(
      eventTypeHeaderIter->second.GetEventHeaderValueAsString())) {
    case InvokeAgentRuntimeCommandEventType::INITIAL_RESPONSE: {
      InvokeAgentRuntimeCommandInitialResponse event(GetEventHeadersAsHttpHeaders());
      m_onInitialResponse(event, Utils::Event::InitialResponseType::ON_EVENT);
      break;
    }

    case InvokeAgentRuntimeCommandEventType::CHUNK: {
      JsonValue json(GetEventPayloadAsString());
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG,
                           "Unable to generate a proper ResponseChunk object from the response in JSON format.");
        break;
      }

      m_onResponseChunk(ResponseChunk{json.View()});
      break;
    }
    default:
      AWS_LOGSTREAM_WARN(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG,
...
void InvokeAgentRuntimeCommandHandler::HandleErrorInMessage() {
  const auto& headers = GetEventHeaders();
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }

    JsonValue exceptionPayload(GetEventPayloadAsString());
    if (!exceptionPayload.WasParseSuccessful()) {
      AWS_LOGSTREAM_ERROR(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG,
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(INVOKEAGENTRUNTIMECOMMAND_HANDLER_CLASS_TAG,
...
void InvokeAgentRuntimeCommandHandler::MarshallError(const Aws::String& errorCode, const Aws::String& errorMessage) {
...
InvokeAgentRuntimeCommandEventType GetInvokeAgentRuntimeCommandEventTypeForName(const Aws::String& name) {
  int hashCode = Aws::Utils::HashingUtils::HashString(name.c_str());

  if (hashCode == INITIAL_RESPONSE_HASH) {
    return InvokeAgentRuntimeCommandEventType::INITIAL_RESPONSE;
  } else if (hashCode == CHUNK_HASH) {
    return InvokeAgentRuntimeCommandEventType::CHUNK;
  }
  return InvokeAgentRuntimeCommandEventType::UNKNOWN;
}

Aws::String GetNameForInvokeAgentRuntimeCommandEventType(InvokeAgentRuntimeCommandEventType value) {
  switch (value) {
    case InvokeAgentRuntimeCommandEventType::INITIAL_RESPONSE:
