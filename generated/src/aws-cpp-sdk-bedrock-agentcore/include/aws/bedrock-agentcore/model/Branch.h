...
class Branch {
 public:
  ...
  void SetRootEventId(RootEventIdT&& value) {
    m_rootEventIdHasBeenSet = true;
    m_rootEventId = std::forward<RootEventIdT>(value);
  }
  template <typename RootEventIdT = Aws::String>
  Branch& WithRootEventId(RootEventIdT&& value) {
    SetRootEventId(std::forward<RootEventIdT>(value));
    return *this;
  }
  ///@}
