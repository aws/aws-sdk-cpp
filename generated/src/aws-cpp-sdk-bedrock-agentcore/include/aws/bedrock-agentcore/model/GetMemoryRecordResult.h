...
class GetMemoryRecordResult {
 public:
  ...
  void SetMemoryRecord(MemoryRecordT&& value) {
    m_memoryRecordHasBeenSet = true;
    m_memoryRecord = std::forward<MemoryRecordT>(value);
  }
  template <typename MemoryRecordT = MemoryRecord>
  GetMemoryRecordResult& WithMemoryRecord(MemoryRecordT&& value) {
    SetMemoryRecord(std::forward<MemoryRecordT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetMemoryRecordResult& WithRequestId(RequestIdT&& value) {
