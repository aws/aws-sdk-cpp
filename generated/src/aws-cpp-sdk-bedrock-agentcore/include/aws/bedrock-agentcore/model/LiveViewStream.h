...
class LiveViewStream {
 public:
  ...
  void SetStreamEndpoint(StreamEndpointT&& value) {
    m_streamEndpointHasBeenSet = true;
    m_streamEndpoint = std::forward<StreamEndpointT>(value);
  }
  template <typename StreamEndpointT = Aws::String>
  LiveViewStream& WithStreamEndpoint(StreamEndpointT&& value) {
    SetStreamEndpoint(std::forward<StreamEndpointT>(value));
    return *this;
  }
  ///@}
 private:
