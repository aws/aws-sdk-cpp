...
class BatchPutMetricsRequest : public SageMakerMetricsRequest {
 public:
  ...
  template <typename TrialComponentNameT = Aws::String>
  BatchPutMetricsRequest& WithTrialComponentName(TrialComponentNameT&& value) {
  ...
  template <typename MetricDataT = Aws::Vector<RawMetricData>>
  BatchPutMetricsRequest& WithMetricData(MetricDataT&& value) {
    SetMetricData(std::forward<MetricDataT>(value));
    return *this;
  }
  template <typename MetricDataT = RawMetricData>
  BatchPutMetricsRequest& AddMetricData(MetricDataT&& value) {
    m_metricDataHasBeenSet = true;
