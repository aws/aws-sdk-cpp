...
class CalculatedSpend {
 public:
  ...
  void SetActualSpend(ActualSpendT&& value) {
    ...
    m_actualSpend = std::forward<ActualSpendT>(value);
  }
  template <typename ActualSpendT = Spend>
  CalculatedSpend& WithActualSpend(ActualSpendT&& value) {
    SetActualSpend(std::forward<ActualSpendT>(value));
    return *this;
  }
  ///@}
  ...
  void SetForecastedSpend(ForecastedSpendT&& value) {
    ...
    m_forecastedSpend = std::forward<ForecastedSpendT>(value);
  }
  template <typename ForecastedSpendT = Spend>
  CalculatedSpend& WithForecastedSpend(ForecastedSpendT&& value) {
    SetForecastedSpend(std::forward<ForecastedSpendT>(value));
    return *this;
  }
  ///@}
 private:
