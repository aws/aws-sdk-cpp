...
class FailedRemediationExceptionBatch {
 public:
  ...
  template <typename FailureMessageT = Aws::String>
  FailedRemediationExceptionBatch& WithFailureMessage(FailureMessageT&& value) {
  ...
  template <typename FailedItemsT = Aws::Vector<RemediationException>>
  FailedRemediationExceptionBatch& WithFailedItems(FailedItemsT&& value) {
    SetFailedItems(std::forward<FailedItemsT>(value));
    return *this;
  }
  template <typename FailedItemsT = RemediationException>
  FailedRemediationExceptionBatch& AddFailedItems(FailedItemsT&& value) {
    m_failedItemsHasBeenSet = true;
