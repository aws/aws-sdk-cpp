...
class FailedRemediationBatch {
 public:
  ...
  template <typename FailureMessageT = Aws::String>
  FailedRemediationBatch& WithFailureMessage(FailureMessageT&& value) {
  ...
  template <typename FailedItemsT = Aws::Vector<RemediationConfiguration>>
  FailedRemediationBatch& WithFailedItems(FailedItemsT&& value) {
    SetFailedItems(std::forward<FailedItemsT>(value));
    return *this;
  }
  template <typename FailedItemsT = RemediationConfiguration>
  FailedRemediationBatch& AddFailedItems(FailedItemsT&& value) {
    m_failedItemsHasBeenSet = true;
