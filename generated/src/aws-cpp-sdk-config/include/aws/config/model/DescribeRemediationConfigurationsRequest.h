...
class DescribeRemediationConfigurationsRequest : public ConfigServiceRequest {
 public:
  ...
  template <typename ConfigRuleNamesT = Aws::Vector<Aws::String>>
  DescribeRemediationConfigurationsRequest& WithConfigRuleNames(ConfigRuleNamesT&& value) {
    SetConfigRuleNames(std::forward<ConfigRuleNamesT>(value));
    return *this;
  }
  template <typename ConfigRuleNamesT = Aws::String>
  DescribeRemediationConfigurationsRequest& AddConfigRuleNames(ConfigRuleNamesT&& value) {
    m_configRuleNamesHasBeenSet = true;
