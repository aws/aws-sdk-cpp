...
class SelectAggregateResourceConfigResult {
 public:
  ...
  AWS_CONFIGSERVICE_API SelectAggregateResourceConfigResult& operator=(
  ...
  void SetQueryInfo(QueryInfoT&& value) {
    m_queryInfoHasBeenSet = true;
    m_queryInfo = std::forward<QueryInfoT>(value);
  }
  template <typename QueryInfoT = QueryInfo>
  SelectAggregateResourceConfigResult& WithQueryInfo(QueryInfoT&& value) {
    SetQueryInfo(std::forward<QueryInfoT>(value));
    return *this;
  }
  ///@}
