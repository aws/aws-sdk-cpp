...
class DeliverConfigSnapshotRequest : public ConfigServiceRequest {
 public:
  AWS_CONFIGSERVICE_API DeliverConfigSnapshotRequest() = default;
  ...
  void SetDeliveryChannelName(DeliveryChannelNameT&& value) {
    m_deliveryChannelNameHasBeenSet = true;
    m_deliveryChannelName = std::forward<DeliveryChannelNameT>(value);
  }
  template <typename DeliveryChannelNameT = Aws::String>
  DeliverConfigSnapshotRequest& WithDeliveryChannelName(DeliveryChannelNameT&& value) {
    SetDeliveryChannelName(std::forward<DeliveryChannelNameT>(value));
    return *this;
  }
  ///@}
 private:
