...
class PutDeliveryChannelRequest : public ConfigServiceRequest {
 public:
  ...
  void SetDeliveryChannel(DeliveryChannelT&& value) {
    m_deliveryChannelHasBeenSet = true;
    m_deliveryChannel = std::forward<DeliveryChannelT>(value);
  }
  template <typename DeliveryChannelT = DeliveryChannel>
  PutDeliveryChannelRequest& WithDeliveryChannel(DeliveryChannelT&& value) {
    SetDeliveryChannel(std::forward<DeliveryChannelT>(value));
    return *this;
  }
  ///@}
 private:
