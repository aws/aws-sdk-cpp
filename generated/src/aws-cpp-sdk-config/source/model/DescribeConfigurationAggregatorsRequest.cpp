...
Aws::String DescribeConfigurationAggregatorsRequest::SerializePayload() const {
  ...
  if (m_configurationAggregatorNamesHasBeenSet) {
    ...
    for (unsigned configurationAggregatorNamesIndex = 0;
         configurationAggregatorNamesIndex < configurationAggregatorNamesJsonList.GetLength(); ++configurationAggregatorNamesIndex) {
      configurationAggregatorNamesJsonList[configurationAggregatorNamesIndex].AsString(
          m_configurationAggregatorNames[configurationAggregatorNamesIndex]);
    }
