...
class RequestFilter {
 public:
  ...
  AWS_BCMRECOMMENDEDACTIONS_API RequestFilter(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_BCMRECOMMENDEDACTIONS_API RequestFilter& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  template <typename ActionsT = Aws::Vector<ActionFilter>>
  RequestFilter& WithActions(ActionsT&& value) {
    SetActions(std::forward<ActionsT>(value));
    return *this;
  }
  template <typename ActionsT = ActionFilter>
  RequestFilter& AddActions(ActionsT&& value) {
    m_actionsHasBeenSet = true;
