...
ConfigurationManagerSummary::ConfigurationManagerSummary(JsonView jsonValue) { *this = jsonValue; }
...
ConfigurationManagerSummary& ConfigurationManagerSummary::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("ConfigurationDefinitionSummaries")) {
    ...
    for (unsigned configurationDefinitionSummariesIndex = 0;
         configurationDefinitionSummariesIndex < configurationDefinitionSummariesJsonList.GetLength();
         ++configurationDefinitionSummariesIndex) {
      m_configurationDefinitionSummaries.push_back(
          configurationDefinitionSummariesJsonList[configurationDefinitionSummariesIndex].AsObject());
    }
...
JsonValue ConfigurationManagerSummary::Jsonize() const {
  ...
  if (m_configurationDefinitionSummariesHasBeenSet) {
    ...
    for (unsigned configurationDefinitionSummariesIndex = 0;
         ...
         ++configurationDefinitionSummariesIndex) {
      configurationDefinitionSummariesJsonList[configurationDefinitionSummariesIndex].AsObject(
          m_configurationDefinitionSummaries[configurationDefinitionSummariesIndex].Jsonize());
    }
