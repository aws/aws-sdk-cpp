...
Aws::String CreateConfigurationManagerRequest::SerializePayload() const {
  ...
  if (m_configurationDefinitionsHasBeenSet) {
    ...
    for (unsigned configurationDefinitionsIndex = 0; configurationDefinitionsIndex < configurationDefinitionsJsonList.GetLength();
         ++configurationDefinitionsIndex) {
      configurationDefinitionsJsonList[configurationDefinitionsIndex].AsObject(
          m_configurationDefinitions[configurationDefinitionsIndex].Jsonize());
    }
