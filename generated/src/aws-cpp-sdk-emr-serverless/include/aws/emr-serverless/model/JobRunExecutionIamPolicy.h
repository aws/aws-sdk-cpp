...
class JobRunExecutionIamPolicy {
 public:
  ...
  template <typename PolicyT = Aws::String>
  JobRunExecutionIamPolicy& WithPolicy(PolicyT&& value) {
  ...
  template <typename PolicyArnsT = Aws::Vector<Aws::String>>
  JobRunExecutionIamPolicy& WithPolicyArns(PolicyArnsT&& value) {
    SetPolicyArns(std::forward<PolicyArnsT>(value));
    return *this;
  }
  template <typename PolicyArnsT = Aws::String>
  JobRunExecutionIamPolicy& AddPolicyArns(PolicyArnsT&& value) {
    m_policyArnsHasBeenSet = true;
