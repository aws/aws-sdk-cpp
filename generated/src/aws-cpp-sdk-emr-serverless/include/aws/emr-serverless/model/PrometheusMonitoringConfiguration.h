...
class PrometheusMonitoringConfiguration {
 public:
  ...
  void SetRemoteWriteUrl(RemoteWriteUrlT&& value) {
    m_remoteWriteUrlHasBeenSet = true;
    m_remoteWriteUrl = std::forward<RemoteWriteUrlT>(value);
  }
  template <typename RemoteWriteUrlT = Aws::String>
  PrometheusMonitoringConfiguration& WithRemoteWriteUrl(RemoteWriteUrlT&& value) {
    SetRemoteWriteUrl(std::forward<RemoteWriteUrlT>(value));
    return *this;
  }
  ///@}
 private:
