...
class BedrockFoundationModelConfigurationForParsing {
 public:
  ...
  template <typename ModelArnT = Aws::String>
  BedrockFoundationModelConfigurationForParsing& WithModelArn(ModelArnT&& value) {
  ...
  void SetParsingPrompt(ParsingPromptT&& value) {
    m_parsingPromptHasBeenSet = true;
    m_parsingPrompt = std::forward<ParsingPromptT>(value);
  }
  template <typename ParsingPromptT = ParsingPrompt>
  BedrockFoundationModelConfigurationForParsing& WithParsingPrompt(ParsingPromptT&& value) {
    SetParsingPrompt(std::forward<ParsingPromptT>(value));
    return *this;
  }
  ///@}
 private:
