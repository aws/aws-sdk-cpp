...
ConversationContext::ConversationContext(JsonView jsonValue) { *this = jsonValue; }
...
ConversationContext& ConversationContext::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("selfServiceConversationHistory")) {
    ...
    for (unsigned selfServiceConversationHistoryIndex = 0;
         selfServiceConversationHistoryIndex < selfServiceConversationHistoryJsonList.GetLength(); ++selfServiceConversationHistoryIndex) {
...
JsonValue ConversationContext::Jsonize() const {
  ...
  if (m_selfServiceConversationHistoryHasBeenSet) {
    ...
    for (unsigned selfServiceConversationHistoryIndex = 0;
         selfServiceConversationHistoryIndex < selfServiceConversationHistoryJsonList.GetLength(); ++selfServiceConversationHistoryIndex) {
      selfServiceConversationHistoryJsonList[selfServiceConversationHistoryIndex].AsObject(
          m_selfServiceConversationHistory[selfServiceConversationHistoryIndex].Jsonize());
    }
