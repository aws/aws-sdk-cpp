...
TrustStoreRevocation::TrustStoreRevocation(const XmlNode& xmlNode) { *this = xmlNode; }
...
TrustStoreRevocation& TrustStoreRevocation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!revocationIdNode.IsNull()) {
      m_revocationId =
          StringUtils::ConvertToInt64(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(revocationIdNode.GetText()).c_str()).c_str());
    ...
    if (!numberOfRevokedEntriesNode.IsNull()) {
      m_numberOfRevokedEntries = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(numberOfRevokedEntriesNode.GetText()).c_str()).c_str());
