...
RuleTransform::RuleTransform(const XmlNode& xmlNode) { *this = xmlNode; }
...
RuleTransform& RuleTransform::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = TransformTypeEnumMapper::GetTransformTypeEnumForName(
...
void RuleTransform::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(TransformTypeEnumMapper::GetNameForTransformTypeEnum(m_type)) << "&";
  }
