...
CapacityReservationStatus& CapacityReservationStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!codeNode.IsNull()) {
      m_code = CapacityReservationStateEnumMapper::GetCapacityReservationStateEnumForName(
...
void CapacityReservationStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_codeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Code=" << StringUtils::URLEncode(CapacityReservationStateEnumMapper::GetNameForCapacityReservationStateEnum(m_code))
            << "&";
  }
