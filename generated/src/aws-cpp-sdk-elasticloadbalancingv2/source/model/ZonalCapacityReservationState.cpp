...
ZonalCapacityReservationState::ZonalCapacityReservationState(const XmlNode& xmlNode) { *this = xmlNode; }
...
ZonalCapacityReservationState& ZonalCapacityReservationState::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!effectiveCapacityUnitsNode.IsNull()) {
      m_effectiveCapacityUnits = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(effectiveCapacityUnitsNode.GetText()).c_str()).c_str());
...
void ZonalCapacityReservationState::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
