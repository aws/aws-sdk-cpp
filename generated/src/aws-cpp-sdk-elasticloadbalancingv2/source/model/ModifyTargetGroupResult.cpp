...
ModifyTargetGroupResult::ModifyTargetGroupResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }

ModifyTargetGroupResult& ModifyTargetGroupResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "ModifyTargetGroupResult")) {
    resultNode = rootNode.FirstChild("ModifyTargetGroupResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::ElasticLoadBalancingv2::Model::ModifyTargetGroupResult",
