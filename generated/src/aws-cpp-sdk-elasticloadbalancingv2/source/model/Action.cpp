...
Action::Action(const XmlNode& xmlNode) { *this = xmlNode; }
...
Action& Action::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = ActionTypeEnumMapper::GetActionTypeEnumForName(
...
void Action::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(ActionTypeEnumMapper::GetNameForActionTypeEnum(m_type)) << "&";
  }
