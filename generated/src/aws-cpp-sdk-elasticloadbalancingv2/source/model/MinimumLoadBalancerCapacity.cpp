...
MinimumLoadBalancerCapacity::MinimumLoadBalancerCapacity(const XmlNode& xmlNode) { *this = xmlNode; }
...
MinimumLoadBalancerCapacity& MinimumLoadBalancerCapacity::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!capacityUnitsNode.IsNull()) {
      m_capacityUnits = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(capacityUnitsNode.GetText()).c_str()).c_str());
...
void MinimumLoadBalancerCapacity::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
