...
TrustStore::TrustStore(const XmlNode& xmlNode) { *this = xmlNode; }
...
TrustStore& TrustStore::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!totalRevokedEntriesNode.IsNull()) {
      m_totalRevokedEntries = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalRevokedEntriesNode.GetText()).c_str()).c_str());
...
void TrustStore::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(TrustStoreStatusMapper::GetNameForTrustStoreStatus(m_status)) << "&";
  }
