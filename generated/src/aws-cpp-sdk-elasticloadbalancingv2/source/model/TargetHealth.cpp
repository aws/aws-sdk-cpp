...
TargetHealth::TargetHealth(const XmlNode& xmlNode) { *this = xmlNode; }
...
TargetHealth& TargetHealth::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = TargetHealthStateEnumMapper::GetTargetHealthStateEnumForName(
    ...
    if (!reasonNode.IsNull()) {
      m_reason = TargetHealthReasonEnumMapper::GetTargetHealthReasonEnumForName(
...
void TargetHealth::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(TargetHealthStateEnumMapper::GetNameForTargetHealthStateEnum(m_state)) << "&";
  }

  if (m_reasonHasBeenSet) {
    oStream << location << index << locationValue
            << ".Reason=" << StringUtils::URLEncode(TargetHealthReasonEnumMapper::GetNameForTargetHealthReasonEnum(m_reason)) << "&";
  }
