...
RedirectActionConfig::RedirectActionConfig(const XmlNode& xmlNode) { *this = xmlNode; }
...
RedirectActionConfig& RedirectActionConfig::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusCodeNode.IsNull()) {
      m_statusCode = RedirectActionStatusCodeEnumMapper::GetRedirectActionStatusCodeEnumForName(
...
void RedirectActionConfig::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusCodeHasBeenSet) {
    oStream << location << index << locationValue << ".StatusCode="
            << StringUtils::URLEncode(RedirectActionStatusCodeEnumMapper::GetNameForRedirectActionStatusCodeEnum(m_statusCode)) << "&";
  }
}
