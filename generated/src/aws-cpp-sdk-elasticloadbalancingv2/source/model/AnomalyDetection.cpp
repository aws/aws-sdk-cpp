...
AnomalyDetection::AnomalyDetection(const XmlNode& xmlNode) { *this = xmlNode; }
...
AnomalyDetection& AnomalyDetection::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!anomalyDetectionResultNode.IsNull()) {
      m_result = AnomalyResultEnumMapper::GetAnomalyResultEnumForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(anomalyDetectionResultNode.GetText()).c_str()));
      m_resultHasBeenSet = true;
    }
    XmlNode mitigationInEffectNode = resultNode.FirstChild("MitigationInEffect");
    if (!mitigationInEffectNode.IsNull()) {
      m_mitigationInEffect = MitigationInEffectEnumMapper::GetMitigationInEffectEnumForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(mitigationInEffectNode.GetText()).c_str()));
...
void AnomalyDetection::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_resultHasBeenSet) {
    oStream << location << index << locationValue
            << ".Result=" << StringUtils::URLEncode(AnomalyResultEnumMapper::GetNameForAnomalyResultEnum(m_result)) << "&";
  }

  if (m_mitigationInEffectHasBeenSet) {
    oStream << location << index << locationValue << ".MitigationInEffect="
            << StringUtils::URLEncode(MitigationInEffectEnumMapper::GetNameForMitigationInEffectEnum(m_mitigationInEffect)) << "&";
  }
}
...
void AnomalyDetection::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_mitigationInEffectHasBeenSet) {
    oStream << location << ".MitigationInEffect="
            << StringUtils::URLEncode(MitigationInEffectEnumMapper::GetNameForMitigationInEffectEnum(m_mitigationInEffect)) << "&";
  }
}
