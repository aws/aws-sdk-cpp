  ...
  if (m_trustStoreArnsHasBeenSet) {
    if (m_trustStoreArns.empty()) {
      ...
      for (auto& item : m_trustStoreArns) {
        ss << "TrustStoreArns.member." << trustStoreArnsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        trustStoreArnsCount++;
      }
    }
  }
...
void DescribeTrustStoresRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
