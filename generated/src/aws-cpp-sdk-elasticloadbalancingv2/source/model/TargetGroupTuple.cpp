...
TargetGroupTuple::TargetGroupTuple(const XmlNode& xmlNode) { *this = xmlNode; }
...
TargetGroupTuple& TargetGroupTuple::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!weightNode.IsNull()) {
      m_weight =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(weightNode.GetText()).c_str()).c_str());
