...
HostHeaderRewriteConfig::HostHeaderRewriteConfig(const XmlNode& xmlNode) { *this = xmlNode; }

HostHeaderRewriteConfig& HostHeaderRewriteConfig::operator=(const XmlNode& xmlNode) {
...
void HostHeaderRewriteConfig::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
...
void HostHeaderRewriteConfig::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_rewritesHasBeenSet) {
    unsigned rewritesIdx = 1;
    for (auto& item : m_rewrites) {
      Aws::StringStream rewritesSs;
