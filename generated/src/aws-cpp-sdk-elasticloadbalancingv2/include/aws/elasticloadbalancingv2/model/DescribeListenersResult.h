...
class DescribeListenersResult {
 public:
  ...
  AWS_ELASTICLOADBALANCINGV2_API DescribeListenersResult& operator=(
  ...
  template <typename ListenersT = Aws::Vector<Listener>>
  DescribeListenersResult& WithListeners(ListenersT&& value) {
    SetListeners(std::forward<ListenersT>(value));
    return *this;
  }
  template <typename ListenersT = Listener>
  DescribeListenersResult& AddListeners(ListenersT&& value) {
    m_listenersHasBeenSet = true;
  ...
  template <typename NextMarkerT = Aws::String>
  DescribeListenersResult& WithNextMarker(NextMarkerT&& value) {
  ...
  template <typename ResponseMetadataT = ResponseMetadata>
  DescribeListenersResult& WithResponseMetadata(ResponseMetadataT&& value) {
