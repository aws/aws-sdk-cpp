...
class AddTrustStoreRevocationsRequest : public ElasticLoadBalancingv2Request {
 ...
 public:
  ...
  void SetRevocationContents(RevocationContentsT&& value) {
    m_revocationContentsHasBeenSet = true;
    m_revocationContents = std::forward<RevocationContentsT>(value);
  }
  template <typename RevocationContentsT = Aws::Vector<RevocationContent>>
  AddTrustStoreRevocationsRequest& WithRevocationContents(RevocationContentsT&& value) {
    SetRevocationContents(std::forward<RevocationContentsT>(value));
    return *this;
  }
  ...
  AddTrustStoreRevocationsRequest& AddRevocationContents(RevocationContentsT&& value) {
    m_revocationContentsHasBeenSet = true;
    m_revocationContents.emplace_back(std::forward<RevocationContentsT>(value));
    return *this;
  }
  ///@}
 private:
