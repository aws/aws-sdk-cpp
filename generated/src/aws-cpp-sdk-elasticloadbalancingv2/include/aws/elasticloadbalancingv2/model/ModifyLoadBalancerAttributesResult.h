...
class ModifyLoadBalancerAttributesResult {
 public:
  ...
  AWS_ELASTICLOADBALANCINGV2_API ModifyLoadBalancerAttributesResult& operator=(
      const Aws::AmazonWebServiceResult<Aws::Utils::Xml::XmlDocument>& result);
  ...
  template <typename AttributesT = Aws::Vector<LoadBalancerAttribute>>
  ModifyLoadBalancerAttributesResult& WithAttributes(AttributesT&& value) {
    SetAttributes(std::forward<AttributesT>(value));
    return *this;
  }
  template <typename AttributesT = LoadBalancerAttribute>
  ModifyLoadBalancerAttributesResult& AddAttributes(AttributesT&& value) {
    m_attributesHasBeenSet = true;
