...
class ModifyRuleResult {
 public:
  AWS_ELASTICLOADBALANCINGV2_API ModifyRuleResult() = default;
  ...
  template <typename RulesT = Aws::Vector<Rule>>
  ModifyRuleResult& WithRules(RulesT&& value) {
    SetRules(std::forward<RulesT>(value));
    return *this;
  }
  template <typename RulesT = Rule>
  ModifyRuleResult& AddRules(RulesT&& value) {
    m_rulesHasBeenSet = true;
  ...
  template <typename ResponseMetadataT = ResponseMetadata>
  ModifyRuleResult& WithResponseMetadata(ResponseMetadataT&& value) {
