...
class PutPartnerEventsResult {
 public:
  ...
  inline PutPartnerEventsResult& WithFailedEntryCount(int value) {
  ...
  template <typename EntriesT = Aws::Vector<PutPartnerEventsResultEntry>>
  PutPartnerEventsResult& WithEntries(EntriesT&& value) {
    SetEntries(std::forward<EntriesT>(value));
    return *this;
  }
  template <typename EntriesT = PutPartnerEventsResultEntry>
  PutPartnerEventsResult& AddEntries(EntriesT&& value) {
    m_entriesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  PutPartnerEventsResult& WithRequestId(RequestIdT&& value) {
