...
class ListVersionsByFunctionResult {
 public:
  ...
  template <typename NextMarkerT = Aws::String>
  ListVersionsByFunctionResult& WithNextMarker(NextMarkerT&& value) {
  ...
  template <typename VersionsT = Aws::Vector<FunctionConfiguration>>
  ListVersionsByFunctionResult& WithVersions(VersionsT&& value) {
    SetVersions(std::forward<VersionsT>(value));
    return *this;
  }
  template <typename VersionsT = FunctionConfiguration>
  ListVersionsByFunctionResult& AddVersions(VersionsT&& value) {
    m_versionsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListVersionsByFunctionResult& WithRequestId(RequestIdT&& value) {
