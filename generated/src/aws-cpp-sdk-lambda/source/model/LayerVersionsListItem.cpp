...
LayerVersionsListItem::LayerVersionsListItem(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue LayerVersionsListItem::Jsonize() const {
  ...
  if (m_compatibleRuntimesHasBeenSet) {
    ...
    for (unsigned compatibleRuntimesIndex = 0; compatibleRuntimesIndex < compatibleRuntimesJsonList.GetLength();
         ++compatibleRuntimesIndex) {
      compatibleRuntimesJsonList[compatibleRuntimesIndex].AsString(
          RuntimeMapper::GetNameForRuntime(m_compatibleRuntimes[compatibleRuntimesIndex]));
    }
