...
KafkaSchemaRegistryConfig::KafkaSchemaRegistryConfig(JsonView jsonValue) { *this = jsonValue; }
...
KafkaSchemaRegistryConfig& KafkaSchemaRegistryConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("EventRecordFormat")) {
    m_eventRecordFormat =
        SchemaRegistryEventRecordFormatMapper::GetSchemaRegistryEventRecordFormatForName(jsonValue.GetString("EventRecordFormat"));
  ...
  if (jsonValue.ValueExists("SchemaValidationConfigs")) {
    ...
    for (unsigned schemaValidationConfigsIndex = 0; schemaValidationConfigsIndex < schemaValidationConfigsJsonList.GetLength();
         ++schemaValidationConfigsIndex) {
...
JsonValue KafkaSchemaRegistryConfig::Jsonize() const {
  ...
  if (m_eventRecordFormatHasBeenSet) {
    payload.WithString("EventRecordFormat",
                       SchemaRegistryEventRecordFormatMapper::GetNameForSchemaRegistryEventRecordFormat(m_eventRecordFormat));
  }
  ...
  if (m_schemaValidationConfigsHasBeenSet) {
    ...
    for (unsigned schemaValidationConfigsIndex = 0; schemaValidationConfigsIndex < schemaValidationConfigsJsonList.GetLength();
         ++schemaValidationConfigsIndex) {
      schemaValidationConfigsJsonList[schemaValidationConfigsIndex].AsObject(
          m_schemaValidationConfigs[schemaValidationConfigsIndex].Jsonize());
    }
