...
/**
 */
class DescribeProxyRuleGroupRequest : public NetworkFirewallRequest {
 public:
  AWS_NETWORKFIREWALL_API DescribeProxyRuleGroupRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DescribeProxyRuleGroup"; }
  ...
  template <typename ProxyRuleGroupNameT = Aws::String>
  DescribeProxyRuleGroupRequest& WithProxyRuleGroupName(ProxyRuleGroupNameT&& value) {
  ...
  template <typename ProxyRuleGroupArnT = Aws::String>
  DescribeProxyRuleGroupRequest& WithProxyRuleGroupArn(ProxyRuleGroupArnT&& value) {
