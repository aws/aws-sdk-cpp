...
class TCPFlagField {
 public:
  ...
  template <typename FlagsT = Aws::Vector<TCPFlag>>
  TCPFlagField& WithFlags(FlagsT&& value) {
    SetFlags(std::forward<FlagsT>(value));
    return *this;
  }
  inline TCPFlagField& AddFlags(TCPFlag value) {
    m_flagsHasBeenSet = true;
  ...
  void SetMasks(MasksT&& value) {
    m_masksHasBeenSet = true;
    m_masks = std::forward<MasksT>(value);
  }
  template <typename MasksT = Aws::Vector<TCPFlag>>
  TCPFlagField& WithMasks(MasksT&& value) {
    SetMasks(std::forward<MasksT>(value));
    return *this;
  }
  inline TCPFlagField& AddMasks(TCPFlag value) {
    m_masksHasBeenSet = true;
    m_masks.push_back(value);
    return *this;
  }
  ///@}
 private:
