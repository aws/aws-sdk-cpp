...
class FlowOperation {
 public:
  ...
  inline FlowOperation& WithMinimumFlowAgeInSeconds(int value) {
  ...
  template <typename FlowFiltersT = Aws::Vector<FlowFilter>>
  FlowOperation& WithFlowFilters(FlowFiltersT&& value) {
    SetFlowFilters(std::forward<FlowFiltersT>(value));
    return *this;
  }
  template <typename FlowFiltersT = FlowFilter>
  FlowOperation& AddFlowFilters(FlowFiltersT&& value) {
    m_flowFiltersHasBeenSet = true;
