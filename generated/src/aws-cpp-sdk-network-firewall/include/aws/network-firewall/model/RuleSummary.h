...
class RuleSummary {
 public:
  ...
  template <typename SIDT = Aws::String>
  RuleSummary& WithSID(SIDT&& value) {
  ...
  void SetMsg(MsgT&& value) {
    m_msgHasBeenSet = true;
    m_msg = std::forward<MsgT>(value);
  }
  template <typename MsgT = Aws::String>
  RuleSummary& WithMsg(MsgT&& value) {
    SetMsg(std::forward<MsgT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename MetadataT = Aws::String>
  RuleSummary& WithMetadata(MetadataT&& value) {
