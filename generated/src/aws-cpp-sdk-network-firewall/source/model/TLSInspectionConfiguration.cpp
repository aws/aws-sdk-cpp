...
TLSInspectionConfiguration::TLSInspectionConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
TLSInspectionConfiguration& TLSInspectionConfiguration::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("ServerCertificateConfigurations")) {
    ...
    for (unsigned serverCertificateConfigurationsIndex = 0;
         serverCertificateConfigurationsIndex < serverCertificateConfigurationsJsonList.GetLength();
         ++serverCertificateConfigurationsIndex) {
...
JsonValue TLSInspectionConfiguration::Jsonize() const {
  ...
  if (m_serverCertificateConfigurationsHasBeenSet) {
    ...
    for (unsigned serverCertificateConfigurationsIndex = 0;
         serverCertificateConfigurationsIndex < serverCertificateConfigurationsJsonList.GetLength();
         ++serverCertificateConfigurationsIndex) {
      serverCertificateConfigurationsJsonList[serverCertificateConfigurationsIndex].AsObject(
          m_serverCertificateConfigurations[serverCertificateConfigurationsIndex].Jsonize());
    }
