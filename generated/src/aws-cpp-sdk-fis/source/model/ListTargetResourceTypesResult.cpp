...
ListTargetResourceTypesResult::ListTargetResourceTypesResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListTargetResourceTypesResult& ListTargetResourceTypesResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("targetResourceTypes")) {
    ...
    for (unsigned targetResourceTypesIndex = 0; targetResourceTypesIndex < targetResourceTypesJsonList.GetLength();
         ++targetResourceTypesIndex) {
      m_targetResourceTypes.push_back(targetResourceTypesJsonList[targetResourceTypesIndex].AsObject());
    }
    m_targetResourceTypesHasBeenSet = true;
  }
