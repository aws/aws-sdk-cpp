...
class ExperimentTemplateReportConfigurationDataSourcesInput {
 public:
  ...
  void SetCloudWatchDashboards(CloudWatchDashboardsT&& value) {
    ...
    m_cloudWatchDashboards = std::forward<CloudWatchDashboardsT>(value);
  }
  template <typename CloudWatchDashboardsT = Aws::Vector<ReportConfigurationCloudWatchDashboardInput>>
  ExperimentTemplateReportConfigurationDataSourcesInput& WithCloudWatchDashboards(CloudWatchDashboardsT&& value) {
    SetCloudWatchDashboards(std::forward<CloudWatchDashboardsT>(value));
    return *this;
  }
  template <typename CloudWatchDashboardsT = ReportConfigurationCloudWatchDashboardInput>
  ExperimentTemplateReportConfigurationDataSourcesInput& AddCloudWatchDashboards(CloudWatchDashboardsT&& value) {
    m_cloudWatchDashboardsHasBeenSet = true;
    m_cloudWatchDashboards.emplace_back(std::forward<CloudWatchDashboardsT>(value));
    return *this;
  }
  ///@}
 private:
