...
class EventTriggerLimits {
 public:
  ...
  inline void SetEventExpiration(long long value) {
    m_eventExpirationHasBeenSet = true;
    m_eventExpiration = value;
  }
  inline EventTriggerLimits& WithEventExpiration(long long value) {
    SetEventExpiration(value);
    return *this;
  }
  ///@}
  ...
  void SetPeriods(PeriodsT&& value) {
    ...
    m_periods = std::forward<PeriodsT>(value);
  }
  template <typename PeriodsT = Aws::Vector<Period>>
  EventTriggerLimits& WithPeriods(PeriodsT&& value) {
    SetPeriods(std::forward<PeriodsT>(value));
    return *this;
  }
  template <typename PeriodsT = Period>
  EventTriggerLimits& AddPeriods(PeriodsT&& value) {
    m_periodsHasBeenSet = true;
    m_periods.emplace_back(std::forward<PeriodsT>(value));
    return *this;
  }
  ///@}
 private:
