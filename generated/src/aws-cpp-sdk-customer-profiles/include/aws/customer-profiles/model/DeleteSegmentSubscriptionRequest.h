...
/**
 */
class DeleteSegmentSubscriptionRequest : public CustomerProfilesRequest {
 public:
  AWS_CUSTOMERPROFILES_API DeleteSegmentSubscriptionRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteSegmentSubscription"; }
  ...
  template <typename DomainNameT = Aws::String>
  DeleteSegmentSubscriptionRequest& WithDomainName(DomainNameT&& value) {
  ...
  template <typename SegmentDefinitionNameT = Aws::String>
  DeleteSegmentSubscriptionRequest& WithSegmentDefinitionName(SegmentDefinitionNameT&& value) {
