...
ListOrganizationsResult::ListOrganizationsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListOrganizationsResult& ListOrganizationsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("OrganizationSummaries")) {
    ...
    for (unsigned organizationSummariesIndex = 0; organizationSummariesIndex < organizationSummariesJsonList.GetLength();
         ++organizationSummariesIndex) {
      m_organizationSummaries.push_back(organizationSummariesJsonList[organizationSummariesIndex].AsObject());
    }
    m_organizationSummariesHasBeenSet = true;
  }
