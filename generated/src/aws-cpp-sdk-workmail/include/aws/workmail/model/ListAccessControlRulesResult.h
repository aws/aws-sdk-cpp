...
class ListAccessControlRulesResult {
 public:
  ...
  template <typename RulesT = Aws::Vector<AccessControlRule>>
  ListAccessControlRulesResult& WithRules(RulesT&& value) {
    SetRules(std::forward<RulesT>(value));
    return *this;
  }
  template <typename RulesT = AccessControlRule>
  ListAccessControlRulesResult& AddRules(RulesT&& value) {
    m_rulesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListAccessControlRulesResult& WithRequestId(RequestIdT&& value) {
