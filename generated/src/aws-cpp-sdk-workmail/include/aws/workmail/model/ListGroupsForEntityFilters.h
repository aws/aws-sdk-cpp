...
class ListGroupsForEntityFilters {
 public:
  ...
  void SetGroupNamePrefix(GroupNamePrefixT&& value) {
    m_groupNamePrefixHasBeenSet = true;
    m_groupNamePrefix = std::forward<GroupNamePrefixT>(value);
  }
  template <typename GroupNamePrefixT = Aws::String>
  ListGroupsForEntityFilters& WithGroupNamePrefix(GroupNamePrefixT&& value) {
    SetGroupNamePrefix(std::forward<GroupNamePrefixT>(value));
    return *this;
  }
  ///@}
 private:
