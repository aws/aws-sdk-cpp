...
Aws::String UpdateDecoderManifestRequest::SerializePayload() const {
  ...
  if (m_signalDecodersToAddHasBeenSet) {
    ...
    for (unsigned signalDecodersToAddIndex = 0; signalDecodersToAddIndex < signalDecodersToAddJsonList.GetLength();
         ++signalDecodersToAddIndex) {
  ...
  if (m_signalDecodersToUpdateHasBeenSet) {
    ...
    for (unsigned signalDecodersToUpdateIndex = 0; signalDecodersToUpdateIndex < signalDecodersToUpdateJsonList.GetLength();
         ++signalDecodersToUpdateIndex) {
  ...
  if (m_signalDecodersToRemoveHasBeenSet) {
    ...
    for (unsigned signalDecodersToRemoveIndex = 0; signalDecodersToRemoveIndex < signalDecodersToRemoveJsonList.GetLength();
         ++signalDecodersToRemoveIndex) {
  ...
  if (m_networkInterfacesToAddHasBeenSet) {
    ...
    for (unsigned networkInterfacesToAddIndex = 0; networkInterfacesToAddIndex < networkInterfacesToAddJsonList.GetLength();
         ++networkInterfacesToAddIndex) {
  ...
  if (m_networkInterfacesToUpdateHasBeenSet) {
    ...
    for (unsigned networkInterfacesToUpdateIndex = 0; networkInterfacesToUpdateIndex < networkInterfacesToUpdateJsonList.GetLength();
         ++networkInterfacesToUpdateIndex) {
      networkInterfacesToUpdateJsonList[networkInterfacesToUpdateIndex].AsObject(
          m_networkInterfacesToUpdate[networkInterfacesToUpdateIndex].Jsonize());
    }
  ...
  if (m_networkInterfacesToRemoveHasBeenSet) {
    ...
    for (unsigned networkInterfacesToRemoveIndex = 0; networkInterfacesToRemoveIndex < networkInterfacesToRemoveJsonList.GetLength();
         ++networkInterfacesToRemoveIndex) {
      networkInterfacesToRemoveJsonList[networkInterfacesToRemoveIndex].AsString(
          m_networkInterfacesToRemove[networkInterfacesToRemoveIndex]);
    }
  ...
  if (m_defaultForUnmappedSignalsHasBeenSet) {
    payload.WithString("defaultForUnmappedSignals",
                       DefaultForUnmappedSignalsTypeMapper::GetNameForDefaultForUnmappedSignalsType(m_defaultForUnmappedSignals));
  }
