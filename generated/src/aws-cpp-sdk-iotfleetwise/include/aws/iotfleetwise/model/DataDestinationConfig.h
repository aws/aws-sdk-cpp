...
class DataDestinationConfig {
 public:
  ...
  void SetTimestreamConfig(TimestreamConfigT&& value) {
    ...
    m_timestreamConfig = std::forward<TimestreamConfigT>(value);
  }
  template <typename TimestreamConfigT = TimestreamConfig>
  DataDestinationConfig& WithTimestreamConfig(TimestreamConfigT&& value) {
    SetTimestreamConfig(std::forward<TimestreamConfigT>(value));
    return *this;
  }
  ///@}
  ...
  void SetMqttTopicConfig(MqttTopicConfigT&& value) {
    ...
    m_mqttTopicConfig = std::forward<MqttTopicConfigT>(value);
  }
  template <typename MqttTopicConfigT = MqttTopicConfig>
  DataDestinationConfig& WithMqttTopicConfig(MqttTopicConfigT&& value) {
    SetMqttTopicConfig(std::forward<MqttTopicConfigT>(value));
    return *this;
  }
  ///@}
 private:
