...
class StateTemplateUpdateStrategy {
 public:
  ...
  void SetPeriodic(PeriodicT&& value) {
    ...
    m_periodic = std::forward<PeriodicT>(value);
  }
  template <typename PeriodicT = PeriodicStateTemplateUpdateStrategy>
  StateTemplateUpdateStrategy& WithPeriodic(PeriodicT&& value) {
    SetPeriodic(std::forward<PeriodicT>(value));
    return *this;
  }
  ///@}
  ...
  void SetOnChange(OnChangeT&& value) {
    ...
    m_onChange = std::forward<OnChangeT>(value);
  }
  template <typename OnChangeT = OnChangeStateTemplateUpdateStrategy>
  StateTemplateUpdateStrategy& WithOnChange(OnChangeT&& value) {
    SetOnChange(std::forward<OnChangeT>(value));
    return *this;
  }
  ///@}
 private:
