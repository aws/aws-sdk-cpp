...
class SignalFetchConfig {
 public:
  ...
  void SetTimeBased(TimeBasedT&& value) {
    ...
    m_timeBased = std::forward<TimeBasedT>(value);
  }
  template <typename TimeBasedT = TimeBasedSignalFetchConfig>
  SignalFetchConfig& WithTimeBased(TimeBasedT&& value) {
    SetTimeBased(std::forward<TimeBasedT>(value));
    return *this;
  }
  ///@}
  ...
  void SetConditionBased(ConditionBasedT&& value) {
    ...
    m_conditionBased = std::forward<ConditionBasedT>(value);
  }
  template <typename ConditionBasedT = ConditionBasedSignalFetchConfig>
  SignalFetchConfig& WithConditionBased(ConditionBasedT&& value) {
    SetConditionBased(std::forward<ConditionBasedT>(value));
    return *this;
  }
  ///@}
 private:
