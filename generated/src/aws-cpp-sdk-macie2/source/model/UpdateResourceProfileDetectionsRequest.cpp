...
Aws::String UpdateResourceProfileDetectionsRequest::SerializePayload() const {
  ...
  if (m_suppressDataIdentifiersHasBeenSet) {
    ...
    for (unsigned suppressDataIdentifiersIndex = 0; suppressDataIdentifiersIndex < suppressDataIdentifiersJsonList.GetLength();
         ++suppressDataIdentifiersIndex) {
      suppressDataIdentifiersJsonList[suppressDataIdentifiersIndex].AsObject(
          m_suppressDataIdentifiers[suppressDataIdentifiersIndex].Jsonize());
    }
