...
ReplicationDetails::ReplicationDetails(JsonView jsonValue) { *this = jsonValue; }
...
ReplicationDetails& ReplicationDetails::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("replicationAccounts")) {
    ...
    for (unsigned replicationAccountsIndex = 0; replicationAccountsIndex < replicationAccountsJsonList.GetLength();
         ++replicationAccountsIndex) {
