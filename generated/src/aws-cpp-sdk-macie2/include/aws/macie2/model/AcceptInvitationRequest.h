...
class AcceptInvitationRequest : public Macie2Request {
 public:
  ...
  void SetMasterAccount(MasterAccountT&& value) {
    m_masterAccountHasBeenSet = true;
    m_masterAccount = std::forward<MasterAccountT>(value);
  }
  template <typename MasterAccountT = Aws::String>
  AcceptInvitationRequest& WithMasterAccount(MasterAccountT&& value) {
    SetMasterAccount(std::forward<MasterAccountT>(value));
    return *this;
  }
  ///@}
 private:
