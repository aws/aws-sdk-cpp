...
namespace Model
{
  ...
  public:
     ...
     * discovery result isn't available, is malformed or corrupted, or uses an
     * unsupported storage format. Macie can't verify the location of the sensitive
     * data to retrieve.</p></li> <li><p>INVALID_RESULT_SIGNATURE - The corresponding
     ...
     * data to retrieve.</p></li> <li><p>MEMBER_ROLE_TOO_PERMISSIVE - The affected
     * member account is configured to retrieve occurrences of sensitive data by using
     * an IAM role whose trust or permissions policy doesn't meet Macie requirements
     * for restricting access to the role. Or the role's trust policy doesn't specify
     * the correct external ID. Macie can't assume the role to retrieve the sensitive
     * data.</p></li> <li><p>MISSING_GET_MEMBER_PERMISSION - You're not allowed to
     * retrieve information about the association between your account and the affected
     * account. Macie can't determine whether you’re allowed to access the affected S3
     * object as the delegated Macie administrator for the affected account.</p></li>
     ...
     * available. The object was renamed, moved, or deleted. Or the object was changed
     * after Macie created the finding.</p></li> <li><p>RESULT_NOT_SIGNED - The
     * corresponding sensitive data discovery result is stored in an S3 object that
     * hasn't been signed. Macie can't verify the integrity and authenticity of the
     * sensitive data discovery result. Therefore, Macie can't verify the location of
     * the sensitive data to retrieve.</p></li> <li><p>ROLE_TOO_PERMISSIVE - Your
     * account is configured to retrieve occurrences of sensitive data by using an IAM
     * role whose trust or permissions policy doesn't meet Macie requirements for
     * restricting access to the role. Macie can’t assume the role to retrieve the
     * sensitive data.</p></li> <li><p>UNSUPPORTED_FINDING_TYPE - The specified finding
     * isn't a sensitive data finding.</p></li> <li><p>UNSUPPORTED_OBJECT_TYPE - The
     * affected S3 object uses a file or storage format that Macie doesn't support for
     * retrieving occurrences of sensitive data.</p></li></ul> <p>This value is null if
     * sensitive data can be retrieved for the finding.</p>
     */
