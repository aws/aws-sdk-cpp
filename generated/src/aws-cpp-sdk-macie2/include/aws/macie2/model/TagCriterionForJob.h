...
class TagCriterionForJob {
 public:
  ...
  inline TagCriterionForJob& WithComparator(JobComparator value) {
    SetComparator(value);
    return *this;
  }
  ///@}
  ...
  template <typename TagValuesT = Aws::Vector<TagCriterionPairForJob>>
  TagCriterionForJob& WithTagValues(TagValuesT&& value) {
    SetTagValues(std::forward<TagValuesT>(value));
    return *this;
  }
  template <typename TagValuesT = TagCriterionPairForJob>
  TagCriterionForJob& AddTagValues(TagValuesT&& value) {
    m_tagValuesHasBeenSet = true;
