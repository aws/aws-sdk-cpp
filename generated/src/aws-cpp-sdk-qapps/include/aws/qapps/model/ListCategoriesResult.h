...
class ListCategoriesResult {
 public:
  ...
  template <typename CategoriesT = Aws::Vector<Category>>
  ListCategoriesResult& WithCategories(CategoriesT&& value) {
    SetCategories(std::forward<CategoriesT>(value));
    return *this;
  }
  template <typename CategoriesT = Category>
  ListCategoriesResult& AddCategories(CategoriesT&& value) {
    m_categoriesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListCategoriesResult& WithRequestId(RequestIdT&& value) {
