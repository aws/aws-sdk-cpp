...
GetQAppResult::GetQAppResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
GetQAppResult& GetQAppResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("requiredCapabilities")) {
    ...
    for (unsigned requiredCapabilitiesIndex = 0; requiredCapabilitiesIndex < requiredCapabilitiesJsonList.GetLength();
         ++requiredCapabilitiesIndex) {
      m_requiredCapabilities.push_back(
          AppRequiredCapabilityMapper::GetAppRequiredCapabilityForName(requiredCapabilitiesJsonList[requiredCapabilitiesIndex].AsString()));
    }
