...
namespace QApps {
template <>
AWS_QAPPS_API ConflictException QAppsError::GetModeledError() {
  assert(this->GetErrorType() == QAppsErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_QAPPS_API ServiceQuotaExceededException QAppsError::GetModeledError() {
  assert(this->GetErrorType() == QAppsErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_QAPPS_API ThrottlingException QAppsError::GetModeledError() {
  assert(this->GetErrorType() == QAppsErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_QAPPS_API ResourceNotFoundException QAppsError::GetModeledError() {
  assert(this->GetErrorType() == QAppsErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_QAPPS_API InternalServerException QAppsError::GetModeledError() {
  assert(this->GetErrorType() == QAppsErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_QAPPS_API ContentTooLargeException QAppsError::GetModeledError() {
