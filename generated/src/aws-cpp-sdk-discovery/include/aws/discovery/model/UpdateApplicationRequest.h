...
class UpdateApplicationRequest : public ApplicationDiscoveryServiceRequest {
 public:
  ...
  template <typename ConfigurationIdT = Aws::String>
  UpdateApplicationRequest& WithConfigurationId(ConfigurationIdT&& value) {
  ...
  void SetWave(WaveT&& value) {
    m_waveHasBeenSet = true;
    m_wave = std::forward<WaveT>(value);
  }
  template <typename WaveT = Aws::String>
  UpdateApplicationRequest& WithWave(WaveT&& value) {
    SetWave(std::forward<WaveT>(value));
    return *this;
  }
  ///@}
 private:
