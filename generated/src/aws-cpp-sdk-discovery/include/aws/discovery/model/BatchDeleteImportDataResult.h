...
class BatchDeleteImportDataResult {
 public:
  ...
  AWS_APPLICATIONDISCOVERYSERVICE_API BatchDeleteImportDataResult& operator=(
  ...
  template <typename ErrorsT = Aws::Vector<BatchDeleteImportDataError>>
  BatchDeleteImportDataResult& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = BatchDeleteImportDataError>
  BatchDeleteImportDataResult& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchDeleteImportDataResult& WithRequestId(RequestIdT&& value) {
