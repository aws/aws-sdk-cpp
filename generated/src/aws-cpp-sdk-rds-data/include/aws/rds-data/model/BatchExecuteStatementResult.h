...
class BatchExecuteStatementResult {
 public:
  ...
  void SetUpdateResults(UpdateResultsT&& value) {
    m_updateResultsHasBeenSet = true;
    m_updateResults = std::forward<UpdateResultsT>(value);
  }
  template <typename UpdateResultsT = Aws::Vector<UpdateResult>>
  BatchExecuteStatementResult& WithUpdateResults(UpdateResultsT&& value) {
    SetUpdateResults(std::forward<UpdateResultsT>(value));
    return *this;
  }
  template <typename UpdateResultsT = UpdateResult>
  BatchExecuteStatementResult& AddUpdateResults(UpdateResultsT&& value) {
    m_updateResultsHasBeenSet = true;
    m_updateResults.emplace_back(std::forward<UpdateResultsT>(value));
    return *this;
  }
  ///@}
