...
ListSystemVersionsResult::ListSystemVersionsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }

ListSystemVersionsResult& ListSystemVersionsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("systemVersions")) {
    ...
    for (unsigned systemVersionsIndex = 0; systemVersionsIndex < systemVersionsJsonList.GetLength(); ++systemVersionsIndex) {
      m_systemVersions.push_back(systemVersionsJsonList[systemVersionsIndex].AsObject());
    }
