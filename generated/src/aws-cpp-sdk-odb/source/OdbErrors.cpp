...
namespace odb {
template <>
AWS_ODB_API ConflictException OdbError::GetModeledError() {
  assert(this->GetErrorType() == OdbErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_ODB_API ThrottlingException OdbError::GetModeledError() {
  assert(this->GetErrorType() == OdbErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_ODB_API ServiceQuotaExceededException OdbError::GetModeledError() {
  assert(this->GetErrorType() == OdbErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_ODB_API InternalServerException OdbError::GetModeledError() {
  assert(this->GetErrorType() == OdbErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_ODB_API ResourceNotFoundException OdbError::GetModeledError() {
  assert(this->GetErrorType() == OdbErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_ODB_API ValidationException OdbError::GetModeledError() {
