...
class GetDbServerResult {
 public:
  ...
  void SetDbServer(DbServerT&& value) {
    m_dbServerHasBeenSet = true;
    m_dbServer = std::forward<DbServerT>(value);
  }
  template <typename DbServerT = DbServer>
  GetDbServerResult& WithDbServer(DbServerT&& value) {
    SetDbServer(std::forward<DbServerT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetDbServerResult& WithRequestId(RequestIdT&& value) {
