...
namespace Model
{
  ...
  /**
   */
  class DeleteOdbPeeringConnectionRequest : public OdbRequest
  {
  public:
    AWS_ODB_API DeleteOdbPeeringConnectionRequest() = default;

    // Service request name is the Operation name which will send this request out,
    // each operation should has unique request name, so that we can get operation's name from this request.
    // Note: this is not true for response, multiple operations may have the same response name,
    // so we can not get operation's name from response.
    inline virtual const char* GetServiceRequestName() const override { return "DeleteOdbPeeringConnection"; }
    ...
    template<typename OdbPeeringConnectionIdT = Aws::String>
    DeleteOdbPeeringConnectionRequest& WithOdbPeeringConnectionId(OdbPeeringConnectionIdT&& value) { SetOdbPeeringConnectionId(std::forward<OdbPeeringConnectionIdT>(value)); return *this;}
    ///@}
  private:
