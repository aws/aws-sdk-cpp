...
class GetOdbNetworkResult {
 public:
  ...
  void SetOdbNetwork(OdbNetworkT&& value) {
    m_odbNetworkHasBeenSet = true;
    m_odbNetwork = std::forward<OdbNetworkT>(value);
  }
  template <typename OdbNetworkT = OdbNetwork>
  GetOdbNetworkResult& WithOdbNetwork(OdbNetworkT&& value) {
    SetOdbNetwork(std::forward<OdbNetworkT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetOdbNetworkResult& WithRequestId(RequestIdT&& value) {
