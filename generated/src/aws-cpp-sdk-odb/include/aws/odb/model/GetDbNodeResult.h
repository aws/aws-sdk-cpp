...
class GetDbNodeResult {
 public:
  ...
  void SetDbNode(DbNodeT&& value) {
    m_dbNodeHasBeenSet = true;
    m_dbNode = std::forward<DbNodeT>(value);
  }
  template <typename DbNodeT = DbNode>
  GetDbNodeResult& WithDbNode(DbNodeT&& value) {
    SetDbNode(std::forward<DbNodeT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetDbNodeResult& WithRequestId(RequestIdT&& value) {
