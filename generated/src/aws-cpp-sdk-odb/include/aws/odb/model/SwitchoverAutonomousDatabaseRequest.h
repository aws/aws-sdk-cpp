...
/**
 */
class SwitchoverAutonomousDatabaseRequest : public OdbRequest {
 public:
  AWS_ODB_API SwitchoverAutonomousDatabaseRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "SwitchoverAutonomousDatabase"; }
  ...
  template <typename AutonomousDatabaseIdT = Aws::String>
  SwitchoverAutonomousDatabaseRequest& WithAutonomousDatabaseId(AutonomousDatabaseIdT&& value) {
  ...
  ///@{
  /**
   * <p>The Amazon Resource Name (ARN) of the peer Autonomous Database to switch over
   * to.</p>
   */
  ...
  template <typename PeerDbArnT = Aws::String>
  SwitchoverAutonomousDatabaseRequest& WithPeerDbArn(PeerDbArnT&& value) {
