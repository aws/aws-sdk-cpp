...
/**
 */
class GetAutonomousDatabaseWalletDetailsRequest : public OdbRequest {
 public:
  AWS_ODB_API GetAutonomousDatabaseWalletDetailsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetAutonomousDatabaseWalletDetails"; }
  ...
  ///@{
  /**
   * <p>The unique identifier of the Autonomous Database to retrieve wallet details
   * for.</p>
   */
  ...
  template <typename AutonomousDatabaseIdT = Aws::String>
  GetAutonomousDatabaseWalletDetailsRequest& WithAutonomousDatabaseId(AutonomousDatabaseIdT&& value) {
