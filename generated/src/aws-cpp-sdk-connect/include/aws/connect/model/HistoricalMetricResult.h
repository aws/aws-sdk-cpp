...
class HistoricalMetricResult {
 public:
  ...
  template <typename DimensionsT = Dimensions>
  HistoricalMetricResult& WithDimensions(DimensionsT&& value) {
  ...
  template <typename CollectionsT = Aws::Vector<HistoricalMetricData>>
  HistoricalMetricResult& WithCollections(CollectionsT&& value) {
    SetCollections(std::forward<CollectionsT>(value));
    return *this;
  }
  template <typename CollectionsT = HistoricalMetricData>
  HistoricalMetricResult& AddCollections(CollectionsT&& value) {
    m_collectionsHasBeenSet = true;
