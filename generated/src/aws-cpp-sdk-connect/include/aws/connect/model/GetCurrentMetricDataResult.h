...
class GetCurrentMetricDataResult {
 public:
  ...
  void SetDataSnapshotTime(DataSnapshotTimeT&& value) {
    m_dataSnapshotTimeHasBeenSet = true;
    m_dataSnapshotTime = std::forward<DataSnapshotTimeT>(value);
  }
  template <typename DataSnapshotTimeT = Aws::Utils::DateTime>
  GetCurrentMetricDataResult& WithDataSnapshotTime(DataSnapshotTimeT&& value) {
    SetDataSnapshotTime(std::forward<DataSnapshotTimeT>(value));
    return *this;
  }
  ///@}
  ...
  inline GetCurrentMetricDataResult& WithApproximateTotalCount(long long value) {
