...
class RealTimeContactAnalysisIssueDetected {
 public:
  ...
  void SetTranscriptItems(TranscriptItemsT&& value) {
    m_transcriptItemsHasBeenSet = true;
    m_transcriptItems = std::forward<TranscriptItemsT>(value);
  }
  template <typename TranscriptItemsT = Aws::Vector<RealTimeContactAnalysisTranscriptItemWithContent>>
  RealTimeContactAnalysisIssueDetected& WithTranscriptItems(TranscriptItemsT&& value) {
    SetTranscriptItems(std::forward<TranscriptItemsT>(value));
    return *this;
  }
  template <typename TranscriptItemsT = RealTimeContactAnalysisTranscriptItemWithContent>
  RealTimeContactAnalysisIssueDetected& AddTranscriptItems(TranscriptItemsT&& value) {
    m_transcriptItemsHasBeenSet = true;
    m_transcriptItems.emplace_back(std::forward<TranscriptItemsT>(value));
    return *this;
  }
  ///@}
 private:
