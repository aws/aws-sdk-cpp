...
class InboundEmailContent {
 public:
  ...
  inline void SetMessageSourceType(InboundMessageSourceType value) {
    m_messageSourceTypeHasBeenSet = true;
    m_messageSourceType = value;
  }
  inline InboundEmailContent& WithMessageSourceType(InboundMessageSourceType value) {
    SetMessageSourceType(value);
    return *this;
  }
  ///@}
  ...
  void SetRawMessage(RawMessageT&& value) {
    ...
    m_rawMessage = std::forward<RawMessageT>(value);
  }
  template <typename RawMessageT = InboundRawMessage>
  InboundEmailContent& WithRawMessage(RawMessageT&& value) {
    SetRawMessage(std::forward<RawMessageT>(value));
    return *this;
  }
  ///@}
 private:
