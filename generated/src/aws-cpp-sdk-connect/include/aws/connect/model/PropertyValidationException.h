...
class PropertyValidationException {
 public:
  ...
  void SetPropertyList(PropertyListT&& value) {
    ...
    m_propertyList = std::forward<PropertyListT>(value);
  }
  template <typename PropertyListT = Aws::Vector<PropertyValidationExceptionProperty>>
  PropertyValidationException& WithPropertyList(PropertyListT&& value) {
    SetPropertyList(std::forward<PropertyListT>(value));
    return *this;
  }
  ...
  PropertyValidationException& AddPropertyList(PropertyListT&& value) {
    m_propertyListHasBeenSet = true;
    m_propertyList.emplace_back(std::forward<PropertyListT>(value));
    return *this;
  }
  ///@}
 private:
