...
class GetMetricDataV2Request : public ConnectRequest {
 public:
  ...
  ///@{
  /**
   * <p>Filtering is an operation that selects records that match a set of specified
   * criteria. By narrowing the dataset before aggregation, filters ensure that only
   * relevant records are included in the computation.</p> <p> <b>Filter keys</b>
   * </p> <p>The following are valid filter keys for a <code>GetMetricDataV2</code>
   * request:</p> <p> <code>AGENT</code> | <code>AGENT_HIERARCHY_LEVEL_ONE</code> |
   ...
   * <code>EVALUATION_FORM</code>, <code>EVALUATOR_ID</code> </p> <p>You can use up
   * to 5 filter keys in a single request, and up to 100 filter values across all
   * filter keys.</p>  <p> <b>Filter values</b> </p> <ul> <li> <p>VOICE, CHAT,
   * TASK, and EMAIL are valid filter values for the <code>CHANNEL</code> filter key.
   * They do not count towards the limit of 100 filter values. For example, a
   * <code>GetMetricDataV2</code> request can filter by 50 queues, 35 agents, and 15
   * routing profiles for a total of 100 filter values, along with 4 channel
   * filters.</p> </li> <li> <p> <code>contact_lens_conversational_analytics</code>
   * is a valid filter value for the <code>FEATURE</code> filter key. It is available
   * only for contacts analyzed by Contact Lens conversational analytics.</p> </li>
   * <li> <p> <code>connect:Chat</code>, <code>connect:SMS</code>,
   * <code>connect:Telephony</code>, and <code>connect:WebRTC</code> are valid filter
   ...
   * <li> <p>FALSE includes all contacts that did not have Connect AI Agents enabled
   * as part of the flow.</p> </li> </ul> </li> <li> <p>
   * <code>EXPERIENCE_VALIDATION</code> and <code>FLOW_VALIDATION</code> are the only
   * valid filter values for the
   * <code>contact/segmentAttributes/connect:ValidationTestType</code> filter key.
   * This filter is available only for contact record-driven metrics.</p> </li> <li>
   * <p> <a
   ...
   * Handoffs</a> </p> </dd> <dt>AI_AGENT_INVOCATION_SUCCESS</dt> <dd> <p>Unit:
   ...
   * Agent Invocations</a> </p>  <p>AI Agent Name Version is not a valid filter
   * but a valid grouping.</p>  </dd> <dt>AI_RESPONSE_COMPLETION_RATE</dt>
   * <dd> <p>Unit: Percent</p> <p>Valid groupings and filters: AI Use Case, Channel,
   ...
   * and filters: AI Use Case, Channel, Queue, Routing Profile</p> <p>UI name: <a
   ...
   * valid filter but a valid grouping.</p>  </dd>
   * <dt>KNOWLEDGE_CONTENT_REFERENCES</dt> <dd> <p>Unit: Count</p> <p>Valid groupings
   * and filters: AI Agent, AI Agent Name, AI Agent Type, AI Use Case, Channel,
   * Knowledge Base Name, Queue, Routing Profile</p> <p>UI name: <a
   ...
   * <p>Unit: Percent</p> <p>Valid groupings and filters: AI Use Case, Channel,
   * Queue, Routing Profile</p> <p>UI name: <a
   ...
   * Count</p> <p>Valid groupings and filters: AI Use Case, Channel, Queue, Routing
   * Profile</p> <p>UI name: <a
