...
class ControlPlaneTagFilter {
 public:
  ...
  template <typename OrConditionsT = Aws::Vector<Aws::Vector<TagCondition>>>
  ControlPlaneTagFilter& WithOrConditions(OrConditionsT&& value) {
    SetOrConditions(std::forward<OrConditionsT>(value));
    return *this;
  }
  template <typename OrConditionsT = Aws::Vector<TagCondition>>
  ControlPlaneTagFilter& AddOrConditions(OrConditionsT&& value) {
    m_orConditionsHasBeenSet = true;
  ...
  template <typename AndConditionsT = Aws::Vector<TagCondition>>
  ControlPlaneTagFilter& WithAndConditions(AndConditionsT&& value) {
    SetAndConditions(std::forward<AndConditionsT>(value));
    return *this;
  }
  template <typename AndConditionsT = TagCondition>
  ControlPlaneTagFilter& AddAndConditions(AndConditionsT&& value) {
    m_andConditionsHasBeenSet = true;
  ...
  template <typename TagConditionT = TagCondition>
  ControlPlaneTagFilter& WithTagCondition(TagConditionT&& value) {
