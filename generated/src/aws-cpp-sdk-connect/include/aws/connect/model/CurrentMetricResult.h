...
class CurrentMetricResult {
 public:
  ...
  template <typename DimensionsT = Dimensions>
  CurrentMetricResult& WithDimensions(DimensionsT&& value) {
  ...
  void SetCollections(CollectionsT&& value) {
    ...
    m_collections = std::forward<CollectionsT>(value);
  }
  template <typename CollectionsT = Aws::Vector<CurrentMetricData>>
  CurrentMetricResult& WithCollections(CollectionsT&& value) {
    SetCollections(std::forward<CollectionsT>(value));
    return *this;
  }
  template <typename CollectionsT = CurrentMetricData>
  CurrentMetricResult& AddCollections(CollectionsT&& value) {
    m_collectionsHasBeenSet = true;
    m_collections.emplace_back(std::forward<CollectionsT>(value));
    return *this;
  }
  ///@}
 private:
