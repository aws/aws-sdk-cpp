...
/**
 */
class DescribeMetricRequest : public ConnectRequest {
 public:
  AWS_CONNECT_API DescribeMetricRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DescribeMetric"; }
  ...
  template <typename InstanceIdT = Aws::String>
  DescribeMetricRequest& WithInstanceId(InstanceIdT&& value) {
  ...
  ///@{
  /**
   * <p>The identifier of the metric to describe. Adding the <code>$SAVED</code>
   * qualifier will describe the saved version of the metric. Adding
   * <code>$LATEST</code> or omitting a qualifier will describe the published
   * version.</p>
   */
  ...
  template <typename MetricIdT = Aws::String>
  DescribeMetricRequest& WithMetricId(MetricIdT&& value) {
