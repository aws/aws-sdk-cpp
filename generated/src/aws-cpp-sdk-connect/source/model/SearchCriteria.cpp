...
SearchCriteria& SearchCriteria::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("InitiationMethods")) {
    ...
    for (unsigned initiationMethodsIndex = 0; initiationMethodsIndex < initiationMethodsJsonList.GetLength(); ++initiationMethodsIndex) {
      m_initiationMethods.push_back(
          ContactInitiationMethodMapper::GetContactInitiationMethodForName(initiationMethodsJsonList[initiationMethodsIndex].AsString()));
    }
...
JsonValue SearchCriteria::Jsonize() const {
  ...
  if (m_initiationMethodsHasBeenSet) {
    ...
    for (unsigned initiationMethodsIndex = 0; initiationMethodsIndex < initiationMethodsJsonList.GetLength(); ++initiationMethodsIndex) {
      initiationMethodsJsonList[initiationMethodsIndex].AsString(
          ContactInitiationMethodMapper::GetNameForContactInitiationMethod(m_initiationMethods[initiationMethodsIndex]));
    }
