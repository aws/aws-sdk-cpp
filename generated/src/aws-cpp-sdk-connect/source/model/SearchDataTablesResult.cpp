...
SearchDataTablesResult& SearchDataTablesResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("DataTables")) {
    ...
    for (unsigned dataTablesIndex = 0; dataTablesIndex < dataTablesJsonList.GetLength(); ++dataTablesIndex) {
      m_dataTables.push_back(dataTablesJsonList[dataTablesIndex].AsObject());
    }
