...
SearchContactEvaluationsResult& SearchContactEvaluationsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("EvaluationSearchSummaryList")) {
    ...
    for (unsigned evaluationSearchSummaryListIndex = 0; evaluationSearchSummaryListIndex < evaluationSearchSummaryListJsonList.GetLength();
         ++evaluationSearchSummaryListIndex) {
      m_evaluationSearchSummaryList.push_back(evaluationSearchSummaryListJsonList[evaluationSearchSummaryListIndex].AsObject());
    }
