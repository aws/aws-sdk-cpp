...
ListViewVersionsResult::ListViewVersionsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListViewVersionsResult& ListViewVersionsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("ViewVersionSummaryList")) {
    ...
    for (unsigned viewVersionSummaryListIndex = 0; viewVersionSummaryListIndex < viewVersionSummaryListJsonList.GetLength();
         ++viewVersionSummaryListIndex) {
      m_viewVersionSummaryList.push_back(viewVersionSummaryListJsonList[viewVersionSummaryListIndex].AsObject());
    }
    m_viewVersionSummaryListHasBeenSet = true;
  }
