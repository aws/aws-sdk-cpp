...
RoutingProfile::RoutingProfile(JsonView jsonValue) { *this = jsonValue; }
...
RoutingProfile& RoutingProfile::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AgentAvailabilityTimer")) {
    m_agentAvailabilityTimer =
        AgentAvailabilityTimerMapper::GetAgentAvailabilityTimerForName(jsonValue.GetString("AgentAvailabilityTimer"));
  ...
  if (jsonValue.ValueExists("AssociatedQueueIds")) {
    ...
    for (unsigned associatedQueueIdsIndex = 0; associatedQueueIdsIndex < associatedQueueIdsJsonList.GetLength();
         ++associatedQueueIdsIndex) {
  ...
  if (jsonValue.ValueExists("AssociatedManualAssignmentQueueIds")) {
    ...
    for (unsigned associatedManualAssignmentQueueIdsIndex = 0;
         associatedManualAssignmentQueueIdsIndex < associatedManualAssignmentQueueIdsJsonList.GetLength();
         ++associatedManualAssignmentQueueIdsIndex) {
      m_associatedManualAssignmentQueueIds.push_back(
          associatedManualAssignmentQueueIdsJsonList[associatedManualAssignmentQueueIdsIndex].AsString());
    }
...
JsonValue RoutingProfile::Jsonize() const {
  ...
  if (m_associatedManualAssignmentQueueIdsHasBeenSet) {
    ...
    for (unsigned associatedManualAssignmentQueueIdsIndex = 0;
         ...
         ++associatedManualAssignmentQueueIdsIndex) {
      associatedManualAssignmentQueueIdsJsonList[associatedManualAssignmentQueueIdsIndex].AsString(
          m_associatedManualAssignmentQueueIds[associatedManualAssignmentQueueIdsIndex]);
    }
