...
ViewSearchCriteria::ViewSearchCriteria(JsonView jsonValue) { *this = jsonValue; }

ViewSearchCriteria& ViewSearchCriteria::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ViewTypeCondition")) {
    m_viewTypeCondition = ViewTypeMapper::GetViewTypeForName(jsonValue.GetString("ViewTypeCondition"));
    m_viewTypeConditionHasBeenSet = true;
  }
  if (jsonValue.ValueExists("ViewStatusCondition")) {
    m_viewStatusCondition = ViewStatusMapper::GetViewStatusForName(jsonValue.GetString("ViewStatusCondition"));
    m_viewStatusConditionHasBeenSet = true;
  }
  return *this;
}

JsonValue ViewSearchCriteria::Jsonize() const {
  JsonValue payload;
  ...
  if (m_viewTypeConditionHasBeenSet) {
    payload.WithString("ViewTypeCondition", ViewTypeMapper::GetNameForViewType(m_viewTypeCondition));
  }

  if (m_viewStatusConditionHasBeenSet) {
    payload.WithString("ViewStatusCondition", ViewStatusMapper::GetNameForViewStatus(m_viewStatusCondition));
  }
