...
TestCaseSearchCriteria::TestCaseSearchCriteria(JsonView jsonValue) { *this = jsonValue; }

TestCaseSearchCriteria& TestCaseSearchCriteria::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("StatusCondition")) {
    m_statusCondition = TestCaseStatusMapper::GetTestCaseStatusForName(jsonValue.GetString("StatusCondition"));
    m_statusConditionHasBeenSet = true;
  }
  return *this;
}

JsonValue TestCaseSearchCriteria::Jsonize() const {
  JsonValue payload;
  ...
  if (m_statusConditionHasBeenSet) {
    payload.WithString("StatusCondition", TestCaseStatusMapper::GetNameForTestCaseStatus(m_statusCondition));
  }
