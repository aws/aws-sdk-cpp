...
SearchEvaluationFormsResult& SearchEvaluationFormsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("EvaluationFormSearchSummaryList")) {
    ...
    for (unsigned evaluationFormSearchSummaryListIndex = 0;
         evaluationFormSearchSummaryListIndex < evaluationFormSearchSummaryListJsonList.GetLength();
         ++evaluationFormSearchSummaryListIndex) {
      m_evaluationFormSearchSummaryList.push_back(evaluationFormSearchSummaryListJsonList[evaluationFormSearchSummaryListIndex].AsObject());
    }
