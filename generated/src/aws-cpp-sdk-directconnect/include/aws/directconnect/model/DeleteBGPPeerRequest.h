...
class DeleteBGPPeerRequest : public DirectConnectRequest {
 public:
  ...
  template <typename VirtualInterfaceIdT = Aws::String>
  DeleteBGPPeerRequest& WithVirtualInterfaceId(VirtualInterfaceIdT&& value) {
  ...
  inline DeleteBGPPeerRequest& WithAsn(int value) {
    SetAsn(value);
    return *this;
  }
  ///@}
   ...
   * <p>Note the following limitations when using <code>asnLong</code>:</p> <ul> <li>
   ...
   * </p> </li> <li> <p> <code>asnLong</code> accepts any valid ASN value, regardless
   * if it's 2-byte or 4-byte. </p> </li> <li> <p>When using a 4-byte
   * <code>asnLong</code>, the API response returns <code>0</code> for the legacy
   * <code>asn</code> attribute since 4-byte ASN values exceed the maximum supported
   * value of 2,147,483,647.</p> </li> <li> <p>If you are using a 2-byte ASN, the API
   * response will include the 2-byte value for both the <code>asn</code> and
   * <code>asnLong</code> fields.</p> </li> <li> <p>If you provide a value in the
   * same API call for both <code>asn</code> and <code>asnLong</code>, the API will
   * only accept the value for <code>asnLong</code>.</p> </li> </ul>
   */
  ...
  inline DeleteBGPPeerRequest& WithAsnLong(long long value) {
    SetAsnLong(value);
    return *this;
  }
  ///@}
  ...
  template <typename CustomerAddressT = Aws::String>
  DeleteBGPPeerRequest& WithCustomerAddress(CustomerAddressT&& value) {
  ...
  template <typename BgpPeerIdT = Aws::String>
  DeleteBGPPeerRequest& WithBgpPeerId(BgpPeerIdT&& value) {
