...
class DescribeLoaResult {
 public:
  ...
  void SetLoaContent(LoaContentT&& value) {
    ...
    m_loaContent = std::forward<LoaContentT>(value);
  }
  template <typename LoaContentT = Aws::Utils::ByteBuffer>
  DescribeLoaResult& WithLoaContent(LoaContentT&& value) {
    SetLoaContent(std::forward<LoaContentT>(value));
    return *this;
  }
  ///@}
  ...
  inline DescribeLoaResult& WithLoaContentType(LoaContentType value) {
  ...
  template <typename RequestIdT = Aws::String>
  DescribeLoaResult& WithRequestId(RequestIdT&& value) {
