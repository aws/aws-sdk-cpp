...
class DescribeTagsRequest : public DirectConnectRequest {
 public:
  AWS_DIRECTCONNECT_API DescribeTagsRequest() = default;
  ...
  template <typename ResourceArnsT = Aws::Vector<Aws::String>>
  DescribeTagsRequest& WithResourceArns(ResourceArnsT&& value) {
    SetResourceArns(std::forward<ResourceArnsT>(value));
    return *this;
  }
  template <typename ResourceArnsT = Aws::String>
  DescribeTagsRequest& AddResourceArns(ResourceArnsT&& value) {
    m_resourceArnsHasBeenSet = true;
