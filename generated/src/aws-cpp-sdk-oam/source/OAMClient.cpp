...
OAMClient::OAMClient(const AWSCredentials& credentials, const Client::ClientConfiguration& clientConfiguration)
    : BASECLASS(clientConfiguration,
                Aws::MakeShared<AWSAuthV4Signer>(ALLOCATION_TAG, Aws::MakeShared<SimpleAWSCredentialsProvider>(ALLOCATION_TAG, credentials),
                                                 SERVICE_NAME, Aws::Region::ComputeSignerRegion(clientConfiguration.region)),
                Aws::MakeShared<OAMErrorMarshaller>(ALLOCATION_TAG)),
...
void OAMClient::init(const OAM::OAMClientConfiguration& config) {
  ...
  if (!config.endpointOverride.empty()) {
    ...
    Aws::Config::EndpointResolver::EndpointSource("oam", config.profileName, *m_endpointProvider);
  }
}
...
ListAttachedLinksOutcome OAMClient::ListAttachedLinks(const ListAttachedLinksRequest& request) const {
  ...
  return TracingUtils::MakeCallWithTiming<ListAttachedLinksOutcome>(
      [&]() -> ListAttachedLinksOutcome {
        ...
        return ListAttachedLinksOutcome(
            MakeRequest(request, endpointResolutionOutcome.GetResult(), Aws::Http::HttpMethod::HTTP_POST, Aws::Auth::SIGV4_SIGNER));
      },
      TracingUtils::SMITHY_CLIENT_DURATION_METRIC, *meter,
      {{TracingUtils::SMITHY_METHOD_DIMENSION, request.GetServiceRequestName()},
