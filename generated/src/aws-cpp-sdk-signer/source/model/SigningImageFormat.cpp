...
SigningImageFormat::SigningImageFormat(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue SigningImageFormat::Jsonize() const {
  ...
  if (m_supportedFormatsHasBeenSet) {
    ...
    for (unsigned supportedFormatsIndex = 0; supportedFormatsIndex < supportedFormatsJsonList.GetLength(); ++supportedFormatsIndex) {
      supportedFormatsJsonList[supportedFormatsIndex].AsString(
          ImageFormatMapper::GetNameForImageFormat(m_supportedFormats[supportedFormatsIndex]));
    }
