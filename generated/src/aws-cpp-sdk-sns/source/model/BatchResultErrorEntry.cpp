...
BatchResultErrorEntry::BatchResultErrorEntry(const XmlNode& xmlNode) { *this = xmlNode; }
...
BatchResultErrorEntry& BatchResultErrorEntry::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!senderFaultNode.IsNull()) {
      m_senderFault =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(senderFaultNode.GetText()).c_str()).c_str());
