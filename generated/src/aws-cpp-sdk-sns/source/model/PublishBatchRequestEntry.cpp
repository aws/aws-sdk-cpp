...
PublishBatchRequestEntry::PublishBatchRequestEntry(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!messageAttributesNode.IsNull()) {
      ...
      while (!messageAttributesEntry.IsNull()) {
        ...
        m_messageAttributes[keyNode.GetText()] = valueNode;
...
void PublishBatchRequestEntry::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_messageAttributesHasBeenSet) {
    ...
    for (auto& item : m_messageAttributes) {
      oStream << location << index << locationValue << ".MessageAttributes.entry." << messageAttributesIdx
              << ".Name=" << StringUtils::URLEncode(item.first.c_str()) << "&";
...
void PublishBatchRequestEntry::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_messageAttributesHasBeenSet) {
    unsigned messageAttributesIdx = 1;
    for (auto& item : m_messageAttributes) {
      oStream << location << ".MessageAttributes.entry." << messageAttributesIdx << ".Name=" << StringUtils::URLEncode(item.first.c_str())
              << "&";
      Aws::StringStream messageAttributesSs;
