...
class PublishRequest : public SNSRequest {
 ...
 public:
   ...
   * messages must be UTF-8 encoded strings. By default, a message can be at most 256
   * KiB in size (262,144 bytes, not 262,144 characters).</p> <p>When you publish to
   * a topic, the maximum size is determined by the topic's
   * <code>MaximumMessageSize</code> attribute, which supports values up to 1 MiB
   * (1,048,576 bytes). Amazon SNS validates the combined size of the message body
   * and message attributes against this value and returns an
   * <code>InvalidParameter</code> error if the limit is exceeded.</p> <p>For more
   * information, see <a
   ...
   * characters, 140 ASCII characters, or 70 UCS-2 characters.</p> <p>If you publish
   ...
   * but are cut off at whole-word boundaries.</p> <p>The total size limit for a
   * single SMS <code>Publish</code> action is 1,600 characters.</p> </li> </ul>
