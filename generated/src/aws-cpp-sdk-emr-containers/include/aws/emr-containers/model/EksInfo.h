...
class EksInfo {
 public:
  ...
  void SetNodeLabel(NodeLabelT&& value) {
    m_nodeLabelHasBeenSet = true;
    m_nodeLabel = std::forward<NodeLabelT>(value);
  }
  template <typename NodeLabelT = Aws::String>
  EksInfo& WithNodeLabel(NodeLabelT&& value) {
    SetNodeLabel(std::forward<NodeLabelT>(value));
    return *this;
  }
  ///@}
 private:
