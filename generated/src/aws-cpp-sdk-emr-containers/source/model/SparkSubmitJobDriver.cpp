...
SparkSubmitJobDriver::SparkSubmitJobDriver(JsonView jsonValue) { *this = jsonValue; }
...
SparkSubmitJobDriver& SparkSubmitJobDriver::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("entryPointArguments")) {
    ...
    for (unsigned entryPointArgumentsIndex = 0; entryPointArgumentsIndex < entryPointArgumentsJsonList.GetLength();
         ++entryPointArgumentsIndex) {
