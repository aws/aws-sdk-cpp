...
class SearchResult {
 public:
  ...
  template <typename ResourcesT = Aws::Vector<Resource>>
  SearchResult& WithResources(ResourcesT&& value) {
    SetResources(std::forward<ResourcesT>(value));
    return *this;
  }
  template <typename ResourcesT = Resource>
  SearchResult& AddResources(ResourcesT&& value) {
    m_resourcesHasBeenSet = true;
  ...
  template <typename ViewArnT = Aws::String>
  SearchResult& WithViewArn(ViewArnT&& value) {
  ...
  template <typename CountT = ResourceCount>
  SearchResult& WithCount(CountT&& value) {
