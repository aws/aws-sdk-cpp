...
class FunctionExecutionConfig {
 public:
  ...
  inline void SetIsolationMode(FunctionIsolationMode value) {
    m_isolationModeHasBeenSet = true;
    m_isolationMode = value;
  }
  inline FunctionExecutionConfig& WithIsolationMode(FunctionIsolationMode value) {
  ...
  void SetRunAs(RunAsT&& value) {
    ...
    m_runAs = std::forward<RunAsT>(value);
  }
  template <typename RunAsT = FunctionRunAsConfig>
  FunctionExecutionConfig& WithRunAs(RunAsT&& value) {
    SetRunAs(std::forward<RunAsT>(value));
    return *this;
  }
  ///@}
 private:
