...
class ResourceDownloadOwnerSetting {
 public:
  ...
  void SetGroupOwner(GroupOwnerT&& value) {
    ...
    m_groupOwner = std::forward<GroupOwnerT>(value);
  }
  template <typename GroupOwnerT = Aws::String>
  ResourceDownloadOwnerSetting& WithGroupOwner(GroupOwnerT&& value) {
    SetGroupOwner(std::forward<GroupOwnerT>(value));
    return *this;
  }
  ///@}
  ...
  inline void SetGroupPermission(Permission value) {
    m_groupPermissionHasBeenSet = true;
    m_groupPermission = value;
  }
  inline ResourceDownloadOwnerSetting& WithGroupPermission(Permission value) {
    SetGroupPermission(value);
    return *this;
  }
  ///@}
 private:
