...
class Forecast {
 public:
  ...
  void SetPredictions(PredictionsT&& value) {
    m_predictionsHasBeenSet = true;
    m_predictions = std::forward<PredictionsT>(value);
  }
  template <typename PredictionsT = Aws::Map<Aws::String, Aws::Vector<DataPoint>>>
  Forecast& WithPredictions(PredictionsT&& value) {
    SetPredictions(std::forward<PredictionsT>(value));
    return *this;
  }
  ...
  Forecast& AddPredictions(PredictionsKeyT&& key, PredictionsValueT&& value) {
    m_predictionsHasBeenSet = true;
    m_predictions.emplace(std::forward<PredictionsKeyT>(key), std::forward<PredictionsValueT>(value));
    return *this;
  }
  ///@}
 private:
