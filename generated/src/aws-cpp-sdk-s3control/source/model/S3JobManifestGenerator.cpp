...
S3JobManifestGenerator::S3JobManifestGenerator(const XmlNode& xmlNode) { *this = xmlNode; }
...
S3JobManifestGenerator& S3JobManifestGenerator::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!enableManifestOutputNode.IsNull()) {
      m_enableManifestOutput = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(enableManifestOutputNode.GetText()).c_str()).c_str());
