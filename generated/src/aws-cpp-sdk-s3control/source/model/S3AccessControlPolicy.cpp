...
S3AccessControlPolicy::S3AccessControlPolicy(const XmlNode& xmlNode) { *this = xmlNode; }
...
S3AccessControlPolicy& S3AccessControlPolicy::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!cannedAccessControlListNode.IsNull()) {
      m_cannedAccessControlList = S3CannedAccessControlListMapper::GetS3CannedAccessControlListForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(cannedAccessControlListNode.GetText()).c_str()));
