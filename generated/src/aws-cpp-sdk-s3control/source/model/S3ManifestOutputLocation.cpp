...
S3ManifestOutputLocation::S3ManifestOutputLocation(const XmlNode& xmlNode) { *this = xmlNode; }
...
S3ManifestOutputLocation& S3ManifestOutputLocation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!manifestFormatNode.IsNull()) {
      m_manifestFormat = GeneratedManifestFormatMapper::GetGeneratedManifestFormatForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(manifestFormatNode.GetText()).c_str()));
