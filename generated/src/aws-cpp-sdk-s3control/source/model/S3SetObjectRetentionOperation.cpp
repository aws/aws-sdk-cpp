...
S3SetObjectRetentionOperation::S3SetObjectRetentionOperation(const XmlNode& xmlNode) { *this = xmlNode; }
...
S3SetObjectRetentionOperation& S3SetObjectRetentionOperation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!bypassGovernanceRetentionNode.IsNull()) {
      m_bypassGovernanceRetention = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(bypassGovernanceRetentionNode.GetText()).c_str()).c_str());
