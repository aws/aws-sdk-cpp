...
GetAccessPointResult::GetAccessPointResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
GetAccessPointResult& GetAccessPointResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!networkOriginNode.IsNull()) {
      m_networkOrigin = NetworkOriginMapper::GetNetworkOriginForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(networkOriginNode.GetText()).c_str()));
    ...
    if (!endpointsNode.IsNull()) {
      ...
      while (!endpointsEntry.IsNull()) {
        ...
        m_endpoints[keyNode.GetText()] = valueNode.GetText();
