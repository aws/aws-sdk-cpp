...
ObjectLambdaTransformationConfiguration::ObjectLambdaTransformationConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!actionsNode.IsNull()) {
      ...
      while (!actionsMember.IsNull()) {
        m_actions.push_back(ObjectLambdaTransformationConfigurationActionMapper::GetObjectLambdaTransformationConfigurationActionForName(
            StringUtils::Trim(actionsMember.GetText().c_str())));
...
void ObjectLambdaTransformationConfiguration::AddToNode(XmlNode& parentNode) const {
  ...
  if (m_actionsHasBeenSet) {
    ...
    for (const auto& item : m_actions) {
      ...
      actionsNode.SetText(
          ObjectLambdaTransformationConfigurationActionMapper::GetNameForObjectLambdaTransformationConfigurationAction(item));
    }
  }
