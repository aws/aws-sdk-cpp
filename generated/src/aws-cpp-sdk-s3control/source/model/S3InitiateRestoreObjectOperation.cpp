...
S3InitiateRestoreObjectOperation::S3InitiateRestoreObjectOperation(const XmlNode& xmlNode) { *this = xmlNode; }
...
S3InitiateRestoreObjectOperation& S3InitiateRestoreObjectOperation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!expirationInDaysNode.IsNull()) {
      m_expirationInDays = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(expirationInDaysNode.GetText()).c_str()).c_str());
    ...
    if (!glacierJobTierNode.IsNull()) {
      m_glacierJobTier = S3GlacierJobTierMapper::GetS3GlacierJobTierForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(glacierJobTierNode.GetText()).c_str()));
