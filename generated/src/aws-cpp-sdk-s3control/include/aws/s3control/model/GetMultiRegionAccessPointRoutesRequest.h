...
class GetMultiRegionAccessPointRoutesRequest : public S3ControlRequest {
 public:
  ...
  template <typename AccountIdT = Aws::String>
  GetMultiRegionAccessPointRoutesRequest& WithAccountId(AccountIdT&& value) {
  ...
  void SetMrap(MrapT&& value) {
    m_mrapHasBeenSet = true;
    m_mrap = std::forward<MrapT>(value);
  }
  template <typename MrapT = Aws::String>
  GetMultiRegionAccessPointRoutesRequest& WithMrap(MrapT&& value) {
    SetMrap(std::forward<MrapT>(value));
    return *this;
  }
  ///@}
 private:
