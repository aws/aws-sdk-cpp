...
class GetJobTaggingResult {
 public:
  ...
  template <typename TagsT = Aws::Vector<S3Tag>>
  GetJobTaggingResult& WithTags(TagsT&& value) {
    SetTags(std::forward<TagsT>(value));
    return *this;
  }
  template <typename TagsT = S3Tag>
  GetJobTaggingResult& AddTags(TagsT&& value) {
    m_tagsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  GetJobTaggingResult& WithRequestId(RequestIdT&& value) {
  ...
  template <typename HostIdT = Aws::String>
  GetJobTaggingResult& WithHostId(HostIdT&& value) {
