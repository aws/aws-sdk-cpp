...
class WrappedKey {
 public:
  ...
  void SetWrappingKeyArn(WrappingKeyArnT&& value) {
    ...
    m_wrappingKeyArn = std::forward<WrappingKeyArnT>(value);
  }
  template <typename WrappingKeyArnT = Aws::String>
  WrappedKey& WithWrappingKeyArn(WrappingKeyArnT&& value) {
    SetWrappingKeyArn(std::forward<WrappingKeyArnT>(value));
    return *this;
  }
  ///@}
  ...
  inline WrappedKey& WithWrappedKeyMaterialFormat(WrappedKeyMaterialFormat value) {
  ...
  inline WrappedKey& WithKeyCheckValueAlgorithm(KeyCheckValueAlgorithm value) {
