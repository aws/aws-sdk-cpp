    ...
    public:
      ...
      typedef PaymentCryptographyEndpointProvider EndpointProviderType;
        ...
        */
         ...
         * Cryptography supports ANSI X9 TR-34 norm . Asymmetric key exchange methods are
         ...
         * key block.</p> <p>You can also use <code>ExportKey</code> functionality to
         ...
         * export KEK or IPEK using TR-34</b> </p> <p>Using this operation, you can export
         * initial key using TR-34 asymmetric key exchange. You can only export KEK
         * generated within Amazon Web Services Payment Cryptography. In TR-34 terminology,
         * the sending party of the key is called Key Distribution Host (KDH) and the
         * receiving party of the key is called Key Receiving Device (KRD). During key
         * export process, KDH is Amazon Web Services Payment Cryptography which initiates
         * key export and KRD is the user receiving the key.</p> <p>To initiate TR-34 key
         * export, the KRD must obtain an export token by calling
         ...
         * the KEK or IPEK as a TR-34 WrappedKeyBlock. </p> <p> <b>To export WK (Working
         * Key) or IPEK using TR-31</b> </p> <p>Using this operation, you can export
         * working keys or IPEK using TR-31 symmetric key exchange. In TR-31, you must use
         * an initial key such as KEK to encrypt or wrap the key under export. To establish
         * a KEK, you can use <a>CreateKey</a> or <a>ImportKey</a>. </p> <p>Set the
         * following parameters:</p> <ul> <li> <p> <code>ExportAttributes</code>: Specify
         * export attributes in case of IPEK export. This parameter is optional for KEK
         * export.</p> </li> <li> <p> <code>ExportKeyIdentifier</code>: The
         * <code>KeyARN</code> of the KEK or BDK (in case of IPEK) under export.</p> </li>
         * <li> <p> <code>KeyMaterial</code>: Use <code>Tr31KeyBlock</code> parameters.</p>
         * </li> </ul> <p>When this operation is successful, Amazon Web Services Payment
         * Cryptography returns the WK or IPEK as a TR-31 WrappedKeyBlock.</p> <p>
         * <b>Cross-account use:</b> This operation can't be used across different Amazon
         * Web Services accounts.</p> <p> <b>Related operations:</b> </p> <ul> <li> <p>
         * <a>GetParametersForExport</a> </p> </li> <li> <p> <a>ImportKey</a> </p> </li>
         * </ul><p><h3>See Also:</h3>   <a
         ...
         * encoded) to initiate a TR-34 WrappedKeyBlock.</p> <p>The wrapping key
         ...
         * Services Payment Cryptography.</p> <p>The TR-34 norm is intended for exchanging
         * 3DES keys only and keys are imported in a WrappedKeyBlock format. Key attributes
         * (such as KeyUsage, KeyAlgorithm, KeyModesOfUse, Exportability) are contained
         * within the key block. </p> <p>You can also import a <i>root public key
         * certificate</i>, used to sign other public key certificates, or a <i>trusted
         * public key certificate</i> under an already established root public key
         * certificate.</p> <p> <b>To import a public root key certificate</b> </p> <p>You
         * can also import a <i>root public key certificate</i>, used to sign other public
         * key certificates, or a <i>trusted public key certificate</i> under an already
         ...
         * format (base64 encoded) under import.</p> </li> </ul> <p> <b>To import KEK or
         * ZMK using TR-34</b> </p> <p>Using this operation, you can import initial key
         * using TR-34 asymmetric key exchange. In TR-34 terminology, the sending party of
         * the key is called Key Distribution Host (KDH) and the receiving party of the key
         * is called Key Receiving Device (KRD). During the key import process, KDH is the
         * user who initiates the key import and KRD is Amazon Web Services Payment
         * Cryptography who receives the key.</p> <p>To initiate TR-34 key import, the KDH
         * must obtain an import token by calling <a>GetParametersForImport</a>. This
         * operation generates an encryption keypair for the purpose of key import, signs
         * the key and returns back the wrapping key certificate (also known as KRD
         ...
         * KRD wrapping certificate expires after 7 days.</p> <p>Next the KDH generates a
         ...
         * Amazon Web Services Payment Cryptography.</p> </li> </ul> <p> <b>To import WK
         * (Working Key) using TR-31</b> </p> <p>Amazon Web Services Payment Cryptography
         ...
         * import or by using <a>CreateKey</a>. To initiate a TR-31 key import, set the
         * following parameters:</p> <ul> <li> <p> <code>KeyMaterial</code>: Use
         * <code>Tr31KeyBlock</code> parameters.</p> </li> <li> <p>
         * <code>WrappedKeyBlock</code>: The TR-31 wrapped key material. It contains the
         * key under import, encrypted using KEK. The TR-31 key block is typically
         * generated by a HSM outside of Amazon Web Services Payment Cryptography. </p>
         * </li> <li> <p> <code>WrappingKeyIdentifier</code>: The <code>KeyArn</code> of
         * the KEK that Amazon Web Services Payment Cryptography uses to decrypt or unwrap
         * the key under import.</p> </li> </ul> <p> <b>Cross-account use:</b> This
         * operation can't be used across different Amazon Web Services accounts.</p> <p>
         * <b>Related operations:</b> </p> <ul> <li> <p> <a>ExportKey</a> </p> </li> <li>
         * <p> <a>GetParametersForImport</a> </p> </li> </ul><p><h3>See Also:</h3>   <a
