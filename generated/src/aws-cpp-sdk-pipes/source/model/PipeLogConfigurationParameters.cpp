...
PipeLogConfigurationParameters::PipeLogConfigurationParameters(JsonView jsonValue) { *this = jsonValue; }
...
PipeLogConfigurationParameters& PipeLogConfigurationParameters::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("IncludeExecutionData")) {
    ...
    for (unsigned includeExecutionDataIndex = 0; includeExecutionDataIndex < includeExecutionDataJsonList.GetLength();
         ++includeExecutionDataIndex) {
      m_includeExecutionData.push_back(IncludeExecutionDataOptionMapper::GetIncludeExecutionDataOptionForName(
          includeExecutionDataJsonList[includeExecutionDataIndex].AsString()));
    }
...
JsonValue PipeLogConfigurationParameters::Jsonize() const {
  ...
  if (m_includeExecutionDataHasBeenSet) {
    ...
    for (unsigned includeExecutionDataIndex = 0; includeExecutionDataIndex < includeExecutionDataJsonList.GetLength();
         ++includeExecutionDataIndex) {
      includeExecutionDataJsonList[includeExecutionDataIndex].AsString(
          IncludeExecutionDataOptionMapper::GetNameForIncludeExecutionDataOption(m_includeExecutionData[includeExecutionDataIndex]));
    }
