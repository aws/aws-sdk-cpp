...
SpaceSummary::SpaceSummary(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

SpaceSummary& SpaceSummary::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "spaceId") {
              ...
              else if (initialKeyStr == "spaceArn") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_spaceArn = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_status = SpaceStatusMapper::GetSpaceStatusForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("SpaceSummary", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
        ...
        while (decoder->LastError() == AWS_ERROR_UNKNOWN) {
          ...
          if (initialKey.has_value()) {
            ...
            if (initialKeyStr == "spaceId") {
              ...
              if (val.has_value()) {
                m_status =
                    SpaceStatusMapper::GetSpaceStatusForName(Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
              }
              m_statusHasBeenSet = true;
            }
...
void SpaceSummary::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_spaceArnHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("spaceArn"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_spaceArn.c_str()));
  }
  ...
  if (m_statusHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(SpaceStatusMapper::GetNameForSpaceStatus(m_status).c_str()));
  }
