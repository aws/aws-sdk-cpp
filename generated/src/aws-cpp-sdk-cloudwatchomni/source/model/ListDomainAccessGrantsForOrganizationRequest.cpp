/**
 * Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
 * SPDX-License-Identifier: Apache-2.0.
 */

#include <aws/cloudwatchomni/model/ListDomainAccessGrantsForOrganizationRequest.h>
#include <aws/crt/cbor/Cbor.h>

#include <utility>

using namespace Aws::CloudWatchOmni::Model;
using namespace Aws::Crt::Cbor;
using namespace Aws::Utils;

Aws::String ListDomainAccessGrantsForOrganizationRequest::SerializePayload() const {
  Aws::Crt::Cbor::CborEncoder encoder;

  // Calculate map size
  size_t mapSize = 0;
  if (m_domainIdHasBeenSet) {
    mapSize++;
  }
  if (m_principalIdHasBeenSet) {
    mapSize++;
  }
  if (m_principalTypeHasBeenSet) {
    mapSize++;
  }
  if (m_permissionHasBeenSet) {
    mapSize++;
  }
  if (m_nextTokenHasBeenSet) {
    mapSize++;
  }
  if (m_maxResultsHasBeenSet) {
    mapSize++;
  }

  encoder.WriteMapStart(mapSize);

  if (m_domainIdHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("domainId"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_domainId.c_str()));
  }

  if (m_principalIdHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("principalId"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_principalId.c_str()));
  }

  if (m_principalTypeHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("principalType"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(
        OrganizationGrantPrincipalTypeMapper::GetNameForOrganizationGrantPrincipalType(m_principalType).c_str()));
  }

  if (m_permissionHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("permission"));
    encoder.WriteText(
        Aws::Crt::ByteCursorFromCString(OrganizationGrantPermissionMapper::GetNameForOrganizationGrantPermission(m_permission).c_str()));
  }

  if (m_nextTokenHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("nextToken"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_nextToken.c_str()));
  }

  if (m_maxResultsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("maxResults"));
    (m_maxResults >= 0) ? encoder.WriteUInt(m_maxResults) : encoder.WriteNegInt(m_maxResults);
  }
  const auto str = Aws::String(reinterpret_cast<char*>(encoder.GetEncodedData().ptr), encoder.GetEncodedData().len);
  return str;
}

Aws::Http::HeaderValueCollection ListDomainAccessGrantsForOrganizationRequest::GetRequestSpecificHeaders() const {
  Aws::Http::HeaderValueCollection headers;
  headers.emplace(Aws::Http::CONTENT_TYPE_HEADER, Aws::CBOR_CONTENT_TYPE);
  headers.emplace(Aws::Http::SMITHY_PROTOCOL_HEADER, Aws::RPC_V2_CBOR);
  headers.emplace(Aws::Http::ACCEPT_HEADER, Aws::CBOR_CONTENT_TYPE);
  return headers;
}
