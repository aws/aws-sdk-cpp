...
DomainSummary::DomainSummary(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

DomainSummary& DomainSummary::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "identityCenterInstanceArn") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_identityCenterInstanceArn = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
                    ...
                    m_identityCenterInstanceArn = ss.str();
                  }
                }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("DomainSummary", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void DomainSummary::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_identityCenterInstanceArnHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("identityCenterInstanceArn"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_identityCenterInstanceArn.c_str()));
  }
