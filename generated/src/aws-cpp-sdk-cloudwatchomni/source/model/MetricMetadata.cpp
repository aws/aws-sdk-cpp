...
MetricMetadata::MetricMetadata(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

MetricMetadata& MetricMetadata::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "preferredStat") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_preferredStat = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
                    ...
                    m_preferredStat = ss.str();
                  }
                }
                m_preferredStatHasBeenSet = true;
              }

              else if (initialKeyStr == "metricType") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_metricType = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
              ...
              else if (initialKeyStr == "semantics") {
                m_semantics = MetricSemantics(decoder);
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("MetricMetadata", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void MetricMetadata::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_nameHasBeenSet) {
    mapSize++;
  }
  if (m_preferredStatHasBeenSet) {
    mapSize++;
  }
  ...
  if (m_preferredStatHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("preferredStat"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_preferredStat.c_str()));
  }

  if (m_metricTypeHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("metricType"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_metricType.c_str()));
  }
  ...
  if (m_semanticsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("semantics"));
    m_semantics.CborEncode(encoder);
  }
}
