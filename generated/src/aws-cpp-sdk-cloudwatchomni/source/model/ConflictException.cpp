        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "conflictType") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_conflictType = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
...
void ConflictException::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  ...
  if (m_conflictTypeHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("conflictType"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_conflictType.c_str()));
  }
