...
Alert::Alert(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

Alert& Alert::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "name") {
              ...
              else if (initialKeyStr == "alertId") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_alertId = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
              ...
              else if (initialKeyStr == "spaceId") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_spaceId = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
              ...
              else if (initialKeyStr == "profileId") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
              ...
              else if (initialKeyStr == "rule") {
                m_rule = Rule(decoder);
                m_ruleHasBeenSet = true;
              }

              else if (initialKeyStr == "notificationStatus") {
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_notificationStatus = NotificationStatusMapper::GetNotificationStatusForName(
                ...
                m_state = AlertStateInfo(decoder);
                m_stateHasBeenSet = true;
              }

              else if (initialKeyStr == "notificationRules") {
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_notificationRules.push_back(NotificationRule(decoder));
                      }
                    }
              ...
              else if (initialKeyStr == "alertArn") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_alertArn = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("Alert", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void Alert::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_nameHasBeenSet) {
    mapSize++;
  ...
  if (m_notificationStatusHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("notificationStatus"));
    encoder.WriteText(
        Aws::Crt::ByteCursorFromCString(NotificationStatusMapper::GetNameForNotificationStatus(m_notificationStatus).c_str()));
  }

  if (m_stateHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("state"));
    m_state.CborEncode(encoder);
  }
  ...
  if (m_alertArnHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("alertArn"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_alertArn.c_str()));
  }
}
