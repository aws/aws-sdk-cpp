        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "telemetryRule") {
                m_telemetryRule = TelemetryRule(decoder);
...
void Rule::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  ...
  if (m_telemetryRuleHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("telemetryRule"));
    m_telemetryRule.CborEncode(encoder);
  }
}
