...
AccessProfile::AccessProfile(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

AccessProfile& AccessProfile::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "assumeStatus") {
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_assumeStatus = AssumeStatusMapper::GetAssumeStatusForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("AccessProfile", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void AccessProfile::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_assumeStatusHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("assumeStatus"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(AssumeStatusMapper::GetNameForAssumeStatus(m_assumeStatus).c_str()));
  }
