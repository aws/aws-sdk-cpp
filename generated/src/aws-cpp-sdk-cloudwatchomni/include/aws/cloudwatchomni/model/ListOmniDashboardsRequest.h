...
/**
 */
class ListOmniDashboardsRequest : public CloudWatchOmniRequest {
 public:
  AWS_CLOUDWATCHOMNI_API ListOmniDashboardsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListOmniDashboards"; }
  ...
  template <typename SpaceIdT = Aws::String>
  ListOmniDashboardsRequest& WithSpaceId(SpaceIdT&& value) {
  ...
  ///@{
  /**
   * <p>Filter to dashboards whose name starts with this prefix.</p>
   */
  ...
  template <typename NamePrefixT = Aws::String>
  ListOmniDashboardsRequest& WithNamePrefix(NamePrefixT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListOmniDashboardsRequest& WithNextToken(NextTokenT&& value) {
  ...
  ///@{
  /**
   * <p>The maximum number of dashboards to return per page. Defaults to 100. A page
  ...
  inline ListOmniDashboardsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
