...
/**
 */
class GetOmniDashboardRequest : public CloudWatchOmniRequest {
 public:
  AWS_CLOUDWATCHOMNI_API GetOmniDashboardRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetOmniDashboard"; }
  ...
  template <typename SpaceIdT = Aws::String>
  GetOmniDashboardRequest& WithSpaceId(SpaceIdT&& value) {
  ...
  template <typename DashboardIdT = Aws::String>
  GetOmniDashboardRequest& WithDashboardId(DashboardIdT&& value) {
