...
CustomRoutingEndpointGroup::CustomRoutingEndpointGroup(JsonView jsonValue) { *this = jsonValue; }
...
CustomRoutingEndpointGroup& CustomRoutingEndpointGroup::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("DestinationDescriptions")) {
    ...
    for (unsigned destinationDescriptionsIndex = 0; destinationDescriptionsIndex < destinationDescriptionsJsonList.GetLength();
         ++destinationDescriptionsIndex) {
...
JsonValue CustomRoutingEndpointGroup::Jsonize() const {
  ...
  if (m_destinationDescriptionsHasBeenSet) {
    ...
    for (unsigned destinationDescriptionsIndex = 0; destinationDescriptionsIndex < destinationDescriptionsJsonList.GetLength();
         ++destinationDescriptionsIndex) {
      destinationDescriptionsJsonList[destinationDescriptionsIndex].AsObject(
          m_destinationDescriptions[destinationDescriptionsIndex].Jsonize());
    }
