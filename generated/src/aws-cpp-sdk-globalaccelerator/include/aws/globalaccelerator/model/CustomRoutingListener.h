...
class CustomRoutingListener {
 public:
  ...
  template <typename ListenerArnT = Aws::String>
  CustomRoutingListener& WithListenerArn(ListenerArnT&& value) {
  ...
  template <typename PortRangesT = Aws::Vector<PortRange>>
  CustomRoutingListener& WithPortRanges(PortRangesT&& value) {
    SetPortRanges(std::forward<PortRangesT>(value));
    return *this;
  }
  template <typename PortRangesT = PortRange>
  CustomRoutingListener& AddPortRanges(PortRangesT&& value) {
    m_portRangesHasBeenSet = true;
