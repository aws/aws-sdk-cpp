 ...
 * restore accidentally deleted EBS volumes, EBS snapshots, and EBS-backed AMIs.
 * When using Recycle Bin, if your resources are deleted, they are retained in the
 * Recycle Bin for a time period that you specify.</p> <p>You can restore a
 * resource from the Recycle Bin at any time before its retention period expires.
 * After you restore a resource from the Recycle Bin, the resource is removed from
 * the Recycle Bin, and you can then use it in the same way you use any other
 * resource of that type in your account. If the retention period expires and the
 * resource is not restored, the resource is permanently deleted from the Recycle
...
class AWS_RECYCLEBIN_API RecycleBinClient : public Aws::Client::AWSJsonClient,
 ...
 public:
   ...
   */
  RecycleBinClient(
      const Aws::Auth::AWSCredentials& credentials, std::shared_ptr<RecycleBinEndpointProviderBase> endpointProvider = nullptr,
      const Aws::RecycleBin::RecycleBinClientConfiguration& clientConfiguration = Aws::RecycleBin::RecycleBinClientConfiguration());
  ...
  /**
   * An Async wrapper for LockRule that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename LockRuleRequestT = Model::LockRuleRequest>
  void LockRuleAsync(const LockRuleRequestT& request, const LockRuleResponseReceivedHandler& handler,
                     const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * An Async wrapper for UnlockRule that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename UnlockRuleRequestT = Model::UnlockRuleRequest>
  void UnlockRuleAsync(const UnlockRuleRequestT& request, const UnlockRuleResponseReceivedHandler& handler,
                       const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
