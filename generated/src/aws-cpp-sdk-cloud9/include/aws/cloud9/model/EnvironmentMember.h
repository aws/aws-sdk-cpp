...
class EnvironmentMember {
 public:
  ...
  inline void SetPermissions(Permissions value) {
    m_permissionsHasBeenSet = true;
    m_permissions = value;
  }
  inline EnvironmentMember& WithPermissions(Permissions value) {
  ...
  void SetLastAccess(LastAccessT&& value) {
    ...
    m_lastAccess = std::forward<LastAccessT>(value);
  }
  template <typename LastAccessT = Aws::Utils::DateTime>
  EnvironmentMember& WithLastAccess(LastAccessT&& value) {
    SetLastAccess(std::forward<LastAccessT>(value));
    return *this;
  }
  ///@}
 private:
