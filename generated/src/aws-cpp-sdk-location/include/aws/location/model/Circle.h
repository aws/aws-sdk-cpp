...
class Circle {
 public:
  ...
  void SetCenter(CenterT&& value) {
    ...
    m_center = std::forward<CenterT>(value);
  }
  template <typename CenterT = Aws::Vector<double>>
  Circle& WithCenter(CenterT&& value) {
    SetCenter(std::forward<CenterT>(value));
    return *this;
  }
  inline Circle& AddCenter(double value) {
    m_centerHasBeenSet = true;
    m_center.push_back(value);
    return *this;
  }
  ///@}
  ...
  inline Circle& WithRadius(double value) {
    SetRadius(value);
    return *this;
  }
  ///@}
