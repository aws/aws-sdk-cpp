...
class ListPlaceIndexesResult {
 public:
  ...
  template <typename EntriesT = Aws::Vector<ListPlaceIndexesResponseEntry>>
  ListPlaceIndexesResult& WithEntries(EntriesT&& value) {
    SetEntries(std::forward<EntriesT>(value));
    return *this;
  }
  template <typename EntriesT = ListPlaceIndexesResponseEntry>
  ListPlaceIndexesResult& AddEntries(EntriesT&& value) {
    m_entriesHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListPlaceIndexesResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListPlaceIndexesResult& WithRequestId(RequestIdT&& value) {
