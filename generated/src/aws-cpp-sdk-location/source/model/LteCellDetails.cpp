...
LteCellDetails::LteCellDetails(JsonView jsonValue) { *this = jsonValue; }
...
LteCellDetails& LteCellDetails::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("NetworkMeasurements")) {
    ...
    for (unsigned networkMeasurementsIndex = 0; networkMeasurementsIndex < networkMeasurementsJsonList.GetLength();
         ++networkMeasurementsIndex) {
