...
namespace Model
{
  ...
  public:
    ...
    template<typename NodesT = Aws::Vector<Node>>
    DependencyGraph& WithNodes(NodesT&& value) { SetNodes(std::forward<NodesT>(value)); return *this;}
    template<typename NodesT = Node>
    DependencyGraph& AddNodes(NodesT&& value) { m_nodesHasBeenSet = true; m_nodes.emplace_back(std::forward<NodesT>(value)); return *this; }
    ///@}
    ...
    template<typename EdgesT = Aws::Vector<Edge>>
    DependencyGraph& WithEdges(EdgesT&& value) { SetEdges(std::forward<EdgesT>(value)); return *this;}
    template<typename EdgesT = Edge>
    DependencyGraph& AddEdges(EdgesT&& value) { m_edgesHasBeenSet = true; m_edges.emplace_back(std::forward<EdgesT>(value)); return *this; }
    ///@}
  private:
