...
class BatchGetServiceLevelObjectiveBudgetReportRequest : public ApplicationSignalsRequest {
 public:
  ...
  template <typename TimestampT = Aws::Utils::DateTime>
  BatchGetServiceLevelObjectiveBudgetReportRequest& WithTimestamp(TimestampT&& value) {
  ...
  template <typename SloIdsT = Aws::Vector<Aws::String>>
  BatchGetServiceLevelObjectiveBudgetReportRequest& WithSloIds(SloIdsT&& value) {
    SetSloIds(std::forward<SloIdsT>(value));
    return *this;
  }
  template <typename SloIdsT = Aws::String>
  BatchGetServiceLevelObjectiveBudgetReportRequest& AddSloIds(SloIdsT&& value) {
    m_sloIdsHasBeenSet = true;
