...
AccountDetails::AccountDetails(JsonView jsonValue) { *this = jsonValue; }
...
AccountDetails& AccountDetails::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AdditionalContactEmailAddresses")) {
    ...
    for (unsigned additionalContactEmailAddressesIndex = 0;
         additionalContactEmailAddressesIndex < additionalContactEmailAddressesJsonList.GetLength();
         ++additionalContactEmailAddressesIndex) {
...
JsonValue AccountDetails::Jsonize() const {
  ...
  if (m_additionalContactEmailAddressesHasBeenSet) {
    ...
    for (unsigned additionalContactEmailAddressesIndex = 0;
         additionalContactEmailAddressesIndex < additionalContactEmailAddressesJsonList.GetLength();
         ++additionalContactEmailAddressesIndex) {
      additionalContactEmailAddressesJsonList[additionalContactEmailAddressesIndex].AsString(
          m_additionalContactEmailAddresses[additionalContactEmailAddressesIndex]);
    }
