...
MessageInsightsFilters::MessageInsightsFilters(JsonView jsonValue) { *this = jsonValue; }
...
MessageInsightsFilters& MessageInsightsFilters::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("LastDeliveryEvent")) {
    ...
    for (unsigned lastDeliveryEventIndex = 0; lastDeliveryEventIndex < lastDeliveryEventJsonList.GetLength(); ++lastDeliveryEventIndex) {
      m_lastDeliveryEvent.push_back(
          DeliveryEventTypeMapper::GetDeliveryEventTypeForName(lastDeliveryEventJsonList[lastDeliveryEventIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("LastEngagementEvent")) {
    ...
    for (unsigned lastEngagementEventIndex = 0; lastEngagementEventIndex < lastEngagementEventJsonList.GetLength();
         ++lastEngagementEventIndex) {
      m_lastEngagementEvent.push_back(
          EngagementEventTypeMapper::GetEngagementEventTypeForName(lastEngagementEventJsonList[lastEngagementEventIndex].AsString()));
    }
...
JsonValue MessageInsightsFilters::Jsonize() const {
  ...
  if (m_lastDeliveryEventHasBeenSet) {
    ...
    for (unsigned lastDeliveryEventIndex = 0; lastDeliveryEventIndex < lastDeliveryEventJsonList.GetLength(); ++lastDeliveryEventIndex) {
      lastDeliveryEventJsonList[lastDeliveryEventIndex].AsString(
          DeliveryEventTypeMapper::GetNameForDeliveryEventType(m_lastDeliveryEvent[lastDeliveryEventIndex]));
    }
  ...
  if (m_lastEngagementEventHasBeenSet) {
    ...
    for (unsigned lastEngagementEventIndex = 0; lastEngagementEventIndex < lastEngagementEventJsonList.GetLength();
         ++lastEngagementEventIndex) {
      lastEngagementEventJsonList[lastEngagementEventIndex].AsString(
          EngagementEventTypeMapper::GetNameForEngagementEventType(m_lastEngagementEvent[lastEngagementEventIndex]));
    }
