...
BulkEmailEntry::BulkEmailEntry(JsonView jsonValue) { *this = jsonValue; }
...
BulkEmailEntry& BulkEmailEntry::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ReplacementHeaders")) {
    ...
    for (unsigned replacementHeadersIndex = 0; replacementHeadersIndex < replacementHeadersJsonList.GetLength();
         ++replacementHeadersIndex) {
