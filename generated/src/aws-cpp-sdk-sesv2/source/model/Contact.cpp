...
Contact& Contact::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("TopicDefaultPreferences")) {
    ...
    for (unsigned topicDefaultPreferencesIndex = 0; topicDefaultPreferencesIndex < topicDefaultPreferencesJsonList.GetLength();
         ++topicDefaultPreferencesIndex) {
...
JsonValue Contact::Jsonize() const {
  ...
  if (m_topicDefaultPreferencesHasBeenSet) {
    ...
    for (unsigned topicDefaultPreferencesIndex = 0; topicDefaultPreferencesIndex < topicDefaultPreferencesJsonList.GetLength();
         ++topicDefaultPreferencesIndex) {
      topicDefaultPreferencesJsonList[topicDefaultPreferencesIndex].AsObject(
          m_topicDefaultPreferences[topicDefaultPreferencesIndex].Jsonize());
    }
