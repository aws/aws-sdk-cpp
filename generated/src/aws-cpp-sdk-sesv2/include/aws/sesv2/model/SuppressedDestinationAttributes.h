...
class SuppressedDestinationAttributes {
 public:
  ...
  template <typename MessageIdT = Aws::String>
  SuppressedDestinationAttributes& WithMessageId(MessageIdT&& value) {
  ...
  void SetFeedbackId(FeedbackIdT&& value) {
    m_feedbackIdHasBeenSet = true;
    m_feedbackId = std::forward<FeedbackIdT>(value);
  }
  template <typename FeedbackIdT = Aws::String>
  SuppressedDestinationAttributes& WithFeedbackId(FeedbackIdT&& value) {
    SetFeedbackId(std::forward<FeedbackIdT>(value));
    return *this;
  }
  ///@}
 private:
