...
Aws::String RebootWorkspacesRequest::SerializePayload() const {
  ...
  if (m_rebootWorkspaceRequestsHasBeenSet) {
    ...
    for (unsigned rebootWorkspaceRequestsIndex = 0; rebootWorkspaceRequestsIndex < rebootWorkspaceRequestsJsonList.GetLength();
         ++rebootWorkspaceRequestsIndex) {
      rebootWorkspaceRequestsJsonList[rebootWorkspaceRequestsIndex].AsObject(
          m_rebootWorkspaceRequests[rebootWorkspaceRequestsIndex].Jsonize());
    }
