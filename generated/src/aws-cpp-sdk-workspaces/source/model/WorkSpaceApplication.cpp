...
WorkSpaceApplication::WorkSpaceApplication(JsonView jsonValue) { *this = jsonValue; }
...
WorkSpaceApplication& WorkSpaceApplication::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("SupportedComputeTypeNames")) {
    ...
    for (unsigned supportedComputeTypeNamesIndex = 0; supportedComputeTypeNamesIndex < supportedComputeTypeNamesJsonList.GetLength();
         ++supportedComputeTypeNamesIndex) {
      m_supportedComputeTypeNames.push_back(
          ComputeMapper::GetComputeForName(supportedComputeTypeNamesJsonList[supportedComputeTypeNamesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("SupportedOperatingSystemNames")) {
    ...
    for (unsigned supportedOperatingSystemNamesIndex = 0;
         supportedOperatingSystemNamesIndex < supportedOperatingSystemNamesJsonList.GetLength(); ++supportedOperatingSystemNamesIndex) {
      m_supportedOperatingSystemNames.push_back(OperatingSystemNameMapper::GetOperatingSystemNameForName(
          supportedOperatingSystemNamesJsonList[supportedOperatingSystemNamesIndex].AsString()));
    }
...
JsonValue WorkSpaceApplication::Jsonize() const {
  ...
  if (m_supportedComputeTypeNamesHasBeenSet) {
    ...
    for (unsigned supportedComputeTypeNamesIndex = 0; supportedComputeTypeNamesIndex < supportedComputeTypeNamesJsonList.GetLength();
         ++supportedComputeTypeNamesIndex) {
      supportedComputeTypeNamesJsonList[supportedComputeTypeNamesIndex].AsString(
          ComputeMapper::GetNameForCompute(m_supportedComputeTypeNames[supportedComputeTypeNamesIndex]));
    }
  ...
  if (m_supportedOperatingSystemNamesHasBeenSet) {
    ...
    for (unsigned supportedOperatingSystemNamesIndex = 0;
         supportedOperatingSystemNamesIndex < supportedOperatingSystemNamesJsonList.GetLength(); ++supportedOperatingSystemNamesIndex) {
      supportedOperatingSystemNamesJsonList[supportedOperatingSystemNamesIndex].AsString(
          OperatingSystemNameMapper::GetNameForOperatingSystemName(m_supportedOperatingSystemNames[supportedOperatingSystemNamesIndex]));
    }
