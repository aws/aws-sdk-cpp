...
class RebuildWorkspacesRequest : public WorkSpacesRequest {
 public:
  ...
  void SetRebuildWorkspaceRequests(RebuildWorkspaceRequestsT&& value) {
    m_rebuildWorkspaceRequestsHasBeenSet = true;
    m_rebuildWorkspaceRequests = std::forward<RebuildWorkspaceRequestsT>(value);
  }
  template <typename RebuildWorkspaceRequestsT = Aws::Vector<RebuildRequest>>
  RebuildWorkspacesRequest& WithRebuildWorkspaceRequests(RebuildWorkspaceRequestsT&& value) {
    SetRebuildWorkspaceRequests(std::forward<RebuildWorkspaceRequestsT>(value));
    return *this;
  }
  template <typename RebuildWorkspaceRequestsT = RebuildRequest>
  RebuildWorkspacesRequest& AddRebuildWorkspaceRequests(RebuildWorkspaceRequestsT&& value) {
    m_rebuildWorkspaceRequestsHasBeenSet = true;
    m_rebuildWorkspaceRequests.emplace_back(std::forward<RebuildWorkspaceRequestsT>(value));
    return *this;
  }
  ///@}
 private:
