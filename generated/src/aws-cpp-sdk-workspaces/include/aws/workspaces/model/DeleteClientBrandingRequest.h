...
class DeleteClientBrandingRequest : public WorkSpacesRequest {
 public:
  ...
  template <typename ResourceIdT = Aws::String>
  DeleteClientBrandingRequest& WithResourceId(ResourceIdT&& value) {
  ...
  template <typename PlatformsT = Aws::Vector<ClientDeviceType>>
  DeleteClientBrandingRequest& WithPlatforms(PlatformsT&& value) {
    SetPlatforms(std::forward<PlatformsT>(value));
    return *this;
  }
  inline DeleteClientBrandingRequest& AddPlatforms(ClientDeviceType value) {
    m_platformsHasBeenSet = true;
    m_platforms.push_back(value);
    return *this;
  }
  ///@}
 private:
