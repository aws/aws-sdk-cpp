...
/**
 */
class ListAcmeAccountsRequest : public ACMRequest {
 public:
  AWS_ACM_API ListAcmeAccountsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListAcmeAccounts"; }
  ...
  template <typename NextTokenT = Aws::String>
  ListAcmeAccountsRequest& WithNextToken(NextTokenT&& value) {
  ...
  inline ListAcmeAccountsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  template <typename AcmeEndpointArnT = Aws::String>
  ListAcmeAccountsRequest& WithAcmeEndpointArn(AcmeEndpointArnT&& value) {
