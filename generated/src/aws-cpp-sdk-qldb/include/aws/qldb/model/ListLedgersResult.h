...
class ListLedgersResult {
 public:
  ...
  void SetLedgers(LedgersT&& value) {
    m_ledgersHasBeenSet = true;
    m_ledgers = std::forward<LedgersT>(value);
  }
  template <typename LedgersT = Aws::Vector<LedgerSummary>>
  ListLedgersResult& WithLedgers(LedgersT&& value) {
    SetLedgers(std::forward<LedgersT>(value));
    return *this;
  }
  template <typename LedgersT = LedgerSummary>
  ListLedgersResult& AddLedgers(LedgersT&& value) {
    m_ledgersHasBeenSet = true;
    m_ledgers.emplace_back(std::forward<LedgersT>(value));
    return *this;
  }
  ///@}
