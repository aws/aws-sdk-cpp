...
ListFHIRImportJobsResult::ListFHIRImportJobsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListFHIRImportJobsResult& ListFHIRImportJobsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("ImportJobPropertiesList")) {
    ...
    for (unsigned importJobPropertiesListIndex = 0; importJobPropertiesListIndex < importJobPropertiesListJsonList.GetLength();
         ++importJobPropertiesListIndex) {
      m_importJobPropertiesList.push_back(importJobPropertiesListJsonList[importJobPropertiesListIndex].AsObject());
    }
    m_importJobPropertiesListHasBeenSet = true;
  }
