...
ListTemplateStepsResult::ListTemplateStepsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListTemplateStepsResult& ListTemplateStepsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("templateStepSummaryList")) {
    ...
    for (unsigned templateStepSummaryListIndex = 0; templateStepSummaryListIndex < templateStepSummaryListJsonList.GetLength();
         ++templateStepSummaryListIndex) {
      m_templateStepSummaryList.push_back(templateStepSummaryListJsonList[templateStepSummaryListIndex].AsObject());
    }
    m_templateStepSummaryListHasBeenSet = true;
  }
