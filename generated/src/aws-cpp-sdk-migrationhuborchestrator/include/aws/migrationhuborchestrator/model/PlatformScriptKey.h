...
class PlatformScriptKey {
 public:
  ...
  void SetLinux(LinuxT&& value) {
    m_linuxHasBeenSet = true;
    m_linux = std::forward<LinuxT>(value);
  }
  template <typename LinuxT = Aws::String>
  PlatformScriptKey& WithLinux(LinuxT&& value) {
    SetLinux(std::forward<LinuxT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename WindowsT = Aws::String>
  PlatformScriptKey& WithWindows(WindowsT&& value) {
