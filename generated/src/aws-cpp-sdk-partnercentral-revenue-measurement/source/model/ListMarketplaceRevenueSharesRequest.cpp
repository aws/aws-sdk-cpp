...
Aws::String ListMarketplaceRevenueSharesRequest::SerializePayload() const {
  ...
  if (m_productIdsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("ProductIds"));
    encoder.WriteArrayStart(m_productIds.size());
    for (const auto& item_0 : m_productIds) {
      encoder.WriteText(Aws::Crt::ByteCursorFromCString(item_0.c_str()));
    }
  }

  if (m_productCodesHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("ProductCodes"));
    encoder.WriteArrayStart(m_productCodes.size());
    for (const auto& item_0 : m_productCodes) {
      encoder.WriteText(Aws::Crt::ByteCursorFromCString(item_0.c_str()));
    }
  }
  ...
  if (m_sortByHasBeenSet) {
    ...
    encoder.WriteText(
        Aws::Crt::ByteCursorFromCString(MarketplaceRevenueShareSortByMapper::GetNameForMarketplaceRevenueShareSortBy(m_sortBy).c_str()));
  }
...
Aws::Http::HeaderValueCollection ListMarketplaceRevenueSharesRequest::GetRequestSpecificHeaders() const {
