...
namespace SSOAdmin {
template <>
AWS_SSOADMIN_API ThrottlingException SSOAdminError::GetModeledError() {
  assert(this->GetErrorType() == SSOAdminErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_SSOADMIN_API ResourceNotFoundException SSOAdminError::GetModeledError() {
  assert(this->GetErrorType() == SSOAdminErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_SSOADMIN_API ValidationException SSOAdminError::GetModeledError() {
  assert(this->GetErrorType() == SSOAdminErrors::VALIDATION);
  return ValidationException(this->GetJsonPayload().View());
}

template <>
AWS_SSOADMIN_API AccessDeniedException SSOAdminError::GetModeledError() {
