...
AuthorizedTokenIssuer::AuthorizedTokenIssuer(JsonView jsonValue) { *this = jsonValue; }
...
AuthorizedTokenIssuer& AuthorizedTokenIssuer::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AuthorizedAudiences")) {
    ...
    for (unsigned authorizedAudiencesIndex = 0; authorizedAudiencesIndex < authorizedAudiencesJsonList.GetLength();
         ++authorizedAudiencesIndex) {
