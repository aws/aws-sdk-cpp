...
InstanceAccessControlAttributeConfiguration::InstanceAccessControlAttributeConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
InstanceAccessControlAttributeConfiguration& InstanceAccessControlAttributeConfiguration::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("AccessControlAttributes")) {
    ...
    for (unsigned accessControlAttributesIndex = 0; accessControlAttributesIndex < accessControlAttributesJsonList.GetLength();
         ++accessControlAttributesIndex) {
...
JsonValue InstanceAccessControlAttributeConfiguration::Jsonize() const {
  ...
  if (m_accessControlAttributesHasBeenSet) {
    ...
    for (unsigned accessControlAttributesIndex = 0; accessControlAttributesIndex < accessControlAttributesJsonList.GetLength();
         ++accessControlAttributesIndex) {
      accessControlAttributesJsonList[accessControlAttributesIndex].AsObject(
          m_accessControlAttributes[accessControlAttributesIndex].Jsonize());
    }
