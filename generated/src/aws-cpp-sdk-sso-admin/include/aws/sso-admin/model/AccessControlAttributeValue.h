...
class AccessControlAttributeValue {
 public:
  ...
  template <typename SourceT = Aws::Vector<Aws::String>>
  AccessControlAttributeValue& WithSource(SourceT&& value) {
    SetSource(std::forward<SourceT>(value));
    return *this;
  }
  template <typename SourceT = Aws::String>
  AccessControlAttributeValue& AddSource(SourceT&& value) {
    m_sourceHasBeenSet = true;
    m_source.emplace_back(std::forward<SourceT>(value));
    return *this;
  }
  ///@}
 private:
