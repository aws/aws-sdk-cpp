...
class GetApisResult {
 public:
  ...
  template <typename ItemsT = Aws::Vector<Api>>
  GetApisResult& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = Api>
  GetApisResult& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  GetApisResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  GetApisResult& WithRequestId(RequestIdT&& value) {
