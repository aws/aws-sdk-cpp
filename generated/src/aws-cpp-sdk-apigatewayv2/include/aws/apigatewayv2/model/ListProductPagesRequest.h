...
/**
 */
class ListProductPagesRequest : public ApiGatewayV2Request {
 public:
  AWS_APIGATEWAYV2_API ListProductPagesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListProductPages"; }
  ...
  template <typename MaxResultsT = Aws::String>
  ListProductPagesRequest& WithMaxResults(MaxResultsT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListProductPagesRequest& WithNextToken(NextTokenT&& value) {
  ...
  template <typename PortalProductIdT = Aws::String>
  ListProductPagesRequest& WithPortalProductId(PortalProductIdT&& value) {
  ...
  template <typename ResourceOwnerAccountIdT = Aws::String>
  ListProductPagesRequest& WithResourceOwnerAccountId(ResourceOwnerAccountIdT&& value) {
