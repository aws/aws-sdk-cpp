...
/**
 */
class ListPortalProductsRequest : public ApiGatewayV2Request {
 public:
  AWS_APIGATEWAYV2_API ListPortalProductsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListPortalProducts"; }
  ...
  template <typename MaxResultsT = Aws::String>
  ListPortalProductsRequest& WithMaxResults(MaxResultsT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListPortalProductsRequest& WithNextToken(NextTokenT&& value) {
  ...
  template <typename ResourceOwnerT = Aws::String>
  ListPortalProductsRequest& WithResourceOwner(ResourceOwnerT&& value) {
