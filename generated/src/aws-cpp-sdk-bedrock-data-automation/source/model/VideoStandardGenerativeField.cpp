...
VideoStandardGenerativeField::VideoStandardGenerativeField(JsonView jsonValue) { *this = jsonValue; }
...
VideoStandardGenerativeField& VideoStandardGenerativeField::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("types")) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      m_types.push_back(
          VideoStandardGenerativeFieldTypeMapper::GetVideoStandardGenerativeFieldTypeForName(typesJsonList[typesIndex].AsString()));
    }
  ...
  if (m_typesHasBeenSet) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      typesJsonList[typesIndex].AsString(
          VideoStandardGenerativeFieldTypeMapper::GetNameForVideoStandardGenerativeFieldType(m_types[typesIndex]));
    }
