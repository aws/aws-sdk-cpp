...
DocumentExtractionGranularity::DocumentExtractionGranularity(JsonView jsonValue) { *this = jsonValue; }
...
DocumentExtractionGranularity& DocumentExtractionGranularity::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("types")) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      m_types.push_back(
          DocumentExtractionGranularityTypeMapper::GetDocumentExtractionGranularityTypeForName(typesJsonList[typesIndex].AsString()));
    }
  ...
  if (m_typesHasBeenSet) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      typesJsonList[typesIndex].AsString(
          DocumentExtractionGranularityTypeMapper::GetNameForDocumentExtractionGranularityType(m_types[typesIndex]));
    }
