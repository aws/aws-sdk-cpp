...
AudioStandardGenerativeField::AudioStandardGenerativeField(JsonView jsonValue) { *this = jsonValue; }
...
AudioStandardGenerativeField& AudioStandardGenerativeField::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("types")) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      m_types.push_back(
          AudioStandardGenerativeFieldTypeMapper::GetAudioStandardGenerativeFieldTypeForName(typesJsonList[typesIndex].AsString()));
    }
...
JsonValue AudioStandardGenerativeField::Jsonize() const {
  ...
  if (m_typesHasBeenSet) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      typesJsonList[typesIndex].AsString(
          AudioStandardGenerativeFieldTypeMapper::GetNameForAudioStandardGenerativeFieldType(m_types[typesIndex]));
    }
