...
ImageStandardGenerativeField::ImageStandardGenerativeField(JsonView jsonValue) { *this = jsonValue; }
...
ImageStandardGenerativeField& ImageStandardGenerativeField::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("types")) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      m_types.push_back(
          ImageStandardGenerativeFieldTypeMapper::GetImageStandardGenerativeFieldTypeForName(typesJsonList[typesIndex].AsString()));
    }
  ...
  if (m_typesHasBeenSet) {
    ...
    for (unsigned typesIndex = 0; typesIndex < typesJsonList.GetLength(); ++typesIndex) {
      typesJsonList[typesIndex].AsString(
          ImageStandardGenerativeFieldTypeMapper::GetNameForImageStandardGenerativeFieldType(m_types[typesIndex]));
    }
