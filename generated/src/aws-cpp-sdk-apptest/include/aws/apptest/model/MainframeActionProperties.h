...
class MainframeActionProperties {
 public:
  ...
  void SetDmsTaskArn(DmsTaskArnT&& value) {
    m_dmsTaskArnHasBeenSet = true;
    m_dmsTaskArn = std::forward<DmsTaskArnT>(value);
  }
  template <typename DmsTaskArnT = Aws::String>
  MainframeActionProperties& WithDmsTaskArn(DmsTaskArnT&& value) {
    SetDmsTaskArn(std::forward<DmsTaskArnT>(value));
    return *this;
  }
  ///@}
 private:
