...
class CreateTestCaseRequest : public AppTestRequest {
 public:
  AWS_APPTEST_API CreateTestCaseRequest() = default;
  ...
  template <typename StepsT = Aws::Vector<Step>>
  CreateTestCaseRequest& WithSteps(StepsT&& value) {
    SetSteps(std::forward<StepsT>(value));
    return *this;
  }
  template <typename StepsT = Step>
  CreateTestCaseRequest& AddSteps(StepsT&& value) {
    m_stepsHasBeenSet = true;
  ...
  template <typename ClientTokenT = Aws::String>
  CreateTestCaseRequest& WithClientToken(ClientTokenT&& value) {
