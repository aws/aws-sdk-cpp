...
class CompareFileType {
 public:
  ...
  template <typename DatasetsT = CompareDataSetsSummary>
  CompareFileType& WithDatasets(DatasetsT&& value) {
  ...
  void SetDatabaseCDC(DatabaseCDCT&& value) {
    m_databaseCDCHasBeenSet = true;
    m_databaseCDC = std::forward<DatabaseCDCT>(value);
  }
  template <typename DatabaseCDCT = CompareDatabaseCDCSummary>
  CompareFileType& WithDatabaseCDC(DatabaseCDCT&& value) {
    SetDatabaseCDC(std::forward<DatabaseCDCT>(value));
    return *this;
  }
  ///@}
 private:
