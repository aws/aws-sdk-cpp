...
class CreateCloudFormationStepOutput {
 public:
  ...
  template <typename StackIdT = Aws::String>
  CreateCloudFormationStepOutput& WithStackId(StackIdT&& value) {
  ...
  template <typename ExportsT = Aws::Map<Aws::String, Aws::String>>
  CreateCloudFormationStepOutput& WithExports(ExportsT&& value) {
  ...
  CreateCloudFormationStepOutput& AddExports(ExportsKeyT&& key, ExportsValueT&& value) {
    m_exportsHasBeenSet = true;
    m_exports.emplace(std::forward<ExportsKeyT>(key), std::forward<ExportsValueT>(value));
    return *this;
  }
  ///@}
 private:
