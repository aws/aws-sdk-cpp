...
namespace AppTest {
template <>
AWS_APPTEST_API ConflictException AppTestError::GetModeledError() {
  assert(this->GetErrorType() == AppTestErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_APPTEST_API ServiceQuotaExceededException AppTestError::GetModeledError() {
  assert(this->GetErrorType() == AppTestErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_APPTEST_API ThrottlingException AppTestError::GetModeledError() {
  assert(this->GetErrorType() == AppTestErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_APPTEST_API InternalServerException AppTestError::GetModeledError() {
  assert(this->GetErrorType() == AppTestErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_APPTEST_API ResourceNotFoundException AppTestError::GetModeledError() {
  assert(this->GetErrorType() == AppTestErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_APPTEST_API ValidationException AppTestError::GetModeledError() {
