...
ToolbarConfiguration::ToolbarConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
ToolbarConfiguration& ToolbarConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("hiddenToolbarItems")) {
    ...
    for (unsigned hiddenToolbarItemsIndex = 0; hiddenToolbarItemsIndex < hiddenToolbarItemsJsonList.GetLength();
         ++hiddenToolbarItemsIndex) {
      m_hiddenToolbarItems.push_back(
          ToolbarItemMapper::GetToolbarItemForName(hiddenToolbarItemsJsonList[hiddenToolbarItemsIndex].AsString()));
    }
...
JsonValue ToolbarConfiguration::Jsonize() const {
  ...
  if (m_hiddenToolbarItemsHasBeenSet) {
    ...
    for (unsigned hiddenToolbarItemsIndex = 0; hiddenToolbarItemsIndex < hiddenToolbarItemsJsonList.GetLength();
         ++hiddenToolbarItemsIndex) {
      hiddenToolbarItemsJsonList[hiddenToolbarItemsIndex].AsString(
          ToolbarItemMapper::GetNameForToolbarItem(m_hiddenToolbarItems[hiddenToolbarItemsIndex]));
    }
