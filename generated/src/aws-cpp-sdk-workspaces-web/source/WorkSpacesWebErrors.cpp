...
template <>
AWS_WORKSPACESWEB_API ConflictException WorkSpacesWebError::GetModeledError() {
  assert(this->GetErrorType() == WorkSpacesWebErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_WORKSPACESWEB_API ThrottlingException WorkSpacesWebError::GetModeledError() {
  assert(this->GetErrorType() == WorkSpacesWebErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_WORKSPACESWEB_API ServiceQuotaExceededException WorkSpacesWebError::GetModeledError() {
  assert(this->GetErrorType() == WorkSpacesWebErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_WORKSPACESWEB_API InternalServerException WorkSpacesWebError::GetModeledError() {
  assert(this->GetErrorType() == WorkSpacesWebErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_WORKSPACESWEB_API ResourceNotFoundException WorkSpacesWebError::GetModeledError() {
  assert(this->GetErrorType() == WorkSpacesWebErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_WORKSPACESWEB_API ValidationException WorkSpacesWebError::GetModeledError() {
  assert(this->GetErrorType() == WorkSpacesWebErrors::VALIDATION);
  return ValidationException(this->GetJsonPayload().View());
}

template <>
AWS_WORKSPACESWEB_API TooManyTagsException WorkSpacesWebError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(WorkSpacesWebErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
