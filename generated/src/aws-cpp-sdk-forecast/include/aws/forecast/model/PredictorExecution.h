...
class PredictorExecution {
 public:
  ...
  template <typename AlgorithmArnT = Aws::String>
  PredictorExecution& WithAlgorithmArn(AlgorithmArnT&& value) {
  ...
  template <typename TestWindowsT = Aws::Vector<TestWindowSummary>>
  PredictorExecution& WithTestWindows(TestWindowsT&& value) {
    SetTestWindows(std::forward<TestWindowsT>(value));
    return *this;
  }
  template <typename TestWindowsT = TestWindowSummary>
  PredictorExecution& AddTestWindows(TestWindowsT&& value) {
    m_testWindowsHasBeenSet = true;
