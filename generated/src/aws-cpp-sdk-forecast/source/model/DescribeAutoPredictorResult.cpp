...
DescribeAutoPredictorResult::DescribeAutoPredictorResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
DescribeAutoPredictorResult& DescribeAutoPredictorResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("ForecastDimensions")) {
    ...
    for (unsigned forecastDimensionsIndex = 0; forecastDimensionsIndex < forecastDimensionsJsonList.GetLength();
         ++forecastDimensionsIndex) {
  ...
  if (jsonValue.ValueExists("DatasetImportJobArns")) {
    ...
    for (unsigned datasetImportJobArnsIndex = 0; datasetImportJobArnsIndex < datasetImportJobArnsJsonList.GetLength();
