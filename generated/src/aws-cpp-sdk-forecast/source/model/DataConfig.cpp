...
DataConfig::DataConfig(JsonView jsonValue) { *this = jsonValue; }
...
DataConfig& DataConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AdditionalDatasets")) {
    ...
    for (unsigned additionalDatasetsIndex = 0; additionalDatasetsIndex < additionalDatasetsJsonList.GetLength();
         ++additionalDatasetsIndex) {
