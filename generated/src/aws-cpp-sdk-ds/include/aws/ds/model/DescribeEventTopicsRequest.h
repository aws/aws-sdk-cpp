...
class DescribeEventTopicsRequest : public DirectoryServiceRequest {
 public:
  ...
  void SetTopicNames(TopicNamesT&& value) {
    m_topicNamesHasBeenSet = true;
    m_topicNames = std::forward<TopicNamesT>(value);
  }
  template <typename TopicNamesT = Aws::Vector<Aws::String>>
  DescribeEventTopicsRequest& WithTopicNames(TopicNamesT&& value) {
    SetTopicNames(std::forward<TopicNamesT>(value));
    return *this;
  }
  template <typename TopicNamesT = Aws::String>
  DescribeEventTopicsRequest& AddTopicNames(TopicNamesT&& value) {
    m_topicNamesHasBeenSet = true;
    m_topicNames.emplace_back(std::forward<TopicNamesT>(value));
    return *this;
  }
  ///@}
 private:
