...
class ListLogSubscriptionsResult {
 public:
  ...
  void SetLogSubscriptions(LogSubscriptionsT&& value) {
    m_logSubscriptionsHasBeenSet = true;
    m_logSubscriptions = std::forward<LogSubscriptionsT>(value);
  }
  template <typename LogSubscriptionsT = Aws::Vector<LogSubscription>>
  ListLogSubscriptionsResult& WithLogSubscriptions(LogSubscriptionsT&& value) {
    SetLogSubscriptions(std::forward<LogSubscriptionsT>(value));
    return *this;
  }
  ...
  ListLogSubscriptionsResult& AddLogSubscriptions(LogSubscriptionsT&& value) {
    m_logSubscriptionsHasBeenSet = true;
    m_logSubscriptions.emplace_back(std::forward<LogSubscriptionsT>(value));
    return *this;
  }
  ///@}
