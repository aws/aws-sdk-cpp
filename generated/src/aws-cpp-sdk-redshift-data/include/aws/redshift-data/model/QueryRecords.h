...
class QueryRecords {
 public:
  ...
  void SetCSVRecords(CSVRecordsT&& value) {
    m_cSVRecordsHasBeenSet = true;
    m_cSVRecords = std::forward<CSVRecordsT>(value);
  }
  template <typename CSVRecordsT = Aws::String>
  QueryRecords& WithCSVRecords(CSVRecordsT&& value) {
    SetCSVRecords(std::forward<CSVRecordsT>(value));
    return *this;
  }
  ///@}
 private:
