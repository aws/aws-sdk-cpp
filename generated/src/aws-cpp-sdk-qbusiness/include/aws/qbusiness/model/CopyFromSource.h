...
class CopyFromSource {
 public:
  ...
  void SetConversation(ConversationT&& value) {
    m_conversationHasBeenSet = true;
    m_conversation = std::forward<ConversationT>(value);
  }
  template <typename ConversationT = ConversationSource>
  CopyFromSource& WithConversation(ConversationT&& value) {
    SetConversation(std::forward<ConversationT>(value));
    return *this;
  }
  ///@}
 private:
