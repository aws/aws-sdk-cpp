...
class ChatRequest : public QBusinessRequest {
 public:
   ...
   * Underlying Event Stream Handler which is used to define callback functions.
   */
  inline void SetEventStreamHandler(const ChatHandler& value) {
    m_handler = value;
   ...
   * Underlying Event Stream Handler which is used to define callback functions.
   */
  inline ChatRequest& WithEventStreamHandler(const ChatHandler& value) {
  ...
  void SetParentMessageId(ParentMessageIdT&& value) {
    m_parentMessageIdHasBeenSet = true;
    m_parentMessageId = std::forward<ParentMessageIdT>(value);
  }
  template <typename ParentMessageIdT = Aws::String>
  ChatRequest& WithParentMessageId(ParentMessageIdT&& value) {
    SetParentMessageId(std::forward<ParentMessageIdT>(value));
    return *this;
  }
  ///@}
