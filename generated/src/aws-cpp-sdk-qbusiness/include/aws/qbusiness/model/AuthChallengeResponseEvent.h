...
class AuthChallengeResponseEvent {
 public:
  ...
  void SetResponseMap(ResponseMapT&& value) {
    ...
    m_responseMap = std::forward<ResponseMapT>(value);
  }
  template <typename ResponseMapT = Aws::Map<Aws::String, Aws::String>>
  AuthChallengeResponseEvent& WithResponseMap(ResponseMapT&& value) {
    SetResponseMap(std::forward<ResponseMapT>(value));
    return *this;
  }
  ...
  AuthChallengeResponseEvent& AddResponseMap(ResponseMapKeyT&& key, ResponseMapValueT&& value) {
    m_responseMapHasBeenSet = true;
    m_responseMap.emplace(std::forward<ResponseMapKeyT>(key), std::forward<ResponseMapValueT>(value));
    return *this;
  }
  ///@}
 private:
