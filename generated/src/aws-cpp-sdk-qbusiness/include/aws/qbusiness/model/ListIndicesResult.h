...
class ListIndicesResult {
 public:
  ...
  void SetIndices(IndicesT&& value) {
    m_indicesHasBeenSet = true;
    m_indices = std::forward<IndicesT>(value);
  }
  template <typename IndicesT = Aws::Vector<Index>>
  ListIndicesResult& WithIndices(IndicesT&& value) {
    SetIndices(std::forward<IndicesT>(value));
    return *this;
  }
  template <typename IndicesT = Index>
  ListIndicesResult& AddIndices(IndicesT&& value) {
    m_indicesHasBeenSet = true;
    m_indices.emplace_back(std::forward<IndicesT>(value));
    return *this;
  }
  ///@}
