...
ConnectorCheck::ConnectorCheck(JsonView jsonValue) { *this = jsonValue; }

ConnectorCheck& ConnectorCheck::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("lastCheckAttempt")) {
    m_lastCheckAttempt = jsonValue.GetDouble("lastCheckAttempt");
...
JsonValue ConnectorCheck::Jsonize() const {
  JsonValue payload;
  ...
  if (m_lastCheckAttemptHasBeenSet) {
    payload.WithDouble("lastCheckAttempt", m_lastCheckAttempt.SecondsWithMSPrecision());
  }
