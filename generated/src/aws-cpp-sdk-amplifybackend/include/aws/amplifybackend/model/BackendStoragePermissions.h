...
class BackendStoragePermissions {
 public:
  ...
  void SetAuthenticated(AuthenticatedT&& value) {
    ...
    m_authenticated = std::forward<AuthenticatedT>(value);
  }
  template <typename AuthenticatedT = Aws::Vector<AuthenticatedElement>>
  BackendStoragePermissions& WithAuthenticated(AuthenticatedT&& value) {
    SetAuthenticated(std::forward<AuthenticatedT>(value));
    return *this;
  }
  inline BackendStoragePermissions& AddAuthenticated(AuthenticatedElement value) {
    m_authenticatedHasBeenSet = true;
    m_authenticated.push_back(value);
    return *this;
  }
  ///@}
  ...
  inline BackendStoragePermissions& AddUnAuthenticated(UnAuthenticatedElement value) {
    m_unAuthenticatedHasBeenSet = true;
    m_unAuthenticated.push_back(value);
    return *this;
  }
  ///@}
 private:
