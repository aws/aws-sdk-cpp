...
UpdateBackendAuthOAuthConfig::UpdateBackendAuthOAuthConfig(JsonView jsonValue) { *this = jsonValue; }

UpdateBackendAuthOAuthConfig& UpdateBackendAuthOAuthConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("redirectSignInURIs")) {
    ...
    for (unsigned redirectSignInURIsIndex = 0; redirectSignInURIsIndex < redirectSignInURIsJsonList.GetLength();
         ++redirectSignInURIsIndex) {
  ...
  if (jsonValue.ValueExists("redirectSignOutURIs")) {
    ...
    for (unsigned redirectSignOutURIsIndex = 0; redirectSignOutURIsIndex < redirectSignOutURIsJsonList.GetLength();
         ++redirectSignOutURIsIndex) {
...
JsonValue UpdateBackendAuthOAuthConfig::Jsonize() const {
  JsonValue payload;
  ...
  if (m_oAuthScopesHasBeenSet) {
    ...
    for (unsigned oAuthScopesIndex = 0; oAuthScopesIndex < oAuthScopesJsonList.GetLength(); ++oAuthScopesIndex) {
      oAuthScopesJsonList[oAuthScopesIndex].AsString(
          OAuthScopesElementMapper::GetNameForOAuthScopesElement(m_oAuthScopes[oAuthScopesIndex]));
    }
