...
UpdateBackendAuthPasswordPolicyConfig::UpdateBackendAuthPasswordPolicyConfig(JsonView jsonValue) { *this = jsonValue; }
...
UpdateBackendAuthPasswordPolicyConfig& UpdateBackendAuthPasswordPolicyConfig::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("additionalConstraints")) {
    ...
    for (unsigned additionalConstraintsIndex = 0; additionalConstraintsIndex < additionalConstraintsJsonList.GetLength();
         ++additionalConstraintsIndex) {
      m_additionalConstraints.push_back(AdditionalConstraintsElementMapper::GetAdditionalConstraintsElementForName(
          additionalConstraintsJsonList[additionalConstraintsIndex].AsString()));
    }
...
JsonValue UpdateBackendAuthPasswordPolicyConfig::Jsonize() const {
  ...
  if (m_additionalConstraintsHasBeenSet) {
    ...
    for (unsigned additionalConstraintsIndex = 0; additionalConstraintsIndex < additionalConstraintsJsonList.GetLength();
         ++additionalConstraintsIndex) {
      additionalConstraintsJsonList[additionalConstraintsIndex].AsString(
          AdditionalConstraintsElementMapper::GetNameForAdditionalConstraintsElement(m_additionalConstraints[additionalConstraintsIndex]));
    }
