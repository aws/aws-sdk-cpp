...
CreateBackendAuthUserPoolConfig::CreateBackendAuthUserPoolConfig(JsonView jsonValue) { *this = jsonValue; }
...
CreateBackendAuthUserPoolConfig& CreateBackendAuthUserPoolConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("requiredSignUpAttributes")) {
    ...
    for (unsigned requiredSignUpAttributesIndex = 0; requiredSignUpAttributesIndex < requiredSignUpAttributesJsonList.GetLength();
         ++requiredSignUpAttributesIndex) {
      m_requiredSignUpAttributes.push_back(RequiredSignUpAttributesElementMapper::GetRequiredSignUpAttributesElementForName(
          requiredSignUpAttributesJsonList[requiredSignUpAttributesIndex].AsString()));
    }
...
JsonValue CreateBackendAuthUserPoolConfig::Jsonize() const {
  ...
  if (m_requiredSignUpAttributesHasBeenSet) {
    ...
    for (unsigned requiredSignUpAttributesIndex = 0; requiredSignUpAttributesIndex < requiredSignUpAttributesJsonList.GetLength();
         ++requiredSignUpAttributesIndex) {
      requiredSignUpAttributesJsonList[requiredSignUpAttributesIndex].AsString(
          RequiredSignUpAttributesElementMapper::GetNameForRequiredSignUpAttributesElement(
              m_requiredSignUpAttributes[requiredSignUpAttributesIndex]));
    }
