...
BackendStoragePermissions::BackendStoragePermissions(JsonView jsonValue) { *this = jsonValue; }
...
BackendStoragePermissions& BackendStoragePermissions::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("authenticated")) {
    ...
    for (unsigned authenticatedIndex = 0; authenticatedIndex < authenticatedJsonList.GetLength(); ++authenticatedIndex) {
      m_authenticated.push_back(
          AuthenticatedElementMapper::GetAuthenticatedElementForName(authenticatedJsonList[authenticatedIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("unAuthenticated")) {
    ...
    for (unsigned unAuthenticatedIndex = 0; unAuthenticatedIndex < unAuthenticatedJsonList.GetLength(); ++unAuthenticatedIndex) {
      m_unAuthenticated.push_back(
          UnAuthenticatedElementMapper::GetUnAuthenticatedElementForName(unAuthenticatedJsonList[unAuthenticatedIndex].AsString()));
    }
...
JsonValue BackendStoragePermissions::Jsonize() const {
  ...
  if (m_authenticatedHasBeenSet) {
    ...
    for (unsigned authenticatedIndex = 0; authenticatedIndex < authenticatedJsonList.GetLength(); ++authenticatedIndex) {
      authenticatedJsonList[authenticatedIndex].AsString(
          AuthenticatedElementMapper::GetNameForAuthenticatedElement(m_authenticated[authenticatedIndex]));
    }
  ...
  if (m_unAuthenticatedHasBeenSet) {
    ...
    for (unsigned unAuthenticatedIndex = 0; unAuthenticatedIndex < unAuthenticatedJsonList.GetLength(); ++unAuthenticatedIndex) {
      unAuthenticatedJsonList[unAuthenticatedIndex].AsString(
          UnAuthenticatedElementMapper::GetNameForUnAuthenticatedElement(m_unAuthenticated[unAuthenticatedIndex]));
    }
