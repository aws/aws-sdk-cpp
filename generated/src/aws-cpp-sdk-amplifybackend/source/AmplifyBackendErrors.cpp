...
template <>
AWS_AMPLIFYBACKEND_API NotFoundException AmplifyBackendError::GetModeledError() {
  assert(this->GetErrorType() == AmplifyBackendErrors::NOT_FOUND);
  return NotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_AMPLIFYBACKEND_API TooManyRequestsException AmplifyBackendError::GetModeledError() {
