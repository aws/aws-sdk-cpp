...
namespace Model
{
    ...
    void InvokeInlineAgentHandler::HandleEventInMessage()
    {
        ...
        case InvokeInlineAgentEventType::CHUNK:
        {
            ...
            {
                AWS_LOGSTREAM_WARN(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Unable to generate a proper InlineAgentPayloadPart object from the response in JSON format.");
                break;
            }
        ...
        case InvokeInlineAgentEventType::TRACE:
        {
            ...
            if (!json.WasParseSuccessful())
            {
                AWS_LOGSTREAM_WARN(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Unable to generate a proper InlineAgentTracePart object from the response in JSON format.");
                break;
            }
        ...
        case InvokeInlineAgentEventType::RETURNCONTROL:
        {
            ...
            if (!json.WasParseSuccessful())
            {
                AWS_LOGSTREAM_WARN(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Unable to generate a proper InlineAgentReturnControlPayload object from the response in JSON format.");
                break;
            }
        ...
        case InvokeInlineAgentEventType::FILES:
        {
            ...
            if (!json.WasParseSuccessful())
            {
                AWS_LOGSTREAM_WARN(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Unable to generate a proper InlineAgentFilePart object from the response in JSON format.");
                break;
            }
    ...
    void InvokeInlineAgentHandler::HandleErrorInMessage()
    {
        ...
        if (errorHeaderIter == headers.end())
        {
            ...
            {
                AWS_LOGSTREAM_ERROR(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Unable to generate a proper InlineAgentFilePart object from the response in JSON format.");
                auto contentTypeIter = headers.find(Aws::Utils::Event::CONTENT_TYPE_HEADER);
                if (contentTypeIter != headers.end())
                {
                    AWS_LOGSTREAM_DEBUG(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Error content-type: " << contentTypeIter->second.GetEventHeaderValueAsString());
                }
                return;
            }
    ...
    void InvokeInlineAgentHandler::MarshallError(const Aws::String& errorCode, const Aws::String& errorMessage)
    {
        ...
        else
        {
            ...
            {
                AWS_LOGSTREAM_WARN(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Encountered AWSError '" << errorCode.c_str() << "': " << errorMessage.c_str());
                error.SetExceptionName(errorCode);
                error.SetMessage(errorMessage);
            }
            else
            {
                AWS_LOGSTREAM_WARN(INVOKEINLINEAGENT_HANDLER_CLASS_TAG, "Encountered Unknown AWSError '" << errorCode.c_str() << "': " << errorMessage.c_str());
