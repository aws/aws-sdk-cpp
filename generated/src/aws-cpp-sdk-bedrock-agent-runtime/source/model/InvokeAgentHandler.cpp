...
InvokeAgentHandler::InvokeAgentHandler() : EventStreamHandler() {
  ...
  m_onPayloadPart = [&](const PayloadPart&) { AWS_LOGSTREAM_TRACE(INVOKEAGENT_HANDLER_CLASS_TAG, "PayloadPart received."); };

  m_onFilePart = [&](const FilePart&) { AWS_LOGSTREAM_TRACE(INVOKEAGENT_HANDLER_CLASS_TAG, "FilePart received."); };
  ...
  m_onTracePart = [&](const TracePart&) { AWS_LOGSTREAM_TRACE(INVOKEAGENT_HANDLER_CLASS_TAG, "TracePart received."); };
  ...
  switch (InvokeAgentEventMapper::GetInvokeAgentEventTypeForName(eventTypeHeaderIter->second.GetEventHeaderValueAsString())) {
    ...
    case InvokeAgentEventType::CHUNK: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(INVOKEAGENT_HANDLER_CLASS_TAG,
                           "Unable to generate a proper PayloadPart object from the response in JSON format.");
        break;
      }
    ...
    case InvokeAgentEventType::RETURNCONTROL: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(INVOKEAGENT_HANDLER_CLASS_TAG,
                           "Unable to generate a proper ReturnControlPayload object from the response in JSON format.");
        break;
      }
...
void InvokeAgentHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(INVOKEAGENT_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(INVOKEAGENT_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
