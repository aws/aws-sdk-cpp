...
class FlowTraceCondition {
 public:
  ...
  void SetConditionName(ConditionNameT&& value) {
    m_conditionNameHasBeenSet = true;
    m_conditionName = std::forward<ConditionNameT>(value);
  }
  template <typename ConditionNameT = Aws::String>
  FlowTraceCondition& WithConditionName(ConditionNameT&& value) {
    SetConditionName(std::forward<ConditionNameT>(value));
    return *this;
  }
  ///@}
 private:
