...
class CodeInterpreterInvocationInput {
 public:
  ...
  template <typename CodeT = Aws::String>
  CodeInterpreterInvocationInput& WithCode(CodeT&& value) {
  ...
  template <typename FilesT = Aws::Vector<Aws::String>>
  CodeInterpreterInvocationInput& WithFiles(FilesT&& value) {
    SetFiles(std::forward<FilesT>(value));
    return *this;
  }
  template <typename FilesT = Aws::String>
  CodeInterpreterInvocationInput& AddFiles(FilesT&& value) {
    m_filesHasBeenSet = true;
