...
class GetAgentMemoryResult {
 public:
  ...
  void SetMemoryContents(MemoryContentsT&& value) {
    ...
    m_memoryContents = std::forward<MemoryContentsT>(value);
  }
  template <typename MemoryContentsT = Aws::Vector<Memory>>
  GetAgentMemoryResult& WithMemoryContents(MemoryContentsT&& value) {
    SetMemoryContents(std::forward<MemoryContentsT>(value));
    return *this;
  }
  template <typename MemoryContentsT = Memory>
  GetAgentMemoryResult& AddMemoryContents(MemoryContentsT&& value) {
    m_memoryContentsHasBeenSet = true;
    m_memoryContents.emplace_back(std::forward<MemoryContentsT>(value));
    return *this;
  }
  ///@}
