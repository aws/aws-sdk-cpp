...
class FlowTraceNodeInputSource {
 public:
  ...
  void SetOutputFieldName(OutputFieldNameT&& value) {
    m_outputFieldNameHasBeenSet = true;
    m_outputFieldName = std::forward<OutputFieldNameT>(value);
  }
  template <typename OutputFieldNameT = Aws::String>
  FlowTraceNodeInputSource& WithOutputFieldName(OutputFieldNameT&& value) {
    SetOutputFieldName(std::forward<OutputFieldNameT>(value));
    return *this;
  }
  ///@}
