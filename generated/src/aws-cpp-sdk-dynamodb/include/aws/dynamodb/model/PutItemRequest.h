...
class PutItemRequest : public DynamoDBRequest {
 public:
   ...
   * definition.</p> <p>If the table has vector indexes, the following validations
   * apply to write operations. A violation of any of these constraints results in a
   * <code>ValidationException</code>:</p> <ul> <li> <p>The vector attribute must be
   * a list of numbers with dimensions matching the index configuration.</p> </li>
   * <li> <p>Vector values must fit in 32-bit IEEE-754 floating point format
   * (f32).</p> </li> <li> <p>Partition key and inline filter attributes defined in
   * the search schema must have data types matching the index schema definition.</p>
   * </li> </ul> <p>Empty String and Binary attribute values are allowed. Attribute
   * values of type String and Binary must have a length greater than zero if the
   * attribute is used as a key attribute for a table or index.</p> <p>For more
  ...
  inline PutItemRequest& WithReturnValues(ReturnValue value) {
  ...
  inline PutItemRequest& WithReturnConsumedCapacity(ReturnConsumedCapacity value) {
  ...
  inline PutItemRequest& WithReturnItemCollectionMetrics(ReturnItemCollectionMetrics value) {
  ...
  inline PutItemRequest& WithConditionalOperator(ConditionalOperator value) {
  ...
  inline PutItemRequest& WithReturnValuesOnConditionCheckFailure(ReturnValuesOnConditionCheckFailure value) {
