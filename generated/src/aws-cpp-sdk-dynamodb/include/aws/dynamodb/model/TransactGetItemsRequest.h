...
class TransactGetItemsRequest : public DynamoDBRequest {
 public:
  ...
  template <typename TransactItemsT = Aws::Vector<TransactGetItem>>
  TransactGetItemsRequest& WithTransactItems(TransactItemsT&& value) {
    SetTransactItems(std::forward<TransactItemsT>(value));
    return *this;
  }
  template <typename TransactItemsT = TransactGetItem>
  TransactGetItemsRequest& AddTransactItems(TransactItemsT&& value) {
    m_transactItemsHasBeenSet = true;
  ...
  inline TransactGetItemsRequest& WithReturnConsumedCapacity(ReturnConsumedCapacity value) {
