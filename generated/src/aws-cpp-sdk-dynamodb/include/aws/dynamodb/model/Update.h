...
class Update {
 public:
  ...
  void SetUpdateExpression(UpdateExpressionT&& value) {
    m_updateExpressionHasBeenSet = true;
    m_updateExpression = std::forward<UpdateExpressionT>(value);
  }
  template <typename UpdateExpressionT = Aws::String>
  Update& WithUpdateExpression(UpdateExpressionT&& value) {
    SetUpdateExpression(std::forward<UpdateExpressionT>(value));
    return *this;
  }
  ///@}
  ...
  inline Update& WithReturnValuesOnConditionCheckFailure(ReturnValuesOnConditionCheckFailure value) {
