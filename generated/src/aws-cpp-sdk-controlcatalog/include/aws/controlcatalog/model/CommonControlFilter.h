...
class CommonControlFilter {
 public:
  ...
  template <typename ObjectivesT = Aws::Vector<ObjectiveResourceFilter>>
  CommonControlFilter& WithObjectives(ObjectivesT&& value) {
    SetObjectives(std::forward<ObjectivesT>(value));
    return *this;
  }
  template <typename ObjectivesT = ObjectiveResourceFilter>
  CommonControlFilter& AddObjectives(ObjectivesT&& value) {
    m_objectivesHasBeenSet = true;
    m_objectives.emplace_back(std::forward<ObjectivesT>(value));
    return *this;
  }
  ///@}
 private:
