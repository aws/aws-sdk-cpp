...
class BatchDeleteDetectorRequest : public IoTEventsDataRequest {
 public:
  ...
  void SetDetectors(DetectorsT&& value) {
    ...
    m_detectors = std::forward<DetectorsT>(value);
  }
  template <typename DetectorsT = Aws::Vector<DeleteDetectorRequest>>
  BatchDeleteDetectorRequest& WithDetectors(DetectorsT&& value) {
    SetDetectors(std::forward<DetectorsT>(value));
    return *this;
  }
  template <typename DetectorsT = DeleteDetectorRequest>
  BatchDeleteDetectorRequest& AddDetectors(DetectorsT&& value) {
    m_detectorsHasBeenSet = true;
    m_detectors.emplace_back(std::forward<DetectorsT>(value));
    return *this;
  }
  ///@}
 private:
