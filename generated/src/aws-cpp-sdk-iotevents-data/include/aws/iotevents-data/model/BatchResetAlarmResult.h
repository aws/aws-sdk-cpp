...
class BatchResetAlarmResult {
 public:
  ...
  template <typename ErrorEntriesT = Aws::Vector<BatchAlarmActionErrorEntry>>
  BatchResetAlarmResult& WithErrorEntries(ErrorEntriesT&& value) {
    SetErrorEntries(std::forward<ErrorEntriesT>(value));
    return *this;
  }
  template <typename ErrorEntriesT = BatchAlarmActionErrorEntry>
  BatchResetAlarmResult& AddErrorEntries(ErrorEntriesT&& value) {
    m_errorEntriesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchResetAlarmResult& WithRequestId(RequestIdT&& value) {
