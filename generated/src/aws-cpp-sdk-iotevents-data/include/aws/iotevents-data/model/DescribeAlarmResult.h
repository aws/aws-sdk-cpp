...
class DescribeAlarmResult {
 public:
  ...
  void SetAlarm(AlarmT&& value) {
    m_alarmHasBeenSet = true;
    m_alarm = std::forward<AlarmT>(value);
  }
  template <typename AlarmT = Alarm>
  DescribeAlarmResult& WithAlarm(AlarmT&& value) {
    SetAlarm(std::forward<AlarmT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  DescribeAlarmResult& WithRequestId(RequestIdT&& value) {
