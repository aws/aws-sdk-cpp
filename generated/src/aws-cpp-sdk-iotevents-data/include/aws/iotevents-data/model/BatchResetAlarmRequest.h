...
class BatchResetAlarmRequest : public IoTEventsDataRequest {
 public:
  ...
  void SetResetActionRequests(ResetActionRequestsT&& value) {
    m_resetActionRequestsHasBeenSet = true;
    m_resetActionRequests = std::forward<ResetActionRequestsT>(value);
  }
  template <typename ResetActionRequestsT = Aws::Vector<ResetAlarmActionRequest>>
  BatchResetAlarmRequest& WithResetActionRequests(ResetActionRequestsT&& value) {
    SetResetActionRequests(std::forward<ResetActionRequestsT>(value));
    return *this;
  }
  template <typename ResetActionRequestsT = ResetAlarmActionRequest>
  BatchResetAlarmRequest& AddResetActionRequests(ResetActionRequestsT&& value) {
    m_resetActionRequestsHasBeenSet = true;
    m_resetActionRequests.emplace_back(std::forward<ResetActionRequestsT>(value));
    return *this;
  }
  ///@}
 private:
