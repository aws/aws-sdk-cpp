...
class RealtimeContactAnalysisSegment {
 public:
  ...
  void SetTranscript(TranscriptT&& value) {
    ...
    m_transcript = std::forward<TranscriptT>(value);
  }
  template <typename TranscriptT = Transcript>
  RealtimeContactAnalysisSegment& WithTranscript(TranscriptT&& value) {
    SetTranscript(std::forward<TranscriptT>(value));
    return *this;
  }
  ///@}
  ...
  void SetPostContactSummary(PostContactSummaryT&& value) {
    m_postContactSummaryHasBeenSet = true;
    m_postContactSummary = std::forward<PostContactSummaryT>(value);
  }
  template <typename PostContactSummaryT = PostContactSummary>
  RealtimeContactAnalysisSegment& WithPostContactSummary(PostContactSummaryT&& value) {
    SetPostContactSummary(std::forward<PostContactSummaryT>(value));
    return *this;
  }
  ///@}
 private:
