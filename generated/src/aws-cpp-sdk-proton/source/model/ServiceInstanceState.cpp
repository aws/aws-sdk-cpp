...
ServiceInstanceState::ServiceInstanceState(JsonView jsonValue) { *this = jsonValue; }
...
ServiceInstanceState& ServiceInstanceState::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("lastSuccessfulComponentDeploymentIds")) {
    ...
    for (unsigned lastSuccessfulComponentDeploymentIdsIndex = 0;
         lastSuccessfulComponentDeploymentIdsIndex < lastSuccessfulComponentDeploymentIdsJsonList.GetLength();
         ++lastSuccessfulComponentDeploymentIdsIndex) {
      m_lastSuccessfulComponentDeploymentIds.push_back(
          lastSuccessfulComponentDeploymentIdsJsonList[lastSuccessfulComponentDeploymentIdsIndex].AsString());
    }
...
JsonValue ServiceInstanceState::Jsonize() const {
  ...
  if (m_lastSuccessfulComponentDeploymentIdsHasBeenSet) {
    ...
    for (unsigned lastSuccessfulComponentDeploymentIdsIndex = 0;
         ...
         ++lastSuccessfulComponentDeploymentIdsIndex) {
      lastSuccessfulComponentDeploymentIdsJsonList[lastSuccessfulComponentDeploymentIdsIndex].AsString(
          m_lastSuccessfulComponentDeploymentIds[lastSuccessfulComponentDeploymentIdsIndex]);
    }
