...
template <>
AWS_WELLARCHITECTED_API ConflictException WellArchitectedError::GetModeledError() {
  assert(this->GetErrorType() == WellArchitectedErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_WELLARCHITECTED_API ThrottlingException WellArchitectedError::GetModeledError() {
  assert(this->GetErrorType() == WellArchitectedErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_WELLARCHITECTED_API ServiceQuotaExceededException WellArchitectedError::GetModeledError() {
  assert(this->GetErrorType() == WellArchitectedErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_WELLARCHITECTED_API ResourceNotFoundException WellArchitectedError::GetModeledError() {
  assert(this->GetErrorType() == WellArchitectedErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_WELLARCHITECTED_API ValidationException WellArchitectedError::GetModeledError() {
