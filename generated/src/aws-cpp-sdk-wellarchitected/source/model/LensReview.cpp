...
LensReview::LensReview(JsonView jsonValue) { *this = jsonValue; }
...
LensReview& LensReview::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("PillarReviewSummaries")) {
    ...
    for (unsigned pillarReviewSummariesIndex = 0; pillarReviewSummariesIndex < pillarReviewSummariesJsonList.GetLength();
         ++pillarReviewSummariesIndex) {
...
JsonValue LensReview::Jsonize() const {
  ...
  if (m_prioritizedRiskCountsHasBeenSet) {
    ...
    for (auto& prioritizedRiskCountsItem : m_prioritizedRiskCounts) {
      prioritizedRiskCountsJsonMap.WithInteger(RiskMapper::GetNameForRisk(prioritizedRiskCountsItem.first),
                                               prioritizedRiskCountsItem.second);
    }
