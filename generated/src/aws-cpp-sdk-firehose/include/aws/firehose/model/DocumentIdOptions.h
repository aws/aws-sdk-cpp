...
namespace Model
{
   ...
   * document ID.</p> <p/><p><h3>See Also:</h3>   <a
  ...
  public:
    ...
    /**
     * <p>When the <code>FIREHOSE_DEFAULT</code> option is chosen, Kinesis Data
     * Firehose generates a unique document ID for each record based on a unique
     * internal identifier. The generated document ID is stable across multiple
     * delivery attempts, which helps prevent the same record from being indexed
     * multiple times with different document IDs.</p> <p>When the
     * <code>NO_DOCUMENT_ID</code> option is chosen, Kinesis Data Firehose does not
     ...
     * IDs. In case of multiple delivery attempts, this may cause the same record to be
     * indexed more than once with different document IDs. This option enables
     * write-heavy operations, such as the ingestion of logs and observability data, to
     * consume less resources in the Amazon OpenSearch Service domain, resulting in
