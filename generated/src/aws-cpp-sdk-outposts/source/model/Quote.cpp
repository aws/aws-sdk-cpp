...
Quote::Quote(JsonView jsonValue) { *this = jsonValue; }

Quote& Quote::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("OrderingRequirements")) {
    Aws::Utils::Array<JsonView> orderingRequirementsJsonList = jsonValue.GetArray("OrderingRequirements");
    for (unsigned orderingRequirementsIndex = 0; orderingRequirementsIndex < orderingRequirementsJsonList.GetLength();
         ++orderingRequirementsIndex) {
      m_orderingRequirements.push_back(orderingRequirementsJsonList[orderingRequirementsIndex].AsObject());
    }
...
JsonValue Quote::Jsonize() const {
  ...
  if (m_orderingRequirementsHasBeenSet) {
    Aws::Utils::Array<JsonValue> orderingRequirementsJsonList(m_orderingRequirements.size());
    for (unsigned orderingRequirementsIndex = 0; orderingRequirementsIndex < orderingRequirementsJsonList.GetLength();
         ++orderingRequirementsIndex) {
      orderingRequirementsJsonList[orderingRequirementsIndex].AsObject(m_orderingRequirements[orderingRequirementsIndex].Jsonize());
    }
    payload.WithArray("OrderingRequirements", std::move(orderingRequirementsJsonList));
  }
