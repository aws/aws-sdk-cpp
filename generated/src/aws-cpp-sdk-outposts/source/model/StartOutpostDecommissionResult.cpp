...
StartOutpostDecommissionResult::StartOutpostDecommissionResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
StartOutpostDecommissionResult& StartOutpostDecommissionResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("BlockingResourceTypes")) {
    ...
    for (unsigned blockingResourceTypesIndex = 0; blockingResourceTypesIndex < blockingResourceTypesJsonList.GetLength();
         ++blockingResourceTypesIndex) {
      m_blockingResourceTypes.push_back(
          BlockingResourceTypeMapper::GetBlockingResourceTypeForName(blockingResourceTypesJsonList[blockingResourceTypesIndex].AsString()));
    }
