...
class RuleGroupUpdate {
 public:
  ...
  inline RuleGroupUpdate& WithAction(ChangeAction value) {
    SetAction(value);
    return *this;
  }
  ///@}
  ...
  void SetActivatedRule(ActivatedRuleT&& value) {
    m_activatedRuleHasBeenSet = true;
    m_activatedRule = std::forward<ActivatedRuleT>(value);
  }
  template <typename ActivatedRuleT = ActivatedRule>
  RuleGroupUpdate& WithActivatedRule(ActivatedRuleT&& value) {
    SetActivatedRule(std::forward<ActivatedRuleT>(value));
    return *this;
  }
  ///@}
 private:
