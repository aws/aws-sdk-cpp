...
class IPSetUpdate {
 public:
  ...
  inline IPSetUpdate& WithAction(ChangeAction value) {
    SetAction(value);
    return *this;
  }
  ///@}
  ...
  void SetIPSetDescriptor(IPSetDescriptorT&& value) {
    ...
    m_iPSetDescriptor = std::forward<IPSetDescriptorT>(value);
  }
  template <typename IPSetDescriptorT = IPSetDescriptor>
  IPSetUpdate& WithIPSetDescriptor(IPSetDescriptorT&& value) {
    SetIPSetDescriptor(std::forward<IPSetDescriptorT>(value));
    return *this;
  }
  ///@}
 private:
