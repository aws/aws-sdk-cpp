...
class CreateWebACLResult {
 public:
  AWS_WAFREGIONAL_API CreateWebACLResult() = default;
  ...
  void SetWebACL(WebACLT&& value) {
    ...
    m_webACL = std::forward<WebACLT>(value);
  }
  template <typename WebACLT = WebACL>
  CreateWebACLResult& WithWebACL(WebACLT&& value) {
    SetWebACL(std::forward<WebACLT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename ChangeTokenT = Aws::String>
  CreateWebACLResult& WithChangeToken(ChangeTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  CreateWebACLResult& WithRequestId(RequestIdT&& value) {
