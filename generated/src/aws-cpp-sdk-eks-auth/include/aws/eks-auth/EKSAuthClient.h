...
class AWS_EKSAUTH_API EKSAuthClient : public Aws::Client::AWSJsonClient,
 ...
 public:
   ...
   */
  EKSAuthClient(const Aws::Auth::AWSCredentials& credentials, std::shared_ptr<EKSAuthEndpointProviderBase> endpointProvider = nullptr,
  ...
  /**
   * A Callable wrapper for AssumeRoleForPodIdentity that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for AssumeRoleForPodIdentity that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename AssumeRoleForPodIdentityRequestT = Model::AssumeRoleForPodIdentityRequest>
  void AssumeRoleForPodIdentityAsync(const AssumeRoleForPodIdentityRequestT& request,
                                     const AssumeRoleForPodIdentityResponseReceivedHandler& handler,
                                     const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
    return SubmitAsync(&EKSAuthClient::AssumeRoleForPodIdentity, request, handler, context);
  }

  virtual void OverrideEndpoint(const Aws::String& endpoint);
  virtual std::shared_ptr<EKSAuthEndpointProviderBase>& accessEndpointProvider();
