...
EKSAuthClient::EKSAuthClient(const AWSCredentials& credentials, std::shared_ptr<EKSAuthEndpointProviderBase> endpointProvider,
...
EKSAuthClient::EKSAuthClient(const Aws::Client::ClientConfiguration& clientConfiguration)
    : BASECLASS(clientConfiguration,
                Aws::MakeShared<AWSAuthV4Signer>(
                    ALLOCATION_TAG,
                    Aws::MakeShared<DefaultAWSCredentialsProviderChain>(ALLOCATION_TAG, clientConfiguration.credentialProviderConfig),
...
EKSAuthClient::EKSAuthClient(const AWSCredentials& credentials, const Aws::Client::ClientConfiguration& clientConfiguration)
    : BASECLASS(clientConfiguration,
                Aws::MakeShared<AWSAuthV4Signer>(ALLOCATION_TAG, Aws::MakeShared<SimpleAWSCredentialsProvider>(ALLOCATION_TAG, credentials),
                                                 SERVICE_NAME, Aws::Region::ComputeSignerRegion(clientConfiguration.region)),
                Aws::MakeShared<EKSAuthErrorMarshaller>(ALLOCATION_TAG)),
...
EKSAuthClient::~EKSAuthClient() { ShutdownSdkClient(this, -1); }

std::shared_ptr<EKSAuthEndpointProviderBase>& EKSAuthClient::accessEndpointProvider() { return m_endpointProvider; }
...
EKSAuthClient::InvokeOperationOutcome EKSAuthClient::InvokeServiceOperation(
...
AssumeRoleForPodIdentityOutcome EKSAuthClient::AssumeRoleForPodIdentity(const AssumeRoleForPodIdentityRequest& request) const {
  if (!request.ClusterNameHasBeenSet()) {
    ...
    return AssumeRoleForPodIdentityOutcome(Aws::Client::AWSError<EKSAuthErrors>(EKSAuthErrors::MISSING_PARAMETER, "MISSING_PARAMETER",
  ...
  return TracingUtils::MakeCallWithTiming<AssumeRoleForPodIdentityOutcome>(
      [&]() -> AssumeRoleForPodIdentityOutcome {
        auto endpointResolutionOutcome = TracingUtils::MakeCallWithTiming<ResolveEndpointOutcome>(
            ...
            {{TracingUtils::SMITHY_METHOD_DIMENSION, request.GetServiceRequestName()},
             {TracingUtils::SMITHY_SERVICE_DIMENSION, this->GetServiceClientName()}});
        AWS_OPERATION_CHECK_SUCCESS(endpointResolutionOutcome, AssumeRoleForPodIdentity, CoreErrors,
        ...
        return result.IsSuccess() ? AssumeRoleForPodIdentityOutcome(AssumeRoleForPodIdentityResult(result.GetResultWithOwnership()))
                                  : AssumeRoleForPodIdentityOutcome(result.GetError());
      },
      TracingUtils::SMITHY_CLIENT_DURATION_METRIC, *meter,
      {{TracingUtils::SMITHY_METHOD_DIMENSION, request.GetServiceRequestName()},
       {TracingUtils::SMITHY_SERVICE_DIMENSION, this->GetServiceClientName()}});
