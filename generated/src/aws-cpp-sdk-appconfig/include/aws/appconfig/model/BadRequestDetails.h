...
class BadRequestDetails {
 public:
  ...
  void SetInvalidConfiguration(InvalidConfigurationT&& value) {
    m_invalidConfigurationHasBeenSet = true;
    m_invalidConfiguration = std::forward<InvalidConfigurationT>(value);
  }
  template <typename InvalidConfigurationT = Aws::Vector<InvalidConfigurationDetail>>
  BadRequestDetails& WithInvalidConfiguration(InvalidConfigurationT&& value) {
    SetInvalidConfiguration(std::forward<InvalidConfigurationT>(value));
    return *this;
  }
  template <typename InvalidConfigurationT = InvalidConfigurationDetail>
  BadRequestDetails& AddInvalidConfiguration(InvalidConfigurationT&& value) {
    m_invalidConfigurationHasBeenSet = true;
    m_invalidConfiguration.emplace_back(std::forward<InvalidConfigurationT>(value));
    return *this;
  }
  ///@}
 private:
