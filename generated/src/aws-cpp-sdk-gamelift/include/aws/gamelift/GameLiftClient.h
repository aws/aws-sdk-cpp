...
class AWS_GAMELIFT_API GameLiftClient : public Aws::Client::AWSJsonClient,
 ...
 public:
   ...
   */
  GameLiftClient(const Aws::Auth::AWSCredentials& credentials, std::shared_ptr<GameLiftEndpointProviderBase> endpointProvider = nullptr,
   ...
   * Container</p> <p>Registers a player's acceptance or rejection of a proposed
   * FlexMatch match. A matchmaking configuration may require player acceptance; if
   * so, then matches built with that configuration cannot be completed unless all
   * players accept the proposed match within a specified time limit. </p> <p>When
   * FlexMatch builds a match, all the matchmaking tickets involved in the proposed
   * match are placed into status <code>REQUIRES_ACCEPTANCE</code>. This is a trigger
   ...
   * in this status result in an error.</p> <p>To register acceptance, specify the
   * ticket ID, one or more players, and an acceptance response. When all players
   * have accepted, Amazon GameLift Servers advances the matchmaking tickets to
   * status <code>PLACING</code>, and attempts to create a new game session for the
   * match. </p> <p>If any player rejects the match, or if acceptances are not
  ...
  /**
   * An Async wrapper for AcceptMatch that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename AcceptMatchRequestT = Model::AcceptMatchRequest>
  void AcceptMatchAsync(const AcceptMatchRequestT& request, const AcceptMatchResponseReceivedHandler& handler,
   ...
   * <p>Locates an available game server and temporarily reserves it to host gameplay
   ...
   * ID, although this approach bypasses Amazon GameLift Servers FleetIQ placement
   * optimization. Optionally, include game data to pass to the game server at the
   * start of a game session, such as a game map or player information. Add filter
   * options to further restrict how a game server is chosen, such as only allowing
   * game servers on <code>ACTIVE</code> instances to be claimed.</p> <p>When a game
   * server is successfully claimed, connection information is returned. A claimed
   * game server's utilization status remains <code>AVAILABLE</code> while the claim
   ...
   * server claim status is <code>CLAIMED</code>.</p> </li> <li> <p>If the game
   * server is running on an instance in <code>DRAINING</code> status and the
  ...
  /**
   * An Async wrapper for ClaimGameServer that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ClaimGameServerRequestT = Model::ClaimGameServerRequest>
  void ClaimGameServerAsync(const ClaimGameServerRequestT& request, const ClaimGameServerResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Creates an alias for a fleet. In most situations, you can use
   * an alias ID in place of a fleet ID. An alias provides a level of abstraction for
   * a fleet that is useful when redirecting player traffic from one fleet to
   * another, such as when updating your game build. </p> <p>Amazon GameLift Servers
   * supports two types of routing strategies for aliases: simple and terminal. A
   * simple alias points to an active fleet. A terminal alias is used to display
   * messaging or link to a URL instead of routing players to an active fleet. For
   * example, you might use a terminal alias when a game version is no longer
   * supported and you want to direct players to an upgrade site. </p> <p>To create a
   * fleet alias, specify an alias name, routing strategy, and optional description.
   * Each simple alias can point to only one fleet, but a fleet can have multiple
   * aliases. If successful, a new alias record is returned, including an alias ID
   * and an ARN. You can reassign an alias to another fleet by calling
   * <code>UpdateAlias</code>.</p> <p> <b>Related actions</b> </p> <p> <a
  ...
  /**
   * An Async wrapper for CreateBuild that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename CreateBuildRequestT = Model::CreateBuildRequest>
  void CreateBuildAsync(const CreateBuildResponseReceivedHandler& handler,
                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                        const CreateBuildRequestT& request = {}) const {
   ...
   * definitions, and deploy the update across all fleet instances. Use <a
   * href="https://docs.aws.amazon.com/gamelift/latest/apireference/API_UpdateContainerFleet.html">UpdateContainerFleet</a>
   ...
   * Image (AMI) version it uses. When a new fleet is created, Amazon GameLift
   * Servers assigns the latest available AMI version to the fleet, and all compute
   * instances in that fleet are deployed with that version. To update the AMI
   * version, you must create a new fleet. As a best practice, we recommend replacing
   * your managed fleets every 30 days to maintain a secure and up-to-date runtime
   * environment for your hosted game servers. For guidance, see <a
   * href="https://docs.aws.amazon.com/gameliftservers/latest/developerguide/security-best-practices.html">
   * Security best practices for Amazon GameLift Servers</a>.</p> <p><h3>See
   * Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for CreateContainerFleet that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateContainerFleet that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateContainerFleetRequestT = Model::CreateContainerFleetRequest>
  void CreateContainerFleetAsync(const CreateContainerFleetRequestT& request, const CreateContainerFleetResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::CreateContainerGroupDefinitionOutcome CreateContainerGroupDefinition(
      const Model::CreateContainerGroupDefinitionRequest& request) const;

  /**
   * A Callable wrapper for CreateContainerGroupDefinition that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename CreateContainerGroupDefinitionRequestT = Model::CreateContainerGroupDefinitionRequest>
  Model::CreateContainerGroupDefinitionOutcomeCallable CreateContainerGroupDefinitionCallable(
      const CreateContainerGroupDefinitionRequestT& request) const {
    return SubmitCallable(&GameLiftClient::CreateContainerGroupDefinition, request);
  }

  /**
   * An Async wrapper for CreateContainerGroupDefinition that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename CreateContainerGroupDefinitionRequestT = Model::CreateContainerGroupDefinitionRequest>
  void CreateContainerGroupDefinitionAsync(const CreateContainerGroupDefinitionRequestT& request,
                                           const CreateContainerGroupDefinitionResponseReceivedHandler& handler,
                                           const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Creates a fleet of compute resources to host your game servers.
   * Use this operation to set up a fleet for the following compute types: </p> <p>
   * <b>Managed EC2 fleet</b> </p> <p>An EC2 fleet is a set of Amazon Elastic Compute
   * Cloud (Amazon EC2) instances. Your game server build is deployed to each fleet
   * instance. Amazon GameLift Servers manages the fleet's instances and controls the
   ...
   * for each location.</p>  <p>A managed fleet's runtime environment depends
   * on the Amazon Machine Image (AMI) version it uses. When a new fleet is created,
   * Amazon GameLift Servers assigns the latest available AMI version to the fleet,
   * and all compute instances in that fleet are deployed with that version. To
   * update the AMI version, you must create a new fleet. As a best practice, we
   * recommend replacing your managed fleets every 30 days to maintain a secure and
   * up-to-date runtime environment for your hosted game servers. For guidance, see
   * <a
   * href="https://docs.aws.amazon.com/gameliftservers/latest/developerguide/security-best-practices.html">
   ...
   * that are not owned or managed by Amazon GameLift Servers. You might create an
   * Anywhere fleet with your local machine for testing, or use one to host game
   * servers with on-premises hardware or other game hosting solutions. </p> <p>To
   * create an Anywhere fleet, provide these required parameters:</p> <ul> <li> <p>
  ...
  /**
   * A Callable wrapper for CreateFleetLocations that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateFleetLocations that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateFleetLocationsRequestT = Model::CreateFleetLocationsRequest>
  void CreateFleetLocationsAsync(const CreateFleetLocationsRequestT& request, const CreateFleetLocationsResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Creates a Amazon GameLift Servers FleetIQ game server group for managing game
   ...
   * Amazon CloudWatch.</p> <p>Before creating a new game server group, you must have
   * the following: </p> <ul> <li> <p>An Amazon Elastic Compute Cloud launch template
  ...
  /**
   * A Callable wrapper for CreateGameServerGroup that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateGameServerGroup that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateGameServerGroupRequestT = Model::CreateGameServerGroupRequest>
  void CreateGameServerGroupAsync(const CreateGameServerGroupRequestT& request, const CreateGameServerGroupResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Creates a multiplayer game session for players in a specific
   * fleet location. This operation prompts an available server process to start a
   * game session and retrieves connection information for the new game session. As
   * an alternative, consider using the Amazon GameLift Servers game session
   * placement feature with <a
  ...
  /**
   * A Callable wrapper for CreateGameSession that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateGameSession that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename CreateGameSessionRequestT = Model::CreateGameSessionRequest>
  void CreateGameSessionAsync(const CreateGameSessionRequestT& request, const CreateGameSessionResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Creates a placement queue that processes requests for new game
   * sessions. A queue uses FleetIQ algorithms to locate the best available placement
   * locations for a new game session, and then prompts the game server process to
   * start a new game session.</p> <p>A game session queue is configured with a set
   * of destinations (Amazon GameLift Servers fleets or aliases) that determine where
  ...
  /**
   * A Callable wrapper for CreateGameSessionQueue that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateGameSessionQueue that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateGameSessionQueueRequestT = Model::CreateGameSessionQueueRequest>
  void CreateGameSessionQueueAsync(const CreateGameSessionQueueRequestT& request,
  ...
  /**
   * An Async wrapper for CreateLocation that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename CreateLocationRequestT = Model::CreateLocationRequest>
  void CreateLocationAsync(const CreateLocationRequestT& request, const CreateLocationResponseReceivedHandler& handler,
                           const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * system can use multiple configurations to handle different game scenarios. All
   ...
   * API Reference</a></p>
   */
  virtual Model::CreateMatchmakingConfigurationOutcome CreateMatchmakingConfiguration(
      const Model::CreateMatchmakingConfigurationRequest& request) const;

  /**
   * A Callable wrapper for CreateMatchmakingConfiguration that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename CreateMatchmakingConfigurationRequestT = Model::CreateMatchmakingConfigurationRequest>
  Model::CreateMatchmakingConfigurationOutcomeCallable CreateMatchmakingConfigurationCallable(
      const CreateMatchmakingConfigurationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::CreateMatchmakingConfiguration, request);
  }

  /**
   * An Async wrapper for CreateMatchmakingConfiguration that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename CreateMatchmakingConfigurationRequestT = Model::CreateMatchmakingConfigurationRequest>
  void CreateMatchmakingConfigurationAsync(const CreateMatchmakingConfigurationRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Creates a new rule set for FlexMatch matchmaking. A rule set
   * describes the type of match to create, such as the number and size of teams. It
   * also sets the parameters for acceptable player matches, such as minimum skill
   * level or character type.</p> <p>To create a matchmaking rule set, provide unique
   * rule set name and the rule set body in JSON format. Rule sets must be defined in
   * the same Region as the matchmaking configuration they are used with.</p>
   * <p>Since matchmaking rule sets cannot be edited, it is a good idea to check the
   * rule set syntax using <a
  ...
  /**
   * A Callable wrapper for CreateMatchmakingRuleSet that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateMatchmakingRuleSet that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateMatchmakingRuleSetRequestT = Model::CreateMatchmakingRuleSetRequest>
  void CreateMatchmakingRuleSetAsync(const CreateMatchmakingRuleSetRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Reserves an open player slot in a game session for a player.
   * New player sessions can be created in any game session with an open slot that is
   * in <code>ACTIVE</code> status and has a player creation policy of
  ...
  /**
   * A Callable wrapper for CreatePlayerSession that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreatePlayerSession that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename CreatePlayerSessionRequestT = Model::CreatePlayerSessionRequest>
  void CreatePlayerSessionAsync(const CreatePlayerSessionRequestT& request, const CreatePlayerSessionResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Reserves open slots in a game session for a group of players.
   * New player sessions can be created in any game session with an open slot that is
   * in <code>ACTIVE</code> status and has a player creation policy of
  ...
  /**
   * A Callable wrapper for CreatePlayerSessions that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreatePlayerSessions that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreatePlayerSessionsRequestT = Model::CreatePlayerSessionsRequest>
  void CreatePlayerSessionsAsync(const CreatePlayerSessionsRequestT& request, const CreatePlayerSessionsResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Requests
   * authorization to create or delete a peer connection between the VPC for your
  ...
  virtual Model::CreateVpcPeeringAuthorizationOutcome CreateVpcPeeringAuthorization(
      const Model::CreateVpcPeeringAuthorizationRequest& request) const;

  /**
   * A Callable wrapper for CreateVpcPeeringAuthorization that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename CreateVpcPeeringAuthorizationRequestT = Model::CreateVpcPeeringAuthorizationRequest>
  Model::CreateVpcPeeringAuthorizationOutcomeCallable CreateVpcPeeringAuthorizationCallable(
      const CreateVpcPeeringAuthorizationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::CreateVpcPeeringAuthorization, request);
  }

  /**
   * An Async wrapper for CreateVpcPeeringAuthorization that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateVpcPeeringAuthorizationRequestT = Model::CreateVpcPeeringAuthorizationRequest>
  void CreateVpcPeeringAuthorizationAsync(const CreateVpcPeeringAuthorizationRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Establishes
   * a VPC peering connection between a virtual private cloud (VPC) in an Amazon Web
   * Services account with the VPC for your Amazon GameLift Servers fleet. VPC
   ...
   * manage your Amazon GameLift Servers fleets. You cannot peer with VPCs that are
   ...
   * API Reference</a></p>
   */
  virtual Model::CreateVpcPeeringConnectionOutcome CreateVpcPeeringConnection(
      const Model::CreateVpcPeeringConnectionRequest& request) const;

  /**
   * A Callable wrapper for CreateVpcPeeringConnection that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename CreateVpcPeeringConnectionRequestT = Model::CreateVpcPeeringConnectionRequest>
  Model::CreateVpcPeeringConnectionOutcomeCallable CreateVpcPeeringConnectionCallable(
      const CreateVpcPeeringConnectionRequestT& request) const {
    return SubmitCallable(&GameLiftClient::CreateVpcPeeringConnection, request);
  }

  /**
   * An Async wrapper for CreateVpcPeeringConnection that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateVpcPeeringConnectionRequestT = Model::CreateVpcPeeringConnectionRequest>
  void CreateVpcPeeringConnectionAsync(const CreateVpcPeeringConnectionRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Deletes an alias. This operation removes all record of the
   * alias. Game clients attempting to access a server process using the deleted
   * alias receive an error. To delete an alias, specify the alias ID to be
   * deleted.</p> <p> <b>Related actions</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
   * APIs by task</a> </p><p><h3>See Also:</h3>   <a
   ...
   * build. This operation permanently deletes the build resource and any uploaded
   * build files. Deleting a build does not affect the status of any active fleets
  ...
  /**
   * An Async wrapper for DeleteBuild that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename DeleteBuildRequestT = Model::DeleteBuildRequest>
  void DeleteBuildAsync(const DeleteBuildRequestT& request, const DeleteBuildResponseReceivedHandler& handler,
                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for DeleteContainerFleet that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeleteContainerFleet that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteContainerFleetRequestT = Model::DeleteContainerFleetRequest>
  void DeleteContainerFleetAsync(const DeleteContainerFleetRequestT& request, const DeleteContainerFleetResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
    return SubmitAsync(&GameLiftClient::DeleteContainerFleet, request, handler, context);
  }

  /**
   * <p> <b>This API works with the following fleet types:</b> Container</p> <p>
   * <b>Request options:</b> </p> <p>Deletes a container group definition. </p> <ul>
   * <li> <p>Delete an entire container group definition, including all versions.
   * Specify the container group definition name, or use an ARN value without the
   * version number.</p> </li> <li> <p>Delete a particular version. Specify the
   * container group definition name and a version number, or use an ARN value that
   * includes the version number.</p> </li> <li> <p>Keep the newest versions and
   * delete all older versions. Specify the container group definition name and the
   * number of versions to retain. For example, set <code>VersionCountToRetain</code>
   * to 5 to delete all but the five most recent versions.</p> </li> </ul> <p>
   * <b>Result</b> </p> <p>If successful, Amazon GameLift Servers removes the
   * container group definition versions that you request deletion for. This request
   * will fail for any requested versions if the following is true: </p> <ul> <li>
   * <p>If the version is being used in an active fleet</p> </li> <li> <p>If the
   * version is being deployed to a fleet in a deployment that's currently in
   * progress.</p> </li> <li> <p>If the version is designated as a rollback
   * definition in a fleet deployment that's currently in progress.</p> </li> </ul>
   ...
   * API Reference</a></p>
   */
  virtual Model::DeleteContainerGroupDefinitionOutcome DeleteContainerGroupDefinition(
      const Model::DeleteContainerGroupDefinitionRequest& request) const;

  /**
   * A Callable wrapper for DeleteContainerGroupDefinition that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DeleteContainerGroupDefinitionRequestT = Model::DeleteContainerGroupDefinitionRequest>
  Model::DeleteContainerGroupDefinitionOutcomeCallable DeleteContainerGroupDefinitionCallable(
      const DeleteContainerGroupDefinitionRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DeleteContainerGroupDefinition, request);
  }

  /**
   * An Async wrapper for DeleteContainerGroupDefinition that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DeleteContainerGroupDefinitionRequestT = Model::DeleteContainerGroupDefinitionRequest>
  void DeleteContainerGroupDefinitionAsync(const DeleteContainerGroupDefinitionRequestT& request,
                                           const DeleteContainerGroupDefinitionResponseReceivedHandler& handler,
                                           const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for DeleteFleetLocations that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeleteFleetLocations that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteFleetLocationsRequestT = Model::DeleteFleetLocationsRequest>
  void DeleteFleetLocationsAsync(const DeleteFleetLocationsRequestT& request, const DeleteFleetLocationsResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Terminates a game server group and permanently deletes the game server group
   ...
   * selected, this operation might affect these resources:</p> <ul> <li> <p>The game
   * server group</p> </li> <li> <p>The corresponding Auto Scaling group</p> </li>
   * <li> <p>All game servers that are currently running in the group</p> </li> </ul>
   * <p>To delete a game server group, identify the game server group to delete and
   * specify the type of delete operation to initiate. Game server groups can only be
   * deleted if they are in <code>ACTIVE</code> or <code>ERROR</code> status.</p>
   * <p>If the delete request is successful, a series of operations are kicked off.
   * The game server group status is changed to <code>DELETE_SCHEDULED</code>, which
   ...
   * Amazon GameLift Servers FleetIQ can begin deleting resources. If any of the
   * delete operations fail, the game server group is placed in <code>ERROR</code>
   * status.</p> <p>Amazon GameLift Servers FleetIQ emits delete events to Amazon
   * CloudWatch.</p> <p> <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/fleetiqguide/gsg-intro.html">Amazon
   * GameLift Servers FleetIQ Guide</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for DeleteGameServerGroup that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeleteGameServerGroup that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteGameServerGroupRequestT = Model::DeleteGameServerGroupRequest>
  void DeleteGameServerGroupAsync(const DeleteGameServerGroupRequestT& request, const DeleteGameServerGroupResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Deletes a game session queue. Once a queue is successfully
   * deleted, unfulfilled <a
   * href="https://docs.aws.amazon.com/gamelift/latest/apireference/API_StartGameSessionPlacement.html">StartGameSessionPlacement</a>
  ...
  /**
   * A Callable wrapper for DeleteGameSessionQueue that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeleteGameSessionQueue that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteGameSessionQueueRequestT = Model::DeleteGameSessionQueueRequest>
  void DeleteGameSessionQueueAsync(const DeleteGameSessionQueueRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Permanently removes a FlexMatch matchmaking configuration. To
   * delete, specify the configuration name. A matchmaking configuration cannot be
   * deleted if it is being used in any active matchmaking tickets.</p><p><h3>See
   * Also:</h3>   <a
   * href="http://docs.aws.amazon.com/goto/WebAPI/gamelift-2015-10-01/DeleteMatchmakingConfiguration">AWS
   * API Reference</a></p>
   */
  virtual Model::DeleteMatchmakingConfigurationOutcome DeleteMatchmakingConfiguration(
      const Model::DeleteMatchmakingConfigurationRequest& request) const;

  /**
   * A Callable wrapper for DeleteMatchmakingConfiguration that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DeleteMatchmakingConfigurationRequestT = Model::DeleteMatchmakingConfigurationRequest>
  Model::DeleteMatchmakingConfigurationOutcomeCallable DeleteMatchmakingConfigurationCallable(
      const DeleteMatchmakingConfigurationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DeleteMatchmakingConfiguration, request);
  }

  /**
   * An Async wrapper for DeleteMatchmakingConfiguration that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DeleteMatchmakingConfigurationRequestT = Model::DeleteMatchmakingConfigurationRequest>
  void DeleteMatchmakingConfigurationAsync(const DeleteMatchmakingConfigurationRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Deletes an existing matchmaking rule set. To delete the rule
   * set, provide the rule set name. Rule sets cannot be deleted if they are
   * currently being used by a matchmaking configuration. </p> <p> <b>Learn more</b>
   * </p> <ul> <li> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/flexmatchguide/match-rulesets.html">Build
  ...
  /**
   * A Callable wrapper for DeleteMatchmakingRuleSet that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeleteMatchmakingRuleSet that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteMatchmakingRuleSetRequestT = Model::DeleteMatchmakingRuleSetRequest>
  void DeleteMatchmakingRuleSetAsync(const DeleteMatchmakingRuleSetRequestT& request,
  ...
  /**
   * A Callable wrapper for DeleteScalingPolicy that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeleteScalingPolicy that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DeleteScalingPolicyRequestT = Model::DeleteScalingPolicyRequest>
  void DeleteScalingPolicyAsync(const DeleteScalingPolicyRequestT& request, const DeleteScalingPolicyResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Deletes a
   * Realtime script. This operation permanently deletes the script record. If script
   * files were uploaded, they are also deleted (files stored in an S3 bucket are not
   * deleted). </p> <p>To delete a script, specify the script ID. Before deleting a
  ...
  /**
   * An Async wrapper for DeleteScript that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename DeleteScriptRequestT = Model::DeleteScriptRequest>
  void DeleteScriptAsync(const DeleteScriptRequestT& request, const DeleteScriptResponseReceivedHandler& handler,
                         const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
    return SubmitAsync(&GameLiftClient::DeleteScript, request, handler, context);
  }

  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Cancels a
   * pending VPC peering authorization for the specified VPC. If you need to delete
   * an existing VPC peering connection, use <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DeleteVpcPeeringAuthorizationOutcome DeleteVpcPeeringAuthorization(
      const Model::DeleteVpcPeeringAuthorizationRequest& request) const;

  /**
   * A Callable wrapper for DeleteVpcPeeringAuthorization that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DeleteVpcPeeringAuthorizationRequestT = Model::DeleteVpcPeeringAuthorizationRequest>
  Model::DeleteVpcPeeringAuthorizationOutcomeCallable DeleteVpcPeeringAuthorizationCallable(
      const DeleteVpcPeeringAuthorizationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DeleteVpcPeeringAuthorization, request);
  }

  /**
   * An Async wrapper for DeleteVpcPeeringAuthorization that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteVpcPeeringAuthorizationRequestT = Model::DeleteVpcPeeringAuthorizationRequest>
  void DeleteVpcPeeringAuthorizationAsync(const DeleteVpcPeeringAuthorizationRequestT& request,
   ...
   * API Reference</a></p>
   */
  virtual Model::DeleteVpcPeeringConnectionOutcome DeleteVpcPeeringConnection(
      const Model::DeleteVpcPeeringConnectionRequest& request) const;

  /**
   * A Callable wrapper for DeleteVpcPeeringConnection that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DeleteVpcPeeringConnectionRequestT = Model::DeleteVpcPeeringConnectionRequest>
  Model::DeleteVpcPeeringConnectionOutcomeCallable DeleteVpcPeeringConnectionCallable(
      const DeleteVpcPeeringConnectionRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DeleteVpcPeeringConnection, request);
  }

  /**
   * An Async wrapper for DeleteVpcPeeringConnection that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteVpcPeeringConnectionRequestT = Model::DeleteVpcPeeringConnectionRequest>
  void DeleteVpcPeeringConnectionAsync(const DeleteVpcPeeringConnectionRequestT& request,
  ...
  /**
   * A Callable wrapper for DeregisterCompute that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeregisterCompute that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DeregisterComputeRequestT = Model::DeregisterComputeRequest>
  void DeregisterComputeAsync(const DeregisterComputeRequestT& request, const DeregisterComputeResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Removes the game server from a game server group. As a result of this
   * operation, the deregistered game server can no longer be claimed and will not be
   * returned in a list of active game servers. </p> <p>To deregister a game server,
   * specify the game server group and game server ID. If successful, this operation
   * emits a CloudWatch event with termination timestamp and reason.</p> <p> <b>Learn
   * more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/fleetiqguide/gsg-intro.html">Amazon
   * GameLift Servers FleetIQ Guide</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for DeregisterGameServer that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeregisterGameServer that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeregisterGameServerRequestT = Model::DeregisterGameServerRequest>
  void DeregisterGameServerAsync(const DeregisterGameServerRequestT& request, const DeregisterGameServerResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves properties for an alias. This operation returns all
   * alias metadata and settings. To get an alias's target fleet ID only, use
  ...
  /**
   * An Async wrapper for DescribeAlias that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename DescribeAliasRequestT = Model::DescribeAliasRequest>
  void DescribeAliasAsync(const DescribeAliasRequestT& request, const DescribeAliasResponseReceivedHandler& handler,
                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * properties for a custom game build. To request a build resource, specify a build
  ...
  /**
   * An Async wrapper for DescribeBuild that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename DescribeBuildRequestT = Model::DescribeBuildRequest>
  void DescribeBuildAsync(const DescribeBuildRequestT& request, const DescribeBuildResponseReceivedHandler& handler,
                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves properties for a specific compute resource in an
   * Amazon GameLift Servers fleet. You can list all computes in a fleet by calling
   * <a
  ...
  /**
   * An Async wrapper for DescribeCompute that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename DescribeComputeRequestT = Model::DescribeComputeRequest>
  void DescribeComputeAsync(const DescribeComputeRequestT& request, const DescribeComputeResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for DescribeContainerFleet that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeContainerFleet that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeContainerFleetRequestT = Model::DescribeContainerFleetRequest>
  void DescribeContainerFleetAsync(const DescribeContainerFleetRequestT& request,
                                   const DescribeContainerFleetResponseReceivedHandler& handler,
                                   const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  virtual Model::DescribeContainerGroupDefinitionOutcome DescribeContainerGroupDefinition(
      const Model::DescribeContainerGroupDefinitionRequest& request) const;

  /**
   * A Callable wrapper for DescribeContainerGroupDefinition that returns a future to the operation so that it can be executed in parallel
   * to other requests.
   */
  template <typename DescribeContainerGroupDefinitionRequestT = Model::DescribeContainerGroupDefinitionRequest>
  Model::DescribeContainerGroupDefinitionOutcomeCallable DescribeContainerGroupDefinitionCallable(
      const DescribeContainerGroupDefinitionRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeContainerGroupDefinition, request);
  }

  /**
   * An Async wrapper for DescribeContainerGroupDefinition that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DescribeContainerGroupDefinitionRequestT = Model::DescribeContainerGroupDefinitionRequest>
  void DescribeContainerGroupDefinitionAsync(const DescribeContainerGroupDefinitionRequestT& request,
                                             const DescribeContainerGroupDefinitionResponseReceivedHandler& handler,
                                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * per location, that your Amazon Web Services account can use. Learn more at <a
   ...
   * ability to scale your Amazon GameLift Servers fleets. You can request a limit
   * increase for your account by using the <b>Service limits</b> page in the Amazon
   * GameLift Servers console.</p> <p>Instance limits differ based on whether the
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeEC2InstanceLimitsOutcome DescribeEC2InstanceLimits(
      const Model::DescribeEC2InstanceLimitsRequest& request = {}) const;

  /**
   * A Callable wrapper for DescribeEC2InstanceLimits that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename DescribeEC2InstanceLimitsRequestT = Model::DescribeEC2InstanceLimitsRequest>
  Model::DescribeEC2InstanceLimitsOutcomeCallable DescribeEC2InstanceLimitsCallable(
      const DescribeEC2InstanceLimitsRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeEC2InstanceLimits, request);
  }

  /**
   * An Async wrapper for DescribeEC2InstanceLimits that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeEC2InstanceLimitsRequestT = Model::DescribeEC2InstanceLimitsRequest>
  void DescribeEC2InstanceLimitsAsync(const DescribeEC2InstanceLimitsResponseReceivedHandler& handler,
                                      const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                      const DescribeEC2InstanceLimitsRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for DescribeFleetAttributes that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeFleetAttributes that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeFleetAttributesRequestT = Model::DescribeFleetAttributesRequest>
  void DescribeFleetAttributesAsync(const DescribeFleetAttributesResponseReceivedHandler& handler,
                                    const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                    const DescribeFleetAttributesRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for DescribeFleetCapacity that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeFleetCapacity that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeFleetCapacityRequestT = Model::DescribeFleetCapacityRequest>
  void DescribeFleetCapacityAsync(const DescribeFleetCapacityResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                  const DescribeFleetCapacityRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for DescribeFleetDeployment that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeFleetDeployment that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeFleetDeploymentRequestT = Model::DescribeFleetDeploymentRequest>
  void DescribeFleetDeploymentAsync(const DescribeFleetDeploymentRequestT& request,
                                    const DescribeFleetDeploymentResponseReceivedHandler& handler,
                                    const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves entries from a fleet's event log. Fleet events are
   * initiated by changes in status, such as during fleet creation and termination,
   * changes in capacity, etc. If a fleet has multiple locations, events are also
   * initiated by changes to status and capacity in remote locations.</p> <p>You can
   * specify a time range to limit the result set. Use the pagination parameters to
   * retrieve results as a set of sequential pages. </p> <p>If successful, a
   * collection of event log entries matching the request are returned.</p> <p>
   * <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/fleets-intro.html">Setting
   * up Amazon GameLift Servers fleets</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for DescribeFleetEvents that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeFleetEvents that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DescribeFleetEventsRequestT = Model::DescribeFleetEventsRequest>
  void DescribeFleetEventsAsync(const DescribeFleetEventsRequestT& request, const DescribeFleetEventsResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  virtual Model::DescribeFleetLocationAttributesOutcome DescribeFleetLocationAttributes(
      const Model::DescribeFleetLocationAttributesRequest& request) const;

  /**
   * A Callable wrapper for DescribeFleetLocationAttributes that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeFleetLocationAttributesRequestT = Model::DescribeFleetLocationAttributesRequest>
  Model::DescribeFleetLocationAttributesOutcomeCallable DescribeFleetLocationAttributesCallable(
      const DescribeFleetLocationAttributesRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeFleetLocationAttributes, request);
  }

  /**
   * An Async wrapper for DescribeFleetLocationAttributes that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DescribeFleetLocationAttributesRequestT = Model::DescribeFleetLocationAttributesRequest>
  void DescribeFleetLocationAttributesAsync(const DescribeFleetLocationAttributesRequestT& request,
  ...
  virtual Model::DescribeFleetLocationCapacityOutcome DescribeFleetLocationCapacity(
      const Model::DescribeFleetLocationCapacityRequest& request) const;

  /**
   * A Callable wrapper for DescribeFleetLocationCapacity that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeFleetLocationCapacityRequestT = Model::DescribeFleetLocationCapacityRequest>
  Model::DescribeFleetLocationCapacityOutcomeCallable DescribeFleetLocationCapacityCallable(
      const DescribeFleetLocationCapacityRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeFleetLocationCapacity, request);
  }

  /**
   * An Async wrapper for DescribeFleetLocationCapacity that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeFleetLocationCapacityRequestT = Model::DescribeFleetLocationCapacityRequest>
  void DescribeFleetLocationCapacityAsync(const DescribeFleetLocationCapacityRequestT& request,
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeFleetLocationUtilizationOutcome DescribeFleetLocationUtilization(
      const Model::DescribeFleetLocationUtilizationRequest& request) const;

  /**
   * A Callable wrapper for DescribeFleetLocationUtilization that returns a future to the operation so that it can be executed in parallel
   * to other requests.
   */
  template <typename DescribeFleetLocationUtilizationRequestT = Model::DescribeFleetLocationUtilizationRequest>
  Model::DescribeFleetLocationUtilizationOutcomeCallable DescribeFleetLocationUtilizationCallable(
      const DescribeFleetLocationUtilizationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeFleetLocationUtilization, request);
  }

  /**
   * An Async wrapper for DescribeFleetLocationUtilization that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DescribeFleetLocationUtilizationRequestT = Model::DescribeFleetLocationUtilizationRequest>
  void DescribeFleetLocationUtilizationAsync(const DescribeFleetLocationUtilizationRequestT& request,
  ...
  /**
   * A Callable wrapper for DescribeFleetPortSettings that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename DescribeFleetPortSettingsRequestT = Model::DescribeFleetPortSettingsRequest>
  Model::DescribeFleetPortSettingsOutcomeCallable DescribeFleetPortSettingsCallable(
      const DescribeFleetPortSettingsRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeFleetPortSettings, request);
  }

  /**
   * An Async wrapper for DescribeFleetPortSettings that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeFleetPortSettingsRequestT = Model::DescribeFleetPortSettingsRequest>
  void DescribeFleetPortSettingsAsync(const DescribeFleetPortSettingsRequestT& request,
  ...
  /**
   * A Callable wrapper for DescribeFleetUtilization that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename DescribeFleetUtilizationRequestT = Model::DescribeFleetUtilizationRequest>
  Model::DescribeFleetUtilizationOutcomeCallable DescribeFleetUtilizationCallable(
      const DescribeFleetUtilizationRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeFleetUtilization, request);
  }

  /**
   * An Async wrapper for DescribeFleetUtilization that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeFleetUtilizationRequestT = Model::DescribeFleetUtilizationRequest>
  void DescribeFleetUtilizationAsync(const DescribeFleetUtilizationResponseReceivedHandler& handler,
                                     const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                     const DescribeFleetUtilizationRequestT& request = {}) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Retrieves information for a registered game server. Information includes game
   * server status, health check info, and the instance that the game server is
   * running on. </p> <p>To retrieve game server information, specify the game server
   * ID. If successful, the requested game server object is returned. </p> <p>
   * <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/fleetiqguide/gsg-intro.html">Amazon
   * GameLift Servers FleetIQ Guide</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for DescribeGameServer that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeGameServer that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DescribeGameServerRequestT = Model::DescribeGameServerRequest>
  void DescribeGameServerAsync(const DescribeGameServerRequestT& request, const DescribeGameServerResponseReceivedHandler& handler,
                               const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Retrieves information on a game server group. This operation returns only
  ...
  /**
   * A Callable wrapper for DescribeGameServerGroup that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeGameServerGroup that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeGameServerGroupRequestT = Model::DescribeGameServerGroupRequest>
  void DescribeGameServerGroupAsync(const DescribeGameServerGroupRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Retrieves status information about the Amazon EC2 instances associated with a
   * Amazon GameLift Servers FleetIQ game server group. Use this operation to detect
   * when instances are active or not available to host new game servers.</p> <p>To
   * request status for all instances in the game server group, provide a game server
   * group ID only. To request status for specific instances, provide the game server
   * group ID and one or more instance IDs. Use the pagination parameters to retrieve
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeGameServerInstancesOutcome DescribeGameServerInstances(
      const Model::DescribeGameServerInstancesRequest& request) const;

  /**
   * A Callable wrapper for DescribeGameServerInstances that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeGameServerInstancesRequestT = Model::DescribeGameServerInstancesRequest>
  Model::DescribeGameServerInstancesOutcomeCallable DescribeGameServerInstancesCallable(
      const DescribeGameServerInstancesRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeGameServerInstances, request);
  }

  /**
   * An Async wrapper for DescribeGameServerInstances that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeGameServerInstancesRequestT = Model::DescribeGameServerInstancesRequest>
  void DescribeGameServerInstancesAsync(const DescribeGameServerInstancesRequestT& request,
   ...
   * retrieve details for a specific game session, provide the game session ID. This
   * approach looks for the game session ID in all fleets that reside in the Amazon
   * Web Services Region defined in the request.</p> </li> </ul> <p>Use the
   * pagination parameters to retrieve results as a set of sequential pages. </p>
   * <p>If successful, a <code>GameSessionDetail</code> object is returned for each
   * game session that matches the request.</p> <p> <b>Learn more</b> </p> <p> <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeGameSessionDetailsOutcome DescribeGameSessionDetails(
      const Model::DescribeGameSessionDetailsRequest& request = {}) const;

  /**
   * A Callable wrapper for DescribeGameSessionDetails that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeGameSessionDetailsRequestT = Model::DescribeGameSessionDetailsRequest>
  Model::DescribeGameSessionDetailsOutcomeCallable DescribeGameSessionDetailsCallable(
      const DescribeGameSessionDetailsRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeGameSessionDetails, request);
  }

  /**
   * An Async wrapper for DescribeGameSessionDetails that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeGameSessionDetailsRequestT = Model::DescribeGameSessionDetailsRequest>
  void DescribeGameSessionDetailsAsync(const DescribeGameSessionDetailsResponseReceivedHandler& handler,
                                       const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                       const DescribeGameSessionDetailsRequestT& request = {}) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves information, including current status, about a game
   * session placement request. </p> <p>To get game session placement details,
   * specify the placement ID.</p> <p>This operation is not designed to be
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeGameSessionPlacementOutcome DescribeGameSessionPlacement(
      const Model::DescribeGameSessionPlacementRequest& request) const;

  /**
   * A Callable wrapper for DescribeGameSessionPlacement that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeGameSessionPlacementRequestT = Model::DescribeGameSessionPlacementRequest>
  Model::DescribeGameSessionPlacementOutcomeCallable DescribeGameSessionPlacementCallable(
      const DescribeGameSessionPlacementRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeGameSessionPlacement, request);
  }

  /**
   * An Async wrapper for DescribeGameSessionPlacement that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeGameSessionPlacementRequestT = Model::DescribeGameSessionPlacementRequest>
  void DescribeGameSessionPlacementAsync(const DescribeGameSessionPlacementRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves the properties for one or more game session queues.
   ...
   * are returned only for queues that currently exist in the Region.</p> <p>
   * <b>Learn more</b> </p> <p> <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeGameSessionQueuesOutcome DescribeGameSessionQueues(
      const Model::DescribeGameSessionQueuesRequest& request = {}) const;

  /**
   * A Callable wrapper for DescribeGameSessionQueues that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename DescribeGameSessionQueuesRequestT = Model::DescribeGameSessionQueuesRequest>
  Model::DescribeGameSessionQueuesOutcomeCallable DescribeGameSessionQueuesCallable(
      const DescribeGameSessionQueuesRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeGameSessionQueues, request);
  }

  /**
   * An Async wrapper for DescribeGameSessionQueues that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeGameSessionQueuesRequestT = Model::DescribeGameSessionQueuesRequest>
  void DescribeGameSessionQueuesAsync(const DescribeGameSessionQueuesResponseReceivedHandler& handler,
                                      const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                      const DescribeGameSessionQueuesRequestT& request = {}) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves a set of one or more game sessions in a specific
   ...
   * <p>To retrieve all game sessions that are currently running on all locations in
   * a fleet, provide a fleet or alias ID, with an optional status filter. This
  ...
  /**
   * A Callable wrapper for DescribeGameSessions that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeGameSessions that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeGameSessionsRequestT = Model::DescribeGameSessionsRequest>
  void DescribeGameSessionsAsync(const DescribeGameSessionsResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                 const DescribeGameSessionsRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for DescribeInstances that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeInstances that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DescribeInstancesRequestT = Model::DescribeInstancesRequest>
  void DescribeInstancesAsync(const DescribeInstancesRequestT& request, const DescribeInstancesResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves one or more matchmaking tickets. Use this operation
   * to retrieve ticket information, including--after a successful match is
   * made--connection information for the resulting new game session. </p> <p>To
   ...
   * currently exists.</p> <p>This operation is not designed to be continually called
   * to track matchmaking ticket status. This practice can cause you to exceed your
   * API limit, which results in errors. Instead, as a best practice, set up an
   * Amazon Simple Notification Service to receive notifications, and provide the
   * topic ARN in the matchmaking configuration.</p> <p/> <p> <b>Learn more</b> </p>
   * <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/flexmatchguide/match-client.html">
   * Add FlexMatch to a game client</a> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/flexmatchguide/match-notification.html">
  ...
  /**
   * A Callable wrapper for DescribeMatchmaking that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeMatchmaking that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DescribeMatchmakingRequestT = Model::DescribeMatchmakingRequest>
  void DescribeMatchmakingAsync(const DescribeMatchmakingRequestT& request, const DescribeMatchmakingResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves the details of FlexMatch matchmaking configurations.
   * </p> <p>This operation offers the following options: (1) retrieve all
   * matchmaking configurations, (2) retrieve configurations for a specified list, or
   * (3) retrieve all configurations that use a specified rule set name. When
   * requesting multiple items, use the pagination parameters to retrieve results as
   * a set of sequential pages. </p> <p>If successful, a configuration is returned
   * for each requested name. When specifying a list of names, only configurations
   * that currently exist are returned. </p> <p> <b>Learn more</b> </p> <p> <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeMatchmakingConfigurationsOutcome DescribeMatchmakingConfigurations(
      const Model::DescribeMatchmakingConfigurationsRequest& request = {}) const;

  /**
   * A Callable wrapper for DescribeMatchmakingConfigurations that returns a future to the operation so that it can be executed in parallel
   * to other requests.
   */
  template <typename DescribeMatchmakingConfigurationsRequestT = Model::DescribeMatchmakingConfigurationsRequest>
  Model::DescribeMatchmakingConfigurationsOutcomeCallable DescribeMatchmakingConfigurationsCallable(
      const DescribeMatchmakingConfigurationsRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeMatchmakingConfigurations, request);
  }

  /**
   * An Async wrapper for DescribeMatchmakingConfigurations that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DescribeMatchmakingConfigurationsRequestT = Model::DescribeMatchmakingConfigurationsRequest>
  void DescribeMatchmakingConfigurationsAsync(const DescribeMatchmakingConfigurationsResponseReceivedHandler& handler,
                                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                              const DescribeMatchmakingConfigurationsRequestT& request = {}) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves the details for FlexMatch matchmaking rule sets. You
   * can request all existing rule sets for the Region, or provide a list of one or
   ...
   * rule set is returned for each requested name. </p> <p> <b>Learn more</b> </p>
   * <ul> <li> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/flexmatchguide/match-rulesets.html">Build
   * a rule set</a> </p> </li> </ul><p><h3>See Also:</h3>   <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeMatchmakingRuleSetsOutcome DescribeMatchmakingRuleSets(
      const Model::DescribeMatchmakingRuleSetsRequest& request = {}) const;

  /**
   * A Callable wrapper for DescribeMatchmakingRuleSets that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeMatchmakingRuleSetsRequestT = Model::DescribeMatchmakingRuleSetsRequest>
  Model::DescribeMatchmakingRuleSetsOutcomeCallable DescribeMatchmakingRuleSetsCallable(
      const DescribeMatchmakingRuleSetsRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeMatchmakingRuleSets, request);
  }

  /**
   * An Async wrapper for DescribeMatchmakingRuleSets that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeMatchmakingRuleSetsRequestT = Model::DescribeMatchmakingRuleSetsRequest>
  void DescribeMatchmakingRuleSetsAsync(const DescribeMatchmakingRuleSetsResponseReceivedHandler& handler,
                                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                        const DescribeMatchmakingRuleSetsRequestT& request = {}) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves properties for one or more player sessions. </p>
   * <p>This action can be used in the following ways: </p> <ul> <li> <p>To retrieve
   * a specific player session, provide the player session ID only.</p> </li> <li>
   * <p>To retrieve all player sessions in a game session, provide the game session
   * ID only.</p> </li> <li> <p>To retrieve all player sessions for a specific
   * player, provide a player ID only.</p> </li> </ul> <p>To request player sessions,
   * specify either a player session ID, game session ID, or player ID. You can
   * filter this request by player session status. If you provide a specific
   * <code>PlayerSessionId</code> or <code>PlayerId</code>, Amazon GameLift Servers
   * ignores the filter criteria. Use the pagination parameters to retrieve results
   * as a set of sequential pages. </p> <p>If successful, a
   * <code>PlayerSession</code> object is returned for each session that matches the
   * request.</p> <p> <b>Related actions</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
   * APIs by task</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for DescribePlayerSessions that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribePlayerSessions that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribePlayerSessionsRequestT = Model::DescribePlayerSessionsRequest>
  void DescribePlayerSessionsAsync(const DescribePlayerSessionsResponseReceivedHandler& handler,
                                   const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                   const DescribePlayerSessionsRequestT& request = {}) const {
   ...
   * fleet's runtime configuration settings. The runtime configuration determines
   * which server processes run, and how, on computes in the fleet. For managed EC2
   * fleets, the runtime configuration describes server processes that run on each
   * fleet instance. You can update a fleet's runtime configuration at any time using
   * <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeRuntimeConfigurationOutcome DescribeRuntimeConfiguration(
      const Model::DescribeRuntimeConfigurationRequest& request) const;

  /**
   * A Callable wrapper for DescribeRuntimeConfiguration that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeRuntimeConfigurationRequestT = Model::DescribeRuntimeConfigurationRequest>
  Model::DescribeRuntimeConfigurationOutcomeCallable DescribeRuntimeConfigurationCallable(
      const DescribeRuntimeConfigurationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::DescribeRuntimeConfiguration, request);
  }

  /**
   * An Async wrapper for DescribeRuntimeConfiguration that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeRuntimeConfigurationRequestT = Model::DescribeRuntimeConfigurationRequest>
  void DescribeRuntimeConfigurationAsync(const DescribeRuntimeConfigurationRequestT& request,
  ...
  /**
   * A Callable wrapper for DescribeScalingPolicies that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeScalingPolicies that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeScalingPoliciesRequestT = Model::DescribeScalingPoliciesRequest>
  void DescribeScalingPoliciesAsync(const DescribeScalingPoliciesRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Retrieves
   * properties for a Realtime script. </p> <p>To request a script record, specify
  ...
  /**
   * An Async wrapper for DescribeScript that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename DescribeScriptRequestT = Model::DescribeScriptRequest>
  void DescribeScriptAsync(const DescribeScriptRequestT& request, const DescribeScriptResponseReceivedHandler& handler,
                           const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Retrieves
   * valid VPC peering authorizations that are pending for the Amazon Web Services
   * account. This operation returns all VPC peering authorizations and requests for
   * peering. This includes those initiated and received by this account. </p> <p>
   * <b>Related actions</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
   * APIs by task</a> </p><p><h3>See Also:</h3>   <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeVpcPeeringAuthorizationsOutcome DescribeVpcPeeringAuthorizations(
      const Model::DescribeVpcPeeringAuthorizationsRequest& request = {}) const;

  /**
   * A Callable wrapper for DescribeVpcPeeringAuthorizations that returns a future to the operation so that it can be executed in parallel
   * to other requests.
   */
  template <typename DescribeVpcPeeringAuthorizationsRequestT = Model::DescribeVpcPeeringAuthorizationsRequest>
  Model::DescribeVpcPeeringAuthorizationsOutcomeCallable DescribeVpcPeeringAuthorizationsCallable(
      const DescribeVpcPeeringAuthorizationsRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeVpcPeeringAuthorizations, request);
  }

  /**
   * An Async wrapper for DescribeVpcPeeringAuthorizations that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DescribeVpcPeeringAuthorizationsRequestT = Model::DescribeVpcPeeringAuthorizationsRequest>
  void DescribeVpcPeeringAuthorizationsAsync(const DescribeVpcPeeringAuthorizationsResponseReceivedHandler& handler,
                                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                             const DescribeVpcPeeringAuthorizationsRequestT& request = {}) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Retrieves
   * information on VPC peering connections. Use this operation to get peering
   * information for all fleets or for one specific fleet ID. </p> <p>To retrieve
   * connection information, call this operation from the Amazon Web Services account
   * that is used to manage the Amazon GameLift Servers fleets. Specify a fleet ID or
   ...
   * connections identify the IpV4 CIDR block that the VPC uses to connect. </p> <p>
   * <b>Related actions</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
   * APIs by task</a> </p><p><h3>See Also:</h3>   <a
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeVpcPeeringConnectionsOutcome DescribeVpcPeeringConnections(
      const Model::DescribeVpcPeeringConnectionsRequest& request = {}) const;

  /**
   * A Callable wrapper for DescribeVpcPeeringConnections that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeVpcPeeringConnectionsRequestT = Model::DescribeVpcPeeringConnectionsRequest>
  Model::DescribeVpcPeeringConnectionsOutcomeCallable DescribeVpcPeeringConnectionsCallable(
      const DescribeVpcPeeringConnectionsRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::DescribeVpcPeeringConnections, request);
  }

  /**
   * An Async wrapper for DescribeVpcPeeringConnections that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeVpcPeeringConnectionsRequestT = Model::DescribeVpcPeeringConnectionsRequest>
  void DescribeVpcPeeringConnectionsAsync(const DescribeVpcPeeringConnectionsResponseReceivedHandler& handler,
                                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                          const DescribeVpcPeeringConnectionsRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for GetComputeAccess that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for GetComputeAccess that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename GetComputeAccessRequestT = Model::GetComputeAccessRequest>
  void GetComputeAccessAsync(const GetComputeAccessRequestT& request, const GetComputeAccessResponseReceivedHandler& handler,
                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Requests an authentication token from Amazon GameLift Servers
   * for a compute resource in an Amazon GameLift Servers fleet. Game servers that
   * are running on the compute use this token to communicate with the Amazon
   * GameLift Servers service, such as when calling the Amazon GameLift Servers
   * server SDK action <code>InitSDK()</code>. Authentication tokens are valid for a
   * limited time span, so you need to request a fresh token before the current token
   * expires.</p> <p> <b>Request options</b> </p> <ul> <li> <p>For managed EC2 fleets
   * (compute type <code>EC2</code>), auth token retrieval and refresh is handled
   * automatically. All game servers that are running on all fleet instances have
   * access to a valid auth token.</p> </li> <li> <p>For Anywhere fleets (compute
   * type <code>ANYWHERE</code>), if you're using the Amazon GameLift Servers Agent,
   * auth token retrieval and refresh is handled automatically for any compute where
   * the Agent is running. If you're not using the Agent, create a mechanism to
   * retrieve and refresh auth tokens for computes that are running game server
   * processes.</p> </li> </ul> <p> <b>Learn more</b> </p> <ul> <li> <p> <a
  ...
  /**
   * A Callable wrapper for GetComputeAuthToken that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for GetComputeAuthToken that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename GetComputeAuthTokenRequestT = Model::GetComputeAuthTokenRequest>
  void GetComputeAuthTokenAsync(const GetComputeAuthTokenRequestT& request, const GetComputeAuthTokenResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Retrieves
   * the location of stored game session logs for a specified game session on Amazon
   * GameLift Servers managed fleets. When a game session is terminated, Amazon
   * GameLift Servers automatically stores the logs in Amazon S3 and retains them for
  ...
  /**
   * A Callable wrapper for GetGameSessionLogUrl that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for GetGameSessionLogUrl that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename GetGameSessionLogUrlRequestT = Model::GetGameSessionLogUrlRequest>
  void GetGameSessionLogUrlAsync(const GetGameSessionLogUrlRequestT& request, const GetGameSessionLogUrlResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Requests
   * authorization to remotely connect to an instance in an Amazon GameLift Servers
   * managed fleet. Use this operation to connect to instances with game servers that
   * use Amazon GameLift Servers server SDK 4.x or earlier. To connect to instances
   * with game servers that use server SDK 5.x or later, call <a
  ...
  /**
   * A Callable wrapper for GetInstanceAccess that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for GetInstanceAccess that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename GetInstanceAccessRequestT = Model::GetInstanceAccessRequest>
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves all aliases for this Amazon Web Services account. You
   * can filter the result set by alias name and/or routing strategy type. Use the
   * pagination parameters to retrieve results in sequential pages.</p>
   * <p>Returned aliases are not listed in any particular order.</p>  <p>
   * <b>Related actions</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
   * APIs by task</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Retrieves
   * build resources for all builds associated with the Amazon Web Services account
   * in use. You can limit results to builds that are in a specific status by using
   * the <code>Status</code> parameter. Use the pagination parameters to retrieve
   * results in </p>  <p>Build resources are not listed in any particular
  ...
  /**
   * An Async wrapper for ListBuilds that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListBuildsRequestT = Model::ListBuildsRequest>
  void ListBuildsAsync(const ListBuildsResponseReceivedHandler& handler,
                       const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                       const ListBuildsRequestT& request = {}) const {
    return SubmitAsync(&GameLiftClient::ListBuilds, request, handler, context);
  }

  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves information on the compute resources in an Amazon
   * GameLift Servers fleet. Use the pagination parameters to retrieve results in a
   * set of sequential pages.</p> <p> <b>Request options</b> </p> <ul> <li>
   * <p>Retrieve a list of all computes in a fleet. Specify a fleet ID. </p> </li>
   * <li> <p>Retrieve a list of all computes in a specific fleet location. Specify a
   * fleet ID and location.</p> </li> </ul> <p> <b>Results</b> </p> <p>If successful,
   * this operation returns information on a set of computes. Depending on the type
   * of fleet, the result includes the following information: </p> <ul> <li> <p>For a
   * managed EC2 fleet (compute type <code>EC2</code>), this operation returns
   * information about the EC2 instance. Compute names are EC2 instance IDs.</p>
   * </li> <li> <p>For an Anywhere fleet (compute type <code>ANYWHERE</code>), this
   * operation returns compute names and details from when the compute was registered
  ...
  /**
   * An Async wrapper for ListCompute that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListComputeRequestT = Model::ListComputeRequest>
  void ListComputeAsync(const ListComputeRequestT& request, const ListComputeResponseReceivedHandler& handler,
                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for ListContainerFleets that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ListContainerFleets that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename ListContainerFleetsRequestT = Model::ListContainerFleetsRequest>
  void ListContainerFleetsAsync(const ListContainerFleetsResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                const ListContainerFleetsRequestT& request = {}) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListContainerGroupDefinitionVersionsOutcome ListContainerGroupDefinitionVersions(
      const Model::ListContainerGroupDefinitionVersionsRequest& request) const;

  /**
   * A Callable wrapper for ListContainerGroupDefinitionVersions that returns a future to the operation so that it can be executed in
   * parallel to other requests.
   */
  template <typename ListContainerGroupDefinitionVersionsRequestT = Model::ListContainerGroupDefinitionVersionsRequest>
  Model::ListContainerGroupDefinitionVersionsOutcomeCallable ListContainerGroupDefinitionVersionsCallable(
      const ListContainerGroupDefinitionVersionsRequestT& request) const {
    return SubmitCallable(&GameLiftClient::ListContainerGroupDefinitionVersions, request);
  }

  /**
   * An Async wrapper for ListContainerGroupDefinitionVersions that queues the request into a thread executor and triggers associated
   * callback when operation has finished.
   */
  template <typename ListContainerGroupDefinitionVersionsRequestT = Model::ListContainerGroupDefinitionVersionsRequest>
  void ListContainerGroupDefinitionVersionsAsync(const ListContainerGroupDefinitionVersionsRequestT& request,
                                                 const ListContainerGroupDefinitionVersionsResponseReceivedHandler& handler,
                                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListContainerGroupDefinitionsOutcome ListContainerGroupDefinitions(
      const Model::ListContainerGroupDefinitionsRequest& request = {}) const;

  /**
   * A Callable wrapper for ListContainerGroupDefinitions that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename ListContainerGroupDefinitionsRequestT = Model::ListContainerGroupDefinitionsRequest>
  Model::ListContainerGroupDefinitionsOutcomeCallable ListContainerGroupDefinitionsCallable(
      const ListContainerGroupDefinitionsRequestT& request = {}) const {
    return SubmitCallable(&GameLiftClient::ListContainerGroupDefinitions, request);
  }

  /**
   * An Async wrapper for ListContainerGroupDefinitions that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListContainerGroupDefinitionsRequestT = Model::ListContainerGroupDefinitionsRequest>
  void ListContainerGroupDefinitionsAsync(const ListContainerGroupDefinitionsResponseReceivedHandler& handler,
                                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                          const ListContainerGroupDefinitionsRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for ListFleetDeployments that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ListFleetDeployments that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListFleetDeploymentsRequestT = Model::ListFleetDeploymentsRequest>
  void ListFleetDeploymentsAsync(const ListFleetDeploymentsResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                 const ListFleetDeploymentsRequestT& request = {}) const {
    return SubmitAsync(&GameLiftClient::ListFleetDeployments, request, handler, context);
  }

  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves a collection of fleet resources in an Amazon Web
  ...
  /**
   * A Callable wrapper for ListGameServerGroups that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ListGameServerGroups that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListGameServerGroupsRequestT = Model::ListGameServerGroupsRequest>
  void ListGameServerGroupsAsync(const ListGameServerGroupsResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                 const ListGameServerGroupsRequestT& request = {}) const {
    return SubmitAsync(&GameLiftClient::ListGameServerGroups, request, handler, context);
  }

  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Retrieves information on all game servers that are currently active in a
   ...
   * segments. </p> <p> <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/fleetiqguide/gsg-intro.html">Amazon
   * GameLift Servers FleetIQ Guide</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * An Async wrapper for ListGameServers that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListGameServersRequestT = Model::ListGameServersRequest>
  void ListGameServersAsync(const ListGameServersRequestT& request, const ListGameServersResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Retrieves
   * script records for all Realtime scripts that are associated with the Amazon Web
   * Services account in use. </p> <p> <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/realtime-intro.html">Amazon
   * GameLift Servers Amazon GameLift Servers Realtime</a> </p> <p> <b>Related
   * actions</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
  ...
  /**
   * An Async wrapper for ListScripts that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListScriptsRequestT = Model::ListScriptsRequest>
  void ListScriptsAsync(const ListScriptsResponseReceivedHandler& handler,
                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                        const ListScriptsRequestT& request = {}) const {
    return SubmitAsync(&GameLiftClient::ListScripts, request, handler, context);
  }

  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves all tags assigned to a Amazon GameLift Servers
   * resource. Use resource tags to organize Amazon Web Services resources for a
   * range of purposes. This operation handles the permissions necessary to manage
   * tags for Amazon GameLift Servers resources that support tagging.</p> <p>To list
   * tags for a resource, specify the unique ARN value for the resource.</p> <p>
   * <b>Learn more</b> </p> <p> <a
  ...
  /**
   * An Async wrapper for RegisterCompute that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename RegisterComputeRequestT = Model::RegisterComputeRequest>
  void RegisterComputeAsync(const RegisterComputeRequestT& request, const RegisterComputeResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Creates a new game server resource and notifies Amazon GameLift Servers
   ...
   * claim a game server for a new game session. </p> <p>To register a game server,
   * identify the game server group and instance where the game server is running,
   * and provide a unique identifier for the game server. You can also include
   * connection and game server data.</p> <p>Once a game server is successfully
   * registered, it is put in status <code>AVAILABLE</code>. A request to register a
   * game server may fail if the instance it is running on is in the process of
   * shutting down as part of instance balancing or scale-down activity. </p> <p>
   * <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/fleetiqguide/gsg-intro.html">Amazon
   * GameLift Servers FleetIQ Guide</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for RegisterGameServer that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for RegisterGameServer that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename RegisterGameServerRequestT = Model::RegisterGameServerRequest>
  void RegisterGameServerAsync(const RegisterGameServerRequestT& request, const RegisterGameServerResponseReceivedHandler& handler,
                               const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Retrieves a
   * fresh set of credentials for use when uploading a new set of game build files to
   * Amazon GameLift Servers's Amazon S3. This is done as part of the build creation
   * process; see <a
  ...
  /**
   * A Callable wrapper for RequestUploadCredentials that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for RequestUploadCredentials that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename RequestUploadCredentialsRequestT = Model::RequestUploadCredentialsRequest>
  void RequestUploadCredentialsAsync(const RequestUploadCredentialsRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Attempts to retrieve a fleet ID that is associated with an
   * alias. Specify a unique alias identifier.</p> <p>If the alias has a
   * <code>SIMPLE</code> routing strategy, Amazon GameLift Servers returns a fleet
   * ID. If the alias has a <code>TERMINAL</code> routing strategy, the result is a
  ...
  /**
   * An Async wrapper for ResolveAlias that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ResolveAliasRequestT = Model::ResolveAliasRequest>
  void ResolveAliasAsync(const ResolveAliasRequestT& request, const ResolveAliasResponseReceivedHandler& handler,
                         const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Reinstates activity on a game server group after it has been suspended. A
  ...
  /**
   * A Callable wrapper for ResumeGameServerGroup that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ResumeGameServerGroup that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ResumeGameServerGroupRequestT = Model::ResumeGameServerGroupRequest>
  void ResumeGameServerGroupAsync(const ResumeGameServerGroupRequestT& request, const ResumeGameServerGroupResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Retrieves all active game sessions that match a set of search
   * criteria and sorts them into a specified order. </p> <p>This operation is not
   * designed to continually track game session status because that practice can
   * cause you to exceed your API limit and generate errors. Instead, configure an
   * Amazon Simple Notification Service (Amazon SNS) topic to receive notifications
   * from a matchmaker or a game session placement queue.</p> <p>When searching for
   * game sessions, you specify exactly where you want to search and provide a search
  ...
  /**
   * A Callable wrapper for SearchGameSessions that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for SearchGameSessions that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename SearchGameSessionsRequestT = Model::SearchGameSessionsRequest>
  void SearchGameSessionsAsync(const SearchGameSessionsResponseReceivedHandler& handler,
                               const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                               const SearchGameSessionsRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for StartFleetActions that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for StartFleetActions that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename StartFleetActionsRequestT = Model::StartFleetActionsRequest>
  void StartFleetActionsAsync(const StartFleetActionsRequestT& request, const StartFleetActionsResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Makes a request to start a new game session using a game
   * session queue. When processing a placement request, Amazon GameLift Servers
   * looks for the best possible available resource to host the game session, based
   * on how the queue is configured to prioritize factors such as resource cost,
   * latency, and location. After selecting an available resource, Amazon GameLift
   * Servers prompts the resource to start a game session. A placement request can
   * include a list of players to create a set of player sessions. The request can
   * also include information to pass to the new game session, such as to specify a
   * game map or other options.</p> <p> <b>Request options</b> </p> <p>Use this
   * operation to make the following types of requests. </p> <ul> <li> <p>Request a
   * placement using the queue's default prioritization process (see the default
   * prioritization described in <a
  ...
  /**
   * A Callable wrapper for StartGameSessionPlacement that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename StartGameSessionPlacementRequestT = Model::StartGameSessionPlacementRequest>
  Model::StartGameSessionPlacementOutcomeCallable StartGameSessionPlacementCallable(
      const StartGameSessionPlacementRequestT& request) const {
    return SubmitCallable(&GameLiftClient::StartGameSessionPlacement, request);
  }

  /**
   * An Async wrapper for StartGameSessionPlacement that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename StartGameSessionPlacementRequestT = Model::StartGameSessionPlacementRequest>
  void StartGameSessionPlacementAsync(const StartGameSessionPlacementRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Finds new players to fill open slots in currently running game
   * sessions. The backfill match process is essentially identical to the process of
   ...
   * making backfill requests directly from your game server. In response to a
   * request, FlexMatch creates player sessions for the new players, updates the
  ...
  /**
   * A Callable wrapper for StartMatchBackfill that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for StartMatchBackfill that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename StartMatchBackfillRequestT = Model::StartMatchBackfillRequest>
  void StartMatchBackfillAsync(const StartMatchBackfillRequestT& request, const StartMatchBackfillResponseReceivedHandler& handler,
                               const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Uses FlexMatch to create a game match for a group of players
   * based on custom matchmaking rules. With games that use Amazon GameLift Servers
   * managed hosting, this operation also triggers Amazon GameLift Servers to find
   * hosting resources and start a new game session for the new match. Each
   * matchmaking request includes information on one or more players and specifies
   * the FlexMatch matchmaker to use. When a request is for multiple players,
   * FlexMatch attempts to build a match that includes all players in the request,
   * placing them in the same team and finding additional players as needed to fill
   * the match. </p> <p>To start matchmaking, provide a unique ticket ID, specify a
   * matchmaking configuration, and include the players to be matched. You must also
  ...
  /**
   * A Callable wrapper for StartMatchmaking that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for StartMatchmaking that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename StartMatchmakingRequestT = Model::StartMatchmakingRequest>
  void StartMatchmakingAsync(const StartMatchmakingRequestT& request, const StartMatchmakingResponseReceivedHandler& handler,
                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for StopFleetActions that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for StopFleetActions that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename StopFleetActionsRequestT = Model::StopFleetActionsRequest>
  void StopFleetActionsAsync(const StopFleetActionsRequestT& request, const StopFleetActionsResponseReceivedHandler& handler,
                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Cancels a game session placement that's in <code>PENDING</code>
  ...
  /**
   * A Callable wrapper for StopGameSessionPlacement that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for StopGameSessionPlacement that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename StopGameSessionPlacementRequestT = Model::StopGameSessionPlacementRequest>
  void StopGameSessionPlacementAsync(const StopGameSessionPlacementRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Cancels a matchmaking ticket or match backfill ticket that is
   * currently being processed. To stop the matchmaking operation, specify the ticket
   * ID. If successful, work on the ticket is stopped, and the ticket status is
   * changed to <code>CANCELLED</code>.</p> <p>This call is also used to turn off
   * automatic backfill for an individual game session. This is for game sessions
   * that are created with a matchmaking configuration that has automatic backfill
   * enabled. The ticket ID is included in the <code>MatchmakerData</code> of an
   * updated game session object, which is provided to the game server.</p>
   * <p>If the operation is successful, the service sends back an empty JSON struct
   * with the HTTP 200 response (not an empty HTTP body).</p>  <p> <b>Learn
   * more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/flexmatchguide/match-client.html">
  ...
  /**
   * An Async wrapper for StopMatchmaking that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename StopMatchmakingRequestT = Model::StopMatchmakingRequest>
  void StopMatchmakingAsync(const StopMatchmakingRequestT& request, const StopMatchmakingResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Temporarily stops activity on a game server group without terminating
  ...
  /**
   * A Callable wrapper for SuspendGameServerGroup that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for SuspendGameServerGroup that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename SuspendGameServerGroupRequestT = Model::SuspendGameServerGroupRequest>
  void SuspendGameServerGroupAsync(const SuspendGameServerGroupRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Assigns a tag to an Amazon GameLift Servers resource. You can
   * use tags to organize resources, create IAM permissions policies to manage access
   * to groups of resources, customize Amazon Web Services cost breakdowns, and more.
   * This operation handles the permissions necessary to manage tags for Amazon
   * GameLift Servers resources that support tagging.</p> <p>To add a tag to a
   * resource, specify the unique ARN value for the resource and provide a tag list
   * containing one or more tags. The operation succeeds even if the list includes
   * tags that are already assigned to the resource. </p> <p> <b>Learn more</b> </p>
   * <p> <a
   * href="https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html">Tagging
   * Amazon Web Services Resources</a> in the <i>Amazon Web Services General
   * Reference</i> </p> <p> <a
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Ends a game session that's currently in progress. Use this
   * action to terminate any game session that isn't in <code>ERROR</code> status.
   * Terminating a game session is the most efficient way to free up a server process
   * when it's hosting a game session that's in a bad state or not ending properly.
   * You can use this action to terminate a game session that's being hosted on any
   * type of Amazon GameLift Servers fleet compute, including computes for managed
   * EC2, managed container, and Anywhere fleets. The game server must be integrated
   * with Amazon GameLift Servers server SDK 5.x or greater.</p> <p> <b>Request
  ...
  /**
   * A Callable wrapper for TerminateGameSession that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for TerminateGameSession that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename TerminateGameSessionRequestT = Model::TerminateGameSessionRequest>
  void TerminateGameSessionAsync(const TerminateGameSessionRequestT& request, const TerminateGameSessionResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Removes a tag assigned to a Amazon GameLift Servers resource.
   * You can use resource tags to organize Amazon Web Services resources for a range
   * of purposes. This operation handles the permissions necessary to manage tags for
   * Amazon GameLift Servers resources that support tagging.</p> <p>To remove a tag
   * from a resource, specify the unique ARN value for the resource and provide a
   * string list containing one or more tags to remove. This operation succeeds even
   * if the list includes tags that aren't assigned to the resource.</p> <p> <b>Learn
   * more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html">Tagging
   * Amazon Web Services Resources</a> in the <i>Amazon Web Services General
   * Reference</i> </p> <p> <a
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Updates properties for an alias. Specify the unique identifier
   * of the alias to be updated and the new property values.</p> <p>When reassigning
   * an alias to a new fleet, provide an updated routing strategy. If successful, the
   * updated alias record is returned.</p> <p> <b>Related actions</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
   * APIs by task</a> </p><p><h3>See Also:</h3>   <a
   ...
   * metadata in a build resource, including the build name and version. To update
   * the metadata, specify the build ID to update and provide the new values. If
   * successful, a build object containing the updated metadata is returned.</p> <p>
   * <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/gamelift-build-intro.html">
   * Upload a Custom Server Build</a> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
  ...
  /**
   * An Async wrapper for UpdateBuild that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename UpdateBuildRequestT = Model::UpdateBuildRequest>
  void UpdateBuildAsync(const UpdateBuildRequestT& request, const UpdateBuildResponseReceivedHandler& handler,
                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for UpdateContainerFleet that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateContainerFleet that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename UpdateContainerFleetRequestT = Model::UpdateContainerFleetRequest>
  void UpdateContainerFleetAsync(const UpdateContainerFleetRequestT& request, const UpdateContainerFleetResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::UpdateContainerGroupDefinitionOutcome UpdateContainerGroupDefinition(
      const Model::UpdateContainerGroupDefinitionRequest& request) const;

  /**
   * A Callable wrapper for UpdateContainerGroupDefinition that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename UpdateContainerGroupDefinitionRequestT = Model::UpdateContainerGroupDefinitionRequest>
  Model::UpdateContainerGroupDefinitionOutcomeCallable UpdateContainerGroupDefinitionCallable(
      const UpdateContainerGroupDefinitionRequestT& request) const {
    return SubmitCallable(&GameLiftClient::UpdateContainerGroupDefinition, request);
  }

  /**
   * An Async wrapper for UpdateContainerGroupDefinition that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename UpdateContainerGroupDefinitionRequestT = Model::UpdateContainerGroupDefinitionRequest>
  void UpdateContainerGroupDefinitionAsync(const UpdateContainerGroupDefinitionRequestT& request,
                                           const UpdateContainerGroupDefinitionResponseReceivedHandler& handler,
                                           const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for UpdateFleetAttributes that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateFleetAttributes that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename UpdateFleetAttributesRequestT = Model::UpdateFleetAttributesRequest>
  void UpdateFleetAttributesAsync(const UpdateFleetAttributesRequestT& request, const UpdateFleetAttributesResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for UpdateFleetCapacity that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateFleetCapacity that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename UpdateFleetCapacityRequestT = Model::UpdateFleetCapacityRequest>
  void UpdateFleetCapacityAsync(const UpdateFleetCapacityRequestT& request, const UpdateFleetCapacityResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for UpdateFleetPortSettings that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateFleetPortSettings that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename UpdateFleetPortSettingsRequestT = Model::UpdateFleetPortSettingsRequest>
  void UpdateFleetPortSettingsAsync(const UpdateFleetPortSettingsRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Updates information about a registered game server to help Amazon GameLift
   * Servers FleetIQ track game server availability. This operation is called by a
   * game server process that is running on an instance in a game server group. </p>
   * <p>Use this operation to update the following types of game server information.
   * You can make all three types of updates in the same request:</p> <ul> <li> <p>To
   * update the game server's utilization status from <code>AVAILABLE</code> (when
  ...
  /**
   * A Callable wrapper for UpdateGameServer that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateGameServer that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename UpdateGameServerRequestT = Model::UpdateGameServerRequest>
  void UpdateGameServerAsync(const UpdateGameServerRequestT& request, const UpdateGameServerResponseReceivedHandler& handler,
                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2 (FleetIQ)</p>
   * <p>Updates Amazon GameLift Servers FleetIQ-specific properties for a game server
  ...
  /**
   * A Callable wrapper for UpdateGameServerGroup that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateGameServerGroup that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename UpdateGameServerGroupRequestT = Model::UpdateGameServerGroupRequest>
  void UpdateGameServerGroupAsync(const UpdateGameServerGroupRequestT& request, const UpdateGameServerGroupResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Updates the mutable properties of a game session. </p> <p>To
   * update a game session, specify the game session ID and the values you want to
   * change. </p> <p>If successful, the updated <code>GameSession</code> object is
   * returned. </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/reference-awssdk.html#reference-awssdk-resources-fleets">All
   * APIs by task</a> </p><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for UpdateGameSession that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateGameSession that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename UpdateGameSessionRequestT = Model::UpdateGameSessionRequest>
  void UpdateGameSessionAsync(const UpdateGameSessionRequestT& request, const UpdateGameSessionResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Updates the configuration of a game session queue, which
   * determines how the queue processes new game session requests. To update
   * settings, specify the queue name to be updated and provide the new settings.
   * When updating destinations, provide a complete list of destinations. </p> <p>
   * <b>Learn more</b> </p> <p> <a
  ...
  /**
   * A Callable wrapper for UpdateGameSessionQueue that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateGameSessionQueue that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename UpdateGameSessionQueueRequestT = Model::UpdateGameSessionQueueRequest>
  void UpdateGameSessionQueueAsync(const UpdateGameSessionQueueRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Updates settings for a FlexMatch matchmaking configuration.
   ...
   * provide the new settings. </p> <p> <b>Learn more</b> </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/flexmatchguide/match-configuration.html">
   ...
   * API Reference</a></p>
   */
  virtual Model::UpdateMatchmakingConfigurationOutcome UpdateMatchmakingConfiguration(
      const Model::UpdateMatchmakingConfigurationRequest& request) const;

  /**
   * A Callable wrapper for UpdateMatchmakingConfiguration that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename UpdateMatchmakingConfigurationRequestT = Model::UpdateMatchmakingConfigurationRequest>
  Model::UpdateMatchmakingConfigurationOutcomeCallable UpdateMatchmakingConfigurationCallable(
      const UpdateMatchmakingConfigurationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::UpdateMatchmakingConfiguration, request);
  }

  /**
   * An Async wrapper for UpdateMatchmakingConfiguration that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename UpdateMatchmakingConfigurationRequestT = Model::UpdateMatchmakingConfigurationRequest>
  void UpdateMatchmakingConfigurationAsync(const UpdateMatchmakingConfigurationRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Updates the
   * runtime configuration for the specified fleet. The runtime configuration tells
   * Amazon GameLift Servers how to launch server processes on computes in managed
   * EC2 and Anywhere fleets. You can update a fleet's runtime configuration at any
   * time after the fleet is created; it does not need to be in <code>ACTIVE</code>
   * status.</p> <p>To update runtime configuration, specify the fleet ID and provide
   * a <code>RuntimeConfiguration</code> with an updated set of server process
   * configurations.</p> <p>If successful, the fleet's runtime configuration settings
   * are updated. Fleet computes that run game server processes regularly check for
   * and receive updated runtime configurations. The computes immediately take action
   * to comply with the new configuration by launching new server processes or by not
   * replacing existing processes when they shut down. Updating a fleet's runtime
   * configuration never affects existing server processes.</p> <p> <b>Learn more</b>
   * </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/fleets-intro.html">Setting
   * up Amazon GameLift Servers fleets</a> </p><p><h3>See Also:</h3>   <a
   ...
   * API Reference</a></p>
   */
  virtual Model::UpdateRuntimeConfigurationOutcome UpdateRuntimeConfiguration(
      const Model::UpdateRuntimeConfigurationRequest& request) const;

  /**
   * A Callable wrapper for UpdateRuntimeConfiguration that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename UpdateRuntimeConfigurationRequestT = Model::UpdateRuntimeConfigurationRequest>
  Model::UpdateRuntimeConfigurationOutcomeCallable UpdateRuntimeConfigurationCallable(
      const UpdateRuntimeConfigurationRequestT& request) const {
    return SubmitCallable(&GameLiftClient::UpdateRuntimeConfiguration, request);
  }

  /**
   * An Async wrapper for UpdateRuntimeConfiguration that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename UpdateRuntimeConfigurationRequestT = Model::UpdateRuntimeConfigurationRequest>
  void UpdateRuntimeConfigurationAsync(const UpdateRuntimeConfigurationRequestT& request,
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2</p> <p>Updates
   * Realtime script metadata and content.</p> <p>To update script metadata, specify
   * the script ID and provide updated name and/or version values. </p> <p>To update
   ...
   * original script was uploaded. Use the <i>Version</i> parameter to track updates
   * to the script.</p> <p>If the call is successful, the updated metadata is stored
   * in the script record and a revised script is uploaded to the Amazon GameLift
   * Servers service. Once the script is updated and acquired by a fleet instance,
   * the new version is used for all new game sessions. </p> <p> <b>Learn more</b>
   * </p> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/developerguide/realtime-intro.html">Amazon
   * GameLift Servers Amazon GameLift Servers Realtime</a> </p> <p> <b>Related
   * actions</b> </p> <p> <a
  ...
  /**
   * An Async wrapper for UpdateScript that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename UpdateScriptRequestT = Model::UpdateScriptRequest>
  void UpdateScriptAsync(const UpdateScriptRequestT& request, const UpdateScriptResponseReceivedHandler& handler,
                         const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * <p> <b>This API works with the following fleet types:</b> EC2, Anywhere,
   * Container</p> <p>Validates the syntax of a matchmaking rule or rule set. This
   * operation checks that the rule set is using syntactically correct JSON and that
   * it conforms to allowed property expressions. To validate syntax, provide a rule
   * set JSON string.</p> <p> <b>Learn more</b> </p> <ul> <li> <p> <a
   * href="https://docs.aws.amazon.com/gamelift/latest/flexmatchguide/match-rulesets.html">Build
   * a rule set</a> </p> </li> </ul><p><h3>See Also:</h3>   <a
   * href="http://docs.aws.amazon.com/goto/WebAPI/gamelift-2015-10-01/ValidateMatchmakingRuleSet">AWS
   * API Reference</a></p>
   */
  virtual Model::ValidateMatchmakingRuleSetOutcome ValidateMatchmakingRuleSet(
      const Model::ValidateMatchmakingRuleSetRequest& request) const;

  /**
   * A Callable wrapper for ValidateMatchmakingRuleSet that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename ValidateMatchmakingRuleSetRequestT = Model::ValidateMatchmakingRuleSetRequest>
  Model::ValidateMatchmakingRuleSetOutcomeCallable ValidateMatchmakingRuleSetCallable(
      const ValidateMatchmakingRuleSetRequestT& request) const {
    return SubmitCallable(&GameLiftClient::ValidateMatchmakingRuleSet, request);
  }

  /**
   * An Async wrapper for ValidateMatchmakingRuleSet that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ValidateMatchmakingRuleSetRequestT = Model::ValidateMatchmakingRuleSetRequest>
  void ValidateMatchmakingRuleSetAsync(const ValidateMatchmakingRuleSetRequestT& request,
