...
class ResumeGameServerGroupRequest : public GameLiftRequest {
 public:
  ...
  void SetResumeActions(ResumeActionsT&& value) {
    m_resumeActionsHasBeenSet = true;
    m_resumeActions = std::forward<ResumeActionsT>(value);
  }
  template <typename ResumeActionsT = Aws::Vector<GameServerGroupAction>>
  ResumeGameServerGroupRequest& WithResumeActions(ResumeActionsT&& value) {
    SetResumeActions(std::forward<ResumeActionsT>(value));
    return *this;
  }
  inline ResumeGameServerGroupRequest& AddResumeActions(GameServerGroupAction value) {
    m_resumeActionsHasBeenSet = true;
    m_resumeActions.push_back(value);
    return *this;
  }
  ///@}
 private:
