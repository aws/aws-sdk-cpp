...
class ListBuildsResult {
 public:
  ...
  AWS_GAMELIFT_API ListBuildsResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  AWS_GAMELIFT_API ListBuildsResult& operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  ...
  template <typename BuildsT = Aws::Vector<Build>>
  ListBuildsResult& WithBuilds(BuildsT&& value) {
    SetBuilds(std::forward<BuildsT>(value));
    return *this;
  }
  template <typename BuildsT = Build>
  ListBuildsResult& AddBuilds(BuildsT&& value) {
    m_buildsHasBeenSet = true;
