...
class PriorityConfiguration {
 public:
  ...
  AWS_GAMELIFT_API PriorityConfiguration(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_GAMELIFT_API PriorityConfiguration& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  void SetPriorityOrder(PriorityOrderT&& value) {
    ...
    m_priorityOrder = std::forward<PriorityOrderT>(value);
  }
  template <typename PriorityOrderT = Aws::Vector<PriorityType>>
  PriorityConfiguration& WithPriorityOrder(PriorityOrderT&& value) {
    SetPriorityOrder(std::forward<PriorityOrderT>(value));
    return *this;
  }
  inline PriorityConfiguration& AddPriorityOrder(PriorityType value) {
    m_priorityOrderHasBeenSet = true;
    m_priorityOrder.push_back(value);
    return *this;
  }
  ///@}
