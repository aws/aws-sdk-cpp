...
class ListFleetsResult {
 public:
  ...
  AWS_GAMELIFT_API ListFleetsResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  AWS_GAMELIFT_API ListFleetsResult& operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  ...
  template <typename FleetIdsT = Aws::Vector<Aws::String>>
  ListFleetsResult& WithFleetIds(FleetIdsT&& value) {
    SetFleetIds(std::forward<FleetIdsT>(value));
    return *this;
  }
  template <typename FleetIdsT = Aws::String>
  ListFleetsResult& AddFleetIds(FleetIdsT&& value) {
    m_fleetIdsHasBeenSet = true;
