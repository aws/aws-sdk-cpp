...
Aws::String UpdateContainerGroupDefinitionRequest::SerializePayload() const {
  ...
  if (m_sourceVersionNumberHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("SourceVersionNumber"));
    (m_sourceVersionNumber >= 0) ? encoder.WriteUInt(m_sourceVersionNumber) : encoder.WriteNegInt(m_sourceVersionNumber);
  }
