...
ClaimFilterOption::ClaimFilterOption(JsonView jsonValue) { *this = jsonValue; }
...
ClaimFilterOption& ClaimFilterOption::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("InstanceStatuses")) {
    ...
    for (unsigned instanceStatusesIndex = 0; instanceStatusesIndex < instanceStatusesJsonList.GetLength(); ++instanceStatusesIndex) {
      m_instanceStatuses.push_back(
          FilterInstanceStatusMapper::GetFilterInstanceStatusForName(instanceStatusesJsonList[instanceStatusesIndex].AsString()));
    }
...
JsonValue ClaimFilterOption::Jsonize() const {
  ...
  if (m_instanceStatusesHasBeenSet) {
    ...
    for (unsigned instanceStatusesIndex = 0; instanceStatusesIndex < instanceStatusesJsonList.GetLength(); ++instanceStatusesIndex) {
      instanceStatusesJsonList[instanceStatusesIndex].AsString(
          FilterInstanceStatusMapper::GetNameForFilterInstanceStatus(m_instanceStatuses[instanceStatusesIndex]));
    }
