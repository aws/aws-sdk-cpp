...
MatchmakingConfiguration::MatchmakingConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
MatchmakingConfiguration& MatchmakingConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("GameSessionQueueArns")) {
    ...
    for (unsigned gameSessionQueueArnsIndex = 0; gameSessionQueueArnsIndex < gameSessionQueueArnsJsonList.GetLength();
         ++gameSessionQueueArnsIndex) {
