...
Aws::String SearchGameSessionsRequest::SerializePayload() const {
  ...
  if (m_filterExpressionHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_filterExpression.c_str()));
  }

  if (m_sortExpressionHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("SortExpression"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_sortExpression.c_str()));
  }
