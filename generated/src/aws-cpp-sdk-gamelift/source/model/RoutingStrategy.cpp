...
RoutingStrategy::RoutingStrategy(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

RoutingStrategy& RoutingStrategy::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Type") {
                ...
                if (val.has_value()) {
                  m_type = RoutingStrategyTypeMapper::GetRoutingStrategyTypeForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("RoutingStrategy", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void RoutingStrategy::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_typeHasBeenSet) {
    mapSize++;
  ...
  if (m_typeHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(RoutingStrategyTypeMapper::GetNameForRoutingStrategyType(m_type).c_str()));
  }
