...
Instance::Instance(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

Instance& Instance::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "FleetId") {
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_status = InstanceStatusMapper::GetInstanceStatusForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("Instance", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void Instance::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_statusHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(InstanceStatusMapper::GetNameForInstanceStatus(m_status).c_str()));
  }
