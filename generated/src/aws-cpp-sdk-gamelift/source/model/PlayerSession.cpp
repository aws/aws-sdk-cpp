...
PlayerSession::PlayerSession(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

PlayerSession& PlayerSession::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "PlayerSessionId") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_playerSessionId = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    ...
                    m_playerSessionId = ss.str();
                  }
                }
                m_playerSessionIdHasBeenSet = true;
              }

              else if (initialKeyStr == "PlayerId") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
              ...
              else if (initialKeyStr == "GameSessionId") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_status = PlayerSessionStatusMapper::GetPlayerSessionStatusForName(
              ...
              else if (initialKeyStr == "PlayerData") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_playerData = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    decoder->ConsumeNextSingleElement();
                    Aws::StringStream ss;
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("PlayerSession", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void PlayerSession::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_statusHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(PlayerSessionStatusMapper::GetNameForPlayerSessionStatus(m_status).c_str()));
  }
