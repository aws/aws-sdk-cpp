...
ArcRoutingControlConfiguration::ArcRoutingControlConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue ArcRoutingControlConfiguration::Jsonize() const {
  ...
  if (m_regionAndRoutingControlsHasBeenSet) {
    ...
    for (auto& regionAndRoutingControlsItem : m_regionAndRoutingControls) {
      ...
      for (unsigned arcRoutingControlStatesIndex = 0; arcRoutingControlStatesIndex < arcRoutingControlStatesJsonList.GetLength();
           ++arcRoutingControlStatesIndex) {
        arcRoutingControlStatesJsonList[arcRoutingControlStatesIndex].AsObject(
            regionAndRoutingControlsItem.second[arcRoutingControlStatesIndex].Jsonize());
      }
