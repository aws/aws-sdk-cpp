...
DocumentDbConfiguration::DocumentDbConfiguration(JsonView jsonValue) { *this = jsonValue; }

DocumentDbConfiguration& DocumentDbConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("behavior")) {
    m_behavior = DocumentDbDefaultBehaviorMapper::GetDocumentDbDefaultBehaviorForName(jsonValue.GetString("behavior"));
  ...
  if (jsonValue.ValueExists("databaseClusterArns")) {
    ...
    for (unsigned databaseClusterArnsIndex = 0; databaseClusterArnsIndex < databaseClusterArnsJsonList.GetLength();
         ++databaseClusterArnsIndex) {
...
JsonValue DocumentDbConfiguration::Jsonize() const {
  JsonValue payload;
  ...
  if (m_behaviorHasBeenSet) {
    payload.WithString("behavior", DocumentDbDefaultBehaviorMapper::GetNameForDocumentDbDefaultBehavior(m_behavior));
  }
