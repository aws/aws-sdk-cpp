...
class ExecutionApprovalConfiguration {
 public:
  ...
  inline ExecutionApprovalConfiguration& WithTimeoutMinutes(int value) {
  ...
  void SetApprovalRole(ApprovalRoleT&& value) {
    m_approvalRoleHasBeenSet = true;
    m_approvalRole = std::forward<ApprovalRoleT>(value);
  }
  template <typename ApprovalRoleT = Aws::String>
  ExecutionApprovalConfiguration& WithApprovalRole(ApprovalRoleT&& value) {
    SetApprovalRole(std::forward<ApprovalRoleT>(value));
    return *this;
  }
  ///@}
 private:
