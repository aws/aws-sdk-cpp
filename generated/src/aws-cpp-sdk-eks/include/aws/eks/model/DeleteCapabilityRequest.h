...
/**
 */
class DeleteCapabilityRequest : public EKSRequest {
 public:
  AWS_EKS_API DeleteCapabilityRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteCapability"; }
  ...
  template <typename ClusterNameT = Aws::String>
  DeleteCapabilityRequest& WithClusterName(ClusterNameT&& value) {
  ...
  template <typename CapabilityNameT = Aws::String>
  DeleteCapabilityRequest& WithCapabilityName(CapabilityNameT&& value) {
