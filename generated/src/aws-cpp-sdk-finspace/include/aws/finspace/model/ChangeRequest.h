...
class ChangeRequest {
 public:
  ...
  inline void SetChangeType(ChangeType value) {
    m_changeTypeHasBeenSet = true;
    m_changeType = value;
  }
  inline ChangeRequest& WithChangeType(ChangeType value) {
    SetChangeType(value);
    return *this;
  }
  ///@}
  ...
  void SetDbPath(DbPathT&& value) {
    ...
    m_dbPath = std::forward<DbPathT>(value);
  }
  template <typename DbPathT = Aws::String>
  ChangeRequest& WithDbPath(DbPathT&& value) {
    SetDbPath(std::forward<DbPathT>(value));
    return *this;
  }
  ///@}
 private:
