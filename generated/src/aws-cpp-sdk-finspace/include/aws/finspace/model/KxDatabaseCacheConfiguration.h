...
class KxDatabaseCacheConfiguration {
 public:
  ...
  void SetCacheType(CacheTypeT&& value) {
    m_cacheTypeHasBeenSet = true;
    m_cacheType = std::forward<CacheTypeT>(value);
  }
  template <typename CacheTypeT = Aws::String>
  KxDatabaseCacheConfiguration& WithCacheType(CacheTypeT&& value) {
    SetCacheType(std::forward<CacheTypeT>(value));
    return *this;
  }
  ///@}
