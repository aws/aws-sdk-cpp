...
class TimeRange {
 public:
  ...
  void SetFromInclusive(FromInclusiveT&& value) {
    ...
    m_fromInclusive = std::forward<FromInclusiveT>(value);
  }
  template <typename FromInclusiveT = Aws::Utils::DateTime>
  TimeRange& WithFromInclusive(FromInclusiveT&& value) {
    SetFromInclusive(std::forward<FromInclusiveT>(value));
    return *this;
  }
  ///@}
  ...
  void SetToExclusive(ToExclusiveT&& value) {
    ...
    m_toExclusive = std::forward<ToExclusiveT>(value);
  }
  template <typename ToExclusiveT = Aws::Utils::DateTime>
  TimeRange& WithToExclusive(ToExclusiveT&& value) {
    SetToExclusive(std::forward<ToExclusiveT>(value));
    return *this;
  }
  ///@}
 private:
