...
class PropertyDifference {
 public:
  ...
  void SetPropertyPath(PropertyPathT&& value) {
    ...
    m_propertyPath = std::forward<PropertyPathT>(value);
  }
  template <typename PropertyPathT = Aws::String>
  PropertyDifference& WithPropertyPath(PropertyPathT&& value) {
    SetPropertyPath(std::forward<PropertyPathT>(value));
    return *this;
  }
  ///@}
  ...
  inline void SetDifferenceType(DifferenceType value) {
    m_differenceTypeHasBeenSet = true;
    m_differenceType = value;
  }
  inline PropertyDifference& WithDifferenceType(DifferenceType value) {
