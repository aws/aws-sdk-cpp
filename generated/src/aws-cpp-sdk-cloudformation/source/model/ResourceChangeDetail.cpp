...
ResourceChangeDetail::ResourceChangeDetail(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResourceChangeDetail& ResourceChangeDetail::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!evaluationNode.IsNull()) {
      m_evaluation = EvaluationTypeMapper::GetEvaluationTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(evaluationNode.GetText()).c_str()));
      m_evaluationHasBeenSet = true;
    }
    XmlNode changeSourceNode = resultNode.FirstChild("ChangeSource");
    if (!changeSourceNode.IsNull()) {
      m_changeSource = ChangeSourceMapper::GetChangeSourceForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(changeSourceNode.GetText()).c_str()));
...
void ResourceChangeDetail::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_evaluationHasBeenSet) {
    oStream << location << index << locationValue
            << ".Evaluation=" << StringUtils::URLEncode(EvaluationTypeMapper::GetNameForEvaluationType(m_evaluation)) << "&";
  }

  if (m_changeSourceHasBeenSet) {
    oStream << location << index << locationValue
            << ".ChangeSource=" << StringUtils::URLEncode(ChangeSourceMapper::GetNameForChangeSource(m_changeSource)) << "&";
  }
