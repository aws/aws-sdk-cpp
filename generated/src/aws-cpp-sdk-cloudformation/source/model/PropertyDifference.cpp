...
PropertyDifference::PropertyDifference(const XmlNode& xmlNode) { *this = xmlNode; }
...
PropertyDifference& PropertyDifference::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!differenceTypeNode.IsNull()) {
      m_differenceType = DifferenceTypeMapper::GetDifferenceTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(differenceTypeNode.GetText()).c_str()));
...
void PropertyDifference::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_differenceTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".DifferenceType=" << StringUtils::URLEncode(DifferenceTypeMapper::GetNameForDifferenceType(m_differenceType)) << "&";
  }
}
