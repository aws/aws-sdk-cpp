...
WarningProperty::WarningProperty(const XmlNode& xmlNode) { *this = xmlNode; }
...
WarningProperty& WarningProperty::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!requiredNode.IsNull()) {
      m_required =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(requiredNode.GetText()).c_str()).c_str());
