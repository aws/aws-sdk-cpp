...
DeploymentTargets::DeploymentTargets(const XmlNode& xmlNode) { *this = xmlNode; }
...
DeploymentTargets& DeploymentTargets::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!accountFilterTypeNode.IsNull()) {
      m_accountFilterType = AccountFilterTypeMapper::GetAccountFilterTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(accountFilterTypeNode.GetText()).c_str()));
...
void DeploymentTargets::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_organizationalUnitIdsHasBeenSet) {
    ...
    for (auto& item : m_organizationalUnitIds) {
      oStream << location << index << locationValue << ".OrganizationalUnitIds.member." << organizationalUnitIdsIdx++ << "="
  ...
  if (m_accountFilterTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".AccountFilterType=" << StringUtils::URLEncode(AccountFilterTypeMapper::GetNameForAccountFilterType(m_accountFilterType))
            << "&";
  }
}
