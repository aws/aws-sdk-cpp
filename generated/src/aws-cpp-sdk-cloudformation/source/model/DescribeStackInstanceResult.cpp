...
DescribeStackInstanceResult::DescribeStackInstanceResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
DescribeStackInstanceResult& DescribeStackInstanceResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stackInstanceNode.IsNull()) {
      ...
      m_stackInstanceHasBeenSet = true;
    }
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::CloudFormation::Model::DescribeStackInstanceResult",
