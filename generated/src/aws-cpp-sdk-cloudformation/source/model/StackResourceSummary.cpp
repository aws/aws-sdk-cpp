...
StackResourceSummary::StackResourceSummary(const XmlNode& xmlNode) { *this = xmlNode; }
...
StackResourceSummary& StackResourceSummary::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lastUpdatedTimestampNode.IsNull()) {
      m_lastUpdatedTimestamp =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastUpdatedTimestampNode.GetText()).c_str()).c_str(),
...
void StackResourceSummary::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastUpdatedTimestampHasBeenSet) {
    oStream << location << index << locationValue << ".LastUpdatedTimestamp="
            << StringUtils::URLEncode(m_lastUpdatedTimestamp.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
...
void StackResourceSummary::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_lastUpdatedTimestampHasBeenSet) {
    oStream << location << ".LastUpdatedTimestamp="
            << StringUtils::URLEncode(m_lastUpdatedTimestamp.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  if (m_resourceStatusHasBeenSet) {
