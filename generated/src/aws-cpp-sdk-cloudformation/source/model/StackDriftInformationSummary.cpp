...
StackDriftInformationSummary::StackDriftInformationSummary(const XmlNode& xmlNode) { *this = xmlNode; }
...
void StackDriftInformationSummary::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                  const char* locationValue) const {
  if (m_stackDriftStatusHasBeenSet) {
    oStream << location << index << locationValue
            << ".StackDriftStatus=" << StringUtils::URLEncode(StackDriftStatusMapper::GetNameForStackDriftStatus(m_stackDriftStatus))
            << "&";
  }
