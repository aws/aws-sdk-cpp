...
StackRefactorAction::StackRefactorAction(const XmlNode& xmlNode) { *this = xmlNode; }
...
StackRefactorAction& StackRefactorAction::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!actionNode.IsNull()) {
      m_action = StackRefactorActionTypeMapper::GetStackRefactorActionTypeForName(
    ...
    if (!entityNode.IsNull()) {
      m_entity = StackRefactorActionEntityMapper::GetStackRefactorActionEntityForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(entityNode.GetText()).c_str()));
    ...
    if (!detectionNode.IsNull()) {
      m_detection = StackRefactorDetectionMapper::GetStackRefactorDetectionForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(detectionNode.GetText()).c_str()));
...
void StackRefactorAction::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_actionHasBeenSet) {
    oStream << location << index << locationValue
            << ".Action=" << StringUtils::URLEncode(StackRefactorActionTypeMapper::GetNameForStackRefactorActionType(m_action)) << "&";
  }

  if (m_entityHasBeenSet) {
    oStream << location << index << locationValue
            << ".Entity=" << StringUtils::URLEncode(StackRefactorActionEntityMapper::GetNameForStackRefactorActionEntity(m_entity)) << "&";
  }
  ...
  if (m_detectionHasBeenSet) {
    oStream << location << index << locationValue
            << ".Detection=" << StringUtils::URLEncode(StackRefactorDetectionMapper::GetNameForStackRefactorDetection(m_detection)) << "&";
  }
  ...
  if (m_untagResourcesHasBeenSet) {
    ...
    for (auto& item : m_untagResources) {
      oStream << location << index << locationValue << ".UntagResources.member." << untagResourcesIdx++ << "="
