...
AccountGateResult::AccountGateResult(const XmlNode& xmlNode) { *this = xmlNode; }
...
AccountGateResult& AccountGateResult::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = AccountGateStatusMapper::GetAccountGateStatusForName(
...
void AccountGateResult::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(AccountGateStatusMapper::GetNameForAccountGateStatus(m_status)) << "&";
  }
