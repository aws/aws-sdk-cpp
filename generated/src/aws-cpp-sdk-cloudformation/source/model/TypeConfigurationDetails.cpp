...
TypeConfigurationDetails::TypeConfigurationDetails(const XmlNode& xmlNode) { *this = xmlNode; }
...
TypeConfigurationDetails& TypeConfigurationDetails::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!isDefaultConfigurationNode.IsNull()) {
      m_isDefaultConfiguration = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isDefaultConfigurationNode.GetText()).c_str()).c_str());
...
void TypeConfigurationDetails::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
