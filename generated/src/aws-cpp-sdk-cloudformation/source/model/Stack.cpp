...
Stack::Stack(const XmlNode& xmlNode) { *this = xmlNode; }
...
Stack& Stack::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!enableTerminationProtectionNode.IsNull()) {
      m_enableTerminationProtection = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(enableTerminationProtectionNode.GetText()).c_str()).c_str());
    ...
    if (!retainExceptOnCreateNode.IsNull()) {
      m_retainExceptOnCreate = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(retainExceptOnCreateNode.GetText()).c_str()).c_str());
    ...
    if (!deletionModeNode.IsNull()) {
      m_deletionMode = DeletionModeMapper::GetDeletionModeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deletionModeNode.GetText()).c_str()));
...
void Stack::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_notificationARNsHasBeenSet) {
    ...
    for (auto& item : m_notificationARNs) {
      oStream << location << index << locationValue << ".NotificationARNs.member." << notificationARNsIdx++ << "="
  ...
  if (m_capabilitiesHasBeenSet) {
    ...
    for (auto& item : m_capabilities) {
      oStream << location << index << locationValue << ".Capabilities.member." << capabilitiesIdx++ << "="
  ...
  if (m_deletionModeHasBeenSet) {
    oStream << location << index << locationValue
            << ".DeletionMode=" << StringUtils::URLEncode(DeletionModeMapper::GetNameForDeletionMode(m_deletionMode)) << "&";
  }
...
void Stack::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_capabilitiesHasBeenSet) {
    unsigned capabilitiesIdx = 1;
    for (auto& item : m_capabilities) {
      oStream << location << ".Capabilities.member." << capabilitiesIdx++ << "="
              << StringUtils::URLEncode(CapabilityMapper::GetNameForCapability(item)) << "&";
    }
  }
  if (m_outputsHasBeenSet) {
