...
ScannedResource::ScannedResource(const XmlNode& xmlNode) { *this = xmlNode; }
...
ScannedResource& ScannedResource::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!managedByStackNode.IsNull()) {
      m_managedByStack = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(managedByStackNode.GetText()).c_str()).c_str());
