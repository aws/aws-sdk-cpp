...
TypeSummary::TypeSummary(const XmlNode& xmlNode) { *this = xmlNode; }
...
TypeSummary& TypeSummary::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!publisherIdentityNode.IsNull()) {
      m_publisherIdentity = IdentityProviderMapper::GetIdentityProviderForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(publisherIdentityNode.GetText()).c_str()));
...
void TypeSummary::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastUpdatedHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastUpdated=" << StringUtils::URLEncode(m_lastUpdated.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  ...
  if (m_publisherIdentityHasBeenSet) {
    oStream << location << index << locationValue
            << ".PublisherIdentity=" << StringUtils::URLEncode(IdentityProviderMapper::GetNameForIdentityProvider(m_publisherIdentity))
            << "&";
  }
