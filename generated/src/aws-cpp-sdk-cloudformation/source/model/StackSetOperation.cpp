...
StackSetOperation::StackSetOperation(const XmlNode& xmlNode) { *this = xmlNode; }
...
StackSetOperation& StackSetOperation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!retainStacksNode.IsNull()) {
      m_retainStacks =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(retainStacksNode.GetText()).c_str()).c_str());
