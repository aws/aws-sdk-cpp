...
OperationResultFilter::OperationResultFilter(const XmlNode& xmlNode) { *this = xmlNode; }
...
OperationResultFilter& OperationResultFilter::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!nameNode.IsNull()) {
      m_name = OperationResultFilterNameMapper::GetOperationResultFilterNameForName(
...
void OperationResultFilter::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_nameHasBeenSet) {
    oStream << location << index << locationValue
            << ".Name=" << StringUtils::URLEncode(OperationResultFilterNameMapper::GetNameForOperationResultFilterName(m_name)) << "&";
  }
