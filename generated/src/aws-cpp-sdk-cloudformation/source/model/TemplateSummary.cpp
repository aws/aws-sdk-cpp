...
TemplateSummary::TemplateSummary(const XmlNode& xmlNode) { *this = xmlNode; }
...
TemplateSummary& TemplateSummary::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = GeneratedTemplateStatusMapper::GetGeneratedTemplateStatusForName(
    ...
    if (!numberOfResourcesNode.IsNull()) {
      m_numberOfResources = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(numberOfResourcesNode.GetText()).c_str()).c_str());
...
void TemplateSummary::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(GeneratedTemplateStatusMapper::GetNameForGeneratedTemplateStatus(m_status)) << "&";
  }
