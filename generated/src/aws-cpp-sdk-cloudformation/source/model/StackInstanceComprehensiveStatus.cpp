...
StackInstanceComprehensiveStatus::StackInstanceComprehensiveStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
StackInstanceComprehensiveStatus& StackInstanceComprehensiveStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!detailedStatusNode.IsNull()) {
      m_detailedStatus = StackInstanceDetailedStatusMapper::GetStackInstanceDetailedStatusForName(
...
void StackInstanceComprehensiveStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                      const char* locationValue) const {
  if (m_detailedStatusHasBeenSet) {
    oStream << location << index << locationValue << ".DetailedStatus="
            << StringUtils::URLEncode(StackInstanceDetailedStatusMapper::GetNameForStackInstanceDetailedStatus(m_detailedStatus)) << "&";
  }
}

void StackInstanceComprehensiveStatus::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_detailedStatusHasBeenSet) {
    oStream << location << ".DetailedStatus="
            << StringUtils::URLEncode(StackInstanceDetailedStatusMapper::GetNameForStackInstanceDetailedStatus(m_detailedStatus)) << "&";
  }
}
