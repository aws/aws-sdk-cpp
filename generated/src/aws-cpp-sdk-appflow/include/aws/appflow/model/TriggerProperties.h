...
class TriggerProperties {
 public:
  ...
  void SetScheduled(ScheduledT&& value) {
    m_scheduledHasBeenSet = true;
    m_scheduled = std::forward<ScheduledT>(value);
  }
  template <typename ScheduledT = ScheduledTriggerProperties>
  TriggerProperties& WithScheduled(ScheduledT&& value) {
    SetScheduled(std::forward<ScheduledT>(value));
    return *this;
  }
  ///@}
 private:
