...
class SAPODataConnectorProfileCredentials {
 public:
  ...
  template <typename BasicAuthCredentialsT = BasicAuthCredentials>
  SAPODataConnectorProfileCredentials& WithBasicAuthCredentials(BasicAuthCredentialsT&& value) {
  ...
  void SetOAuthCredentials(OAuthCredentialsT&& value) {
    m_oAuthCredentialsHasBeenSet = true;
    m_oAuthCredentials = std::forward<OAuthCredentialsT>(value);
  }
  template <typename OAuthCredentialsT = OAuthCredentials>
  SAPODataConnectorProfileCredentials& WithOAuthCredentials(OAuthCredentialsT&& value) {
    SetOAuthCredentials(std::forward<OAuthCredentialsT>(value));
    return *this;
  }
  ///@}
 private:
