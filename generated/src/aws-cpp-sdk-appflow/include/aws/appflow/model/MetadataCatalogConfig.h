...
class MetadataCatalogConfig {
 public:
  ...
  void SetGlueDataCatalog(GlueDataCatalogT&& value) {
    m_glueDataCatalogHasBeenSet = true;
    m_glueDataCatalog = std::forward<GlueDataCatalogT>(value);
  }
  template <typename GlueDataCatalogT = GlueDataCatalogConfig>
  MetadataCatalogConfig& WithGlueDataCatalog(GlueDataCatalogT&& value) {
    SetGlueDataCatalog(std::forward<GlueDataCatalogT>(value));
    return *this;
  }
  ///@}
 private:
