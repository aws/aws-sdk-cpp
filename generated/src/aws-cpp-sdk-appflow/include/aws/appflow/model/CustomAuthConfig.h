...
class CustomAuthConfig {
 public:
  ...
  void SetCustomAuthenticationType(CustomAuthenticationTypeT&& value) {
    ...
    m_customAuthenticationType = std::forward<CustomAuthenticationTypeT>(value);
  }
  template <typename CustomAuthenticationTypeT = Aws::String>
  CustomAuthConfig& WithCustomAuthenticationType(CustomAuthenticationTypeT&& value) {
    SetCustomAuthenticationType(std::forward<CustomAuthenticationTypeT>(value));
    return *this;
  }
  ///@}
  ...
  CustomAuthConfig& AddAuthParameters(AuthParametersT&& value) {
    m_authParametersHasBeenSet = true;
    m_authParameters.emplace_back(std::forward<AuthParametersT>(value));
    return *this;
  }
  ///@}
 private:
