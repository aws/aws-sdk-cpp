...
OAuth2Defaults::OAuth2Defaults(JsonView jsonValue) { *this = jsonValue; }
...
OAuth2Defaults& OAuth2Defaults::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("oauth2GrantTypesSupported")) {
    ...
    for (unsigned oauth2GrantTypesSupportedIndex = 0; oauth2GrantTypesSupportedIndex < oauth2GrantTypesSupportedJsonList.GetLength();
         ++oauth2GrantTypesSupportedIndex) {
      m_oauth2GrantTypesSupported.push_back(
          OAuth2GrantTypeMapper::GetOAuth2GrantTypeForName(oauth2GrantTypesSupportedJsonList[oauth2GrantTypesSupportedIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("oauth2CustomProperties")) {
    ...
    for (unsigned oauth2CustomPropertiesIndex = 0; oauth2CustomPropertiesIndex < oauth2CustomPropertiesJsonList.GetLength();
         ++oauth2CustomPropertiesIndex) {
...
JsonValue OAuth2Defaults::Jsonize() const {
  ...
  if (m_oauth2GrantTypesSupportedHasBeenSet) {
    ...
    for (unsigned oauth2GrantTypesSupportedIndex = 0; oauth2GrantTypesSupportedIndex < oauth2GrantTypesSupportedJsonList.GetLength();
         ++oauth2GrantTypesSupportedIndex) {
      oauth2GrantTypesSupportedJsonList[oauth2GrantTypesSupportedIndex].AsString(
          OAuth2GrantTypeMapper::GetNameForOAuth2GrantType(m_oauth2GrantTypesSupported[oauth2GrantTypesSupportedIndex]));
    }
