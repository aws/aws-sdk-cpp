...
SalesforceMetadata::SalesforceMetadata(JsonView jsonValue) { *this = jsonValue; }
...
SalesforceMetadata& SalesforceMetadata::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("dataTransferApis")) {
    ...
    for (unsigned dataTransferApisIndex = 0; dataTransferApisIndex < dataTransferApisJsonList.GetLength(); ++dataTransferApisIndex) {
      m_dataTransferApis.push_back(
          SalesforceDataTransferApiMapper::GetSalesforceDataTransferApiForName(dataTransferApisJsonList[dataTransferApisIndex].AsString()));
    }
...
JsonValue SalesforceMetadata::Jsonize() const {
  ...
  if (m_dataTransferApisHasBeenSet) {
    ...
    for (unsigned dataTransferApisIndex = 0; dataTransferApisIndex < dataTransferApisJsonList.GetLength(); ++dataTransferApisIndex) {
      dataTransferApisJsonList[dataTransferApisIndex].AsString(
          SalesforceDataTransferApiMapper::GetNameForSalesforceDataTransferApi(m_dataTransferApis[dataTransferApisIndex]));
    }
