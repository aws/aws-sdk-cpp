...
Task& Task::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("taskProperties")) {
    ...
    for (auto& taskPropertiesItem : taskPropertiesJsonMap) {
      m_taskProperties[OperatorPropertiesKeysMapper::GetOperatorPropertiesKeysForName(taskPropertiesItem.first)] =
          taskPropertiesItem.second.AsString();
    }
...
JsonValue Task::Jsonize() const {
  ...
  if (m_taskPropertiesHasBeenSet) {
    ...
    for (auto& taskPropertiesItem : m_taskProperties) {
      taskPropertiesJsonMap.WithString(OperatorPropertiesKeysMapper::GetNameForOperatorPropertiesKeys(taskPropertiesItem.first),
                                       taskPropertiesItem.second);
    }
