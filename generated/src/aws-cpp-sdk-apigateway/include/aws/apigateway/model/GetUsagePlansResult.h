...
class GetUsagePlansResult {
 public:
  ...
  template <typename PositionT = Aws::String>
  GetUsagePlansResult& WithPosition(PositionT&& value) {
  ...
  template <typename ItemsT = Aws::Vector<UsagePlan>>
  GetUsagePlansResult& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = UsagePlan>
  GetUsagePlansResult& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  GetUsagePlansResult& WithRequestId(RequestIdT&& value) {
