...
class GetRestApisResult {
 public:
  ...
  template <typename PositionT = Aws::String>
  GetRestApisResult& WithPosition(PositionT&& value) {
  ...
  template <typename ItemsT = Aws::Vector<RestApi>>
  GetRestApisResult& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = RestApi>
  GetRestApisResult& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  GetRestApisResult& WithRequestId(RequestIdT&& value) {
