...
class ApiStage {
 public:
  ...
  template <typename ApiIdT = Aws::String>
  ApiStage& WithApiId(ApiIdT&& value) {
  ...
  template <typename StageT = Aws::String>
  ApiStage& WithStage(StageT&& value) {
  ...
  template <typename ThrottleT = Aws::Map<Aws::String, ThrottleSettings>>
  ApiStage& WithThrottle(ThrottleT&& value) {
  ...
  ApiStage& AddThrottle(ThrottleKeyT&& key, ThrottleValueT&& value) {
    m_throttleHasBeenSet = true;
    m_throttle.emplace(std::forward<ThrottleKeyT>(key), std::forward<ThrottleValueT>(value));
    return *this;
  }
  ///@}
 private:
