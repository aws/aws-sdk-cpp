...
class TableLFTagPolicyAndPermissions {
 public:
  ...
  template <typename ExpressionT = Aws::Vector<LFTag>>
  TableLFTagPolicyAndPermissions& WithExpression(ExpressionT&& value) {
    SetExpression(std::forward<ExpressionT>(value));
    return *this;
  }
  template <typename ExpressionT = LFTag>
  TableLFTagPolicyAndPermissions& AddExpression(ExpressionT&& value) {
    m_expressionHasBeenSet = true;
  ...
  template <typename PermissionsT = Aws::Vector<TableTagPolicyLFPermission>>
  TableLFTagPolicyAndPermissions& WithPermissions(PermissionsT&& value) {
    SetPermissions(std::forward<PermissionsT>(value));
    return *this;
  }
  inline TableLFTagPolicyAndPermissions& AddPermissions(TableTagPolicyLFPermission value) {
    m_permissionsHasBeenSet = true;
