...
class AutoExportRevisionToS3RequestDetails {
 public:
  ...
  template <typename EncryptionT = ExportServerSideEncryption>
  AutoExportRevisionToS3RequestDetails& WithEncryption(EncryptionT&& value) {
  ...
  void SetRevisionDestination(RevisionDestinationT&& value) {
    m_revisionDestinationHasBeenSet = true;
    m_revisionDestination = std::forward<RevisionDestinationT>(value);
  }
  template <typename RevisionDestinationT = AutoExportRevisionDestinationEntry>
  AutoExportRevisionToS3RequestDetails& WithRevisionDestination(RevisionDestinationT&& value) {
    SetRevisionDestination(std::forward<RevisionDestinationT>(value));
    return *this;
  }
  ///@}
 private:
