...
class DatabaseLFTagPolicyAndPermissions {
 public:
  ...
  template <typename ExpressionT = Aws::Vector<LFTag>>
  DatabaseLFTagPolicyAndPermissions& WithExpression(ExpressionT&& value) {
    SetExpression(std::forward<ExpressionT>(value));
    return *this;
  }
  template <typename ExpressionT = LFTag>
  DatabaseLFTagPolicyAndPermissions& AddExpression(ExpressionT&& value) {
    m_expressionHasBeenSet = true;
  ...
  template <typename PermissionsT = Aws::Vector<DatabaseLFTagPolicyPermission>>
  DatabaseLFTagPolicyAndPermissions& WithPermissions(PermissionsT&& value) {
    SetPermissions(std::forward<PermissionsT>(value));
    return *this;
  }
  inline DatabaseLFTagPolicyAndPermissions& AddPermissions(DatabaseLFTagPolicyPermission value) {
    m_permissionsHasBeenSet = true;
