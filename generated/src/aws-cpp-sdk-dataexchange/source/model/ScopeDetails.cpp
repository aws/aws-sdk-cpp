...
ScopeDetails::ScopeDetails(JsonView jsonValue) { *this = jsonValue; }
...
ScopeDetails& ScopeDetails::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("LakeFormationTagPolicies")) {
    ...
    for (unsigned lakeFormationTagPoliciesIndex = 0; lakeFormationTagPoliciesIndex < lakeFormationTagPoliciesJsonList.GetLength();
         ++lakeFormationTagPoliciesIndex) {
  ...
  if (jsonValue.ValueExists("RedshiftDataShares")) {
    ...
    for (unsigned redshiftDataSharesIndex = 0; redshiftDataSharesIndex < redshiftDataSharesJsonList.GetLength();
         ++redshiftDataSharesIndex) {
...
JsonValue ScopeDetails::Jsonize() const {
  ...
  if (m_lakeFormationTagPoliciesHasBeenSet) {
    ...
    for (unsigned lakeFormationTagPoliciesIndex = 0; lakeFormationTagPoliciesIndex < lakeFormationTagPoliciesJsonList.GetLength();
         ++lakeFormationTagPoliciesIndex) {
      lakeFormationTagPoliciesJsonList[lakeFormationTagPoliciesIndex].AsObject(
          m_lakeFormationTagPolicies[lakeFormationTagPoliciesIndex].Jsonize());
    }
