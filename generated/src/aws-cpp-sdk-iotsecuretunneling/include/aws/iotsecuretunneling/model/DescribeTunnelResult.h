...
class DescribeTunnelResult {
 public:
  ...
  void SetTunnel(TunnelT&& value) {
    m_tunnelHasBeenSet = true;
    m_tunnel = std::forward<TunnelT>(value);
  }
  template <typename TunnelT = Tunnel>
  DescribeTunnelResult& WithTunnel(TunnelT&& value) {
    SetTunnel(std::forward<TunnelT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  DescribeTunnelResult& WithRequestId(RequestIdT&& value) {
