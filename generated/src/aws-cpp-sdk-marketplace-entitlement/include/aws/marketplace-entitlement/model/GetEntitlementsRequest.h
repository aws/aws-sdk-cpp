...
class GetEntitlementsRequest : public MarketplaceEntitlementServiceRequest {
 public:
  ...
  template <typename ProductCodeT = Aws::String>
  GetEntitlementsRequest& WithProductCode(ProductCodeT&& value) {
   ...
   * and <code>CustomerAWSAccountId</code> are mutually exclusive parameters. You
   * must use one or the other, but not both in the same request. </p>  <p>If
   * you're migrating an existing integration, use <a
  ...
  template <typename FilterT = Aws::Map<GetEntitlementFilterName, Aws::Vector<Aws::String>>>
  GetEntitlementsRequest& WithFilter(FilterT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  GetEntitlementsRequest& WithNextToken(NextTokenT&& value) {
  ...
  inline GetEntitlementsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
