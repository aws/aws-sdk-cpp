...
class FileVersion {
 public:
  ...
  void SetCommit(CommitT&& value) {
    ...
    m_commit = std::forward<CommitT>(value);
  }
  template <typename CommitT = Commit>
  FileVersion& WithCommit(CommitT&& value) {
    SetCommit(std::forward<CommitT>(value));
    return *this;
  }
  ///@}
  ...
  void SetRevisionChildren(RevisionChildrenT&& value) {
    ...
    m_revisionChildren = std::forward<RevisionChildrenT>(value);
  }
  template <typename RevisionChildrenT = Aws::Vector<Aws::String>>
  FileVersion& WithRevisionChildren(RevisionChildrenT&& value) {
    SetRevisionChildren(std::forward<RevisionChildrenT>(value));
    return *this;
  }
  template <typename RevisionChildrenT = Aws::String>
  FileVersion& AddRevisionChildren(RevisionChildrenT&& value) {
    m_revisionChildrenHasBeenSet = true;
    m_revisionChildren.emplace_back(std::forward<RevisionChildrenT>(value));
    return *this;
  }
  ///@}
 private:
