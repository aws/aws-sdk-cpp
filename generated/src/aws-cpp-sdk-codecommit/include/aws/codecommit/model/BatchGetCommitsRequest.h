...
class BatchGetCommitsRequest : public CodeCommitRequest {
 public:
  ...
  void SetCommitIds(CommitIdsT&& value) {
    m_commitIdsHasBeenSet = true;
    m_commitIds = std::forward<CommitIdsT>(value);
  }
  template <typename CommitIdsT = Aws::Vector<Aws::String>>
  BatchGetCommitsRequest& WithCommitIds(CommitIdsT&& value) {
    SetCommitIds(std::forward<CommitIdsT>(value));
    return *this;
  }
  template <typename CommitIdsT = Aws::String>
  BatchGetCommitsRequest& AddCommitIds(CommitIdsT&& value) {
    m_commitIdsHasBeenSet = true;
    m_commitIds.emplace_back(std::forward<CommitIdsT>(value));
    return *this;
  }
  ///@}
