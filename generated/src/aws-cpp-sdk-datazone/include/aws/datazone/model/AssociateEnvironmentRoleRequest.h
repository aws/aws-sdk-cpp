...
class AssociateEnvironmentRoleRequest : public DataZoneRequest {
 public:
  ...
  void SetEnvironmentRoleArn(EnvironmentRoleArnT&& value) {
    m_environmentRoleArnHasBeenSet = true;
    m_environmentRoleArn = std::forward<EnvironmentRoleArnT>(value);
  }
  template <typename EnvironmentRoleArnT = Aws::String>
  AssociateEnvironmentRoleRequest& WithEnvironmentRoleArn(EnvironmentRoleArnT&& value) {
    SetEnvironmentRoleArn(std::forward<EnvironmentRoleArnT>(value));
    return *this;
  }
  ///@}
 private:
