...
class GlossaryTermItemAdditionalAttributes {
 public:
  ...
  template <typename MatchRationaleT = Aws::Vector<MatchRationaleItem>>
  GlossaryTermItemAdditionalAttributes& WithMatchRationale(MatchRationaleT&& value) {
    SetMatchRationale(std::forward<MatchRationaleT>(value));
    return *this;
  }
  template <typename MatchRationaleT = MatchRationaleItem>
  GlossaryTermItemAdditionalAttributes& AddMatchRationale(MatchRationaleT&& value) {
    m_matchRationaleHasBeenSet = true;
