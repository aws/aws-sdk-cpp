...
class TextMatchItem {
 public:
  ...
  void SetMatchOffsets(MatchOffsetsT&& value) {
    ...
    m_matchOffsets = std::forward<MatchOffsetsT>(value);
  }
  template <typename MatchOffsetsT = Aws::Vector<MatchOffset>>
  TextMatchItem& WithMatchOffsets(MatchOffsetsT&& value) {
    SetMatchOffsets(std::forward<MatchOffsetsT>(value));
    return *this;
  }
  template <typename MatchOffsetsT = MatchOffset>
  TextMatchItem& AddMatchOffsets(MatchOffsetsT&& value) {
    m_matchOffsetsHasBeenSet = true;
    m_matchOffsets.emplace_back(std::forward<MatchOffsetsT>(value));
    return *this;
  }
  ///@}
