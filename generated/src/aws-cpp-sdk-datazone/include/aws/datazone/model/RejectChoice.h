...
class RejectChoice {
 public:
  ...
  void SetPredictionChoices(PredictionChoicesT&& value) {
    ...
    m_predictionChoices = std::forward<PredictionChoicesT>(value);
  }
  template <typename PredictionChoicesT = Aws::Vector<int>>
  RejectChoice& WithPredictionChoices(PredictionChoicesT&& value) {
    SetPredictionChoices(std::forward<PredictionChoicesT>(value));
    return *this;
  }
  inline RejectChoice& AddPredictionChoices(int value) {
    m_predictionChoicesHasBeenSet = true;
    m_predictionChoices.push_back(value);
    return *this;
  }
  ///@}
 private:
