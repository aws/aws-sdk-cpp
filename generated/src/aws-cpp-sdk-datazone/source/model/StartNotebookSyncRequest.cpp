/**
 * Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
 * SPDX-License-Identifier: Apache-2.0.
 */

#include <aws/core/utils/json/JsonSerializer.h>
#include <aws/datazone/model/StartNotebookSyncRequest.h>

#include <utility>

using namespace Aws::DataZone::Model;
using namespace Aws::Utils::Json;
using namespace Aws::Utils;

Aws::String StartNotebookSyncRequest::SerializePayload() const {
  JsonValue payload;

  if (m_owningProjectIdentifierHasBeenSet) {
    payload.WithString("owningProjectIdentifier", m_owningProjectIdentifier);
  }

  if (m_sourceLocationHasBeenSet) {
    payload.WithObject("sourceLocation", m_sourceLocation.Jsonize());
  }

  if (m_gitMetadataHasBeenSet) {
    payload.WithObject("gitMetadata", m_gitMetadata.Jsonize());
  }

  if (m_notebookIdHasBeenSet) {
    payload.WithString("notebookId", m_notebookId);
  }

  if (m_nameHasBeenSet) {
    payload.WithString("name", m_name);
  }

  if (m_descriptionHasBeenSet) {
    payload.WithString("description", m_description);
  }

  if (m_clientTokenHasBeenSet) {
    payload.WithString("clientToken", m_clientToken);
  }

  return payload.View().WriteReadable();
}
