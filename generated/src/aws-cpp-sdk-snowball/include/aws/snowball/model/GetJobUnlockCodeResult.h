...
class GetJobUnlockCodeResult {
 public:
  ...
  AWS_SNOWBALL_API GetJobUnlockCodeResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  AWS_SNOWBALL_API GetJobUnlockCodeResult& operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  ...
  void SetUnlockCode(UnlockCodeT&& value) {
    m_unlockCodeHasBeenSet = true;
    m_unlockCode = std::forward<UnlockCodeT>(value);
  }
  template <typename UnlockCodeT = Aws::String>
  GetJobUnlockCodeResult& WithUnlockCode(UnlockCodeT&& value) {
    SetUnlockCode(std::forward<UnlockCodeT>(value));
    return *this;
  }
  ///@}
