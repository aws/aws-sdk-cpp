...
class KeyRange {
 public:
  ...
  AWS_SNOWBALL_API KeyRange(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_SNOWBALL_API KeyRange& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  void SetBeginMarker(BeginMarkerT&& value) {
    ...
    m_beginMarker = std::forward<BeginMarkerT>(value);
  }
  template <typename BeginMarkerT = Aws::String>
  KeyRange& WithBeginMarker(BeginMarkerT&& value) {
    SetBeginMarker(std::forward<BeginMarkerT>(value));
    return *this;
  }
  ///@}
  ...
  void SetEndMarker(EndMarkerT&& value) {
    ...
    m_endMarker = std::forward<EndMarkerT>(value);
  }
  template <typename EndMarkerT = Aws::String>
  KeyRange& WithEndMarker(EndMarkerT&& value) {
    SetEndMarker(std::forward<EndMarkerT>(value));
    return *this;
  }
  ///@}
 private:
