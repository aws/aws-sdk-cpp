...
Notification::Notification(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue Notification::Jsonize() const {
  ...
  if (m_jobStatesToNotifyHasBeenSet) {
    ...
    for (unsigned jobStatesToNotifyIndex = 0; jobStatesToNotifyIndex < jobStatesToNotifyJsonList.GetLength(); ++jobStatesToNotifyIndex) {
      jobStatesToNotifyJsonList[jobStatesToNotifyIndex].AsString(
          JobStateMapper::GetNameForJobState(m_jobStatesToNotify[jobStatesToNotifyIndex]));
    }
