...
Version::Version(JsonView jsonValue) { *this = jsonValue; }
...
Version& Version::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("parameterDefinitions")) {
    ...
    for (unsigned parameterDefinitionsIndex = 0; parameterDefinitionsIndex < parameterDefinitionsJsonList.GetLength();
         ++parameterDefinitionsIndex) {
  ...
  if (jsonValue.ValueExists("requiredCapabilities")) {
    ...
    for (unsigned requiredCapabilitiesIndex = 0; requiredCapabilitiesIndex < requiredCapabilitiesJsonList.GetLength();
      ...
      m_requiredCapabilities.push_back(
          CapabilityMapper::GetCapabilityForName(requiredCapabilitiesJsonList[requiredCapabilitiesIndex].AsString()));
    }
...
JsonValue Version::Jsonize() const {
  ...
  if (m_requiredCapabilitiesHasBeenSet) {
    ...
    for (unsigned requiredCapabilitiesIndex = 0; requiredCapabilitiesIndex < requiredCapabilitiesJsonList.GetLength();
         ++requiredCapabilitiesIndex) {
      requiredCapabilitiesJsonList[requiredCapabilitiesIndex].AsString(
          CapabilityMapper::GetNameForCapability(m_requiredCapabilities[requiredCapabilitiesIndex]));
    }
