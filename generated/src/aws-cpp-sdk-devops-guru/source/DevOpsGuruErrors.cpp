...
template <>
AWS_DEVOPSGURU_API ConflictException DevOpsGuruError::GetModeledError() {
  assert(this->GetErrorType() == DevOpsGuruErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_DEVOPSGURU_API ThrottlingException DevOpsGuruError::GetModeledError() {
  assert(this->GetErrorType() == DevOpsGuruErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_DEVOPSGURU_API InternalServerException DevOpsGuruError::GetModeledError() {
  assert(this->GetErrorType() == DevOpsGuruErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_DEVOPSGURU_API ResourceNotFoundException DevOpsGuruError::GetModeledError() {
  assert(this->GetErrorType() == DevOpsGuruErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_DEVOPSGURU_API ValidationException DevOpsGuruError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(DevOpsGuruErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
  }
