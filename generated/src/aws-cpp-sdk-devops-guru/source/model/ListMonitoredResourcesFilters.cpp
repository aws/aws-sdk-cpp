...
ListMonitoredResourcesFilters::ListMonitoredResourcesFilters(JsonView jsonValue) { *this = jsonValue; }
...
ListMonitoredResourcesFilters& ListMonitoredResourcesFilters::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ResourceTypeFilters")) {
    ...
    for (unsigned resourceTypeFiltersIndex = 0; resourceTypeFiltersIndex < resourceTypeFiltersJsonList.GetLength();
      ...
      m_resourceTypeFilters.push_back(
          ResourceTypeFilterMapper::GetResourceTypeFilterForName(resourceTypeFiltersJsonList[resourceTypeFiltersIndex].AsString()));
    }
...
JsonValue ListMonitoredResourcesFilters::Jsonize() const {
  ...
  if (m_resourceTypeFiltersHasBeenSet) {
    ...
    for (unsigned resourceTypeFiltersIndex = 0; resourceTypeFiltersIndex < resourceTypeFiltersJsonList.GetLength();
         ++resourceTypeFiltersIndex) {
      resourceTypeFiltersJsonList[resourceTypeFiltersIndex].AsString(
          ResourceTypeFilterMapper::GetNameForResourceTypeFilter(m_resourceTypeFilters[resourceTypeFiltersIndex]));
    }
