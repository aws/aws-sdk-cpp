...
class DatasetSchema {
 public:
  ...
  void SetInlineDataSchema(InlineDataSchemaT&& value) {
    m_inlineDataSchemaHasBeenSet = true;
    m_inlineDataSchema = std::forward<InlineDataSchemaT>(value);
  }
  template <typename InlineDataSchemaT = Aws::String>
  DatasetSchema& WithInlineDataSchema(InlineDataSchemaT&& value) {
    SetInlineDataSchema(std::forward<InlineDataSchemaT>(value));
    return *this;
  }
  ///@}
 private:
