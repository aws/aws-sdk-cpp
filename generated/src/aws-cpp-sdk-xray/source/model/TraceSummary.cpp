...
TraceSummary::TraceSummary(JsonView jsonValue) { *this = jsonValue; }
...
TraceSummary& TraceSummary::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ResponseTimeRootCauses")) {
    ...
    for (unsigned responseTimeRootCausesIndex = 0; responseTimeRootCausesIndex < responseTimeRootCausesJsonList.GetLength();
         ++responseTimeRootCausesIndex) {
