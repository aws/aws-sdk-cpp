...
GraphLink::GraphLink(JsonView jsonValue) { *this = jsonValue; }
...
GraphLink& GraphLink::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("DestinationTraceIds")) {
    ...
    for (unsigned destinationTraceIdsIndex = 0; destinationTraceIdsIndex < destinationTraceIdsJsonList.GetLength();
         ++destinationTraceIdsIndex) {
