...
class IndexingRuleValue {
 public:
  ...
  void SetProbabilistic(ProbabilisticT&& value) {
    m_probabilisticHasBeenSet = true;
    m_probabilistic = std::forward<ProbabilisticT>(value);
  }
  template <typename ProbabilisticT = ProbabilisticRuleValue>
  IndexingRuleValue& WithProbabilistic(ProbabilisticT&& value) {
    SetProbabilistic(std::forward<ProbabilisticT>(value));
    return *this;
  }
  ///@}
 private:
