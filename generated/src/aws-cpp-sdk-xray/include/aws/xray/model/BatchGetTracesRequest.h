...
class BatchGetTracesRequest : public XRayRequest {
 public:
  ...
  template <typename TraceIdsT = Aws::Vector<Aws::String>>
  BatchGetTracesRequest& WithTraceIds(TraceIdsT&& value) {
    SetTraceIds(std::forward<TraceIdsT>(value));
    return *this;
  }
  template <typename TraceIdsT = Aws::String>
  BatchGetTracesRequest& AddTraceIds(TraceIdsT&& value) {
    m_traceIdsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  BatchGetTracesRequest& WithNextToken(NextTokenT&& value) {
