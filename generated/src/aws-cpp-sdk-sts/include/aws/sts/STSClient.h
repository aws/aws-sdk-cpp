...
/**
 * <fullname>Security Token Service</fullname> <p>Amazon Web Services provides
 * Security Token Service (STS) as a web service that enables you to request
 * temporary, limited-privilege credentials for users. This guide describes the STS
 * API. For more information, see <a
 * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp.html">Temporary
 * Security Credentials</a> in the <i>IAM User Guide</i>.</p>  <p>As an
 * alternative to using the API, you can use one of the Amazon Web Services SDKs,
 * which consist of libraries and sample code for various programming languages and
 * platforms such as Java, Ruby, .NET, iOS, Android, and others. The SDKs provide a
 * convenient way to create programmatic access to STS. For example, the SDKs can
 * cryptographically sign requests, manage errors, and retry requests
 * automatically. For information about the Amazon Web Services SDKs, see <a
 * href="http://aws.amazon.com/tools/">Tools to Build on Amazon Web
 * Services</a>.</p>  <p>For information about setting up signatures and
 ...
 * Reference</i>. For general information about the Query API, see <a
 * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/IAM_UsingQueryAPI.html">Making
 * Query Requests</a> in the <i>IAM User Guide</i>. For information about using
 * security tokens with other Amazon Web Services products, see <a
 * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_aws-services-that-work-with-iam.html">Amazon
 * Web Services Services That Work with IAM</a> in the <i>IAM User Guide</i>.</p>
 * <p>For information about STS endpoints, see <a
 * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_region-endpoints.html">STS
 * Regions and endpoints</a> in the <i>IAM User Guide</i>. For information about
 * logging STS API calls, see <a
 * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/cloudtrail-integration.html">Logging
 * IAM and STS API calls with CloudTrail</a> in the <i>IAM User Guide</i>.</p>
 */
class AWS_STS_API STSClient
 ...
 public:
   ...
   */
  STSClient(const Aws::Auth::AWSCredentials& credentials, std::shared_ptr<STSEndpointProviderBase> endpointProvider = nullptr,
   ...
   * STS credentials</a> in the <i>IAM User Guide</i>.</p> <p> <b>Permissions</b>
   * </p> <p>The temporary security credentials created by <code>AssumeRole</code>
   ...
   * operations.</p> <p>(Optional) You can pass inline or managed session policies to
   * this operation. You can pass a single JSON policy document to use as an inline
   * session policy. You can also specify up to 10 managed policy Amazon Resource
   * Names (ARNs) to use as managed session policies. The plaintext that you use for
   * both inline and managed session policies can't exceed 2,048 characters. Passing
   * policies to this operation returns new temporary credentials. The resulting
   * session's permissions are the intersection of the role's identity-based policy
   * and the session policies. You can use the role's temporary credentials in
   * subsequent Amazon Web Services API calls to access resources in the account that
  ...
  /**
   * An Async wrapper for AssumeRole that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename AssumeRoleRequestT = Model::AssumeRoleRequest>
  void AssumeRoleAsync(const AssumeRoleRequestT& request, const AssumeRoleResponseReceivedHandler& handler,
  ...
  /**
   * A Callable wrapper for AssumeRoleWithSAML that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for AssumeRoleWithSAML that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename AssumeRoleWithSAMLRequestT = Model::AssumeRoleWithSAMLRequest>
  void AssumeRoleWithSAMLAsync(const AssumeRoleWithSAMLRequestT& request, const AssumeRoleWithSAMLResponseReceivedHandler& handler,
                               const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * the maximum session duration for a role </a> in the <i>IAM User Guide</i>. The
   * maximum session duration limit applies when you use the <code>AssumeRole*</code>
   * API operations or the <code>assume-role*</code> CLI commands. However the limit
   ...
   * how to use OIDC federation and the <code>AssumeRoleWithWebIdentity</code> API,
   * see the following resources: </p> <ul> <li> <p> <a
   ...
   * credentials. </p> </li> </ul><p><h3>See Also:</h3>   <a
  ...
  /**
   * A Callable wrapper for AssumeRoleWithWebIdentity that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename AssumeRoleWithWebIdentityRequestT = Model::AssumeRoleWithWebIdentityRequest>
  Model::AssumeRoleWithWebIdentityOutcomeCallable AssumeRoleWithWebIdentityCallable(
      const AssumeRoleWithWebIdentityRequestT& request) const {
    return SubmitCallable(&STSClient::AssumeRoleWithWebIdentity, request);
  }

  /**
   * An Async wrapper for AssumeRoleWithWebIdentity that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename AssumeRoleWithWebIdentityRequestT = Model::AssumeRoleWithWebIdentityRequest>
  void AssumeRoleWithWebIdentityAsync(const AssumeRoleWithWebIdentityRequestT& request,
   ...
   * request to a Regional STS endpoint. For more information, see <a
   * href="https://docs.aws.amazon.com/STS/latest/APIReference/welcome.html#sts-endpoints">Endpoints</a>.</p>
  ...
  Model::AssumeRootOutcomeCallable AssumeRootCallable(const AssumeRootRequestT& request) const {
    return SubmitCallable(&STSClient::AssumeRoot, request);
  }

  /**
   * An Async wrapper for AssumeRoot that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename AssumeRootRequestT = Model::AssumeRootRequest>
  void AssumeRootAsync(const AssumeRootRequestT& request, const AssumeRootResponseReceivedHandler& handler,
                       const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
    return SubmitAsync(&STSClient::AssumeRoot, request, handler, context);
  }
   ...
   * API Reference</a></p>
   */
  virtual Model::DecodeAuthorizationMessageOutcome DecodeAuthorizationMessage(
      const Model::DecodeAuthorizationMessageRequest& request) const;

  /**
   * A Callable wrapper for DecodeAuthorizationMessage that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DecodeAuthorizationMessageRequestT = Model::DecodeAuthorizationMessageRequest>
  Model::DecodeAuthorizationMessageOutcomeCallable DecodeAuthorizationMessageCallable(
      const DecodeAuthorizationMessageRequestT& request) const {
    return SubmitCallable(&STSClient::DecodeAuthorizationMessage, request);
  }

  /**
   * An Async wrapper for DecodeAuthorizationMessage that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DecodeAuthorizationMessageRequestT = Model::DecodeAuthorizationMessageRequest>
  void DecodeAuthorizationMessageAsync(const DecodeAuthorizationMessageRequestT& request,
                                       const DecodeAuthorizationMessageResponseReceivedHandler& handler,
                                       const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for GetAccessKeyInfo that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for GetAccessKeyInfo that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename GetAccessKeyInfoRequestT = Model::GetAccessKeyInfoRequest>
  void GetAccessKeyInfoAsync(const GetAccessKeyInfoRequestT& request, const GetAccessKeyInfoResponseReceivedHandler& handler,
                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for GetCallerIdentity that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for GetCallerIdentity that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename GetCallerIdentityRequestT = Model::GetCallerIdentityRequest>
  void GetCallerIdentityAsync(const GetCallerIdentityResponseReceivedHandler& handler,
   ...
   * Temporary Security Credentials</a> and <a
   * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_sts-comparison.html">Compare
   * STS credentials</a> in the <i>IAM User Guide</i>.</p> <p>Although it is possible
   * to call <code>GetFederationToken</code> using the security credentials of an
   * Amazon Web Services account root user rather than an IAM user that you create
   * for the purpose of a proxy application, we do not recommend it. For more
   * information, see <a
   ...
   * Temporary Security Credentials</a> and <a
   * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_sts-comparison.html">Compare
   * STS credentials</a> in the <i>IAM User Guide</i>.</p>  <p>No permissions
   * are required for users to perform this operation. The purpose of the
   * <code>sts:GetSessionToken</code> operation is to authenticate the user using
   * MFA. You cannot use policies to control authentication operations. For more
   * information, see <a
  ...
  /**
   * An Async wrapper for GetSessionToken that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename GetSessionTokenRequestT = Model::GetSessionTokenRequest>
  void GetSessionTokenAsync(const GetSessionTokenResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
   ...
   * issuer's JWKS endpoint.</p>  <p>The <code>GetWebIdentityToken</code> API
   * is not available on the STS Global endpoint.</p> <p><h3>See Also:</h3>
   * <a
