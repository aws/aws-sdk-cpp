  ...
  if (m_transitiveTagKeysHasBeenSet) {
    if (m_transitiveTagKeys.empty()) {
      ...
      for (auto& item : m_transitiveTagKeys) {
        ss << "TransitiveTagKeys.member." << transitiveTagKeysCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void AssumeRoleRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
