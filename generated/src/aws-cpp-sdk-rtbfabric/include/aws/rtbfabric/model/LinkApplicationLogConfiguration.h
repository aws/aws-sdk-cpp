...
class LinkApplicationLogConfiguration {
 public:
  ...
  void SetSampling(SamplingT&& value) {
    m_samplingHasBeenSet = true;
    m_sampling = std::forward<SamplingT>(value);
  }
  template <typename SamplingT = LinkApplicationLogSampling>
  LinkApplicationLogConfiguration& WithSampling(SamplingT&& value) {
    SetSampling(std::forward<SamplingT>(value));
    return *this;
  }
  ///@}
 private:
