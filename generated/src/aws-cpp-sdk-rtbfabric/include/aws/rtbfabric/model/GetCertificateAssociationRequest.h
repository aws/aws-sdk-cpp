...
/**
 */
class GetCertificateAssociationRequest : public RTBFabricRequest {
 public:
  AWS_RTBFABRIC_API GetCertificateAssociationRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetCertificateAssociation"; }
  ...
  template <typename GatewayIdT = Aws::String>
  GetCertificateAssociationRequest& WithGatewayId(GatewayIdT&& value) {
  ...
  template <typename AcmCertificateArnT = Aws::String>
  GetCertificateAssociationRequest& WithAcmCertificateArn(AcmCertificateArnT&& value) {
