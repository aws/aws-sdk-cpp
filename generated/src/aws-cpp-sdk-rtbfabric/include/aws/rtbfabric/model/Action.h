...
class Action {
 public:
  ...
  void SetHeaderTag(HeaderTagT&& value) {
    m_headerTagHasBeenSet = true;
    m_headerTag = std::forward<HeaderTagT>(value);
  }
  template <typename HeaderTagT = HeaderTagAction>
  Action& WithHeaderTag(HeaderTagT&& value) {
    SetHeaderTag(std::forward<HeaderTagT>(value));
    return *this;
  }
  ///@}
 private:
