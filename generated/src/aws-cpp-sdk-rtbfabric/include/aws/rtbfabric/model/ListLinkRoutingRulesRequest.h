...
/**
 */
class ListLinkRoutingRulesRequest : public RTBFabricRequest {
 public:
  AWS_RTBFABRIC_API ListLinkRoutingRulesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListLinkRoutingRules"; }
  ...
  template <typename GatewayIdT = Aws::String>
  ListLinkRoutingRulesRequest& WithGatewayId(GatewayIdT&& value) {
  ...
  template <typename LinkIdT = Aws::String>
  ListLinkRoutingRulesRequest& WithLinkId(LinkIdT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListLinkRoutingRulesRequest& WithNextToken(NextTokenT&& value) {
  ...
  inline ListLinkRoutingRulesRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
