...
BatchGetAssetPropertyAggregatesEntry::BatchGetAssetPropertyAggregatesEntry(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue BatchGetAssetPropertyAggregatesEntry::Jsonize() const {
  ...
  if (m_aggregateTypesHasBeenSet) {
    ...
    for (unsigned aggregateTypesIndex = 0; aggregateTypesIndex < aggregateTypesJsonList.GetLength(); ++aggregateTypesIndex) {
      aggregateTypesJsonList[aggregateTypesIndex].AsString(
          AggregateTypeMapper::GetNameForAggregateType(m_aggregateTypes[aggregateTypesIndex]));
    }
