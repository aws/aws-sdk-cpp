...
class BatchAssociateProjectAssetsResult {
 public:
  ...
  template <typename ErrorsT = Aws::Vector<AssetErrorDetails>>
  BatchAssociateProjectAssetsResult& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = AssetErrorDetails>
  BatchAssociateProjectAssetsResult& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchAssociateProjectAssetsResult& WithRequestId(RequestIdT&& value) {
