...
class UpdateWorkspaceResult {
 public:
  AWS_IOTSITEWISE_API UpdateWorkspaceResult() = default;
  AWS_IOTSITEWISE_API UpdateWorkspaceResult(const Aws::AmazonWebServiceResult<Aws::Utils::Json::JsonValue>& result);
  AWS_IOTSITEWISE_API UpdateWorkspaceResult& operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Json::JsonValue>& result);

  ///@{
  /**
   * <p>The status of the workspace after the update, which is <code>UPDATING</code>
   * when the operation returns.</p>
   */
  ...
  template <typename WorkspaceStatusT = WorkspaceStatus>
  UpdateWorkspaceResult& WithWorkspaceStatus(WorkspaceStatusT&& value) {
