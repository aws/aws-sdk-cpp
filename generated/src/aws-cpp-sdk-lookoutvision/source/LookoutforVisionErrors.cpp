...
template <>
AWS_LOOKOUTFORVISION_API ConflictException LookoutforVisionError::GetModeledError() {
  assert(this->GetErrorType() == LookoutforVisionErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_LOOKOUTFORVISION_API ThrottlingException LookoutforVisionError::GetModeledError() {
  assert(this->GetErrorType() == LookoutforVisionErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_LOOKOUTFORVISION_API ServiceQuotaExceededException LookoutforVisionError::GetModeledError() {
  assert(this->GetErrorType() == LookoutforVisionErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_LOOKOUTFORVISION_API InternalServerException LookoutforVisionError::GetModeledError() {
  assert(this->GetErrorType() == LookoutforVisionErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_LOOKOUTFORVISION_API ResourceNotFoundException LookoutforVisionError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(LookoutforVisionErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
  }
