...
class SearchPrincipalType {
 public:
  ...
  template <typename IdT = Aws::String>
  SearchPrincipalType& WithId(IdT&& value) {
  ...
  template <typename RolesT = Aws::Vector<PrincipalRoleType>>
  SearchPrincipalType& WithRoles(RolesT&& value) {
    SetRoles(std::forward<RolesT>(value));
    return *this;
  }
  inline SearchPrincipalType& AddRoles(PrincipalRoleType value) {
    m_rolesHasBeenSet = true;
    m_roles.push_back(value);
    return *this;
  }
  ///@}
 private:
