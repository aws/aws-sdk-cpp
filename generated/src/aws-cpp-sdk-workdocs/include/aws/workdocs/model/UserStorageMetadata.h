...
class UserStorageMetadata {
 public:
  ...
  inline void SetStorageUtilizedInBytes(long long value) {
    m_storageUtilizedInBytesHasBeenSet = true;
    m_storageUtilizedInBytes = value;
  }
  inline UserStorageMetadata& WithStorageUtilizedInBytes(long long value) {
    SetStorageUtilizedInBytes(value);
    return *this;
  }
  ///@}
  ...
  void SetStorageRule(StorageRuleT&& value) {
    ...
    m_storageRule = std::forward<StorageRuleT>(value);
  }
  template <typename StorageRuleT = StorageRuleType>
  UserStorageMetadata& WithStorageRule(StorageRuleT&& value) {
    SetStorageRule(std::forward<StorageRuleT>(value));
    return *this;
  }
  ///@}
 private:
