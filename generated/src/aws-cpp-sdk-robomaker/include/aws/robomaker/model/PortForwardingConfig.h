...
class PortForwardingConfig {
 public:
  ...
  void SetPortMappings(PortMappingsT&& value) {
    ...
    m_portMappings = std::forward<PortMappingsT>(value);
  }
  template <typename PortMappingsT = Aws::Vector<PortMapping>>
  PortForwardingConfig& WithPortMappings(PortMappingsT&& value) {
    SetPortMappings(std::forward<PortMappingsT>(value));
    return *this;
  }
  template <typename PortMappingsT = PortMapping>
  PortForwardingConfig& AddPortMappings(PortMappingsT&& value) {
    m_portMappingsHasBeenSet = true;
    m_portMappings.emplace_back(std::forward<PortMappingsT>(value));
    return *this;
  }
  ///@}
 private:
