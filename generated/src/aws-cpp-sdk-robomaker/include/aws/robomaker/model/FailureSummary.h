...
class FailureSummary {
 public:
  ...
  inline void SetTotalFailureCount(int value) {
    m_totalFailureCountHasBeenSet = true;
    m_totalFailureCount = value;
  }
  inline FailureSummary& WithTotalFailureCount(int value) {
  ...
  template <typename FailuresT = Aws::Vector<WorldFailure>>
  FailureSummary& WithFailures(FailuresT&& value) {
    SetFailures(std::forward<FailuresT>(value));
    return *this;
  }
  template <typename FailuresT = WorldFailure>
  FailureSummary& AddFailures(FailuresT&& value) {
    m_failuresHasBeenSet = true;
