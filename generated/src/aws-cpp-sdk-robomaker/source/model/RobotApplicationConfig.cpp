...
RobotApplicationConfig::RobotApplicationConfig(JsonView jsonValue) { *this = jsonValue; }
...
RobotApplicationConfig& RobotApplicationConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("uploadConfigurations")) {
    ...
    for (unsigned uploadConfigurationsIndex = 0; uploadConfigurationsIndex < uploadConfigurationsJsonList.GetLength();
         ++uploadConfigurationsIndex) {
