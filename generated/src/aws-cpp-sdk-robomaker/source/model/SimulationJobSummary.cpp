...
SimulationJobSummary::SimulationJobSummary(JsonView jsonValue) { *this = jsonValue; }
...
SimulationJobSummary& SimulationJobSummary::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("simulationApplicationNames")) {
    ...
    for (unsigned simulationApplicationNamesIndex = 0; simulationApplicationNamesIndex < simulationApplicationNamesJsonList.GetLength();
         ++simulationApplicationNamesIndex) {
  ...
  if (jsonValue.ValueExists("robotApplicationNames")) {
    ...
    for (unsigned robotApplicationNamesIndex = 0; robotApplicationNamesIndex < robotApplicationNamesJsonList.GetLength();
         ++robotApplicationNamesIndex) {
...
JsonValue SimulationJobSummary::Jsonize() const {
  ...
  if (m_simulationApplicationNamesHasBeenSet) {
    ...
    for (unsigned simulationApplicationNamesIndex = 0; simulationApplicationNamesIndex < simulationApplicationNamesJsonList.GetLength();
         ++simulationApplicationNamesIndex) {
      simulationApplicationNamesJsonList[simulationApplicationNamesIndex].AsString(
          m_simulationApplicationNames[simulationApplicationNamesIndex]);
    }
