...
DeploymentJob::DeploymentJob(JsonView jsonValue) { *this = jsonValue; }
...
DeploymentJob& DeploymentJob::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("deploymentApplicationConfigs")) {
    ...
    for (unsigned deploymentApplicationConfigsIndex = 0;
         deploymentApplicationConfigsIndex < deploymentApplicationConfigsJsonList.GetLength(); ++deploymentApplicationConfigsIndex) {
...
JsonValue DeploymentJob::Jsonize() const {
  ...
  if (m_deploymentApplicationConfigsHasBeenSet) {
    ...
    for (unsigned deploymentApplicationConfigsIndex = 0;
         deploymentApplicationConfigsIndex < deploymentApplicationConfigsJsonList.GetLength(); ++deploymentApplicationConfigsIndex) {
      deploymentApplicationConfigsJsonList[deploymentApplicationConfigsIndex].AsObject(
          m_deploymentApplicationConfigs[deploymentApplicationConfigsIndex].Jsonize());
    }
