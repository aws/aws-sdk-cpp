...
PossibleRemediationAction::PossibleRemediationAction(JsonView jsonValue) { *this = jsonValue; }
...
PossibleRemediationAction& PossibleRemediationAction::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("OrderedRemediationActions")) {
    ...
    for (unsigned orderedRemediationActionsIndex = 0; orderedRemediationActionsIndex < orderedRemediationActionsJsonList.GetLength();
         ++orderedRemediationActionsIndex) {
...
JsonValue PossibleRemediationAction::Jsonize() const {
  ...
  if (m_orderedRemediationActionsHasBeenSet) {
    ...
    for (unsigned orderedRemediationActionsIndex = 0; orderedRemediationActionsIndex < orderedRemediationActionsJsonList.GetLength();
         ++orderedRemediationActionsIndex) {
      orderedRemediationActionsJsonList[orderedRemediationActionsIndex].AsObject(
          m_orderedRemediationActions[orderedRemediationActionsIndex].Jsonize());
    }
