...
PolicyComplianceStatus::PolicyComplianceStatus(JsonView jsonValue) { *this = jsonValue; }
...
PolicyComplianceStatus& PolicyComplianceStatus::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("IssueInfoMap")) {
    ...
    for (auto& issueInfoMapItem : issueInfoMapJsonMap) {
      m_issueInfoMap[DependentServiceNameMapper::GetDependentServiceNameForName(issueInfoMapItem.first)] =
          issueInfoMapItem.second.AsString();
    }
...
JsonValue PolicyComplianceStatus::Jsonize() const {
  ...
  if (m_issueInfoMapHasBeenSet) {
    ...
    for (auto& issueInfoMapItem : m_issueInfoMap) {
      issueInfoMapJsonMap.WithString(DependentServiceNameMapper::GetNameForDependentServiceName(issueInfoMapItem.first),
                                     issueInfoMapItem.second);
    }
