...
class SecurityServicePolicyData {
 public:
  ...
  inline void SetType(SecurityServiceType value) {
    m_typeHasBeenSet = true;
    m_type = value;
  }
  inline SecurityServicePolicyData& WithType(SecurityServiceType value) {
    SetType(value);
    return *this;
  }
  ///@}
   ...
   * 10000.</p>  </li> <li> <p>Example: <code>IMPORT_NETWORK_FIREWALL</code>
   * </p> <p>
   ...
   * </p> <p>If you set <code>optimizeUnassociatedWebACL</code> to <code>true</code>,
   * Firewall Manager creates web ACLs in accounts within the policy scope if the web
   * ACLs will be used by at least one resource. Firewall Manager creates web ACLs in
   * the accounts within policy scope only if the web ACLs will be used by at least
   * one resource. If at any time an account comes into policy scope, Firewall
   * Manager automatically creates a web ACL in the account if at least one resource
   * will use the web ACL.</p> <p>Upon enablement, Firewall Manager performs a
   ...
   * creates a web ACL, Firewall Manager doesn't disassociate the resource from the
   * web ACL. If you want Firewall Manager to clean up the web ACL, you must first
   * manually disassociate the resources from the web ACL, and then enable the manage
   * unused web ACLs option in your policy.</p> <p>If you set
   * <code>optimizeUnassociatedWebACL</code> to <code>false</code>, and Firewall
   * Manager automatically creates an empty web ACL in each account that's within
   * policy scope.</p> </li> <li> <p>Specification for <code>SHIELD_ADVANCED</code>
   * for Amazon CloudFront distributions </p> <p>
   * <code>"{\"type\":\"SHIELD_ADVANCED\",\"automaticResponseConfiguration\":
   * {\"automaticResponseStatus\":\"ENABLED|IGNORED|DISABLED\",
   * \"automaticResponseAction\":\"BLOCK|COUNT\"},
   * \"overrideCustomerWebaclClassic\":true|false,
   * \"optimizeUnassociatedWebACL\":true|false}"</code> </p> <p>For example:
   * <code>"{\"type\":\"SHIELD_ADVANCED\",\"automaticResponseConfiguration\":
   ...
   * <li> <p>Example: <code>THIRD_PARTY_FIREWALL</code> </p> <p>Replace
   * <code>THIRD_PARTY_FIREWALL_NAME</code> with the name of the third-party
   * firewall.</p> <p> <code>"{ "type":"THIRD_PARTY_FIREWALL",
   ...
   * } } } }"</code> </p> </li> <li> <p>Example: <code>WAFV2</code> - Account
   * takeover prevention, Bot Control managed rule groups, optimize unassociated web
   * ACL, and rule action override </p> <p>
   ...
   * </p> <ul> <li> <p>Bot Control - For information about
   ...
   * in the <i>WAF API Reference</i>.</p> </li> <li> <p>Fraud Control account
   * takeover prevention (ATP) - For information about the properties available for
   ...
   * in the <i>WAF API Reference</i>.</p> </li> <li> <p>Optimize unassociated web ACL
   ...
   * web ACLs when you first enable management of unused web ACLs in a policy.</p>
   * <p>If you set <code>optimizeUnassociatedWebACL</code> to <code>false</code>
   ...
   * scope.</p> </li> <li> <p>Rule action overrides - Firewall Manager supports rule
   * action overrides only for managed rule groups. To configure a
   * <code>RuleActionOverrides</code> add the <code>Name</code> of the rule to
   * override, and <code>ActionToUse</code>, which is the new action to use for the
   * rule. For information about using rule action override, see <a
   ...
   * <code>WAFV2</code> - <code>CAPTCHA</code> and <code>Challenge</code> configs
   * </p> <p>
   ...
   * </p> <ul> <li> <p> <code>CAPTCHA</code> and <code>Challenge</code> configs - If
   * you update the policy's values for <code>associationConfig</code>,
   * <code>captchaConfig</code>, <code>challengeConfig</code>, or
   * <code>tokenDomains</code>, Firewall Manager will overwrite your local web ACLs
   * to contain the new value(s). However, if you don't update the policy's
   * <code>associationConfig</code>, <code>captchaConfig</code>,
   * <code>challengeConfig</code>, or <code>tokenDomains</code> values, the values in
   * your local web ACLs will remain unchanged. For information about association
   * configs, see <a
   ...
   * in the <i>WAF API Reference</i>.</p> </li> <li> <p>
   * <code>defaultSizeInspectionLimit</code> - Specifies the maximum size of the web
   * request body component that an associated Amazon CloudFront distribution should
   * send to WAF for inspection. For more information, see <a
   ...
   * the WAF managed rule group. </p> </li> <li> <p>Example: <code>WAFV2</code> -
   * Logging configurations </p> <p>
   ...
   * </p> <p>Firewall Manager supports Amazon Kinesis Data Firehose and Amazon S3 as
   * the <code>logDestinationConfigs</code> in your
   ...
   * in the <i>WAF API Reference</i> </p> <p>In the
  ...
  void SetManagedServiceData(ManagedServiceDataT&& value) {
    ...
    m_managedServiceData = std::forward<ManagedServiceDataT>(value);
  }
  template <typename ManagedServiceDataT = Aws::String>
  SecurityServicePolicyData& WithManagedServiceData(ManagedServiceDataT&& value) {
    SetManagedServiceData(std::forward<ManagedServiceDataT>(value));
    return *this;
  }
  ///@}
  ...
  void SetPolicyOption(PolicyOptionT&& value) {
    ...
    m_policyOption = std::forward<PolicyOptionT>(value);
  }
  template <typename PolicyOptionT = PolicyOption>
  SecurityServicePolicyData& WithPolicyOption(PolicyOptionT&& value) {
    SetPolicyOption(std::forward<PolicyOptionT>(value));
    return *this;
  }
  ///@}
 private:
