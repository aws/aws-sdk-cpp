...
class PartialMatch {
 public:
  ...
  void SetTargetViolationReasons(TargetViolationReasonsT&& value) {
    ...
    m_targetViolationReasons = std::forward<TargetViolationReasonsT>(value);
  }
  template <typename TargetViolationReasonsT = Aws::Vector<Aws::String>>
  PartialMatch& WithTargetViolationReasons(TargetViolationReasonsT&& value) {
    SetTargetViolationReasons(std::forward<TargetViolationReasonsT>(value));
    return *this;
  }
  template <typename TargetViolationReasonsT = Aws::String>
  PartialMatch& AddTargetViolationReasons(TargetViolationReasonsT&& value) {
    m_targetViolationReasonsHasBeenSet = true;
    m_targetViolationReasons.emplace_back(std::forward<TargetViolationReasonsT>(value));
    return *this;
  }
  ///@}
 private:
