...
class LambdaTarget {
 public:
  ...
  inline LambdaTarget& WithStatus(TargetStatus value) {
    SetStatus(value);
    return *this;
  }
  ///@}
  ...
  void SetLambdaFunctionInfo(LambdaFunctionInfoT&& value) {
    ...
    m_lambdaFunctionInfo = std::forward<LambdaFunctionInfoT>(value);
  }
  template <typename LambdaFunctionInfoT = LambdaFunctionInfo>
  LambdaTarget& WithLambdaFunctionInfo(LambdaFunctionInfoT&& value) {
    SetLambdaFunctionInfo(std::forward<LambdaFunctionInfoT>(value));
    return *this;
  }
  ///@}
 private:
