...
LoadBalancerInfo::LoadBalancerInfo(JsonView jsonValue) { *this = jsonValue; }
...
LoadBalancerInfo& LoadBalancerInfo::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("targetGroupInfoList")) {
    ...
    for (unsigned targetGroupInfoListIndex = 0; targetGroupInfoListIndex < targetGroupInfoListJsonList.GetLength();
         ++targetGroupInfoListIndex) {
  ...
  if (jsonValue.ValueExists("targetGroupPairInfoList")) {
    ...
    for (unsigned targetGroupPairInfoListIndex = 0; targetGroupPairInfoListIndex < targetGroupPairInfoListJsonList.GetLength();
         ++targetGroupPairInfoListIndex) {
...
JsonValue LoadBalancerInfo::Jsonize() const {
  ...
  if (m_targetGroupPairInfoListHasBeenSet) {
    ...
    for (unsigned targetGroupPairInfoListIndex = 0; targetGroupPairInfoListIndex < targetGroupPairInfoListJsonList.GetLength();
         ++targetGroupPairInfoListIndex) {
      targetGroupPairInfoListJsonList[targetGroupPairInfoListIndex].AsObject(
          m_targetGroupPairInfoList[targetGroupPairInfoListIndex].Jsonize());
    }
