...
GetGuardrailResult::GetGuardrailResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
GetGuardrailResult& GetGuardrailResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("failureRecommendations")) {
    ...
    for (unsigned failureRecommendationsIndex = 0; failureRecommendationsIndex < failureRecommendationsJsonList.GetLength();
         ++failureRecommendationsIndex) {
