...
AutomatedReasoningCheckTranslation::AutomatedReasoningCheckTranslation(JsonView jsonValue) { *this = jsonValue; }

AutomatedReasoningCheckTranslation& AutomatedReasoningCheckTranslation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("untranslatedPremises")) {
    ...
    for (unsigned untranslatedPremisesIndex = 0; untranslatedPremisesIndex < untranslatedPremisesJsonList.GetLength();
         ++untranslatedPremisesIndex) {
  ...
  if (jsonValue.ValueExists("untranslatedClaims")) {
    ...
    for (unsigned untranslatedClaimsIndex = 0; untranslatedClaimsIndex < untranslatedClaimsJsonList.GetLength();
         ++untranslatedClaimsIndex) {
...
JsonValue AutomatedReasoningCheckTranslation::Jsonize() const {
  JsonValue payload;
