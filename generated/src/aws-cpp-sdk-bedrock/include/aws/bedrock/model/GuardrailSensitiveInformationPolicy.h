...
class GuardrailSensitiveInformationPolicy {
 public:
  ...
  template <typename PiiEntitiesT = Aws::Vector<GuardrailPiiEntity>>
  GuardrailSensitiveInformationPolicy& WithPiiEntities(PiiEntitiesT&& value) {
    SetPiiEntities(std::forward<PiiEntitiesT>(value));
    return *this;
  }
  template <typename PiiEntitiesT = GuardrailPiiEntity>
  GuardrailSensitiveInformationPolicy& AddPiiEntities(PiiEntitiesT&& value) {
    m_piiEntitiesHasBeenSet = true;
  ...
  template <typename RegexesT = Aws::Vector<GuardrailRegex>>
  GuardrailSensitiveInformationPolicy& WithRegexes(RegexesT&& value) {
    SetRegexes(std::forward<RegexesT>(value));
    return *this;
  }
  template <typename RegexesT = GuardrailRegex>
  GuardrailSensitiveInformationPolicy& AddRegexes(RegexesT&& value) {
    m_regexesHasBeenSet = true;
