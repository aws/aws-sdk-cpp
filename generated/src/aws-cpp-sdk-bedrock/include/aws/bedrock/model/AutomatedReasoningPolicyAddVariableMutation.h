...
class AutomatedReasoningPolicyAddVariableMutation {
 public:
  ...
  void SetVariable(VariableT&& value) {
    m_variableHasBeenSet = true;
    m_variable = std::forward<VariableT>(value);
  }
  template <typename VariableT = AutomatedReasoningPolicyDefinitionVariable>
  AutomatedReasoningPolicyAddVariableMutation& WithVariable(VariableT&& value) {
    SetVariable(std::forward<VariableT>(value));
    return *this;
  }
  ///@}
 private:
