...
class AutomatedReasoningPolicyBuildWorkflowRepairContent {
 public:
  ...
  template <typename AnnotationsT = Aws::Vector<AutomatedReasoningPolicyAnnotation>>
  AutomatedReasoningPolicyBuildWorkflowRepairContent& WithAnnotations(AnnotationsT&& value) {
    SetAnnotations(std::forward<AnnotationsT>(value));
    return *this;
  }
  template <typename AnnotationsT = AutomatedReasoningPolicyAnnotation>
  AutomatedReasoningPolicyBuildWorkflowRepairContent& AddAnnotations(AnnotationsT&& value) {
    m_annotationsHasBeenSet = true;
