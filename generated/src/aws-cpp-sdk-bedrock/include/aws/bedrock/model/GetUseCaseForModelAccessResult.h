...
class GetUseCaseForModelAccessResult {
 public:
  ...
  void SetFormData(FormDataT&& value) {
    m_formDataHasBeenSet = true;
    m_formData = std::forward<FormDataT>(value);
  }
  template <typename FormDataT = Aws::Utils::ByteBuffer>
  GetUseCaseForModelAccessResult& WithFormData(FormDataT&& value) {
    SetFormData(std::forward<FormDataT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetUseCaseForModelAccessResult& WithRequestId(RequestIdT&& value) {
