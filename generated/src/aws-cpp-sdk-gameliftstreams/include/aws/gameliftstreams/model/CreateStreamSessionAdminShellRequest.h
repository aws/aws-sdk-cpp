...
/**
 */
class CreateStreamSessionAdminShellRequest : public GameLiftStreamsRequest {
 public:
  AWS_GAMELIFTSTREAMS_API CreateStreamSessionAdminShellRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "CreateStreamSessionAdminShell"; }
  ...
  template <typename IdentifierT = Aws::String>
  CreateStreamSessionAdminShellRequest& WithIdentifier(IdentifierT&& value) {
  ...
  template <typename StreamSessionIdentifierT = Aws::String>
  CreateStreamSessionAdminShellRequest& WithStreamSessionIdentifier(StreamSessionIdentifierT&& value) {
