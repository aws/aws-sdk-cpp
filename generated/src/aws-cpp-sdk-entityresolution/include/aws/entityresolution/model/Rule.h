...
class Rule {
 public:
  ...
  void SetMatchingKeys(MatchingKeysT&& value) {
    ...
    m_matchingKeys = std::forward<MatchingKeysT>(value);
  }
  template <typename MatchingKeysT = Aws::Vector<Aws::String>>
  Rule& WithMatchingKeys(MatchingKeysT&& value) {
    SetMatchingKeys(std::forward<MatchingKeysT>(value));
    return *this;
  }
  template <typename MatchingKeysT = Aws::String>
  Rule& AddMatchingKeys(MatchingKeysT&& value) {
    m_matchingKeysHasBeenSet = true;
    m_matchingKeys.emplace_back(std::forward<MatchingKeysT>(value));
    return *this;
  }
  ///@}
 private:
