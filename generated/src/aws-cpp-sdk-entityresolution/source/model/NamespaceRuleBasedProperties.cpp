...
NamespaceRuleBasedProperties::NamespaceRuleBasedProperties(JsonView jsonValue) { *this = jsonValue; }
...
NamespaceRuleBasedProperties& NamespaceRuleBasedProperties::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ruleDefinitionTypes")) {
    ...
    for (unsigned ruleDefinitionTypesIndex = 0; ruleDefinitionTypesIndex < ruleDefinitionTypesJsonList.GetLength();
         ++ruleDefinitionTypesIndex) {
      m_ruleDefinitionTypes.push_back(IdMappingWorkflowRuleDefinitionTypeMapper::GetIdMappingWorkflowRuleDefinitionTypeForName(
          ruleDefinitionTypesJsonList[ruleDefinitionTypesIndex].AsString()));
    }
    m_ruleDefinitionTypesHasBeenSet = true;
  }
  if (jsonValue.ValueExists("attributeMatchingModel")) {
    m_attributeMatchingModel =
        AttributeMatchingModelMapper::GetAttributeMatchingModelForName(jsonValue.GetString("attributeMatchingModel"));
  ...
  if (jsonValue.ValueExists("recordMatchingModels")) {
    ...
    for (unsigned recordMatchingModelsIndex = 0; recordMatchingModelsIndex < recordMatchingModelsJsonList.GetLength();
         ++recordMatchingModelsIndex) {
      m_recordMatchingModels.push_back(
          RecordMatchingModelMapper::GetRecordMatchingModelForName(recordMatchingModelsJsonList[recordMatchingModelsIndex].AsString()));
    }
...
JsonValue NamespaceRuleBasedProperties::Jsonize() const {
  ...
  if (m_ruleDefinitionTypesHasBeenSet) {
    ...
    for (unsigned ruleDefinitionTypesIndex = 0; ruleDefinitionTypesIndex < ruleDefinitionTypesJsonList.GetLength();
         ++ruleDefinitionTypesIndex) {
      ruleDefinitionTypesJsonList[ruleDefinitionTypesIndex].AsString(
          IdMappingWorkflowRuleDefinitionTypeMapper::GetNameForIdMappingWorkflowRuleDefinitionType(
              m_ruleDefinitionTypes[ruleDefinitionTypesIndex]));
    }
  ...
  if (m_recordMatchingModelsHasBeenSet) {
    ...
    for (unsigned recordMatchingModelsIndex = 0; recordMatchingModelsIndex < recordMatchingModelsJsonList.GetLength();
         ++recordMatchingModelsIndex) {
      recordMatchingModelsJsonList[recordMatchingModelsIndex].AsString(
          RecordMatchingModelMapper::GetNameForRecordMatchingModel(m_recordMatchingModels[recordMatchingModelsIndex]));
    }
