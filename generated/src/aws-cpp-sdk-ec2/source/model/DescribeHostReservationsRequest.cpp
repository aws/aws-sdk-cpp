...
Aws::String DescribeHostReservationsRequest::SerializePayload() const {
  ...
  if (m_hostReservationIdSetHasBeenSet) {
    ...
    for (auto& item : m_hostReservationIdSet) {
      ss << "HostReservationIdSet." << hostReservationIdSetCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeHostReservationsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
