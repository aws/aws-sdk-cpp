...
AnalysisAclRule::AnalysisAclRule(const XmlNode& xmlNode) { *this = xmlNode; }
...
AnalysisAclRule& AnalysisAclRule::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!ruleNumberNode.IsNull()) {
      m_ruleNumber =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(ruleNumberNode.GetText()).c_str()).c_str());
