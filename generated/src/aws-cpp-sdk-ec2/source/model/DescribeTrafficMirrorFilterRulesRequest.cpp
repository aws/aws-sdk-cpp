...
Aws::String DescribeTrafficMirrorFilterRulesRequest::SerializePayload() const {
  ...
  if (m_trafficMirrorFilterRuleIdsHasBeenSet) {
    ...
    for (auto& item : m_trafficMirrorFilterRuleIds) {
      ss << "TrafficMirrorFilterRuleId." << trafficMirrorFilterRuleIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeTrafficMirrorFilterRulesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
