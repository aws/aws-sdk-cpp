...
GetNetworkInsightsAccessScopeAnalysisFindingsResponse& GetNetworkInsightsAccessScopeAnalysisFindingsResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!analysisStatusNode.IsNull()) {
      m_analysisStatus = AnalysisStatusMapper::GetAnalysisStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(analysisStatusNode.GetText()).c_str()));
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::GetNetworkInsightsAccessScopeAnalysisFindingsResponse",
