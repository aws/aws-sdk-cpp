...
RouteServerPeer::RouteServerPeer(const XmlNode& xmlNode) { *this = xmlNode; }
...
RouteServerPeer& RouteServerPeer::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = RouteServerPeerStateMapper::GetRouteServerPeerStateForName(
...
void RouteServerPeer::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(RouteServerPeerStateMapper::GetNameForRouteServerPeerState(m_state)) << "&";
  }
