...
BlockPublicAccessStates::BlockPublicAccessStates(const XmlNode& xmlNode) { *this = xmlNode; }
...
BlockPublicAccessStates& BlockPublicAccessStates::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!internetGatewayBlockModeNode.IsNull()) {
      m_internetGatewayBlockMode = BlockPublicAccessModeMapper::GetBlockPublicAccessModeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(internetGatewayBlockModeNode.GetText()).c_str()));
...
void BlockPublicAccessStates::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_internetGatewayBlockModeHasBeenSet) {
    oStream << location << index << locationValue << ".InternetGatewayBlockMode="
            << StringUtils::URLEncode(BlockPublicAccessModeMapper::GetNameForBlockPublicAccessMode(m_internetGatewayBlockMode)) << "&";
  }
}

void BlockPublicAccessStates::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_internetGatewayBlockModeHasBeenSet) {
    oStream << location << ".InternetGatewayBlockMode="
            << StringUtils::URLEncode(BlockPublicAccessModeMapper::GetNameForBlockPublicAccessMode(m_internetGatewayBlockMode)) << "&";
  }
}
