...
NetworkInsightsPath::NetworkInsightsPath(const XmlNode& xmlNode) { *this = xmlNode; }
...
NetworkInsightsPath& NetworkInsightsPath::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!protocolNode.IsNull()) {
      m_protocol =
          ProtocolMapper::GetProtocolForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(protocolNode.GetText()).c_str()));
      m_protocolHasBeenSet = true;
    }
    XmlNode destinationPortNode = resultNode.FirstChild("destinationPort");
    if (!destinationPortNode.IsNull()) {
      m_destinationPort = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(destinationPortNode.GetText()).c_str()).c_str());
