...
Aws::String DescribeLocalGatewayVirtualInterfacesRequest::SerializePayload() const {
  ...
  if (m_localGatewayVirtualInterfaceIdsHasBeenSet) {
    ...
    for (auto& item : m_localGatewayVirtualInterfaceIds) {
      ss << "LocalGatewayVirtualInterfaceId." << localGatewayVirtualInterfaceIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeLocalGatewayVirtualInterfacesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
