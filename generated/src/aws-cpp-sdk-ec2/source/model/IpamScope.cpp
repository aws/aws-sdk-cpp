...
IpamScope::IpamScope(const XmlNode& xmlNode) { *this = xmlNode; }
...
IpamScope& IpamScope::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!ipamScopeTypeNode.IsNull()) {
      m_ipamScopeType = IpamScopeTypeMapper::GetIpamScopeTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(ipamScopeTypeNode.GetText()).c_str()));
    ...
    if (!poolCountNode.IsNull()) {
      m_poolCount =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(poolCountNode.GetText()).c_str()).c_str());
      m_poolCountHasBeenSet = true;
    }
    XmlNode stateNode = resultNode.FirstChild("state");
    if (!stateNode.IsNull()) {
      m_state = IpamScopeStateMapper::GetIpamScopeStateForName(
...
void IpamScope::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_ipamScopeTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".IpamScopeType=" << StringUtils::URLEncode(IpamScopeTypeMapper::GetNameForIpamScopeType(m_ipamScopeType)) << "&";
  }
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(IpamScopeStateMapper::GetNameForIpamScopeState(m_state)) << "&";
  }
