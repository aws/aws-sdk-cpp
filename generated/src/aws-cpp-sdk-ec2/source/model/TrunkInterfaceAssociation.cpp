...
TrunkInterfaceAssociation::TrunkInterfaceAssociation(const XmlNode& xmlNode) { *this = xmlNode; }
...
TrunkInterfaceAssociation& TrunkInterfaceAssociation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!interfaceProtocolNode.IsNull()) {
      m_interfaceProtocol = InterfaceProtocolTypeMapper::GetInterfaceProtocolTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(interfaceProtocolNode.GetText()).c_str()));
      m_interfaceProtocolHasBeenSet = true;
    }
    XmlNode vlanIdNode = resultNode.FirstChild("vlanId");
    if (!vlanIdNode.IsNull()) {
      m_vlanId =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(vlanIdNode.GetText()).c_str()).c_str());
      m_vlanIdHasBeenSet = true;
    }
    XmlNode greKeyNode = resultNode.FirstChild("greKey");
    if (!greKeyNode.IsNull()) {
      m_greKey =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(greKeyNode.GetText()).c_str()).c_str());
...
void TrunkInterfaceAssociation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_interfaceProtocolHasBeenSet) {
    oStream << location << index << locationValue << ".InterfaceProtocol="
            << StringUtils::URLEncode(InterfaceProtocolTypeMapper::GetNameForInterfaceProtocolType(m_interfaceProtocol)) << "&";
  }
...
void TrunkInterfaceAssociation::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_interfaceProtocolHasBeenSet) {
    oStream << location << ".InterfaceProtocol="
            << StringUtils::URLEncode(InterfaceProtocolTypeMapper::GetNameForInterfaceProtocolType(m_interfaceProtocol)) << "&";
  }
  if (m_vlanIdHasBeenSet) {
