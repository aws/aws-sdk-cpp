...
MacModificationTask::MacModificationTask(const XmlNode& xmlNode) { *this = xmlNode; }
...
MacModificationTask& MacModificationTask::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!taskStateNode.IsNull()) {
      m_taskState = MacModificationTaskStateMapper::GetMacModificationTaskStateForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(taskStateNode.GetText()).c_str()));
      m_taskStateHasBeenSet = true;
    }
    XmlNode taskTypeNode = resultNode.FirstChild("taskType");
    if (!taskTypeNode.IsNull()) {
      m_taskType = MacModificationTaskTypeMapper::GetMacModificationTaskTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(taskTypeNode.GetText()).c_str()));
...
void MacModificationTask::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_taskStateHasBeenSet) {
    oStream << location << index << locationValue
            << ".TaskState=" << StringUtils::URLEncode(MacModificationTaskStateMapper::GetNameForMacModificationTaskState(m_taskState))
            << "&";
  }

  if (m_taskTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".TaskType=" << StringUtils::URLEncode(MacModificationTaskTypeMapper::GetNameForMacModificationTaskType(m_taskType)) << "&";
  }
}
