...
ImageMetadata::ImageMetadata(const XmlNode& xmlNode) { *this = xmlNode; }
...
ImageMetadata& ImageMetadata::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state =
          ImageStateMapper::GetImageStateForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(stateNode.GetText()).c_str()));
    ...
    if (!imageAllowedNode.IsNull()) {
      m_imageAllowed =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(imageAllowedNode.GetText()).c_str()).c_str());
