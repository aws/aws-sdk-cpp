...
ImageUsageResourceTypeOption::ImageUsageResourceTypeOption(const XmlNode& xmlNode) { *this = xmlNode; }
...
void ImageUsageResourceTypeOption::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_optionValuesHasBeenSet) {
    ...
    for (auto& item : m_optionValues) {
      oStream << location << index << locationValue << ".OptionValueSet." << optionValuesIdx++ << "="
