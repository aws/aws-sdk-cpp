...
TrafficMirrorSession::TrafficMirrorSession(const XmlNode& xmlNode) { *this = xmlNode; }
...
TrafficMirrorSession& TrafficMirrorSession::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!packetLengthNode.IsNull()) {
      m_packetLength =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(packetLengthNode.GetText()).c_str()).c_str());
      m_packetLengthHasBeenSet = true;
    }
    XmlNode sessionNumberNode = resultNode.FirstChild("sessionNumber");
    if (!sessionNumberNode.IsNull()) {
      m_sessionNumber = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sessionNumberNode.GetText()).c_str()).c_str());
    ...
    if (!virtualNetworkIdNode.IsNull()) {
      m_virtualNetworkId = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(virtualNetworkIdNode.GetText()).c_str()).c_str());
