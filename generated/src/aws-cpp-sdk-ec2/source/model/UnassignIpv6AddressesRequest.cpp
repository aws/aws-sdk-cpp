...
Aws::String UnassignIpv6AddressesRequest::SerializePayload() const {
  ...
  if (m_ipv6PrefixesHasBeenSet) {
    ...
    for (auto& item : m_ipv6Prefixes) {
      ss << "Ipv6Prefix." << ipv6PrefixesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      ipv6PrefixesCount++;
    }
  }
  ...
  if (m_ipv6AddressesHasBeenSet) {
    ...
    for (auto& item : m_ipv6Addresses) {
      ss << "Ipv6Addresses." << ipv6AddressesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      ipv6AddressesCount++;
    }
  }
...
void UnassignIpv6AddressesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
