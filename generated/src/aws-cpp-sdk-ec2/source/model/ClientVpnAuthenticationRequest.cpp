...
ClientVpnAuthenticationRequest::ClientVpnAuthenticationRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
ClientVpnAuthenticationRequest& ClientVpnAuthenticationRequest::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = ClientVpnAuthenticationTypeMapper::GetClientVpnAuthenticationTypeForName(
...
void ClientVpnAuthenticationRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                    const char* locationValue) const {
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(ClientVpnAuthenticationTypeMapper::GetNameForClientVpnAuthenticationType(m_type)) << "&";
  }
