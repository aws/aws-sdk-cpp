...
DescribeClientVpnAuthorizationRulesResponse& DescribeClientVpnAuthorizationRulesResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "DescribeClientVpnAuthorizationRulesResponse")) {
    resultNode = rootNode.FirstChild("DescribeClientVpnAuthorizationRulesResponse");
  }

  if (!resultNode.IsNull()) {
    XmlNode authorizationRulesNode = resultNode.FirstChild("authorizationRule");
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::DescribeClientVpnAuthorizationRulesResponse",
