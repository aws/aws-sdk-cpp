...
BundleTask::BundleTask(const XmlNode& xmlNode) { *this = xmlNode; }
...
BundleTask& BundleTask::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = BundleTaskStateMapper::GetBundleTaskStateForName(
...
void BundleTask::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(BundleTaskStateMapper::GetNameForBundleTaskState(m_state)) << "&";
  }
