...
Route::Route(const XmlNode& xmlNode) { *this = xmlNode; }
...
Route& Route::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!originNode.IsNull()) {
      m_origin =
          RouteOriginMapper::GetRouteOriginForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(originNode.GetText()).c_str()));
      m_originHasBeenSet = true;
    }
    XmlNode stateNode = resultNode.FirstChild("state");
    if (!stateNode.IsNull()) {
      m_state =
          RouteStateMapper::GetRouteStateForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(stateNode.GetText()).c_str()));
...
void Route::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_destinationIpv6CidrBlockHasBeenSet) {
    oStream << location << index << locationValue
            << ".DestinationIpv6CidrBlock=" << StringUtils::URLEncode(m_destinationIpv6CidrBlock.c_str()) << "&";
  }
  ...
  if (m_egressOnlyInternetGatewayIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".EgressOnlyInternetGatewayId=" << StringUtils::URLEncode(m_egressOnlyInternetGatewayId.c_str()) << "&";
  }
  ...
  if (m_originHasBeenSet) {
    oStream << location << index << locationValue
            << ".Origin=" << StringUtils::URLEncode(RouteOriginMapper::GetNameForRouteOrigin(m_origin)) << "&";
  }
