...
MaintenanceDetails::MaintenanceDetails(const XmlNode& xmlNode) { *this = xmlNode; }
...
MaintenanceDetails& MaintenanceDetails::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!maintenanceAutoAppliedAfterNode.IsNull()) {
      m_maintenanceAutoAppliedAfter =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maintenanceAutoAppliedAfterNode.GetText()).c_str()).c_str(),
    ...
    if (!lastMaintenanceAppliedNode.IsNull()) {
      m_lastMaintenanceApplied =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastMaintenanceAppliedNode.GetText()).c_str()).c_str(),
                   Aws::Utils::DateFormat::ISO_8601);
...
void MaintenanceDetails::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_maintenanceAutoAppliedAfterHasBeenSet) {
    oStream << location << index << locationValue << ".MaintenanceAutoAppliedAfter="
            << StringUtils::URLEncode(m_maintenanceAutoAppliedAfter.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }

  if (m_lastMaintenanceAppliedHasBeenSet) {
    oStream << location << index << locationValue << ".LastMaintenanceApplied="
            << StringUtils::URLEncode(m_lastMaintenanceApplied.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
...
void MaintenanceDetails::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_maintenanceAutoAppliedAfterHasBeenSet) {
    oStream << location << ".MaintenanceAutoAppliedAfter="
            << StringUtils::URLEncode(m_maintenanceAutoAppliedAfter.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  if (m_lastMaintenanceAppliedHasBeenSet) {
    oStream << location << ".LastMaintenanceApplied="
            << StringUtils::URLEncode(m_lastMaintenanceApplied.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
