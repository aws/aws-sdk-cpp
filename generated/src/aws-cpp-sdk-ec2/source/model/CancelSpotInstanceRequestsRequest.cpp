...
Aws::String CancelSpotInstanceRequestsRequest::SerializePayload() const {
  ...
  if (m_spotInstanceRequestIdsHasBeenSet) {
    ...
    for (auto& item : m_spotInstanceRequestIds) {
      ss << "SpotInstanceRequestId." << spotInstanceRequestIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void CancelSpotInstanceRequestsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
