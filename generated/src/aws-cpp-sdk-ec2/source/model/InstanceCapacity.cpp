...
InstanceCapacity::InstanceCapacity(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceCapacity& InstanceCapacity::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!availableCapacityNode.IsNull()) {
      m_availableCapacity = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(availableCapacityNode.GetText()).c_str()).c_str());
    ...
    if (!totalCapacityNode.IsNull()) {
      m_totalCapacity = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalCapacityNode.GetText()).c_str()).c_str());
