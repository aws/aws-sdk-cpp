...
ResponseError::ResponseError(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResponseError& ResponseError::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!codeNode.IsNull()) {
      m_code = LaunchTemplateErrorCodeMapper::GetLaunchTemplateErrorCodeForName(
...
void ResponseError::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_codeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Code=" << StringUtils::URLEncode(LaunchTemplateErrorCodeMapper::GetNameForLaunchTemplateErrorCode(m_code)) << "&";
  }
