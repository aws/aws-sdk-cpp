...
TransitGateway::TransitGateway(const XmlNode& xmlNode) { *this = xmlNode; }
...
TransitGateway& TransitGateway::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = TransitGatewayStateMapper::GetTransitGatewayStateForName(
...
void TransitGateway::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(TransitGatewayStateMapper::GetNameForTransitGatewayState(m_state)) << "&";
  }
