...
EbsBlockDeviceResponse::EbsBlockDeviceResponse(const XmlNode& xmlNode) { *this = xmlNode; }

EbsBlockDeviceResponse& EbsBlockDeviceResponse::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!volumeSizeNode.IsNull()) {
      m_volumeSize =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(volumeSizeNode.GetText()).c_str()).c_str());
...
void EbsBlockDeviceResponse::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
...
void EbsBlockDeviceResponse::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_encryptedHasBeenSet) {
