...
Aws::String DescribeEgressOnlyInternetGatewaysRequest::SerializePayload() const {
  ...
  if (m_egressOnlyInternetGatewayIdsHasBeenSet) {
    ...
    for (auto& item : m_egressOnlyInternetGatewayIds) {
      ss << "EgressOnlyInternetGatewayId." << egressOnlyInternetGatewayIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeEgressOnlyInternetGatewaysRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
