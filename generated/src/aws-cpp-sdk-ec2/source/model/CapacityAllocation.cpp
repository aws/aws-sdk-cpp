...
CapacityAllocation::CapacityAllocation(const XmlNode& xmlNode) { *this = xmlNode; }
...
CapacityAllocation& CapacityAllocation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!allocationTypeNode.IsNull()) {
      m_allocationType = AllocationTypeMapper::GetAllocationTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(allocationTypeNode.GetText()).c_str()));
...
void CapacityAllocation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_allocationTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".AllocationType=" << StringUtils::URLEncode(AllocationTypeMapper::GetNameForAllocationType(m_allocationType)) << "&";
  }
