...
IpamPoolCidr::IpamPoolCidr(const XmlNode& xmlNode) { *this = xmlNode; }
...
IpamPoolCidr& IpamPoolCidr::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = IpamPoolCidrStateMapper::GetIpamPoolCidrStateForName(
    ...
    if (!netmaskLengthNode.IsNull()) {
      m_netmaskLength = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(netmaskLengthNode.GetText()).c_str()).c_str());
...
void IpamPoolCidr::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(IpamPoolCidrStateMapper::GetNameForIpamPoolCidrState(m_state)) << "&";
  }
