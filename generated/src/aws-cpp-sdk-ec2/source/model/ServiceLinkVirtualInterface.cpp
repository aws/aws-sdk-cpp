...
ServiceLinkVirtualInterface::ServiceLinkVirtualInterface(const XmlNode& xmlNode) { *this = xmlNode; }
...
ServiceLinkVirtualInterface& ServiceLinkVirtualInterface::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!peerBgpAsnNode.IsNull()) {
      m_peerBgpAsn =
          StringUtils::ConvertToInt64(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(peerBgpAsnNode.GetText()).c_str()).c_str());
    ...
    if (!configurationStateNode.IsNull()) {
      m_configurationState = ServiceLinkVirtualInterfaceConfigurationStateMapper::GetServiceLinkVirtualInterfaceConfigurationStateForName(
...
void ServiceLinkVirtualInterface::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                 const char* locationValue) const {
  if (m_serviceLinkVirtualInterfaceIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".ServiceLinkVirtualInterfaceId=" << StringUtils::URLEncode(m_serviceLinkVirtualInterfaceId.c_str()) << "&";
  }

  if (m_serviceLinkVirtualInterfaceArnHasBeenSet) {
    oStream << location << index << locationValue
            << ".ServiceLinkVirtualInterfaceArn=" << StringUtils::URLEncode(m_serviceLinkVirtualInterfaceArn.c_str()) << "&";
  }
  ...
  if (m_configurationStateHasBeenSet) {
    oStream << location << index << locationValue << ".ConfigurationState="
            << StringUtils::URLEncode(
                   ServiceLinkVirtualInterfaceConfigurationStateMapper::GetNameForServiceLinkVirtualInterfaceConfigurationState(
...
void ServiceLinkVirtualInterface::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_configurationStateHasBeenSet) {
    oStream << location << ".ConfigurationState="
            << StringUtils::URLEncode(
                   ServiceLinkVirtualInterfaceConfigurationStateMapper::GetNameForServiceLinkVirtualInterfaceConfigurationState(
                       m_configurationState))
            << "&";
