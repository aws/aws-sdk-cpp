...
IpamRouteProtectionFinding::IpamRouteProtectionFinding(const XmlNode& xmlNode) { *this = xmlNode; }

IpamRouteProtectionFinding& IpamRouteProtectionFinding::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    XmlNode rpkiStatusNode = resultNode.FirstChild("rpkiStatus");
    if (!rpkiStatusNode.IsNull()) {
      m_rpkiStatus = IpamRpkiStatusMapper::GetIpamRpkiStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(rpkiStatusNode.GetText()).c_str()));
      m_rpkiStatusHasBeenSet = true;
    }
    XmlNode rpkiStrengthNode = resultNode.FirstChild("rpkiStrength");
    if (!rpkiStrengthNode.IsNull()) {
      m_rpkiStrength = IpamRpkiStrengthMapper::GetIpamRpkiStrengthForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(rpkiStrengthNode.GetText()).c_str()));
      m_rpkiStrengthHasBeenSet = true;
    }
    XmlNode roasNode = resultNode.FirstChild("roaSet");
    if (!roasNode.IsNull()) {
      XmlNode roasMember = roasNode.FirstChild("item");
      m_roasHasBeenSet = !roasMember.IsNull();
      while (!roasMember.IsNull()) {
        m_roas.push_back(roasMember);
        roasMember = roasMember.NextNode("item");
      }

      m_roasHasBeenSet = true;
    }
    XmlNode routeOverlapsNode = resultNode.FirstChild("routeOverlapSet");
    if (!routeOverlapsNode.IsNull()) {
      XmlNode routeOverlapsMember = routeOverlapsNode.FirstChild("item");
      m_routeOverlapsHasBeenSet = !routeOverlapsMember.IsNull();
      while (!routeOverlapsMember.IsNull()) {
        m_routeOverlaps.push_back(routeOverlapsMember);
        routeOverlapsMember = routeOverlapsMember.NextNode("item");
      }
    ...
    XmlNode roaSampleTimeNode = resultNode.FirstChild("roaSampleTime");
    if (!roaSampleTimeNode.IsNull()) {
      m_roaSampleTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(roaSampleTimeNode.GetText()).c_str()).c_str(),
                                 Aws::Utils::DateFormat::ISO_8601);
...
void IpamRouteProtectionFinding::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_rpkiStatusHasBeenSet) {
    oStream << location << index << locationValue
            << ".RpkiStatus=" << StringUtils::URLEncode(IpamRpkiStatusMapper::GetNameForIpamRpkiStatus(m_rpkiStatus)) << "&";
  }

  if (m_rpkiStrengthHasBeenSet) {
    oStream << location << index << locationValue
            << ".RpkiStrength=" << StringUtils::URLEncode(IpamRpkiStrengthMapper::GetNameForIpamRpkiStrength(m_rpkiStrength)) << "&";
  }

  if (m_roasHasBeenSet) {
    unsigned roasIdx = 1;
    for (auto& item : m_roas) {
      Aws::StringStream roasSs;
      roasSs << location << index << locationValue << ".RoaSet." << roasIdx++;
      item.OutputToStream(oStream, roasSs.str().c_str());
    }
  }

  if (m_routeOverlapsHasBeenSet) {
    unsigned routeOverlapsIdx = 1;
    for (auto& item : m_routeOverlaps) {
      Aws::StringStream routeOverlapsSs;
      routeOverlapsSs << location << index << locationValue << ".RouteOverlapSet." << routeOverlapsIdx++;
      item.OutputToStream(oStream, routeOverlapsSs.str().c_str());
    }
  }
  ...
  if (m_roaSampleTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".RoaSampleTime=" << StringUtils::URLEncode(m_roaSampleTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}

void IpamRouteProtectionFinding::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_resourceOwnerIdHasBeenSet) {
  ...
  if (m_rpkiStatusHasBeenSet) {
    oStream << location << ".RpkiStatus=" << StringUtils::URLEncode(IpamRpkiStatusMapper::GetNameForIpamRpkiStatus(m_rpkiStatus)) << "&";
  }
  if (m_rpkiStrengthHasBeenSet) {
    oStream << location << ".RpkiStrength=" << StringUtils::URLEncode(IpamRpkiStrengthMapper::GetNameForIpamRpkiStrength(m_rpkiStrength))
            << "&";
  }
  if (m_roasHasBeenSet) {
    unsigned roasIdx = 1;
    for (auto& item : m_roas) {
      Aws::StringStream roasSs;
      roasSs << location << ".RoaSet." << roasIdx++;
      item.OutputToStream(oStream, roasSs.str().c_str());
    }
  }
  if (m_routeOverlapsHasBeenSet) {
    ...
    for (auto& item : m_routeOverlaps) {
      Aws::StringStream routeOverlapsSs;
      routeOverlapsSs << location << ".RouteOverlapSet." << routeOverlapsIdx++;
      item.OutputToStream(oStream, routeOverlapsSs.str().c_str());
    }
  }
  if (m_sampleTimeHasBeenSet) {
