...
Aws::String DescribeRouteServerPeersRequest::SerializePayload() const {
  ...
  if (m_routeServerPeerIdsHasBeenSet) {
    ...
    for (auto& item : m_routeServerPeerIds) {
      ss << "RouteServerPeerId." << routeServerPeerIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeRouteServerPeersRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
