...
Aws::String DescribeReplaceRootVolumeTasksRequest::SerializePayload() const {
  ...
  if (m_replaceRootVolumeTaskIdsHasBeenSet) {
    ...
    for (auto& item : m_replaceRootVolumeTaskIds) {
      ss << "ReplaceRootVolumeTaskId." << replaceRootVolumeTaskIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeReplaceRootVolumeTasksRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
