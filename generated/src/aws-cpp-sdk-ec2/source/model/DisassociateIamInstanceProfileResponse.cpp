...
DisassociateIamInstanceProfileResponse& DisassociateIamInstanceProfileResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "DisassociateIamInstanceProfileResponse")) {
    resultNode = rootNode.FirstChild("DisassociateIamInstanceProfileResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::DisassociateIamInstanceProfileResponse",
