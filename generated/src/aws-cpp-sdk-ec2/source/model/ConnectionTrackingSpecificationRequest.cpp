...
ConnectionTrackingSpecificationRequest::ConnectionTrackingSpecificationRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
ConnectionTrackingSpecificationRequest& ConnectionTrackingSpecificationRequest::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!tcpEstablishedTimeoutNode.IsNull()) {
      m_tcpEstablishedTimeout = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(tcpEstablishedTimeoutNode.GetText()).c_str()).c_str());
    ...
    if (!udpStreamTimeoutNode.IsNull()) {
      m_udpStreamTimeout = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(udpStreamTimeoutNode.GetText()).c_str()).c_str());
      m_udpStreamTimeoutHasBeenSet = true;
    }
    XmlNode udpTimeoutNode = resultNode.FirstChild("UdpTimeout");
    if (!udpTimeoutNode.IsNull()) {
      m_udpTimeout =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(udpTimeoutNode.GetText()).c_str()).c_str());
...
void ConnectionTrackingSpecificationRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
