...
Aws::String DescribeImportSnapshotTasksRequest::SerializePayload() const {
  ...
  if (m_importTaskIdsHasBeenSet) {
    ...
    for (auto& item : m_importTaskIds) {
      ss << "ImportTaskId." << importTaskIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      importTaskIdsCount++;
    }
  }
...
void DescribeImportSnapshotTasksRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
