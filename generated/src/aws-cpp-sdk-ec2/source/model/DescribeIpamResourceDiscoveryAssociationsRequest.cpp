...
Aws::String DescribeIpamResourceDiscoveryAssociationsRequest::SerializePayload() const {
  ...
  if (m_ipamResourceDiscoveryAssociationIdsHasBeenSet) {
    ...
    for (auto& item : m_ipamResourceDiscoveryAssociationIds) {
      ss << "IpamResourceDiscoveryAssociationId." << ipamResourceDiscoveryAssociationIdsCount << "=" << StringUtils::URLEncode(item.c_str())
         << "&";
...
void DescribeIpamResourceDiscoveryAssociationsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
