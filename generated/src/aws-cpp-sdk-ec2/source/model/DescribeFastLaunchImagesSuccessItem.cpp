...
DescribeFastLaunchImagesSuccessItem::DescribeFastLaunchImagesSuccessItem(const XmlNode& xmlNode) { *this = xmlNode; }
...
DescribeFastLaunchImagesSuccessItem& DescribeFastLaunchImagesSuccessItem::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!resourceTypeNode.IsNull()) {
      m_resourceType = FastLaunchResourceTypeMapper::GetFastLaunchResourceTypeForName(
    ...
    if (!maxParallelLaunchesNode.IsNull()) {
      m_maxParallelLaunches = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maxParallelLaunchesNode.GetText()).c_str()).c_str());
    ...
    if (!stateNode.IsNull()) {
      m_state = FastLaunchStateCodeMapper::GetFastLaunchStateCodeForName(
    ...
    if (!stateTransitionTimeNode.IsNull()) {
      m_stateTransitionTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(stateTransitionTimeNode.GetText()).c_str()).c_str(),
...
void DescribeFastLaunchImagesSuccessItem::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_resourceTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".ResourceType=" << StringUtils::URLEncode(FastLaunchResourceTypeMapper::GetNameForFastLaunchResourceType(m_resourceType))
            << "&";
  }
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(FastLaunchStateCodeMapper::GetNameForFastLaunchStateCode(m_state)) << "&";
  }
  ...
  if (m_stateTransitionTimeHasBeenSet) {
    oStream << location << index << locationValue << ".StateTransitionTime="
            << StringUtils::URLEncode(m_stateTransitionTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
...
void DescribeFastLaunchImagesSuccessItem::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_stateTransitionTimeHasBeenSet) {
    oStream << location << ".StateTransitionTime="
            << StringUtils::URLEncode(m_stateTransitionTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
