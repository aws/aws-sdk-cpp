...
Aws::String DescribeOutpostLagsRequest::SerializePayload() const {
  ...
  if (m_outpostLagIdsHasBeenSet) {
    ...
    for (auto& item : m_outpostLagIds) {
      ss << "OutpostLagId." << outpostLagIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      outpostLagIdsCount++;
    }
  }
...
void DescribeOutpostLagsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
