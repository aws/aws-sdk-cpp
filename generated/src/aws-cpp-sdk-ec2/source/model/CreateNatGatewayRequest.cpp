...
Aws::String CreateNatGatewayRequest::SerializePayload() const {
  ...
  if (m_secondaryAllocationIdsHasBeenSet) {
    ...
    for (auto& item : m_secondaryAllocationIds) {
      ss << "SecondaryAllocationId." << secondaryAllocationIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_secondaryPrivateIpAddressesHasBeenSet) {
    ...
    for (auto& item : m_secondaryPrivateIpAddresses) {
      ss << "SecondaryPrivateIpAddress." << secondaryPrivateIpAddressesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void CreateNatGatewayRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
