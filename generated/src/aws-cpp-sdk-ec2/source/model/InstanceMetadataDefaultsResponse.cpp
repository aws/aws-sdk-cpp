...
InstanceMetadataDefaultsResponse::InstanceMetadataDefaultsResponse(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceMetadataDefaultsResponse& InstanceMetadataDefaultsResponse::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!httpTokensNode.IsNull()) {
      m_httpTokens = HttpTokensStateMapper::GetHttpTokensStateForName(
    ...
    if (!httpEndpointNode.IsNull()) {
      m_httpEndpoint = InstanceMetadataEndpointStateMapper::GetInstanceMetadataEndpointStateForName(
    ...
    if (!instanceMetadataTagsNode.IsNull()) {
      m_instanceMetadataTags = InstanceMetadataTagsStateMapper::GetInstanceMetadataTagsStateForName(
...
void InstanceMetadataDefaultsResponse::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                      const char* locationValue) const {
  if (m_httpTokensHasBeenSet) {
    oStream << location << index << locationValue
            << ".HttpTokens=" << StringUtils::URLEncode(HttpTokensStateMapper::GetNameForHttpTokensState(m_httpTokens)) << "&";
  }
  ...
  if (m_httpEndpointHasBeenSet) {
    oStream << location << index << locationValue << ".HttpEndpoint="
            << StringUtils::URLEncode(InstanceMetadataEndpointStateMapper::GetNameForInstanceMetadataEndpointState(m_httpEndpoint)) << "&";
  }

  if (m_instanceMetadataTagsHasBeenSet) {
    oStream << location << index << locationValue << ".InstanceMetadataTags="
            << StringUtils::URLEncode(InstanceMetadataTagsStateMapper::GetNameForInstanceMetadataTagsState(m_instanceMetadataTags)) << "&";
  }
