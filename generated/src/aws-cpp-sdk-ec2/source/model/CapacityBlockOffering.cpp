...
CapacityBlockOffering::CapacityBlockOffering(const XmlNode& xmlNode) { *this = xmlNode; }
...
CapacityBlockOffering& CapacityBlockOffering::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!capacityBlockDurationHoursNode.IsNull()) {
      m_capacityBlockDurationHours = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(capacityBlockDurationHoursNode.GetText()).c_str()).c_str());
    ...
    if (!ultraserverCountNode.IsNull()) {
      m_ultraserverCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(ultraserverCountNode.GetText()).c_str()).c_str());
    ...
    if (!capacityBlockDurationMinutesNode.IsNull()) {
      m_capacityBlockDurationMinutes = StringUtils::ConvertToInt32(
...
void CapacityBlockOffering::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_capacityBlockOfferingIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".CapacityBlockOfferingId=" << StringUtils::URLEncode(m_capacityBlockOfferingId.c_str()) << "&";
  }
