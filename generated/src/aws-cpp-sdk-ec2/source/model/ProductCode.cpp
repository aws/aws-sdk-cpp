...
ProductCode::ProductCode(const XmlNode& xmlNode) { *this = xmlNode; }
...
ProductCode& ProductCode::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!productCodeTypeNode.IsNull()) {
      m_productCodeType = ProductCodeValuesMapper::GetProductCodeValuesForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(productCodeTypeNode.GetText()).c_str()));
...
void ProductCode::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_productCodeTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".ProductCodeType=" << StringUtils::URLEncode(ProductCodeValuesMapper::GetNameForProductCodeValues(m_productCodeType))
            << "&";
  }
}
