...
InstanceCount::InstanceCount(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceCount& InstanceCount::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state =
          ListingStateMapper::GetListingStateForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(stateNode.GetText()).c_str()));
...
void InstanceCount::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(ListingStateMapper::GetNameForListingState(m_state)) << "&";
  }
}
