...
DeprecationTimeConditionRequest::DeprecationTimeConditionRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
DeprecationTimeConditionRequest& DeprecationTimeConditionRequest::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!maximumDaysSinceDeprecatedNode.IsNull()) {
      m_maximumDaysSinceDeprecated = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maximumDaysSinceDeprecatedNode.GetText()).c_str()).c_str());
...
void DeprecationTimeConditionRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
