...
LaunchTemplatePrivateDnsNameOptions::LaunchTemplatePrivateDnsNameOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
LaunchTemplatePrivateDnsNameOptions& LaunchTemplatePrivateDnsNameOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!hostnameTypeNode.IsNull()) {
      m_hostnameType = HostnameTypeMapper::GetHostnameTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(hostnameTypeNode.GetText()).c_str()));
    ...
    if (!enableResourceNameDnsARecordNode.IsNull()) {
      m_enableResourceNameDnsARecord = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(enableResourceNameDnsARecordNode.GetText()).c_str()).c_str());
    ...
    if (!enableResourceNameDnsAAAARecordNode.IsNull()) {
      m_enableResourceNameDnsAAAARecord = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(enableResourceNameDnsAAAARecordNode.GetText()).c_str()).c_str());
...
void LaunchTemplatePrivateDnsNameOptions::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                         const char* locationValue) const {
  if (m_hostnameTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".HostnameType=" << StringUtils::URLEncode(HostnameTypeMapper::GetNameForHostnameType(m_hostnameType)) << "&";
  }
  ...
  if (m_enableResourceNameDnsAAAARecordHasBeenSet) {
    oStream << location << index << locationValue << ".EnableResourceNameDnsAAAARecord=" << std::boolalpha
