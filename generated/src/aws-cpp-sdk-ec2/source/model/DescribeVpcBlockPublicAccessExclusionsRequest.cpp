...
Aws::String DescribeVpcBlockPublicAccessExclusionsRequest::SerializePayload() const {
  ...
  if (m_exclusionIdsHasBeenSet) {
    ...
    for (auto& item : m_exclusionIds) {
      ss << "ExclusionId." << exclusionIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      exclusionIdsCount++;
    }
  }
...
void DescribeVpcBlockPublicAccessExclusionsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
