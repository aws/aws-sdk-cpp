...
VerifiedAccessInstanceUserTrustProviderClientConfiguration& VerifiedAccessInstanceUserTrustProviderClientConfiguration::operator=(
    const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = UserTrustProviderTypeMapper::GetUserTrustProviderTypeForName(
    ...
    if (!pkceEnabledNode.IsNull()) {
      m_pkceEnabled =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(pkceEnabledNode.GetText()).c_str()).c_str());
...
void VerifiedAccessInstanceUserTrustProviderClientConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                                                const char* locationValue) const {
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(UserTrustProviderTypeMapper::GetNameForUserTrustProviderType(m_type)) << "&";
  }
  ...
  if (m_publicSigningKeyEndpointHasBeenSet) {
    oStream << location << index << locationValue
            << ".PublicSigningKeyEndpoint=" << StringUtils::URLEncode(m_publicSigningKeyEndpoint.c_str()) << "&";
  }
