...
Aws::String DescribeLocalGatewaysRequest::SerializePayload() const {
  ...
  if (m_localGatewayIdsHasBeenSet) {
    ...
    for (auto& item : m_localGatewayIds) {
      ss << "LocalGatewayId." << localGatewayIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeLocalGatewaysRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
