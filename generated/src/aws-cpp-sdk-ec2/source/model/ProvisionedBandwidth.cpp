...
ProvisionedBandwidth::ProvisionedBandwidth(const XmlNode& xmlNode) { *this = xmlNode; }
...
ProvisionedBandwidth& ProvisionedBandwidth::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!provisionTimeNode.IsNull()) {
      m_provisionTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(provisionTimeNode.GetText()).c_str()).c_str(),
...
void ProvisionedBandwidth::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_provisionTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".ProvisionTime=" << StringUtils::URLEncode(m_provisionTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
