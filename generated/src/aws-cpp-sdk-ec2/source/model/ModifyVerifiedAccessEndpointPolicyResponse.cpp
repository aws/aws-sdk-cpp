...
ModifyVerifiedAccessEndpointPolicyResponse& ModifyVerifiedAccessEndpointPolicyResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!policyEnabledNode.IsNull()) {
      m_policyEnabled =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(policyEnabledNode.GetText()).c_str()).c_str());
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::ModifyVerifiedAccessEndpointPolicyResponse",
