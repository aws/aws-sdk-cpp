...
Aws::String CreateClientVpnEndpointRequest::SerializePayload() const {
  ...
  if (m_dnsServersHasBeenSet) {
    ...
    for (auto& item : m_dnsServers) {
      ss << "DnsServers." << dnsServersCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      dnsServersCount++;
    }
  }
...
void CreateClientVpnEndpointRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
