...
AuthorizationRule::AuthorizationRule(const XmlNode& xmlNode) { *this = xmlNode; }
...
AuthorizationRule& AuthorizationRule::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!accessAllNode.IsNull()) {
      m_accessAll =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(accessAllNode.GetText()).c_str()).c_str());
