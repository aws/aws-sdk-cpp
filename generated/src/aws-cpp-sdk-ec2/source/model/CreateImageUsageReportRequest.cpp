...
Aws::String CreateImageUsageReportRequest::SerializePayload() const {
  ...
  if (m_accountIdsHasBeenSet) {
    ...
    for (auto& item : m_accountIds) {
      ss << "AccountId." << accountIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      accountIdsCount++;
    }
  }
...
void CreateImageUsageReportRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
