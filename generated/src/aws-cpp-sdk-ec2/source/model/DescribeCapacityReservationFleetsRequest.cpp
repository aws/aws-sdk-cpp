...
Aws::String DescribeCapacityReservationFleetsRequest::SerializePayload() const {
  ...
  if (m_capacityReservationFleetIdsHasBeenSet) {
    ...
    for (auto& item : m_capacityReservationFleetIds) {
      ss << "CapacityReservationFleetId." << capacityReservationFleetIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeCapacityReservationFleetsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
