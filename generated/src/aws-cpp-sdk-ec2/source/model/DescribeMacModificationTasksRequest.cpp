...
Aws::String DescribeMacModificationTasksRequest::SerializePayload() const {
  ...
  if (m_macModificationTaskIdsHasBeenSet) {
    ...
    for (auto& item : m_macModificationTaskIds) {
      ss << "MacModificationTaskId." << macModificationTaskIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeMacModificationTasksRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
