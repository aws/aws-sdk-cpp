...
DescribeIpamPoolAllocationsResponse::DescribeIpamPoolAllocationsResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  *this = result;
}

DescribeIpamPoolAllocationsResponse& DescribeIpamPoolAllocationsResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  m_HttpResponseCode = result.GetResponseCode();
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "DescribeIpamPoolAllocationsResponse")) {
    resultNode = rootNode.FirstChild("DescribeIpamPoolAllocationsResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::DescribeIpamPoolAllocationsResponse", "x-amzn-request-id: " << m_responseMetadata.GetRequestId());
  }
  return *this;
}
