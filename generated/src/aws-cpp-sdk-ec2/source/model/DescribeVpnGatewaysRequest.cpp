...
Aws::String DescribeVpnGatewaysRequest::SerializePayload() const {
  ...
  if (m_vpnGatewayIdsHasBeenSet) {
    ...
    for (auto& item : m_vpnGatewayIds) {
      ss << "VpnGatewayId." << vpnGatewayIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      vpnGatewayIdsCount++;
    }
  }
...
void DescribeVpnGatewaysRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
