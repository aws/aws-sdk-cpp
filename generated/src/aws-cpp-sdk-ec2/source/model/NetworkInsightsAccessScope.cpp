...
NetworkInsightsAccessScope::NetworkInsightsAccessScope(const XmlNode& xmlNode) { *this = xmlNode; }
...
NetworkInsightsAccessScope& NetworkInsightsAccessScope::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!updatedDateNode.IsNull()) {
      m_updatedDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(updatedDateNode.GetText()).c_str()).c_str(),
                               Aws::Utils::DateFormat::ISO_8601);
...
void NetworkInsightsAccessScope::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_networkInsightsAccessScopeArnHasBeenSet) {
    oStream << location << index << locationValue
            << ".NetworkInsightsAccessScopeArn=" << StringUtils::URLEncode(m_networkInsightsAccessScopeArn.c_str()) << "&";
  }
  ...
  if (m_updatedDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".UpdatedDate=" << StringUtils::URLEncode(m_updatedDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
