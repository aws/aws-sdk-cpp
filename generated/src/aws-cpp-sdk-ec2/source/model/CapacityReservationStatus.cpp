...
CapacityReservationStatus::CapacityReservationStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
CapacityReservationStatus& CapacityReservationStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!totalAvailableCapacityNode.IsNull()) {
      m_totalAvailableCapacity = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalAvailableCapacityNode.GetText()).c_str()).c_str());
    ...
    if (!totalUnavailableCapacityNode.IsNull()) {
      m_totalUnavailableCapacity = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalUnavailableCapacityNode.GetText()).c_str()).c_str());
...
void CapacityReservationStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
