...
HostProperties::HostProperties(const XmlNode& xmlNode) { *this = xmlNode; }
...
HostProperties& HostProperties::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!socketsNode.IsNull()) {
      m_sockets =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(socketsNode.GetText()).c_str()).c_str());
      m_socketsHasBeenSet = true;
    }
    XmlNode totalVCpusNode = resultNode.FirstChild("totalVCpus");
    if (!totalVCpusNode.IsNull()) {
      m_totalVCpus =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalVCpusNode.GetText()).c_str()).c_str());
