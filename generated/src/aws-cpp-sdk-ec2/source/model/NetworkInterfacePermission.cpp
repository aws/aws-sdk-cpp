...
NetworkInterfacePermission::NetworkInterfacePermission(const XmlNode& xmlNode) { *this = xmlNode; }
...
NetworkInterfacePermission& NetworkInterfacePermission::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!permissionNode.IsNull()) {
      m_permission = InterfacePermissionTypeMapper::GetInterfacePermissionTypeForName(
...
void NetworkInterfacePermission::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                const char* locationValue) const {
  if (m_networkInterfacePermissionIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".NetworkInterfacePermissionId=" << StringUtils::URLEncode(m_networkInterfacePermissionId.c_str()) << "&";
  }
  ...
  if (m_permissionHasBeenSet) {
    oStream << location << index << locationValue
            << ".Permission=" << StringUtils::URLEncode(InterfacePermissionTypeMapper::GetNameForInterfacePermissionType(m_permission))
            << "&";
  }
