...
Vpc& Vpc::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!instanceTenancyNode.IsNull()) {
      m_instanceTenancy =
          TenancyMapper::GetTenancyForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(instanceTenancyNode.GetText()).c_str()));
...
void Vpc::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_instanceTenancyHasBeenSet) {
    oStream << location << index << locationValue
            << ".InstanceTenancy=" << StringUtils::URLEncode(TenancyMapper::GetNameForTenancy(m_instanceTenancy)) << "&";
  }
  ...
  if (m_ipv6CidrBlockAssociationSetHasBeenSet) {
    ...
    for (auto& item : m_ipv6CidrBlockAssociationSet) {
      ...
      ipv6CidrBlockAssociationSetSs << location << index << locationValue << ".Ipv6CidrBlockAssociationSet."
                                    << ipv6CidrBlockAssociationSetIdx++;
