...
SecurityGroupRule::SecurityGroupRule(const XmlNode& xmlNode) { *this = xmlNode; }
...
SecurityGroupRule& SecurityGroupRule::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!isEgressNode.IsNull()) {
      m_isEgress =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isEgressNode.GetText()).c_str()).c_str());
