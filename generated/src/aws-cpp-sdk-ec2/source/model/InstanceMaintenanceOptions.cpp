...
InstanceMaintenanceOptions::InstanceMaintenanceOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
void InstanceMaintenanceOptions::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                const char* locationValue) const {
  if (m_autoRecoveryHasBeenSet) {
    oStream << location << index << locationValue << ".AutoRecovery="
            << StringUtils::URLEncode(InstanceAutoRecoveryStateMapper::GetNameForInstanceAutoRecoveryState(m_autoRecovery)) << "&";
  }

  if (m_rebootMigrationHasBeenSet) {
    oStream << location << index << locationValue << ".RebootMigration="
...
void InstanceMaintenanceOptions::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_rebootMigrationHasBeenSet) {
    oStream << location << ".RebootMigration="
            << StringUtils::URLEncode(InstanceRebootMigrationStateMapper::GetNameForInstanceRebootMigrationState(m_rebootMigration)) << "&";
  }
}
