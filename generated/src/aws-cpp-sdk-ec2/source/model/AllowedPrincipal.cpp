...
AllowedPrincipal::AllowedPrincipal(const XmlNode& xmlNode) { *this = xmlNode; }
...
AllowedPrincipal& AllowedPrincipal::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!principalTypeNode.IsNull()) {
      m_principalType = PrincipalTypeMapper::GetPrincipalTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(principalTypeNode.GetText()).c_str()));
...
void AllowedPrincipal::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_principalTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".PrincipalType=" << StringUtils::URLEncode(PrincipalTypeMapper::GetNameForPrincipalType(m_principalType)) << "&";
  }
