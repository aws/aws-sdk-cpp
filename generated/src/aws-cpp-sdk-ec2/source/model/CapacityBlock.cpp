...
CapacityBlock::CapacityBlock(const XmlNode& xmlNode) { *this = xmlNode; }
...
CapacityBlock& CapacityBlock::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = CapacityBlockResourceStateMapper::GetCapacityBlockResourceStateForName(
...
void CapacityBlock::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_capacityReservationIdsHasBeenSet) {
    ...
    for (auto& item : m_capacityReservationIds) {
      oStream << location << index << locationValue << ".CapacityReservationIdSet." << capacityReservationIdsIdx++ << "="
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(CapacityBlockResourceStateMapper::GetNameForCapacityBlockResourceState(m_state)) << "&";
  }
