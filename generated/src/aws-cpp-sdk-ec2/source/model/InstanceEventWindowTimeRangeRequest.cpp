...
InstanceEventWindowTimeRangeRequest::InstanceEventWindowTimeRangeRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceEventWindowTimeRangeRequest& InstanceEventWindowTimeRangeRequest::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!startWeekDayNode.IsNull()) {
      m_startWeekDay =
          WeekDayMapper::GetWeekDayForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(startWeekDayNode.GetText()).c_str()));
      m_startWeekDayHasBeenSet = true;
    }
    XmlNode startHourNode = resultNode.FirstChild("StartHour");
    if (!startHourNode.IsNull()) {
      m_startHour =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(startHourNode.GetText()).c_str()).c_str());
      m_startHourHasBeenSet = true;
    }
    XmlNode endWeekDayNode = resultNode.FirstChild("EndWeekDay");
    if (!endWeekDayNode.IsNull()) {
      m_endWeekDay =
          WeekDayMapper::GetWeekDayForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(endWeekDayNode.GetText()).c_str()));
      m_endWeekDayHasBeenSet = true;
    }
    XmlNode endHourNode = resultNode.FirstChild("EndHour");
    if (!endHourNode.IsNull()) {
      m_endHour =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(endHourNode.GetText()).c_str()).c_str());
...
void InstanceEventWindowTimeRangeRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                         const char* locationValue) const {
  if (m_startWeekDayHasBeenSet) {
    oStream << location << index << locationValue
            << ".StartWeekDay=" << StringUtils::URLEncode(WeekDayMapper::GetNameForWeekDay(m_startWeekDay)) << "&";
  }
  ...
  if (m_endWeekDayHasBeenSet) {
    oStream << location << index << locationValue
            << ".EndWeekDay=" << StringUtils::URLEncode(WeekDayMapper::GetNameForWeekDay(m_endWeekDay)) << "&";
  }
