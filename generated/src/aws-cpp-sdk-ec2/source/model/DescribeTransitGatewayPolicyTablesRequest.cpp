...
Aws::String DescribeTransitGatewayPolicyTablesRequest::SerializePayload() const {
  ...
  if (m_transitGatewayPolicyTableIdsHasBeenSet) {
    ...
    for (auto& item : m_transitGatewayPolicyTableIds) {
      ss << "TransitGatewayPolicyTableIds." << transitGatewayPolicyTableIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeTransitGatewayPolicyTablesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
