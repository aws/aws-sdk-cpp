...
EnableApplicationStatusCheckSuppressionResponse::EnableApplicationStatusCheckSuppressionResponse(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  *this = result;
}

EnableApplicationStatusCheckSuppressionResponse& EnableApplicationStatusCheckSuppressionResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  m_HttpResponseCode = result.GetResponseCode();
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "EnableApplicationStatusCheckSuppressionResponse")) {
    resultNode = rootNode.FirstChild("EnableApplicationStatusCheckSuppressionResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::EnableApplicationStatusCheckSuppressionResponse",
