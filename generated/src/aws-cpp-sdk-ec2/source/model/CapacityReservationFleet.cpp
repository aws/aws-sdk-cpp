...
CapacityReservationFleet::CapacityReservationFleet(const XmlNode& xmlNode) { *this = xmlNode; }
...
CapacityReservationFleet& CapacityReservationFleet::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = CapacityReservationFleetStateMapper::GetCapacityReservationFleetStateForName(
    ...
    if (!totalFulfilledCapacityNode.IsNull()) {
      m_totalFulfilledCapacity = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalFulfilledCapacityNode.GetText()).c_str()).c_str());
      m_totalFulfilledCapacityHasBeenSet = true;
    }
    XmlNode tenancyNode = resultNode.FirstChild("tenancy");
    if (!tenancyNode.IsNull()) {
      m_tenancy = FleetCapacityReservationTenancyMapper::GetFleetCapacityReservationTenancyForName(
    ...
    if (!instanceMatchCriteriaNode.IsNull()) {
      m_instanceMatchCriteria = FleetInstanceMatchCriteriaMapper::GetFleetInstanceMatchCriteriaForName(
...
void CapacityReservationFleet::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_capacityReservationFleetArnHasBeenSet) {
    oStream << location << index << locationValue
            << ".CapacityReservationFleetArn=" << StringUtils::URLEncode(m_capacityReservationFleetArn.c_str()) << "&";
  }

  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(CapacityReservationFleetStateMapper::GetNameForCapacityReservationFleetState(m_state))
            << "&";
  }
  ...
  if (m_tenancyHasBeenSet) {
    oStream << location << index << locationValue << ".Tenancy="
            << StringUtils::URLEncode(FleetCapacityReservationTenancyMapper::GetNameForFleetCapacityReservationTenancy(m_tenancy)) << "&";
  }
  ...
  if (m_instanceMatchCriteriaHasBeenSet) {
    oStream << location << index << locationValue << ".InstanceMatchCriteria="
            << StringUtils::URLEncode(FleetInstanceMatchCriteriaMapper::GetNameForFleetInstanceMatchCriteria(m_instanceMatchCriteria))
            << "&";
  }
  ...
  if (m_instanceTypeSpecificationsHasBeenSet) {
    ...
    for (auto& item : m_instanceTypeSpecifications) {
      ...
      instanceTypeSpecificationsSs << location << index << locationValue << ".InstanceTypeSpecificationSet."
                                   << instanceTypeSpecificationsIdx++;
