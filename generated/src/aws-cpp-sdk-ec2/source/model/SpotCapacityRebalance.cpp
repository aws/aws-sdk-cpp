...
SpotCapacityRebalance::SpotCapacityRebalance(const XmlNode& xmlNode) { *this = xmlNode; }
...
SpotCapacityRebalance& SpotCapacityRebalance::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!replacementStrategyNode.IsNull()) {
      m_replacementStrategy = ReplacementStrategyMapper::GetReplacementStrategyForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(replacementStrategyNode.GetText()).c_str()));
    ...
    if (!terminationDelayNode.IsNull()) {
      m_terminationDelay = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(terminationDelayNode.GetText()).c_str()).c_str());
...
void SpotCapacityRebalance::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_replacementStrategyHasBeenSet) {
    oStream << location << index << locationValue << ".ReplacementStrategy="
            << StringUtils::URLEncode(ReplacementStrategyMapper::GetNameForReplacementStrategy(m_replacementStrategy)) << "&";
  }
...
void SpotCapacityRebalance::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_replacementStrategyHasBeenSet) {
    oStream << location << ".ReplacementStrategy="
            << StringUtils::URLEncode(ReplacementStrategyMapper::GetNameForReplacementStrategy(m_replacementStrategy)) << "&";
  }
  if (m_terminationDelayHasBeenSet) {
