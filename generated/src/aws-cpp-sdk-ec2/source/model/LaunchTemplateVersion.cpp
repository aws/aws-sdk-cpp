...
LaunchTemplateVersion::LaunchTemplateVersion(const XmlNode& xmlNode) { *this = xmlNode; }
...
LaunchTemplateVersion& LaunchTemplateVersion::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!defaultVersionNode.IsNull()) {
      m_defaultVersion = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(defaultVersionNode.GetText()).c_str()).c_str());
