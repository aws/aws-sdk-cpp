...
ModifyTransitGatewayPrefixListReferenceResponse& ModifyTransitGatewayPrefixListReferenceResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "ModifyTransitGatewayPrefixListReferenceResponse")) {
    resultNode = rootNode.FirstChild("ModifyTransitGatewayPrefixListReferenceResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::ModifyTransitGatewayPrefixListReferenceResponse",
