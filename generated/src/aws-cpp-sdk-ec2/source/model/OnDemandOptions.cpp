...
OnDemandOptions::OnDemandOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
OnDemandOptions& OnDemandOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!allocationStrategyNode.IsNull()) {
      m_allocationStrategy = FleetOnDemandAllocationStrategyMapper::GetFleetOnDemandAllocationStrategyForName(
  ...
  if (m_allocationStrategyHasBeenSet) {
    oStream << location << index << locationValue << ".AllocationStrategy="
            << StringUtils::URLEncode(
                   FleetOnDemandAllocationStrategyMapper::GetNameForFleetOnDemandAllocationStrategy(m_allocationStrategy))
            << "&";
  }
