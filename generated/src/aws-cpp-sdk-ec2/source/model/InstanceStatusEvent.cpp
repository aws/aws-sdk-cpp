...
InstanceStatusEvent::InstanceStatusEvent(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceStatusEvent& InstanceStatusEvent::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!notBeforeNode.IsNull()) {
      m_notBefore = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(notBeforeNode.GetText()).c_str()).c_str(),
                             Aws::Utils::DateFormat::ISO_8601);
    ...
    if (!notBeforeDeadlineNode.IsNull()) {
      m_notBeforeDeadline =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(notBeforeDeadlineNode.GetText()).c_str()).c_str(),
                   Aws::Utils::DateFormat::ISO_8601);
...
void InstanceStatusEvent::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_notBeforeHasBeenSet) {
    oStream << location << index << locationValue
            << ".NotBefore=" << StringUtils::URLEncode(m_notBefore.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }

  if (m_notBeforeDeadlineHasBeenSet) {
    oStream << location << index << locationValue
            << ".NotBeforeDeadline=" << StringUtils::URLEncode(m_notBeforeDeadline.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
}
