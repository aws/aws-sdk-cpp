...
GetInstanceTpmEkPubResponse::GetInstanceTpmEkPubResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
GetInstanceTpmEkPubResponse& GetInstanceTpmEkPubResponse::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!keyTypeNode.IsNull()) {
      m_keyType = EkPubKeyTypeMapper::GetEkPubKeyTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(keyTypeNode.GetText()).c_str()));
      m_keyTypeHasBeenSet = true;
    }
    XmlNode keyFormatNode = resultNode.FirstChild("keyFormat");
    if (!keyFormatNode.IsNull()) {
      m_keyFormat = EkPubKeyFormatMapper::GetEkPubKeyFormatForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(keyFormatNode.GetText()).c_str()));
