...
CapacityReservationBillingRequest::CapacityReservationBillingRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
CapacityReservationBillingRequest& CapacityReservationBillingRequest::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lastUpdateTimeNode.IsNull()) {
      m_lastUpdateTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastUpdateTimeNode.GetText()).c_str()).c_str(),
                                  Aws::Utils::DateFormat::ISO_8601);
      m_lastUpdateTimeHasBeenSet = true;
    }
    XmlNode statusNode = resultNode.FirstChild("status");
    if (!statusNode.IsNull()) {
      m_status = CapacityReservationBillingRequestStatusMapper::GetCapacityReservationBillingRequestStatusForName(
...
void CapacityReservationBillingRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_unusedReservationBillingOwnerIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".UnusedReservationBillingOwnerId=" << StringUtils::URLEncode(m_unusedReservationBillingOwnerId.c_str()) << "&";
  }

  if (m_lastUpdateTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastUpdateTime=" << StringUtils::URLEncode(m_lastUpdateTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }

  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue << ".Status="
            << StringUtils::URLEncode(
                   CapacityReservationBillingRequestStatusMapper::GetNameForCapacityReservationBillingRequestStatus(m_status))
            << "&";
  }
