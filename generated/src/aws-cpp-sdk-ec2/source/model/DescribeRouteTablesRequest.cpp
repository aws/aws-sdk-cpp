...
Aws::String DescribeRouteTablesRequest::SerializePayload() const {
  ...
  if (m_routeTableIdsHasBeenSet) {
    ...
    for (auto& item : m_routeTableIds) {
      ss << "RouteTableId." << routeTableIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      routeTableIdsCount++;
    }
  }
...
void DescribeRouteTablesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
