...
SnapshotDetail::SnapshotDetail(const XmlNode& xmlNode) { *this = xmlNode; }
...
SnapshotDetail& SnapshotDetail::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!diskImageSizeNode.IsNull()) {
      m_diskImageSize = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(diskImageSizeNode.GetText()).c_str()).c_str());
