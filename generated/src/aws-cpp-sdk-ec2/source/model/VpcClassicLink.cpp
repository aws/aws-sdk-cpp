...
VpcClassicLink::VpcClassicLink(const XmlNode& xmlNode) { *this = xmlNode; }
...
VpcClassicLink& VpcClassicLink::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!classicLinkEnabledNode.IsNull()) {
      m_classicLinkEnabled = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(classicLinkEnabledNode.GetText()).c_str()).c_str());
