...
TransitGatewayRoute::TransitGatewayRoute(const XmlNode& xmlNode) { *this = xmlNode; }
...
TransitGatewayRoute& TransitGatewayRoute::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = TransitGatewayRouteTypeMapper::GetTransitGatewayRouteTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(typeNode.GetText()).c_str()));
      m_typeHasBeenSet = true;
    }
    XmlNode stateNode = resultNode.FirstChild("state");
    if (!stateNode.IsNull()) {
      m_state = TransitGatewayRouteStateMapper::GetTransitGatewayRouteStateForName(
...
void TransitGatewayRoute::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(TransitGatewayRouteTypeMapper::GetNameForTransitGatewayRouteType(m_type)) << "&";
  }

  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(TransitGatewayRouteStateMapper::GetNameForTransitGatewayRouteState(m_state)) << "&";
  }
}
