...
InstanceExportDetails::InstanceExportDetails(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceExportDetails& InstanceExportDetails::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!targetEnvironmentNode.IsNull()) {
      m_targetEnvironment = ExportEnvironmentMapper::GetExportEnvironmentForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(targetEnvironmentNode.GetText()).c_str()));
...
void InstanceExportDetails::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_targetEnvironmentHasBeenSet) {
    oStream << location << index << locationValue
            << ".TargetEnvironment=" << StringUtils::URLEncode(ExportEnvironmentMapper::GetNameForExportEnvironment(m_targetEnvironment))
            << "&";
  }
}
