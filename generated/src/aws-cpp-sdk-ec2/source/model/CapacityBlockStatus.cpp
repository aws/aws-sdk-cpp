...
CapacityBlockStatus::CapacityBlockStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
CapacityBlockStatus& CapacityBlockStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!interconnectStatusNode.IsNull()) {
      m_interconnectStatus = CapacityBlockInterconnectStatusMapper::GetCapacityBlockInterconnectStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(interconnectStatusNode.GetText()).c_str()));
...
void CapacityBlockStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_interconnectStatusHasBeenSet) {
    oStream << location << index << locationValue << ".InterconnectStatus="
            << StringUtils::URLEncode(
                   CapacityBlockInterconnectStatusMapper::GetNameForCapacityBlockInterconnectStatus(m_interconnectStatus))
            << "&";
  }
  ...
  if (m_capacityReservationStatusesHasBeenSet) {
    ...
    for (auto& item : m_capacityReservationStatuses) {
      ...
      capacityReservationStatusesSs << location << index << locationValue << ".CapacityReservationStatusSet."
                                    << capacityReservationStatusesIdx++;
...
void CapacityBlockStatus::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_interconnectStatusHasBeenSet) {
    oStream << location << ".InterconnectStatus="
            << StringUtils::URLEncode(
                   CapacityBlockInterconnectStatusMapper::GetNameForCapacityBlockInterconnectStatus(m_interconnectStatus))
            << "&";
  }
