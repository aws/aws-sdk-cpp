...
DeclarativePoliciesReport::DeclarativePoliciesReport(const XmlNode& xmlNode) { *this = xmlNode; }
...
DeclarativePoliciesReport& DeclarativePoliciesReport::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status =
          ReportStateMapper::GetReportStateForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(statusNode.GetText()).c_str()));
...
void DeclarativePoliciesReport::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(ReportStateMapper::GetNameForReportState(m_status)) << "&";
  }
