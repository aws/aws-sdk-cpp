...
InstanceTypeOffering::InstanceTypeOffering(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceTypeOffering& InstanceTypeOffering::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!locationTypeNode.IsNull()) {
      m_locationType = LocationTypeMapper::GetLocationTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(locationTypeNode.GetText()).c_str()));
...
void InstanceTypeOffering::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_locationTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".LocationType=" << StringUtils::URLEncode(LocationTypeMapper::GetNameForLocationType(m_locationType)) << "&";
  }
