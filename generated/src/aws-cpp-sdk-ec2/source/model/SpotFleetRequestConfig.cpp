...
SpotFleetRequestConfig::SpotFleetRequestConfig(const XmlNode& xmlNode) { *this = xmlNode; }
...
SpotFleetRequestConfig& SpotFleetRequestConfig::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!activityStatusNode.IsNull()) {
      m_activityStatus = ActivityStatusMapper::GetActivityStatusForName(
    ...
    if (!spotFleetRequestStateNode.IsNull()) {
      m_spotFleetRequestState = BatchStateMapper::GetBatchStateForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(spotFleetRequestStateNode.GetText()).c_str()));
...
void SpotFleetRequestConfig::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_activityStatusHasBeenSet) {
    oStream << location << index << locationValue
            << ".ActivityStatus=" << StringUtils::URLEncode(ActivityStatusMapper::GetNameForActivityStatus(m_activityStatus)) << "&";
  }
  ...
  if (m_spotFleetRequestStateHasBeenSet) {
    oStream << location << index << locationValue
            << ".SpotFleetRequestState=" << StringUtils::URLEncode(BatchStateMapper::GetNameForBatchState(m_spotFleetRequestState)) << "&";
  }
