...
MemoryInfo::MemoryInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
MemoryInfo& MemoryInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!sizeInMiBNode.IsNull()) {
      m_sizeInMiB =
          StringUtils::ConvertToInt64(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sizeInMiBNode.GetText()).c_str()).c_str());
