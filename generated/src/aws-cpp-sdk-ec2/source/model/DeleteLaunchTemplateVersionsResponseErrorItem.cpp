...
DeleteLaunchTemplateVersionsResponseErrorItem::DeleteLaunchTemplateVersionsResponseErrorItem(const XmlNode& xmlNode) { *this = xmlNode; }
...
DeleteLaunchTemplateVersionsResponseErrorItem& DeleteLaunchTemplateVersionsResponseErrorItem::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!versionNumberNode.IsNull()) {
      m_versionNumber = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(versionNumberNode.GetText()).c_str()).c_str());
...
void DeleteLaunchTemplateVersionsResponseErrorItem::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
