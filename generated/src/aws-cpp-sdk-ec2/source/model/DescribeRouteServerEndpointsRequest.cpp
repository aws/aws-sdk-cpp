...
Aws::String DescribeRouteServerEndpointsRequest::SerializePayload() const {
  ...
  if (m_routeServerEndpointIdsHasBeenSet) {
    ...
    for (auto& item : m_routeServerEndpointIds) {
      ss << "RouteServerEndpointId." << routeServerEndpointIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeRouteServerEndpointsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
