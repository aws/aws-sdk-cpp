...
Aws::String DescribeImageUsageReportEntriesRequest::SerializePayload() const {
  ...
  if (m_reportIdsHasBeenSet) {
    ...
    for (auto& item : m_reportIds) {
      ss << "ReportId." << reportIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      reportIdsCount++;
    }
  }
...
void DescribeImageUsageReportEntriesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
