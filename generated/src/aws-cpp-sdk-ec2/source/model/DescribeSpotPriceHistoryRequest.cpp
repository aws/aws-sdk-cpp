...
Aws::String DescribeSpotPriceHistoryRequest::SerializePayload() const {
  ...
  if (m_instanceTypesHasBeenSet) {
    ...
    for (auto& item : m_instanceTypes) {
      ss << "InstanceType." << instanceTypesCount << "=" << StringUtils::URLEncode(InstanceTypeMapper::GetNameForInstanceType(item)) << "&";
      instanceTypesCount++;
    }
  }
  ...
  if (m_productDescriptionsHasBeenSet) {
    ...
    for (auto& item : m_productDescriptions) {
      ss << "ProductDescription." << productDescriptionsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeSpotPriceHistoryRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
