...
ProcessorInfo::ProcessorInfo(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!supportedArchitecturesNode.IsNull()) {
      ...
      while (!supportedArchitecturesMember.IsNull()) {
        m_supportedArchitectures.push_back(
            ArchitectureTypeMapper::GetArchitectureTypeForName(StringUtils::Trim(supportedArchitecturesMember.GetText().c_str())));
    ...
    if (!sustainedClockSpeedInGhzNode.IsNull()) {
      m_sustainedClockSpeedInGhz = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sustainedClockSpeedInGhzNode.GetText()).c_str()).c_str());
    ...
    if (!supportedFeaturesNode.IsNull()) {
      ...
      while (!supportedFeaturesMember.IsNull()) {
        m_supportedFeatures.push_back(SupportedAdditionalProcessorFeatureMapper::GetSupportedAdditionalProcessorFeatureForName(
            StringUtils::Trim(supportedFeaturesMember.GetText().c_str())));
...
void ProcessorInfo::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_supportedArchitecturesHasBeenSet) {
    ...
    for (auto& item : m_supportedArchitectures) {
      oStream << location << index << locationValue << ".SupportedArchitectures." << supportedArchitecturesIdx++ << "="
  ...
  if (m_supportedFeaturesHasBeenSet) {
    ...
    for (auto& item : m_supportedFeatures) {
      oStream << location << index << locationValue << ".SupportedFeatures." << supportedFeaturesIdx++ << "="
              << StringUtils::URLEncode(SupportedAdditionalProcessorFeatureMapper::GetNameForSupportedAdditionalProcessorFeature(item))
              << "&";
    }
  }
...
void ProcessorInfo::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_supportedArchitecturesHasBeenSet) {
    unsigned supportedArchitecturesIdx = 1;
    for (auto& item : m_supportedArchitectures) {
      oStream << location << ".SupportedArchitectures." << supportedArchitecturesIdx++ << "="
              << StringUtils::URLEncode(ArchitectureTypeMapper::GetNameForArchitectureType(item)) << "&";
    }
  }
  if (m_sustainedClockSpeedInGhzHasBeenSet) {
  ...
  if (m_supportedFeaturesHasBeenSet) {
    unsigned supportedFeaturesIdx = 1;
    for (auto& item : m_supportedFeatures) {
      oStream << location << ".SupportedFeatures." << supportedFeaturesIdx++ << "="
              << StringUtils::URLEncode(SupportedAdditionalProcessorFeatureMapper::GetNameForSupportedAdditionalProcessorFeature(item))
              << "&";
    }
  }
