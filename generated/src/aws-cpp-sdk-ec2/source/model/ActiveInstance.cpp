...
ActiveInstance::ActiveInstance(const XmlNode& xmlNode) { *this = xmlNode; }
...
ActiveInstance& ActiveInstance::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!instanceHealthNode.IsNull()) {
      m_instanceHealth = InstanceHealthStatusMapper::GetInstanceHealthStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(instanceHealthNode.GetText()).c_str()));
...
void ActiveInstance::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_instanceHealthHasBeenSet) {
    oStream << location << index << locationValue
            << ".InstanceHealth=" << StringUtils::URLEncode(InstanceHealthStatusMapper::GetNameForInstanceHealthStatus(m_instanceHealth))
            << "&";
  }
}
