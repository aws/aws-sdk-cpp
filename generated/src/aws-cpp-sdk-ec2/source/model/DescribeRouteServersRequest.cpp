...
Aws::String DescribeRouteServersRequest::SerializePayload() const {
  ...
  if (m_routeServerIdsHasBeenSet) {
    ...
    for (auto& item : m_routeServerIds) {
      ss << "RouteServerId." << routeServerIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      routeServerIdsCount++;
    }
  }
...
void DescribeRouteServersRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
