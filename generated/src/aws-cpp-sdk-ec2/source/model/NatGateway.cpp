...
NatGateway::NatGateway(const XmlNode& xmlNode) { *this = xmlNode; }
...
NatGateway& NatGateway::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!deleteTimeNode.IsNull()) {
      m_deleteTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deleteTimeNode.GetText()).c_str()).c_str(),
                              Aws::Utils::DateFormat::ISO_8601);
    ...
    if (!stateNode.IsNull()) {
      m_state = NatGatewayStateMapper::GetNatGatewayStateForName(
    ...
    if (!connectivityTypeNode.IsNull()) {
      m_connectivityType = ConnectivityTypeMapper::GetConnectivityTypeForName(
...
void NatGateway::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_deleteTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".DeleteTime=" << StringUtils::URLEncode(m_deleteTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(NatGatewayStateMapper::GetNameForNatGatewayState(m_state)) << "&";
  }
