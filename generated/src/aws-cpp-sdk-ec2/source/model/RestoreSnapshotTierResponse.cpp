...
RestoreSnapshotTierResponse::RestoreSnapshotTierResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
RestoreSnapshotTierResponse& RestoreSnapshotTierResponse::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!restoreStartTimeNode.IsNull()) {
      m_restoreStartTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(restoreStartTimeNode.GetText()).c_str()).c_str(),
    ...
    if (!restoreDurationNode.IsNull()) {
      m_restoreDuration = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(restoreDurationNode.GetText()).c_str()).c_str());
    ...
    if (!isPermanentRestoreNode.IsNull()) {
      m_isPermanentRestore = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isPermanentRestoreNode.GetText()).c_str()).c_str());
