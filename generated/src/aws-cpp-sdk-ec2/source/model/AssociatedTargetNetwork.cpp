...
AssociatedTargetNetwork::AssociatedTargetNetwork(const XmlNode& xmlNode) { *this = xmlNode; }
...
AssociatedTargetNetwork& AssociatedTargetNetwork::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!networkTypeNode.IsNull()) {
      m_networkType = AssociatedNetworkTypeMapper::GetAssociatedNetworkTypeForName(
...
void AssociatedTargetNetwork::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_networkTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".NetworkType=" << StringUtils::URLEncode(AssociatedNetworkTypeMapper::GetNameForAssociatedNetworkType(m_networkType))
            << "&";
  }
}
