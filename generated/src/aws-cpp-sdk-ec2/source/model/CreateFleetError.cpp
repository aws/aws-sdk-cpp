...
CreateFleetError::CreateFleetError(const XmlNode& xmlNode) { *this = xmlNode; }
...
CreateFleetError& CreateFleetError::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lifecycleNode.IsNull()) {
      m_lifecycle = InstanceLifecycleMapper::GetInstanceLifecycleForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lifecycleNode.GetText()).c_str()));
...
void CreateFleetError::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lifecycleHasBeenSet) {
    oStream << location << index << locationValue
            << ".Lifecycle=" << StringUtils::URLEncode(InstanceLifecycleMapper::GetNameForInstanceLifecycle(m_lifecycle)) << "&";
  }
