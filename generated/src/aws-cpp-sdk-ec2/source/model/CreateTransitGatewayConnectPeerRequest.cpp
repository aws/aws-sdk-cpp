...
Aws::String CreateTransitGatewayConnectPeerRequest::SerializePayload() const {
  ...
  if (m_insideCidrBlocksHasBeenSet) {
    ...
    for (auto& item : m_insideCidrBlocks) {
      ss << "InsideCidrBlocks." << insideCidrBlocksCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void CreateTransitGatewayConnectPeerRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
