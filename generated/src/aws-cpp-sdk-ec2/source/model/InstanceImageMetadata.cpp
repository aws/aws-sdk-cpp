...
InstanceImageMetadata::InstanceImageMetadata(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceImageMetadata& InstanceImageMetadata::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!launchTimeNode.IsNull()) {
      m_launchTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(launchTimeNode.GetText()).c_str()).c_str(),
...
void InstanceImageMetadata::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_launchTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".LaunchTime=" << StringUtils::URLEncode(m_launchTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
