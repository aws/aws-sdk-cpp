...
ScheduledInstanceRecurrence::ScheduledInstanceRecurrence(const XmlNode& xmlNode) { *this = xmlNode; }
...
ScheduledInstanceRecurrence& ScheduledInstanceRecurrence::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!occurrenceRelativeToEndNode.IsNull()) {
      m_occurrenceRelativeToEnd = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(occurrenceRelativeToEndNode.GetText()).c_str()).c_str());
...
void ScheduledInstanceRecurrence::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
