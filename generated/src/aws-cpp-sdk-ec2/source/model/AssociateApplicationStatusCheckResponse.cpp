...
AssociateApplicationStatusCheckResponse::AssociateApplicationStatusCheckResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  *this = result;
}

AssociateApplicationStatusCheckResponse& AssociateApplicationStatusCheckResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  m_HttpResponseCode = result.GetResponseCode();
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "AssociateApplicationStatusCheckResponse")) {
    resultNode = rootNode.FirstChild("AssociateApplicationStatusCheckResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::AssociateApplicationStatusCheckResponse",
