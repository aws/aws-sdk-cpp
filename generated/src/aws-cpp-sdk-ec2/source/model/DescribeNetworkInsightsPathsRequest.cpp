...
Aws::String DescribeNetworkInsightsPathsRequest::SerializePayload() const {
  ...
  if (m_networkInsightsPathIdsHasBeenSet) {
    ...
    for (auto& item : m_networkInsightsPathIds) {
      ss << "NetworkInsightsPathId." << networkInsightsPathIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeNetworkInsightsPathsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
