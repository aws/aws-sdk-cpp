...
Aws::String DescribeReservedInstancesOfferingsRequest::SerializePayload() const {
  ...
  if (m_reservedInstancesOfferingIdsHasBeenSet) {
    ...
    for (auto& item : m_reservedInstancesOfferingIds) {
      ss << "ReservedInstancesOfferingId." << reservedInstancesOfferingIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeReservedInstancesOfferingsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
