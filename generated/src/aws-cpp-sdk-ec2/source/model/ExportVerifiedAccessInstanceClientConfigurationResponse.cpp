...
ExportVerifiedAccessInstanceClientConfigurationResponse& ExportVerifiedAccessInstanceClientConfigurationResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!deviceTrustProvidersNode.IsNull()) {
      ...
      while (!deviceTrustProvidersMember.IsNull()) {
        m_deviceTrustProviders.push_back(DeviceTrustProviderTypeMapper::GetDeviceTrustProviderTypeForName(
            StringUtils::Trim(deviceTrustProvidersMember.GetText().c_str())));
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::ExportVerifiedAccessInstanceClientConfigurationResponse",
