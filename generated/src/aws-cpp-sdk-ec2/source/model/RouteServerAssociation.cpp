...
RouteServerAssociation::RouteServerAssociation(const XmlNode& xmlNode) { *this = xmlNode; }
...
RouteServerAssociation& RouteServerAssociation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = RouteServerAssociationStateMapper::GetRouteServerAssociationStateForName(
...
void RouteServerAssociation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(RouteServerAssociationStateMapper::GetNameForRouteServerAssociationState(m_state))
            << "&";
  }
}
