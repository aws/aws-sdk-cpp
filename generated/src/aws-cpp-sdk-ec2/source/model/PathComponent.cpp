...
PathComponent::PathComponent(const XmlNode& xmlNode) { *this = xmlNode; }
...
PathComponent& PathComponent::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!sequenceNumberNode.IsNull()) {
      m_sequenceNumber = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sequenceNumberNode.GetText()).c_str()).c_str());
