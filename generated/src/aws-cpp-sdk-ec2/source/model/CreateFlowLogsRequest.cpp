...
Aws::String CreateFlowLogsRequest::SerializePayload() const {
  ...
  if (m_resourceIdsHasBeenSet) {
    ...
    for (auto& item : m_resourceIds) {
      ss << "ResourceId." << resourceIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      resourceIdsCount++;
    }
  }
...
void CreateFlowLogsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
