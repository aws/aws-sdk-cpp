...
Aws::String ModifyVpcEndpointServicePermissionsRequest::SerializePayload() const {
  ...
  if (m_addAllowedPrincipalsHasBeenSet) {
    ...
    for (auto& item : m_addAllowedPrincipals) {
      ss << "AddAllowedPrincipals." << addAllowedPrincipalsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_removeAllowedPrincipalsHasBeenSet) {
    ...
    for (auto& item : m_removeAllowedPrincipals) {
      ss << "RemoveAllowedPrincipals." << removeAllowedPrincipalsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void ModifyVpcEndpointServicePermissionsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
