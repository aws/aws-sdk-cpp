...
ImageUsageReportEntry::ImageUsageReportEntry(const XmlNode& xmlNode) { *this = xmlNode; }
...
ImageUsageReportEntry& ImageUsageReportEntry::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!usageCountNode.IsNull()) {
      m_usageCount =
          StringUtils::ConvertToInt64(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(usageCountNode.GetText()).c_str()).c_str());
    ...
    if (!reportCreationTimeNode.IsNull()) {
      m_reportCreationTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(reportCreationTimeNode.GetText()).c_str()).c_str(),
...
void ImageUsageReportEntry::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_reportCreationTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".ReportCreationTime=" << StringUtils::URLEncode(m_reportCreationTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
}
