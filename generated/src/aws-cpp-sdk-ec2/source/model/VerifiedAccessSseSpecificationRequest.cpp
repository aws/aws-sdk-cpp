...
VerifiedAccessSseSpecificationRequest::VerifiedAccessSseSpecificationRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
VerifiedAccessSseSpecificationRequest& VerifiedAccessSseSpecificationRequest::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!customerManagedKeyEnabledNode.IsNull()) {
      m_customerManagedKeyEnabled = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(customerManagedKeyEnabledNode.GetText()).c_str()).c_str());
...
void VerifiedAccessSseSpecificationRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
