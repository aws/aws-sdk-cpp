...
VpcEncryptionControl::VpcEncryptionControl(const XmlNode& xmlNode) { *this = xmlNode; }
...
VpcEncryptionControl& VpcEncryptionControl::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = VpcEncryptionControlStateMapper::GetVpcEncryptionControlStateForName(
...
void VpcEncryptionControl::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(VpcEncryptionControlStateMapper::GetNameForVpcEncryptionControlState(m_state)) << "&";
  }
