...
EbsStatusSummary::EbsStatusSummary(const XmlNode& xmlNode) { *this = xmlNode; }
...
EbsStatusSummary& EbsStatusSummary::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = SummaryStatusMapper::GetSummaryStatusForName(
...
void EbsStatusSummary::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(SummaryStatusMapper::GetNameForSummaryStatus(m_status)) << "&";
  }
}
