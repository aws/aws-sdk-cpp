...
DiskImageDescription::DiskImageDescription(const XmlNode& xmlNode) { *this = xmlNode; }
...
DiskImageDescription& DiskImageDescription::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!formatNode.IsNull()) {
      m_format = DiskImageFormatMapper::GetDiskImageFormatForName(
...
void DiskImageDescription::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_formatHasBeenSet) {
    oStream << location << index << locationValue
            << ".Format=" << StringUtils::URLEncode(DiskImageFormatMapper::GetNameForDiskImageFormat(m_format)) << "&";
  }
