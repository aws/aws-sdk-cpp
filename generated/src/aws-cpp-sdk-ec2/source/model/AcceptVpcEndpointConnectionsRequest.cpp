...
Aws::String AcceptVpcEndpointConnectionsRequest::SerializePayload() const {
  ...
  if (m_vpcEndpointIdsHasBeenSet) {
    ...
    for (auto& item : m_vpcEndpointIds) {
      ss << "VpcEndpointId." << vpcEndpointIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      vpcEndpointIdsCount++;
    }
  }
...
void AcceptVpcEndpointConnectionsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
