...
Aws::String CreateVpcEndpointServiceConfigurationRequest::SerializePayload() const {
  ...
  if (m_networkLoadBalancerArnsHasBeenSet) {
    ...
    for (auto& item : m_networkLoadBalancerArns) {
      ss << "NetworkLoadBalancerArn." << networkLoadBalancerArnsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_gatewayLoadBalancerArnsHasBeenSet) {
    ...
    for (auto& item : m_gatewayLoadBalancerArns) {
      ss << "GatewayLoadBalancerArn." << gatewayLoadBalancerArnsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_supportedIpAddressTypesHasBeenSet) {
    ...
    for (auto& item : m_supportedIpAddressTypes) {
      ss << "SupportedIpAddressType." << supportedIpAddressTypesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_supportedRegionsHasBeenSet) {
    ...
    for (auto& item : m_supportedRegions) {
      ss << "SupportedRegion." << supportedRegionsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void CreateVpcEndpointServiceConfigurationRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
