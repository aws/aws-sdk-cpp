...
Aws::String DescribeElasticGpusRequest::SerializePayload() const {
  ...
  if (m_elasticGpuIdsHasBeenSet) {
    ...
    for (auto& item : m_elasticGpuIds) {
      ss << "ElasticGpuId." << elasticGpuIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      elasticGpuIdsCount++;
    }
  }
...
void DescribeElasticGpusRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
