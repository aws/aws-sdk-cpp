...
Aws::String AllocateHostsRequest::SerializePayload() const {
  ...
  if (m_assetIdsHasBeenSet) {
    ...
    for (auto& item : m_assetIds) {
      ss << "AssetId." << assetIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      assetIdsCount++;
    }
  }
...
void AllocateHostsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
