...
Aws::String DescribeTransitGatewaysRequest::SerializePayload() const {
  ...
  if (m_transitGatewayIdsHasBeenSet) {
    ...
    for (auto& item : m_transitGatewayIds) {
      ss << "TransitGatewayIds." << transitGatewayIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeTransitGatewaysRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
