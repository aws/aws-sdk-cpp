...
Aws::String DescribeVpcClassicLinkDnsSupportRequest::SerializePayload() const {
  ...
  if (m_vpcIdsHasBeenSet) {
    ...
    for (auto& item : m_vpcIds) {
      ss << "VpcIds." << vpcIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      vpcIdsCount++;
    }
  }
...
void DescribeVpcClassicLinkDnsSupportRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
