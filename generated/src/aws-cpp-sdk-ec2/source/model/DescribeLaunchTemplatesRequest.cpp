...
Aws::String DescribeLaunchTemplatesRequest::SerializePayload() const {
  ...
  if (m_launchTemplateIdsHasBeenSet) {
    ...
    for (auto& item : m_launchTemplateIds) {
      ss << "LaunchTemplateId." << launchTemplateIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_launchTemplateNamesHasBeenSet) {
    ...
    for (auto& item : m_launchTemplateNames) {
      ss << "LaunchTemplateName." << launchTemplateNamesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeLaunchTemplatesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
