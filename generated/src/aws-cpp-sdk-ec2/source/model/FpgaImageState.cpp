...
FpgaImageState::FpgaImageState(const XmlNode& xmlNode) { *this = xmlNode; }
...
FpgaImageState& FpgaImageState::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!codeNode.IsNull()) {
      m_code = FpgaImageStateCodeMapper::GetFpgaImageStateCodeForName(
...
void FpgaImageState::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_codeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Code=" << StringUtils::URLEncode(FpgaImageStateCodeMapper::GetNameForFpgaImageStateCode(m_code)) << "&";
  }
