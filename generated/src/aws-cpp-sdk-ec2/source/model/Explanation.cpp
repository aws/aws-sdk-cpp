...
Explanation::Explanation(const XmlNode& xmlNode) { *this = xmlNode; }
...
Explanation& Explanation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!loadBalancerListenerPortNode.IsNull()) {
      m_loadBalancerListenerPort = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(loadBalancerListenerPortNode.GetText()).c_str()).c_str());
    ...
    if (!loadBalancerTargetPortNode.IsNull()) {
      m_loadBalancerTargetPort = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(loadBalancerTargetPortNode.GetText()).c_str()).c_str());
