...
RouteServerBfdStatus::RouteServerBfdStatus(const XmlNode& xmlNode)
{
  *this = xmlNode;
}
...
RouteServerBfdStatus& RouteServerBfdStatus::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    {
      m_status = RouteServerBfdStateMapper::GetRouteServerBfdStateForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(statusNode.GetText()).c_str()));
...
void RouteServerBfdStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const
{
  ...
  {
      oStream << location << index << locationValue << ".Status=" << StringUtils::URLEncode(RouteServerBfdStateMapper::GetNameForRouteServerBfdState(m_status)) << "&";
  }
