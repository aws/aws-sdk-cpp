...
ClientVpnRouteStatus::ClientVpnRouteStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
ClientVpnRouteStatus& ClientVpnRouteStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!codeNode.IsNull()) {
      m_code = ClientVpnRouteStatusCodeMapper::GetClientVpnRouteStatusCodeForName(
...
void ClientVpnRouteStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_codeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Code=" << StringUtils::URLEncode(ClientVpnRouteStatusCodeMapper::GetNameForClientVpnRouteStatusCode(m_code)) << "&";
  }
