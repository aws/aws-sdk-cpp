...
Aws::String DescribeDhcpOptionsRequest::SerializePayload() const {
  ...
  if (m_dhcpOptionsIdsHasBeenSet) {
    ...
    for (auto& item : m_dhcpOptionsIds) {
      ss << "DhcpOptionsId." << dhcpOptionsIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      dhcpOptionsIdsCount++;
    }
  }
...
void DescribeDhcpOptionsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
