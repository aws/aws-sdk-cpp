...
VgwTelemetry::VgwTelemetry(const XmlNode& xmlNode) { *this = xmlNode; }
...
VgwTelemetry& VgwTelemetry::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!acceptedRouteCountNode.IsNull()) {
      m_acceptedRouteCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(acceptedRouteCountNode.GetText()).c_str()).c_str());
    ...
    if (!lastStatusChangeNode.IsNull()) {
      m_lastStatusChange =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastStatusChangeNode.GetText()).c_str()).c_str(),
    ...
    if (!statusNode.IsNull()) {
      m_status = TelemetryStatusMapper::GetTelemetryStatusForName(
...
void VgwTelemetry::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastStatusChangeHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastStatusChange=" << StringUtils::URLEncode(m_lastStatusChange.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(TelemetryStatusMapper::GetNameForTelemetryStatus(m_status)) << "&";
  }
