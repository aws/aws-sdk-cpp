...
Aws::String DescribeTrafficMirrorSessionsRequest::SerializePayload() const {
  ...
  if (m_trafficMirrorSessionIdsHasBeenSet) {
    ...
    for (auto& item : m_trafficMirrorSessionIds) {
      ss << "TrafficMirrorSessionId." << trafficMirrorSessionIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeTrafficMirrorSessionsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
