...
ResourceTypeOption::ResourceTypeOption(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResourceTypeOption& ResourceTypeOption::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!optionNameNode.IsNull()) {
      m_optionName = ImageReferenceOptionNameMapper::GetImageReferenceOptionNameForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(optionNameNode.GetText()).c_str()));
...
void ResourceTypeOption::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_optionNameHasBeenSet) {
    oStream << location << index << locationValue
            << ".OptionName=" << StringUtils::URLEncode(ImageReferenceOptionNameMapper::GetNameForImageReferenceOptionName(m_optionName))
            << "&";
  }
