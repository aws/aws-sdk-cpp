...
DescribeCapacityBlockExtensionOfferingsResponse& DescribeCapacityBlockExtensionOfferingsResponse::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!capacityBlockExtensionOfferingsNode.IsNull()) {
      ...
      m_capacityBlockExtensionOfferingsHasBeenSet = !capacityBlockExtensionOfferingsMember.IsNull();
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::DescribeCapacityBlockExtensionOfferingsResponse",
