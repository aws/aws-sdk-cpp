...
LocalGatewayVirtualInterfaceGroup::LocalGatewayVirtualInterfaceGroup(const XmlNode& xmlNode) { *this = xmlNode; }
...
LocalGatewayVirtualInterfaceGroup& LocalGatewayVirtualInterfaceGroup::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!localBgpAsnExtendedNode.IsNull()) {
      m_localBgpAsnExtended = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(localBgpAsnExtendedNode.GetText()).c_str()).c_str());
    ...
    if (!configurationStateNode.IsNull()) {
      m_configurationState =
          LocalGatewayVirtualInterfaceGroupConfigurationStateMapper::GetLocalGatewayVirtualInterfaceGroupConfigurationStateForName(
...
void LocalGatewayVirtualInterfaceGroup::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_localGatewayVirtualInterfaceIdsHasBeenSet) {
    ...
    for (auto& item : m_localGatewayVirtualInterfaceIds) {
      oStream << location << index << locationValue << ".LocalGatewayVirtualInterfaceIdSet." << localGatewayVirtualInterfaceIdsIdx++ << "="
  ...
  if (m_localGatewayVirtualInterfaceGroupArnHasBeenSet) {
    oStream << location << index << locationValue
            << ".LocalGatewayVirtualInterfaceGroupArn=" << StringUtils::URLEncode(m_localGatewayVirtualInterfaceGroupArn.c_str()) << "&";
  }
  ...
  if (m_configurationStateHasBeenSet) {
    oStream << location << index << locationValue << ".ConfigurationState="
            << StringUtils::URLEncode(
                   LocalGatewayVirtualInterfaceGroupConfigurationStateMapper::GetNameForLocalGatewayVirtualInterfaceGroupConfigurationState(
                       m_configurationState))
            << "&";
  }
}
...
void LocalGatewayVirtualInterfaceGroup::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_localGatewayVirtualInterfaceIdsHasBeenSet) {
    unsigned localGatewayVirtualInterfaceIdsIdx = 1;
    for (auto& item : m_localGatewayVirtualInterfaceIds) {
      oStream << location << ".LocalGatewayVirtualInterfaceIdSet." << localGatewayVirtualInterfaceIdsIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
  if (m_localGatewayIdHasBeenSet) {
