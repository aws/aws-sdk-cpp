...
ExportToS3Task::ExportToS3Task(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!containerFormatNode.IsNull()) {
      m_containerFormat = ContainerFormatMapper::GetContainerFormatForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(containerFormatNode.GetText()).c_str()));
    ...
    if (!diskImageFormatNode.IsNull()) {
      m_diskImageFormat = DiskImageFormatMapper::GetDiskImageFormatForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(diskImageFormatNode.GetText()).c_str()));
...
void ExportToS3Task::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_containerFormatHasBeenSet) {
    oStream << location << index << locationValue
            << ".ContainerFormat=" << StringUtils::URLEncode(ContainerFormatMapper::GetNameForContainerFormat(m_containerFormat)) << "&";
  }

  if (m_diskImageFormatHasBeenSet) {
    oStream << location << index << locationValue
            << ".DiskImageFormat=" << StringUtils::URLEncode(DiskImageFormatMapper::GetNameForDiskImageFormat(m_diskImageFormat)) << "&";
  }
