...
HistoryRecordEntry::HistoryRecordEntry(const XmlNode& xmlNode) { *this = xmlNode; }
...
HistoryRecordEntry& HistoryRecordEntry::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!eventTypeNode.IsNull()) {
      m_eventType = FleetEventTypeMapper::GetFleetEventTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(eventTypeNode.GetText()).c_str()));
...
void HistoryRecordEntry::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_eventTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".EventType=" << StringUtils::URLEncode(FleetEventTypeMapper::GetNameForFleetEventType(m_eventType)) << "&";
  }
...
void HistoryRecordEntry::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_eventTypeHasBeenSet) {
    oStream << location << ".EventType=" << StringUtils::URLEncode(FleetEventTypeMapper::GetNameForFleetEventType(m_eventType)) << "&";
  }
  if (m_timestampHasBeenSet) {
