...
VpcAttachment::VpcAttachment(const XmlNode& xmlNode) { *this = xmlNode; }
...
VpcAttachment& VpcAttachment::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = AttachmentStatusMapper::GetAttachmentStatusForName(
...
void VpcAttachment::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(AttachmentStatusMapper::GetNameForAttachmentStatus(m_state)) << "&";
  }
}
