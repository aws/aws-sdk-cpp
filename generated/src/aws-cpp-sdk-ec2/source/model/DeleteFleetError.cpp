...
DeleteFleetError::DeleteFleetError(const XmlNode& xmlNode) { *this = xmlNode; }
...
DeleteFleetError& DeleteFleetError::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!codeNode.IsNull()) {
      m_code = DeleteFleetErrorCodeMapper::GetDeleteFleetErrorCodeForName(
...
void DeleteFleetError::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_codeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Code=" << StringUtils::URLEncode(DeleteFleetErrorCodeMapper::GetNameForDeleteFleetErrorCode(m_code)) << "&";
  }
