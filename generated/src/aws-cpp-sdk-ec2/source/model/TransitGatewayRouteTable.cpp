...
TransitGatewayRouteTable::TransitGatewayRouteTable(const XmlNode& xmlNode) { *this = xmlNode; }
...
TransitGatewayRouteTable& TransitGatewayRouteTable::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = TransitGatewayRouteTableStateMapper::GetTransitGatewayRouteTableStateForName(
    ...
    if (!defaultAssociationRouteTableNode.IsNull()) {
      m_defaultAssociationRouteTable = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(defaultAssociationRouteTableNode.GetText()).c_str()).c_str());
    ...
    if (!defaultPropagationRouteTableNode.IsNull()) {
      m_defaultPropagationRouteTable = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(defaultPropagationRouteTableNode.GetText()).c_str()).c_str());
...
void TransitGatewayRouteTable::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(TransitGatewayRouteTableStateMapper::GetNameForTransitGatewayRouteTableState(m_state))
            << "&";
  }
