...
Ec2InstanceConnectEndpoint::Ec2InstanceConnectEndpoint(const XmlNode& xmlNode) { *this = xmlNode; }
...
Ec2InstanceConnectEndpoint& Ec2InstanceConnectEndpoint::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = Ec2InstanceConnectEndpointStateMapper::GetEc2InstanceConnectEndpointStateForName(
    ...
    if (!preserveClientIpNode.IsNull()) {
      m_preserveClientIp = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(preserveClientIpNode.GetText()).c_str()).c_str());
...
void Ec2InstanceConnectEndpoint::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_instanceConnectEndpointIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".InstanceConnectEndpointId=" << StringUtils::URLEncode(m_instanceConnectEndpointId.c_str()) << "&";
  }

  if (m_instanceConnectEndpointArnHasBeenSet) {
    oStream << location << index << locationValue
            << ".InstanceConnectEndpointArn=" << StringUtils::URLEncode(m_instanceConnectEndpointArn.c_str()) << "&";
  }

  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue << ".State="
            << StringUtils::URLEncode(Ec2InstanceConnectEndpointStateMapper::GetNameForEc2InstanceConnectEndpointState(m_state)) << "&";
  }
  ...
  if (m_networkInterfaceIdsHasBeenSet) {
    ...
    for (auto& item : m_networkInterfaceIds) {
      oStream << location << index << locationValue << ".NetworkInterfaceIdSet." << networkInterfaceIdsIdx++ << "="
