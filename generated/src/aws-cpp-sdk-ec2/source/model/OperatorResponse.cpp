...
OperatorResponse::OperatorResponse(const XmlNode& xmlNode) { *this = xmlNode; }
...
OperatorResponse& OperatorResponse::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!managedNode.IsNull()) {
      m_managed =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(managedNode.GetText()).c_str()).c_str());
