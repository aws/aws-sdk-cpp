...
Aws::String DescribeIpv6PoolsRequest::SerializePayload() const {
  ...
  if (m_poolIdsHasBeenSet) {
    ...
    for (auto& item : m_poolIds) {
      ss << "PoolId." << poolIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      poolIdsCount++;
    }
  }
...
void DescribeIpv6PoolsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
