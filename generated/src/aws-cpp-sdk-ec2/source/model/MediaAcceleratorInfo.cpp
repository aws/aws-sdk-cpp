...
MediaAcceleratorInfo::MediaAcceleratorInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
MediaAcceleratorInfo& MediaAcceleratorInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!totalMediaMemoryInMiBNode.IsNull()) {
      m_totalMediaMemoryInMiB = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalMediaMemoryInMiBNode.GetText()).c_str()).c_str());
