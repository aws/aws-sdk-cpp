...
TransitGatewayMulticastRegisteredGroupMembers::TransitGatewayMulticastRegisteredGroupMembers(const XmlNode& xmlNode) { *this = xmlNode; }
...
void TransitGatewayMulticastRegisteredGroupMembers::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_registeredNetworkInterfaceIdsHasBeenSet) {
    ...
    for (auto& item : m_registeredNetworkInterfaceIds) {
      oStream << location << index << locationValue << ".RegisteredNetworkInterfaceIds." << registeredNetworkInterfaceIdsIdx++ << "="
...
void TransitGatewayMulticastRegisteredGroupMembers::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_registeredNetworkInterfaceIdsHasBeenSet) {
    unsigned registeredNetworkInterfaceIdsIdx = 1;
    for (auto& item : m_registeredNetworkInterfaceIds) {
      oStream << location << ".RegisteredNetworkInterfaceIds." << registeredNetworkInterfaceIdsIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
  if (m_groupIpAddressHasBeenSet) {
