...
CertificateAuthentication::CertificateAuthentication(const XmlNode& xmlNode) { *this = xmlNode; }
...
void CertificateAuthentication::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                               const char* locationValue) const {
  if (m_clientRootCertificateChainHasBeenSet) {
    oStream << location << index << locationValue
            << ".ClientRootCertificateChain=" << StringUtils::URLEncode(m_clientRootCertificateChain.c_str()) << "&";
  }
}
