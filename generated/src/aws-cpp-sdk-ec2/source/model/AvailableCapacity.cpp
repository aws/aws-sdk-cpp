...
AvailableCapacity::AvailableCapacity(const XmlNode& xmlNode) { *this = xmlNode; }
...
AvailableCapacity& AvailableCapacity::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!availableVCpusNode.IsNull()) {
      m_availableVCpus = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(availableVCpusNode.GetText()).c_str()).c_str());
