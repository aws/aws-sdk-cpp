...
ManagedPrefixList::ManagedPrefixList(const XmlNode& xmlNode) { *this = xmlNode; }
...
ManagedPrefixList& ManagedPrefixList::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = PrefixListStateMapper::GetPrefixListStateForName(
    ...
    if (!maxEntriesNode.IsNull()) {
      m_maxEntries =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maxEntriesNode.GetText()).c_str()).c_str());
...
void ManagedPrefixList::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(PrefixListStateMapper::GetNameForPrefixListState(m_state)) << "&";
  }
