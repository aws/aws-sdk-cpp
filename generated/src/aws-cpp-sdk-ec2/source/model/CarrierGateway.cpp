...
CarrierGateway::CarrierGateway(const XmlNode& xmlNode) { *this = xmlNode; }
...
CarrierGateway& CarrierGateway::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = CarrierGatewayStateMapper::GetCarrierGatewayStateForName(
...
void CarrierGateway::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(CarrierGatewayStateMapper::GetNameForCarrierGatewayState(m_state)) << "&";
  }
