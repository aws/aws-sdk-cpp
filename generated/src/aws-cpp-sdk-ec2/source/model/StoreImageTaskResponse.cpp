...
StoreImageTaskResponse::StoreImageTaskResponse(const XmlNode& xmlNode) { *this = xmlNode; }
...
StoreImageTaskResponse& StoreImageTaskResponse::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!progressPercentageNode.IsNull()) {
      m_progressPercentage = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(progressPercentageNode.GetText()).c_str()).c_str());
...
void StoreImageTaskResponse::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_taskStartTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".TaskStartTime=" << StringUtils::URLEncode(m_taskStartTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
