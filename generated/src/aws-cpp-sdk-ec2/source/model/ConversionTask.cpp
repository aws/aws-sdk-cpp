...
ConversionTask::ConversionTask(const XmlNode& xmlNode) { *this = xmlNode; }
...
ConversionTask& ConversionTask::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = ConversionTaskStateMapper::GetConversionTaskStateForName(
...
void ConversionTask::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(ConversionTaskStateMapper::GetNameForConversionTaskState(m_state)) << "&";
  }
