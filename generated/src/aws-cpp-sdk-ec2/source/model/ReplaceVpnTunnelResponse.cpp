...
ReplaceVpnTunnelResponse::ReplaceVpnTunnelResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }

ReplaceVpnTunnelResponse& ReplaceVpnTunnelResponse::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "ReplaceVpnTunnelResponse")) {
    resultNode = rootNode.FirstChild("ReplaceVpnTunnelResponse");
  }
