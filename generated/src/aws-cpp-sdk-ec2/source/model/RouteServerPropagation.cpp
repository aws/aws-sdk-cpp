...
RouteServerPropagation::RouteServerPropagation(const XmlNode& xmlNode) { *this = xmlNode; }
...
RouteServerPropagation& RouteServerPropagation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = RouteServerPropagationStateMapper::GetRouteServerPropagationStateForName(
...
void RouteServerPropagation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(RouteServerPropagationStateMapper::GetNameForRouteServerPropagationState(m_state))
            << "&";
  }
}
