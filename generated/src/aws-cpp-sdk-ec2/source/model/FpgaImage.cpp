...
FpgaImage::FpgaImage(const XmlNode& xmlNode) { *this = xmlNode; }
...
FpgaImage& FpgaImage::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!dataRetentionSupportNode.IsNull()) {
      m_dataRetentionSupport = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(dataRetentionSupportNode.GetText()).c_str()).c_str());
...
void FpgaImage::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_instanceTypesHasBeenSet) {
    ...
    for (auto& item : m_instanceTypes) {
      oStream << location << index << locationValue << ".InstanceTypes." << instanceTypesIdx++ << "="
