...
InstanceMetadataOptionsResponse::InstanceMetadataOptionsResponse(const XmlNode& xmlNode) { *this = xmlNode; }
...
InstanceMetadataOptionsResponse& InstanceMetadataOptionsResponse::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = InstanceMetadataOptionsStateMapper::GetInstanceMetadataOptionsStateForName(
    ...
    if (!httpProtocolIpv6Node.IsNull()) {
      m_httpProtocolIpv6 = InstanceMetadataProtocolStateMapper::GetInstanceMetadataProtocolStateForName(
...
void InstanceMetadataOptionsResponse::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                     const char* locationValue) const {
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(InstanceMetadataOptionsStateMapper::GetNameForInstanceMetadataOptionsState(m_state))
            << "&";
  }
  ...
  if (m_httpProtocolIpv6HasBeenSet) {
    oStream << location << index << locationValue << ".HttpProtocolIpv6="
            << StringUtils::URLEncode(InstanceMetadataProtocolStateMapper::GetNameForInstanceMetadataProtocolState(m_httpProtocolIpv6))
            << "&";
  }
