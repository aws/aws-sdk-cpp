...
Aws::String DescribePrincipalIdFormatRequest::SerializePayload() const {
  ...
  if (m_resourcesHasBeenSet) {
    ...
    for (auto& item : m_resources) {
      ss << "Resource." << resourcesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      resourcesCount++;
    }
  }
...
void DescribePrincipalIdFormatRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
