...
DestinationOptionsResponse::DestinationOptionsResponse(const XmlNode& xmlNode) { *this = xmlNode; }
...
DestinationOptionsResponse& DestinationOptionsResponse::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!fileFormatNode.IsNull()) {
      m_fileFormat = DestinationFileFormatMapper::GetDestinationFileFormatForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(fileFormatNode.GetText()).c_str()));
    ...
    if (!hiveCompatiblePartitionsNode.IsNull()) {
      m_hiveCompatiblePartitions = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(hiveCompatiblePartitionsNode.GetText()).c_str()).c_str());
    ...
    if (!perHourPartitionNode.IsNull()) {
      m_perHourPartition = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(perHourPartitionNode.GetText()).c_str()).c_str());
...
void DestinationOptionsResponse::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                const char* locationValue) const {
  if (m_fileFormatHasBeenSet) {
    oStream << location << index << locationValue
            << ".FileFormat=" << StringUtils::URLEncode(DestinationFileFormatMapper::GetNameForDestinationFileFormat(m_fileFormat)) << "&";
  }
