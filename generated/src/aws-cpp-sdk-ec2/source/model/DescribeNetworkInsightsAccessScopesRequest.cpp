...
Aws::String DescribeNetworkInsightsAccessScopesRequest::SerializePayload() const {
  ...
  if (m_networkInsightsAccessScopeIdsHasBeenSet) {
    ...
    for (auto& item : m_networkInsightsAccessScopeIds) {
      ss << "NetworkInsightsAccessScopeId." << networkInsightsAccessScopeIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeNetworkInsightsAccessScopesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
