...
NeuronInfo::NeuronInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
NeuronInfo& NeuronInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!totalNeuronDeviceMemoryInMiBNode.IsNull()) {
      m_totalNeuronDeviceMemoryInMiB = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalNeuronDeviceMemoryInMiBNode.GetText()).c_str()).c_str());
