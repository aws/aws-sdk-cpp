...
NeuronDeviceMemoryInfo::NeuronDeviceMemoryInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
NeuronDeviceMemoryInfo& NeuronDeviceMemoryInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!sizeInMiBNode.IsNull()) {
      m_sizeInMiB =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sizeInMiBNode.GetText()).c_str()).c_str());
