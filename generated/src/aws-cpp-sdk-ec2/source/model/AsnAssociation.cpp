...
AsnAssociation::AsnAssociation(const XmlNode& xmlNode) { *this = xmlNode; }
...
AsnAssociation& AsnAssociation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = AsnAssociationStateMapper::GetAsnAssociationStateForName(
...
void AsnAssociation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(AsnAssociationStateMapper::GetNameForAsnAssociationState(m_state)) << "&";
  }
}
