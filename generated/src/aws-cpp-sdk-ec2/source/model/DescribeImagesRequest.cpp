...
Aws::String DescribeImagesRequest::SerializePayload() const {
  ...
  if (m_executableUsersHasBeenSet) {
    ...
    for (auto& item : m_executableUsers) {
      ss << "ExecutableBy." << executableUsersCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeImagesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
