...
Aws::String DescribeScheduledInstancesRequest::SerializePayload() const {
  ...
  if (m_scheduledInstanceIdsHasBeenSet) {
    ...
    for (auto& item : m_scheduledInstanceIds) {
      ss << "ScheduledInstanceId." << scheduledInstanceIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeScheduledInstancesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
