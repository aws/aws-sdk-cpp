...
DisableFastSnapshotRestoreErrorItem::DisableFastSnapshotRestoreErrorItem(const XmlNode& xmlNode) { *this = xmlNode; }

DisableFastSnapshotRestoreErrorItem& DisableFastSnapshotRestoreErrorItem::operator=(const XmlNode& xmlNode) {
...
void DisableFastSnapshotRestoreErrorItem::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_fastSnapshotRestoreStateErrorsHasBeenSet) {
    ...
    for (auto& item : m_fastSnapshotRestoreStateErrors) {
      ...
      fastSnapshotRestoreStateErrorsSs << location << index << locationValue << ".FastSnapshotRestoreStateErrorSet."
                                       << fastSnapshotRestoreStateErrorsIdx++;
      item.OutputToStream(oStream, fastSnapshotRestoreStateErrorsSs.str().c_str());
    }
  }
}

void DisableFastSnapshotRestoreErrorItem::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_snapshotIdHasBeenSet) {
