...
PublicIpv4PoolRange::PublicIpv4PoolRange(const XmlNode& xmlNode) { *this = xmlNode; }
...
PublicIpv4PoolRange& PublicIpv4PoolRange::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!addressCountNode.IsNull()) {
      m_addressCount =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(addressCountNode.GetText()).c_str()).c_str());
    ...
    if (!availableAddressCountNode.IsNull()) {
      m_availableAddressCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(availableAddressCountNode.GetText()).c_str()).c_str());
