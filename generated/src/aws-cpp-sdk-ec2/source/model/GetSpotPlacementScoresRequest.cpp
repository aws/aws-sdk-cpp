...
Aws::String GetSpotPlacementScoresRequest::SerializePayload() const {
  ...
  if (m_instanceTypesHasBeenSet) {
    ...
    for (auto& item : m_instanceTypes) {
      ss << "InstanceType." << instanceTypesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      instanceTypesCount++;
    }
  }
...
void GetSpotPlacementScoresRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
