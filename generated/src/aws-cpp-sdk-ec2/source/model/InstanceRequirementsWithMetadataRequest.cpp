...
InstanceRequirementsWithMetadataRequest::InstanceRequirementsWithMetadataRequest(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!architectureTypesNode.IsNull()) {
      ...
      while (!architectureTypesMember.IsNull()) {
        m_architectureTypes.push_back(
            ArchitectureTypeMapper::GetArchitectureTypeForName(StringUtils::Trim(architectureTypesMember.GetText().c_str())));
    ...
    if (!virtualizationTypesNode.IsNull()) {
      ...
      while (!virtualizationTypesMember.IsNull()) {
        m_virtualizationTypes.push_back(
            VirtualizationTypeMapper::GetVirtualizationTypeForName(StringUtils::Trim(virtualizationTypesMember.GetText().c_str())));
...
void InstanceRequirementsWithMetadataRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                             const char* locationValue) const {
  if (m_architectureTypesHasBeenSet) {
    unsigned architectureTypesIdx = 1;
    for (auto& item : m_architectureTypes) {
      oStream << location << index << locationValue << ".ArchitectureType." << architectureTypesIdx++ << "="
  ...
  if (m_virtualizationTypesHasBeenSet) {
    ...
    for (auto& item : m_virtualizationTypes) {
      oStream << location << index << locationValue << ".VirtualizationType." << virtualizationTypesIdx++ << "="
...
void InstanceRequirementsWithMetadataRequest::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_architectureTypesHasBeenSet) {
    unsigned architectureTypesIdx = 1;
    for (auto& item : m_architectureTypes) {
      oStream << location << ".ArchitectureType." << architectureTypesIdx++ << "="
              << StringUtils::URLEncode(ArchitectureTypeMapper::GetNameForArchitectureType(item)) << "&";
    }
  }
  if (m_virtualizationTypesHasBeenSet) {
    unsigned virtualizationTypesIdx = 1;
    for (auto& item : m_virtualizationTypes) {
      oStream << location << ".VirtualizationType." << virtualizationTypesIdx++ << "="
              << StringUtils::URLEncode(VirtualizationTypeMapper::GetNameForVirtualizationType(item)) << "&";
    }
  }
  if (m_instanceRequirementsHasBeenSet) {
