...
CreationDateCondition::CreationDateCondition(const XmlNode& xmlNode) { *this = xmlNode; }
...
CreationDateCondition& CreationDateCondition::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!maximumDaysSinceCreatedNode.IsNull()) {
      m_maximumDaysSinceCreated = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maximumDaysSinceCreatedNode.GetText()).c_str()).c_str());
