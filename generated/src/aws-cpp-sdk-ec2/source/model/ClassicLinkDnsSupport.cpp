...
ClassicLinkDnsSupport::ClassicLinkDnsSupport(const XmlNode& xmlNode) { *this = xmlNode; }
...
ClassicLinkDnsSupport& ClassicLinkDnsSupport::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!classicLinkDnsSupportedNode.IsNull()) {
      m_classicLinkDnsSupported = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(classicLinkDnsSupportedNode.GetText()).c_str()).c_str());
