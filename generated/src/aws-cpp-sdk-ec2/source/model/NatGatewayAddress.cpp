...
NatGatewayAddress::NatGatewayAddress(const XmlNode& xmlNode) { *this = xmlNode; }
...
NatGatewayAddress& NatGatewayAddress::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!isPrimaryNode.IsNull()) {
      m_isPrimary =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isPrimaryNode.GetText()).c_str()).c_str());
    ...
    if (!statusNode.IsNull()) {
      m_status = NatGatewayAddressStatusMapper::GetNatGatewayAddressStatusForName(
