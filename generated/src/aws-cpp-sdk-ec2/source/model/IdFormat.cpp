...
IdFormat::IdFormat(const XmlNode& xmlNode) { *this = xmlNode; }
...
IdFormat& IdFormat::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!deadlineNode.IsNull()) {
      m_deadline = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deadlineNode.GetText()).c_str()).c_str(),
    ...
    if (!useLongIdsNode.IsNull()) {
      m_useLongIds =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(useLongIdsNode.GetText()).c_str()).c_str());
...
void IdFormat::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_deadlineHasBeenSet) {
    oStream << location << index << locationValue
            << ".Deadline=" << StringUtils::URLEncode(m_deadline.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
