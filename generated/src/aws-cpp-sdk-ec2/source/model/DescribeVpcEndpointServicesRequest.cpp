...
Aws::String DescribeVpcEndpointServicesRequest::SerializePayload() const {
  ...
  if (m_serviceNamesHasBeenSet) {
    ...
    for (auto& item : m_serviceNames) {
      ss << "ServiceName." << serviceNamesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      serviceNamesCount++;
    }
  }
  ...
  if (m_serviceRegionsHasBeenSet) {
    ...
    for (auto& item : m_serviceRegions) {
      ss << "ServiceRegion." << serviceRegionsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      serviceRegionsCount++;
    }
  }
...
void DescribeVpcEndpointServicesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
