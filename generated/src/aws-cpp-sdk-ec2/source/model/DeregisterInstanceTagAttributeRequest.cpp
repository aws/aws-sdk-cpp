...
DeregisterInstanceTagAttributeRequest::DeregisterInstanceTagAttributeRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
void DeregisterInstanceTagAttributeRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_instanceTagKeysHasBeenSet) {
    ...
    for (auto& item : m_instanceTagKeys) {
      oStream << location << index << locationValue << ".InstanceTagKey." << instanceTagKeysIdx++ << "="
