...
AttachmentEnaSrdSpecification::AttachmentEnaSrdSpecification(const XmlNode& xmlNode) { *this = xmlNode; }
...
AttachmentEnaSrdSpecification& AttachmentEnaSrdSpecification::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!enaSrdEnabledNode.IsNull()) {
      m_enaSrdEnabled =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(enaSrdEnabledNode.GetText()).c_str()).c_str());
...
void AttachmentEnaSrdSpecification::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
