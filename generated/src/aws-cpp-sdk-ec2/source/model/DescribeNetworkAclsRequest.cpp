...
Aws::String DescribeNetworkAclsRequest::SerializePayload() const {
  ...
  if (m_networkAclIdsHasBeenSet) {
    ...
    for (auto& item : m_networkAclIds) {
      ss << "NetworkAclId." << networkAclIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      networkAclIdsCount++;
    }
  }
...
void DescribeNetworkAclsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
