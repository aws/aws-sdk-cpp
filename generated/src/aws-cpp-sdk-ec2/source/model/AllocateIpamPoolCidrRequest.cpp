...
Aws::String AllocateIpamPoolCidrRequest::SerializePayload() const {
  ...
  if (m_allowedCidrsHasBeenSet) {
    ...
    for (auto& item : m_allowedCidrs) {
      ss << "AllowedCidr." << allowedCidrsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      allowedCidrsCount++;
    }
  }
  ...
  if (m_disallowedCidrsHasBeenSet) {
    ...
    for (auto& item : m_disallowedCidrs) {
      ss << "DisallowedCidr." << disallowedCidrsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void AllocateIpamPoolCidrRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
