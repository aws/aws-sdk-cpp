...
ByoipCidr::ByoipCidr(const XmlNode& xmlNode) { *this = xmlNode; }
...
ByoipCidr& ByoipCidr::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = ByoipCidrStateMapper::GetByoipCidrStateForName(
...
void ByoipCidr::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(ByoipCidrStateMapper::GetNameForByoipCidrState(m_state)) << "&";
  }
