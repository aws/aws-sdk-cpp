...
VpcCidrBlockState::VpcCidrBlockState(const XmlNode& xmlNode) { *this = xmlNode; }
...
VpcCidrBlockState& VpcCidrBlockState::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = VpcCidrBlockStateCodeMapper::GetVpcCidrBlockStateCodeForName(
...
void VpcCidrBlockState::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(VpcCidrBlockStateCodeMapper::GetNameForVpcCidrBlockStateCode(m_state)) << "&";
  }
