...
DeleteFleetSuccessItem::DeleteFleetSuccessItem(const XmlNode& xmlNode) { *this = xmlNode; }
...
DeleteFleetSuccessItem& DeleteFleetSuccessItem::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!currentFleetStateNode.IsNull()) {
      m_currentFleetState = FleetStateCodeMapper::GetFleetStateCodeForName(
    ...
    if (!previousFleetStateNode.IsNull()) {
      m_previousFleetState = FleetStateCodeMapper::GetFleetStateCodeForName(
...
void DeleteFleetSuccessItem::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_currentFleetStateHasBeenSet) {
    oStream << location << index << locationValue
            << ".CurrentFleetState=" << StringUtils::URLEncode(FleetStateCodeMapper::GetNameForFleetStateCode(m_currentFleetState)) << "&";
  }

  if (m_previousFleetStateHasBeenSet) {
    oStream << location << index << locationValue
            << ".PreviousFleetState=" << StringUtils::URLEncode(FleetStateCodeMapper::GetNameForFleetStateCode(m_previousFleetState))
            << "&";
  }
