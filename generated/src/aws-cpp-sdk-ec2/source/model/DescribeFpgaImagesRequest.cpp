...
Aws::String DescribeFpgaImagesRequest::SerializePayload() const {
  ...
  if (m_fpgaImageIdsHasBeenSet) {
    ...
    for (auto& item : m_fpgaImageIds) {
      ss << "FpgaImageId." << fpgaImageIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      fpgaImageIdsCount++;
    }
  }
  ...
  if (m_ownersHasBeenSet) {
    ...
    for (auto& item : m_owners) {
      ss << "Owner." << ownersCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      ownersCount++;
    }
  }
...
void DescribeFpgaImagesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
