...
NetworkInterfacePermissionState::NetworkInterfacePermissionState(const XmlNode& xmlNode) { *this = xmlNode; }
...
NetworkInterfacePermissionState& NetworkInterfacePermissionState::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = NetworkInterfacePermissionStateCodeMapper::GetNetworkInterfacePermissionStateCodeForName(
...
void NetworkInterfacePermissionState::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                     const char* locationValue) const {
  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue << ".State="
            << StringUtils::URLEncode(NetworkInterfacePermissionStateCodeMapper::GetNameForNetworkInterfacePermissionStateCode(m_state))
            << "&";
  }
