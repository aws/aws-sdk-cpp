...
VpnConnectionOptionsSpecification::VpnConnectionOptionsSpecification(const XmlNode& xmlNode) { *this = xmlNode; }
...
VpnConnectionOptionsSpecification& VpnConnectionOptionsSpecification::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!enableAccelerationNode.IsNull()) {
      m_enableAcceleration = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(enableAccelerationNode.GetText()).c_str()).c_str());
    ...
    if (!tunnelInsideIpVersionNode.IsNull()) {
      m_tunnelInsideIpVersion = TunnelInsideIpVersionMapper::GetTunnelInsideIpVersionForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(tunnelInsideIpVersionNode.GetText()).c_str()));
    ...
    if (!staticRoutesOnlyNode.IsNull()) {
      m_staticRoutesOnly = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(staticRoutesOnlyNode.GetText()).c_str()).c_str());
...
void VpnConnectionOptionsSpecification::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_tunnelInsideIpVersionHasBeenSet) {
    oStream << location << index << locationValue << ".TunnelInsideIpVersion="
            << StringUtils::URLEncode(TunnelInsideIpVersionMapper::GetNameForTunnelInsideIpVersion(m_tunnelInsideIpVersion)) << "&";
  }
...
void VpnConnectionOptionsSpecification::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_tunnelInsideIpVersionHasBeenSet) {
    oStream << location << ".TunnelInsideIpVersion="
            << StringUtils::URLEncode(TunnelInsideIpVersionMapper::GetNameForTunnelInsideIpVersion(m_tunnelInsideIpVersion)) << "&";
  }
  if (m_tunnelOptionsHasBeenSet) {
    unsigned tunnelOptionsIdx = 1;
