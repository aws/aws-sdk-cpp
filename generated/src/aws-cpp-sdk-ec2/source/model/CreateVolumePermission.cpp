...
CreateVolumePermission::CreateVolumePermission(const XmlNode& xmlNode) { *this = xmlNode; }
...
CreateVolumePermission& CreateVolumePermission::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!groupNode.IsNull()) {
      m_group = PermissionGroupMapper::GetPermissionGroupForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(groupNode.GetText()).c_str()));
...
void CreateVolumePermission::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_groupHasBeenSet) {
    oStream << location << index << locationValue
            << ".Group=" << StringUtils::URLEncode(PermissionGroupMapper::GetNameForPermissionGroup(m_group)) << "&";
  }
}
