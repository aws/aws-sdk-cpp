...
Aws::String DescribeInstanceEventWindowsRequest::SerializePayload() const {
  ...
  if (m_instanceEventWindowIdsHasBeenSet) {
    ...
    for (auto& item : m_instanceEventWindowIds) {
      ss << "InstanceEventWindowId." << instanceEventWindowIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void DescribeInstanceEventWindowsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
