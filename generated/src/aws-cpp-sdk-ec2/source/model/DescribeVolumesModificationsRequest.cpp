...
Aws::String DescribeVolumesModificationsRequest::SerializePayload() const {
  ...
  if (m_volumeIdsHasBeenSet) {
    ...
    for (auto& item : m_volumeIds) {
      ss << "VolumeId." << volumeIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      volumeIdsCount++;
    }
  }
...
void DescribeVolumesModificationsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
