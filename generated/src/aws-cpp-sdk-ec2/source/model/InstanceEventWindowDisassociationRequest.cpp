...
InstanceEventWindowDisassociationRequest::InstanceEventWindowDisassociationRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
void InstanceEventWindowDisassociationRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_dedicatedHostIdsHasBeenSet) {
    ...
    for (auto& item : m_dedicatedHostIds) {
      oStream << location << index << locationValue << ".DedicatedHostId." << dedicatedHostIdsIdx++ << "="
