...
InitializationStatusDetails::InitializationStatusDetails(const XmlNode& xmlNode) { *this = xmlNode; }
...
InitializationStatusDetails& InitializationStatusDetails::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!initializationTypeNode.IsNull()) {
      m_initializationType = InitializationTypeMapper::GetInitializationTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(initializationTypeNode.GetText()).c_str()));
    ...
    if (!estimatedTimeToCompleteInSecondsNode.IsNull()) {
      m_estimatedTimeToCompleteInSeconds = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(estimatedTimeToCompleteInSecondsNode.GetText()).c_str()).c_str());
...
void InitializationStatusDetails::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                 const char* locationValue) const {
  if (m_initializationTypeHasBeenSet) {
    oStream << location << index << locationValue << ".InitializationType="
            << StringUtils::URLEncode(InitializationTypeMapper::GetNameForInitializationType(m_initializationType)) << "&";
  }
...
void InitializationStatusDetails::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_initializationTypeHasBeenSet) {
    oStream << location << ".InitializationType="
            << StringUtils::URLEncode(InitializationTypeMapper::GetNameForInitializationType(m_initializationType)) << "&";
  }
  if (m_progressHasBeenSet) {
