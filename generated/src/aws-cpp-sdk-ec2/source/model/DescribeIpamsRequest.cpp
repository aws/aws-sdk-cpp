...
Aws::String DescribeIpamsRequest::SerializePayload() const {
  ...
  if (m_ipamIdsHasBeenSet) {
    ...
    for (auto& item : m_ipamIds) {
      ss << "IpamId." << ipamIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      ipamIdsCount++;
    }
  }
...
void DescribeIpamsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
