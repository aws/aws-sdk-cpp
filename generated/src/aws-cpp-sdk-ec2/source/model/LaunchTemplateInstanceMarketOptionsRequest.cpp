...
LaunchTemplateInstanceMarketOptionsRequest::LaunchTemplateInstanceMarketOptionsRequest(const XmlNode& xmlNode) { *this = xmlNode; }
...
LaunchTemplateInstanceMarketOptionsRequest& LaunchTemplateInstanceMarketOptionsRequest::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!marketTypeNode.IsNull()) {
      m_marketType = MarketTypeMapper::GetMarketTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(marketTypeNode.GetText()).c_str()));
...
void LaunchTemplateInstanceMarketOptionsRequest::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                                const char* locationValue) const {
  if (m_marketTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".MarketType=" << StringUtils::URLEncode(MarketTypeMapper::GetNameForMarketType(m_marketType)) << "&";
  }
