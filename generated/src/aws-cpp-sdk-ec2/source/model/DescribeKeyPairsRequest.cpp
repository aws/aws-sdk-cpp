...
Aws::String DescribeKeyPairsRequest::SerializePayload() const {
  ...
  if (m_keyNamesHasBeenSet) {
    ...
    for (auto& item : m_keyNames) {
      ss << "KeyName." << keyNamesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      keyNamesCount++;
    }
  }
  ...
  if (m_keyPairIdsHasBeenSet) {
    ...
    for (auto& item : m_keyPairIds) {
      ss << "KeyPairId." << keyPairIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
      keyPairIdsCount++;
    }
  }
...
void DescribeKeyPairsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
