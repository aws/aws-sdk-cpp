...
class NitroTpmInfo {
 public:
  ...
  void SetSupportedVersions(SupportedVersionsT&& value) {
    m_supportedVersionsHasBeenSet = true;
    m_supportedVersions = std::forward<SupportedVersionsT>(value);
  }
  template <typename SupportedVersionsT = Aws::Vector<Aws::String>>
  NitroTpmInfo& WithSupportedVersions(SupportedVersionsT&& value) {
    SetSupportedVersions(std::forward<SupportedVersionsT>(value));
    return *this;
  }
  template <typename SupportedVersionsT = Aws::String>
  NitroTpmInfo& AddSupportedVersions(SupportedVersionsT&& value) {
    m_supportedVersionsHasBeenSet = true;
    m_supportedVersions.emplace_back(std::forward<SupportedVersionsT>(value));
    return *this;
  }
  ///@}
 private:
