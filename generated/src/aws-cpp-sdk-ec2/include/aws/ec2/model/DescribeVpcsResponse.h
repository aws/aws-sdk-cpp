...
class DescribeVpcsResponse {
 public:
  ...
  template <typename NextTokenT = Aws::String>
  DescribeVpcsResponse& WithNextToken(NextTokenT&& value) {
  ...
  template <typename VpcsT = Aws::Vector<Vpc>>
  DescribeVpcsResponse& WithVpcs(VpcsT&& value) {
    SetVpcs(std::forward<VpcsT>(value));
    return *this;
  }
  template <typename VpcsT = Vpc>
  DescribeVpcsResponse& AddVpcs(VpcsT&& value) {
    m_vpcsHasBeenSet = true;
  ...
  template <typename ResponseMetadataT = ResponseMetadata>
  DescribeVpcsResponse& WithResponseMetadata(ResponseMetadataT&& value) {
