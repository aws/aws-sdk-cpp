...
class IpamPublicAddressTags {
 public:
  ...
  void SetEipTags(EipTagsT&& value) {
    m_eipTagsHasBeenSet = true;
    m_eipTags = std::forward<EipTagsT>(value);
  }
  template <typename EipTagsT = Aws::Vector<IpamPublicAddressTag>>
  IpamPublicAddressTags& WithEipTags(EipTagsT&& value) {
    SetEipTags(std::forward<EipTagsT>(value));
    return *this;
  }
  template <typename EipTagsT = IpamPublicAddressTag>
  IpamPublicAddressTags& AddEipTags(EipTagsT&& value) {
    m_eipTagsHasBeenSet = true;
    m_eipTags.emplace_back(std::forward<EipTagsT>(value));
    return *this;
  }
  ///@}
 private:
