...
class FleetSpotMaintenanceStrategiesRequest {
 public:
  ...
  void SetCapacityRebalance(CapacityRebalanceT&& value) {
    m_capacityRebalanceHasBeenSet = true;
    m_capacityRebalance = std::forward<CapacityRebalanceT>(value);
  }
  template <typename CapacityRebalanceT = FleetSpotCapacityRebalanceRequest>
  FleetSpotMaintenanceStrategiesRequest& WithCapacityRebalance(CapacityRebalanceT&& value) {
    SetCapacityRebalance(std::forward<CapacityRebalanceT>(value));
    return *this;
  }
  ///@}
 private:
