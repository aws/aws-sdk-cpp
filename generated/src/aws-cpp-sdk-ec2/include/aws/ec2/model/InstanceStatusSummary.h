...
class InstanceStatusSummary {
 public:
  ...
  template <typename DetailsT = Aws::Vector<InstanceStatusDetails>>
  InstanceStatusSummary& WithDetails(DetailsT&& value) {
    SetDetails(std::forward<DetailsT>(value));
    return *this;
  }
  template <typename DetailsT = InstanceStatusDetails>
  InstanceStatusSummary& AddDetails(DetailsT&& value) {
    m_detailsHasBeenSet = true;
  ...
  inline InstanceStatusSummary& WithStatus(SummaryStatus value) {
    SetStatus(value);
    return *this;
  }
  ///@}
