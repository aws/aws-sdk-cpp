...
namespace Panorama {
template <>
AWS_PANORAMA_API ConflictException PanoramaError::GetModeledError() {
  assert(this->GetErrorType() == PanoramaErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_PANORAMA_API ServiceQuotaExceededException PanoramaError::GetModeledError() {
  assert(this->GetErrorType() == PanoramaErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_PANORAMA_API InternalServerException PanoramaError::GetModeledError() {
  assert(this->GetErrorType() == PanoramaErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_PANORAMA_API ResourceNotFoundException PanoramaError::GetModeledError() {
  assert(this->GetErrorType() == PanoramaErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_PANORAMA_API ValidationException PanoramaError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(PanoramaErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
  }
