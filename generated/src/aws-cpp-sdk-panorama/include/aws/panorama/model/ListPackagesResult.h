...
class ListPackagesResult {
 public:
  ...
  void SetPackages(PackagesT&& value) {
    ...
    m_packages = std::forward<PackagesT>(value);
  }
  template <typename PackagesT = Aws::Vector<PackageListItem>>
  ListPackagesResult& WithPackages(PackagesT&& value) {
    SetPackages(std::forward<PackagesT>(value));
    return *this;
  }
  template <typename PackagesT = PackageListItem>
  ListPackagesResult& AddPackages(PackagesT&& value) {
    m_packagesHasBeenSet = true;
    m_packages.emplace_back(std::forward<PackagesT>(value));
    return *this;
  }
  ///@}
