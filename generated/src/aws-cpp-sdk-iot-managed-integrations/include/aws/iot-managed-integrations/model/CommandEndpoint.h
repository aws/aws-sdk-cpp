...
class CommandEndpoint {
 public:
  ...
  template <typename EndpointIdT = Aws::String>
  CommandEndpoint& WithEndpointId(EndpointIdT&& value) {
  ...
  template <typename CapabilitiesT = Aws::Vector<CommandCapability>>
  CommandEndpoint& WithCapabilities(CapabilitiesT&& value) {
    SetCapabilities(std::forward<CapabilitiesT>(value));
    return *this;
  }
  template <typename CapabilitiesT = CommandCapability>
  CommandEndpoint& AddCapabilities(CapabilitiesT&& value) {
    m_capabilitiesHasBeenSet = true;
