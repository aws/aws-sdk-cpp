...
class ListCloudConnectorsResult {
 public:
  ...
  AWS_IOTMANAGEDINTEGRATIONS_API ListCloudConnectorsResult& operator=(
  ...
  template <typename ItemsT = Aws::Vector<ConnectorItem>>
  ListCloudConnectorsResult& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = ConnectorItem>
  ListCloudConnectorsResult& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
