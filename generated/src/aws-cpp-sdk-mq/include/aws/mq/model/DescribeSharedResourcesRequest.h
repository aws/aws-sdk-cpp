...
/**
 */
class DescribeSharedResourcesRequest : public MQRequest {
 public:
  AWS_MQ_API DescribeSharedResourcesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DescribeSharedResources"; }
  ...
  template <typename BrokerIdT = Aws::String>
  DescribeSharedResourcesRequest& WithBrokerId(BrokerIdT&& value) {
  ...
  ///@{
  /**
   * <p>The maximum number of resources that Amazon MQ can return per page (20 by
  ...
  inline DescribeSharedResourcesRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  template <typename NextTokenT = Aws::String>
  DescribeSharedResourcesRequest& WithNextToken(NextTokenT&& value) {
