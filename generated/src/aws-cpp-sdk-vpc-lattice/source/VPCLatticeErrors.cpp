...
template <>
AWS_VPCLATTICE_API ConflictException VPCLatticeError::GetModeledError() {
  assert(this->GetErrorType() == VPCLatticeErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_VPCLATTICE_API ThrottlingException VPCLatticeError::GetModeledError() {
  assert(this->GetErrorType() == VPCLatticeErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_VPCLATTICE_API ServiceQuotaExceededException VPCLatticeError::GetModeledError() {
  assert(this->GetErrorType() == VPCLatticeErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_VPCLATTICE_API ResourceNotFoundException VPCLatticeError::GetModeledError() {
  assert(this->GetErrorType() == VPCLatticeErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_VPCLATTICE_API InternalServerException VPCLatticeError::GetModeledError() {
  assert(this->GetErrorType() == VPCLatticeErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_VPCLATTICE_API ValidationException VPCLatticeError::GetModeledError() {
