...
class HttpMatch {
 public:
  ...
  void SetPathMatch(PathMatchT&& value) {
    ...
    m_pathMatch = std::forward<PathMatchT>(value);
  }
  template <typename PathMatchT = PathMatch>
  HttpMatch& WithPathMatch(PathMatchT&& value) {
    SetPathMatch(std::forward<PathMatchT>(value));
    return *this;
  }
  ///@}
  ...
  void SetHeaderMatches(HeaderMatchesT&& value) {
    ...
    m_headerMatches = std::forward<HeaderMatchesT>(value);
  }
  template <typename HeaderMatchesT = Aws::Vector<HeaderMatch>>
  HttpMatch& WithHeaderMatches(HeaderMatchesT&& value) {
    SetHeaderMatches(std::forward<HeaderMatchesT>(value));
    return *this;
  }
  template <typename HeaderMatchesT = HeaderMatch>
  HttpMatch& AddHeaderMatches(HeaderMatchesT&& value) {
    m_headerMatchesHasBeenSet = true;
    m_headerMatches.emplace_back(std::forward<HeaderMatchesT>(value));
    return *this;
  }
  ///@}
 private:
