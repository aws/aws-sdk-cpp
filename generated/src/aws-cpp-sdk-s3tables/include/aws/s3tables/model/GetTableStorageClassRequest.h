...
/**
 */
class GetTableStorageClassRequest : public S3TablesRequest {
 public:
  AWS_S3TABLES_API GetTableStorageClassRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetTableStorageClass"; }
  ...
  template <typename TableBucketARNT = Aws::String>
  GetTableStorageClassRequest& WithTableBucketARN(TableBucketARNT&& value) {
  ...
  template <typename NamespaceT = Aws::String>
  GetTableStorageClassRequest& WithNamespace(NamespaceT&& value) {
  ...
  template <typename NameT = Aws::String>
  GetTableStorageClassRequest& WithName(NameT&& value) {
