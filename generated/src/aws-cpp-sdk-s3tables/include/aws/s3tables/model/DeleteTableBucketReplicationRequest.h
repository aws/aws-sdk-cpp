...
/**
 */
class DeleteTableBucketReplicationRequest : public S3TablesRequest {
 public:
  AWS_S3TABLES_API DeleteTableBucketReplicationRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteTableBucketReplication"; }
  ...
  template <typename TableBucketARNT = Aws::String>
  DeleteTableBucketReplicationRequest& WithTableBucketARN(TableBucketARNT&& value) {
   ...
   * token to ensure you're deleting the expected version of the configuration.</p>
   */
  ...
  template <typename VersionTokenT = Aws::String>
  DeleteTableBucketReplicationRequest& WithVersionToken(VersionTokenT&& value) {
