...
/**
 */
class PutTableBucketReplicationRequest : public S3TablesRequest {
 public:
  AWS_S3TABLES_API PutTableBucketReplicationRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "PutTableBucketReplication"; }
  ...
  template <typename TableBucketARNT = Aws::String>
  PutTableBucketReplicationRequest& WithTableBucketARN(TableBucketARNT&& value) {
  ...
  ///@{
  /**
   * <p>A version token from a previous GetTableBucketReplication call. Use this
   * token to ensure you're updating the expected version of the configuration.</p>
   */
  ...
  template <typename VersionTokenT = Aws::String>
  PutTableBucketReplicationRequest& WithVersionToken(VersionTokenT&& value) {
  ...
  ///@{
  /**
   * <p>The replication configuration to apply, including the IAM role and
   * replication rules.</p>
   */
  ...
  template <typename ConfigurationT = TableBucketReplicationConfiguration>
  PutTableBucketReplicationRequest& WithConfiguration(ConfigurationT&& value) {
