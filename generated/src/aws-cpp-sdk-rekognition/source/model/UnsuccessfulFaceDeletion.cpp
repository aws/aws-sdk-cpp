...
UnsuccessfulFaceDeletion::UnsuccessfulFaceDeletion(JsonView jsonValue) { *this = jsonValue; }
...
UnsuccessfulFaceDeletion& UnsuccessfulFaceDeletion::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("Reasons")) {
    ...
    for (unsigned reasonsIndex = 0; reasonsIndex < reasonsJsonList.GetLength(); ++reasonsIndex) {
      m_reasons.push_back(
          UnsuccessfulFaceDeletionReasonMapper::GetUnsuccessfulFaceDeletionReasonForName(reasonsJsonList[reasonsIndex].AsString()));
    }
...
JsonValue UnsuccessfulFaceDeletion::Jsonize() const {
  ...
  if (m_reasonsHasBeenSet) {
    ...
    for (unsigned reasonsIndex = 0; reasonsIndex < reasonsJsonList.GetLength(); ++reasonsIndex) {
      reasonsJsonList[reasonsIndex].AsString(
          UnsuccessfulFaceDeletionReasonMapper::GetNameForUnsuccessfulFaceDeletionReason(m_reasons[reasonsIndex]));
    }
