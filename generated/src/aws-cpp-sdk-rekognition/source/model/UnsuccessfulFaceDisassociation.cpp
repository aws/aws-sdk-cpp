...
UnsuccessfulFaceDisassociation::UnsuccessfulFaceDisassociation(JsonView jsonValue) { *this = jsonValue; }
...
UnsuccessfulFaceDisassociation& UnsuccessfulFaceDisassociation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("Reasons")) {
    ...
    for (unsigned reasonsIndex = 0; reasonsIndex < reasonsJsonList.GetLength(); ++reasonsIndex) {
      m_reasons.push_back(UnsuccessfulFaceDisassociationReasonMapper::GetUnsuccessfulFaceDisassociationReasonForName(
...
JsonValue UnsuccessfulFaceDisassociation::Jsonize() const {
  ...
  if (m_reasonsHasBeenSet) {
    ...
    for (unsigned reasonsIndex = 0; reasonsIndex < reasonsJsonList.GetLength(); ++reasonsIndex) {
      reasonsJsonList[reasonsIndex].AsString(
          UnsuccessfulFaceDisassociationReasonMapper::GetNameForUnsuccessfulFaceDisassociationReason(m_reasons[reasonsIndex]));
    }
