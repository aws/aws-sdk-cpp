...
DeleteFacesResult::DeleteFacesResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
DeleteFacesResult& DeleteFacesResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("UnsuccessfulFaceDeletions")) {
    ...
    for (unsigned unsuccessfulFaceDeletionsIndex = 0; unsuccessfulFaceDeletionsIndex < unsuccessfulFaceDeletionsJsonList.GetLength();
         ++unsuccessfulFaceDeletionsIndex) {
