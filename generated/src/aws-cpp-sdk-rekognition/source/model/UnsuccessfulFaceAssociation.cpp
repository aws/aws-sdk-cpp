...
UnsuccessfulFaceAssociation::UnsuccessfulFaceAssociation(JsonView jsonValue) { *this = jsonValue; }
...
UnsuccessfulFaceAssociation& UnsuccessfulFaceAssociation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("Reasons")) {
    ...
    for (unsigned reasonsIndex = 0; reasonsIndex < reasonsJsonList.GetLength(); ++reasonsIndex) {
      m_reasons.push_back(
          UnsuccessfulFaceAssociationReasonMapper::GetUnsuccessfulFaceAssociationReasonForName(reasonsJsonList[reasonsIndex].AsString()));
    }
  ...
  if (m_reasonsHasBeenSet) {
    ...
    for (unsigned reasonsIndex = 0; reasonsIndex < reasonsJsonList.GetLength(); ++reasonsIndex) {
      reasonsJsonList[reasonsIndex].AsString(
          UnsuccessfulFaceAssociationReasonMapper::GetNameForUnsuccessfulFaceAssociationReason(m_reasons[reasonsIndex]));
    }
