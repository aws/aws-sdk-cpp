...
ProtectiveEquipmentSummarizationAttributes::ProtectiveEquipmentSummarizationAttributes(JsonView jsonValue) { *this = jsonValue; }
...
ProtectiveEquipmentSummarizationAttributes& ProtectiveEquipmentSummarizationAttributes::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("RequiredEquipmentTypes")) {
    ...
    for (unsigned requiredEquipmentTypesIndex = 0; requiredEquipmentTypesIndex < requiredEquipmentTypesJsonList.GetLength();
         ++requiredEquipmentTypesIndex) {
      m_requiredEquipmentTypes.push_back(ProtectiveEquipmentTypeMapper::GetProtectiveEquipmentTypeForName(
          requiredEquipmentTypesJsonList[requiredEquipmentTypesIndex].AsString()));
    }
...
JsonValue ProtectiveEquipmentSummarizationAttributes::Jsonize() const {
  ...
  if (m_requiredEquipmentTypesHasBeenSet) {
    ...
    for (unsigned requiredEquipmentTypesIndex = 0; requiredEquipmentTypesIndex < requiredEquipmentTypesJsonList.GetLength();
         ++requiredEquipmentTypesIndex) {
      requiredEquipmentTypesJsonList[requiredEquipmentTypesIndex].AsString(
          ProtectiveEquipmentTypeMapper::GetNameForProtectiveEquipmentType(m_requiredEquipmentTypes[requiredEquipmentTypesIndex]));
    }
