...
class AWS_KINESISVIDEOARCHIVEDMEDIA_API KinesisVideoArchivedMediaClient
 ...
 public:
   ...
   */
  KinesisVideoArchivedMediaClient(const Aws::KinesisVideoArchivedMedia::KinesisVideoArchivedMediaClientConfiguration& clientConfiguration =
  ...
  KinesisVideoArchivedMediaClient(const Aws::Auth::AWSCredentials& credentials,
                                  std::shared_ptr<KinesisVideoArchivedMediaEndpointProviderBase> endpointProvider = nullptr,
                                  const Aws::KinesisVideoArchivedMedia::KinesisVideoArchivedMediaClientConfiguration& clientConfiguration =
                                      Aws::KinesisVideoArchivedMedia::KinesisVideoArchivedMediaClientConfiguration());
   ...
   * Video Streams Pricing</a> and <a href="https://aws.amazon.com/pricing/"> Amazon
   ...
   */
  template <typename GetClipRequestT = Model::GetClipRequest>
  Model::GetClipOutcomeCallable GetClipCallable(const GetClipRequestT& request) const {
    return SubmitCallable(&KinesisVideoArchivedMediaClient::GetClip, request);
  }

  /**
   * An Async wrapper for GetClip that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename GetClipRequestT = Model::GetClipRequest>
  void GetClipAsync(const GetClipRequestT& request, const GetClipResponseReceivedHandler& handler,
                    const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
    return SubmitAsync(&KinesisVideoArchivedMediaClient::GetClip, request, handler, context);
  }
   ...
   * Services credentials.</p>  <p>The media that is made available through
   * the manifest consists only of the requested stream, time range, and format. No
   * other media data (such as frames outside the requested window or alternate
   * bitrates) is made available.</p> </li> <li> <p>Provide the URL (containing the
   * encrypted session token) for the MPEG-DASH manifest to a media player that
   ...
   * fragments contain encoded video frames or encoded audio samples.</p> </li> <li>
   ...
   * following actions:</p> <ul> <li> <p> <b>GetDASHManifest:</b> Retrieves an MPEG
   * DASH manifest, which contains the metadata for the media that you want to
   ...
   * href="https://aws.amazon.com/kinesis/video-streams/pricing/">Amazon Kinesis
   * Video Streams Pricing</a> and <a href="https://aws.amazon.com/pricing/">Amazon
   * Web Services Pricing</a>. Charges for both HLS sessions and outgoing Amazon Web
   * Services data apply.</p> <p>For more information about HLS, see <a
   ...
   * header – if you want to report an issue to Amazon Web Services the support team
   * can better diagnose the problem if given the Request Id.</p> </li> </ul> <p>Both
   * the HTTP status code and the ErrorType header can be utilized to make
   * programmatic decisions about whether errors are retry-able and under what
   * conditions, as well as provide information on what actions the client programmer
   * might need to take in order to successfully try again.</p> <p>For more
   * information, see the <b>Errors</b> section at the bottom of this topic, as well
   * as <a
   ...
   * API Reference</a></p>
   */
  virtual Model::GetDASHStreamingSessionURLOutcome GetDASHStreamingSessionURL(
      const Model::GetDASHStreamingSessionURLRequest& request = {}) const;

  /**
   * A Callable wrapper for GetDASHStreamingSessionURL that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename GetDASHStreamingSessionURLRequestT = Model::GetDASHStreamingSessionURLRequest>
  Model::GetDASHStreamingSessionURLOutcomeCallable GetDASHStreamingSessionURLCallable(
      const GetDASHStreamingSessionURLRequestT& request = {}) const {
    return SubmitCallable(&KinesisVideoArchivedMediaClient::GetDASHStreamingSessionURL, request);
  }

  /**
   * An Async wrapper for GetDASHStreamingSessionURL that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename GetDASHStreamingSessionURLRequestT = Model::GetDASHStreamingSessionURLRequest>
  void GetDASHStreamingSessionURLAsync(const GetDASHStreamingSessionURLResponseReceivedHandler& handler,
   ...
   * Kinesis video stream has the following requirements for providing data through
   * HLS:</p> <ul> <li> <p>For streaming video, the media must contain H.264 or H.265
   * encoded video and, optionally, AAC encoded audio. Specifically, the codec ID of
   * track 1 should be <code>V_MPEG/ISO/AVC</code> (for H.264) or
   * <code>V_MPEG/ISO/HEVC</code> (for H.265). Optionally, the codec ID of track 2
   * should be <code>A_AAC</code>. For audio only streaming, the codec ID of track 1
   * should be <code>A_AAC</code>.</p> </li> <li> <p>Data retention must be greater
   * than 0.</p> </li> <li> <p>The video track of each fragment must contain codec
   * private data in the Advanced Video Coding (AVC) for H.264 format or HEVC for
   * H.265 format (<a href="https://www.iso.org/standard/55980.html">MPEG-4
   ...
   * the same measures that you would use with your Amazon Web Services
   * credentials.</p>  <p>The media that is made available through the
   * playlist consists only of the requested stream, time range, and format. No other
   ...
   * initialization fragment contains the codec private data for the stream, and
   * other data needed to set up the video or audio decoder and renderer. The media
   * fragments contain H.264-encoded video frames or AAC-encoded audio samples.</p>
   ...
   * calls the following actions:</p> <ul> <li> <p> <b>GetHLSMasterPlaylist:</b>
   * Retrieves an HLS master playlist, which contains a URL for the
   * <code>GetHLSMediaPlaylist</code> action for each track, and additional metadata
   * for the media player, including estimated bitrate and resolution.</p> </li> <li>
   ...
   * playlist for the video track and the audio track (if applicable) that contains
   * MP4 media URLs for the specific track. </p> </li> <li> <p>
   ...
   * fragment's media frames and their timestamps. </p>  <p>For the HLS
   ...
   * fragments can contain CPD changes for each track. Therefore, the fragments in a
   * session can have a different resolution, bit rate, or other information in the
   * CPD without interrupting playback. However, any change made in the track number
   * or track codec format can return an error when those different media fragments
   * are loaded. For example, streaming will fail if the fragments in the stream
   * change from having only video to having both audio and video, or if an AAC audio
   * track is changed to an ALAW audio track. For each streaming session, only 500
   * CPD changes are allowed.</p>  <p>Data retrieved with this action is
   * billable. For information, see <a
   * href="https://aws.amazon.com/kinesis/video-streams/pricing/">Pricing</a>.</p>
   * </li> <li> <p> <b>GetTSFragment:</b> Retrieves MPEG TS fragments containing both
   * initialization and media data for all tracks in the stream.</p>  <p>If the
   * <code>ContainerFormat</code> is <code>MPEG_TS</code>, this API is used instead
   * of <code>GetMP4InitFragment</code> and <code>GetMP4MediaFragment</code> to
   * retrieve stream media.</p>  <p>Data retrieved with this action is
   ...
   * API Reference</a></p>
   */
  virtual Model::GetHLSStreamingSessionURLOutcome GetHLSStreamingSessionURL(
      const Model::GetHLSStreamingSessionURLRequest& request = {}) const;

  /**
   * A Callable wrapper for GetHLSStreamingSessionURL that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename GetHLSStreamingSessionURLRequestT = Model::GetHLSStreamingSessionURLRequest>
  Model::GetHLSStreamingSessionURLOutcomeCallable GetHLSStreamingSessionURLCallable(
      const GetHLSStreamingSessionURLRequestT& request = {}) const {
    return SubmitCallable(&KinesisVideoArchivedMediaClient::GetHLSStreamingSessionURL, request);
  }

  /**
   * An Async wrapper for GetHLSStreamingSessionURL that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename GetHLSStreamingSessionURLRequestT = Model::GetHLSStreamingSessionURLRequest>
  void GetHLSStreamingSessionURLAsync(const GetHLSStreamingSessionURLResponseReceivedHandler& handler,
                                      const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                      const GetHLSStreamingSessionURLRequestT& request = {}) const {
   ...
   */
  template <typename GetImagesRequestT = Model::GetImagesRequest>
  Model::GetImagesOutcomeCallable GetImagesCallable(const GetImagesRequestT& request) const {
    return SubmitCallable(&KinesisVideoArchivedMediaClient::GetImages, request);
  }

  /**
   * An Async wrapper for GetImages that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename GetImagesRequestT = Model::GetImagesRequest>
  void GetImagesAsync(const GetImagesRequestT& request, const GetImagesResponseReceivedHandler& handler,
                      const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
    return SubmitAsync(&KinesisVideoArchivedMediaClient::GetImages, request, handler, context);
  }
   ...
   * want to report an issue to Amazon Web Services, the support team can better
  ...
  /**
   * A Callable wrapper for GetMediaForFragmentList that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename GetMediaForFragmentListRequestT = Model::GetMediaForFragmentListRequest>
  Model::GetMediaForFragmentListOutcomeCallable GetMediaForFragmentListCallable(const GetMediaForFragmentListRequestT& request) const {
    return SubmitCallable(&KinesisVideoArchivedMediaClient::GetMediaForFragmentList, request);
  }

  /**
   * An Async wrapper for GetMediaForFragmentList that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename GetMediaForFragmentListRequestT = Model::GetMediaForFragmentListRequest>
  void GetMediaForFragmentListAsync(const GetMediaForFragmentListRequestT& request,
                                    const GetMediaForFragmentListResponseReceivedHandler& handler,
                                    const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
    return SubmitAsync(&KinesisVideoArchivedMediaClient::GetMediaForFragmentList, request, handler, context);
  }
   ...
   * problem if given the Request Id.</p> </li> </ul> <p>Both the HTTP status code
   * and the ErrorType header can be utilized to make programmatic decisions about
   * whether errors are retry-able and under what conditions, as well as provide
  ...
  /**
   * An Async wrapper for ListFragments that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListFragmentsRequestT = Model::ListFragmentsRequest>
  void ListFragmentsAsync(const ListFragmentsResponseReceivedHandler& handler,
                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                          const ListFragmentsRequestT& request = {}) const {
