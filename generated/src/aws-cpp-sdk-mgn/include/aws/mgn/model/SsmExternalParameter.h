...
class SsmExternalParameter {
 public:
  ...
  void SetDynamicPath(DynamicPathT&& value) {
    m_dynamicPathHasBeenSet = true;
    m_dynamicPath = std::forward<DynamicPathT>(value);
  }
  template <typename DynamicPathT = Aws::String>
  SsmExternalParameter& WithDynamicPath(DynamicPathT&& value) {
    SetDynamicPath(std::forward<DynamicPathT>(value));
    return *this;
  }
  ///@}
 private:
