...
class Remediation {
 public:
  ...
  void SetSuggestedFixes(SuggestedFixesT&& value) {
    ...
    m_suggestedFixes = std::forward<SuggestedFixesT>(value);
  }
  template <typename SuggestedFixesT = Aws::Vector<SuggestedFix>>
  Remediation& WithSuggestedFixes(SuggestedFixesT&& value) {
    SetSuggestedFixes(std::forward<SuggestedFixesT>(value));
    return *this;
  }
  template <typename SuggestedFixesT = SuggestedFix>
  Remediation& AddSuggestedFixes(SuggestedFixesT&& value) {
    m_suggestedFixesHasBeenSet = true;
    m_suggestedFixes.emplace_back(std::forward<SuggestedFixesT>(value));
    return *this;
  }
  ///@}
 private:
