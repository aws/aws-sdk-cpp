...
class FormatOptions {
 public:
  ...
  void SetTsvOptions(TsvOptionsT&& value) {
    ...
    m_tsvOptions = std::forward<TsvOptionsT>(value);
  }
  template <typename TsvOptionsT = TsvOptions>
  FormatOptions& WithTsvOptions(TsvOptionsT&& value) {
    SetTsvOptions(std::forward<TsvOptionsT>(value));
    return *this;
  }
  ///@}
  ...
  void SetVcfOptions(VcfOptionsT&& value) {
    ...
    m_vcfOptions = std::forward<VcfOptionsT>(value);
  }
  template <typename VcfOptionsT = VcfOptions>
  FormatOptions& WithVcfOptions(VcfOptionsT&& value) {
    SetVcfOptions(std::forward<VcfOptionsT>(value));
    return *this;
  }
  ///@}
 private:
