...
class BatchDeleteReadSetResult {
 public:
  ...
  template <typename ErrorsT = Aws::Vector<ReadSetBatchError>>
  BatchDeleteReadSetResult& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = ReadSetBatchError>
  BatchDeleteReadSetResult& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchDeleteReadSetResult& WithRequestId(RequestIdT&& value) {
