...
class ListAnnotationImportJobsFilter {
 public:
  ...
  inline ListAnnotationImportJobsFilter& WithStatus(JobStatus value) {
    SetStatus(value);
    return *this;
  }
  ///@}
  ...
  void SetStoreName(StoreNameT&& value) {
    m_storeNameHasBeenSet = true;
    m_storeName = std::forward<StoreNameT>(value);
  }
  template <typename StoreNameT = Aws::String>
  ListAnnotationImportJobsFilter& WithStoreName(StoreNameT&& value) {
    SetStoreName(std::forward<StoreNameT>(value));
    return *this;
  }
  ///@}
 private:
