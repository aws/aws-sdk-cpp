...
class MetricsExportConfig {
 public:
  ...
  void SetMqttTopic(MqttTopicT&& value) {
    m_mqttTopicHasBeenSet = true;
    m_mqttTopic = std::forward<MqttTopicT>(value);
  }
  template <typename MqttTopicT = Aws::String>
  MetricsExportConfig& WithMqttTopic(MqttTopicT&& value) {
    SetMqttTopic(std::forward<MqttTopicT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RoleArnT = Aws::String>
  MetricsExportConfig& WithRoleArn(RoleArnT&& value) {
