...
class Mqtt5Configuration {
 public:
  ...
  void SetPropagatingAttributes(PropagatingAttributesT&& value) {
    ...
    m_propagatingAttributes = std::forward<PropagatingAttributesT>(value);
  }
  template <typename PropagatingAttributesT = Aws::Vector<PropagatingAttribute>>
  Mqtt5Configuration& WithPropagatingAttributes(PropagatingAttributesT&& value) {
    SetPropagatingAttributes(std::forward<PropagatingAttributesT>(value));
    return *this;
  }
  ...
  Mqtt5Configuration& AddPropagatingAttributes(PropagatingAttributesT&& value) {
    m_propagatingAttributesHasBeenSet = true;
    m_propagatingAttributes.emplace_back(std::forward<PropagatingAttributesT>(value));
    return *this;
  }
  ///@}
 private:
