...
class JobExecutionStatusDetails {
 public:
  ...
  void SetDetailsMap(DetailsMapT&& value) {
    m_detailsMapHasBeenSet = true;
    m_detailsMap = std::forward<DetailsMapT>(value);
  }
  template <typename DetailsMapT = Aws::Map<Aws::String, Aws::String>>
  JobExecutionStatusDetails& WithDetailsMap(DetailsMapT&& value) {
    SetDetailsMap(std::forward<DetailsMapT>(value));
    return *this;
  }
  ...
  JobExecutionStatusDetails& AddDetailsMap(DetailsMapKeyT&& key, DetailsMapValueT&& value) {
    m_detailsMapHasBeenSet = true;
    m_detailsMap.emplace(std::forward<DetailsMapKeyT>(key), std::forward<DetailsMapValueT>(value));
    return *this;
  }
  ///@}
 private:
