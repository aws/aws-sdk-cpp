...
class AssetPropertyTimestamp {
 public:
  ...
  void SetTimeInSeconds(TimeInSecondsT&& value) {
    ...
    m_timeInSeconds = std::forward<TimeInSecondsT>(value);
  }
  template <typename TimeInSecondsT = Aws::String>
  AssetPropertyTimestamp& WithTimeInSeconds(TimeInSecondsT&& value) {
    SetTimeInSeconds(std::forward<TimeInSecondsT>(value));
    return *this;
  }
  ///@}
  ...
  void SetOffsetInNanos(OffsetInNanosT&& value) {
    ...
    m_offsetInNanos = std::forward<OffsetInNanosT>(value);
  }
  template <typename OffsetInNanosT = Aws::String>
  AssetPropertyTimestamp& WithOffsetInNanos(OffsetInNanosT&& value) {
    SetOffsetInNanos(std::forward<OffsetInNanosT>(value));
    return *this;
  }
  ///@}
 private:
