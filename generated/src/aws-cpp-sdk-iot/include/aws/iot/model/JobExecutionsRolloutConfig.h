...
class JobExecutionsRolloutConfig {
 public:
  ...
  inline JobExecutionsRolloutConfig& WithMaximumPerMinute(int value) {
  ...
  void SetExponentialRate(ExponentialRateT&& value) {
    ...
    m_exponentialRate = std::forward<ExponentialRateT>(value);
  }
  template <typename ExponentialRateT = ExponentialRolloutRate>
  JobExecutionsRolloutConfig& WithExponentialRate(ExponentialRateT&& value) {
    SetExponentialRate(std::forward<ExponentialRateT>(value));
    return *this;
  }
  ///@}
 private:
