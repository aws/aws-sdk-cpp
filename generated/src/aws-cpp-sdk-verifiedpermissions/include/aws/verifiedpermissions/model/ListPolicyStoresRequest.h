...
class ListPolicyStoresRequest : public VerifiedPermissionsRequest {
 public:
  AWS_VERIFIEDPERMISSIONS_API ListPolicyStoresRequest() = default;
  ...
  template <typename NextTokenT = Aws::String>
  ListPolicyStoresRequest& WithNextToken(NextTokenT&& value) {
   ...
   * parameter, the operation defaults to 10 policy stores per response. You can
   * specify a maximum of 50 policy stores per response.</p>
   */
  ...
  inline ListPolicyStoresRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
