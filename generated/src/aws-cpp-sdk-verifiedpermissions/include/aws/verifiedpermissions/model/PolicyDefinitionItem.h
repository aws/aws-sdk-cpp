...
class PolicyDefinitionItem {
 public:
  ...
  void SetStatic(StaticT&& value) {
    m_staticHasBeenSet = true;
    m_static = std::forward<StaticT>(value);
  }
  template <typename StaticT = StaticPolicyDefinitionItem>
  PolicyDefinitionItem& WithStatic(StaticT&& value) {
    SetStatic(std::forward<StaticT>(value));
    return *this;
  }
  ///@}
  ...
  void SetTemplateLinked(TemplateLinkedT&& value) {
    m_templateLinkedHasBeenSet = true;
    m_templateLinked = std::forward<TemplateLinkedT>(value);
  }
  template <typename TemplateLinkedT = TemplateLinkedPolicyDefinitionItem>
  PolicyDefinitionItem& WithTemplateLinked(TemplateLinkedT&& value) {
    SetTemplateLinked(std::forward<TemplateLinkedT>(value));
    return *this;
  }
  ///@}
 private:
