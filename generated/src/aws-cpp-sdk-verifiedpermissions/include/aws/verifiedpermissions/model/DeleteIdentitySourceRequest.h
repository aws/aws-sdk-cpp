...
class DeleteIdentitySourceRequest : public VerifiedPermissionsRequest {
 public:
   ...
   * you want to delete.</p> <p>To specify a policy store, use its ID or alias name.
  ...
  void SetIdentitySourceId(IdentitySourceIdT&& value) {
    ...
    m_identitySourceId = std::forward<IdentitySourceIdT>(value);
  }
  template <typename IdentitySourceIdT = Aws::String>
  DeleteIdentitySourceRequest& WithIdentitySourceId(IdentitySourceIdT&& value) {
    SetIdentitySourceId(std::forward<IdentitySourceIdT>(value));
    return *this;
  }
  ///@}
 private:
