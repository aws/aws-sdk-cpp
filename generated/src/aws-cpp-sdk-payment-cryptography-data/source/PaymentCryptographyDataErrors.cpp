...
template <>
AWS_PAYMENTCRYPTOGRAPHYDATA_API ResourceNotFoundException PaymentCryptographyDataError::GetModeledError() {
  assert(this->GetErrorType() == PaymentCryptographyDataErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_PAYMENTCRYPTOGRAPHYDATA_API ValidationException PaymentCryptographyDataError::GetModeledError() {
  assert(this->GetErrorType() == PaymentCryptographyDataErrors::VALIDATION);
  return ValidationException(this->GetJsonPayload().View());
}

template <>
AWS_PAYMENTCRYPTOGRAPHYDATA_API VerificationFailedException PaymentCryptographyDataError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == INTERNAL_SERVER_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(PaymentCryptographyDataErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
