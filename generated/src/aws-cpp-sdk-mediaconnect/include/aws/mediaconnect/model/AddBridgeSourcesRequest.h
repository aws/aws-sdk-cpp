...
class AddBridgeSourcesRequest : public MediaConnectRequest {
 public:
  ...
  template <typename BridgeArnT = Aws::String>
  AddBridgeSourcesRequest& WithBridgeArn(BridgeArnT&& value) {
  ...
  template <typename SourcesT = Aws::Vector<AddBridgeSourceRequest>>
  AddBridgeSourcesRequest& WithSources(SourcesT&& value) {
    SetSources(std::forward<SourcesT>(value));
    return *this;
  }
  template <typename SourcesT = AddBridgeSourceRequest>
  AddBridgeSourcesRequest& AddSources(SourcesT&& value) {
    m_sourcesHasBeenSet = true;
