...
/**
 */
class BatchGetRouterNetworkInterfaceRequest : public MediaConnectRequest {
 public:
  AWS_MEDIACONNECT_API BatchGetRouterNetworkInterfaceRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "BatchGetRouterNetworkInterface"; }
  ...
  ///@{
  /**
   * <p>The Amazon Resource Names (ARNs) of the router network interfaces you want to
  ...
  template <typename ArnsT = Aws::Vector<Aws::String>>
  BatchGetRouterNetworkInterfaceRequest& WithArns(ArnsT&& value) {
    SetArns(std::forward<ArnsT>(value));
    return *this;
  }
  template <typename ArnsT = Aws::String>
  BatchGetRouterNetworkInterfaceRequest& AddArns(ArnsT&& value) {
    m_arnsHasBeenSet = true;
