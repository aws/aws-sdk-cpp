...
class AddFlowMediaStreamsRequest : public MediaConnectRequest {
 public:
  ...
  template <typename FlowArnT = Aws::String>
  AddFlowMediaStreamsRequest& WithFlowArn(FlowArnT&& value) {
  ...
  void SetMediaStreams(MediaStreamsT&& value) {
    ...
    m_mediaStreams = std::forward<MediaStreamsT>(value);
  }
  template <typename MediaStreamsT = Aws::Vector<AddMediaStreamRequest>>
  AddFlowMediaStreamsRequest& WithMediaStreams(MediaStreamsT&& value) {
    SetMediaStreams(std::forward<MediaStreamsT>(value));
    return *this;
  }
  template <typename MediaStreamsT = AddMediaStreamRequest>
  AddFlowMediaStreamsRequest& AddMediaStreams(MediaStreamsT&& value) {
    m_mediaStreamsHasBeenSet = true;
    m_mediaStreams.emplace_back(std::forward<MediaStreamsT>(value));
    return *this;
  }
  ///@}
 private:
