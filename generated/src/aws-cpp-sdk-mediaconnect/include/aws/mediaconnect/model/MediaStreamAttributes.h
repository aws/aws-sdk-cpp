...
class MediaStreamAttributes {
 public:
  ...
  void SetFmtp(FmtpT&& value) {
    m_fmtpHasBeenSet = true;
    m_fmtp = std::forward<FmtpT>(value);
  }
  template <typename FmtpT = Fmtp>
  MediaStreamAttributes& WithFmtp(FmtpT&& value) {
    SetFmtp(std::forward<FmtpT>(value));
    return *this;
  }
  ///@}
  ...
  void SetLang(LangT&& value) {
    m_langHasBeenSet = true;
    m_lang = std::forward<LangT>(value);
  }
  template <typename LangT = Aws::String>
  MediaStreamAttributes& WithLang(LangT&& value) {
    SetLang(std::forward<LangT>(value));
    return *this;
  }
  ///@}
 private:
