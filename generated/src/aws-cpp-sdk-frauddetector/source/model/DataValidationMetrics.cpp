...
DataValidationMetrics::DataValidationMetrics(JsonView jsonValue) { *this = jsonValue; }
...
DataValidationMetrics& DataValidationMetrics::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("fieldLevelMessages")) {
    ...
    for (unsigned fieldLevelMessagesIndex = 0; fieldLevelMessagesIndex < fieldLevelMessagesJsonList.GetLength();
         ++fieldLevelMessagesIndex) {
