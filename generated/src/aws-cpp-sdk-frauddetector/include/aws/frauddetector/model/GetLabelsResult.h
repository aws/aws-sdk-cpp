...
class GetLabelsResult {
 public:
  ...
  template <typename LabelsT = Aws::Vector<Label>>
  GetLabelsResult& WithLabels(LabelsT&& value) {
    SetLabels(std::forward<LabelsT>(value));
    return *this;
  }
  template <typename LabelsT = Label>
  GetLabelsResult& AddLabels(LabelsT&& value) {
    m_labelsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  GetLabelsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  GetLabelsResult& WithRequestId(RequestIdT&& value) {
