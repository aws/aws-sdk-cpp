...
class BatchCreateVariableRequest : public FraudDetectorRequest {
 public:
  ...
  void SetVariableEntries(VariableEntriesT&& value) {
    m_variableEntriesHasBeenSet = true;
    m_variableEntries = std::forward<VariableEntriesT>(value);
  }
  template <typename VariableEntriesT = Aws::Vector<VariableEntry>>
  BatchCreateVariableRequest& WithVariableEntries(VariableEntriesT&& value) {
    SetVariableEntries(std::forward<VariableEntriesT>(value));
    return *this;
  }
  template <typename VariableEntriesT = VariableEntry>
  BatchCreateVariableRequest& AddVariableEntries(VariableEntriesT&& value) {
    m_variableEntriesHasBeenSet = true;
    m_variableEntries.emplace_back(std::forward<VariableEntriesT>(value));
    return *this;
  }
  ///@}
