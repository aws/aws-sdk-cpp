...
class GetModelsResult {
 public:
  ...
  template <typename NextTokenT = Aws::String>
  GetModelsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename ModelsT = Aws::Vector<Model>>
  GetModelsResult& WithModels(ModelsT&& value) {
    SetModels(std::forward<ModelsT>(value));
    return *this;
  }
  template <typename ModelsT = Model>
  GetModelsResult& AddModels(ModelsT&& value) {
    m_modelsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  GetModelsResult& WithRequestId(RequestIdT&& value) {
