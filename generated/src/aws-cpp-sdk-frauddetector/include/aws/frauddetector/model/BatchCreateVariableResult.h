...
class BatchCreateVariableResult {
 public:
  ...
  template <typename ErrorsT = Aws::Vector<BatchCreateVariableError>>
  BatchCreateVariableResult& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = BatchCreateVariableError>
  BatchCreateVariableResult& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchCreateVariableResult& WithRequestId(RequestIdT&& value) {
