...
class PutKMSEncryptionKeyRequest : public FraudDetectorRequest {
 public:
  ...
  void SetKmsEncryptionKeyArn(KmsEncryptionKeyArnT&& value) {
    m_kmsEncryptionKeyArnHasBeenSet = true;
    m_kmsEncryptionKeyArn = std::forward<KmsEncryptionKeyArnT>(value);
  }
  template <typename KmsEncryptionKeyArnT = Aws::String>
  PutKMSEncryptionKeyRequest& WithKmsEncryptionKeyArn(KmsEncryptionKeyArnT&& value) {
    SetKmsEncryptionKeyArn(std::forward<KmsEncryptionKeyArnT>(value));
    return *this;
  }
  ///@}
 private:
