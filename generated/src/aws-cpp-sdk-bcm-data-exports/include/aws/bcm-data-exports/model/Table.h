...
class Table {
 public:
  ...
  template <typename TablePropertiesT = Aws::Vector<TablePropertyDescription>>
  Table& WithTableProperties(TablePropertiesT&& value) {
    SetTableProperties(std::forward<TablePropertiesT>(value));
    return *this;
  }
  template <typename TablePropertiesT = TablePropertyDescription>
  Table& AddTableProperties(TablePropertiesT&& value) {
    m_tablePropertiesHasBeenSet = true;
    m_tableProperties.emplace_back(std::forward<TablePropertiesT>(value));
    return *this;
  }
  ///@}
 private:
