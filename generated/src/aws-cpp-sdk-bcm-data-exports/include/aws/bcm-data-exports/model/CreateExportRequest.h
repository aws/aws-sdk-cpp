...
class CreateExportRequest : public BCMDataExportsRequest {
 public:
  ...
  template <typename ExportT = Export>
  CreateExportRequest& WithExport(ExportT&& value) {
  ...
  template <typename ResourceTagsT = Aws::Vector<ResourceTag>>
  CreateExportRequest& WithResourceTags(ResourceTagsT&& value) {
    SetResourceTags(std::forward<ResourceTagsT>(value));
    return *this;
  }
  template <typename ResourceTagsT = ResourceTag>
  CreateExportRequest& AddResourceTags(ResourceTagsT&& value) {
    m_resourceTagsHasBeenSet = true;
