...
class ListMLModelTrainingJobsResult {
 public:
  ...
  template <typename IdsT = Aws::Vector<Aws::String>>
  ListMLModelTrainingJobsResult& WithIds(IdsT&& value) {
    SetIds(std::forward<IdsT>(value));
    return *this;
  }
  template <typename IdsT = Aws::String>
  ListMLModelTrainingJobsResult& AddIds(IdsT&& value) {
    m_idsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListMLModelTrainingJobsResult& WithRequestId(RequestIdT&& value) {
