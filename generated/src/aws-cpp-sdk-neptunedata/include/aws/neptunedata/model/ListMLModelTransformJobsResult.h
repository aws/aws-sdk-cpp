...
class ListMLModelTransformJobsResult {
 public:
  ...
  template <typename IdsT = Aws::Vector<Aws::String>>
  ListMLModelTransformJobsResult& WithIds(IdsT&& value) {
    SetIds(std::forward<IdsT>(value));
    return *this;
  }
  template <typename IdsT = Aws::String>
  ListMLModelTransformJobsResult& AddIds(IdsT&& value) {
    m_idsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListMLModelTransformJobsResult& WithRequestId(RequestIdT&& value) {
