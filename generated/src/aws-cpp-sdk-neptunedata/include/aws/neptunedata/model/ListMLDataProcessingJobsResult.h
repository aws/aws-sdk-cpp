...
class ListMLDataProcessingJobsResult {
 public:
  ...
  template <typename IdsT = Aws::Vector<Aws::String>>
  ListMLDataProcessingJobsResult& WithIds(IdsT&& value) {
    SetIds(std::forward<IdsT>(value));
    return *this;
  }
  template <typename IdsT = Aws::String>
  ListMLDataProcessingJobsResult& AddIds(IdsT&& value) {
    m_idsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListMLDataProcessingJobsResult& WithRequestId(RequestIdT&& value) {
