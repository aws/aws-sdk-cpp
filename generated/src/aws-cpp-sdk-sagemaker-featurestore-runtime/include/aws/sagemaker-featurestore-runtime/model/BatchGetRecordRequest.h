...
class BatchGetRecordRequest : public SageMakerFeatureStoreRuntimeRequest {
 public:
  ...
  template <typename IdentifiersT = Aws::Vector<BatchGetRecordIdentifier>>
  BatchGetRecordRequest& WithIdentifiers(IdentifiersT&& value) {
    SetIdentifiers(std::forward<IdentifiersT>(value));
    return *this;
  }
  template <typename IdentifiersT = BatchGetRecordIdentifier>
  BatchGetRecordRequest& AddIdentifiers(IdentifiersT&& value) {
    m_identifiersHasBeenSet = true;
  ...
  inline BatchGetRecordRequest& WithExpirationTimeResponse(ExpirationTimeResponse value) {
