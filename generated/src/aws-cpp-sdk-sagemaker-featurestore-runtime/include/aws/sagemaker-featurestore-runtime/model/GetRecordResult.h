...
class GetRecordResult {
 public:
  ...
  template <typename RecordT = Aws::Vector<FeatureValue>>
  GetRecordResult& WithRecord(RecordT&& value) {
    SetRecord(std::forward<RecordT>(value));
    return *this;
  }
  template <typename RecordT = FeatureValue>
  GetRecordResult& AddRecord(RecordT&& value) {
    m_recordHasBeenSet = true;
  ...
  template <typename ExpiresAtT = Aws::String>
  GetRecordResult& WithExpiresAt(ExpiresAtT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  GetRecordResult& WithRequestId(RequestIdT&& value) {
