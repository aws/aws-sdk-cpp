...
/**
 */
class ListKeyPoliciesRequest : public KMSRequest {
 public:
  AWS_KMS_API ListKeyPoliciesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListKeyPolicies"; }
  ...
  ///@{
  /**
   * <p>Gets the names of key policies for the specified KMS key.</p> <p>Specify the
  ...
  template <typename KeyIdT = Aws::String>
  ListKeyPoliciesRequest& WithKeyId(KeyIdT&& value) {
  ...
  inline ListKeyPoliciesRequest& WithLimit(int value) {
    SetLimit(value);
    return *this;
  }
  ///@}
  ...
  template <typename MarkerT = Aws::String>
  ListKeyPoliciesRequest& WithMarker(MarkerT&& value) {
