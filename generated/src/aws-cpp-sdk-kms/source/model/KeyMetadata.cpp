...
KeyMetadata::KeyMetadata(JsonView jsonValue) { *this = jsonValue; }
...
KeyMetadata& KeyMetadata::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("EncryptionAlgorithms")) {
    ...
    for (unsigned encryptionAlgorithmsIndex = 0; encryptionAlgorithmsIndex < encryptionAlgorithmsJsonList.GetLength();
         ++encryptionAlgorithmsIndex) {
      m_encryptionAlgorithms.push_back(EncryptionAlgorithmSpecMapper::GetEncryptionAlgorithmSpecForName(
          encryptionAlgorithmsJsonList[encryptionAlgorithmsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("SigningAlgorithms")) {
    ...
    for (unsigned signingAlgorithmsIndex = 0; signingAlgorithmsIndex < signingAlgorithmsJsonList.GetLength(); ++signingAlgorithmsIndex) {
      m_signingAlgorithms.push_back(
          SigningAlgorithmSpecMapper::GetSigningAlgorithmSpecForName(signingAlgorithmsJsonList[signingAlgorithmsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("KeyAgreementAlgorithms")) {
    ...
    for (unsigned keyAgreementAlgorithmsIndex = 0; keyAgreementAlgorithmsIndex < keyAgreementAlgorithmsJsonList.GetLength();
         ++keyAgreementAlgorithmsIndex) {
      m_keyAgreementAlgorithms.push_back(KeyAgreementAlgorithmSpecMapper::GetKeyAgreementAlgorithmSpecForName(
          keyAgreementAlgorithmsJsonList[keyAgreementAlgorithmsIndex].AsString()));
    }
...
JsonValue KeyMetadata::Jsonize() const {
  ...
  if (m_encryptionAlgorithmsHasBeenSet) {
    ...
    for (unsigned encryptionAlgorithmsIndex = 0; encryptionAlgorithmsIndex < encryptionAlgorithmsJsonList.GetLength();
         ++encryptionAlgorithmsIndex) {
      encryptionAlgorithmsJsonList[encryptionAlgorithmsIndex].AsString(
          EncryptionAlgorithmSpecMapper::GetNameForEncryptionAlgorithmSpec(m_encryptionAlgorithms[encryptionAlgorithmsIndex]));
    }
  ...
  if (m_signingAlgorithmsHasBeenSet) {
    ...
    for (unsigned signingAlgorithmsIndex = 0; signingAlgorithmsIndex < signingAlgorithmsJsonList.GetLength(); ++signingAlgorithmsIndex) {
      signingAlgorithmsJsonList[signingAlgorithmsIndex].AsString(
          SigningAlgorithmSpecMapper::GetNameForSigningAlgorithmSpec(m_signingAlgorithms[signingAlgorithmsIndex]));
    }
  ...
  if (m_keyAgreementAlgorithmsHasBeenSet) {
    ...
    for (unsigned keyAgreementAlgorithmsIndex = 0; keyAgreementAlgorithmsIndex < keyAgreementAlgorithmsJsonList.GetLength();
         ++keyAgreementAlgorithmsIndex) {
      keyAgreementAlgorithmsJsonList[keyAgreementAlgorithmsIndex].AsString(
          KeyAgreementAlgorithmSpecMapper::GetNameForKeyAgreementAlgorithmSpec(m_keyAgreementAlgorithms[keyAgreementAlgorithmsIndex]));
    }
  ...
  if (m_macAlgorithmsHasBeenSet) {
    ...
    for (unsigned macAlgorithmsIndex = 0; macAlgorithmsIndex < macAlgorithmsJsonList.GetLength(); ++macAlgorithmsIndex) {
      macAlgorithmsJsonList[macAlgorithmsIndex].AsString(
          MacAlgorithmSpecMapper::GetNameForMacAlgorithmSpec(m_macAlgorithms[macAlgorithmsIndex]));
    }
