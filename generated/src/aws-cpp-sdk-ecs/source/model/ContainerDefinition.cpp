...
ContainerDefinition::ContainerDefinition(JsonView jsonValue) { *this = jsonValue; }
...
ContainerDefinition& ContainerDefinition::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("dockerSecurityOptions")) {
    ...
    for (unsigned dockerSecurityOptionsIndex = 0; dockerSecurityOptionsIndex < dockerSecurityOptionsJsonList.GetLength();
         ++dockerSecurityOptionsIndex) {
  ...
  if (jsonValue.ValueExists("resourceRequirements")) {
    ...
    for (unsigned resourceRequirementsIndex = 0; resourceRequirementsIndex < resourceRequirementsJsonList.GetLength();
