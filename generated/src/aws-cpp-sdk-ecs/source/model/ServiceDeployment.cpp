...
ServiceDeployment::ServiceDeployment(JsonView jsonValue) { *this = jsonValue; }
...
ServiceDeployment& ServiceDeployment::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("sourceServiceRevisions")) {
    ...
    for (unsigned sourceServiceRevisionsIndex = 0; sourceServiceRevisionsIndex < sourceServiceRevisionsJsonList.GetLength();
         ++sourceServiceRevisionsIndex) {
