...
Deployment& Deployment::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("serviceConnectResources")) {
    ...
    for (unsigned serviceConnectResourcesIndex = 0; serviceConnectResourcesIndex < serviceConnectResourcesJsonList.GetLength();
         ++serviceConnectResourcesIndex) {
...
JsonValue Deployment::Jsonize() const {
  ...
  if (m_serviceConnectResourcesHasBeenSet) {
    ...
    for (unsigned serviceConnectResourcesIndex = 0; serviceConnectResourcesIndex < serviceConnectResourcesJsonList.GetLength();
         ++serviceConnectResourcesIndex) {
      serviceConnectResourcesJsonList[serviceConnectResourcesIndex].AsObject(
          m_serviceConnectResources[serviceConnectResourcesIndex].Jsonize());
    }
