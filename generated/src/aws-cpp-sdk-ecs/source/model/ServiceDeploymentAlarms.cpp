...
ServiceDeploymentAlarms::ServiceDeploymentAlarms(JsonView jsonValue) { *this = jsonValue; }

ServiceDeploymentAlarms& ServiceDeploymentAlarms::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("status")) {
    m_status =
        ServiceDeploymentRollbackMonitorsStatusMapper::GetServiceDeploymentRollbackMonitorsStatusForName(jsonValue.GetString("status"));
  ...
  if (jsonValue.ValueExists("triggeredAlarmNames")) {
    ...
    for (unsigned triggeredAlarmNamesIndex = 0; triggeredAlarmNamesIndex < triggeredAlarmNamesJsonList.GetLength();
         ++triggeredAlarmNamesIndex) {
...
JsonValue ServiceDeploymentAlarms::Jsonize() const {
  ...
  if (m_statusHasBeenSet) {
    payload.WithString("status",
                       ServiceDeploymentRollbackMonitorsStatusMapper::GetNameForServiceDeploymentRollbackMonitorsStatus(m_status));
  }
