 ...
 * href="http://docs.aws.amazon.com/goto/WebAPI/ecs-2014-11-13/RunTaskResponse">AWS
 * API Reference</a></p>
 */
class RunTaskResult {
 public:
  ...
  template <typename TasksT = Aws::Vector<Task>>
  RunTaskResult& WithTasks(TasksT&& value) {
    SetTasks(std::forward<TasksT>(value));
    return *this;
  }
  template <typename TasksT = Task>
  RunTaskResult& AddTasks(TasksT&& value) {
    m_tasksHasBeenSet = true;
  ...
  template <typename FailuresT = Aws::Vector<Failure>>
  RunTaskResult& WithFailures(FailuresT&& value) {
    SetFailures(std::forward<FailuresT>(value));
    return *this;
  }
  template <typename FailuresT = Failure>
  RunTaskResult& AddFailures(FailuresT&& value) {
    m_failuresHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  RunTaskResult& WithRequestId(RequestIdT&& value) {
