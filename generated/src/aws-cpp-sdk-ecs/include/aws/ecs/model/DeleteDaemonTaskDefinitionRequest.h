...
/**
 */
class DeleteDaemonTaskDefinitionRequest : public ECSRequest {
 public:
  AWS_ECS_API DeleteDaemonTaskDefinitionRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteDaemonTaskDefinition"; }
   ...
   * (<code>family:revision</code>) or full Amazon Resource Name (ARN) of the daemon
   * task definition to delete.</p>
   */
  ...
  template <typename DaemonTaskDefinitionT = Aws::String>
  DeleteDaemonTaskDefinitionRequest& WithDaemonTaskDefinition(DaemonTaskDefinitionT&& value) {
