...
class DescribeServiceRevisionsRequest : public ECSRequest {
 public:
  ...
  void SetServiceRevisionArns(ServiceRevisionArnsT&& value) {
    m_serviceRevisionArnsHasBeenSet = true;
    m_serviceRevisionArns = std::forward<ServiceRevisionArnsT>(value);
  }
  template <typename ServiceRevisionArnsT = Aws::Vector<Aws::String>>
  DescribeServiceRevisionsRequest& WithServiceRevisionArns(ServiceRevisionArnsT&& value) {
    SetServiceRevisionArns(std::forward<ServiceRevisionArnsT>(value));
    return *this;
  }
  template <typename ServiceRevisionArnsT = Aws::String>
  DescribeServiceRevisionsRequest& AddServiceRevisionArns(ServiceRevisionArnsT&& value) {
    m_serviceRevisionArnsHasBeenSet = true;
    m_serviceRevisionArns.emplace_back(std::forward<ServiceRevisionArnsT>(value));
    return *this;
  }
  ///@}
 private:
