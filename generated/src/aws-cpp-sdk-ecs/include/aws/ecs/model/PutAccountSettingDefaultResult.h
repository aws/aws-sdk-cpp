...
class PutAccountSettingDefaultResult {
 public:
  ...
  void SetSetting(SettingT&& value) {
    m_settingHasBeenSet = true;
    m_setting = std::forward<SettingT>(value);
  }
  template <typename SettingT = Setting>
  PutAccountSettingDefaultResult& WithSetting(SettingT&& value) {
    SetSetting(std::forward<SettingT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  PutAccountSettingDefaultResult& WithRequestId(RequestIdT&& value) {
