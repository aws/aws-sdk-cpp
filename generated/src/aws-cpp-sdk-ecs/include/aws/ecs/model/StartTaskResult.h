 ...
 * href="http://docs.aws.amazon.com/goto/WebAPI/ecs-2014-11-13/StartTaskResponse">AWS
 * API Reference</a></p>
 */
class StartTaskResult {
 public:
  ...
  template <typename TasksT = Aws::Vector<Task>>
  StartTaskResult& WithTasks(TasksT&& value) {
    SetTasks(std::forward<TasksT>(value));
    return *this;
  }
  template <typename TasksT = Task>
  StartTaskResult& AddTasks(TasksT&& value) {
    m_tasksHasBeenSet = true;
  ...
  template <typename FailuresT = Aws::Vector<Failure>>
  StartTaskResult& WithFailures(FailuresT&& value) {
    SetFailures(std::forward<FailuresT>(value));
    return *this;
  }
  template <typename FailuresT = Failure>
  StartTaskResult& AddFailures(FailuresT&& value) {
    m_failuresHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  StartTaskResult& WithRequestId(RequestIdT&& value) {
