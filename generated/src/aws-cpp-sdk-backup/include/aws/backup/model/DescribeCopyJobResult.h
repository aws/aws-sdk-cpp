...
class DescribeCopyJobResult {
 public:
  ...
  void SetCopyJob(CopyJobT&& value) {
    m_copyJobHasBeenSet = true;
    m_copyJob = std::forward<CopyJobT>(value);
  }
  template <typename CopyJobT = CopyJob>
  DescribeCopyJobResult& WithCopyJob(CopyJobT&& value) {
    SetCopyJob(std::forward<CopyJobT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  DescribeCopyJobResult& WithRequestId(RequestIdT&& value) {
