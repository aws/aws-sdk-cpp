...
/**
 */
class DeleteTieringConfigurationRequest : public BackupRequest {
 public:
  AWS_BACKUP_API DeleteTieringConfigurationRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteTieringConfiguration"; }
  ...
  template <typename TieringConfigurationNameT = Aws::String>
  DeleteTieringConfigurationRequest& WithTieringConfigurationName(TieringConfigurationNameT&& value) {
