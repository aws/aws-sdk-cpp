...
class Condition {
 public:
  ...
  inline Condition& WithConditionType(ConditionType value) {
  ...
  void SetConditionValue(ConditionValueT&& value) {
    ...
    m_conditionValue = std::forward<ConditionValueT>(value);
  }
  template <typename ConditionValueT = Aws::String>
  Condition& WithConditionValue(ConditionValueT&& value) {
    SetConditionValue(std::forward<ConditionValueT>(value));
    return *this;
  }
  ///@}
 private:
