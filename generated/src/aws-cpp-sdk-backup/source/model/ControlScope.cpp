...
ControlScope::ControlScope(JsonView jsonValue) { *this = jsonValue; }
...
ControlScope& ControlScope::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("ComplianceResourceIds")) {
    ...
    for (unsigned complianceResourceIdsIndex = 0; complianceResourceIdsIndex < complianceResourceIdsJsonList.GetLength();
         ++complianceResourceIdsIndex) {
