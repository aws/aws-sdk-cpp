...
RestoreTestingRecoveryPointSelection::RestoreTestingRecoveryPointSelection(JsonView jsonValue) { *this = jsonValue; }
...
RestoreTestingRecoveryPointSelection& RestoreTestingRecoveryPointSelection::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("Algorithm")) {
    m_algorithm = RestoreTestingRecoveryPointSelectionAlgorithmMapper::GetRestoreTestingRecoveryPointSelectionAlgorithmForName(
        jsonValue.GetString("Algorithm"));
  ...
  if (jsonValue.ValueExists("RecoveryPointTypes")) {
    ...
    for (unsigned recoveryPointTypesIndex = 0; recoveryPointTypesIndex < recoveryPointTypesJsonList.GetLength();
         ++recoveryPointTypesIndex) {
      m_recoveryPointTypes.push_back(RestoreTestingRecoveryPointTypeMapper::GetRestoreTestingRecoveryPointTypeForName(
          recoveryPointTypesJsonList[recoveryPointTypesIndex].AsString()));
    }
...
JsonValue RestoreTestingRecoveryPointSelection::Jsonize() const {
  ...
  if (m_algorithmHasBeenSet) {
    payload.WithString(
        "Algorithm",
        RestoreTestingRecoveryPointSelectionAlgorithmMapper::GetNameForRestoreTestingRecoveryPointSelectionAlgorithm(m_algorithm));
  }
  ...
  if (m_recoveryPointTypesHasBeenSet) {
    ...
    for (unsigned recoveryPointTypesIndex = 0; recoveryPointTypesIndex < recoveryPointTypesJsonList.GetLength();
         ++recoveryPointTypesIndex) {
      recoveryPointTypesJsonList[recoveryPointTypesIndex].AsString(
          RestoreTestingRecoveryPointTypeMapper::GetNameForRestoreTestingRecoveryPointType(m_recoveryPointTypes[recoveryPointTypesIndex]));
    }
