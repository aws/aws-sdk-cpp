...
AWSError<Aws::Client::CoreErrors> S3CrtErrorMarshaller::Marshall(const Aws::Http::HttpResponse& httpResponse) const {
  ...
  if (!body.good()) {
    return Aws::Client::AWSError<Aws::Client::CoreErrors>(Aws::Client::CoreErrors::INVALID_PARAMETER_VALUE, "", message, false);
  }
  ...
  if (doc.WasParseSuccessful() && !doc.GetRootElement().IsNull() && doc.GetRootElement().GetName() == Aws::String("Error")) {
