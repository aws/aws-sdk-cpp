...
S3ExpressSigner::S3ExpressSigner(std::shared_ptr<S3ExpressIdentityProvider> S3ExpressIdentityProvider,
                                 const std::shared_ptr<Aws::Auth::AWSCredentialsProvider> &credentialsProvider,
                                 const Aws::String &serviceName, const Aws::String &region,
                                 AWSAuthV4Signer::PayloadSigningPolicy signingPolicy, bool urlEscapePath,
                                 Aws::Auth::AWSSigningAlgorithm signingAlgorithm)
    : AWSAuthV4Signer(credentialsProvider, serviceName.c_str(), region, signingPolicy, urlEscapePath, signingAlgorithm),
      m_S3ExpressIdentityProvider(std::move(S3ExpressIdentityProvider)) {}

const char *S3ExpressSigner::GetName() const { return S3_EXPRESS_SIGNER_NAME; }

bool S3ExpressSigner::SignRequest(Aws::Http::HttpRequest &request, const char *region, const char *serviceName, bool signBody) const {
  const auto requestId =
      Aws::GetWithDefault(request.GetServiceSpecificParameters()->parameterMap, Aws::String("dedupeId"), Aws::String(UUID::RandomUUID()));
...
bool S3ExpressSigner::PresignRequest(Aws::Http::HttpRequest &request, const char *region, const char *serviceName,
                                     long long int expirationInSeconds) const {
  const auto requestId =
      Aws::GetWithDefault(request.GetServiceSpecificParameters()->parameterMap, Aws::String("dedupeId"), Aws::String(UUID::RandomUUID()));
  if (hasRequestId(requestId)) {
    // We were already processing this request and are attempting to sign
...
Aws::Auth::AWSCredentials S3ExpressSigner::GetCredentials(
