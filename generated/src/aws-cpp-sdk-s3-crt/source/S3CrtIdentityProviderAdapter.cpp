...
S3CrtIdentityProviderUserData::S3CrtIdentityProviderUserData(std::shared_ptr<S3ExpressIdentityProvider> identity_provider)
    : m_identityProvider(identity_provider), m_impl(Aws::MakeUnique<struct aws_s3express_credentials_provider_vtable>(ALLOC_TAG)) {
  m_impl->get_credentials = [](struct aws_s3express_credentials_provider* provider, const struct aws_credentials* original_credentials,
                               const struct aws_credentials_properties_s3express* s3express_properties,
                               aws_on_get_credentials_callback_fn* callback, void* user_data) -> int {
    ...
    Aws::UniquePtr<aws_string, std::function<void(aws_string*)>> hostnameCStr{
        aws_string_new_from_cursor(get_aws_allocator(), &s3express_properties->host), aws_string_destroy};
    ...
    Aws::UniquePtr<aws_credentials, std::function<void(aws_credentials*)>> credentials{
        aws_credentials_new(get_aws_allocator(), access_key_id_cursor, secret_access_key_cursor, session_token_cursor,
...
aws_s3express_credentials_provider* S3CrtIdentityProviderAdapter::ProviderFactory(
  ...
  provider =
      static_cast<aws_s3express_credentials_provider*>(aws_mem_calloc(allocator, 1, sizeof(struct aws_s3express_credentials_provider)));

  auto userData = static_cast<S3CrtIdentityProviderUserData*>(factory_user_data);
  aws_s3express_credentials_provider_init_base(provider, allocator, userData->GetImpl().get(), userData->GetIdentityProvider().get());
