...
void S3CrtClientConfiguration::LoadS3CrtSpecificConfig(const Aws::String& inputProfileName) {
  if (Aws::S3Crt::US_EAST_1_REGIONAL_ENDPOINT_OPTION::NOT_SET == this->useUSEast1RegionalEndPointOption) {
    const Aws::String& useUSEastOption = BaseClientConfigClass::LoadConfigFromEnvOrProfile(
        US_EAST_1_REGIONAL_ENDPOINT_ENV_VAR, inputProfileName, US_EAST_1_REGIONAL_ENDPOINT_CONFIG_VAR, {"legacy", "regional"}, "regional");
  ...
  Aws::String s3DisableMultiRegionAccessPoints =
      ClientConfiguration::LoadConfigFromEnvOrProfile(S3_DISABLE_MULTIREGION_ACCESS_POINTS_ENV_VAR, inputProfileName,
                                                      S3_DISABLE_MULTIREGION_ACCESS_POINTS_CONFIG_VAR, {"true", "false"}, "false");
  if (s3DisableMultiRegionAccessPoints == "true") {
    disableMultiRegionAccessPoints = true;
  }

  Aws::String disableS3ExpressAuthCfg =
      ClientConfiguration::LoadConfigFromEnvOrProfile(S3_DISABLE_EXPRESS_SESSION_ENVIRONMENT_VARIABLE, inputProfileName,
                                                      S3_DISABLE_EXPRESS_SESSION_CONFIG_FILE_OPTION, {"true", "false"}, "false");
...
S3CrtClientConfiguration::S3CrtConfigFactories S3CrtClientConfiguration::S3CrtConfigFactories::defaultFactories = []() {
  ...
  factories.retryStrategyCreateFn = [](const S3Crt::S3CrtClientConfiguration& config) -> aws_retry_strategy* {
    aws_event_loop_group* el_group = config.clientBootstrap ? config.clientBootstrap->GetUnderlyingHandle()->event_loop_group
                                                            : Aws::GetDefaultClientBootstrap()->GetUnderlyingHandle()->event_loop_group;
    ...
    if (strategyToUse == CrtRetryStrategyType::EXPONENTIAL_BACKOFF) {
      ...
      options.el_group = el_group, options.max_retries = config.crtRetryStrategyConfig.config.maxRetries;
