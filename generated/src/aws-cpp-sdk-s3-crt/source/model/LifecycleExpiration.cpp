...
LifecycleExpiration::LifecycleExpiration(const XmlNode& xmlNode) { *this = xmlNode; }
...
LifecycleExpiration& LifecycleExpiration::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!expiredObjectDeleteMarkerNode.IsNull()) {
      m_expiredObjectDeleteMarker = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(expiredObjectDeleteMarkerNode.GetText()).c_str()).c_str());
