...
InventoryConfiguration& InventoryConfiguration::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!includedObjectVersionsNode.IsNull()) {
      m_includedObjectVersions = InventoryIncludedObjectVersionsMapper::GetInventoryIncludedObjectVersionsForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(includedObjectVersionsNode.GetText()).c_str()));
    ...
    if (!optionalFieldsNode.IsNull()) {
      ...
      while (!optionalFieldsMember.IsNull()) {
        m_optionalFields.push_back(
            InventoryOptionalFieldMapper::GetInventoryOptionalFieldForName(StringUtils::Trim(optionalFieldsMember.GetText().c_str())));
...
void InventoryConfiguration::AddToNode(XmlNode& parentNode) const {
  ...
  if (m_includedObjectVersionsHasBeenSet) {
    ...
    includedObjectVersionsNode.SetText(
        InventoryIncludedObjectVersionsMapper::GetNameForInventoryIncludedObjectVersions(m_includedObjectVersions));
  }
