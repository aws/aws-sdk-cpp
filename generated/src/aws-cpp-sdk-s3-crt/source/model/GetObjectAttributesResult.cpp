...
GetObjectAttributesResult::GetObjectAttributesResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
GetObjectAttributesResult& GetObjectAttributesResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!objectSizeNode.IsNull()) {
      m_objectSize =
          StringUtils::ConvertToInt64(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(objectSizeNode.GetText()).c_str()).c_str());
  ...
  if (lastModifiedIter != headers.end()) {
    ...
    if (!m_lastModified.WasParseSuccessful()) {
      AWS_LOGSTREAM_WARN("S3Crt::GetObjectAttributesResult",
