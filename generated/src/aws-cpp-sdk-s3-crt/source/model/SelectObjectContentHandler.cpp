...
SelectObjectContentHandler::SelectObjectContentHandler() : EventStreamHandler() {
  ...
  m_onError = [&](const AWSError<S3CrtErrors>& error) {
    AWS_LOGSTREAM_TRACE(SELECTOBJECTCONTENT_HANDLER_CLASS_TAG, "S3Crt Errors received, " << error);
  };
}
...
void SelectObjectContentHandler::OnEvent() {
  ...
  if (!*this) {
    ...
    m_onError(AWSError<S3CrtErrors>(error));
    return;
  }
...
void SelectObjectContentHandler::HandleEventInMessage() {
  ...
  switch (
      SelectObjectContentEventMapper::GetSelectObjectContentEventTypeForName(eventTypeHeaderIter->second.GetEventHeaderValueAsString())) {
    case SelectObjectContentEventType::INITIAL_RESPONSE: {
      ...
      if (!xmlDoc.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(SELECTOBJECTCONTENT_HANDLER_CLASS_TAG,
                           "Unable to generate a proper InitialResponse object from the response in XML format.");
        break;
      }
...
void SelectObjectContentHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(SELECTOBJECTCONTENT_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
...
void SelectObjectContentHandler::MarshallError(const Aws::String& errorCode, const Aws::String& errorMessage) {
  S3CrtErrorMarshaller errorMarshaller;
