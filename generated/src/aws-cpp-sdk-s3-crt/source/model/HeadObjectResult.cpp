...
HeadObjectResult::HeadObjectResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
HeadObjectResult& HeadObjectResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (lastModifiedIter != headers.end()) {
    ...
    if (!m_lastModified.WasParseSuccessful()) {
      AWS_LOGSTREAM_WARN("S3Crt::HeadObjectResult",
  ...
  if (expiresIter != headers.end()) {
    ...
    if (!m_expires.WasParseSuccessful()) {
      AWS_LOGSTREAM_WARN("S3Crt::HeadObjectResult",
                         "Failed to parse expires header as an RFC822 timestamp: " << expiresIter->second.c_str());
    }
  ...
  if (objectLockRetainUntilDateIter != headers.end()) {
    ...
    if (!m_objectLockRetainUntilDate.WasParseSuccessful()) {
      AWS_LOGSTREAM_WARN("S3Crt::HeadObjectResult", "Failed to parse objectLockRetainUntilDate header as an ISO_8601 timestamp: "
