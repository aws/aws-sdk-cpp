...
class RestoreObjectResult {
 public:
  AWS_S3CRT_API RestoreObjectResult() = default;
  ...
  inline RestoreObjectResult& WithRequestCharged(RequestCharged value) {
  ...
  void SetRestoreOutputPath(RestoreOutputPathT&& value) {
    ...
    m_restoreOutputPath = std::forward<RestoreOutputPathT>(value);
  }
  template <typename RestoreOutputPathT = Aws::String>
  RestoreObjectResult& WithRestoreOutputPath(RestoreOutputPathT&& value) {
    SetRestoreOutputPath(std::forward<RestoreOutputPathT>(value));
    return *this;
  }
  ///@}
