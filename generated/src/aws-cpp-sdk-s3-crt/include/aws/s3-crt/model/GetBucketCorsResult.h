...
class GetBucketCorsResult {
 public:
  AWS_S3CRT_API GetBucketCorsResult() = default;
  ...
  template <typename CORSRulesT = Aws::Vector<CORSRule>>
  GetBucketCorsResult& WithCORSRules(CORSRulesT&& value) {
    SetCORSRules(std::forward<CORSRulesT>(value));
    return *this;
  }
  template <typename CORSRulesT = CORSRule>
  GetBucketCorsResult& AddCORSRules(CORSRulesT&& value) {
    m_cORSRulesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  GetBucketCorsResult& WithRequestId(RequestIdT&& value) {
