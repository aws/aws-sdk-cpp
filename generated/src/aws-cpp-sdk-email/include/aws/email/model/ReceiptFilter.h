...
class ReceiptFilter {
 public:
  ...
  template <typename NameT = Aws::String>
  ReceiptFilter& WithName(NameT&& value) {
  ...
  void SetIpFilter(IpFilterT&& value) {
    m_ipFilterHasBeenSet = true;
    m_ipFilter = std::forward<IpFilterT>(value);
  }
  template <typename IpFilterT = ReceiptIpFilter>
  ReceiptFilter& WithIpFilter(IpFilterT&& value) {
    SetIpFilter(std::forward<IpFilterT>(value));
    return *this;
  }
  ///@}
 private:
