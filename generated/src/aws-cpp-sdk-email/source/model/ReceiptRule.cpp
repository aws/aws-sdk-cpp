...
ReceiptRule::ReceiptRule(const XmlNode& xmlNode) { *this = xmlNode; }
...
ReceiptRule& ReceiptRule::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!tlsPolicyNode.IsNull()) {
      m_tlsPolicy =
          TlsPolicyMapper::GetTlsPolicyForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(tlsPolicyNode.GetText()).c_str()));
    ...
    if (!scanEnabledNode.IsNull()) {
      m_scanEnabled =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(scanEnabledNode.GetText()).c_str()).c_str());
...
void ReceiptRule::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_tlsPolicyHasBeenSet) {
    oStream << location << index << locationValue
            << ".TlsPolicy=" << StringUtils::URLEncode(TlsPolicyMapper::GetNameForTlsPolicy(m_tlsPolicy)) << "&";
  }

  if (m_recipientsHasBeenSet) {
    unsigned recipientsIdx = 1;
    for (auto& item : m_recipients) {
      oStream << location << index << locationValue << ".Recipients.member." << recipientsIdx++ << "="
