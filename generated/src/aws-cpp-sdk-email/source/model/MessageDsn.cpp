...
MessageDsn::MessageDsn(const XmlNode& xmlNode) { *this = xmlNode; }
...
MessageDsn& MessageDsn::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!arrivalDateNode.IsNull()) {
      m_arrivalDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(arrivalDateNode.GetText()).c_str()).c_str(),
...
void MessageDsn::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_arrivalDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".ArrivalDate=" << StringUtils::URLEncode(m_arrivalDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
