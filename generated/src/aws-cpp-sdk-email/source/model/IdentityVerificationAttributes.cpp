...
IdentityVerificationAttributes::IdentityVerificationAttributes(const XmlNode& xmlNode) { *this = xmlNode; }
...
IdentityVerificationAttributes& IdentityVerificationAttributes::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!verificationStatusNode.IsNull()) {
      m_verificationStatus = VerificationStatusMapper::GetVerificationStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(verificationStatusNode.GetText()).c_str()));
...
void IdentityVerificationAttributes::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                    const char* locationValue) const {
  if (m_verificationStatusHasBeenSet) {
    oStream << location << index << locationValue << ".VerificationStatus="
            << StringUtils::URLEncode(VerificationStatusMapper::GetNameForVerificationStatus(m_verificationStatus)) << "&";
  }
...
void IdentityVerificationAttributes::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_verificationStatusHasBeenSet) {
    oStream << location << ".VerificationStatus="
            << StringUtils::URLEncode(VerificationStatusMapper::GetNameForVerificationStatus(m_verificationStatus)) << "&";
  }
  if (m_verificationTokenHasBeenSet) {
