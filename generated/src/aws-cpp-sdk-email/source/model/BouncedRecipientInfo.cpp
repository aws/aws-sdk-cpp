...
BouncedRecipientInfo::BouncedRecipientInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
BouncedRecipientInfo& BouncedRecipientInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!bounceTypeNode.IsNull()) {
      m_bounceType = BounceTypeMapper::GetBounceTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(bounceTypeNode.GetText()).c_str()));
...
void BouncedRecipientInfo::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_bounceTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".BounceType=" << StringUtils::URLEncode(BounceTypeMapper::GetNameForBounceType(m_bounceType)) << "&";
  }
