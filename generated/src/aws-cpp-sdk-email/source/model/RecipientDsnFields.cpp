...
RecipientDsnFields::RecipientDsnFields(const XmlNode& xmlNode) { *this = xmlNode; }
...
RecipientDsnFields& RecipientDsnFields::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!actionNode.IsNull()) {
      m_action =
          DsnActionMapper::GetDsnActionForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(actionNode.GetText()).c_str()));
    ...
    if (!lastAttemptDateNode.IsNull()) {
      m_lastAttemptDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastAttemptDateNode.GetText()).c_str()).c_str(),
...
void RecipientDsnFields::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastAttemptDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastAttemptDate=" << StringUtils::URLEncode(m_lastAttemptDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
