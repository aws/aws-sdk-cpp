...
LambdaAction::LambdaAction(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!invocationTypeNode.IsNull()) {
      m_invocationType = InvocationTypeMapper::GetInvocationTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(invocationTypeNode.GetText()).c_str()));
...
void LambdaAction::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_invocationTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".InvocationType=" << StringUtils::URLEncode(InvocationTypeMapper::GetNameForInvocationType(m_invocationType)) << "&";
  }
}
