...
SendTemplatedEmailResult::SendTemplatedEmailResult(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  *this = result;
}
...
SendTemplatedEmailResult& SendTemplatedEmailResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "SendTemplatedEmailResult"))
  {
    resultNode = rootNode.FirstChild("SendTemplatedEmailResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::SES::Model::SendTemplatedEmailResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
