  ...
  if (m_destinationsHasBeenSet) {
    if (m_destinations.empty()) {
      ...
      for (auto& item : m_destinations) {
        ss << "Destinations.member." << destinationsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        destinationsCount++;
      }
    }
  }
...
void SendRawEmailRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
