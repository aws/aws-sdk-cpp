...
GetIdentityMailFromDomainAttributesResult& GetIdentityMailFromDomainAttributesResult::operator=(
    const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!mailFromDomainAttributesNode.IsNull()) {
      ...
      while (!mailFromDomainAttributesEntry.IsNull()) {
        ...
        m_mailFromDomainAttributes[keyNode.GetText()] = valueNode;
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::SES::Model::GetIdentityMailFromDomainAttributesResult",
