...
IdentityDkimAttributes::IdentityDkimAttributes(const XmlNode& xmlNode) { *this = xmlNode; }
...
IdentityDkimAttributes& IdentityDkimAttributes::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!dkimEnabledNode.IsNull()) {
      m_dkimEnabled =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(dkimEnabledNode.GetText()).c_str()).c_str());
    ...
    if (!dkimVerificationStatusNode.IsNull()) {
      m_dkimVerificationStatus = VerificationStatusMapper::GetVerificationStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(dkimVerificationStatusNode.GetText()).c_str()));
...
void IdentityDkimAttributes::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_dkimVerificationStatusHasBeenSet) {
    oStream << location << index << locationValue << ".DkimVerificationStatus="
            << StringUtils::URLEncode(VerificationStatusMapper::GetNameForVerificationStatus(m_dkimVerificationStatus)) << "&";
  }

  if (m_dkimTokensHasBeenSet) {
    unsigned dkimTokensIdx = 1;
    for (auto& item : m_dkimTokens) {
      oStream << location << index << locationValue << ".DkimTokens.member." << dkimTokensIdx++ << "="
...
void IdentityDkimAttributes::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_dkimVerificationStatusHasBeenSet) {
    oStream << location << ".DkimVerificationStatus="
            << StringUtils::URLEncode(VerificationStatusMapper::GetNameForVerificationStatus(m_dkimVerificationStatus)) << "&";
  }
  if (m_dkimTokensHasBeenSet) {
    unsigned dkimTokensIdx = 1;
