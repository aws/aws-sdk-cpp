...
ReputationOptions::ReputationOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
ReputationOptions& ReputationOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!sendingEnabledNode.IsNull()) {
      m_sendingEnabled = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sendingEnabledNode.GetText()).c_str()).c_str());
    ...
    if (!reputationMetricsEnabledNode.IsNull()) {
      m_reputationMetricsEnabled = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(reputationMetricsEnabledNode.GetText()).c_str()).c_str());
    ...
    if (!lastFreshStartNode.IsNull()) {
      m_lastFreshStart = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastFreshStartNode.GetText()).c_str()).c_str(),
...
void ReputationOptions::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastFreshStartHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastFreshStart=" << StringUtils::URLEncode(m_lastFreshStart.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
