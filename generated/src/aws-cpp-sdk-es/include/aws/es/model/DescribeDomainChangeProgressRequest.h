...
class DescribeDomainChangeProgressRequest : public ElasticsearchServiceRequest {
 public:
  ...
  template <typename DomainNameT = Aws::String>
  DescribeDomainChangeProgressRequest& WithDomainName(DomainNameT&& value) {
  ...
  void SetChangeId(ChangeIdT&& value) {
    ...
    m_changeId = std::forward<ChangeIdT>(value);
  }
  template <typename ChangeIdT = Aws::String>
  DescribeDomainChangeProgressRequest& WithChangeId(ChangeIdT&& value) {
    SetChangeId(std::forward<ChangeIdT>(value));
    return *this;
  }
  ///@}
 private:
