...
class SelectResult {
 public:
  ...
  template <typename ItemsT = Aws::Vector<Item>>
  SelectResult& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = Item>
  SelectResult& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  SelectResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename ResponseMetadataT = ResponseMetadata>
  SelectResult& WithResponseMetadata(ResponseMetadataT&& value) {
