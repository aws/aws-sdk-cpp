...
class BatchDeleteAttributesRequest : public SimpleDBRequest {
 ...
 public:
  ...
  template <typename DomainNameT = Aws::String>
  BatchDeleteAttributesRequest& WithDomainName(DomainNameT&& value) {
  ...
  template <typename ItemsT = Aws::Vector<DeletableItem>>
  BatchDeleteAttributesRequest& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = DeletableItem>
  BatchDeleteAttributesRequest& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
