...
class BatchPutAttributesRequest : public SimpleDBRequest {
 ...
 public:
  ...
  template <typename DomainNameT = Aws::String>
  BatchPutAttributesRequest& WithDomainName(DomainNameT&& value) {
  ...
  template <typename ItemsT = Aws::Vector<ReplaceableItem>>
  BatchPutAttributesRequest& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = ReplaceableItem>
  BatchPutAttributesRequest& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
