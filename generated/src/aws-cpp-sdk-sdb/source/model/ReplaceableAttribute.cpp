...
ReplaceableAttribute::ReplaceableAttribute(const XmlNode& xmlNode) { *this = xmlNode; }
...
ReplaceableAttribute& ReplaceableAttribute::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!replaceNode.IsNull()) {
      m_replace =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(replaceNode.GetText()).c_str()).c_str());
