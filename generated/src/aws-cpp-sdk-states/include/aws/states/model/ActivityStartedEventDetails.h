...
class ActivityStartedEventDetails {
 public:
  ...
  void SetWorkerName(WorkerNameT&& value) {
    m_workerNameHasBeenSet = true;
    m_workerName = std::forward<WorkerNameT>(value);
  }
  template <typename WorkerNameT = Aws::String>
  ActivityStartedEventDetails& WithWorkerName(WorkerNameT&& value) {
    SetWorkerName(std::forward<WorkerNameT>(value));
    return *this;
  }
  ///@}
 private:
