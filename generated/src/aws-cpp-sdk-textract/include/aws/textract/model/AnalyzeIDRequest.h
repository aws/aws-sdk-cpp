...
class AnalyzeIDRequest : public TextractRequest {
 public:
  ...
  void SetDocumentPages(DocumentPagesT&& value) {
    m_documentPagesHasBeenSet = true;
    m_documentPages = std::forward<DocumentPagesT>(value);
  }
  template <typename DocumentPagesT = Aws::Vector<Document>>
  AnalyzeIDRequest& WithDocumentPages(DocumentPagesT&& value) {
    SetDocumentPages(std::forward<DocumentPagesT>(value));
    return *this;
  }
  template <typename DocumentPagesT = Document>
  AnalyzeIDRequest& AddDocumentPages(DocumentPagesT&& value) {
    m_documentPagesHasBeenSet = true;
    m_documentPages.emplace_back(std::forward<DocumentPagesT>(value));
    return *this;
  }
  ///@}
 private:
