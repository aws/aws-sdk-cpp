...
class DescribeWebAppResult {
 public:
  ...
  void SetWebApp(WebAppT&& value) {
    m_webAppHasBeenSet = true;
    m_webApp = std::forward<WebAppT>(value);
  }
  template <typename WebAppT = DescribedWebApp>
  DescribeWebAppResult& WithWebApp(WebAppT&& value) {
    SetWebApp(std::forward<WebAppT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  DescribeWebAppResult& WithRequestId(RequestIdT&& value) {
