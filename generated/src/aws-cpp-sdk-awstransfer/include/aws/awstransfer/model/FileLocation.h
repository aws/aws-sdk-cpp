...
class FileLocation {
 public:
  ...
  void SetS3FileLocation(S3FileLocationT&& value) {
    ...
    m_s3FileLocation = std::forward<S3FileLocationT>(value);
  }
  template <typename S3FileLocationT = S3FileLocation>
  FileLocation& WithS3FileLocation(S3FileLocationT&& value) {
    SetS3FileLocation(std::forward<S3FileLocationT>(value));
    return *this;
  }
  ///@}
  ...
  void SetEfsFileLocation(EfsFileLocationT&& value) {
    ...
    m_efsFileLocation = std::forward<EfsFileLocationT>(value);
  }
  template <typename EfsFileLocationT = EfsFileLocation>
  FileLocation& WithEfsFileLocation(EfsFileLocationT&& value) {
    SetEfsFileLocation(std::forward<EfsFileLocationT>(value));
    return *this;
  }
  ///@}
 private:
