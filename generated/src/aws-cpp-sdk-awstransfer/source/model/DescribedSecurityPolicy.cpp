...
DescribedSecurityPolicy::DescribedSecurityPolicy(JsonView jsonValue) { *this = jsonValue; }
...
DescribedSecurityPolicy& DescribedSecurityPolicy::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("SshHostKeyAlgorithms")) {
    ...
    for (unsigned sshHostKeyAlgorithmsIndex = 0; sshHostKeyAlgorithmsIndex < sshHostKeyAlgorithmsJsonList.GetLength();
         ++sshHostKeyAlgorithmsIndex) {
...
JsonValue DescribedSecurityPolicy::Jsonize() const {
  ...
  if (m_protocolsHasBeenSet) {
    ...
    for (unsigned protocolsIndex = 0; protocolsIndex < protocolsJsonList.GetLength(); ++protocolsIndex) {
      protocolsJsonList[protocolsIndex].AsString(
          SecurityPolicyProtocolMapper::GetNameForSecurityPolicyProtocol(m_protocols[protocolsIndex]));
    }
