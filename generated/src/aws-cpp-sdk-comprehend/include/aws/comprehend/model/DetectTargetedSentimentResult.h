...
class DetectTargetedSentimentResult {
 public:
  ...
  template <typename EntitiesT = Aws::Vector<TargetedSentimentEntity>>
  DetectTargetedSentimentResult& WithEntities(EntitiesT&& value) {
    SetEntities(std::forward<EntitiesT>(value));
    return *this;
  }
  template <typename EntitiesT = TargetedSentimentEntity>
  DetectTargetedSentimentResult& AddEntities(EntitiesT&& value) {
    m_entitiesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  DetectTargetedSentimentResult& WithRequestId(RequestIdT&& value) {
