...
class DetectToxicContentResult {
 public:
  ...
  template <typename ResultListT = Aws::Vector<ToxicLabels>>
  DetectToxicContentResult& WithResultList(ResultListT&& value) {
    SetResultList(std::forward<ResultListT>(value));
    return *this;
  }
  template <typename ResultListT = ToxicLabels>
  DetectToxicContentResult& AddResultList(ResultListT&& value) {
    m_resultListHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  DetectToxicContentResult& WithRequestId(RequestIdT&& value) {
