...
class DetectPiiEntitiesResult {
 public:
  ...
  template <typename EntitiesT = Aws::Vector<PiiEntity>>
  DetectPiiEntitiesResult& WithEntities(EntitiesT&& value) {
    SetEntities(std::forward<EntitiesT>(value));
    return *this;
  }
  template <typename EntitiesT = PiiEntity>
  DetectPiiEntitiesResult& AddEntities(EntitiesT&& value) {
    m_entitiesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  DetectPiiEntitiesResult& WithRequestId(RequestIdT&& value) {
