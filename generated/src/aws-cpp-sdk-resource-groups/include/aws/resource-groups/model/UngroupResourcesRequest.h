...
class UngroupResourcesRequest : public ResourceGroupsRequest {
 public:
  AWS_RESOURCEGROUPS_API UngroupResourcesRequest() = default;
  ...
  template <typename GroupT = Aws::String>
  UngroupResourcesRequest& WithGroup(GroupT&& value) {
  ...
  template <typename ResourceArnsT = Aws::Vector<Aws::String>>
  UngroupResourcesRequest& WithResourceArns(ResourceArnsT&& value) {
    SetResourceArns(std::forward<ResourceArnsT>(value));
    return *this;
  }
  template <typename ResourceArnsT = Aws::String>
  UngroupResourcesRequest& AddResourceArns(ResourceArnsT&& value) {
    m_resourceArnsHasBeenSet = true;
