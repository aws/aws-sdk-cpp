...
SearchFilter::SearchFilter(JsonView jsonValue) { *this = jsonValue; }

SearchFilter& SearchFilter::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("filterType")) {
    m_filterType = SearchFilterTypeMapper::GetSearchFilterTypeForName(jsonValue.GetString("filterType"));
...
JsonValue SearchFilter::Jsonize() const {
  ...
  if (m_filterTypeHasBeenSet) {
    payload.WithString("filterType", SearchFilterTypeMapper::GetNameForSearchFilterType(m_filterType));
  }
