...
/**
 */
class SampleWithResponseStreamRequest : public StreamingSagemakerJobRuntimeRequest {
 public:
  AWS_SAGEMAKERJOBRUNTIME_API SampleWithResponseStreamRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "SampleWithResponseStream"; }
  ...
  template <typename JobArnT = Aws::String>
  SampleWithResponseStreamRequest& WithJobArn(JobArnT&& value) {
  ...
  ///@{
  /**
   * <p>The trajectory ID for grouping turns into a single rollout. Each turn is
   * captured for later use.</p>
   */
  ...
  template <typename TrajectoryIdT = Aws::String>
  SampleWithResponseStreamRequest& WithTrajectoryId(TrajectoryIdT&& value) {
