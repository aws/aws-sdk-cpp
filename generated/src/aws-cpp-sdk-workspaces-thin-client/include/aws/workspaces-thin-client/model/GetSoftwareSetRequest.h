...
namespace Model
{
  ...
  /**
   */
  class GetSoftwareSetRequest : public WorkSpacesThinClientRequest
  {
  public:
    AWS_WORKSPACESTHINCLIENT_API GetSoftwareSetRequest();

    // Service request name is the Operation name which will send this request out,
    // each operation should has unique request name, so that we can get operation's name from this request.
    // Note: this is not true for response, multiple operations may have the same response name,
    // so we can not get operation's name from response.
    inline virtual const char* GetServiceRequestName() const override { return "GetSoftwareSet"; }
     ...
     * <p>The ID of the software set for which to return information.</p>
     */
    inline GetSoftwareSetRequest& WithId(const char* value) { SetId(value); return *this;}
