...
class PutCorsPolicyRequest : public MediaStoreRequest {
 public:
  ...
  template <typename ContainerNameT = Aws::String>
  PutCorsPolicyRequest& WithContainerName(ContainerNameT&& value) {
  ...
  template <typename CorsPolicyT = Aws::Vector<CorsRule>>
  PutCorsPolicyRequest& WithCorsPolicy(CorsPolicyT&& value) {
    SetCorsPolicy(std::forward<CorsPolicyT>(value));
    return *this;
  }
  template <typename CorsPolicyT = CorsRule>
  PutCorsPolicyRequest& AddCorsPolicy(CorsPolicyT&& value) {
    m_corsPolicyHasBeenSet = true;
