...
class CreateHsmResult {
 public:
  ...
  void SetHsm(HsmT&& value) {
    m_hsmHasBeenSet = true;
    m_hsm = std::forward<HsmT>(value);
  }
  template <typename HsmT = Hsm>
  CreateHsmResult& WithHsm(HsmT&& value) {
    SetHsm(std::forward<HsmT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  CreateHsmResult& WithRequestId(RequestIdT&& value) {
