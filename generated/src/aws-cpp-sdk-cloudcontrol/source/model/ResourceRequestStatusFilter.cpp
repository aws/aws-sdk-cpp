...
ResourceRequestStatusFilter::ResourceRequestStatusFilter(JsonView jsonValue) { *this = jsonValue; }
...
ResourceRequestStatusFilter& ResourceRequestStatusFilter::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("OperationStatuses")) {
    ...
    for (unsigned operationStatusesIndex = 0; operationStatusesIndex < operationStatusesJsonList.GetLength(); ++operationStatusesIndex) {
      m_operationStatuses.push_back(
          OperationStatusMapper::GetOperationStatusForName(operationStatusesJsonList[operationStatusesIndex].AsString()));
    }
...
JsonValue ResourceRequestStatusFilter::Jsonize() const {
  ...
  if (m_operationStatusesHasBeenSet) {
    ...
    for (unsigned operationStatusesIndex = 0; operationStatusesIndex < operationStatusesJsonList.GetLength(); ++operationStatusesIndex) {
      operationStatusesJsonList[operationStatusesIndex].AsString(
          OperationStatusMapper::GetNameForOperationStatus(m_operationStatuses[operationStatusesIndex]));
    }
