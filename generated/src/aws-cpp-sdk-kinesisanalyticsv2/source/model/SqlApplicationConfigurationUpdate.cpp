...
SqlApplicationConfigurationUpdate::SqlApplicationConfigurationUpdate(JsonView jsonValue) { *this = jsonValue; }
...
SqlApplicationConfigurationUpdate& SqlApplicationConfigurationUpdate::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ReferenceDataSourceUpdates")) {
    ...
    for (unsigned referenceDataSourceUpdatesIndex = 0; referenceDataSourceUpdatesIndex < referenceDataSourceUpdatesJsonList.GetLength();
         ++referenceDataSourceUpdatesIndex) {
...
JsonValue SqlApplicationConfigurationUpdate::Jsonize() const {
  ...
  if (m_referenceDataSourceUpdatesHasBeenSet) {
    ...
    for (unsigned referenceDataSourceUpdatesIndex = 0; referenceDataSourceUpdatesIndex < referenceDataSourceUpdatesJsonList.GetLength();
         ++referenceDataSourceUpdatesIndex) {
      referenceDataSourceUpdatesJsonList[referenceDataSourceUpdatesIndex].AsObject(
          m_referenceDataSourceUpdates[referenceDataSourceUpdatesIndex].Jsonize());
    }
