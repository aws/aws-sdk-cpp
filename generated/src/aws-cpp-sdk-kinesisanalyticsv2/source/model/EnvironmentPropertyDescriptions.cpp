...
EnvironmentPropertyDescriptions::EnvironmentPropertyDescriptions(JsonView jsonValue) { *this = jsonValue; }
...
EnvironmentPropertyDescriptions& EnvironmentPropertyDescriptions::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("PropertyGroupDescriptions")) {
    ...
    for (unsigned propertyGroupDescriptionsIndex = 0; propertyGroupDescriptionsIndex < propertyGroupDescriptionsJsonList.GetLength();
         ++propertyGroupDescriptionsIndex) {
...
JsonValue EnvironmentPropertyDescriptions::Jsonize() const {
  ...
  if (m_propertyGroupDescriptionsHasBeenSet) {
    ...
    for (unsigned propertyGroupDescriptionsIndex = 0; propertyGroupDescriptionsIndex < propertyGroupDescriptionsJsonList.GetLength();
         ++propertyGroupDescriptionsIndex) {
      propertyGroupDescriptionsJsonList[propertyGroupDescriptionsIndex].AsObject(
          m_propertyGroupDescriptions[propertyGroupDescriptionsIndex].Jsonize());
    }
