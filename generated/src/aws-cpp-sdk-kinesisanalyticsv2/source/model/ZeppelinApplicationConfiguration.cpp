...
ZeppelinApplicationConfiguration::ZeppelinApplicationConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
ZeppelinApplicationConfiguration& ZeppelinApplicationConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("CustomArtifactsConfiguration")) {
    ...
    for (unsigned customArtifactsConfigurationIndex = 0;
         customArtifactsConfigurationIndex < customArtifactsConfigurationJsonList.GetLength(); ++customArtifactsConfigurationIndex) {
...
JsonValue ZeppelinApplicationConfiguration::Jsonize() const {
  ...
  if (m_customArtifactsConfigurationHasBeenSet) {
    ...
    for (unsigned customArtifactsConfigurationIndex = 0;
         customArtifactsConfigurationIndex < customArtifactsConfigurationJsonList.GetLength(); ++customArtifactsConfigurationIndex) {
      customArtifactsConfigurationJsonList[customArtifactsConfigurationIndex].AsObject(
          m_customArtifactsConfiguration[customArtifactsConfigurationIndex].Jsonize());
    }
