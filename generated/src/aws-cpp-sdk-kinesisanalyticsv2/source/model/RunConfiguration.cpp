...
RunConfiguration::RunConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
RunConfiguration& RunConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("SqlRunConfigurations")) {
    ...
    for (unsigned sqlRunConfigurationsIndex = 0; sqlRunConfigurationsIndex < sqlRunConfigurationsJsonList.GetLength();
         ++sqlRunConfigurationsIndex) {
