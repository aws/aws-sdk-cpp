...
SqlApplicationConfigurationDescription::SqlApplicationConfigurationDescription(JsonView jsonValue) { *this = jsonValue; }
...
SqlApplicationConfigurationDescription& SqlApplicationConfigurationDescription::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("OutputDescriptions")) {
    ...
    for (unsigned outputDescriptionsIndex = 0; outputDescriptionsIndex < outputDescriptionsJsonList.GetLength();
         ++outputDescriptionsIndex) {
  ...
  if (jsonValue.ValueExists("ReferenceDataSourceDescriptions")) {
    ...
    for (unsigned referenceDataSourceDescriptionsIndex = 0;
         referenceDataSourceDescriptionsIndex < referenceDataSourceDescriptionsJsonList.GetLength();
         ++referenceDataSourceDescriptionsIndex) {
...
JsonValue SqlApplicationConfigurationDescription::Jsonize() const {
  ...
  if (m_referenceDataSourceDescriptionsHasBeenSet) {
    ...
    for (unsigned referenceDataSourceDescriptionsIndex = 0;
         referenceDataSourceDescriptionsIndex < referenceDataSourceDescriptionsJsonList.GetLength();
         ++referenceDataSourceDescriptionsIndex) {
      referenceDataSourceDescriptionsJsonList[referenceDataSourceDescriptionsIndex].AsObject(
          m_referenceDataSourceDescriptions[referenceDataSourceDescriptionsIndex].Jsonize());
    }
