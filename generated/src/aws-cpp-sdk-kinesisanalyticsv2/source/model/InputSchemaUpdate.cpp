...
InputSchemaUpdate::InputSchemaUpdate(JsonView jsonValue) { *this = jsonValue; }
...
InputSchemaUpdate& InputSchemaUpdate::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("RecordColumnUpdates")) {
    ...
    for (unsigned recordColumnUpdatesIndex = 0; recordColumnUpdatesIndex < recordColumnUpdatesJsonList.GetLength();
         ++recordColumnUpdatesIndex) {
