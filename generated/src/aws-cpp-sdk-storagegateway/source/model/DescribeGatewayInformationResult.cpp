...
DescribeGatewayInformationResult::DescribeGatewayInformationResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
DescribeGatewayInformationResult& DescribeGatewayInformationResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("GatewayNetworkInterfaces")) {
    ...
    for (unsigned gatewayNetworkInterfacesIndex = 0; gatewayNetworkInterfacesIndex < gatewayNetworkInterfacesJsonList.GetLength();
         ++gatewayNetworkInterfacesIndex) {
  ...
  if (jsonValue.ValueExists("SupportedGatewayCapacities")) {
    ...
    for (unsigned supportedGatewayCapacitiesIndex = 0; supportedGatewayCapacitiesIndex < supportedGatewayCapacitiesJsonList.GetLength();
         ++supportedGatewayCapacitiesIndex) {
      m_supportedGatewayCapacities.push_back(
          GatewayCapacityMapper::GetGatewayCapacityForName(supportedGatewayCapacitiesJsonList[supportedGatewayCapacitiesIndex].AsString()));
    }
