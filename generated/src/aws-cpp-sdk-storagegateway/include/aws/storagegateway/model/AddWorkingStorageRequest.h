...
class AddWorkingStorageRequest : public StorageGatewayRequest {
 public:
  ...
  template <typename GatewayARNT = Aws::String>
  AddWorkingStorageRequest& WithGatewayARN(GatewayARNT&& value) {
  ...
  template <typename DiskIdsT = Aws::Vector<Aws::String>>
  AddWorkingStorageRequest& WithDiskIds(DiskIdsT&& value) {
    SetDiskIds(std::forward<DiskIdsT>(value));
    return *this;
  }
  template <typename DiskIdsT = Aws::String>
  AddWorkingStorageRequest& AddDiskIds(DiskIdsT&& value) {
    m_diskIdsHasBeenSet = true;
