...
class CreateServiceSpecificCredentialRequest : public IAMRequest {
 ...
 public:
  ...
  template <typename UserNameT = Aws::String>
  CreateServiceSpecificCredentialRequest& WithUserName(UserNameT&& value) {
  ...
  template <typename ServiceNameT = Aws::String>
  CreateServiceSpecificCredentialRequest& WithServiceName(ServiceNameT&& value) {
   ...
   * is only valid for services that support long-term API keys and must be a
   * positive integer. When not specified, the credential will not expire.</p> <p>To
   * see which services support long-term API keys, refer to <a
   * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_api_keys_for_aws_services.html">API
   * keys for Amazon Web Services services</a> in the <i>IAM User Guide</i>.</p>
   */
  ...
  inline void SetCredentialAgeDays(int value) {
    m_credentialAgeDaysHasBeenSet = true;
    m_credentialAgeDays = value;
  }
  inline CreateServiceSpecificCredentialRequest& WithCredentialAgeDays(int value) {
    SetCredentialAgeDays(value);
    return *this;
  }
  ///@}
 private:
