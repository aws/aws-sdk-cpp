...
/**
 */
class UntagUserRequest : public IAMRequest {
 public:
  AWS_IAM_API UntagUserRequest() = default;
 ...
 public:
  ...
  template <typename UserNameT = Aws::String>
  UntagUserRequest& WithUserName(UserNameT&& value) {
  ...
  template <typename TagKeysT = Aws::Vector<Aws::String>>
  UntagUserRequest& WithTagKeys(TagKeysT&& value) {
    SetTagKeys(std::forward<TagKeysT>(value));
    return *this;
  }
  template <typename TagKeysT = Aws::String>
  UntagUserRequest& AddTagKeys(TagKeysT&& value) {
    m_tagKeysHasBeenSet = true;
