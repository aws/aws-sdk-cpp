...
/**
 */
class RejectDelegationRequestRequest : public IAMRequest {
 public:
  AWS_IAM_API RejectDelegationRequestRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "RejectDelegationRequest"; }
 ...
 public:
  ...
  template <typename DelegationRequestIdT = Aws::String>
  RejectDelegationRequestRequest& WithDelegationRequestId(DelegationRequestIdT&& value) {
  ...
  ///@{
  /**
   * <p>Optional notes explaining the reason for rejecting the delegation
   * request.</p>
   */
  ...
  template <typename NotesT = Aws::String>
  RejectDelegationRequestRequest& WithNotes(NotesT&& value) {
