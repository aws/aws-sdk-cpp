...
class GetAccountSummaryResult {
 public:
  ...
  void SetSummaryMap(SummaryMapT&& value) {
    m_summaryMapHasBeenSet = true;
    m_summaryMap = std::forward<SummaryMapT>(value);
  }
  template <typename SummaryMapT = Aws::Map<SummaryKeyType, int>>
  GetAccountSummaryResult& WithSummaryMap(SummaryMapT&& value) {
    SetSummaryMap(std::forward<SummaryMapT>(value));
    return *this;
  }
  inline GetAccountSummaryResult& AddSummaryMap(SummaryKeyType key, int value) {
    m_summaryMapHasBeenSet = true;
    m_summaryMap.emplace(key, value);
    return *this;
  }
  ///@}
  ...
  template <typename ResponseMetadataT = ResponseMetadata>
  GetAccountSummaryResult& WithResponseMetadata(ResponseMetadataT&& value) {
