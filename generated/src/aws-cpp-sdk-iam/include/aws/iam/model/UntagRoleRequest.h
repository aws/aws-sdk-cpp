...
class UntagRoleRequest : public IAMRequest {
 public:
  AWS_IAM_API UntagRoleRequest() = default;
 ...
 public:
  ...
  template <typename RoleNameT = Aws::String>
  UntagRoleRequest& WithRoleName(RoleNameT&& value) {
  ...
  template <typename TagKeysT = Aws::Vector<Aws::String>>
  UntagRoleRequest& WithTagKeys(TagKeysT&& value) {
    SetTagKeys(std::forward<TagKeysT>(value));
    return *this;
  }
  template <typename TagKeysT = Aws::String>
  UntagRoleRequest& AddTagKeys(TagKeysT&& value) {
    m_tagKeysHasBeenSet = true;
