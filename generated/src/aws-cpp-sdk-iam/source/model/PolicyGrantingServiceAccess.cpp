...
PolicyGrantingServiceAccess::PolicyGrantingServiceAccess(const XmlNode& xmlNode) { *this = xmlNode; }
...
PolicyGrantingServiceAccess& PolicyGrantingServiceAccess::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!policyTypeNode.IsNull()) {
      m_policyType = PolicyTypeMapper::GetPolicyTypeForName(
    ...
    if (!entityTypeNode.IsNull()) {
      m_entityType = PolicyOwnerEntityTypeMapper::GetPolicyOwnerEntityTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(entityTypeNode.GetText()).c_str()));
...
void PolicyGrantingServiceAccess::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_policyTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".PolicyType=" << StringUtils::URLEncode(PolicyTypeMapper::GetNameForPolicyType(m_policyType)) << "&";
  }
  ...
  if (m_entityTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".EntityType=" << StringUtils::URLEncode(PolicyOwnerEntityTypeMapper::GetNameForPolicyOwnerEntityType(m_entityType)) << "&";
  }
