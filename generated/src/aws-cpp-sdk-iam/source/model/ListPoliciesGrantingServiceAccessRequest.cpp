  ...
  if (m_serviceNamespacesHasBeenSet) {
    if (m_serviceNamespaces.empty()) {
      ...
      for (auto& item : m_serviceNamespaces) {
        ss << "ServiceNamespaces.member." << serviceNamespacesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void ListPoliciesGrantingServiceAccessRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
