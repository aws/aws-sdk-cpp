...
ServiceSpecificCredential::ServiceSpecificCredential(const XmlNode& xmlNode) { *this = xmlNode; }
...
void ServiceSpecificCredential::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_serviceCredentialSecretHasBeenSet) {
    oStream << location << index << locationValue
            << ".ServiceCredentialSecret=" << StringUtils::URLEncode(m_serviceCredentialSecret.c_str()) << "&";
  }
