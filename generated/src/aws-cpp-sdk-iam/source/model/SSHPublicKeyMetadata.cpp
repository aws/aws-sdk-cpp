...
SSHPublicKeyMetadata::SSHPublicKeyMetadata(const XmlNode& xmlNode) { *this = xmlNode; }
...
SSHPublicKeyMetadata& SSHPublicKeyMetadata::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!uploadDateNode.IsNull()) {
      m_uploadDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(uploadDateNode.GetText()).c_str()).c_str(),
...
void SSHPublicKeyMetadata::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_uploadDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".UploadDate=" << StringUtils::URLEncode(m_uploadDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
