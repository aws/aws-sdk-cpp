...
LoginProfile::LoginProfile(const XmlNode& xmlNode) { *this = xmlNode; }
...
LoginProfile& LoginProfile::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!passwordResetRequiredNode.IsNull()) {
      m_passwordResetRequired = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(passwordResetRequiredNode.GetText()).c_str()).c_str());
