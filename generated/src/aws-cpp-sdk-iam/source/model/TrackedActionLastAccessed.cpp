...
TrackedActionLastAccessed::TrackedActionLastAccessed(const XmlNode& xmlNode) { *this = xmlNode; }
...
TrackedActionLastAccessed& TrackedActionLastAccessed::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lastAccessedTimeNode.IsNull()) {
      m_lastAccessedTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastAccessedTimeNode.GetText()).c_str()).c_str(),
...
void TrackedActionLastAccessed::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_lastAccessedTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastAccessedTime=" << StringUtils::URLEncode(m_lastAccessedTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
