...
AccessDetail::AccessDetail(const XmlNode& xmlNode) { *this = xmlNode; }
...
AccessDetail& AccessDetail::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lastAuthenticatedTimeNode.IsNull()) {
      m_lastAuthenticatedTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastAuthenticatedTimeNode.GetText()).c_str()).c_str(),
    ...
    if (!totalAuthenticatedEntitiesNode.IsNull()) {
      m_totalAuthenticatedEntities = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalAuthenticatedEntitiesNode.GetText()).c_str()).c_str());
...
void AccessDetail::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastAuthenticatedTimeHasBeenSet) {
    oStream << location << index << locationValue << ".LastAuthenticatedTime="
            << StringUtils::URLEncode(m_lastAuthenticatedTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
...
void AccessDetail::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_lastAuthenticatedTimeHasBeenSet) {
    oStream << location << ".LastAuthenticatedTime="
            << StringUtils::URLEncode(m_lastAuthenticatedTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  if (m_totalAuthenticatedEntitiesHasBeenSet) {
