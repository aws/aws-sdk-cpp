...
AccessKeyLastUsed::AccessKeyLastUsed(const XmlNode& xmlNode) { *this = xmlNode; }
...
AccessKeyLastUsed& AccessKeyLastUsed::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lastUsedDateNode.IsNull()) {
      m_lastUsedDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastUsedDateNode.GetText()).c_str()).c_str(),
...
void AccessKeyLastUsed::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_lastUsedDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastUsedDate=" << StringUtils::URLEncode(m_lastUsedDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
