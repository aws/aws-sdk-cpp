...
GetSAMLProviderResult::GetSAMLProviderResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
GetSAMLProviderResult& GetSAMLProviderResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!assertionEncryptionModeNode.IsNull()) {
      m_assertionEncryptionMode = AssertionEncryptionModeTypeMapper::GetAssertionEncryptionModeTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(assertionEncryptionModeNode.GetText()).c_str()));
