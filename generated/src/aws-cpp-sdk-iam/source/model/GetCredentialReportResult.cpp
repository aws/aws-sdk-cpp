...
GetCredentialReportResult::GetCredentialReportResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
GetCredentialReportResult& GetCredentialReportResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!reportFormatNode.IsNull()) {
      m_reportFormat = ReportFormatTypeMapper::GetReportFormatTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(reportFormatNode.GetText()).c_str()));
      m_reportFormatHasBeenSet = true;
    }
    XmlNode generatedTimeNode = resultNode.FirstChild("GeneratedTime");
    if (!generatedTimeNode.IsNull()) {
      m_generatedTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(generatedTimeNode.GetText()).c_str()).c_str(),
