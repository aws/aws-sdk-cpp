...
EntityInfo::EntityInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
EntityInfo& EntityInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = PolicyOwnerEntityTypeMapper::GetPolicyOwnerEntityTypeForName(
...
void EntityInfo::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(PolicyOwnerEntityTypeMapper::GetNameForPolicyOwnerEntityType(m_type)) << "&";
  }
