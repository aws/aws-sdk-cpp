  ...
  if (m_clientIDListHasBeenSet) {
    if (m_clientIDList.empty()) {
      ...
      for (auto& item : m_clientIDList) {
        ss << "ClientIDList.member." << clientIDListCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        clientIDListCount++;
      }
    }
  }
...
void CreateOpenIDConnectProviderRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
