...
EvaluationResult::EvaluationResult(const XmlNode& xmlNode) { *this = xmlNode; }
...
EvaluationResult& EvaluationResult::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!evalDecisionNode.IsNull()) {
      m_evalDecision = PolicyEvaluationDecisionTypeMapper::GetPolicyEvaluationDecisionTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(evalDecisionNode.GetText()).c_str()));
...
void EvaluationResult::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_evalDecisionHasBeenSet) {
    oStream << location << index << locationValue << ".EvalDecision="
            << StringUtils::URLEncode(PolicyEvaluationDecisionTypeMapper::GetNameForPolicyEvaluationDecisionType(m_evalDecision)) << "&";
  }
  ...
  if (m_missingContextValuesHasBeenSet) {
    ...
    for (auto& item : m_missingContextValues) {
      oStream << location << index << locationValue << ".MissingContextValues.member." << missingContextValuesIdx++ << "="
  ...
  if (m_evalDecisionDetailsHasBeenSet) {
    ...
    for (auto& item : m_evalDecisionDetails) {
      oStream << location << index << locationValue << ".EvalDecisionDetails.entry." << evalDecisionDetailsIdx
...
void EvaluationResult::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_evalDecisionDetailsHasBeenSet) {
    unsigned evalDecisionDetailsIdx = 1;
    for (auto& item : m_evalDecisionDetails) {
      oStream << location << ".EvalDecisionDetails.entry." << evalDecisionDetailsIdx
              << ".key=" << StringUtils::URLEncode(item.first.c_str()) << "&";
