...
ManagedPolicyDetail::ManagedPolicyDetail(const XmlNode& xmlNode) { *this = xmlNode; }
...
ManagedPolicyDetail& ManagedPolicyDetail::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!attachmentCountNode.IsNull()) {
      m_attachmentCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(attachmentCountNode.GetText()).c_str()).c_str());
    ...
    if (!permissionsBoundaryUsageCountNode.IsNull()) {
      m_permissionsBoundaryUsageCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(permissionsBoundaryUsageCountNode.GetText()).c_str()).c_str());
    ...
    if (!isAttachableNode.IsNull()) {
      m_isAttachable =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isAttachableNode.GetText()).c_str()).c_str());
