...
PermissionsBoundaryDecisionDetail::PermissionsBoundaryDecisionDetail(const XmlNode& xmlNode) { *this = xmlNode; }
...
PermissionsBoundaryDecisionDetail& PermissionsBoundaryDecisionDetail::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!allowedByPermissionsBoundaryNode.IsNull()) {
      m_allowedByPermissionsBoundary = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(allowedByPermissionsBoundaryNode.GetText()).c_str()).c_str());
...
void PermissionsBoundaryDecisionDetail::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
