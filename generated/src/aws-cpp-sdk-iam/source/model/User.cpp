...
User& User::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!passwordLastUsedNode.IsNull()) {
      m_passwordLastUsed =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(passwordLastUsedNode.GetText()).c_str()).c_str(),
                   Aws::Utils::DateFormat::ISO_8601);
...
void User::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_passwordLastUsedHasBeenSet) {
    oStream << location << index << locationValue
            << ".PasswordLastUsed=" << StringUtils::URLEncode(m_passwordLastUsed.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
