...
OrganizationsDecisionDetail::OrganizationsDecisionDetail(const XmlNode& xmlNode) { *this = xmlNode; }
...
OrganizationsDecisionDetail& OrganizationsDecisionDetail::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!allowedByOrganizationsNode.IsNull()) {
      m_allowedByOrganizations = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(allowedByOrganizationsNode.GetText()).c_str()).c_str());
...
void OrganizationsDecisionDetail::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
