...
ServiceLastAccessed::ServiceLastAccessed(const XmlNode& xmlNode) { *this = xmlNode; }
...
ServiceLastAccessed& ServiceLastAccessed::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lastAuthenticatedNode.IsNull()) {
      m_lastAuthenticated =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastAuthenticatedNode.GetText()).c_str()).c_str(),
...
void ServiceLastAccessed::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastAuthenticatedHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastAuthenticated=" << StringUtils::URLEncode(m_lastAuthenticated.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
  ...
  if (m_lastAuthenticatedEntityHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastAuthenticatedEntity=" << StringUtils::URLEncode(m_lastAuthenticatedEntity.c_str()) << "&";
  }

  if (m_lastAuthenticatedRegionHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastAuthenticatedRegion=" << StringUtils::URLEncode(m_lastAuthenticatedRegion.c_str()) << "&";
  }
  ...
  if (m_trackedActionsLastAccessedHasBeenSet) {
    ...
    for (auto& item : m_trackedActionsLastAccessed) {
      ...
      trackedActionsLastAccessedSs << location << index << locationValue << ".TrackedActionsLastAccessed.member."
                                   << trackedActionsLastAccessedIdx++;
