...
/**
 * <p>Specifies the jobs to cancel and the reason for the
 * cancellation.</p><p><h3>See Also:</h3>   <a
 * href="http://docs.aws.amazon.com/goto/WebAPI/batch-2016-08-10/CancelJobsRequest">AWS
 * API Reference</a></p>
 */
class CancelJobsRequest : public BatchRequest {
 public:
  AWS_BATCH_API CancelJobsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "CancelJobs"; }
  ...
  template <typename JobsT = Aws::Vector<Aws::String>>
  CancelJobsRequest& WithJobs(JobsT&& value) {
    SetJobs(std::forward<JobsT>(value));
    return *this;
  }
  template <typename JobsT = Aws::String>
  CancelJobsRequest& AddJobs(JobsT&& value) {
    m_jobsHasBeenSet = true;
  ...
  template <typename ReasonT = Aws::String>
  CancelJobsRequest& WithReason(ReasonT&& value) {
