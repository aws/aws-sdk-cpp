...
class UpdateComputeEnvironmentRequest : public BatchRequest {
 public:
   ...
   * billing charges, for example, if they have running instances due to jobs that
   * are still executing or a non-zero <code>minvCpus</code> setting. To prevent
   * additional charges, disable and delete the compute environment.</p>
  ...
  inline UpdateComputeEnvironmentRequest& WithState(CEState value) {
    SetState(value);
    return *this;
  }
  ///@}
  ...
  inline UpdateComputeEnvironmentRequest& WithUnmanagedvCpus(int value) {
