...
Aws::String UpdateJobQueueRequest::SerializePayload() const {
  ...
  if (m_computeEnvironmentOrderHasBeenSet) {
    ...
    for (unsigned computeEnvironmentOrderIndex = 0; computeEnvironmentOrderIndex < computeEnvironmentOrderJsonList.GetLength();
         ++computeEnvironmentOrderIndex) {
      computeEnvironmentOrderJsonList[computeEnvironmentOrderIndex].AsObject(
          m_computeEnvironmentOrder[computeEnvironmentOrderIndex].Jsonize());
    }
  ...
  if (m_serviceEnvironmentOrderHasBeenSet) {
    ...
    for (unsigned serviceEnvironmentOrderIndex = 0; serviceEnvironmentOrderIndex < serviceEnvironmentOrderJsonList.GetLength();
         ++serviceEnvironmentOrderIndex) {
      serviceEnvironmentOrderJsonList[serviceEnvironmentOrderIndex].AsObject(
          m_serviceEnvironmentOrder[serviceEnvironmentOrderIndex].Jsonize());
    }
  ...
  if (m_jobStateTimeLimitActionsHasBeenSet) {
    ...
    for (unsigned jobStateTimeLimitActionsIndex = 0; jobStateTimeLimitActionsIndex < jobStateTimeLimitActionsJsonList.GetLength();
         ++jobStateTimeLimitActionsIndex) {
      jobStateTimeLimitActionsJsonList[jobStateTimeLimitActionsIndex].AsObject(
          m_jobStateTimeLimitActions[jobStateTimeLimitActionsIndex].Jsonize());
    }
