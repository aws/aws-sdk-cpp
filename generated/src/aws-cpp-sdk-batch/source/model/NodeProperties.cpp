...
NodeProperties::NodeProperties(JsonView jsonValue) { *this = jsonValue; }
...
NodeProperties& NodeProperties::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("nodeRangeProperties")) {
    ...
    for (unsigned nodeRangePropertiesIndex = 0; nodeRangePropertiesIndex < nodeRangePropertiesJsonList.GetLength();
         ++nodeRangePropertiesIndex) {
