...
class AWS_MARKETPLACEMETERING_API MarketplaceMeteringClient : public Aws::Client::AWSJsonClient,
 ...
 public:
   ...
   * event. Usage records aren't accepted 24 hours or more after an event.</p> <p>
  ...
  /**
   * <p>As a seller, your software hosted in the buyer's Amazon Web Services account
   ...
   * credentials to sign the API request.</p> <ul> <li> <p>For <b>Amazon EC2</b>
   * deployments, your software must use the <a
   * href="https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/iam-roles-for-amazon-ec2.html">IAM
   * role for Amazon EC2</a> to sign the API call for <code>MeterUsage</code> API
   * operation.</p> </li> <li> <p>For <b>Amazon EKS</b> deployments, your software
   * must use <a
   ...
   * Pod Identity</a>, the node role, or long-term access keys is not supported.</p>
   * </li> <li> <p>For <b>Amazon ECS</b> deployments, your software must use <a
   * href="https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-iam-roles.html">Amazon
   * ECS task IAM</a> role to sign the API call for the <code>MeterUsage</code> API
   * operation. Using the node role or long-term access keys are not supported.</p>
   * </li> <li> <p>For <b>Amazon Bedrock AgentCore Runtime</b> deployments, your
   * software must use the <a
   ...
   * Runtime execution role</a> to sign the API call for the <code>MeterUsage</code>
   * API operation. Long-term access keys are not supported.</p> </li> </ul> <p>The
   * handling of <code>MeterUsage</code> requests varies between Amazon Bedrock
   * AgentCore Runtime and non-Amazon Bedrock AgentCore deployments.</p> <ul> <li>
   * <p>For <b>non-Amazon Bedrock AgentCore Runtime</b> deployments, you can only
   * report usage once per hour for each dimension. For AMI-based products, this is
   * per dimension and per EC2 instance. For container products, this is per
   * dimension and per ECS task or EKS pod. You can't modify values after they're
   * recorded. If you report usage before a current hour ends, you will be unable to
   * report additional usage until the next hour begins. The <code>Timestamp</code>
   * request parameter is rounded down to the hour and used to enforce this
   * once-per-hour rule for idempotency. For requests that are identical after the
   * <code>Timestamp</code> is rounded down, the API is idempotent and returns the
   * metering record ID.</p> </li> <li> <p>For <b>Amazon Bedrock AgentCore
   * Runtime</b> deployments, you can report usage multiple times per hour for the
   * same dimension. You do not need to aggregate metering records by the hour. You
   * must include an idempotency token in the <code>ClientToken</code> request
   * parameter. If using an Amazon SDK or the Amazon Web Services CLI, you must use
   * the latest version which automatically includes an idempotency token in the
   * <code>ClientToken</code> request parameter so that the request is processed
   * successfully. The <code>Timestamp</code> request parameter is not rounded down
   * to the hour and is not used for duplicate validation. Requests with duplicate
   * <code>Timestamps</code> are aggregated as long as the <code>ClientToken</code>
   * is unique.</p> </li> </ul> <p>If you submit records more than six hours after
   * events occur, the records won't be accepted. The timestamp in your request
   * determines when an event is recorded.</p> <p>You can optionally include multiple
   * usage allocations, to provide customers with usage data split into buckets by
   * tags that you define or allow the customer to define.</p> <p>For Amazon Web
   * Services Regions that support <code>MeterUsage</code>, see <a
  ...
  virtual std::shared_ptr<MarketplaceMeteringEndpointProviderBase>& accessEndpointProvider();
