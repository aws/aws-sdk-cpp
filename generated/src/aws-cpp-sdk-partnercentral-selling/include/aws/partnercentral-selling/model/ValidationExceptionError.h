...
namespace Model
{
   ...
   * <li> <p> <i>INVALID_ENUM_VALUE:</i> The enum field value isn't an accepted
   ...
   * </p> </li> <li> <p> <i>INVALID_VALUE:</i> The value isn't valid.</p> <p>Fix:
   * Confirm that the value meets the expected criteria and is within the allowable
   * range or set. </p> </li> <li> <p> <i>TOO_MANY_VALUES:</i> There are too many
   * values in a field that expects fewer entries.</p> <p>Fix: Reduce the number of
   * values to match the expected limit. </p> </li> <li> <p>
   ...
   * state, and that you have the necessary permissions to perform it. </p> </li>
   * <li> <p> <i>DUPLICATE_KEY_VALUE:</i> The value in a field duplicates a value
   * that must be unique.</p> <p>Fix: Verify that the value is unique and doesn't
