...
namespace Model
{
  ...
  public:
     ...
     * read-only and only applicable for partner referrals. Valid values: </p> <ul>
     * <li> <p> Pending Submission: Not submitted for validation (editable). </p> </li>
     * <li> <p> Submitted: Submitted for validation and not yet Amazon Web Services
     * reviewed (read-only). </p> </li> <li> <p> In Review: Undergoing Amazon Web
     * Services validation (read-only). </p> </li> <li> <p> Action Required: Address
     * any issues Amazon Web Services highlights. Use the
     * <code>UpdateOpportunity</code> API action to update the opportunity, and ensure
     * you make all required changes. Only these fields are editable when the
     * <code>Lifecycle.ReviewStatus</code> is <code>Action Required</code>: </p> <ul>
     * <li> <p>Customer.Account.Address.City</p> </li> <li>
