...
Aws::String ListOpportunitiesRequest::SerializePayload() const {
  ...
  if (m_lifeCycleReviewStatusHasBeenSet) {
    ...
    for (unsigned lifeCycleReviewStatusIndex = 0; lifeCycleReviewStatusIndex < lifeCycleReviewStatusJsonList.GetLength();
         ++lifeCycleReviewStatusIndex) {
      lifeCycleReviewStatusJsonList[lifeCycleReviewStatusIndex].AsString(
          ReviewStatusMapper::GetNameForReviewStatus(m_lifeCycleReviewStatus[lifeCycleReviewStatusIndex]));
    }
  ...
  if (m_customerCompanyNameHasBeenSet) {
    ...
    for (unsigned customerCompanyNameIndex = 0; customerCompanyNameIndex < customerCompanyNameJsonList.GetLength();
         ++customerCompanyNameIndex) {
