...
/**
 */
class ListMicrovmImageVersionsRequest : public LambdaMicrovmsRequest {
 public:
  AWS_LAMBDAMICROVMS_API ListMicrovmImageVersionsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListMicrovmImageVersions"; }
  ...
  inline ListMicrovmImageVersionsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  template <typename NextTokenT = Aws::String>
  ListMicrovmImageVersionsRequest& WithNextToken(NextTokenT&& value) {
  ...
  ///@{
  /**
   * <p>The unique identifier (ARN or ID) of the MicroVM image to list versions
   * for.</p>
   */
  ...
  template <typename ImageIdentifierT = Aws::String>
  ListMicrovmImageVersionsRequest& WithImageIdentifier(ImageIdentifierT&& value) {
