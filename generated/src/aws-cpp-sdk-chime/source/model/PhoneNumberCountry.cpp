...
PhoneNumberCountry::PhoneNumberCountry(JsonView jsonValue) { *this = jsonValue; }
...
PhoneNumberCountry& PhoneNumberCountry::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("SupportedPhoneNumberTypes")) {
    ...
    for (unsigned supportedPhoneNumberTypesIndex = 0; supportedPhoneNumberTypesIndex < supportedPhoneNumberTypesJsonList.GetLength();
         ++supportedPhoneNumberTypesIndex) {
      m_supportedPhoneNumberTypes.push_back(
          PhoneNumberTypeMapper::GetPhoneNumberTypeForName(supportedPhoneNumberTypesJsonList[supportedPhoneNumberTypesIndex].AsString()));
    }
...
JsonValue PhoneNumberCountry::Jsonize() const {
  ...
  if (m_supportedPhoneNumberTypesHasBeenSet) {
    ...
    for (unsigned supportedPhoneNumberTypesIndex = 0; supportedPhoneNumberTypesIndex < supportedPhoneNumberTypesJsonList.GetLength();
         ++supportedPhoneNumberTypesIndex) {
      supportedPhoneNumberTypesJsonList[supportedPhoneNumberTypesIndex].AsString(
          PhoneNumberTypeMapper::GetNameForPhoneNumberType(m_supportedPhoneNumberTypes[supportedPhoneNumberTypesIndex]));
    }
