...
class BatchCreateRoomMembershipResult {
 public:
  ...
  template <typename ErrorsT = Aws::Vector<MemberError>>
  BatchCreateRoomMembershipResult& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = MemberError>
  BatchCreateRoomMembershipResult& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchCreateRoomMembershipResult& WithRequestId(RequestIdT&& value) {
