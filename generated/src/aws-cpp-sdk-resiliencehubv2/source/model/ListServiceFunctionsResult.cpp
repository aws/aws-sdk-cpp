...
ListServiceFunctionsResult::ListServiceFunctionsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }

ListServiceFunctionsResult& ListServiceFunctionsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("serviceFunctions")) {
    ...
    for (unsigned serviceFunctionsIndex = 0; serviceFunctionsIndex < serviceFunctionsJsonList.GetLength(); ++serviceFunctionsIndex) {
      m_serviceFunctions.push_back(serviceFunctionsJsonList[serviceFunctionsIndex].AsObject());
    }
