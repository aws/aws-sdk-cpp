...
ListPoliciesResult& ListPoliciesResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("policySummaries")) {
    Aws::Utils::Array<JsonView> policySummariesJsonList = jsonValue.GetArray("policySummaries");
    for (unsigned policySummariesIndex = 0; policySummariesIndex < policySummariesJsonList.GetLength(); ++policySummariesIndex) {
      m_policySummaries.push_back(policySummariesJsonList[policySummariesIndex].AsObject());
    }
