...
/**
 */
class DeleteServiceFunctionResourcesRequest : public Resiliencehubv2Request {
 public:
  AWS_RESILIENCEHUBV2_API DeleteServiceFunctionResourcesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteServiceFunctionResources"; }
  ...
  template <typename ServiceArnT = Aws::String>
  DeleteServiceFunctionResourcesRequest& WithServiceArn(ServiceArnT&& value) {
  ...
  template <typename ServiceFunctionIdT = Aws::String>
  DeleteServiceFunctionResourcesRequest& WithServiceFunctionId(ServiceFunctionIdT&& value) {
  ...
  template <typename ResourcesT = Aws::Vector<Aws::String>>
  DeleteServiceFunctionResourcesRequest& WithResources(ResourcesT&& value) {
    SetResources(std::forward<ResourcesT>(value));
    return *this;
  }
  template <typename ResourcesT = Aws::String>
  DeleteServiceFunctionResourcesRequest& AddResources(ResourcesT&& value) {
    m_resourcesHasBeenSet = true;
