...
/**
 */
class ListUserJourneysRequest : public Resiliencehubv2Request {
 public:
  AWS_RESILIENCEHUBV2_API ListUserJourneysRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListUserJourneys"; }
  ...
  template <typename SystemArnT = Aws::String>
  ListUserJourneysRequest& WithSystemArn(SystemArnT&& value) {
  ...
  inline ListUserJourneysRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  template <typename NextTokenT = Aws::String>
  ListUserJourneysRequest& WithNextToken(NextTokenT&& value) {
