...
/**
 */
class CreateInputSourceRequest : public Resiliencehubv2Request {
 public:
  AWS_RESILIENCEHUBV2_API CreateInputSourceRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "CreateInputSource"; }
  ...
  template <typename ServiceArnT = Aws::String>
  CreateInputSourceRequest& WithServiceArn(ServiceArnT&& value) {
  ...
  template <typename ResourceConfigurationT = ResourceConfiguration>
  CreateInputSourceRequest& WithResourceConfiguration(ResourceConfigurationT&& value) {
  ...
  template <typename ClientTokenT = Aws::String>
  CreateInputSourceRequest& WithClientToken(ClientTokenT&& value) {
