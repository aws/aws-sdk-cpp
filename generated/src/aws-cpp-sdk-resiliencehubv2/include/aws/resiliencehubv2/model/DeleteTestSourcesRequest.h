...
/**
 */
class DeleteTestSourcesRequest : public Resiliencehubv2Request {
 public:
  AWS_RESILIENCEHUBV2_API DeleteTestSourcesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteTestSources"; }
  ...
  template <typename TestIdT = Aws::String>
  DeleteTestSourcesRequest& WithTestId(TestIdT&& value) {
  ...
  template <typename ServiceArnT = Aws::String>
  DeleteTestSourcesRequest& WithServiceArn(ServiceArnT&& value) {
  ...
  template <typename TestSourcesT = Aws::Vector<TestSourceInput>>
  DeleteTestSourcesRequest& WithTestSources(TestSourcesT&& value) {
    SetTestSources(std::forward<TestSourcesT>(value));
    return *this;
  }
  template <typename TestSourcesT = TestSourceInput>
  DeleteTestSourcesRequest& AddTestSources(TestSourcesT&& value) {
    m_testSourcesHasBeenSet = true;
