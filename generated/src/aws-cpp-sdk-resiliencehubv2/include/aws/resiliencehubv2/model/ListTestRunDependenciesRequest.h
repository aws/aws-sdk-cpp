...
/**
 */
class ListTestRunDependenciesRequest : public Resiliencehubv2Request {
 public:
  AWS_RESILIENCEHUBV2_API ListTestRunDependenciesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListTestRunDependencies"; }
  ...
  template <typename TestRunIdT = Aws::String>
  ListTestRunDependenciesRequest& WithTestRunId(TestRunIdT&& value) {
  ...
  template <typename ServiceArnT = Aws::String>
  ListTestRunDependenciesRequest& WithServiceArn(ServiceArnT&& value) {
  ...
  inline ListTestRunDependenciesRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  template <typename NextTokenT = Aws::String>
  ListTestRunDependenciesRequest& WithNextToken(NextTokenT&& value) {
