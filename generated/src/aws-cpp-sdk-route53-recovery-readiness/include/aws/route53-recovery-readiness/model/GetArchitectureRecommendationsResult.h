...
class GetArchitectureRecommendationsResult {
 public:
  ...
  AWS_ROUTE53RECOVERYREADINESS_API GetArchitectureRecommendationsResult& operator=(
      const Aws::AmazonWebServiceResult<Aws::Utils::Json::JsonValue>& result);
  ...
  void SetLastAuditTimestamp(LastAuditTimestampT&& value) {
    ...
    m_lastAuditTimestamp = std::forward<LastAuditTimestampT>(value);
  }
  template <typename LastAuditTimestampT = Aws::Utils::DateTime>
  GetArchitectureRecommendationsResult& WithLastAuditTimestamp(LastAuditTimestampT&& value) {
    SetLastAuditTimestamp(std::forward<LastAuditTimestampT>(value));
    return *this;
  }
  ///@}
