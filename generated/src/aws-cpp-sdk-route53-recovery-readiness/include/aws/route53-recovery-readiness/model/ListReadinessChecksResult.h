...
class ListReadinessChecksResult {
 public:
  ...
  AWS_ROUTE53RECOVERYREADINESS_API ListReadinessChecksResult& operator=(
  ...
  void SetReadinessChecks(ReadinessChecksT&& value) {
    ...
    m_readinessChecks = std::forward<ReadinessChecksT>(value);
  }
  template <typename ReadinessChecksT = Aws::Vector<ReadinessCheckOutput>>
  ListReadinessChecksResult& WithReadinessChecks(ReadinessChecksT&& value) {
    SetReadinessChecks(std::forward<ReadinessChecksT>(value));
    return *this;
  }
  ...
  ListReadinessChecksResult& AddReadinessChecks(ReadinessChecksT&& value) {
    m_readinessChecksHasBeenSet = true;
    m_readinessChecks.emplace_back(std::forward<ReadinessChecksT>(value));
    return *this;
  }
  ///@}
