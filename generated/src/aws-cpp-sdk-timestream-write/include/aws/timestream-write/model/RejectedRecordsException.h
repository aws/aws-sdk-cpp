...
class RejectedRecordsException {
 public:
  ...
  template <typename MessageT = Aws::String>
  RejectedRecordsException& WithMessage(MessageT&& value) {
  ...
  void SetRejectedRecords(RejectedRecordsT&& value) {
    m_rejectedRecordsHasBeenSet = true;
    m_rejectedRecords = std::forward<RejectedRecordsT>(value);
  }
  template <typename RejectedRecordsT = Aws::Vector<RejectedRecord>>
  RejectedRecordsException& WithRejectedRecords(RejectedRecordsT&& value) {
    SetRejectedRecords(std::forward<RejectedRecordsT>(value));
    return *this;
  }
  template <typename RejectedRecordsT = RejectedRecord>
  RejectedRecordsException& AddRejectedRecords(RejectedRecordsT&& value) {
    m_rejectedRecordsHasBeenSet = true;
    m_rejectedRecords.emplace_back(std::forward<RejectedRecordsT>(value));
    return *this;
  }
  ///@}
 private:
