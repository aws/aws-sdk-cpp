...
class WriteRecordsResult {
 public:
  ...
  void SetRecordsIngested(RecordsIngestedT&& value) {
    m_recordsIngestedHasBeenSet = true;
    m_recordsIngested = std::forward<RecordsIngestedT>(value);
  }
  template <typename RecordsIngestedT = RecordsIngested>
  WriteRecordsResult& WithRecordsIngested(RecordsIngestedT&& value) {
    SetRecordsIngested(std::forward<RecordsIngestedT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  WriteRecordsResult& WithRequestId(RequestIdT&& value) {
