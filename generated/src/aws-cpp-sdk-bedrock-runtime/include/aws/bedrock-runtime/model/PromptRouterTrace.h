...
class PromptRouterTrace {
 public:
  ...
  void SetInvokedModelId(InvokedModelIdT&& value) {
    m_invokedModelIdHasBeenSet = true;
    m_invokedModelId = std::forward<InvokedModelIdT>(value);
  }
  template <typename InvokedModelIdT = Aws::String>
  PromptRouterTrace& WithInvokedModelId(InvokedModelIdT&& value) {
    SetInvokedModelId(std::forward<InvokedModelIdT>(value));
    return *this;
  }
  ///@}
 private:
