...
class GuardrailTextBlock {
 public:
  ...
  void SetQualifiers(QualifiersT&& value) {
    ...
    m_qualifiers = std::forward<QualifiersT>(value);
  }
  template <typename QualifiersT = Aws::Vector<GuardrailContentQualifier>>
  GuardrailTextBlock& WithQualifiers(QualifiersT&& value) {
    SetQualifiers(std::forward<QualifiersT>(value));
    return *this;
  }
  inline GuardrailTextBlock& AddQualifiers(GuardrailContentQualifier value) {
    m_qualifiersHasBeenSet = true;
    m_qualifiers.push_back(value);
    return *this;
  }
  ///@}
 private:
