...
GuardrailTraceAssessment::GuardrailTraceAssessment(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue GuardrailTraceAssessment::Jsonize() const {
  ...
  if (m_outputAssessmentsHasBeenSet) {
    ...
    for (auto& outputAssessmentsItem : m_outputAssessments) {
      ...
      for (unsigned guardrailAssessmentListIndex = 0; guardrailAssessmentListIndex < guardrailAssessmentListJsonList.GetLength();
           ++guardrailAssessmentListIndex) {
        guardrailAssessmentListJsonList[guardrailAssessmentListIndex].AsObject(
            outputAssessmentsItem.second[guardrailAssessmentListIndex].Jsonize());
      }
