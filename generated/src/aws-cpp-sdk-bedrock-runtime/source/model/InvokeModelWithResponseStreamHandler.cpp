...
void InvokeModelWithResponseStreamHandler::HandleEventInMessage() {
  ...
  switch (InvokeModelWithResponseStreamEventMapper::GetInvokeModelWithResponseStreamEventTypeForName(
...
void InvokeModelWithResponseStreamHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(INVOKEMODELWITHRESPONSESTREAM_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(INVOKEMODELWITHRESPONSESTREAM_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
    ...
    if (!exceptionPayload.WasParseSuccessful()) {
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(INVOKEMODELWITHRESPONSESTREAM_HANDLER_CLASS_TAG,
