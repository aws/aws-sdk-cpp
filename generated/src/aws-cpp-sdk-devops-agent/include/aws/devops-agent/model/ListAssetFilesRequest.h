...
/**
 * <p>Request structure for listing asset files</p><p><h3>See Also:</h3>   <a
 * href="http://docs.aws.amazon.com/goto/WebAPI/devops-agent-2026-01-01/ListAssetFilesRequest">AWS
 * API Reference</a></p>
 */
class ListAssetFilesRequest : public DevOpsAgentRequest {
 public:
  AWS_DEVOPSAGENT_API ListAssetFilesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListAssetFiles"; }
  ...
  template <typename AgentSpaceIdT = Aws::String>
  ListAssetFilesRequest& WithAgentSpaceId(AgentSpaceIdT&& value) {
  ...
  template <typename AssetIdT = Aws::String>
  ListAssetFilesRequest& WithAssetId(AssetIdT&& value) {
  ...
  ///@{
  /**
   * <p>The specific asset version to list files from. If omitted, files from the
   * latest version are returned.</p>
   */
  ...
  inline ListAssetFilesRequest& WithAssetVersion(int value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListAssetFilesRequest& WithNextToken(NextTokenT&& value) {
  ...
  inline ListAssetFilesRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
