...
TrafficPolicySummary::TrafficPolicySummary(const XmlNode& xmlNode) { *this = xmlNode; }
...
TrafficPolicySummary& TrafficPolicySummary::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!latestVersionNode.IsNull()) {
      m_latestVersion = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(latestVersionNode.GetText()).c_str()).c_str());
    ...
    if (!trafficPolicyCountNode.IsNull()) {
      m_trafficPolicyCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(trafficPolicyCountNode.GetText()).c_str()).c_str());
