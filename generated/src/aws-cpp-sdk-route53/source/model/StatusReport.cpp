...
StatusReport::StatusReport(const XmlNode& xmlNode) { *this = xmlNode; }
...
StatusReport& StatusReport::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!checkedTimeNode.IsNull()) {
      m_checkedTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(checkedTimeNode.GetText()).c_str()).c_str(),
