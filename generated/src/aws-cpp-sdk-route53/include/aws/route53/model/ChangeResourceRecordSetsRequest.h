...
class ChangeResourceRecordSetsRequest : public Route53Request {
 public:
  ...
  template <typename HostedZoneIdT = Aws::String>
  ChangeResourceRecordSetsRequest& WithHostedZoneId(HostedZoneIdT&& value) {
  ...
  void SetChangeBatch(ChangeBatchT&& value) {
    m_changeBatchHasBeenSet = true;
    m_changeBatch = std::forward<ChangeBatchT>(value);
  }
  template <typename ChangeBatchT = ChangeBatch>
  ChangeResourceRecordSetsRequest& WithChangeBatch(ChangeBatchT&& value) {
    SetChangeBatch(std::forward<ChangeBatchT>(value));
    return *this;
  }
  ///@}
 private:
