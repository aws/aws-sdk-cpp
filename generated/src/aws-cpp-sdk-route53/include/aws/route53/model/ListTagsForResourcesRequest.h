...
class ListTagsForResourcesRequest : public Route53Request {
 public:
  ...
  inline ListTagsForResourcesRequest& WithResourceType(TagResourceType value) {
  ...
  template <typename ResourceIdsT = Aws::Vector<Aws::String>>
  ListTagsForResourcesRequest& WithResourceIds(ResourceIdsT&& value) {
    SetResourceIds(std::forward<ResourceIdsT>(value));
    return *this;
  }
  template <typename ResourceIdsT = Aws::String>
  ListTagsForResourcesRequest& AddResourceIds(ResourceIdsT&& value) {
    m_resourceIdsHasBeenSet = true;
