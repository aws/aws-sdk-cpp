...
class StatusReport {
 public:
  ...
  template <typename StatusT = Aws::String>
  StatusReport& WithStatus(StatusT&& value) {
  ...
  void SetCheckedTime(CheckedTimeT&& value) {
    m_checkedTimeHasBeenSet = true;
    m_checkedTime = std::forward<CheckedTimeT>(value);
  }
  template <typename CheckedTimeT = Aws::Utils::DateTime>
  StatusReport& WithCheckedTime(CheckedTimeT&& value) {
    SetCheckedTime(std::forward<CheckedTimeT>(value));
    return *this;
  }
  ///@}
 private:
