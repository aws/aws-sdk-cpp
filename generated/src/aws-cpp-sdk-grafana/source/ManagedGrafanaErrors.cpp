...
template <>
AWS_MANAGEDGRAFANA_API ConflictException ManagedGrafanaError::GetModeledError() {
  assert(this->GetErrorType() == ManagedGrafanaErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_MANAGEDGRAFANA_API ThrottlingException ManagedGrafanaError::GetModeledError() {
  assert(this->GetErrorType() == ManagedGrafanaErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_MANAGEDGRAFANA_API ServiceQuotaExceededException ManagedGrafanaError::GetModeledError() {
  assert(this->GetErrorType() == ManagedGrafanaErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_MANAGEDGRAFANA_API ResourceNotFoundException ManagedGrafanaError::GetModeledError() {
  assert(this->GetErrorType() == ManagedGrafanaErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_MANAGEDGRAFANA_API InternalServerException ManagedGrafanaError::GetModeledError() {
  assert(this->GetErrorType() == ManagedGrafanaErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_MANAGEDGRAFANA_API ValidationException ManagedGrafanaError::GetModeledError() {
