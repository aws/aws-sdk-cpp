...
class SuggestResult {
 public:
  ...
  template <typename StatusT = SuggestStatus>
  SuggestResult& WithStatus(StatusT&& value) {
  ...
  void SetSuggest(SuggestT&& value) {
    m_suggestHasBeenSet = true;
    m_suggest = std::forward<SuggestT>(value);
  }
  template <typename SuggestT = SuggestModel>
  SuggestResult& WithSuggest(SuggestT&& value) {
    SetSuggest(std::forward<SuggestT>(value));
    return *this;
  }
  ///@}
