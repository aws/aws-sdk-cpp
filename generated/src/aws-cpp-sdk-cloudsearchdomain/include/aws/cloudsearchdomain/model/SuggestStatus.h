...
class SuggestStatus {
 public:
  ...
  inline void SetTimems(long long value) {
    m_timemsHasBeenSet = true;
    m_timems = value;
  }
  inline SuggestStatus& WithTimems(long long value) {
    SetTimems(value);
    return *this;
  }
  ///@}
  ...
  void SetRid(RidT&& value) {
    m_ridHasBeenSet = true;
    m_rid = std::forward<RidT>(value);
  }
  template <typename RidT = Aws::String>
  SuggestStatus& WithRid(RidT&& value) {
    SetRid(std::forward<RidT>(value));
    return *this;
  }
  ///@}
 private:
