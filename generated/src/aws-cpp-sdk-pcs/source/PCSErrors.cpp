...
namespace PCS {
template <>
AWS_PCS_API ConflictException PCSError::GetModeledError() {
  assert(this->GetErrorType() == PCSErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_PCS_API ServiceQuotaExceededException PCSError::GetModeledError() {
  assert(this->GetErrorType() == PCSErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_PCS_API ThrottlingException PCSError::GetModeledError() {
  assert(this->GetErrorType() == PCSErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_PCS_API ResourceNotFoundException PCSError::GetModeledError() {
  assert(this->GetErrorType() == PCSErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_PCS_API ValidationException PCSError::GetModeledError() {
