...
UsageStatistics::UsageStatistics(JsonView jsonValue) { *this = jsonValue; }
...
UsageStatistics& UsageStatistics::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("topAccountsByFeature")) {
    ...
    for (unsigned topAccountsByFeatureIndex = 0; topAccountsByFeatureIndex < topAccountsByFeatureJsonList.GetLength();
         ++topAccountsByFeatureIndex) {
