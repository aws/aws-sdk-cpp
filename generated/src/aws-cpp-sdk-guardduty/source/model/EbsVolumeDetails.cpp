...
EbsVolumeDetails::EbsVolumeDetails(JsonView jsonValue) { *this = jsonValue; }
...
EbsVolumeDetails& EbsVolumeDetails::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("scannedVolumeDetails")) {
    ...
    for (unsigned scannedVolumeDetailsIndex = 0; scannedVolumeDetailsIndex < scannedVolumeDetailsJsonList.GetLength();
         ++scannedVolumeDetailsIndex) {
  ...
  if (jsonValue.ValueExists("skippedVolumeDetails")) {
    ...
    for (unsigned skippedVolumeDetailsIndex = 0; skippedVolumeDetailsIndex < skippedVolumeDetailsJsonList.GetLength();
         ++skippedVolumeDetailsIndex) {
