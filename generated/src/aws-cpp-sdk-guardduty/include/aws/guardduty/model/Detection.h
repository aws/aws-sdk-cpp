...
class Detection {
 public:
  ...
  void SetAnomaly(AnomalyT&& value) {
    ...
    m_anomaly = std::forward<AnomalyT>(value);
  }
  template <typename AnomalyT = Anomaly>
  Detection& WithAnomaly(AnomalyT&& value) {
    SetAnomaly(std::forward<AnomalyT>(value));
    return *this;
  }
  ///@}
  ...
  void SetSequence(SequenceT&& value) {
    ...
    m_sequence = std::forward<SequenceT>(value);
  }
  template <typename SequenceT = Sequence>
  Detection& WithSequence(SequenceT&& value) {
    SetSequence(std::forward<SequenceT>(value));
    return *this;
  }
  ///@}
 private:
