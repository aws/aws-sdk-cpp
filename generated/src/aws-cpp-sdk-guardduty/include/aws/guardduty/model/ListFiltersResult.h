...
class ListFiltersResult {
 public:
  ...
  void SetFilterNames(FilterNamesT&& value) {
    m_filterNamesHasBeenSet = true;
    m_filterNames = std::forward<FilterNamesT>(value);
  }
  template <typename FilterNamesT = Aws::Vector<Aws::String>>
  ListFiltersResult& WithFilterNames(FilterNamesT&& value) {
    SetFilterNames(std::forward<FilterNamesT>(value));
    return *this;
  }
  template <typename FilterNamesT = Aws::String>
  ListFiltersResult& AddFilterNames(FilterNamesT&& value) {
    m_filterNamesHasBeenSet = true;
    m_filterNames.emplace_back(std::forward<FilterNamesT>(value));
    return *this;
  }
  ///@}
