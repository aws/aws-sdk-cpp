...
class Anomaly {
 public:
  ...
  Anomaly& AddProfiles(ProfilesKeyT&& key, ProfilesValueT&& value) {
    m_profilesHasBeenSet = true;
    m_profiles.emplace(std::forward<ProfilesKeyT>(key), std::forward<ProfilesValueT>(value));
    return *this;
  }
  ///@}
  ...
  void SetUnusual(UnusualT&& value) {
    ...
    m_unusual = std::forward<UnusualT>(value);
  }
  template <typename UnusualT = AnomalyUnusual>
  Anomaly& WithUnusual(UnusualT&& value) {
    SetUnusual(std::forward<UnusualT>(value));
    return *this;
  }
  ///@}
 private:
