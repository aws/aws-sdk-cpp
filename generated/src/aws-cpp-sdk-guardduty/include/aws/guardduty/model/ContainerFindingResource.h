...
class ContainerFindingResource {
 public:
  ...
  template <typename ImageT = Aws::String>
  ContainerFindingResource& WithImage(ImageT&& value) {
  ...
  void SetImageUid(ImageUidT&& value) {
    m_imageUidHasBeenSet = true;
    m_imageUid = std::forward<ImageUidT>(value);
  }
  template <typename ImageUidT = Aws::String>
  ContainerFindingResource& WithImageUid(ImageUidT&& value) {
    SetImageUid(std::forward<ImageUidT>(value));
    return *this;
  }
  ///@}
 private:
