...
class AddonDetails {
 public:
  ...
  template <typename AddonVersionT = Aws::String>
  AddonDetails& WithAddonVersion(AddonVersionT&& value) {
  ...
  void SetAddonStatus(AddonStatusT&& value) {
    m_addonStatusHasBeenSet = true;
    m_addonStatus = std::forward<AddonStatusT>(value);
  }
  template <typename AddonStatusT = Aws::String>
  AddonDetails& WithAddonStatus(AddonStatusT&& value) {
    SetAddonStatus(std::forward<AddonStatusT>(value));
    return *this;
  }
  ///@}
 private:
