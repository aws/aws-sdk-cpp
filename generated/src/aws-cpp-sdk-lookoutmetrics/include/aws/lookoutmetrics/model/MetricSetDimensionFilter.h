...
class MetricSetDimensionFilter {
 public:
  ...
  template <typename NameT = Aws::String>
  MetricSetDimensionFilter& WithName(NameT&& value) {
  ...
  template <typename FilterListT = Aws::Vector<Filter>>
  MetricSetDimensionFilter& WithFilterList(FilterListT&& value) {
    SetFilterList(std::forward<FilterListT>(value));
    return *this;
  }
  template <typename FilterListT = Filter>
  MetricSetDimensionFilter& AddFilterList(FilterListT&& value) {
    m_filterListHasBeenSet = true;
