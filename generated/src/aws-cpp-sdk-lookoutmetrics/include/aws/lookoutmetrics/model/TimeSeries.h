...
class TimeSeries {
 public:
  ...
  void SetMetricValueList(MetricValueListT&& value) {
    ...
    m_metricValueList = std::forward<MetricValueListT>(value);
  }
  template <typename MetricValueListT = Aws::Vector<double>>
  TimeSeries& WithMetricValueList(MetricValueListT&& value) {
    SetMetricValueList(std::forward<MetricValueListT>(value));
    return *this;
  }
  inline TimeSeries& AddMetricValueList(double value) {
    m_metricValueListHasBeenSet = true;
    m_metricValueList.push_back(value);
    return *this;
  }
  ///@}
 private:
