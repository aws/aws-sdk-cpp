...
template <>
AWS_LOOKOUTMETRICS_API ConflictException LookoutMetricsError::GetModeledError() {
  assert(this->GetErrorType() == LookoutMetricsErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_LOOKOUTMETRICS_API ServiceQuotaExceededException LookoutMetricsError::GetModeledError() {
  assert(this->GetErrorType() == LookoutMetricsErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_LOOKOUTMETRICS_API ResourceNotFoundException LookoutMetricsError::GetModeledError() {
  assert(this->GetErrorType() == LookoutMetricsErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_LOOKOUTMETRICS_API ValidationException LookoutMetricsError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(LookoutMetricsErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
