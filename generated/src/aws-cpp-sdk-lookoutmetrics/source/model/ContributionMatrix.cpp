...
ContributionMatrix::ContributionMatrix(JsonView jsonValue) { *this = jsonValue; }
...
ContributionMatrix& ContributionMatrix::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("DimensionContributionList")) {
    ...
    for (unsigned dimensionContributionListIndex = 0; dimensionContributionListIndex < dimensionContributionListJsonList.GetLength();
         ++dimensionContributionListIndex) {
...
JsonValue ContributionMatrix::Jsonize() const {
  ...
  if (m_dimensionContributionListHasBeenSet) {
    ...
    for (unsigned dimensionContributionListIndex = 0; dimensionContributionListIndex < dimensionContributionListJsonList.GetLength();
         ++dimensionContributionListIndex) {
      dimensionContributionListJsonList[dimensionContributionListIndex].AsObject(
          m_dimensionContributionList[dimensionContributionListIndex].Jsonize());
    }
