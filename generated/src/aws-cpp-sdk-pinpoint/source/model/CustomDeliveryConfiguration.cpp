...
CustomDeliveryConfiguration::CustomDeliveryConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
CustomDeliveryConfiguration& CustomDeliveryConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("EndpointTypes")) {
    ...
    for (unsigned endpointTypesIndex = 0; endpointTypesIndex < endpointTypesJsonList.GetLength(); ++endpointTypesIndex) {
      m_endpointTypes.push_back(
          __EndpointTypesElementMapper::Get__EndpointTypesElementForName(endpointTypesJsonList[endpointTypesIndex].AsString()));
    }
...
JsonValue CustomDeliveryConfiguration::Jsonize() const {
  ...
  if (m_endpointTypesHasBeenSet) {
    ...
    for (unsigned endpointTypesIndex = 0; endpointTypesIndex < endpointTypesJsonList.GetLength(); ++endpointTypesIndex) {
      endpointTypesJsonList[endpointTypesIndex].AsString(
          __EndpointTypesElementMapper::GetNameFor__EndpointTypesElement(m_endpointTypes[endpointTypesIndex]));
    }
