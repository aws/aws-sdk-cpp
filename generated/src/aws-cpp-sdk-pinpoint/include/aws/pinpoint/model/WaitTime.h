...
class WaitTime {
 public:
  ...
  void SetWaitFor(WaitForT&& value) {
    ...
    m_waitFor = std::forward<WaitForT>(value);
  }
  template <typename WaitForT = Aws::String>
  WaitTime& WithWaitFor(WaitForT&& value) {
    SetWaitFor(std::forward<WaitForT>(value));
    return *this;
  }
  ///@}
  ...
  void SetWaitUntil(WaitUntilT&& value) {
    ...
    m_waitUntil = std::forward<WaitUntilT>(value);
  }
  template <typename WaitUntilT = Aws::String>
  WaitTime& WithWaitUntil(WaitUntilT&& value) {
    SetWaitUntil(std::forward<WaitUntilT>(value));
    return *this;
  }
  ///@}
 private:
