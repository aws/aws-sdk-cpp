...
class InAppMessageButton {
 public:
  ...
  void SetAndroid(AndroidT&& value) {
    m_androidHasBeenSet = true;
    m_android = std::forward<AndroidT>(value);
  }
  template <typename AndroidT = OverrideButtonConfiguration>
  InAppMessageButton& WithAndroid(AndroidT&& value) {
    SetAndroid(std::forward<AndroidT>(value));
    return *this;
  }
  ///@}
  ...
  void SetIOS(IOST&& value) {
    m_iOSHasBeenSet = true;
    m_iOS = std::forward<IOST>(value);
  }
  template <typename IOST = OverrideButtonConfiguration>
  InAppMessageButton& WithIOS(IOST&& value) {
    SetIOS(std::forward<IOST>(value));
    return *this;
  }
  ///@}
