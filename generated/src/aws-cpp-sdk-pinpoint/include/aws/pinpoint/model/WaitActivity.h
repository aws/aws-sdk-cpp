...
class WaitActivity {
 public:
  ...
  template <typename NextActivityT = Aws::String>
  WaitActivity& WithNextActivity(NextActivityT&& value) {
  ...
  void SetWaitTime(WaitTimeT&& value) {
    m_waitTimeHasBeenSet = true;
    m_waitTime = std::forward<WaitTimeT>(value);
  }
  template <typename WaitTimeT = WaitTime>
  WaitActivity& WithWaitTime(WaitTimeT&& value) {
    SetWaitTime(std::forward<WaitTimeT>(value));
    return *this;
  }
  ///@}
 private:
