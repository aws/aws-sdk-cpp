...
class GetEventStreamResult {
 public:
  ...
  void SetEventStream(EventStreamT&& value) {
    m_eventStreamHasBeenSet = true;
    m_eventStream = std::forward<EventStreamT>(value);
  }
  template <typename EventStreamT = EventStream>
  GetEventStreamResult& WithEventStream(EventStreamT&& value) {
    SetEventStream(std::forward<EventStreamT>(value));
    return *this;
  }
  ///@}
