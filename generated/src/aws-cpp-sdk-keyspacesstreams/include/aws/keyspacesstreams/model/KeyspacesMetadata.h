...
class KeyspacesMetadata {
 public:
  ...
  template <typename ExpirationTimeT = Aws::String>
  KeyspacesMetadata& WithExpirationTime(ExpirationTimeT&& value) {
  ...
  void SetWriteTime(WriteTimeT&& value) {
    m_writeTimeHasBeenSet = true;
    m_writeTime = std::forward<WriteTimeT>(value);
  }
  template <typename WriteTimeT = Aws::String>
  KeyspacesMetadata& WithWriteTime(WriteTimeT&& value) {
    SetWriteTime(std::forward<WriteTimeT>(value));
    return *this;
  }
  ///@}
 private:
