...
class ListSafetyRulesResult {
 public:
  ...
  AWS_ROUTE53RECOVERYCONTROLCONFIG_API ListSafetyRulesResult& operator=(
  ...
  void SetSafetyRules(SafetyRulesT&& value) {
    ...
    m_safetyRules = std::forward<SafetyRulesT>(value);
  }
  template <typename SafetyRulesT = Aws::Vector<Rule>>
  ListSafetyRulesResult& WithSafetyRules(SafetyRulesT&& value) {
    SetSafetyRules(std::forward<SafetyRulesT>(value));
    return *this;
  }
  template <typename SafetyRulesT = Rule>
  ListSafetyRulesResult& AddSafetyRules(SafetyRulesT&& value) {
    m_safetyRulesHasBeenSet = true;
    m_safetyRules.emplace_back(std::forward<SafetyRulesT>(value));
    return *this;
  }
  ///@}
