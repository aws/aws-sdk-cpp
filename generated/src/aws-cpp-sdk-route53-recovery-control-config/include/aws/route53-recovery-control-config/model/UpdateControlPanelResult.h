...
class UpdateControlPanelResult {
 public:
  ...
  AWS_ROUTE53RECOVERYCONTROLCONFIG_API UpdateControlPanelResult& operator=(
  ...
  void SetControlPanel(ControlPanelT&& value) {
    m_controlPanelHasBeenSet = true;
    m_controlPanel = std::forward<ControlPanelT>(value);
  }
  template <typename ControlPanelT = ControlPanel>
  UpdateControlPanelResult& WithControlPanel(ControlPanelT&& value) {
    SetControlPanel(std::forward<ControlPanelT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  UpdateControlPanelResult& WithRequestId(RequestIdT&& value) {
