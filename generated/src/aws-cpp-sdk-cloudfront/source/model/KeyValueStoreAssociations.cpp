...
KeyValueStoreAssociations::KeyValueStoreAssociations(const XmlNode& xmlNode) { *this = xmlNode; }

KeyValueStoreAssociations& KeyValueStoreAssociations::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      ...
      while (!itemsMember.IsNull()) {
        ...
        itemsMember = itemsMember.NextNode("KeyValueStoreAssociation");
      }
...
void KeyValueStoreAssociations::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("KeyValueStoreAssociation");
