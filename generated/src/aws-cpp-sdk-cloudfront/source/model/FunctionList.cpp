...
FunctionList::FunctionList(const XmlNode& xmlNode) { *this = xmlNode; }

FunctionList& FunctionList::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      XmlNode itemsMember = itemsNode.FirstChild("FunctionSummary");
      m_itemsHasBeenSet = !itemsMember.IsNull();
      while (!itemsMember.IsNull()) {
        m_items.push_back(itemsMember);
        itemsMember = itemsMember.NextNode("FunctionSummary");
      }
...
void FunctionList::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("FunctionSummary");
