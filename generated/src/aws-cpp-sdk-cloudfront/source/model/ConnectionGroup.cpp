...
ConnectionGroup::ConnectionGroup(const XmlNode& xmlNode) { *this = xmlNode; }
...
ConnectionGroup& ConnectionGroup::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!ipv6EnabledNode.IsNull()) {
      m_ipv6Enabled =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(ipv6EnabledNode.GetText()).c_str()).c_str());
