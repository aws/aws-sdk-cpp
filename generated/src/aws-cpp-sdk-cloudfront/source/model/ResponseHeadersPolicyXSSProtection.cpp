...
ResponseHeadersPolicyXSSProtection::ResponseHeadersPolicyXSSProtection(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResponseHeadersPolicyXSSProtection& ResponseHeadersPolicyXSSProtection::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!protectionNode.IsNull()) {
      m_protection =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(protectionNode.GetText()).c_str()).c_str());
      m_protectionHasBeenSet = true;
    }
    XmlNode modeBlockNode = resultNode.FirstChild("ModeBlock");
    if (!modeBlockNode.IsNull()) {
      m_modeBlock =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(modeBlockNode.GetText()).c_str()).c_str());
