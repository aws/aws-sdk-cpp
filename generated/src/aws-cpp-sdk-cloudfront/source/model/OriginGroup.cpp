...
OriginGroup::OriginGroup(const XmlNode& xmlNode) { *this = xmlNode; }
...
OriginGroup& OriginGroup::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!selectionCriteriaNode.IsNull()) {
      m_selectionCriteria = OriginGroupSelectionCriteriaMapper::GetOriginGroupSelectionCriteriaForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(selectionCriteriaNode.GetText()).c_str()));
