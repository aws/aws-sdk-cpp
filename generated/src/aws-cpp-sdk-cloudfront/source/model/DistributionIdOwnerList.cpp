...
DistributionIdOwnerList::DistributionIdOwnerList(const XmlNode& xmlNode) { *this = xmlNode; }

DistributionIdOwnerList& DistributionIdOwnerList::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      XmlNode itemsMember = itemsNode.FirstChild("DistributionIdOwner");
      m_itemsHasBeenSet = !itemsMember.IsNull();
      while (!itemsMember.IsNull()) {
        m_items.push_back(itemsMember);
        itemsMember = itemsMember.NextNode("DistributionIdOwner");
      }
...
void DistributionIdOwnerList::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("DistributionIdOwner");
