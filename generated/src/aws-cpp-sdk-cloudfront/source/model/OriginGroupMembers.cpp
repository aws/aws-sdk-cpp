...
OriginGroupMembers::OriginGroupMembers(const XmlNode& xmlNode) { *this = xmlNode; }

OriginGroupMembers& OriginGroupMembers::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      ...
      while (!itemsMember.IsNull()) {
        ...
        itemsMember = itemsMember.NextNode("OriginGroupMember");
      }
...
void OriginGroupMembers::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("OriginGroupMember");
