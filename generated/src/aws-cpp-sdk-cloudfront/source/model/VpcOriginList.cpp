...
VpcOriginList::VpcOriginList(const XmlNode& xmlNode) { *this = xmlNode; }

VpcOriginList& VpcOriginList::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      XmlNode itemsMember = itemsNode.FirstChild("VpcOriginSummary");
      m_itemsHasBeenSet = !itemsMember.IsNull();
      while (!itemsMember.IsNull()) {
        m_items.push_back(itemsMember);
        itemsMember = itemsMember.NextNode("VpcOriginSummary");
      }
...
void VpcOriginList::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("VpcOriginSummary");
