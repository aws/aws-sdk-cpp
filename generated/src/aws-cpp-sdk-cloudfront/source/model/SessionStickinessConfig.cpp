...
SessionStickinessConfig::SessionStickinessConfig(const XmlNode& xmlNode) { *this = xmlNode; }
...
SessionStickinessConfig& SessionStickinessConfig::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!idleTTLNode.IsNull()) {
      m_idleTTL =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(idleTTLNode.GetText()).c_str()).c_str());
      m_idleTTLHasBeenSet = true;
    }
    XmlNode maximumTTLNode = resultNode.FirstChild("MaximumTTL");
    if (!maximumTTLNode.IsNull()) {
      m_maximumTTL =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maximumTTLNode.GetText()).c_str()).c_str());
