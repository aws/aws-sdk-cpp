...
TrustStoreSummary::TrustStoreSummary(const XmlNode& xmlNode) { *this = xmlNode; }

TrustStoreSummary& TrustStoreSummary::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = TrustStoreStatusMapper::GetTrustStoreStatusForName(
    ...
    if (!numberOfCaCertificatesNode.IsNull()) {
      m_numberOfCaCertificates = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(numberOfCaCertificatesNode.GetText()).c_str()).c_str());
...
void TrustStoreSummary::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  if (m_idHasBeenSet) {
  ...
  if (m_statusHasBeenSet) {
    ...
    statusNode.SetText(TrustStoreStatusMapper::GetNameForTrustStoreStatus(m_status));
  }

  if (m_numberOfCaCertificatesHasBeenSet) {
    XmlNode numberOfCaCertificatesNode = parentNode.CreateChildElement("NumberOfCaCertificates");
    ss << m_numberOfCaCertificates;
    numberOfCaCertificatesNode.SetText(ss.str());
    ss.str("");
  }
