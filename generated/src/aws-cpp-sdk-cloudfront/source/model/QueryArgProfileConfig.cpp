...
QueryArgProfileConfig::QueryArgProfileConfig(const XmlNode& xmlNode) { *this = xmlNode; }
...
QueryArgProfileConfig& QueryArgProfileConfig::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!forwardWhenQueryArgProfileIsUnknownNode.IsNull()) {
      m_forwardWhenQueryArgProfileIsUnknown = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(forwardWhenQueryArgProfileIsUnknownNode.GetText()).c_str()).c_str());
