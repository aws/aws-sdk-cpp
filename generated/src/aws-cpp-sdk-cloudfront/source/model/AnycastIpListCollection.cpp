...
AnycastIpListCollection::AnycastIpListCollection() : 
...
AnycastIpListCollection::AnycastIpListCollection(const XmlNode& xmlNode)
  : AnycastIpListCollection()
{
  *this = xmlNode;
}

AnycastIpListCollection& AnycastIpListCollection::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    if(!itemsNode.IsNull())
    {
      XmlNode itemsMember = itemsNode.FirstChild("AnycastIpListSummary");
      while(!itemsMember.IsNull())
      {
        m_items.push_back(itemsMember);
        itemsMember = itemsMember.NextNode("AnycastIpListSummary");
      }
...
void AnycastIpListCollection::AddToNode(XmlNode& parentNode) const
{
  Aws::StringStream ss;
  if(m_itemsHasBeenSet)
  {
   ...
   {
     XmlNode itemsNode = itemsParentNode.CreateChildElement("AnycastIpListSummary");
