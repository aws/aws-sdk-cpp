...
ResponseHeadersPolicyStrictTransportSecurity::ResponseHeadersPolicyStrictTransportSecurity(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResponseHeadersPolicyStrictTransportSecurity& ResponseHeadersPolicyStrictTransportSecurity::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!includeSubdomainsNode.IsNull()) {
      m_includeSubdomains = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(includeSubdomainsNode.GetText()).c_str()).c_str());
      m_includeSubdomainsHasBeenSet = true;
    }
    XmlNode preloadNode = resultNode.FirstChild("Preload");
    if (!preloadNode.IsNull()) {
      m_preload =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(preloadNode.GetText()).c_str()).c_str());
