...
OriginRequestPolicyList::OriginRequestPolicyList(const XmlNode& xmlNode) { *this = xmlNode; }

OriginRequestPolicyList& OriginRequestPolicyList::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      XmlNode itemsMember = itemsNode.FirstChild("OriginRequestPolicySummary");
      m_itemsHasBeenSet = !itemsMember.IsNull();
      while (!itemsMember.IsNull()) {
        m_items.push_back(itemsMember);
        itemsMember = itemsMember.NextNode("OriginRequestPolicySummary");
      }
...
void OriginRequestPolicyList::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("OriginRequestPolicySummary");
