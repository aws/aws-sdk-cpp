...
Distribution::Distribution(const XmlNode& xmlNode) { *this = xmlNode; }
...
Distribution& Distribution::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!inProgressInvalidationBatchesNode.IsNull()) {
      m_inProgressInvalidationBatches = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(inProgressInvalidationBatchesNode.GetText()).c_str()).c_str());
