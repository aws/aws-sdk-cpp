...
KeyGroupList::KeyGroupList(const XmlNode& xmlNode) { *this = xmlNode; }

KeyGroupList& KeyGroupList::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      ...
      while (!itemsMember.IsNull()) {
        ...
        itemsMember = itemsMember.NextNode("KeyGroupSummary");
      }
...
void KeyGroupList::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("KeyGroupSummary");
