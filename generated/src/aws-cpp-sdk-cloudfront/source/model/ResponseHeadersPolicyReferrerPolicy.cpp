...
ResponseHeadersPolicyReferrerPolicy::ResponseHeadersPolicyReferrerPolicy(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResponseHeadersPolicyReferrerPolicy& ResponseHeadersPolicyReferrerPolicy::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!referrerPolicyNode.IsNull()) {
      m_referrerPolicy = ReferrerPolicyListMapper::GetReferrerPolicyListForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(referrerPolicyNode.GetText()).c_str()));
