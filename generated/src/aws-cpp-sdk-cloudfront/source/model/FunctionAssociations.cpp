...
FunctionAssociations::FunctionAssociations(const XmlNode& xmlNode) { *this = xmlNode; }

FunctionAssociations& FunctionAssociations::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!itemsNode.IsNull()) {
      ...
      while (!itemsMember.IsNull()) {
        ...
        itemsMember = itemsMember.NextNode("FunctionAssociation");
      }
...
void FunctionAssociations::AddToNode(XmlNode& parentNode) const {
  Aws::StringStream ss;
  ...
  if (m_itemsHasBeenSet) {
    ...
    for (const auto& item : m_items) {
      XmlNode itemsNode = itemsParentNode.CreateChildElement("FunctionAssociation");
