...
class StagingDistributionDnsNames {
 public:
  ...
  inline StagingDistributionDnsNames& WithQuantity(int value) {
    SetQuantity(value);
    return *this;
  }
  ///@}
  ...
  template <typename ItemsT = Aws::Vector<Aws::String>>
  StagingDistributionDnsNames& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = Aws::String>
  StagingDistributionDnsNames& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
