...
class OriginGroupMember {
 public:
  ...
  void SetOriginId(OriginIdT&& value) {
    m_originIdHasBeenSet = true;
    m_originId = std::forward<OriginIdT>(value);
  }
  template <typename OriginIdT = Aws::String>
  OriginGroupMember& WithOriginId(OriginIdT&& value) {
    SetOriginId(std::forward<OriginIdT>(value));
    return *this;
  }
  ///@}
 private:
