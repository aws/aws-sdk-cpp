...
class KeyValueStoreAssociations {
 public:
  ...
  inline KeyValueStoreAssociations& WithQuantity(int value) {
    SetQuantity(value);
    return *this;
  }
  ///@}
  ...
  template <typename ItemsT = Aws::Vector<KeyValueStoreAssociation>>
  KeyValueStoreAssociations& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = KeyValueStoreAssociation>
  KeyValueStoreAssociations& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
