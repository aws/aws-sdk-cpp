...
class KeyValueStoreList {
 public:
  ...
  template <typename NextMarkerT = Aws::String>
  KeyValueStoreList& WithNextMarker(NextMarkerT&& value) {
  ...
  inline KeyValueStoreList& WithMaxItems(int value) {
    SetMaxItems(value);
    return *this;
  }
  ///@}
  ...
  inline KeyValueStoreList& WithQuantity(int value) {
    SetQuantity(value);
    return *this;
  }
  ///@}
  ...
  template <typename ItemsT = Aws::Vector<KeyValueStore>>
  KeyValueStoreList& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = KeyValueStore>
  KeyValueStoreList& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
