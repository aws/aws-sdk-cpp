 ...
 * that CloudFront sends to the origin.</p><p><h3>See Also:</h3>   <a
...
class CachePolicyQueryStringsConfig {
 public:
   ...
   * are:</p> <ul> <li> <p> <code>none</code> – No query strings in viewer requests
   * are included in the cache key or in requests that CloudFront sends to the
   * origin. Even when this field is set to <code>none</code>, any query strings that
   * are listed in an <code>OriginRequestPolicy</code> <i>are</i> included in origin
   * requests.</p> </li> <li> <p> <code>whitelist</code> – Only the query strings in
   ...
   * the origin, <i> <b>except</b> </i> those that are listed in the
   * <code>QueryStringNames</code> type, which are not included.</p> </li> <li> <p>
  ...
  inline void SetQueryStringBehavior(CachePolicyQueryStringBehavior value) {
    m_queryStringBehaviorHasBeenSet = true;
    m_queryStringBehavior = value;
  }
  inline CachePolicyQueryStringsConfig& WithQueryStringBehavior(CachePolicyQueryStringBehavior value) {
    SetQueryStringBehavior(value);
    return *this;
  }
  ///@}
   ...
   * requests that CloudFront sends to the origin. The behavior depends on whether
   * the <code>QueryStringBehavior</code> field in the
