...
class KeyValueStoreAssociation {
 public:
  ...
  void SetKeyValueStoreARN(KeyValueStoreARNT&& value) {
    m_keyValueStoreARNHasBeenSet = true;
    m_keyValueStoreARN = std::forward<KeyValueStoreARNT>(value);
  }
  template <typename KeyValueStoreARNT = Aws::String>
  KeyValueStoreAssociation& WithKeyValueStoreARN(KeyValueStoreARNT&& value) {
    SetKeyValueStoreARN(std::forward<KeyValueStoreARNT>(value));
    return *this;
  }
  ///@}
 private:
