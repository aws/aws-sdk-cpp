...
smithy::IdentityResolverBase<S3ExpressIdentity>::ResolveIdentityFutureOutcome S3ExpressIdentityProvider::getIdentity(
  ...
  for (const auto &paramMap : {identityProperties, additionalParameters}) {
    TransformAndInsert<String, Aws::Crt::Variant<Aws::String, bool>, String>(
        paramMap, params->parameterMap, [](const Aws::Crt::Variant<Aws::String, bool> &value) -> Aws::String {
...
S3ExpressIdentity S3ExpressIdentityProvider::getIdentity(const Aws::String &bucketName) {
  ...
  return {credentials.GetAccessKeyId(), credentials.GetSecretAccessKey(), credentials.GetSessionToken(), expiry};
}
...
Aws::S3::DefaultS3ExpressIdentityProvider::DefaultS3ExpressIdentityProvider(const S3Client &s3Client)
    : DefaultS3ExpressIdentityProvider(s3Client, Aws::MakeShared<Aws::Utils::ConcurrentCache<Aws::String, S3ExpressIdentity>>(
...
S3ExpressIdentity DefaultS3ExpressIdentityProvider::GetS3ExpressIdentity(
  ...
  if (!isInCache || identity.getExpiration() - minutes(1) < Aws::Utils::DateTime::Now()) {
    ...
    m_credentialsCache->Put(bucketNameIter->second, identity,
                            std::chrono::milliseconds(identity.getExpiration().Millis() - Aws::Utils::DateTime::Now().Millis()));
  }
  return identity;
}

Aws::S3::DefaultAsyncS3ExpressIdentityProvider::DefaultAsyncS3ExpressIdentityProvider(const S3Client &s3Client,
                                                                                      std::chrono::minutes refreshPeriod)
    : DefaultAsyncS3ExpressIdentityProvider(
          s3Client,
          Aws::MakeShared<Aws::Utils::ConcurrentCache<Aws::String, S3ExpressIdentity>>(S3_EXPRESS_IDENTITY_PROVIDER, DEFAULT_CACHE_SIZE),
          refreshPeriod) {}
...
void DefaultAsyncS3ExpressIdentityProvider::refreshIdentities(std::chrono::minutes refreshPeriod) {
  ...
  auto refreshIdentityWhenCloseToExpiry = [&](const Aws::String &bucketName, const CacheValueType &valueType) -> CacheValueType {
    std::lock_guard<std::mutex> lock(*GetMutexForBucketName(bucketName));
...
S3ExpressIdentity DefaultAsyncS3ExpressIdentityProvider::GetS3ExpressIdentity(
    const std::shared_ptr<ServiceSpecificParameters> &serviceSpecificParameters) {
  auto bucketNameIter = serviceSpecificParameters->parameterMap.find("bucketName");
  if (bucketNameIter == serviceSpecificParameters->parameterMap.end()) {
    AWS_LOGSTREAM_ERROR(S3_EXPRESS_IDENTITY_PROVIDER, "property bucketName Required to make call")
    return {"", "", "", {}};
