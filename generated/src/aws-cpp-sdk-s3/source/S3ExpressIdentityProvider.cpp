...
/* Smithy version */
SmithyS3ExpressIdentityProvider::SmithyS3ExpressIdentityProvider(const S3Client& s3Client) : m_s3Client(s3Client){}


SmithyS3ExpressIdentityProvider::ResolveIdentityFutureOutcome SmithyS3ExpressIdentityProvider::getIdentity(
    const IdentityProperties& identityProperties, const AdditionalParameters& additionalParameters){
    const auto params = Aws::MakeShared<ServiceSpecificParameters>(S3_EXPRESS_IDENTITY_PROVIDER);
    for (const auto& paramMap: {identityProperties, additionalParameters})
    {
    ...
    auto identity =  Aws::MakeUnique<AwsCredentialIdentity>("DefaultAwsCredentialIdentityResolver", GetS3ExpressAwsIdentity(params));
    
    return ResolveIdentityFutureOutcome(std::move(identity));
}


std::shared_ptr<std::mutex> SmithyS3ExpressIdentityProvider::GetMutexForBucketName(const Aws::String& bucketName) {
  std::lock_guard<std::mutex> lock(m_bucketNameMapMutex);
  auto it = m_bucketNameMutex.find(bucketName);
  if (it != m_bucketNameMutex.end()) {
    return it->second;
...
AwsCredentialIdentity SmithyS3ExpressIdentityProvider::GetCredentialsFromBucket(const Aws::String& bucketName) const {
  auto outcome = m_s3Client.CreateSession(Model::CreateSessionRequest().WithBucket(bucketName));
  // If we fail the connect call return empty credentials and log an error message.
  if (!outcome.IsSuccess()) {
      AWS_LOGSTREAM_ERROR(S3_EXPRESS_IDENTITY_PROVIDER, "Failed to make S3Express Connect Call")
...
SmithyDefaultS3ExpressIdentityProvider::SmithyDefaultS3ExpressIdentityProvider(const S3Client& s3Client)
    : SmithyS3ExpressIdentityProvider{s3Client}, m_credentialsCache{Aws::MakeShared<Aws::Utils::ConcurrentCache<Aws::String, AwsCredentialIdentity>>(S3_EXPRESS_IDENTITY_PROVIDER,
            DEFAULT_CACHE_SIZE)}{}

SmithyDefaultS3ExpressIdentityProvider::SmithyDefaultS3ExpressIdentityProvider(
    const S3Client& s3Client,
    std::shared_ptr<Aws::Utils::ConcurrentCache<Aws::String, AwsCredentialIdentity>> credentialsCache)
    : SmithyS3ExpressIdentityProvider{s3Client}, m_credentialsCache{credentialsCache} {}

AwsCredentialIdentity SmithyDefaultS3ExpressIdentityProvider::GetS3ExpressAwsIdentity(const std::shared_ptr<Aws::Http::ServiceSpecificParameters> &serviceSpecificParameters) {
    auto bucketNameIter = serviceSpecificParameters->parameterMap.find("bucketName");
    if (bucketNameIter == serviceSpecificParameters->parameterMap.end()) {
        AWS_LOGSTREAM_ERROR(S3_EXPRESS_IDENTITY_PROVIDER, "property bucketName Required to make call")
        return {"", "", {""}, {}, Aws::Crt::Optional<Aws::String>()};
    ...
    auto isInCache = m_credentialsCache->Get(bucketNameIter->second, identity);
    if (!isInCache || (identity.expiration().has_value() && (identity.expiration().value() - minutes(1) < Aws::Utils::DateTime::Now())) ) {
        identity = SmithyS3ExpressIdentityProvider::GetCredentialsFromBucket(bucketNameIter->second);
        if (identity.expiration().has_value()) {
          m_credentialsCache->Put(bucketNameIter->second,
              identity,
              std::chrono::milliseconds(identity.expiration().value().Millis() - Aws::Utils::DateTime::Now().Millis()));
...
SmithyDefaultAsyncS3ExpressIdentityProvider::SmithyDefaultAsyncS3ExpressIdentityProvider(
    const S3Client& s3Client, std::chrono::minutes refreshPeriod)
    : SmithyDefaultAsyncS3ExpressIdentityProvider(s3Client,
                                            Aws::MakeShared<Aws::Utils::ConcurrentCache<Aws::String, AwsCredentialIdentity>>(
                                                S3_EXPRESS_IDENTITY_PROVIDER, DEFAULT_CACHE_SIZE),
                                            refreshPeriod) {}

SmithyDefaultAsyncS3ExpressIdentityProvider::SmithyDefaultAsyncS3ExpressIdentityProvider(
    const S3Client& s3Client, std::shared_ptr<Aws::Utils::ConcurrentCache<Aws::String, AwsCredentialIdentity>> credentialsCache,
    std::chrono::minutes refreshPeriod)
    : SmithyS3ExpressIdentityProvider(s3Client), m_credentialsCache(std::move(credentialsCache)) {
  // Start a thread to background refresh the keys currently in the cache.
  m_shouldStopBackgroundRefresh = false;
  m_backgroundRefreshThread = Aws::MakeUnique<std::thread>(
      S3_EXPRESS_IDENTITY_PROVIDER, std::thread(&SmithyDefaultAsyncS3ExpressIdentityProvider::refreshIdentities, this, refreshPeriod));
}
...
AwsCredentialIdentity SmithyDefaultAsyncS3ExpressIdentityProvider::GetS3ExpressAwsIdentity(const std::shared_ptr<Aws::Http::ServiceSpecificParameters> &serviceSpecificParameters){
  auto bucketNameIter = serviceSpecificParameters->parameterMap.find("bucketName");
  if (bucketNameIter == serviceSpecificParameters->parameterMap.end()) {
      AWS_LOGSTREAM_ERROR(S3_EXPRESS_IDENTITY_PROVIDER, "property bucketName Required to make call")
      return {"", "", {""}, {}, Aws::Crt::Optional<Aws::String>()};
...
void SmithyDefaultAsyncS3ExpressIdentityProvider::threadSafeKeyEmpty() {
  std::lock_guard<std::mutex> lock(m_keysUsedMutex);
  m_keysUsed.clear();
}

bool SmithyDefaultAsyncS3ExpressIdentityProvider::threadSafeKeyHas(const Aws::String& key) {
  std::lock_guard<std::mutex> lock(m_keysUsedMutex);
  return m_keysUsed.find(key) != m_keysUsed.end();
}

void SmithyDefaultAsyncS3ExpressIdentityProvider::threadSafeKeyInsert(const Aws::String& key) {
  std::lock_guard<std::mutex> lock(m_keysUsedMutex);
  m_keysUsed.insert(key);
}

void SmithyDefaultAsyncS3ExpressIdentityProvider::refreshIdentities(std::chrono::minutes refreshPeriod) {
  auto filterUnusedKeys = [&](const Aws::String &bucketName, const SmithyCacheValueType &valueType) -> bool {
      std::lock_guard<std::mutex> lock(*GetMutexForBucketName(bucketName));
      AWS_UNREFERENCED_PARAM(valueType);
      return !threadSafeKeyHas(bucketName);
  };
  auto refreshIdentityWhenCloseToExpiry = [&](const Aws::String &bucketName,
      const SmithyCacheValueType &valueType) -> SmithyCacheValueType {
      std::lock_guard<std::mutex> lock(*GetMutexForBucketName(bucketName));
      if  ((!valueType.val.expiration().has_value()) || 
          (duration_cast<milliseconds>(refreshPeriod).count() < valueType.val.expiration().value().Millis() &&
          valueType.val.expiration().value() - refreshPeriod < Aws::Utils::DateTime::Now())) {
          auto updatedIdentity = this->SmithyS3ExpressIdentityProvider::GetCredentialsFromBucket(bucketName);
          return {updatedIdentity.expiration().value(), updatedIdentity};
      }
      return valueType;
  };
