...
ObjectVersion::ObjectVersion(const XmlNode& xmlNode) { *this = xmlNode; }
...
ObjectVersion& ObjectVersion::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!storageClassNode.IsNull()) {
      m_storageClass = ObjectVersionStorageClassMapper::GetObjectVersionStorageClassForName(
    ...
    if (!isLatestNode.IsNull()) {
      m_isLatest =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isLatestNode.GetText()).c_str()).c_str());
