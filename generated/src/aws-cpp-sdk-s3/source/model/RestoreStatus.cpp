...
RestoreStatus& RestoreStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!isRestoreInProgressNode.IsNull()) {
      m_isRestoreInProgress = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isRestoreInProgressNode.GetText()).c_str()).c_str());
    ...
    if (!restoreExpiryDateNode.IsNull()) {
      m_restoreExpiryDate =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(restoreExpiryDateNode.GetText()).c_str()).c_str(),
