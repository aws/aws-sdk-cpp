...
CSVOutput::CSVOutput(const XmlNode& xmlNode) { *this = xmlNode; }
...
CSVOutput& CSVOutput::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!quoteFieldsNode.IsNull()) {
      m_quoteFields = QuoteFieldsMapper::GetQuoteFieldsForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(quoteFieldsNode.GetText()).c_str()));
