...
GetObjectResult& GetObjectResult::operator=(Aws::AmazonWebServiceResult<ResponseStream>&& result) {
  ...
  if (lastModifiedIter != headers.end()) {
    ...
    if (!m_lastModified.WasParseSuccessful()) {
      AWS_LOGSTREAM_WARN("S3::GetObjectResult",
  ...
  if (objectLockRetainUntilDateIter != headers.end()) {
    ...
    if (!m_objectLockRetainUntilDate.WasParseSuccessful()) {
      AWS_LOGSTREAM_WARN("S3::GetObjectResult", "Failed to parse objectLockRetainUntilDate header as an ISO_8601 timestamp: "
                                                    << objectLockRetainUntilDateIter->second.c_str());
    }
  ...
  if (objectLockLegalHoldStatusIter != headers.end()) {
    m_objectLockLegalHoldStatus =
        ObjectLockLegalHoldStatusMapper::GetObjectLockLegalHoldStatusForName(objectLockLegalHoldStatusIter->second);
