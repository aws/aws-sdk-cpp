...
Aws::Http::HeaderValueCollection WriteGetObjectResponseRequest::GetRequestSpecificHeaders() const {
  ...
  if (m_objectLockLegalHoldStatusHasBeenSet && m_objectLockLegalHoldStatus != ObjectLockLegalHoldStatus::NOT_SET) {
    headers.emplace("x-amz-fwd-header-x-amz-object-lock-legal-hold",
  ...
  if (m_objectLockRetainUntilDateHasBeenSet) {
    headers.emplace("x-amz-fwd-header-x-amz-object-lock-retain-until-date",
                    m_objectLockRetainUntilDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601));
  }
  ...
  if (m_serverSideEncryptionHasBeenSet && m_serverSideEncryption != ServerSideEncryption::NOT_SET) {
    headers.emplace("x-amz-fwd-header-x-amz-server-side-encryption",
                    ServerSideEncryptionMapper::GetNameForServerSideEncryption(m_serverSideEncryption));
  }
...
void WriteGetObjectResponseRequest::AddQueryStringParameters(Aws::Http::URI& uri) const {
  Aws::StringStream ss;
...
bool WriteGetObjectResponseRequest::HasEmbeddedError(Aws::IOStream& body, const Aws::Http::HeaderValueCollection& header) const {
...
Aws::Vector<smithy::AuthSchemeOption> WriteGetObjectResponseRequest::GetRequestSpecificSupportedAuth() const {
  Aws::Vector<smithy::AuthSchemeOption> authOptions{smithy::SigV4AuthSchemeOption::sigV4AuthSchemeOption};
  return authOptions;
}
