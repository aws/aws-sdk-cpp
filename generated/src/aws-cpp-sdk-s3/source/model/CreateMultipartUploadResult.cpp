...
CreateMultipartUploadResult::CreateMultipartUploadResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
CreateMultipartUploadResult& CreateMultipartUploadResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (abortDateIter != headers.end()) {
    ...
    if (!m_abortDate.WasParseSuccessful()) {
      AWS_LOGSTREAM_WARN("S3::CreateMultipartUploadResult",
                         "Failed to parse abortDate header as an RFC822 timestamp: " << abortDateIter->second.c_str());
    }
