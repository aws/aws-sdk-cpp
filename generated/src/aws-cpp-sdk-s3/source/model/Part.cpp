...
Part::Part(const XmlNode& xmlNode) { *this = xmlNode; }
...
Part& Part::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!partNumberNode.IsNull()) {
      m_partNumber =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(partNumberNode.GetText()).c_str()).c_str());
