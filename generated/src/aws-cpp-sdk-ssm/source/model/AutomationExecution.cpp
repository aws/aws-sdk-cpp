...
AutomationExecution::AutomationExecution(JsonView jsonValue) { *this = jsonValue; }
...
AutomationExecution& AutomationExecution::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AutomationExecutionStatus")) {
    m_automationExecutionStatus =
        AutomationExecutionStatusMapper::GetAutomationExecutionStatusForName(jsonValue.GetString("AutomationExecutionStatus"));
  ...
  if (jsonValue.ValueExists("Variables")) {
    ...
    for (auto& variablesItem : variablesJsonMap) {
      Aws::Utils::Array<JsonView> automationParameterValueList2JsonList = variablesItem.second.AsArray();
      Aws::Vector<Aws::String> automationParameterValueList2List;
      automationParameterValueList2List.reserve((size_t)automationParameterValueList2JsonList.GetLength());
      for (unsigned automationParameterValueList2Index = 0;
           automationParameterValueList2Index < automationParameterValueList2JsonList.GetLength(); ++automationParameterValueList2Index) {
      ...
      m_variables[variablesItem.first] = std::move(automationParameterValueList2List);
    }
...
JsonValue AutomationExecution::Jsonize() const {
  ...
  if (m_automationExecutionStatusHasBeenSet) {
    payload.WithString("AutomationExecutionStatus",
                       AutomationExecutionStatusMapper::GetNameForAutomationExecutionStatus(m_automationExecutionStatus));
  }
  ...
  if (m_outputsHasBeenSet) {
    ...
    for (auto& outputsItem : m_outputs) {
      ...
      for (unsigned automationParameterValueListIndex = 0;
           automationParameterValueListIndex < automationParameterValueListJsonList.GetLength(); ++automationParameterValueListIndex) {
        automationParameterValueListJsonList[automationParameterValueListIndex].AsString(
            outputsItem.second[automationParameterValueListIndex]);
      }
  ...
  if (m_variablesHasBeenSet) {
    ...
    for (auto& variablesItem : m_variables) {
      ...
      for (unsigned automationParameterValueListIndex = 0;
           automationParameterValueListIndex < automationParameterValueListJsonList.GetLength(); ++automationParameterValueListIndex) {
        automationParameterValueListJsonList[automationParameterValueListIndex].AsString(
            variablesItem.second[automationParameterValueListIndex]);
      }
