...
NotificationConfig& NotificationConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("NotificationEvents")) {
    ...
    for (unsigned notificationEventsIndex = 0; notificationEventsIndex < notificationEventsJsonList.GetLength();
      ...
      m_notificationEvents.push_back(
          NotificationEventMapper::GetNotificationEventForName(notificationEventsJsonList[notificationEventsIndex].AsString()));
    }
...
JsonValue NotificationConfig::Jsonize() const {
  ...
  if (m_notificationEventsHasBeenSet) {
    ...
    for (unsigned notificationEventsIndex = 0; notificationEventsIndex < notificationEventsJsonList.GetLength();
         ++notificationEventsIndex) {
      notificationEventsJsonList[notificationEventsIndex].AsString(
          NotificationEventMapper::GetNameForNotificationEvent(m_notificationEvents[notificationEventsIndex]));
    }
