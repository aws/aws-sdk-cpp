...
StepExecution::StepExecution(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue StepExecution::Jsonize() const {
  ...
  if (m_overriddenParametersHasBeenSet) {
    ...
    for (auto& overriddenParametersItem : m_overriddenParameters) {
      ...
      for (unsigned automationParameterValueListIndex = 0;
           automationParameterValueListIndex < automationParameterValueListJsonList.GetLength(); ++automationParameterValueListIndex) {
        automationParameterValueListJsonList[automationParameterValueListIndex].AsString(
            overriddenParametersItem.second[automationParameterValueListIndex]);
      }
  ...
  if (m_parentStepDetailsHasBeenSet) {
    payload.WithObject("ParentStepDetails", m_parentStepDetails.Jsonize());
  }
