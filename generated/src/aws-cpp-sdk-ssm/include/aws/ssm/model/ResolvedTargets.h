...
class ResolvedTargets {
 public:
  ...
  template <typename ParameterValuesT = Aws::Vector<Aws::String>>
  ResolvedTargets& WithParameterValues(ParameterValuesT&& value) {
    SetParameterValues(std::forward<ParameterValuesT>(value));
    return *this;
  }
  template <typename ParameterValuesT = Aws::String>
  ResolvedTargets& AddParameterValues(ParameterValuesT&& value) {
    m_parameterValuesHasBeenSet = true;
  ...
  inline ResolvedTargets& WithTruncated(bool value) {
    SetTruncated(value);
    return *this;
  }
  ///@}
