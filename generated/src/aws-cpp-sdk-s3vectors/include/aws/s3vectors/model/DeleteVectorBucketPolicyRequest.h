...
namespace Model
{
  ...
  /**
   */
  class DeleteVectorBucketPolicyRequest : public S3VectorsRequest
  {
  public:
    AWS_S3VECTORS_API DeleteVectorBucketPolicyRequest() = default;

    // Service request name is the Operation name which will send this request out,
    // each operation should has unique request name, so that we can get operation's name from this request.
    // Note: this is not true for response, multiple operations may have the same response name,
    // so we can not get operation's name from response.
    inline virtual const char* GetServiceRequestName() const override { return "DeleteVectorBucketPolicy"; }
    ...
    template<typename VectorBucketNameT = Aws::String>
    DeleteVectorBucketPolicyRequest& WithVectorBucketName(VectorBucketNameT&& value) { SetVectorBucketName(std::forward<VectorBucketNameT>(value)); return *this;}
    ///@}
    ...
    template<typename VectorBucketArnT = Aws::String>
    DeleteVectorBucketPolicyRequest& WithVectorBucketArn(VectorBucketArnT&& value) { SetVectorBucketArn(std::forward<VectorBucketArnT>(value)); return *this;}
    ///@}
  private:
