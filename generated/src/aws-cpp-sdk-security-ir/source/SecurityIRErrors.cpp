...
template <>
AWS_SECURITYIR_API ConflictException SecurityIRError::GetModeledError() {
  assert(this->GetErrorType() == SecurityIRErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_SECURITYIR_API ServiceQuotaExceededException SecurityIRError::GetModeledError() {
  assert(this->GetErrorType() == SecurityIRErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_SECURITYIR_API ThrottlingException SecurityIRError::GetModeledError() {
  assert(this->GetErrorType() == SecurityIRErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_SECURITYIR_API InternalServerException SecurityIRError::GetModeledError() {
  assert(this->GetErrorType() == SecurityIRErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_SECURITYIR_API ValidationException SecurityIRError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(SecurityIRErrors::SECURITY_INCIDENT_RESPONSE_NOT_ACTIVE),
