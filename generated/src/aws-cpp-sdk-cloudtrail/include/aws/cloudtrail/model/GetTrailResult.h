...
class GetTrailResult {
 public:
  ...
  void SetTrail(TrailT&& value) {
    m_trailHasBeenSet = true;
    m_trail = std::forward<TrailT>(value);
  }
  template <typename TrailT = Trail>
  GetTrailResult& WithTrail(TrailT&& value) {
    SetTrail(std::forward<TrailT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetTrailResult& WithRequestId(RequestIdT&& value) {
