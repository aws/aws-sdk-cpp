...
class PartListElement {
 public:
  ...
  void SetRangeInBytes(RangeInBytesT&& value) {
    ...
    m_rangeInBytes = std::forward<RangeInBytesT>(value);
  }
  template <typename RangeInBytesT = Aws::String>
  PartListElement& WithRangeInBytes(RangeInBytesT&& value) {
    SetRangeInBytes(std::forward<RangeInBytesT>(value));
    return *this;
  }
  ///@}
  ...
  void SetSHA256TreeHash(SHA256TreeHashT&& value) {
    ...
    m_sHA256TreeHash = std::forward<SHA256TreeHashT>(value);
  }
  template <typename SHA256TreeHashT = Aws::String>
  PartListElement& WithSHA256TreeHash(SHA256TreeHashT&& value) {
    SetSHA256TreeHash(std::forward<SHA256TreeHashT>(value));
    return *this;
  }
  ///@}
 private:
