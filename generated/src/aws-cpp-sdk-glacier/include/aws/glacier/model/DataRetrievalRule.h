...
class DataRetrievalRule {
 public:
  ...
  void SetStrategy(StrategyT&& value) {
    ...
    m_strategy = std::forward<StrategyT>(value);
  }
  template <typename StrategyT = Aws::String>
  DataRetrievalRule& WithStrategy(StrategyT&& value) {
    SetStrategy(std::forward<StrategyT>(value));
    return *this;
  }
  ///@}
  ...
  inline void SetBytesPerHour(long long value) {
    m_bytesPerHourHasBeenSet = true;
    m_bytesPerHour = value;
  }
  inline DataRetrievalRule& WithBytesPerHour(long long value) {
    SetBytesPerHour(value);
    return *this;
  }
  ///@}
 private:
