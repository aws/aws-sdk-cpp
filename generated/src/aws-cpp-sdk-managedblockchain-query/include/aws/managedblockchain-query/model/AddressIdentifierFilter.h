...
class AddressIdentifierFilter {
 public:
  ...
  void SetTransactionEventToAddress(TransactionEventToAddressT&& value) {
    m_transactionEventToAddressHasBeenSet = true;
    m_transactionEventToAddress = std::forward<TransactionEventToAddressT>(value);
  }
  template <typename TransactionEventToAddressT = Aws::Vector<Aws::String>>
  AddressIdentifierFilter& WithTransactionEventToAddress(TransactionEventToAddressT&& value) {
    SetTransactionEventToAddress(std::forward<TransactionEventToAddressT>(value));
    return *this;
  }
  template <typename TransactionEventToAddressT = Aws::String>
  AddressIdentifierFilter& AddTransactionEventToAddress(TransactionEventToAddressT&& value) {
    m_transactionEventToAddressHasBeenSet = true;
    m_transactionEventToAddress.emplace_back(std::forward<TransactionEventToAddressT>(value));
    return *this;
  }
  ///@}
 private:
