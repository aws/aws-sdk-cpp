...
class CompositeSlotTypeSetting {
 public:
  ...
  void SetSubSlots(SubSlotsT&& value) {
    ...
    m_subSlots = std::forward<SubSlotsT>(value);
  }
  template <typename SubSlotsT = Aws::Vector<SubSlotTypeComposition>>
  CompositeSlotTypeSetting& WithSubSlots(SubSlotsT&& value) {
    SetSubSlots(std::forward<SubSlotsT>(value));
    return *this;
  }
  template <typename SubSlotsT = SubSlotTypeComposition>
  CompositeSlotTypeSetting& AddSubSlots(SubSlotsT&& value) {
    m_subSlotsHasBeenSet = true;
    m_subSlots.emplace_back(std::forward<SubSlotsT>(value));
    return *this;
  }
  ///@}
 private:
