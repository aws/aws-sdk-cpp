...
class TestSetGenerationDataSource {
 public:
  ...
  void SetConversationLogsDataSource(ConversationLogsDataSourceT&& value) {
    m_conversationLogsDataSourceHasBeenSet = true;
    m_conversationLogsDataSource = std::forward<ConversationLogsDataSourceT>(value);
  }
  template <typename ConversationLogsDataSourceT = ConversationLogsDataSource>
  TestSetGenerationDataSource& WithConversationLogsDataSource(ConversationLogsDataSourceT&& value) {
    SetConversationLogsDataSource(std::forward<ConversationLogsDataSourceT>(value));
    return *this;
  }
  ///@}
 private:
