...
class CodeHookSpecification {
 public:
  ...
  void SetLambdaCodeHook(LambdaCodeHookT&& value) {
    m_lambdaCodeHookHasBeenSet = true;
    m_lambdaCodeHook = std::forward<LambdaCodeHookT>(value);
  }
  template <typename LambdaCodeHookT = LambdaCodeHook>
  CodeHookSpecification& WithLambdaCodeHook(LambdaCodeHookT&& value) {
    SetLambdaCodeHook(std::forward<LambdaCodeHookT>(value));
    return *this;
  }
  ///@}
 private:
