...
class TranscriptSourceSetting {
 public:
  ...
  void SetS3BucketTranscriptSource(S3BucketTranscriptSourceT&& value) {
    m_s3BucketTranscriptSourceHasBeenSet = true;
    m_s3BucketTranscriptSource = std::forward<S3BucketTranscriptSourceT>(value);
  }
  template <typename S3BucketTranscriptSourceT = S3BucketTranscriptSource>
  TranscriptSourceSetting& WithS3BucketTranscriptSource(S3BucketTranscriptSourceT&& value) {
    SetS3BucketTranscriptSource(std::forward<S3BucketTranscriptSourceT>(value));
    return *this;
  }
  ///@}
 private:
