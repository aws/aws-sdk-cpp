...
/**
 */
class StopBotAnalyzerRequest : public LexModelsV2Request {
 public:
  AWS_LEXMODELSV2_API StopBotAnalyzerRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "StopBotAnalyzer"; }
  ...
  template <typename BotIdT = Aws::String>
  StopBotAnalyzerRequest& WithBotId(BotIdT&& value) {
  ...
  template <typename BotAnalyzerRequestIdT = Aws::String>
  StopBotAnalyzerRequest& WithBotAnalyzerRequestId(BotAnalyzerRequestIdT&& value) {
