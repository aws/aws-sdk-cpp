...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == INTERNAL_ERROR_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(CognitoIdentityErrors::LIMIT_EXCEEDED), RetryableType::RETRYABLE_THROTTLING);
  } else if (hashCode == TOO_MANY_REQUESTS_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(CognitoIdentityErrors::TOO_MANY_REQUESTS), RetryableType::RETRYABLE_THROTTLING);
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(CognitoIdentityErrors::INVALID_IDENTITY_POOL_CONFIGURATION),
                                RetryableType::NOT_RETRYABLE);
  } else if (hashCode == DEVELOPER_USER_ALREADY_REGISTERED_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(CognitoIdentityErrors::DEVELOPER_USER_ALREADY_REGISTERED),
                                RetryableType::NOT_RETRYABLE);
  }
