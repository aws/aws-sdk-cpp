...
class DeltaTime {
 public:
  ...
  inline void SetOffsetSeconds(int value) {
    m_offsetSecondsHasBeenSet = true;
    m_offsetSeconds = value;
  }
  inline DeltaTime& WithOffsetSeconds(int value) {
  ...
  void SetTimeExpression(TimeExpressionT&& value) {
    ...
    m_timeExpression = std::forward<TimeExpressionT>(value);
  }
  template <typename TimeExpressionT = Aws::String>
  DeltaTime& WithTimeExpression(TimeExpressionT&& value) {
    SetTimeExpression(std::forward<TimeExpressionT>(value));
    return *this;
  }
  ///@}
 private:
