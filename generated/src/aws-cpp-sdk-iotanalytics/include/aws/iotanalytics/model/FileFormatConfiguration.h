...
class FileFormatConfiguration {
 public:
  ...
  void SetJsonConfiguration(JsonConfigurationT&& value) {
    m_jsonConfigurationHasBeenSet = true;
    m_jsonConfiguration = std::forward<JsonConfigurationT>(value);
  }
  template <typename JsonConfigurationT = JsonConfiguration>
  FileFormatConfiguration& WithJsonConfiguration(JsonConfigurationT&& value) {
    SetJsonConfiguration(std::forward<JsonConfigurationT>(value));
    return *this;
  }
  ///@}
  ...
  void SetParquetConfiguration(ParquetConfigurationT&& value) {
    ...
    m_parquetConfiguration = std::forward<ParquetConfigurationT>(value);
  }
  template <typename ParquetConfigurationT = ParquetConfiguration>
  FileFormatConfiguration& WithParquetConfiguration(ParquetConfigurationT&& value) {
    SetParquetConfiguration(std::forward<ParquetConfigurationT>(value));
    return *this;
  }
  ///@}
 private:
