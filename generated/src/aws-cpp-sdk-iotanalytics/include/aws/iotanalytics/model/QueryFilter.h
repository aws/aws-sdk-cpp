...
class QueryFilter {
 public:
  ...
  void SetDeltaTime(DeltaTimeT&& value) {
    m_deltaTimeHasBeenSet = true;
    m_deltaTime = std::forward<DeltaTimeT>(value);
  }
  template <typename DeltaTimeT = DeltaTime>
  QueryFilter& WithDeltaTime(DeltaTimeT&& value) {
    SetDeltaTime(std::forward<DeltaTimeT>(value));
    return *this;
  }
  ///@}
 private:
