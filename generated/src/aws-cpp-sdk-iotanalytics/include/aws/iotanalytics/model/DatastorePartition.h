...
class DatastorePartition {
 public:
  ...
  void SetAttributePartition(AttributePartitionT&& value) {
    ...
    m_attributePartition = std::forward<AttributePartitionT>(value);
  }
  template <typename AttributePartitionT = Partition>
  DatastorePartition& WithAttributePartition(AttributePartitionT&& value) {
    SetAttributePartition(std::forward<AttributePartitionT>(value));
    return *this;
  }
  ///@}
  ...
  void SetTimestampPartition(TimestampPartitionT&& value) {
    ...
    m_timestampPartition = std::forward<TimestampPartitionT>(value);
  }
  template <typename TimestampPartitionT = TimestampPartition>
  DatastorePartition& WithTimestampPartition(TimestampPartitionT&& value) {
    SetTimestampPartition(std::forward<TimestampPartitionT>(value));
    return *this;
  }
  ///@}
 private:
