...
template <>
AWS_MANAGEDBLOCKCHAIN_API ResourceNotFoundException ManagedBlockchainError::GetModeledError() {
  assert(this->GetErrorType() == ManagedBlockchainErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_MANAGEDBLOCKCHAIN_API TooManyTagsException ManagedBlockchainError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == RESOURCE_NOT_READY_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ManagedBlockchainErrors::INTERNAL_SERVICE_ERROR), RetryableType::RETRYABLE);
