...
class GetProposalResult {
 public:
  ...
  void SetProposal(ProposalT&& value) {
    m_proposalHasBeenSet = true;
    m_proposal = std::forward<ProposalT>(value);
  }
  template <typename ProposalT = Proposal>
  GetProposalResult& WithProposal(ProposalT&& value) {
    SetProposal(std::forward<ProposalT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetProposalResult& WithRequestId(RequestIdT&& value) {
