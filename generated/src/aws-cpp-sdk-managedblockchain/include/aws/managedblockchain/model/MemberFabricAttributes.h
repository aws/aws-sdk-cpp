...
class MemberFabricAttributes {
 public:
  ...
  template <typename AdminUsernameT = Aws::String>
  MemberFabricAttributes& WithAdminUsername(AdminUsernameT&& value) {
  ...
  void SetCaEndpoint(CaEndpointT&& value) {
    m_caEndpointHasBeenSet = true;
    m_caEndpoint = std::forward<CaEndpointT>(value);
  }
  template <typename CaEndpointT = Aws::String>
  MemberFabricAttributes& WithCaEndpoint(CaEndpointT&& value) {
    SetCaEndpoint(std::forward<CaEndpointT>(value));
    return *this;
  }
  ///@}
 private:
