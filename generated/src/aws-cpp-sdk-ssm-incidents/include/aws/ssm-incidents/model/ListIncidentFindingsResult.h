...
class ListIncidentFindingsResult {
 public:
  AWS_SSMINCIDENTS_API ListIncidentFindingsResult() = default;
  ...
  template <typename FindingsT = Aws::Vector<FindingSummary>>
  ListIncidentFindingsResult& WithFindings(FindingsT&& value) {
    SetFindings(std::forward<FindingsT>(value));
    return *this;
  }
  template <typename FindingsT = FindingSummary>
  ListIncidentFindingsResult& AddFindings(FindingsT&& value) {
    m_findingsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListIncidentFindingsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListIncidentFindingsResult& WithRequestId(RequestIdT&& value) {
