...
class ChatChannel {
 public:
  ...
  void SetChatbotSns(ChatbotSnsT&& value) {
    ...
    m_chatbotSns = std::forward<ChatbotSnsT>(value);
  }
  template <typename ChatbotSnsT = Aws::Vector<Aws::String>>
  ChatChannel& WithChatbotSns(ChatbotSnsT&& value) {
    SetChatbotSns(std::forward<ChatbotSnsT>(value));
    return *this;
  }
  template <typename ChatbotSnsT = Aws::String>
  ChatChannel& AddChatbotSns(ChatbotSnsT&& value) {
    m_chatbotSnsHasBeenSet = true;
    m_chatbotSns.emplace_back(std::forward<ChatbotSnsT>(value));
    return *this;
  }
  ///@}
  ...
  void SetEmpty(EmptyT&& value) {
    ...
    m_empty = std::forward<EmptyT>(value);
  }
  template <typename EmptyT = EmptyChatChannel>
  ChatChannel& WithEmpty(EmptyT&& value) {
    SetEmpty(std::forward<EmptyT>(value));
    return *this;
  }
  ///@}
 private:
