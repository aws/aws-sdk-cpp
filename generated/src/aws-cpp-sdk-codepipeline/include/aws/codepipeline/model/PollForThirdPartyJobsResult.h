...
class PollForThirdPartyJobsResult {
 public:
  ...
  template <typename JobsT = Aws::Vector<ThirdPartyJob>>
  PollForThirdPartyJobsResult& WithJobs(JobsT&& value) {
    SetJobs(std::forward<JobsT>(value));
    return *this;
  }
  template <typename JobsT = ThirdPartyJob>
  PollForThirdPartyJobsResult& AddJobs(JobsT&& value) {
    m_jobsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  PollForThirdPartyJobsResult& WithRequestId(RequestIdT&& value) {
