...
class StageConditionState {
 public:
  ...
  void SetLatestExecution(LatestExecutionT&& value) {
    ...
    m_latestExecution = std::forward<LatestExecutionT>(value);
  }
  template <typename LatestExecutionT = StageConditionsExecution>
  StageConditionState& WithLatestExecution(LatestExecutionT&& value) {
    SetLatestExecution(std::forward<LatestExecutionT>(value));
    return *this;
  }
  ///@}
  ...
  void SetConditionStates(ConditionStatesT&& value) {
    m_conditionStatesHasBeenSet = true;
    m_conditionStates = std::forward<ConditionStatesT>(value);
  }
  template <typename ConditionStatesT = Aws::Vector<ConditionState>>
  StageConditionState& WithConditionStates(ConditionStatesT&& value) {
    SetConditionStates(std::forward<ConditionStatesT>(value));
    return *this;
  }
  template <typename ConditionStatesT = ConditionState>
  StageConditionState& AddConditionStates(ConditionStatesT&& value) {
    m_conditionStatesHasBeenSet = true;
    m_conditionStates.emplace_back(std::forward<ConditionStatesT>(value));
    return *this;
  }
  ///@}
 private:
