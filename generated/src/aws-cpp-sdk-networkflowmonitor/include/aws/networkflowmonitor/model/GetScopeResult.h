...
class GetScopeResult {
 public:
  ...
  inline GetScopeResult& WithStatus(ScopeStatus value) {
    SetStatus(value);
    return *this;
  }
  ///@}
  ...
  template <typename TargetsT = Aws::Vector<TargetResource>>
  GetScopeResult& WithTargets(TargetsT&& value) {
    SetTargets(std::forward<TargetsT>(value));
    return *this;
  }
  template <typename TargetsT = TargetResource>
  GetScopeResult& AddTargets(TargetsT&& value) {
    m_targetsHasBeenSet = true;
  ...
  template <typename TagsT = Aws::Map<Aws::String, Aws::String>>
  GetScopeResult& WithTags(TagsT&& value) {
