...
Aws::String UpdateMonitorRequest::SerializePayload() const {
  ...
  if (m_localResourcesToAddHasBeenSet) {
    ...
    for (unsigned localResourcesToAddIndex = 0; localResourcesToAddIndex < localResourcesToAddJsonList.GetLength();
         ++localResourcesToAddIndex) {
  ...
  if (m_localResourcesToRemoveHasBeenSet) {
    ...
    for (unsigned localResourcesToRemoveIndex = 0; localResourcesToRemoveIndex < localResourcesToRemoveJsonList.GetLength();
         ++localResourcesToRemoveIndex) {
  ...
  if (m_remoteResourcesToAddHasBeenSet) {
    ...
    for (unsigned remoteResourcesToAddIndex = 0; remoteResourcesToAddIndex < remoteResourcesToAddJsonList.GetLength();
         ++remoteResourcesToAddIndex) {
  ...
  if (m_remoteResourcesToRemoveHasBeenSet) {
    ...
    for (unsigned remoteResourcesToRemoveIndex = 0; remoteResourcesToRemoveIndex < remoteResourcesToRemoveJsonList.GetLength();
         ++remoteResourcesToRemoveIndex) {
      remoteResourcesToRemoveJsonList[remoteResourcesToRemoveIndex].AsObject(
          m_remoteResourcesToRemove[remoteResourcesToRemoveIndex].Jsonize());
    }
