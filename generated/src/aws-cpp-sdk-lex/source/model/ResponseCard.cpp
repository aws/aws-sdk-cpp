...
ResponseCard::ResponseCard(JsonView jsonValue) { *this = jsonValue; }
...
ResponseCard& ResponseCard::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("genericAttachments")) {
    ...
    for (unsigned genericAttachmentsIndex = 0; genericAttachmentsIndex < genericAttachmentsJsonList.GetLength();
         ++genericAttachmentsIndex) {
