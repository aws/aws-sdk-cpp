...
class RegisterAdminRequest : public RepostspaceRequest {
 public:
  ...
  template <typename SpaceIdT = Aws::String>
  RegisterAdminRequest& WithSpaceId(SpaceIdT&& value) {
  ...
  void SetAdminId(AdminIdT&& value) {
    m_adminIdHasBeenSet = true;
    m_adminId = std::forward<AdminIdT>(value);
  }
  template <typename AdminIdT = Aws::String>
  RegisterAdminRequest& WithAdminId(AdminIdT&& value) {
    SetAdminId(std::forward<AdminIdT>(value));
    return *this;
  }
  ///@}
 private:
