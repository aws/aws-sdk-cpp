...
template <>
AWS_REPOSTSPACE_API ConflictException RepostspaceError::GetModeledError() {
  assert(this->GetErrorType() == RepostspaceErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_REPOSTSPACE_API ThrottlingException RepostspaceError::GetModeledError() {
  assert(this->GetErrorType() == RepostspaceErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_REPOSTSPACE_API ServiceQuotaExceededException RepostspaceError::GetModeledError() {
  assert(this->GetErrorType() == RepostspaceErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_REPOSTSPACE_API ResourceNotFoundException RepostspaceError::GetModeledError() {
  assert(this->GetErrorType() == RepostspaceErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_REPOSTSPACE_API InternalServerException RepostspaceError::GetModeledError() {
  assert(this->GetErrorType() == RepostspaceErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_REPOSTSPACE_API ValidationException RepostspaceError::GetModeledError() {
