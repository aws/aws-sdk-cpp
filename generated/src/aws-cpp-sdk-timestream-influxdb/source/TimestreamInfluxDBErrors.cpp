...
template <>
AWS_TIMESTREAMINFLUXDB_API ConflictException TimestreamInfluxDBError::GetModeledError() {
  assert(this->GetErrorType() == TimestreamInfluxDBErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_TIMESTREAMINFLUXDB_API ThrottlingException TimestreamInfluxDBError::GetModeledError() {
  assert(this->GetErrorType() == TimestreamInfluxDBErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_TIMESTREAMINFLUXDB_API ResourceNotFoundException TimestreamInfluxDBError::GetModeledError() {
  assert(this->GetErrorType() == TimestreamInfluxDBErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_TIMESTREAMINFLUXDB_API ValidationException TimestreamInfluxDBError::GetModeledError() {
