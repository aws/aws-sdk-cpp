...
class ValueHolder {
 public:
  ...
  void SetIonBinary(IonBinaryT&& value) {
    m_ionBinaryHasBeenSet = true;
    m_ionBinary = std::forward<IonBinaryT>(value);
  }
  template <typename IonBinaryT = Aws::Utils::ByteBuffer>
  ValueHolder& WithIonBinary(IonBinaryT&& value) {
    SetIonBinary(std::forward<IonBinaryT>(value));
    return *this;
  }
  ///@}
  ...
  void SetIonText(IonTextT&& value) {
    ...
    m_ionText = std::forward<IonTextT>(value);
  }
  template <typename IonTextT = Aws::String>
  ValueHolder& WithIonText(IonTextT&& value) {
    SetIonText(std::forward<IonTextT>(value));
    return *this;
  }
  ///@}
 private:
