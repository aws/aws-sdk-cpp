...
class ClickFeedback {
 public:
  ...
  template <typename ResultIdT = Aws::String>
  ClickFeedback& WithResultId(ResultIdT&& value) {
  ...
  void SetClickTime(ClickTimeT&& value) {
    m_clickTimeHasBeenSet = true;
    m_clickTime = std::forward<ClickTimeT>(value);
  }
  template <typename ClickTimeT = Aws::Utils::DateTime>
  ClickFeedback& WithClickTime(ClickTimeT&& value) {
    SetClickTime(std::forward<ClickTimeT>(value));
    return *this;
  }
  ///@}
 private:
