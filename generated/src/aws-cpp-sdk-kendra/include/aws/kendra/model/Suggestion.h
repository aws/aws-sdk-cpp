...
class Suggestion {
 public:
  ...
  void SetSourceDocuments(SourceDocumentsT&& value) {
    ...
    m_sourceDocuments = std::forward<SourceDocumentsT>(value);
  }
  template <typename SourceDocumentsT = Aws::Vector<SourceDocument>>
  Suggestion& WithSourceDocuments(SourceDocumentsT&& value) {
    SetSourceDocuments(std::forward<SourceDocumentsT>(value));
    return *this;
  }
  template <typename SourceDocumentsT = SourceDocument>
  Suggestion& AddSourceDocuments(SourceDocumentsT&& value) {
    m_sourceDocumentsHasBeenSet = true;
    m_sourceDocuments.emplace_back(std::forward<SourceDocumentsT>(value));
    return *this;
  }
  ///@}
 private:
