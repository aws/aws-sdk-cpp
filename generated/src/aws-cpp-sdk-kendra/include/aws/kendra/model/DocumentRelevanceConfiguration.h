...
class DocumentRelevanceConfiguration {
 public:
  ...
  template <typename NameT = Aws::String>
  DocumentRelevanceConfiguration& WithName(NameT&& value) {
  ...
  void SetRelevance(RelevanceT&& value) {
    m_relevanceHasBeenSet = true;
    m_relevance = std::forward<RelevanceT>(value);
  }
  template <typename RelevanceT = Relevance>
  DocumentRelevanceConfiguration& WithRelevance(RelevanceT&& value) {
    SetRelevance(std::forward<RelevanceT>(value));
    return *this;
  }
  ///@}
 private:
