...
AttributeSuggestionsGetConfig::AttributeSuggestionsGetConfig(JsonView jsonValue) { *this = jsonValue; }
...
AttributeSuggestionsGetConfig& AttributeSuggestionsGetConfig::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("SuggestionAttributes")) {
    ...
    for (unsigned suggestionAttributesIndex = 0; suggestionAttributesIndex < suggestionAttributesJsonList.GetLength();
         ++suggestionAttributesIndex) {
  ...
  if (jsonValue.ValueExists("AdditionalResponseAttributes")) {
    ...
    for (unsigned additionalResponseAttributesIndex = 0;
         additionalResponseAttributesIndex < additionalResponseAttributesJsonList.GetLength(); ++additionalResponseAttributesIndex) {
...
JsonValue AttributeSuggestionsGetConfig::Jsonize() const {
  ...
  if (m_additionalResponseAttributesHasBeenSet) {
    ...
    for (unsigned additionalResponseAttributesIndex = 0;
         additionalResponseAttributesIndex < additionalResponseAttributesJsonList.GetLength(); ++additionalResponseAttributesIndex) {
      additionalResponseAttributesJsonList[additionalResponseAttributesIndex].AsString(
          m_additionalResponseAttributes[additionalResponseAttributesIndex]);
    }
