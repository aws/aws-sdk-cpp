...
SalesforceChatterFeedConfiguration::SalesforceChatterFeedConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
SalesforceChatterFeedConfiguration& SalesforceChatterFeedConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("IncludeFilterTypes")) {
    ...
    for (unsigned includeFilterTypesIndex = 0; includeFilterTypesIndex < includeFilterTypesJsonList.GetLength();
         ++includeFilterTypesIndex) {
      m_includeFilterTypes.push_back(SalesforceChatterFeedIncludeFilterTypeMapper::GetSalesforceChatterFeedIncludeFilterTypeForName(
          includeFilterTypesJsonList[includeFilterTypesIndex].AsString()));
    }
...
JsonValue SalesforceChatterFeedConfiguration::Jsonize() const {
  ...
  if (m_includeFilterTypesHasBeenSet) {
    ...
    for (unsigned includeFilterTypesIndex = 0; includeFilterTypesIndex < includeFilterTypesJsonList.GetLength();
         ++includeFilterTypesIndex) {
      includeFilterTypesJsonList[includeFilterTypesIndex].AsString(
          SalesforceChatterFeedIncludeFilterTypeMapper::GetNameForSalesforceChatterFeedIncludeFilterType(
              m_includeFilterTypes[includeFilterTypesIndex]));
    }
