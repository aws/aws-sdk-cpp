...
SlackConfiguration& SlackConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("PrivateChannelFilter")) {
    ...
    for (unsigned privateChannelFilterIndex = 0; privateChannelFilterIndex < privateChannelFilterJsonList.GetLength();
         ++privateChannelFilterIndex) {
  ...
  if (jsonValue.ValueExists("PublicChannelFilter")) {
    ...
    for (unsigned publicChannelFilterIndex = 0; publicChannelFilterIndex < publicChannelFilterJsonList.GetLength();
         ++publicChannelFilterIndex) {
...
JsonValue SlackConfiguration::Jsonize() const {
  ...
  if (m_slackEntityListHasBeenSet) {
    ...
    for (unsigned slackEntityListIndex = 0; slackEntityListIndex < slackEntityListJsonList.GetLength(); ++slackEntityListIndex) {
      slackEntityListJsonList[slackEntityListIndex].AsString(
          SlackEntityMapper::GetNameForSlackEntity(m_slackEntityList[slackEntityListIndex]));
    }
