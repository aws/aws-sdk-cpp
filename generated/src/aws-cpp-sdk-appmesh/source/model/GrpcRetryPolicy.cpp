...
GrpcRetryPolicy::GrpcRetryPolicy(JsonView jsonValue) { *this = jsonValue; }
...
GrpcRetryPolicy& GrpcRetryPolicy::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("grpcRetryEvents")) {
    ...
    for (unsigned grpcRetryEventsIndex = 0; grpcRetryEventsIndex < grpcRetryEventsJsonList.GetLength(); ++grpcRetryEventsIndex) {
      m_grpcRetryEvents.push_back(
          GrpcRetryPolicyEventMapper::GetGrpcRetryPolicyEventForName(grpcRetryEventsJsonList[grpcRetryEventsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("tcpRetryEvents")) {
    ...
    for (unsigned tcpRetryEventsIndex = 0; tcpRetryEventsIndex < tcpRetryEventsJsonList.GetLength(); ++tcpRetryEventsIndex) {
      m_tcpRetryEvents.push_back(
          TcpRetryPolicyEventMapper::GetTcpRetryPolicyEventForName(tcpRetryEventsJsonList[tcpRetryEventsIndex].AsString()));
    }
...
JsonValue GrpcRetryPolicy::Jsonize() const {
  ...
  if (m_grpcRetryEventsHasBeenSet) {
    ...
    for (unsigned grpcRetryEventsIndex = 0; grpcRetryEventsIndex < grpcRetryEventsJsonList.GetLength(); ++grpcRetryEventsIndex) {
      grpcRetryEventsJsonList[grpcRetryEventsIndex].AsString(
          GrpcRetryPolicyEventMapper::GetNameForGrpcRetryPolicyEvent(m_grpcRetryEvents[grpcRetryEventsIndex]));
    }
  ...
  if (m_tcpRetryEventsHasBeenSet) {
    ...
    for (unsigned tcpRetryEventsIndex = 0; tcpRetryEventsIndex < tcpRetryEventsJsonList.GetLength(); ++tcpRetryEventsIndex) {
      tcpRetryEventsJsonList[tcpRetryEventsIndex].AsString(
          TcpRetryPolicyEventMapper::GetNameForTcpRetryPolicyEvent(m_tcpRetryEvents[tcpRetryEventsIndex]));
    }
