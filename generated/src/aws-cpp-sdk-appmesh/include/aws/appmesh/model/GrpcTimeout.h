...
class GrpcTimeout {
 public:
  AWS_APPMESH_API GrpcTimeout() = default;
  ...
  void SetIdle(IdleT&& value) {
    ...
    m_idle = std::forward<IdleT>(value);
  }
  template <typename IdleT = Duration>
  GrpcTimeout& WithIdle(IdleT&& value) {
    SetIdle(std::forward<IdleT>(value));
    return *this;
  }
  ///@}
  ...
  void SetPerRequest(PerRequestT&& value) {
    m_perRequestHasBeenSet = true;
    m_perRequest = std::forward<PerRequestT>(value);
  }
  template <typename PerRequestT = Duration>
  GrpcTimeout& WithPerRequest(PerRequestT&& value) {
    SetPerRequest(std::forward<PerRequestT>(value));
    return *this;
  }
  ///@}
 private:
