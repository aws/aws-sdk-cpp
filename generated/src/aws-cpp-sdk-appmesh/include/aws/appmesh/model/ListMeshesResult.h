...
class ListMeshesResult {
 public:
  ...
  void SetMeshes(MeshesT&& value) {
    m_meshesHasBeenSet = true;
    m_meshes = std::forward<MeshesT>(value);
  }
  template <typename MeshesT = Aws::Vector<MeshRef>>
  ListMeshesResult& WithMeshes(MeshesT&& value) {
    SetMeshes(std::forward<MeshesT>(value));
    return *this;
  }
  template <typename MeshesT = MeshRef>
  ListMeshesResult& AddMeshes(MeshesT&& value) {
    m_meshesHasBeenSet = true;
    m_meshes.emplace_back(std::forward<MeshesT>(value));
    return *this;
  }
  ///@}
