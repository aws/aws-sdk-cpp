...
class ListenerTlsValidationContextTrust {
 public:
  ...
  template <typename FileT = TlsValidationContextFileTrust>
  ListenerTlsValidationContextTrust& WithFile(FileT&& value) {
  ...
  void SetSds(SdsT&& value) {
    ...
    m_sds = std::forward<SdsT>(value);
  }
  template <typename SdsT = TlsValidationContextSdsTrust>
  ListenerTlsValidationContextTrust& WithSds(SdsT&& value) {
    SetSds(std::forward<SdsT>(value));
    return *this;
  }
  ///@}
 private:
