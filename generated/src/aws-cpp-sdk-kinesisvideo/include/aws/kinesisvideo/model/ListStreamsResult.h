...
class ListStreamsResult {
 public:
  ...
  void SetStreamInfoList(StreamInfoListT&& value) {
    ...
    m_streamInfoList = std::forward<StreamInfoListT>(value);
  }
  template <typename StreamInfoListT = Aws::Vector<StreamInfo>>
  ListStreamsResult& WithStreamInfoList(StreamInfoListT&& value) {
    SetStreamInfoList(std::forward<StreamInfoListT>(value));
    return *this;
  }
  template <typename StreamInfoListT = StreamInfo>
  ListStreamsResult& AddStreamInfoList(StreamInfoListT&& value) {
    m_streamInfoListHasBeenSet = true;
    m_streamInfoList.emplace_back(std::forward<StreamInfoListT>(value));
    return *this;
  }
  ///@}
