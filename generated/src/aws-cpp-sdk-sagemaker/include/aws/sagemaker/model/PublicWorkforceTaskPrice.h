...
class PublicWorkforceTaskPrice {
 public:
  ...
  void SetAmountInUsd(AmountInUsdT&& value) {
    m_amountInUsdHasBeenSet = true;
    m_amountInUsd = std::forward<AmountInUsdT>(value);
  }
  template <typename AmountInUsdT = USD>
  PublicWorkforceTaskPrice& WithAmountInUsd(AmountInUsdT&& value) {
    SetAmountInUsd(std::forward<AmountInUsdT>(value));
    return *this;
  }
  ///@}
 private:
