...
class ScalingPolicy {
 public:
  ...
  void SetTargetTracking(TargetTrackingT&& value) {
    m_targetTrackingHasBeenSet = true;
    m_targetTracking = std::forward<TargetTrackingT>(value);
  }
  template <typename TargetTrackingT = TargetTrackingScalingPolicyConfiguration>
  ScalingPolicy& WithTargetTracking(TargetTrackingT&& value) {
    SetTargetTracking(std::forward<TargetTrackingT>(value));
    return *this;
  }
  ///@}
 private:
