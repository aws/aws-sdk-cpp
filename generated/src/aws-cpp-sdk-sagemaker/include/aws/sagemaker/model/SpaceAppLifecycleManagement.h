...
class SpaceAppLifecycleManagement {
 public:
  ...
  void SetIdleSettings(IdleSettingsT&& value) {
    m_idleSettingsHasBeenSet = true;
    m_idleSettings = std::forward<IdleSettingsT>(value);
  }
  template <typename IdleSettingsT = SpaceIdleSettings>
  SpaceAppLifecycleManagement& WithIdleSettings(IdleSettingsT&& value) {
    SetIdleSettings(std::forward<IdleSettingsT>(value));
    return *this;
  }
  ///@}
 private:
