...
class ModelDigests {
 public:
  ...
  void SetArtifactDigest(ArtifactDigestT&& value) {
    m_artifactDigestHasBeenSet = true;
    m_artifactDigest = std::forward<ArtifactDigestT>(value);
  }
  template <typename ArtifactDigestT = Aws::String>
  ModelDigests& WithArtifactDigest(ArtifactDigestT&& value) {
    SetArtifactDigest(std::forward<ArtifactDigestT>(value));
    return *this;
  }
  ///@}
 private:
