...
class MetricSpecification {
 public:
  ...
  void SetPredefined(PredefinedT&& value) {
    ...
    m_predefined = std::forward<PredefinedT>(value);
  }
  template <typename PredefinedT = PredefinedMetricSpecification>
  MetricSpecification& WithPredefined(PredefinedT&& value) {
    SetPredefined(std::forward<PredefinedT>(value));
    return *this;
  }
  ///@}
  ...
  void SetCustomized(CustomizedT&& value) {
    ...
    m_customized = std::forward<CustomizedT>(value);
  }
  template <typename CustomizedT = CustomizedMetricSpecification>
  MetricSpecification& WithCustomized(CustomizedT&& value) {
    SetCustomized(std::forward<CustomizedT>(value));
    return *this;
  }
  ///@}
 private:
