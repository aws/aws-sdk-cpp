...
class MemberDefinition {
 public:
  ...
  void SetCognitoMemberDefinition(CognitoMemberDefinitionT&& value) {
    ...
    m_cognitoMemberDefinition = std::forward<CognitoMemberDefinitionT>(value);
  }
  template <typename CognitoMemberDefinitionT = CognitoMemberDefinition>
  MemberDefinition& WithCognitoMemberDefinition(CognitoMemberDefinitionT&& value) {
    SetCognitoMemberDefinition(std::forward<CognitoMemberDefinitionT>(value));
    return *this;
  }
  ///@}
  ...
  void SetOidcMemberDefinition(OidcMemberDefinitionT&& value) {
    ...
    m_oidcMemberDefinition = std::forward<OidcMemberDefinitionT>(value);
  }
  template <typename OidcMemberDefinitionT = OidcMemberDefinition>
  MemberDefinition& WithOidcMemberDefinition(OidcMemberDefinitionT&& value) {
    SetOidcMemberDefinition(std::forward<OidcMemberDefinitionT>(value));
    return *this;
  }
  ///@}
 private:
