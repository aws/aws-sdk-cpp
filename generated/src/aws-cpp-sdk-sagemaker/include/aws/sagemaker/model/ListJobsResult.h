...
class ListJobsResult {
 public:
  AWS_SAGEMAKER_API ListJobsResult() = default;
  AWS_SAGEMAKER_API ListJobsResult(const Aws::AmazonWebServiceResult<Aws::Utils::Json::JsonValue>& result);
  AWS_SAGEMAKER_API ListJobsResult& operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Json::JsonValue>& result);
  ...
  ///@{
  /**
   * <p>An array of <code>JobSummary</code> objects that provide summary information
   * about the jobs.</p>
   */
  ...
  template <typename JobSummariesT = Aws::Vector<JobSummary>>
  ListJobsResult& WithJobSummaries(JobSummariesT&& value) {
    SetJobSummaries(std::forward<JobSummariesT>(value));
    return *this;
  }
  template <typename JobSummariesT = JobSummary>
  ListJobsResult& AddJobSummaries(JobSummariesT&& value) {
    m_jobSummariesHasBeenSet = true;
