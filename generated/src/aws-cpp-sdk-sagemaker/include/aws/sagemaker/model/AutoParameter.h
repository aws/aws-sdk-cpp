...
class AutoParameter {
 public:
  ...
  template <typename NameT = Aws::String>
  AutoParameter& WithName(NameT&& value) {
  ...
  void SetValueHint(ValueHintT&& value) {
    m_valueHintHasBeenSet = true;
    m_valueHint = std::forward<ValueHintT>(value);
  }
  template <typename ValueHintT = Aws::String>
  AutoParameter& WithValueHint(ValueHintT&& value) {
    SetValueHint(std::forward<ValueHintT>(value));
    return *this;
  }
  ///@}
 private:
