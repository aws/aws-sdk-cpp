...
class ListModelsResult {
 public:
  AWS_SAGEMAKER_API ListModelsResult() = default;
  ...
  template <typename ModelsT = Aws::Vector<ModelSummary>>
  void SetModels(ModelsT&& value) {
    m_modelsHasBeenSet = true;
    m_models = std::forward<ModelsT>(value);
  }
  template <typename ModelsT = Aws::Vector<ModelSummary>>
  ListModelsResult& WithModels(ModelsT&& value) {
    SetModels(std::forward<ModelsT>(value));
    return *this;
  }
  template <typename ModelsT = ModelSummary>
  ListModelsResult& AddModels(ModelsT&& value) {
    m_modelsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListModelsResult& WithNextToken(NextTokenT&& value) {
