...
class ListImagesResult {
 public:
  ...
  template <typename ImagesT = Aws::Vector<Image>>
  ListImagesResult& WithImages(ImagesT&& value) {
    SetImages(std::forward<ImagesT>(value));
    return *this;
  }
  template <typename ImagesT = Image>
  ListImagesResult& AddImages(ImagesT&& value) {
    m_imagesHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListImagesResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListImagesResult& WithRequestId(RequestIdT&& value) {
