...
class OfflineStoreConfig {
 public:
   ...
   * <code>True</code> and <code>DataCatalogConfig</code> is provided, Feature Store
   * associates the provided catalog configuration with the feature group without
   * creating a table. In this case, you are responsible for creating and managing
   * the Glue table. If set to <code>True</code> without
   * <code>DataCatalogConfig</code>, no Glue table is created or associated with the
   * feature group. The <code>Iceberg</code> table format is only supported when this
   * is set to <code>False</code>.</p> <p>If set to <code>False</code> and
   * <code>DataCatalogConfig</code> is provided, Feature Store creates the table
   * using the specified names. If set to <code>False</code> without
   * <code>DataCatalogConfig</code>, Feature Store auto-generates the table name
   * following <a
   * href="https://docs.aws.amazon.com/athena/latest/ug/tables-databases-columns-names.html">Athena's
   * naming recommendations</a>. This applies to both Glue and Apache Iceberg table
   * formats.</p> <p>The default value is <code>False</code>.</p>
   */
  ...
  inline void SetDisableGlueTableCreation(bool value) {
    m_disableGlueTableCreationHasBeenSet = true;
    m_disableGlueTableCreation = value;
  }
  inline OfflineStoreConfig& WithDisableGlueTableCreation(bool value) {
    SetDisableGlueTableCreation(value);
    return *this;
  }
  ///@}
  ...
  ///@{
  /**
   * <p>The meta data of the Glue table for the <code>OfflineStore</code>. If not
   * provided, Feature Store auto-generates the table name, database, and catalog
   * when the <code>OfflineStore</code> is created. You can optionally provide this
   * configuration to specify custom values. This applies to both Glue and Apache
   * Iceberg table formats.</p>
   */
  ...
  void SetDataCatalogConfig(DataCatalogConfigT&& value) {
    ...
    m_dataCatalogConfig = std::forward<DataCatalogConfigT>(value);
  }
  template <typename DataCatalogConfigT = DataCatalogConfig>
  OfflineStoreConfig& WithDataCatalogConfig(DataCatalogConfigT&& value) {
    SetDataCatalogConfig(std::forward<DataCatalogConfigT>(value));
    return *this;
  }
  ///@}
  ...
  inline void SetTableFormat(TableFormat value) {
    m_tableFormatHasBeenSet = true;
    m_tableFormat = value;
  }
  inline OfflineStoreConfig& WithTableFormat(TableFormat value) {
