...
class CustomFileSystem {
 public:
  ...
  void SetFSxLustreFileSystem(FSxLustreFileSystemT&& value) {
    ...
    m_fSxLustreFileSystem = std::forward<FSxLustreFileSystemT>(value);
  }
  template <typename FSxLustreFileSystemT = FSxLustreFileSystem>
  CustomFileSystem& WithFSxLustreFileSystem(FSxLustreFileSystemT&& value) {
    SetFSxLustreFileSystem(std::forward<FSxLustreFileSystemT>(value));
    return *this;
  }
  ///@}
  ...
  void SetS3FileSystem(S3FileSystemT&& value) {
    ...
    m_s3FileSystem = std::forward<S3FileSystemT>(value);
  }
  template <typename S3FileSystemT = S3FileSystem>
  CustomFileSystem& WithS3FileSystem(S3FileSystemT&& value) {
    SetS3FileSystem(std::forward<S3FileSystemT>(value));
    return *this;
  }
  ///@}
 private:
