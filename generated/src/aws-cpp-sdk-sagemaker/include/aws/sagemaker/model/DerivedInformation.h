...
class DerivedInformation {
 public:
  ...
  void SetDerivedDataInputConfig(DerivedDataInputConfigT&& value) {
    m_derivedDataInputConfigHasBeenSet = true;
    m_derivedDataInputConfig = std::forward<DerivedDataInputConfigT>(value);
  }
  template <typename DerivedDataInputConfigT = Aws::String>
  DerivedInformation& WithDerivedDataInputConfig(DerivedDataInputConfigT&& value) {
    SetDerivedDataInputConfig(std::forward<DerivedDataInputConfigT>(value));
    return *this;
  }
  ///@}
 private:
