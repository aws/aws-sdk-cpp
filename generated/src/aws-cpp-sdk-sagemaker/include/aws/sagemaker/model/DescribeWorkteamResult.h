...
class DescribeWorkteamResult {
 public:
  ...
  void SetWorkteam(WorkteamT&& value) {
    m_workteamHasBeenSet = true;
    m_workteam = std::forward<WorkteamT>(value);
  }
  template <typename WorkteamT = Workteam>
  DescribeWorkteamResult& WithWorkteam(WorkteamT&& value) {
    SetWorkteam(std::forward<WorkteamT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  DescribeWorkteamResult& WithRequestId(RequestIdT&& value) {
