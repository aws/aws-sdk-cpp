...
class TimeSeriesTransformations {
 public:
  ...
  void SetFilling(FillingT&& value) {
    m_fillingHasBeenSet = true;
    m_filling = std::forward<FillingT>(value);
  }
  template <typename FillingT = Aws::Map<Aws::String, Aws::Map<FillingType, Aws::String>>>
  TimeSeriesTransformations& WithFilling(FillingT&& value) {
    SetFilling(std::forward<FillingT>(value));
    return *this;
  }
  ...
  TimeSeriesTransformations& AddFilling(FillingKeyT&& key, FillingValueT&& value) {
    m_fillingHasBeenSet = true;
    m_filling.emplace(std::forward<FillingKeyT>(key), std::forward<FillingValueT>(value));
    return *this;
  }
  ///@}
