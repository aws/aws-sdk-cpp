...
class ClusterInstanceGroupSpecification {
 public:
  ...
  inline ClusterInstanceGroupSpecification& WithThreadsPerCore(int value) {
  ...
  inline ClusterInstanceGroupSpecification& AddOnStartDeepHealthChecks(DeepHealthCheckType value) {
    m_onStartDeepHealthChecksHasBeenSet = true;
   ...
   * system image. For clusters with continuous scaling node provisioning mode, new
   * instance groups inherit the AMI from the earliest existing instance group</p>
   * </li> </ul> <p>If you choose to use a custom AMI (<code>CustomAmiId</code>),
   * ensure it meets the following requirements:</p> <ul> <li> <p>Encryption: The
   * custom AMI must be unencrypted.</p> </li> <li> <p>Ownership: The custom AMI must
   * be owned by the same Amazon Web Services account that is creating the HyperPod
   * cluster.</p> </li> <li> <p>Volume support: Only the primary AMI snapshot volume
   * is supported; additional AMI volumes are not supported.</p> </li> </ul> <p>When
   * updating the instance group's AMI through the <code>UpdateClusterSoftware</code>
   * operation, if an instance group uses a custom AMI, you must provide an
   * <code>ImageId</code> or use the default as input. Note that if you don't specify
   * an instance group in your <code>UpdateClusterSoftware</code> request, then all
   * of the instance groups are patched with the specified image.</p>
   */
   ...
   */
  inline const ClusterNetworkInterface& GetNetworkInterface() const { return m_networkInterface; }
  inline bool NetworkInterfaceHasBeenSet() const { return m_networkInterfaceHasBeenSet; }
  template <typename NetworkInterfaceT = ClusterNetworkInterface>
 ...
 private:
  ...
  ClusterNetworkInterface m_networkInterface;
