...
class OnlineStoreConfig {
 public:
  ...
  inline void SetEnableOnlineStore(bool value) {
    m_enableOnlineStoreHasBeenSet = true;
    m_enableOnlineStore = value;
  }
  inline OnlineStoreConfig& WithEnableOnlineStore(bool value) {
    SetEnableOnlineStore(value);
    return *this;
  }
  ///@}
   ...
   * store for feature groups.</p> </li> <li> <p> <code>Standard_V2</code>: A managed
   * low latency data store for feature groups that supports partial updates to
   * individual features using the <a
   * href="https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_feature_store_UpdateRecord.html">UpdateRecord</a>
   * operation. Choose this storage type at feature group creation time if your use
   * case requires updating specific feature values without rewriting the entire
   * record.</p> </li> <li> <p> <code>InMemory</code>: A managed data store for
   * feature groups that supports very low latency retrieval.</p> </li> </ul>
   */
  ...
  inline OnlineStoreConfig& WithStorageType(StorageType value) {
