...
ListInferenceExperimentsResult::ListInferenceExperimentsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListInferenceExperimentsResult& ListInferenceExperimentsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("InferenceExperiments")) {
    ...
    for (unsigned inferenceExperimentsIndex = 0; inferenceExperimentsIndex < inferenceExperimentsJsonList.GetLength();
         ++inferenceExperimentsIndex) {
      m_inferenceExperiments.push_back(inferenceExperimentsJsonList[inferenceExperimentsIndex].AsObject());
    }
    m_inferenceExperimentsHasBeenSet = true;
  }
