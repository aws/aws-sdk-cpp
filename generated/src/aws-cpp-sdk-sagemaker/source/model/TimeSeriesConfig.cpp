...
TimeSeriesConfig::TimeSeriesConfig(JsonView jsonValue) { *this = jsonValue; }
...
TimeSeriesConfig& TimeSeriesConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("GroupingAttributeNames")) {
    ...
    for (unsigned groupingAttributeNamesIndex = 0; groupingAttributeNamesIndex < groupingAttributeNamesJsonList.GetLength();
         ++groupingAttributeNamesIndex) {
