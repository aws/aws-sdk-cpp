...
BatchTransformInput::BatchTransformInput(JsonView jsonValue) { *this = jsonValue; }
...
BatchTransformInput& BatchTransformInput::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("S3DataDistributionType")) {
    m_s3DataDistributionType =
        ProcessingS3DataDistributionTypeMapper::GetProcessingS3DataDistributionTypeForName(jsonValue.GetString("S3DataDistributionType"));
...
JsonValue BatchTransformInput::Jsonize() const {
  ...
  if (m_s3DataDistributionTypeHasBeenSet) {
    payload.WithString("S3DataDistributionType",
                       ProcessingS3DataDistributionTypeMapper::GetNameForProcessingS3DataDistributionType(m_s3DataDistributionType));
  }
