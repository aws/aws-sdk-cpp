...
Aws::String UpdateProjectRequest::SerializePayload() const {
  ...
  if (m_templateProvidersToUpdateHasBeenSet) {
    ...
    for (unsigned templateProvidersToUpdateIndex = 0; templateProvidersToUpdateIndex < templateProvidersToUpdateJsonList.GetLength();
         ++templateProvidersToUpdateIndex) {
      templateProvidersToUpdateJsonList[templateProvidersToUpdateIndex].AsObject(
          m_templateProvidersToUpdate[templateProvidersToUpdateIndex].Jsonize());
    }
