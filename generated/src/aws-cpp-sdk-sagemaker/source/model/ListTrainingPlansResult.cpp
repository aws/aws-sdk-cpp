...
ListTrainingPlansResult::ListTrainingPlansResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListTrainingPlansResult& ListTrainingPlansResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("TrainingPlanSummaries")) {
    ...
    for (unsigned trainingPlanSummariesIndex = 0; trainingPlanSummariesIndex < trainingPlanSummariesJsonList.GetLength();
         ++trainingPlanSummariesIndex) {
      m_trainingPlanSummaries.push_back(trainingPlanSummariesJsonList[trainingPlanSummariesIndex].AsObject());
    }
    m_trainingPlanSummariesHasBeenSet = true;
  }
