...
ContainerConfig::ContainerConfig(JsonView jsonValue) { *this = jsonValue; }
...
ContainerConfig& ContainerConfig::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("ContainerArguments")) {
    ...
    for (unsigned containerArgumentsIndex = 0; containerArgumentsIndex < containerArgumentsJsonList.GetLength();
         ++containerArgumentsIndex) {
  ...
  if (jsonValue.ValueExists("ContainerEntrypoint")) {
    ...
    for (unsigned containerEntrypointIndex = 0; containerEntrypointIndex < containerEntrypointJsonList.GetLength();
         ++containerEntrypointIndex) {
  ...
  if (jsonValue.ValueExists("ContainerEnvironmentVariables")) {
    Aws::Map<Aws::String, JsonView> containerEnvironmentVariablesJsonMap =
        jsonValue.GetObject("ContainerEnvironmentVariables").GetAllObjects();
