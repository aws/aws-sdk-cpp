...
TrainingSpecification::TrainingSpecification(JsonView jsonValue) { *this = jsonValue; }
...
TrainingSpecification& TrainingSpecification::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("SupportedHyperParameters")) {
    ...
    for (unsigned supportedHyperParametersIndex = 0; supportedHyperParametersIndex < supportedHyperParametersJsonList.GetLength();
         ++supportedHyperParametersIndex) {
  ...
  if (jsonValue.ValueExists("SupportedTrainingInstanceTypes")) {
    ...
    for (unsigned supportedTrainingInstanceTypesIndex = 0;
         supportedTrainingInstanceTypesIndex < supportedTrainingInstanceTypesJsonList.GetLength(); ++supportedTrainingInstanceTypesIndex) {
      m_supportedTrainingInstanceTypes.push_back(TrainingInstanceTypeMapper::GetTrainingInstanceTypeForName(
          supportedTrainingInstanceTypesJsonList[supportedTrainingInstanceTypesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("SupportedTuningJobObjectiveMetrics")) {
    ...
    for (unsigned supportedTuningJobObjectiveMetricsIndex = 0;
         supportedTuningJobObjectiveMetricsIndex < supportedTuningJobObjectiveMetricsJsonList.GetLength();
         ++supportedTuningJobObjectiveMetricsIndex) {
      m_supportedTuningJobObjectiveMetrics.push_back(
          supportedTuningJobObjectiveMetricsJsonList[supportedTuningJobObjectiveMetricsIndex].AsObject());
    }
...
JsonValue TrainingSpecification::Jsonize() const {
  ...
  if (m_supportedHyperParametersHasBeenSet) {
    ...
    for (unsigned supportedHyperParametersIndex = 0; supportedHyperParametersIndex < supportedHyperParametersJsonList.GetLength();
         ++supportedHyperParametersIndex) {
      supportedHyperParametersJsonList[supportedHyperParametersIndex].AsObject(
          m_supportedHyperParameters[supportedHyperParametersIndex].Jsonize());
    }
  ...
  if (m_supportedTrainingInstanceTypesHasBeenSet) {
    ...
    for (unsigned supportedTrainingInstanceTypesIndex = 0;
         supportedTrainingInstanceTypesIndex < supportedTrainingInstanceTypesJsonList.GetLength(); ++supportedTrainingInstanceTypesIndex) {
      supportedTrainingInstanceTypesJsonList[supportedTrainingInstanceTypesIndex].AsString(
          TrainingInstanceTypeMapper::GetNameForTrainingInstanceType(
              m_supportedTrainingInstanceTypes[supportedTrainingInstanceTypesIndex]));
    }
  ...
  if (m_supportedTuningJobObjectiveMetricsHasBeenSet) {
    ...
    for (unsigned supportedTuningJobObjectiveMetricsIndex = 0;
         ...
         ++supportedTuningJobObjectiveMetricsIndex) {
      supportedTuningJobObjectiveMetricsJsonList[supportedTuningJobObjectiveMetricsIndex].AsObject(
          m_supportedTuningJobObjectiveMetrics[supportedTuningJobObjectiveMetricsIndex].Jsonize());
    }
