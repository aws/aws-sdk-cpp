...
TimeSeriesTransformations::TimeSeriesTransformations(JsonView jsonValue) { *this = jsonValue; }
...
TimeSeriesTransformations& TimeSeriesTransformations::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("Aggregation")) {
    ...
    for (auto& aggregationItem : aggregationJsonMap) {
      m_aggregation[aggregationItem.first] =
          AggregationTransformationValueMapper::GetAggregationTransformationValueForName(aggregationItem.second.AsString());
    }
...
JsonValue TimeSeriesTransformations::Jsonize() const {
  ...
  if (m_fillingHasBeenSet) {
    ...
    for (auto& fillingItem : m_filling) {
      ...
      for (auto& fillingTransformationMapItem : fillingItem.second) {
        fillingTransformationMapJsonMap.WithString(FillingTypeMapper::GetNameForFillingType(fillingTransformationMapItem.first),
                                                   fillingTransformationMapItem.second);
      }
  ...
  if (m_aggregationHasBeenSet) {
    ...
    for (auto& aggregationItem : m_aggregation) {
      aggregationJsonMap.WithString(aggregationItem.first,
                                    AggregationTransformationValueMapper::GetNameForAggregationTransformationValue(aggregationItem.second));
    }
