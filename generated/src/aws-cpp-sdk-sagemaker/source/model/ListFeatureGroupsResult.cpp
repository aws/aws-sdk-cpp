...
ListFeatureGroupsResult::ListFeatureGroupsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListFeatureGroupsResult& ListFeatureGroupsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("FeatureGroupSummaries")) {
    ...
    for (unsigned featureGroupSummariesIndex = 0; featureGroupSummariesIndex < featureGroupSummariesJsonList.GetLength();
         ++featureGroupSummariesIndex) {
      m_featureGroupSummaries.push_back(featureGroupSummariesJsonList[featureGroupSummariesIndex].AsObject());
    }
    m_featureGroupSummariesHasBeenSet = true;
  }
