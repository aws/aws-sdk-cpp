...
Aws::String StopInferenceExperimentRequest::SerializePayload() const {
  ...
  if (m_modelVariantActionsHasBeenSet) {
    ...
    for (auto& modelVariantActionsItem : m_modelVariantActions) {
      modelVariantActionsJsonMap.WithString(modelVariantActionsItem.first,
                                            ModelVariantActionMapper::GetNameForModelVariantAction(modelVariantActionsItem.second));
    }
  ...
  if (m_desiredModelVariantsHasBeenSet) {
    ...
    for (unsigned desiredModelVariantsIndex = 0; desiredModelVariantsIndex < desiredModelVariantsJsonList.GetLength();
         ++desiredModelVariantsIndex) {
  ...
  if (m_desiredStateHasBeenSet) {
    payload.WithString("DesiredState",
                       InferenceExperimentStopDesiredStateMapper::GetNameForInferenceExperimentStopDesiredState(m_desiredState));
  }
