...
UserSettings& UserSettings::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("CustomFileSystemConfigs")) {
    ...
    for (unsigned customFileSystemConfigsIndex = 0; customFileSystemConfigsIndex < customFileSystemConfigsJsonList.GetLength();
         ++customFileSystemConfigsIndex) {
...
JsonValue UserSettings::Jsonize() const {
  ...
  if (m_customFileSystemConfigsHasBeenSet) {
    ...
    for (unsigned customFileSystemConfigsIndex = 0; customFileSystemConfigsIndex < customFileSystemConfigsJsonList.GetLength();
         ++customFileSystemConfigsIndex) {
      customFileSystemConfigsJsonList[customFileSystemConfigsIndex].AsObject(
          m_customFileSystemConfigs[customFileSystemConfigsIndex].Jsonize());
    }
