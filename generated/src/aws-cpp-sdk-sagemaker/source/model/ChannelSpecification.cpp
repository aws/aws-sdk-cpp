...
ChannelSpecification::ChannelSpecification(JsonView jsonValue) { *this = jsonValue; }
...
ChannelSpecification& ChannelSpecification::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("SupportedCompressionTypes")) {
    ...
    for (unsigned supportedCompressionTypesIndex = 0; supportedCompressionTypesIndex < supportedCompressionTypesJsonList.GetLength();
         ++supportedCompressionTypesIndex) {
      m_supportedCompressionTypes.push_back(
          CompressionTypeMapper::GetCompressionTypeForName(supportedCompressionTypesJsonList[supportedCompressionTypesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("SupportedInputModes")) {
    ...
    for (unsigned supportedInputModesIndex = 0; supportedInputModesIndex < supportedInputModesJsonList.GetLength();
         ++supportedInputModesIndex) {
      m_supportedInputModes.push_back(
          TrainingInputModeMapper::GetTrainingInputModeForName(supportedInputModesJsonList[supportedInputModesIndex].AsString()));
    }
...
JsonValue ChannelSpecification::Jsonize() const {
  ...
  if (m_supportedCompressionTypesHasBeenSet) {
    ...
    for (unsigned supportedCompressionTypesIndex = 0; supportedCompressionTypesIndex < supportedCompressionTypesJsonList.GetLength();
         ++supportedCompressionTypesIndex) {
      supportedCompressionTypesJsonList[supportedCompressionTypesIndex].AsString(
          CompressionTypeMapper::GetNameForCompressionType(m_supportedCompressionTypes[supportedCompressionTypesIndex]));
    }
  ...
  if (m_supportedInputModesHasBeenSet) {
    ...
    for (unsigned supportedInputModesIndex = 0; supportedInputModesIndex < supportedInputModesJsonList.GetLength();
         ++supportedInputModesIndex) {
      supportedInputModesJsonList[supportedInputModesIndex].AsString(
          TrainingInputModeMapper::GetNameForTrainingInputMode(m_supportedInputModes[supportedInputModesIndex]));
    }
