...
AutoMLCandidate::AutoMLCandidate(JsonView jsonValue) { *this = jsonValue; }
...
AutoMLCandidate& AutoMLCandidate::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("InferenceContainers")) {
    ...
    for (unsigned inferenceContainersIndex = 0; inferenceContainersIndex < inferenceContainersJsonList.GetLength();
         ++inferenceContainersIndex) {
...
JsonValue AutoMLCandidate::Jsonize() const {
  ...
  if (m_inferenceContainerDefinitionsHasBeenSet) {
    ...
    for (auto& inferenceContainerDefinitionsItem : m_inferenceContainerDefinitions) {
      ...
      for (unsigned autoMLContainerDefinitionsIndex = 0; autoMLContainerDefinitionsIndex < autoMLContainerDefinitionsJsonList.GetLength();
           ++autoMLContainerDefinitionsIndex) {
        autoMLContainerDefinitionsJsonList[autoMLContainerDefinitionsIndex].AsObject(
            inferenceContainerDefinitionsItem.second[autoMLContainerDefinitionsIndex].Jsonize());
      }
      inferenceContainerDefinitionsJsonMap.WithArray(
          AutoMLProcessingUnitMapper::GetNameForAutoMLProcessingUnit(inferenceContainerDefinitionsItem.first),
          std::move(autoMLContainerDefinitionsJsonList));
    }
