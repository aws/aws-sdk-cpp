...
DeploymentRecommendation::DeploymentRecommendation(JsonView jsonValue) { *this = jsonValue; }
...
DeploymentRecommendation& DeploymentRecommendation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("RealTimeInferenceRecommendations")) {
    ...
    for (unsigned realTimeInferenceRecommendationsIndex = 0;
         realTimeInferenceRecommendationsIndex < realTimeInferenceRecommendationsJsonList.GetLength();
         ++realTimeInferenceRecommendationsIndex) {
      m_realTimeInferenceRecommendations.push_back(
          realTimeInferenceRecommendationsJsonList[realTimeInferenceRecommendationsIndex].AsObject());
    }
...
JsonValue DeploymentRecommendation::Jsonize() const {
  ...
  if (m_realTimeInferenceRecommendationsHasBeenSet) {
    ...
    for (unsigned realTimeInferenceRecommendationsIndex = 0;
         ...
         ++realTimeInferenceRecommendationsIndex) {
      realTimeInferenceRecommendationsJsonList[realTimeInferenceRecommendationsIndex].AsObject(
          m_realTimeInferenceRecommendations[realTimeInferenceRecommendationsIndex].Jsonize());
    }
