...
ShadowModeConfig::ShadowModeConfig(JsonView jsonValue) { *this = jsonValue; }
...
ShadowModeConfig& ShadowModeConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ShadowModelVariants")) {
    ...
    for (unsigned shadowModelVariantsIndex = 0; shadowModelVariantsIndex < shadowModelVariantsJsonList.GetLength();
         ++shadowModelVariantsIndex) {
