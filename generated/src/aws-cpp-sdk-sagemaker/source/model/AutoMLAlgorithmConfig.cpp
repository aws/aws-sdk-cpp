...
AutoMLAlgorithmConfig::AutoMLAlgorithmConfig(JsonView jsonValue) { *this = jsonValue; }
...
AutoMLAlgorithmConfig& AutoMLAlgorithmConfig::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("AutoMLAlgorithms")) {
    ...
    for (unsigned autoMLAlgorithmsIndex = 0; autoMLAlgorithmsIndex < autoMLAlgorithmsJsonList.GetLength(); ++autoMLAlgorithmsIndex) {
      m_autoMLAlgorithms.push_back(
          AutoMLAlgorithmMapper::GetAutoMLAlgorithmForName(autoMLAlgorithmsJsonList[autoMLAlgorithmsIndex].AsString()));
    }
...
JsonValue AutoMLAlgorithmConfig::Jsonize() const {
  ...
  if (m_autoMLAlgorithmsHasBeenSet) {
    ...
    for (unsigned autoMLAlgorithmsIndex = 0; autoMLAlgorithmsIndex < autoMLAlgorithmsJsonList.GetLength(); ++autoMLAlgorithmsIndex) {
      autoMLAlgorithmsJsonList[autoMLAlgorithmsIndex].AsString(
          AutoMLAlgorithmMapper::GetNameForAutoMLAlgorithm(m_autoMLAlgorithms[autoMLAlgorithmsIndex]));
    }
