...
DeploymentConfiguration& DeploymentConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AutoRollbackConfiguration")) {
    ...
    for (unsigned autoRollbackConfigurationIndex = 0; autoRollbackConfigurationIndex < autoRollbackConfigurationJsonList.GetLength();
         ++autoRollbackConfigurationIndex) {
...
JsonValue DeploymentConfiguration::Jsonize() const {
  ...
  if (m_autoRollbackConfigurationHasBeenSet) {
    ...
    for (unsigned autoRollbackConfigurationIndex = 0; autoRollbackConfigurationIndex < autoRollbackConfigurationJsonList.GetLength();
         ++autoRollbackConfigurationIndex) {
      autoRollbackConfigurationJsonList[autoRollbackConfigurationIndex].AsObject(
          m_autoRollbackConfiguration[autoRollbackConfigurationIndex].Jsonize());
    }
