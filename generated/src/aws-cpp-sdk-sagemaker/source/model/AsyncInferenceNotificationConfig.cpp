...
AsyncInferenceNotificationConfig::AsyncInferenceNotificationConfig(JsonView jsonValue) { *this = jsonValue; }
...
AsyncInferenceNotificationConfig& AsyncInferenceNotificationConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("IncludeInferenceResponseIn")) {
    ...
    for (unsigned includeInferenceResponseInIndex = 0; includeInferenceResponseInIndex < includeInferenceResponseInJsonList.GetLength();
         ++includeInferenceResponseInIndex) {
      m_includeInferenceResponseIn.push_back(AsyncNotificationTopicTypesMapper::GetAsyncNotificationTopicTypesForName(
          includeInferenceResponseInJsonList[includeInferenceResponseInIndex].AsString()));
    }
...
JsonValue AsyncInferenceNotificationConfig::Jsonize() const {
  ...
  if (m_includeInferenceResponseInHasBeenSet) {
    ...
    for (unsigned includeInferenceResponseInIndex = 0; includeInferenceResponseInIndex < includeInferenceResponseInJsonList.GetLength();
         ++includeInferenceResponseInIndex) {
      includeInferenceResponseInJsonList[includeInferenceResponseInIndex].AsString(
          AsyncNotificationTopicTypesMapper::GetNameForAsyncNotificationTopicTypes(
              m_includeInferenceResponseIn[includeInferenceResponseInIndex]));
    }
