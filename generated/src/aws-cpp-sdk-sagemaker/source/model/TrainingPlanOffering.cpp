...
TrainingPlanOffering::TrainingPlanOffering(JsonView jsonValue) { *this = jsonValue; }
...
TrainingPlanOffering& TrainingPlanOffering::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("TargetResources")) {
    ...
    for (unsigned targetResourcesIndex = 0; targetResourcesIndex < targetResourcesJsonList.GetLength(); ++targetResourcesIndex) {
      m_targetResources.push_back(
          SageMakerResourceNameMapper::GetSageMakerResourceNameForName(targetResourcesJsonList[targetResourcesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("ReservedCapacityOfferings")) {
    ...
    for (unsigned reservedCapacityOfferingsIndex = 0; reservedCapacityOfferingsIndex < reservedCapacityOfferingsJsonList.GetLength();
         ++reservedCapacityOfferingsIndex) {
...
JsonValue TrainingPlanOffering::Jsonize() const {
  ...
  if (m_targetResourcesHasBeenSet) {
    ...
    for (unsigned targetResourcesIndex = 0; targetResourcesIndex < targetResourcesJsonList.GetLength(); ++targetResourcesIndex) {
      targetResourcesJsonList[targetResourcesIndex].AsString(
          SageMakerResourceNameMapper::GetNameForSageMakerResourceName(m_targetResources[targetResourcesIndex]));
    }
  ...
  if (m_reservedCapacityOfferingsHasBeenSet) {
    ...
    for (unsigned reservedCapacityOfferingsIndex = 0; reservedCapacityOfferingsIndex < reservedCapacityOfferingsJsonList.GetLength();
         ++reservedCapacityOfferingsIndex) {
      reservedCapacityOfferingsJsonList[reservedCapacityOfferingsIndex].AsObject(
          m_reservedCapacityOfferings[reservedCapacityOfferingsIndex].Jsonize());
    }
