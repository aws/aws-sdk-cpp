...
StudioWebPortalSettings::StudioWebPortalSettings(JsonView jsonValue) { *this = jsonValue; }
...
StudioWebPortalSettings& StudioWebPortalSettings::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("HiddenInstanceTypes")) {
    ...
    for (unsigned hiddenInstanceTypesIndex = 0; hiddenInstanceTypesIndex < hiddenInstanceTypesJsonList.GetLength();
         ++hiddenInstanceTypesIndex) {
      m_hiddenInstanceTypes.push_back(
          AppInstanceTypeMapper::GetAppInstanceTypeForName(hiddenInstanceTypesJsonList[hiddenInstanceTypesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("HiddenSageMakerImageVersionAliases")) {
    ...
    for (unsigned hiddenSageMakerImageVersionAliasesIndex = 0;
         hiddenSageMakerImageVersionAliasesIndex < hiddenSageMakerImageVersionAliasesJsonList.GetLength();
         ++hiddenSageMakerImageVersionAliasesIndex) {
      m_hiddenSageMakerImageVersionAliases.push_back(
          hiddenSageMakerImageVersionAliasesJsonList[hiddenSageMakerImageVersionAliasesIndex].AsObject());
    }
...
JsonValue StudioWebPortalSettings::Jsonize() const {
  ...
  if (m_hiddenInstanceTypesHasBeenSet) {
    ...
    for (unsigned hiddenInstanceTypesIndex = 0; hiddenInstanceTypesIndex < hiddenInstanceTypesJsonList.GetLength();
         ++hiddenInstanceTypesIndex) {
      hiddenInstanceTypesJsonList[hiddenInstanceTypesIndex].AsString(
          AppInstanceTypeMapper::GetNameForAppInstanceType(m_hiddenInstanceTypes[hiddenInstanceTypesIndex]));
    }
