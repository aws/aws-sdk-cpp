...
DirectoryConfig::DirectoryConfig(JsonView jsonValue) { *this = jsonValue; }
...
DirectoryConfig& DirectoryConfig::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("OrganizationalUnitDistinguishedNames")) {
    ...
    for (unsigned organizationalUnitDistinguishedNamesIndex = 0;
         organizationalUnitDistinguishedNamesIndex < organizationalUnitDistinguishedNamesJsonList.GetLength();
         ++organizationalUnitDistinguishedNamesIndex) {
      m_organizationalUnitDistinguishedNames.push_back(
          organizationalUnitDistinguishedNamesJsonList[organizationalUnitDistinguishedNamesIndex].AsString());
    }
...
JsonValue DirectoryConfig::Jsonize() const {
  ...
  if (m_organizationalUnitDistinguishedNamesHasBeenSet) {
    ...
    for (unsigned organizationalUnitDistinguishedNamesIndex = 0;
         ...
         ++organizationalUnitDistinguishedNamesIndex) {
      organizationalUnitDistinguishedNamesJsonList[organizationalUnitDistinguishedNamesIndex].AsString(
          m_organizationalUnitDistinguishedNames[organizationalUnitDistinguishedNamesIndex]);
    }
