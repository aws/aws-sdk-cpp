...
Aws::String BatchDisassociateUserStackRequest::SerializePayload() const {
  ...
  if (m_userStackAssociationsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("UserStackAssociations"));
    encoder.WriteArrayStart(m_userStackAssociations.size());
    for (const auto& item_0 : m_userStackAssociations) {
