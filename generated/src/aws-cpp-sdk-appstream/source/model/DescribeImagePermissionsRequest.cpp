...
Aws::String DescribeImagePermissionsRequest::SerializePayload() const {
  ...
  if (m_sharedAwsAccountIdsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("SharedAwsAccountIds"));
    encoder.WriteArrayStart(m_sharedAwsAccountIds.size());
    for (const auto& item_0 : m_sharedAwsAccountIds) {
