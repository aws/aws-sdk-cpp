...
AppBlock::AppBlock(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

AppBlock& AppBlock::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Name") {
              ...
              else if (initialKeyStr == "SourceS3Location") {
                m_sourceS3Location = S3Location(decoder);
                m_sourceS3LocationHasBeenSet = true;
              }

              else if (initialKeyStr == "SetupScriptDetails") {
                m_setupScriptDetails = ScriptDetails(decoder);
              ...
              else if (initialKeyStr == "PostSetupScriptDetails") {
                m_postSetupScriptDetails = ScriptDetails(decoder);
                m_postSetupScriptDetailsHasBeenSet = true;
              }

              else if (initialKeyStr == "PackagingType") {
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_packagingType = PackagingTypeMapper::GetPackagingTypeForName(
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_state = AppBlockStateMapper::GetAppBlockStateForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
                m_stateHasBeenSet = true;
              }

              else if (initialKeyStr == "AppBlockErrors") {
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_appBlockErrors.push_back(ErrorDetails(decoder));
                      }
                    }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("AppBlock", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void AppBlock::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_nameHasBeenSet) {
    mapSize++;
  ...
  if (m_stateHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(AppBlockStateMapper::GetNameForAppBlockState(m_state).c_str()));
  }

  if (m_appBlockErrorsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("AppBlockErrors"));
    encoder.WriteArrayStart(m_appBlockErrors.size());
    for (const auto& item_0 : m_appBlockErrors) {
