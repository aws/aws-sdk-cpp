...
class UsageReportSubscription {
 public:
  ...
  AWS_APPSTREAM_API UsageReportSubscription(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_APPSTREAM_API UsageReportSubscription& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
   ...
   * script configuration, WorkSpaces Applications created an S3 bucket to store the
   * script output. The bucket is unique to your account and Region. When you enable
   * usage reporting in this case, WorkSpaces Applications uses the same bucket to
   * store your usage reports. If you haven't already enabled on-instance session
   * scripts, when you enable usage reports, WorkSpaces Applications creates a new S3
   * bucket.</p>
   */
  ...
  inline void SetSchedule(UsageReportSchedule value) {
    m_scheduleHasBeenSet = true;
    m_schedule = value;
  }
  inline UsageReportSubscription& WithSchedule(UsageReportSchedule value) {
    SetSchedule(value);
    return *this;
  }
  ///@}
  ...
  void SetLastGeneratedReportDate(LastGeneratedReportDateT&& value) {
    ...
    m_lastGeneratedReportDate = std::forward<LastGeneratedReportDateT>(value);
  }
  template <typename LastGeneratedReportDateT = Aws::Utils::DateTime>
  UsageReportSubscription& WithLastGeneratedReportDate(LastGeneratedReportDateT&& value) {
    SetLastGeneratedReportDate(std::forward<LastGeneratedReportDateT>(value));
    return *this;
  }
  ///@}
