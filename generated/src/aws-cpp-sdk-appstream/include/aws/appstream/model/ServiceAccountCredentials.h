...
class ServiceAccountCredentials {
 public:
  ...
  AWS_APPSTREAM_API ServiceAccountCredentials(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_APPSTREAM_API ServiceAccountCredentials& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  void SetAccountPassword(AccountPasswordT&& value) {
    ...
    m_accountPassword = std::forward<AccountPasswordT>(value);
  }
  template <typename AccountPasswordT = Aws::String>
  ServiceAccountCredentials& WithAccountPassword(AccountPasswordT&& value) {
    SetAccountPassword(std::forward<AccountPasswordT>(value));
    return *this;
  }
  ///@}
 private:
