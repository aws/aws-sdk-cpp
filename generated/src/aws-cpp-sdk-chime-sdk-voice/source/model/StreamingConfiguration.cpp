...
StreamingConfiguration& StreamingConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("StreamingNotificationTargets")) {
    ...
    for (unsigned streamingNotificationTargetsIndex = 0;
         streamingNotificationTargetsIndex < streamingNotificationTargetsJsonList.GetLength(); ++streamingNotificationTargetsIndex) {
...
JsonValue StreamingConfiguration::Jsonize() const {
  ...
  if (m_streamingNotificationTargetsHasBeenSet) {
    ...
    for (unsigned streamingNotificationTargetsIndex = 0;
         streamingNotificationTargetsIndex < streamingNotificationTargetsJsonList.GetLength(); ++streamingNotificationTargetsIndex) {
      streamingNotificationTargetsJsonList[streamingNotificationTargetsIndex].AsObject(
          m_streamingNotificationTargets[streamingNotificationTargetsIndex].Jsonize());
    }
