...
/**
 * <p> <b>[Custom snapshot policies only]</b> Describes the retention rule for
 * archived snapshots. Once the archive retention threshold is met, the snapshots
 * are permanently deleted from the archive tier.</p>  <p>The archive
 * retention rule must retain snapshots in the archive tier for a minimum of 90
 * days.</p>  <p>For <b>count-based schedules</b>, you must specify
...
class RetentionArchiveTier {
 public:
  ...
  inline RetentionArchiveTier& WithCount(int value) {
    SetCount(value);
    return *this;
  }
  ///@}
  ...
  inline RetentionArchiveTier& WithInterval(int value) {
    SetInterval(value);
    return *this;
  }
  ///@}
  ...
  inline RetentionArchiveTier& WithIntervalUnit(RetentionIntervalUnitValues value) {
