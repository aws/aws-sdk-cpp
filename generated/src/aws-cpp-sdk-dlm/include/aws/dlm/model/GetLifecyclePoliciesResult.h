...
class GetLifecyclePoliciesResult {
 public:
  ...
  template <typename PoliciesT = Aws::Vector<LifecyclePolicySummary>>
  GetLifecyclePoliciesResult& WithPolicies(PoliciesT&& value) {
    SetPolicies(std::forward<PoliciesT>(value));
    return *this;
  }
  template <typename PoliciesT = LifecyclePolicySummary>
  GetLifecyclePoliciesResult& AddPolicies(PoliciesT&& value) {
    m_policiesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  GetLifecyclePoliciesResult& WithRequestId(RequestIdT&& value) {
