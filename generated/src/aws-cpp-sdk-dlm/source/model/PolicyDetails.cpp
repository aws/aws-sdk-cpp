...
PolicyDetails& PolicyDetails::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ResourceTypes")) {
    ...
    for (unsigned resourceTypesIndex = 0; resourceTypesIndex < resourceTypesJsonList.GetLength(); ++resourceTypesIndex) {
      m_resourceTypes.push_back(
          ResourceTypeValuesMapper::GetResourceTypeValuesForName(resourceTypesJsonList[resourceTypesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("ResourceLocations")) {
    ...
    for (unsigned resourceLocationsIndex = 0; resourceLocationsIndex < resourceLocationsJsonList.GetLength(); ++resourceLocationsIndex) {
      m_resourceLocations.push_back(
          ResourceLocationValuesMapper::GetResourceLocationValuesForName(resourceLocationsJsonList[resourceLocationsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("CrossRegionCopyTargets")) {
    ...
    for (unsigned crossRegionCopyTargetsIndex = 0; crossRegionCopyTargetsIndex < crossRegionCopyTargetsJsonList.GetLength();
         ++crossRegionCopyTargetsIndex) {
...
JsonValue PolicyDetails::Jsonize() const {
  ...
  if (m_resourceTypesHasBeenSet) {
    ...
    for (unsigned resourceTypesIndex = 0; resourceTypesIndex < resourceTypesJsonList.GetLength(); ++resourceTypesIndex) {
      resourceTypesJsonList[resourceTypesIndex].AsString(
          ResourceTypeValuesMapper::GetNameForResourceTypeValues(m_resourceTypes[resourceTypesIndex]));
    }
  ...
  if (m_resourceLocationsHasBeenSet) {
    ...
    for (unsigned resourceLocationsIndex = 0; resourceLocationsIndex < resourceLocationsJsonList.GetLength(); ++resourceLocationsIndex) {
      resourceLocationsJsonList[resourceLocationsIndex].AsString(
          ResourceLocationValuesMapper::GetNameForResourceLocationValues(m_resourceLocations[resourceLocationsIndex]));
    }
