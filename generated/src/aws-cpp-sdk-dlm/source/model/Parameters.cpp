...
Parameters::Parameters(JsonView jsonValue) { *this = jsonValue; }
...
Parameters& Parameters::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ExcludeDataVolumeTags")) {
    ...
    for (unsigned excludeDataVolumeTagsIndex = 0; excludeDataVolumeTagsIndex < excludeDataVolumeTagsJsonList.GetLength();
         ++excludeDataVolumeTagsIndex) {
