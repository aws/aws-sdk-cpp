...
InvalidRequestException& InvalidRequestException::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("RequiredParameters")) {
    ...
    for (unsigned requiredParametersIndex = 0; requiredParametersIndex < requiredParametersJsonList.GetLength();
         ++requiredParametersIndex) {
  ...
  if (jsonValue.ValueExists("MutuallyExclusiveParameters")) {
    ...
    for (unsigned mutuallyExclusiveParametersIndex = 0; mutuallyExclusiveParametersIndex < mutuallyExclusiveParametersJsonList.GetLength();
         ++mutuallyExclusiveParametersIndex) {
...
JsonValue InvalidRequestException::Jsonize() const {
  ...
  if (m_mutuallyExclusiveParametersHasBeenSet) {
    ...
    for (unsigned mutuallyExclusiveParametersIndex = 0; mutuallyExclusiveParametersIndex < mutuallyExclusiveParametersJsonList.GetLength();
         ++mutuallyExclusiveParametersIndex) {
      mutuallyExclusiveParametersJsonList[mutuallyExclusiveParametersIndex].AsString(
          m_mutuallyExclusiveParameters[mutuallyExclusiveParametersIndex]);
    }
