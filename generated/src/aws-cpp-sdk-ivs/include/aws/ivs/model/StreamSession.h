...
class StreamSession {
 public:
  ...
  void SetTruncatedEvents(TruncatedEventsT&& value) {
    ...
    m_truncatedEvents = std::forward<TruncatedEventsT>(value);
  }
  template <typename TruncatedEventsT = Aws::Vector<StreamEvent>>
  StreamSession& WithTruncatedEvents(TruncatedEventsT&& value) {
    SetTruncatedEvents(std::forward<TruncatedEventsT>(value));
    return *this;
  }
  template <typename TruncatedEventsT = StreamEvent>
  StreamSession& AddTruncatedEvents(TruncatedEventsT&& value) {
    m_truncatedEventsHasBeenSet = true;
    m_truncatedEvents.emplace_back(std::forward<TruncatedEventsT>(value));
    return *this;
  }
  ///@}
 private:
