...
class GetOfferingStatusResult {
 public:
  ...
  void SetCurrent(CurrentT&& value) {
    ...
    m_current = std::forward<CurrentT>(value);
  }
  template <typename CurrentT = Aws::Map<Aws::String, OfferingStatus>>
  GetOfferingStatusResult& WithCurrent(CurrentT&& value) {
    SetCurrent(std::forward<CurrentT>(value));
    return *this;
  }
  ...
  GetOfferingStatusResult& AddCurrent(CurrentKeyT&& key, CurrentValueT&& value) {
    m_currentHasBeenSet = true;
    m_current.emplace(std::forward<CurrentKeyT>(key), std::forward<CurrentValueT>(value));
    return *this;
  }
  ///@}
  ...
  void SetNextPeriod(NextPeriodT&& value) {
    ...
    m_nextPeriod = std::forward<NextPeriodT>(value);
  }
  template <typename NextPeriodT = Aws::Map<Aws::String, OfferingStatus>>
  GetOfferingStatusResult& WithNextPeriod(NextPeriodT&& value) {
    SetNextPeriod(std::forward<NextPeriodT>(value));
    return *this;
  }
  ...
  GetOfferingStatusResult& AddNextPeriod(NextPeriodKeyT&& key, NextPeriodValueT&& value) {
    m_nextPeriodHasBeenSet = true;
    m_nextPeriod.emplace(std::forward<NextPeriodKeyT>(key), std::forward<NextPeriodValueT>(value));
    return *this;
  }
  ///@}
