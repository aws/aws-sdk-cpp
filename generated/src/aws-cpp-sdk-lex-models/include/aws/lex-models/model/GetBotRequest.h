...
class GetBotRequest : public LexModelBuildingServiceRequest {
 public:
  ...
  template <typename NameT = Aws::String>
  GetBotRequest& WithName(NameT&& value) {
  ...
  void SetVersionOrAlias(VersionOrAliasT&& value) {
    m_versionOrAliasHasBeenSet = true;
    m_versionOrAlias = std::forward<VersionOrAliasT>(value);
  }
  template <typename VersionOrAliasT = Aws::String>
  GetBotRequest& WithVersionOrAlias(VersionOrAliasT&& value) {
    SetVersionOrAlias(std::forward<VersionOrAliasT>(value));
    return *this;
  }
  ///@}
 private:
