...
class GetBotsResult {
 public:
  ...
  template <typename BotsT = Aws::Vector<BotMetadata>>
  GetBotsResult& WithBots(BotsT&& value) {
    SetBots(std::forward<BotsT>(value));
    return *this;
  }
  template <typename BotsT = BotMetadata>
  GetBotsResult& AddBots(BotsT&& value) {
    m_botsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  GetBotsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  GetBotsResult& WithRequestId(RequestIdT&& value) {
