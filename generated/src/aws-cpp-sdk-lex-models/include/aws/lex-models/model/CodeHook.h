...
class CodeHook {
 public:
  ...
  void SetMessageVersion(MessageVersionT&& value) {
    m_messageVersionHasBeenSet = true;
    m_messageVersion = std::forward<MessageVersionT>(value);
  }
  template <typename MessageVersionT = Aws::String>
  CodeHook& WithMessageVersion(MessageVersionT&& value) {
    SetMessageVersion(std::forward<MessageVersionT>(value));
    return *this;
  }
  ///@}
 private:
