...
class BuiltinSlotTypeMetadata {
 public:
  ...
  template <typename SignatureT = Aws::String>
  BuiltinSlotTypeMetadata& WithSignature(SignatureT&& value) {
  ...
  template <typename SupportedLocalesT = Aws::Vector<Locale>>
  BuiltinSlotTypeMetadata& WithSupportedLocales(SupportedLocalesT&& value) {
    SetSupportedLocales(std::forward<SupportedLocalesT>(value));
    return *this;
  }
  inline BuiltinSlotTypeMetadata& AddSupportedLocales(Locale value) {
    m_supportedLocalesHasBeenSet = true;
