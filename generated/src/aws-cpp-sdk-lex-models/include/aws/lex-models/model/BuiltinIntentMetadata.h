...
class BuiltinIntentMetadata {
 public:
  ...
  template <typename SignatureT = Aws::String>
  BuiltinIntentMetadata& WithSignature(SignatureT&& value) {
  ...
  void SetSupportedLocales(SupportedLocalesT&& value) {
    ...
    m_supportedLocales = std::forward<SupportedLocalesT>(value);
  }
  template <typename SupportedLocalesT = Aws::Vector<Locale>>
  BuiltinIntentMetadata& WithSupportedLocales(SupportedLocalesT&& value) {
    SetSupportedLocales(std::forward<SupportedLocalesT>(value));
    return *this;
  }
  inline BuiltinIntentMetadata& AddSupportedLocales(Locale value) {
    m_supportedLocalesHasBeenSet = true;
    m_supportedLocales.push_back(value);
    return *this;
  }
  ///@}
 private:
