...
IntegerCriteriaCondition::IntegerCriteriaCondition(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

IntegerCriteriaCondition& IntegerCriteriaCondition::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
                      ...
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        ...
                        if (peekType_1.has_value()) {
                          if (peekType_1.value() == Aws::Crt::Cbor::CborType::UInt) {
                            ...
                            if (val.has_value()) {
                              m_values.push_back(static_cast<int64_t>(val.value()));
                            }
                          } else {
                            auto val = decoder->PopNextNegativeIntVal();
                            if (val.has_value()) {
                              m_values.push_back(static_cast<int64_t>(1 - val.value()));
                            }
                          }
                        }
                      }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("IntegerCriteriaCondition", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void IntegerCriteriaCondition::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
