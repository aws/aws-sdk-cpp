...
RxNormEntity::RxNormEntity(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

RxNormEntity& RxNormEntity::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Id") {
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_category = RxNormEntityCategoryMapper::GetRxNormEntityCategoryForName(
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_type = RxNormEntityTypeMapper::GetRxNormEntityTypeForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_attributes.push_back(RxNormAttribute(decoder));
                      }
                    }
              ...
              else if (initialKeyStr == "RxNormConcepts") {
                auto peekType_0 = decoder->PeekType();
                if (peekType_0.has_value() &&
                    (peekType_0.value() == CborType::ArrayStart || peekType_0.value() == CborType::IndefArrayStart)) {
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_rxNormConcepts.push_back(RxNormConcept(decoder));
                      }
                    }
                  } else  // IndefArrayStart
                  {
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("RxNormEntity", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void RxNormEntity::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_idHasBeenSet) {
    mapSize++;
  ...
  if (m_categoryHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(RxNormEntityCategoryMapper::GetNameForRxNormEntityCategory(m_category).c_str()));
  }

  if (m_typeHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("Type"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(RxNormEntityTypeMapper::GetNameForRxNormEntityType(m_type).c_str()));
  }
  ...
  if (m_rxNormConceptsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("RxNormConcepts"));
    encoder.WriteArrayStart(m_rxNormConcepts.size());
    for (const auto& item_0 : m_rxNormConcepts) {
      item_0.CborEncode(encoder);
    }
  }
}
