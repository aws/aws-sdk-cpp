...
Attribute::Attribute(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

Attribute& Attribute::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Type") {
                ...
                if (val.has_value()) {
                  m_type = EntitySubTypeMapper::GetEntitySubTypeForName(
                ...
                auto val = decoder->PopNextTextVal();
                if (val.has_value()) {
                  m_relationshipType = RelationshipTypeMapper::GetRelationshipTypeForName(
                      Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_traits.push_back(Trait(decoder));
                      }
                    }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("Attribute", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void Attribute::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_typeHasBeenSet) {
    mapSize++;
  ...
  if (m_typeHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(EntitySubTypeMapper::GetNameForEntitySubType(m_type).c_str()));
  }
  ...
  if (m_relationshipTypeHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(RelationshipTypeMapper::GetNameForRelationshipType(m_relationshipType).c_str()));
  }
  ...
  if (m_categoryHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(EntityTypeMapper::GetNameForEntityType(m_category).c_str()));
  }
