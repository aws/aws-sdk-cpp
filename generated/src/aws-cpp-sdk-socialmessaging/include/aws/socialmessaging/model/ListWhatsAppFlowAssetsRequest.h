...
/**
 */
class ListWhatsAppFlowAssetsRequest : public SocialMessagingRequest {
 public:
  AWS_SOCIALMESSAGING_API ListWhatsAppFlowAssetsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListWhatsAppFlowAssets"; }
  ...
  template <typename IdT = Aws::String>
  ListWhatsAppFlowAssetsRequest& WithId(IdT&& value) {
  ...
  template <typename FlowIdT = Aws::String>
  ListWhatsAppFlowAssetsRequest& WithFlowId(FlowIdT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListWhatsAppFlowAssetsRequest& WithNextToken(NextTokenT&& value) {
  ...
  inline ListWhatsAppFlowAssetsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
