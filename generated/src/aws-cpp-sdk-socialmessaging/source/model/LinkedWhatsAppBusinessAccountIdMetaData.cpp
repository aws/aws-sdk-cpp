...
LinkedWhatsAppBusinessAccountIdMetaData::LinkedWhatsAppBusinessAccountIdMetaData(JsonView jsonValue) { *this = jsonValue; }
...
LinkedWhatsAppBusinessAccountIdMetaData& LinkedWhatsAppBusinessAccountIdMetaData::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("unregisteredWhatsAppPhoneNumbers")) {
    ...
    for (unsigned unregisteredWhatsAppPhoneNumbersIndex = 0;
         unregisteredWhatsAppPhoneNumbersIndex < unregisteredWhatsAppPhoneNumbersJsonList.GetLength();
         ++unregisteredWhatsAppPhoneNumbersIndex) {
      m_unregisteredWhatsAppPhoneNumbers.push_back(
          unregisteredWhatsAppPhoneNumbersJsonList[unregisteredWhatsAppPhoneNumbersIndex].AsObject());
    }
...
JsonValue LinkedWhatsAppBusinessAccountIdMetaData::Jsonize() const {
  ...
  if (m_unregisteredWhatsAppPhoneNumbersHasBeenSet) {
    ...
    for (unsigned unregisteredWhatsAppPhoneNumbersIndex = 0;
         ...
         ++unregisteredWhatsAppPhoneNumbersIndex) {
      unregisteredWhatsAppPhoneNumbersJsonList[unregisteredWhatsAppPhoneNumbersIndex].AsObject(
          m_unregisteredWhatsAppPhoneNumbers[unregisteredWhatsAppPhoneNumbersIndex].Jsonize());
    }
