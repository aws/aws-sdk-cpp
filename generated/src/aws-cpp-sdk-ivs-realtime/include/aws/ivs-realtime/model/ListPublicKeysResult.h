...
class ListPublicKeysResult {
 public:
  ...
  void SetPublicKeys(PublicKeysT&& value) {
    ...
    m_publicKeys = std::forward<PublicKeysT>(value);
  }
  template <typename PublicKeysT = Aws::Vector<PublicKeySummary>>
  ListPublicKeysResult& WithPublicKeys(PublicKeysT&& value) {
    SetPublicKeys(std::forward<PublicKeysT>(value));
    return *this;
  }
  template <typename PublicKeysT = PublicKeySummary>
  ListPublicKeysResult& AddPublicKeys(PublicKeysT&& value) {
    m_publicKeysHasBeenSet = true;
    m_publicKeys.emplace_back(std::forward<PublicKeysT>(value));
    return *this;
  }
  ///@}
