...
class S3Detail {
 public:
  ...
  void SetRecordingPrefix(RecordingPrefixT&& value) {
    m_recordingPrefixHasBeenSet = true;
    m_recordingPrefix = std::forward<RecordingPrefixT>(value);
  }
  template <typename RecordingPrefixT = Aws::String>
  S3Detail& WithRecordingPrefix(RecordingPrefixT&& value) {
    SetRecordingPrefix(std::forward<RecordingPrefixT>(value));
    return *this;
  }
  ///@}
 private:
