...
template <>
AWS_IVSREALTIME_API ConflictException IvsrealtimeError::GetModeledError() {
  assert(this->GetErrorType() == IvsrealtimeErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_IVSREALTIME_API ServiceQuotaExceededException IvsrealtimeError::GetModeledError() {
  assert(this->GetErrorType() == IvsrealtimeErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_IVSREALTIME_API ResourceNotFoundException IvsrealtimeError::GetModeledError() {
  assert(this->GetErrorType() == IvsrealtimeErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_IVSREALTIME_API InternalServerException IvsrealtimeError::GetModeledError() {
  assert(this->GetErrorType() == IvsrealtimeErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_IVSREALTIME_API ValidationException IvsrealtimeError::GetModeledError() {
  assert(this->GetErrorType() == IvsrealtimeErrors::VALIDATION);
  return ValidationException(this->GetJsonPayload().View());
}

template <>
AWS_IVSREALTIME_API PendingVerification IvsrealtimeError::GetModeledError() {
  assert(this->GetErrorType() == IvsrealtimeErrors::PENDING_VERIFICATION);
  return PendingVerification(this->GetJsonPayload().View());
}

template <>
AWS_IVSREALTIME_API AccessDeniedException IvsrealtimeError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(IvsrealtimeErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
