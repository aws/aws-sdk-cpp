...
S3DestinationConfiguration& S3DestinationConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("encoderConfigurationArns")) {
    ...
    for (unsigned encoderConfigurationArnsIndex = 0; encoderConfigurationArnsIndex < encoderConfigurationArnsJsonList.GetLength();
         ++encoderConfigurationArnsIndex) {
  ...
  if (jsonValue.ValueExists("thumbnailConfigurations")) {
    ...
    for (unsigned thumbnailConfigurationsIndex = 0; thumbnailConfigurationsIndex < thumbnailConfigurationsJsonList.GetLength();
         ++thumbnailConfigurationsIndex) {
...
JsonValue S3DestinationConfiguration::Jsonize() const {
  ...
  if (m_thumbnailConfigurationsHasBeenSet) {
    ...
    for (unsigned thumbnailConfigurationsIndex = 0; thumbnailConfigurationsIndex < thumbnailConfigurationsJsonList.GetLength();
         ++thumbnailConfigurationsIndex) {
      thumbnailConfigurationsJsonList[thumbnailConfigurationsIndex].AsObject(
          m_thumbnailConfigurations[thumbnailConfigurationsIndex].Jsonize());
    }
