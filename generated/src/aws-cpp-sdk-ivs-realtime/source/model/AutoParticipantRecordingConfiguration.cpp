...
AutoParticipantRecordingConfiguration::AutoParticipantRecordingConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
AutoParticipantRecordingConfiguration& AutoParticipantRecordingConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("mediaTypes")) {
    ...
    for (unsigned mediaTypesIndex = 0; mediaTypesIndex < mediaTypesJsonList.GetLength(); ++mediaTypesIndex) {
      m_mediaTypes.push_back(
          ParticipantRecordingMediaTypeMapper::GetParticipantRecordingMediaTypeForName(mediaTypesJsonList[mediaTypesIndex].AsString()));
    }
...
JsonValue AutoParticipantRecordingConfiguration::Jsonize() const {
  ...
  if (m_mediaTypesHasBeenSet) {
    ...
    for (unsigned mediaTypesIndex = 0; mediaTypesIndex < mediaTypesJsonList.GetLength(); ++mediaTypesIndex) {
      mediaTypesJsonList[mediaTypesIndex].AsString(
          ParticipantRecordingMediaTypeMapper::GetNameForParticipantRecordingMediaType(m_mediaTypes[mediaTypesIndex]));
    }
