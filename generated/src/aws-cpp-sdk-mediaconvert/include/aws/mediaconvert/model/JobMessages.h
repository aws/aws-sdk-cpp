...
class JobMessages {
 public:
  ...
  template <typename InfoT = Aws::Vector<Aws::String>>
  JobMessages& WithInfo(InfoT&& value) {
    SetInfo(std::forward<InfoT>(value));
    return *this;
  }
  template <typename InfoT = Aws::String>
  JobMessages& AddInfo(InfoT&& value) {
    m_infoHasBeenSet = true;
    m_info.emplace_back(std::forward<InfoT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename WarningT = Aws::Vector<Aws::String>>
  JobMessages& WithWarning(WarningT&& value) {
    SetWarning(std::forward<WarningT>(value));
    return *this;
  }
  template <typename WarningT = Aws::String>
  JobMessages& AddWarning(WarningT&& value) {
    m_warningHasBeenSet = true;
    m_warning.emplace_back(std::forward<WarningT>(value));
    return *this;
  }
  ///@}
 private:
