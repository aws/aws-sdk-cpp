...
class ProbeResult {
 public:
  ...
  template <typename ContainerT = Container>
  ProbeResult& WithContainer(ContainerT&& value) {
  ...
  template <typename MetadataT = Metadata>
  ProbeResult& WithMetadata(MetadataT&& value) {
  ...
  void SetTrackMappings(TrackMappingsT&& value) {
    m_trackMappingsHasBeenSet = true;
    m_trackMappings = std::forward<TrackMappingsT>(value);
  }
  template <typename TrackMappingsT = Aws::Vector<TrackMapping>>
  ProbeResult& WithTrackMappings(TrackMappingsT&& value) {
    SetTrackMappings(std::forward<TrackMappingsT>(value));
    return *this;
  }
  template <typename TrackMappingsT = TrackMapping>
  ProbeResult& AddTrackMappings(TrackMappingsT&& value) {
    m_trackMappingsHasBeenSet = true;
    m_trackMappings.emplace_back(std::forward<TrackMappingsT>(value));
    return *this;
  }
  ///@}
 private:
