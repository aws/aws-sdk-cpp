...
class ProbeRequest : public MediaConvertRequest {
 public:
  ...
  void SetInputFiles(InputFilesT&& value) {
    m_inputFilesHasBeenSet = true;
    m_inputFiles = std::forward<InputFilesT>(value);
  }
  template <typename InputFilesT = Aws::Vector<ProbeInputFile>>
  ProbeRequest& WithInputFiles(InputFilesT&& value) {
    SetInputFiles(std::forward<InputFilesT>(value));
    return *this;
  }
  template <typename InputFilesT = ProbeInputFile>
  ProbeRequest& AddInputFiles(InputFilesT&& value) {
    m_inputFilesHasBeenSet = true;
    m_inputFiles.emplace_back(std::forward<InputFilesT>(value));
    return *this;
  }
  ///@}
 private:
