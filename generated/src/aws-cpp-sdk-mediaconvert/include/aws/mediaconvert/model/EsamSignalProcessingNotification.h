...
class EsamSignalProcessingNotification {
 public:
  ...
  void SetSccXml(SccXmlT&& value) {
    m_sccXmlHasBeenSet = true;
    m_sccXml = std::forward<SccXmlT>(value);
  }
  template <typename SccXmlT = Aws::String>
  EsamSignalProcessingNotification& WithSccXml(SccXmlT&& value) {
    SetSccXml(std::forward<SccXmlT>(value));
    return *this;
  }
  ///@}
 private:
