...
class EsamManifestConfirmConditionNotification {
 public:
  ...
  void SetMccXml(MccXmlT&& value) {
    m_mccXmlHasBeenSet = true;
    m_mccXml = std::forward<MccXmlT>(value);
  }
  template <typename MccXmlT = Aws::String>
  EsamManifestConfirmConditionNotification& WithMccXml(MccXmlT&& value) {
    SetMccXml(std::forward<MccXmlT>(value));
    return *this;
  }
  ///@}
 private:
