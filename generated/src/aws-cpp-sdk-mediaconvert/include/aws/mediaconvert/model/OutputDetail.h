...
class OutputDetail {
 public:
  ...
  inline void SetDurationInMs(int value) {
    m_durationInMsHasBeenSet = true;
    m_durationInMs = value;
  }
  inline OutputDetail& WithDurationInMs(int value) {
  ...
  void SetVideoDetails(VideoDetailsT&& value) {
    ...
    m_videoDetails = std::forward<VideoDetailsT>(value);
  }
  template <typename VideoDetailsT = VideoDetail>
  OutputDetail& WithVideoDetails(VideoDetailsT&& value) {
    SetVideoDetails(std::forward<VideoDetailsT>(value));
    return *this;
  }
  ///@}
 private:
