...
AutomatedAbrRule::AutomatedAbrRule(JsonView jsonValue) { *this = jsonValue; }
...
AutomatedAbrRule& AutomatedAbrRule::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("forceIncludeRenditions")) {
    ...
    for (unsigned forceIncludeRenditionsIndex = 0; forceIncludeRenditionsIndex < forceIncludeRenditionsJsonList.GetLength();
         ++forceIncludeRenditionsIndex) {
