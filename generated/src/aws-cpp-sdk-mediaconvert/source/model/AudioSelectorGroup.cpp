...
AudioSelectorGroup::AudioSelectorGroup(JsonView jsonValue) { *this = jsonValue; }
...
AudioSelectorGroup& AudioSelectorGroup::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("audioSelectorNames")) {
    ...
    for (unsigned audioSelectorNamesIndex = 0; audioSelectorNamesIndex < audioSelectorNamesJsonList.GetLength();
         ++audioSelectorNamesIndex) {
