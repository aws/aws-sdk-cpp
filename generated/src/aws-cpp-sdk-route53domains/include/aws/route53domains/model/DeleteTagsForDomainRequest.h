...
class DeleteTagsForDomainRequest : public Route53DomainsRequest {
 public:
  ...
  template <typename DomainNameT = Aws::String>
  DeleteTagsForDomainRequest& WithDomainName(DomainNameT&& value) {
  ...
  void SetTagsToDelete(TagsToDeleteT&& value) {
    m_tagsToDeleteHasBeenSet = true;
    m_tagsToDelete = std::forward<TagsToDeleteT>(value);
  }
  template <typename TagsToDeleteT = Aws::Vector<Aws::String>>
  DeleteTagsForDomainRequest& WithTagsToDelete(TagsToDeleteT&& value) {
    SetTagsToDelete(std::forward<TagsToDeleteT>(value));
    return *this;
  }
  template <typename TagsToDeleteT = Aws::String>
  DeleteTagsForDomainRequest& AddTagsToDelete(TagsToDeleteT&& value) {
    m_tagsToDeleteHasBeenSet = true;
    m_tagsToDelete.emplace_back(std::forward<TagsToDeleteT>(value));
    return *this;
  }
  ///@}
 private:
