...
class ViewBillingResult {
 public:
  ...
  void SetNextPageMarker(NextPageMarkerT&& value) {
    ...
    m_nextPageMarker = std::forward<NextPageMarkerT>(value);
  }
  template <typename NextPageMarkerT = Aws::String>
  ViewBillingResult& WithNextPageMarker(NextPageMarkerT&& value) {
    SetNextPageMarker(std::forward<NextPageMarkerT>(value));
    return *this;
  }
  ///@}
  ...
  void SetBillingRecords(BillingRecordsT&& value) {
    ...
    m_billingRecords = std::forward<BillingRecordsT>(value);
  }
  template <typename BillingRecordsT = Aws::Vector<BillingRecord>>
  ViewBillingResult& WithBillingRecords(BillingRecordsT&& value) {
    SetBillingRecords(std::forward<BillingRecordsT>(value));
    return *this;
  }
  template <typename BillingRecordsT = BillingRecord>
  ViewBillingResult& AddBillingRecords(BillingRecordsT&& value) {
    m_billingRecordsHasBeenSet = true;
    m_billingRecords.emplace_back(std::forward<BillingRecordsT>(value));
    return *this;
  }
  ///@}
