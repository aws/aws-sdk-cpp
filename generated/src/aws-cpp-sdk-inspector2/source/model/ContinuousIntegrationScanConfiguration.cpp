...
ContinuousIntegrationScanConfiguration::ContinuousIntegrationScanConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
ContinuousIntegrationScanConfiguration& ContinuousIntegrationScanConfiguration::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("supportedEvents")) {
    ...
    for (unsigned supportedEventsIndex = 0; supportedEventsIndex < supportedEventsJsonList.GetLength(); ++supportedEventsIndex) {
      m_supportedEvents.push_back(ContinuousIntegrationScanEventMapper::GetContinuousIntegrationScanEventForName(
          supportedEventsJsonList[supportedEventsIndex].AsString()));
    }
...
JsonValue ContinuousIntegrationScanConfiguration::Jsonize() const {
  ...
  if (m_supportedEventsHasBeenSet) {
    ...
    for (unsigned supportedEventsIndex = 0; supportedEventsIndex < supportedEventsJsonList.GetLength(); ++supportedEventsIndex) {
      supportedEventsJsonList[supportedEventsIndex].AsString(
          ContinuousIntegrationScanEventMapper::GetNameForContinuousIntegrationScanEvent(m_supportedEvents[supportedEventsIndex]));
    }
