...
CodeSecurityScanConfigurationSummary::CodeSecurityScanConfigurationSummary(JsonView jsonValue) { *this = jsonValue; }
...
CodeSecurityScanConfigurationSummary& CodeSecurityScanConfigurationSummary::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("continuousIntegrationScanSupportedEvents")) {
    Aws::Utils::Array<JsonView> continuousIntegrationScanSupportedEventsJsonList =
        jsonValue.GetArray("continuousIntegrationScanSupportedEvents");
    for (unsigned continuousIntegrationScanSupportedEventsIndex = 0;
         continuousIntegrationScanSupportedEventsIndex < continuousIntegrationScanSupportedEventsJsonList.GetLength();
         ++continuousIntegrationScanSupportedEventsIndex) {
      m_continuousIntegrationScanSupportedEvents.push_back(ContinuousIntegrationScanEventMapper::GetContinuousIntegrationScanEventForName(
          continuousIntegrationScanSupportedEventsJsonList[continuousIntegrationScanSupportedEventsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("ruleSetCategories")) {
    ...
    for (unsigned ruleSetCategoriesIndex = 0; ruleSetCategoriesIndex < ruleSetCategoriesJsonList.GetLength(); ++ruleSetCategoriesIndex) {
      m_ruleSetCategories.push_back(
          RuleSetCategoryMapper::GetRuleSetCategoryForName(ruleSetCategoriesJsonList[ruleSetCategoriesIndex].AsString()));
    }
...
JsonValue CodeSecurityScanConfigurationSummary::Jsonize() const {
  ...
  if (m_continuousIntegrationScanSupportedEventsHasBeenSet) {
    ...
    for (unsigned continuousIntegrationScanSupportedEventsIndex = 0;
         ...
         ++continuousIntegrationScanSupportedEventsIndex) {
      continuousIntegrationScanSupportedEventsJsonList[continuousIntegrationScanSupportedEventsIndex].AsString(
          ContinuousIntegrationScanEventMapper::GetNameForContinuousIntegrationScanEvent(
              m_continuousIntegrationScanSupportedEvents[continuousIntegrationScanSupportedEventsIndex]));
    }
  ...
  if (m_ruleSetCategoriesHasBeenSet) {
    ...
    for (unsigned ruleSetCategoriesIndex = 0; ruleSetCategoriesIndex < ruleSetCategoriesJsonList.GetLength(); ++ruleSetCategoriesIndex) {
      ruleSetCategoriesJsonList[ruleSetCategoriesIndex].AsString(
          RuleSetCategoryMapper::GetNameForRuleSetCategory(m_ruleSetCategories[ruleSetCategoriesIndex]));
    }
