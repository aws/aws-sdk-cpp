...
class SearchVulnerabilitiesFilterCriteria {
 public:
  ...
  void SetVulnerabilityIds(VulnerabilityIdsT&& value) {
    ...
    m_vulnerabilityIds = std::forward<VulnerabilityIdsT>(value);
  }
  template <typename VulnerabilityIdsT = Aws::Vector<Aws::String>>
  SearchVulnerabilitiesFilterCriteria& WithVulnerabilityIds(VulnerabilityIdsT&& value) {
    SetVulnerabilityIds(std::forward<VulnerabilityIdsT>(value));
    return *this;
  }
  template <typename VulnerabilityIdsT = Aws::String>
  SearchVulnerabilitiesFilterCriteria& AddVulnerabilityIds(VulnerabilityIdsT&& value) {
    m_vulnerabilityIdsHasBeenSet = true;
    m_vulnerabilityIds.emplace_back(std::forward<VulnerabilityIdsT>(value));
    return *this;
  }
  ///@}
 private:
