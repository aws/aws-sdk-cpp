...
class ListMembersResult {
 public:
  AWS_INSPECTOR2_API ListMembersResult() = default;
  ...
  template <typename MembersT = Aws::Vector<Member>>
  ListMembersResult& WithMembers(MembersT&& value) {
    SetMembers(std::forward<MembersT>(value));
    return *this;
  }
  template <typename MembersT = Member>
  ListMembersResult& AddMembers(MembersT&& value) {
    m_membersHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListMembersResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListMembersResult& WithRequestId(RequestIdT&& value) {
