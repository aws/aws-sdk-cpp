...
namespace ECR {
template <>
AWS_ECR_API KmsException ECRError::GetModeledError() {
  assert(this->GetErrorType() == ECRErrors::KMS);
  return KmsException(this->GetJsonPayload().View());
}

template <>
AWS_ECR_API InvalidLayerPartException ECRError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == IMAGE_DIGEST_DOES_NOT_MATCH_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(ECRErrors::LIMIT_EXCEEDED), RetryableType::RETRYABLE_THROTTLING);
