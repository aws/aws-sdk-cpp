...
class IframeConfig {
 public:
  ...
  IframeConfig& AddAllow(AllowT&& value) {
    m_allowHasBeenSet = true;
    m_allow.emplace_back(std::forward<AllowT>(value));
    return *this;
  }
  ///@}
  ...
  void SetSandbox(SandboxT&& value) {
    ...
    m_sandbox = std::forward<SandboxT>(value);
  }
  template <typename SandboxT = Aws::Vector<Aws::String>>
  IframeConfig& WithSandbox(SandboxT&& value) {
    SetSandbox(std::forward<SandboxT>(value));
    return *this;
  }
  template <typename SandboxT = Aws::String>
  IframeConfig& AddSandbox(SandboxT&& value) {
    m_sandboxHasBeenSet = true;
    m_sandbox.emplace_back(std::forward<SandboxT>(value));
    return *this;
  }
  ///@}
 private:
