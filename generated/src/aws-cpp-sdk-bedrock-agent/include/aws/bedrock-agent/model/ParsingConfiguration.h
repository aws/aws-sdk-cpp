...
class ParsingConfiguration {
 public:
  ...
  ///@{
  /**
   * <p>The parsing strategy for the data source.</p> <p>For managed knowledge bases,
   * the strategy that you can select depends on the embedding model that your
   * knowledge base uses:</p> <ul> <li> <p>If your knowledge base uses a native
   * multimodal embedding model, specify <code>MULTI_MODAL_EMBEDDINGS</code>. With
   * this strategy, files are sent directly to the embedding model instead of being
   * parsed into text. This is the only strategy that is supported for these
   * knowledge bases.</p> </li> <li> <p>Otherwise, specify
   * <code>SMART_PARSING</code>.</p> </li> </ul> <p>For more information, see <a
  ...
  inline void SetParsingStrategy(ParsingStrategy value) {
    m_parsingStrategyHasBeenSet = true;
    m_parsingStrategy = value;
  }
  inline ParsingConfiguration& WithParsingStrategy(ParsingStrategy value) {
    SetParsingStrategy(value);
    return *this;
  }
  ///@}
   ...
   */
  inline const BedrockDataAutomationConfiguration& GetBedrockDataAutomationConfiguration() const {
    return m_bedrockDataAutomationConfiguration;
  }
  ...
  void SetBedrockDataAutomationConfiguration(BedrockDataAutomationConfigurationT&& value) {
    ...
    m_bedrockDataAutomationConfiguration = std::forward<BedrockDataAutomationConfigurationT>(value);
  }
  template <typename BedrockDataAutomationConfigurationT = BedrockDataAutomationConfiguration>
  ParsingConfiguration& WithBedrockDataAutomationConfiguration(BedrockDataAutomationConfigurationT&& value) {
    SetBedrockDataAutomationConfiguration(std::forward<BedrockDataAutomationConfigurationT>(value));
    return *this;
  }
  ///@}
 private:
