...
/**
 */
class GetVpcConfigurationRequest : public BedrockAgentRequest {
 public:
  AWS_BEDROCKAGENT_API GetVpcConfigurationRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetVpcConfiguration"; }
  ...
  template <typename KnowledgeBaseIdT = Aws::String>
  GetVpcConfigurationRequest& WithKnowledgeBaseId(KnowledgeBaseIdT&& value) {
  ...
  template <typename VpcConfigurationIdT = Aws::String>
  GetVpcConfigurationRequest& WithVpcConfigurationId(VpcConfigurationIdT&& value) {
