...
MemoryConfiguration::MemoryConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
MemoryConfiguration& MemoryConfiguration::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("enabledMemoryTypes")) {
    ...
    for (unsigned enabledMemoryTypesIndex = 0; enabledMemoryTypesIndex < enabledMemoryTypesJsonList.GetLength();
         ++enabledMemoryTypesIndex) {
      m_enabledMemoryTypes.push_back(
          MemoryTypeMapper::GetMemoryTypeForName(enabledMemoryTypesJsonList[enabledMemoryTypesIndex].AsString()));
    }
...
JsonValue MemoryConfiguration::Jsonize() const {
  ...
  if (m_enabledMemoryTypesHasBeenSet) {
    ...
    for (unsigned enabledMemoryTypesIndex = 0; enabledMemoryTypesIndex < enabledMemoryTypesJsonList.GetLength();
         ++enabledMemoryTypesIndex) {
      enabledMemoryTypesJsonList[enabledMemoryTypesIndex].AsString(
          MemoryTypeMapper::GetNameForMemoryType(m_enabledMemoryTypes[enabledMemoryTypesIndex]));
    }
