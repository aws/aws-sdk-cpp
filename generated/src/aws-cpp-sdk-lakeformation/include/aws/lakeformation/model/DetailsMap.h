...
class DetailsMap {
 public:
  ...
  template <typename ResourceShareT = Aws::Vector<Aws::String>>
  DetailsMap& WithResourceShare(ResourceShareT&& value) {
    SetResourceShare(std::forward<ResourceShareT>(value));
    return *this;
  }
  template <typename ResourceShareT = Aws::String>
  DetailsMap& AddResourceShare(ResourceShareT&& value) {
    m_resourceShareHasBeenSet = true;
    m_resourceShare.emplace_back(std::forward<ResourceShareT>(value));
    return *this;
  }
  ///@}
 private:
