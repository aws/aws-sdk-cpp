...
class RowFilter {
 public:
  ...
  template <typename FilterExpressionT = Aws::String>
  RowFilter& WithFilterExpression(FilterExpressionT&& value) {
  ...
  void SetAllRowsWildcard(AllRowsWildcardT&& value) {
    m_allRowsWildcardHasBeenSet = true;
    m_allRowsWildcard = std::forward<AllRowsWildcardT>(value);
  }
  template <typename AllRowsWildcardT = AllRowsWildcard>
  RowFilter& WithAllRowsWildcard(AllRowsWildcardT&& value) {
    SetAllRowsWildcard(std::forward<AllRowsWildcardT>(value));
    return *this;
  }
  ///@}
 private:
