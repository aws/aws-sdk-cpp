...
class RemoveLFTagsFromResourceResult {
 public:
  ...
  template <typename FailuresT = Aws::Vector<LFTagError>>
  RemoveLFTagsFromResourceResult& WithFailures(FailuresT&& value) {
    SetFailures(std::forward<FailuresT>(value));
    return *this;
  }
  template <typename FailuresT = LFTagError>
  RemoveLFTagsFromResourceResult& AddFailures(FailuresT&& value) {
    m_failuresHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  RemoveLFTagsFromResourceResult& WithRequestId(RequestIdT&& value) {
