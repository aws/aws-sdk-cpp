...
class BatchRevokePermissionsResult {
 public:
  ...
  template <typename FailuresT = Aws::Vector<BatchPermissionsFailureEntry>>
  BatchRevokePermissionsResult& WithFailures(FailuresT&& value) {
    SetFailures(std::forward<FailuresT>(value));
    return *this;
  }
  template <typename FailuresT = BatchPermissionsFailureEntry>
  BatchRevokePermissionsResult& AddFailures(FailuresT&& value) {
    m_failuresHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchRevokePermissionsResult& WithRequestId(RequestIdT&& value) {
