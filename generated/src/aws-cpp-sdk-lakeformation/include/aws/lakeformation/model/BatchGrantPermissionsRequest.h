...
class BatchGrantPermissionsRequest : public LakeFormationRequest {
 public:
  ...
  template <typename CatalogIdT = Aws::String>
  BatchGrantPermissionsRequest& WithCatalogId(CatalogIdT&& value) {
  ...
  template <typename EntriesT = Aws::Vector<BatchPermissionsRequestEntry>>
  BatchGrantPermissionsRequest& WithEntries(EntriesT&& value) {
    SetEntries(std::forward<EntriesT>(value));
    return *this;
  }
  template <typename EntriesT = BatchPermissionsRequestEntry>
  BatchGrantPermissionsRequest& AddEntries(EntriesT&& value) {
    m_entriesHasBeenSet = true;
