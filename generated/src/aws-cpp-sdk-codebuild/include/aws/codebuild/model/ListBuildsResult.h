...
class ListBuildsResult {
 public:
  AWS_CODEBUILD_API ListBuildsResult() = default;
  ...
  template <typename IdsT = Aws::Vector<Aws::String>>
  ListBuildsResult& WithIds(IdsT&& value) {
    SetIds(std::forward<IdsT>(value));
    return *this;
  }
  template <typename IdsT = Aws::String>
  ListBuildsResult& AddIds(IdsT&& value) {
    m_idsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListBuildsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListBuildsResult& WithRequestId(RequestIdT&& value) {
