...
class StartSandboxConnectionResult {
 public:
  ...
  void SetSsmSession(SsmSessionT&& value) {
    m_ssmSessionHasBeenSet = true;
    m_ssmSession = std::forward<SsmSessionT>(value);
  }
  template <typename SsmSessionT = SSMSession>
  StartSandboxConnectionResult& WithSsmSession(SsmSessionT&& value) {
    SetSsmSession(std::forward<SsmSessionT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  StartSandboxConnectionResult& WithRequestId(RequestIdT&& value) {
