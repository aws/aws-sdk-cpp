...
BuildGroup::BuildGroup(JsonView jsonValue) { *this = jsonValue; }
...
BuildGroup& BuildGroup::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("priorBuildSummaryList")) {
    ...
    for (unsigned priorBuildSummaryListIndex = 0; priorBuildSummaryListIndex < priorBuildSummaryListJsonList.GetLength();
         ++priorBuildSummaryListIndex) {
