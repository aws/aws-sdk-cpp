...
class SearchTextFilter {
 public:
  ...
  template <typename BoundingBoxT = Aws::Vector<double>>
  SearchTextFilter& WithBoundingBox(BoundingBoxT&& value) {
    SetBoundingBox(std::forward<BoundingBoxT>(value));
    return *this;
  }
  inline SearchTextFilter& AddBoundingBox(double value) {
    m_boundingBoxHasBeenSet = true;
  ...
  template <typename CircleT = FilterCircle>
  SearchTextFilter& WithCircle(CircleT&& value) {
  ...
  void SetIncludeCountries(IncludeCountriesT&& value) {
    m_includeCountriesHasBeenSet = true;
    m_includeCountries = std::forward<IncludeCountriesT>(value);
  }
  template <typename IncludeCountriesT = Aws::Vector<Aws::String>>
  SearchTextFilter& WithIncludeCountries(IncludeCountriesT&& value) {
    SetIncludeCountries(std::forward<IncludeCountriesT>(value));
    return *this;
  }
  template <typename IncludeCountriesT = Aws::String>
  SearchTextFilter& AddIncludeCountries(IncludeCountriesT&& value) {
    m_includeCountriesHasBeenSet = true;
    m_includeCountries.emplace_back(std::forward<IncludeCountriesT>(value));
    return *this;
  }
  ///@}
 private:
