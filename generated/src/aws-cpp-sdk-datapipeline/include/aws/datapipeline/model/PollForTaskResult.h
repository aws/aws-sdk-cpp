...
class PollForTaskResult {
 public:
  ...
  void SetTaskObject(TaskObjectT&& value) {
    m_taskObjectHasBeenSet = true;
    m_taskObject = std::forward<TaskObjectT>(value);
  }
  template <typename TaskObjectT = TaskObject>
  PollForTaskResult& WithTaskObject(TaskObjectT&& value) {
    SetTaskObject(std::forward<TaskObjectT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  PollForTaskResult& WithRequestId(RequestIdT&& value) {
