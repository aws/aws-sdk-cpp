...
class PollForTaskRequest : public DataPipelineRequest {
 public:
  ...
  void SetWorkerGroup(WorkerGroupT&& value) {
    m_workerGroupHasBeenSet = true;
    m_workerGroup = std::forward<WorkerGroupT>(value);
  }
  template <typename WorkerGroupT = Aws::String>
  PollForTaskRequest& WithWorkerGroup(WorkerGroupT&& value) {
    SetWorkerGroup(std::forward<WorkerGroupT>(value));
    return *this;
  }
  ///@}
  ...
  void SetInstanceIdentity(InstanceIdentityT&& value) {
    m_instanceIdentityHasBeenSet = true;
    m_instanceIdentity = std::forward<InstanceIdentityT>(value);
  }
  template <typename InstanceIdentityT = InstanceIdentity>
  PollForTaskRequest& WithInstanceIdentity(InstanceIdentityT&& value) {
    SetInstanceIdentity(std::forward<InstanceIdentityT>(value));
    return *this;
  }
  ///@}
 private:
