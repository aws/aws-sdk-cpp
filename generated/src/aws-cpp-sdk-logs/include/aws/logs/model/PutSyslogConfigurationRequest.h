...
/**
 */
class PutSyslogConfigurationRequest : public CloudWatchLogsRequest {
 public:
  AWS_CLOUDWATCHLOGS_API PutSyslogConfigurationRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "PutSyslogConfiguration"; }
  ...
  ///@{
  /**
   * <p>The name or ARN of the log group to associate with the syslog
   * configuration.</p>
   */
  ...
  template <typename LogGroupIdentifierT = Aws::String>
  PutSyslogConfigurationRequest& WithLogGroupIdentifier(LogGroupIdentifierT&& value) {
  ...
  template <typename VpcEndpointIdT = Aws::String>
  PutSyslogConfigurationRequest& WithVpcEndpointId(VpcEndpointIdT&& value) {
