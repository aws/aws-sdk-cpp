...
class MalformedQueryException {
 public:
  ...
  void SetQueryCompileError(QueryCompileErrorT&& value) {
    m_queryCompileErrorHasBeenSet = true;
    m_queryCompileError = std::forward<QueryCompileErrorT>(value);
  }
  template <typename QueryCompileErrorT = QueryCompileError>
  MalformedQueryException& WithQueryCompileError(QueryCompileErrorT&& value) {
    SetQueryCompileError(std::forward<QueryCompileErrorT>(value));
    return *this;
  }
  ///@}
 private:
