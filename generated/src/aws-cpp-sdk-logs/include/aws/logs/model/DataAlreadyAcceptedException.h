...
class DataAlreadyAcceptedException {
 public:
  ...
  void SetExpectedSequenceToken(ExpectedSequenceTokenT&& value) {
    m_expectedSequenceTokenHasBeenSet = true;
    m_expectedSequenceToken = std::forward<ExpectedSequenceTokenT>(value);
  }
  template <typename ExpectedSequenceTokenT = Aws::String>
  DataAlreadyAcceptedException& WithExpectedSequenceToken(ExpectedSequenceTokenT&& value) {
    SetExpectedSequenceToken(std::forward<ExpectedSequenceTokenT>(value));
    return *this;
  }
  ///@}
 private:
