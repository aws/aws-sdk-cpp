...
class UpperCaseString {
 public:
  ...
  void SetWithKeys(WithKeysT&& value) {
    ...
    m_withKeys = std::forward<WithKeysT>(value);
  }
  template <typename WithKeysT = Aws::Vector<Aws::String>>
  UpperCaseString& WithWithKeys(WithKeysT&& value) {
    SetWithKeys(std::forward<WithKeysT>(value));
    return *this;
  }
  template <typename WithKeysT = Aws::String>
  UpperCaseString& AddWithKeys(WithKeysT&& value) {
    m_withKeysHasBeenSet = true;
    m_withKeys.emplace_back(std::forward<WithKeysT>(value));
    return *this;
  }
  ///@}
 private:
