...
class RenameKeyEntry {
 public:
  ...
  template <typename KeyT = Aws::String>
  RenameKeyEntry& WithKey(KeyT&& value) {
  ...
  void SetRenameTo(RenameToT&& value) {
    m_renameToHasBeenSet = true;
    m_renameTo = std::forward<RenameToT>(value);
  }
  template <typename RenameToT = Aws::String>
  RenameKeyEntry& WithRenameTo(RenameToT&& value) {
    SetRenameTo(std::forward<RenameToT>(value));
    return *this;
  }
  ///@}
  ...
  inline RenameKeyEntry& WithOverwriteIfExists(bool value) {
