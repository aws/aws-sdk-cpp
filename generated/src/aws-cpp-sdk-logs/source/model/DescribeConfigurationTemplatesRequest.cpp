...
Aws::String DescribeConfigurationTemplatesRequest::SerializePayload() const {
  ...
  if (m_deliveryDestinationTypesHasBeenSet) {
    ...
    for (unsigned deliveryDestinationTypesIndex = 0; deliveryDestinationTypesIndex < deliveryDestinationTypesJsonList.GetLength();
         ++deliveryDestinationTypesIndex) {
      deliveryDestinationTypesJsonList[deliveryDestinationTypesIndex].AsString(
          DeliveryDestinationTypeMapper::GetNameForDeliveryDestinationType(m_deliveryDestinationTypes[deliveryDestinationTypesIndex]));
    }
