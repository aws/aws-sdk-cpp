...
LogGroup::LogGroup(JsonView jsonValue) { *this = jsonValue; }
...
LogGroup& LogGroup::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("inheritedProperties")) {
    ...
    for (unsigned inheritedPropertiesIndex = 0; inheritedPropertiesIndex < inheritedPropertiesJsonList.GetLength();
         ++inheritedPropertiesIndex) {
      m_inheritedProperties.push_back(
          InheritedPropertyMapper::GetInheritedPropertyForName(inheritedPropertiesJsonList[inheritedPropertiesIndex].AsString()));
    }
...
JsonValue LogGroup::Jsonize() const {
  ...
  if (m_inheritedPropertiesHasBeenSet) {
    ...
    for (unsigned inheritedPropertiesIndex = 0; inheritedPropertiesIndex < inheritedPropertiesJsonList.GetLength();
         ++inheritedPropertiesIndex) {
      inheritedPropertiesJsonList[inheritedPropertiesIndex].AsString(
          InheritedPropertyMapper::GetNameForInheritedProperty(m_inheritedProperties[inheritedPropertiesIndex]));
    }
