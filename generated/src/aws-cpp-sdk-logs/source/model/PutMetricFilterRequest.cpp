...
Aws::String PutMetricFilterRequest::SerializePayload() const {
  ...
  if (m_metricTransformationsHasBeenSet) {
    ...
    for (unsigned metricTransformationsIndex = 0; metricTransformationsIndex < metricTransformationsJsonList.GetLength();
         ++metricTransformationsIndex) {
  ...
  if (m_emitSystemFieldDimensionsHasBeenSet) {
    ...
    for (unsigned emitSystemFieldDimensionsIndex = 0; emitSystemFieldDimensionsIndex < emitSystemFieldDimensionsJsonList.GetLength();
         ++emitSystemFieldDimensionsIndex) {
      emitSystemFieldDimensionsJsonList[emitSystemFieldDimensionsIndex].AsString(
          m_emitSystemFieldDimensions[emitSystemFieldDimensionsIndex]);
    }
