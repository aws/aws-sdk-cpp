...
/**
 */
class ListTemplateSnapshotsRequest : public NetworkSecurityManagerRequest {
 public:
  AWS_NETWORKSECURITYMANAGER_API ListTemplateSnapshotsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListTemplateSnapshots"; }
  ...
  template <typename TemplateIdentifierT = Aws::String>
  ListTemplateSnapshotsRequest& WithTemplateIdentifier(TemplateIdentifierT&& value) {
  ...
  inline ListTemplateSnapshotsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  template <typename NextTokenT = Aws::String>
  ListTemplateSnapshotsRequest& WithNextToken(NextTokenT&& value) {
