...
/**
 */
class CreateDeploymentSnapshotRequest : public NetworkSecurityManagerRequest {
 public:
  AWS_NETWORKSECURITYMANAGER_API CreateDeploymentSnapshotRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "CreateDeploymentSnapshot"; }
  ...
  template <typename DeploymentIdentifierT = Aws::String>
  CreateDeploymentSnapshotRequest& WithDeploymentIdentifier(DeploymentIdentifierT&& value) {
  ...
  template <typename ClientTokenT = Aws::String>
  CreateDeploymentSnapshotRequest& WithClientToken(ClientTokenT&& value) {
  ...
  template <typename TagsT = Aws::Map<Aws::String, Aws::String>>
  CreateDeploymentSnapshotRequest& WithTags(TagsT&& value) {
    SetTags(std::forward<TagsT>(value));
    return *this;
  }
  template <typename TagsKeyT = Aws::String, typename TagsValueT = Aws::String>
  CreateDeploymentSnapshotRequest& AddTags(TagsKeyT&& key, TagsValueT&& value) {
    m_tagsHasBeenSet = true;
