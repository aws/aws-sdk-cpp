...
/**
 */
class CreatePolicySnapshotRequest : public NetworkSecurityManagerRequest {
 public:
  AWS_NETWORKSECURITYMANAGER_API CreatePolicySnapshotRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "CreatePolicySnapshot"; }
  ...
  template <typename PolicyIdentifierT = Aws::String>
  CreatePolicySnapshotRequest& WithPolicyIdentifier(PolicyIdentifierT&& value) {
  ...
  template <typename ClientTokenT = Aws::String>
  CreatePolicySnapshotRequest& WithClientToken(ClientTokenT&& value) {
  ...
  template <typename TagsT = Aws::Map<Aws::String, Aws::String>>
  CreatePolicySnapshotRequest& WithTags(TagsT&& value) {
    SetTags(std::forward<TagsT>(value));
    return *this;
  }
  template <typename TagsKeyT = Aws::String, typename TagsValueT = Aws::String>
  CreatePolicySnapshotRequest& AddTags(TagsKeyT&& key, TagsValueT&& value) {
    m_tagsHasBeenSet = true;
