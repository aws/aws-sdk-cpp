...
/**
 */
class ListRuleSnapshotsRequest : public NetworkSecurityManagerRequest {
 public:
  AWS_NETWORKSECURITYMANAGER_API ListRuleSnapshotsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListRuleSnapshots"; }
  ...
  template <typename RuleIdentifierT = Aws::String>
  ListRuleSnapshotsRequest& WithRuleIdentifier(RuleIdentifierT&& value) {
  ...
  inline ListRuleSnapshotsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  template <typename NextTokenT = Aws::String>
  ListRuleSnapshotsRequest& WithNextToken(NextTokenT&& value) {
