...
UpdateDeploymentResult& UpdateDeploymentResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("deploymentCoverage")) {
    Aws::Utils::Array<JsonView> deploymentCoverageJsonList = jsonValue.GetArray("deploymentCoverage");
    for (unsigned deploymentCoverageIndex = 0; deploymentCoverageIndex < deploymentCoverageJsonList.GetLength();
         ++deploymentCoverageIndex) {
      m_deploymentCoverage.push_back(deploymentCoverageJsonList[deploymentCoverageIndex].AsObject());
    }
