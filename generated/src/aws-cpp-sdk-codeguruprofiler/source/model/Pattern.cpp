...
Pattern::Pattern(JsonView jsonValue) { *this = jsonValue; }
...
Pattern& Pattern::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("countersToAggregate")) {
    ...
    for (unsigned countersToAggregateIndex = 0; countersToAggregateIndex < countersToAggregateJsonList.GetLength();
         ++countersToAggregateIndex) {
