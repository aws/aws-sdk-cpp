...
namespace ivschat {
template <>
AWS_IVSCHAT_API ConflictException IvschatError::GetModeledError() {
  assert(this->GetErrorType() == IvschatErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_IVSCHAT_API ServiceQuotaExceededException IvschatError::GetModeledError() {
  assert(this->GetErrorType() == IvschatErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_IVSCHAT_API ThrottlingException IvschatError::GetModeledError() {
  assert(this->GetErrorType() == IvschatErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_IVSCHAT_API ResourceNotFoundException IvschatError::GetModeledError() {
  assert(this->GetErrorType() == IvschatErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_IVSCHAT_API ValidationException IvschatError::GetModeledError() {
