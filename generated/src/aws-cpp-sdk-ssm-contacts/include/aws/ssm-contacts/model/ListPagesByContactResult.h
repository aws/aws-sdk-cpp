...
class ListPagesByContactResult {
 public:
  ...
  template <typename NextTokenT = Aws::String>
  ListPagesByContactResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename PagesT = Aws::Vector<Page>>
  ListPagesByContactResult& WithPages(PagesT&& value) {
    SetPages(std::forward<PagesT>(value));
    return *this;
  }
  template <typename PagesT = Page>
  ListPagesByContactResult& AddPages(PagesT&& value) {
    m_pagesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListPagesByContactResult& WithRequestId(RequestIdT&& value) {
