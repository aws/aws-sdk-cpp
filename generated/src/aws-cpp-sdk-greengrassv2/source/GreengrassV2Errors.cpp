...
template <>
AWS_GREENGRASSV2_API ConflictException GreengrassV2Error::GetModeledError() {
  assert(this->GetErrorType() == GreengrassV2Errors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_GREENGRASSV2_API ThrottlingException GreengrassV2Error::GetModeledError() {
  assert(this->GetErrorType() == GreengrassV2Errors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_GREENGRASSV2_API ServiceQuotaExceededException GreengrassV2Error::GetModeledError() {
  assert(this->GetErrorType() == GreengrassV2Errors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_GREENGRASSV2_API InternalServerException GreengrassV2Error::GetModeledError() {
  assert(this->GetErrorType() == GreengrassV2Errors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_GREENGRASSV2_API ResourceNotFoundException GreengrassV2Error::GetModeledError() {
  assert(this->GetErrorType() == GreengrassV2Errors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_GREENGRASSV2_API ValidationException GreengrassV2Error::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(GreengrassV2Errors::INTERNAL_SERVER), RetryableType::RETRYABLE);
