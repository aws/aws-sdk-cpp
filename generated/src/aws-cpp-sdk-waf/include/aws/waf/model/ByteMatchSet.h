...
class ByteMatchSet {
 public:
  ...
  void SetByteMatchTuples(ByteMatchTuplesT&& value) {
    ...
    m_byteMatchTuples = std::forward<ByteMatchTuplesT>(value);
  }
  template <typename ByteMatchTuplesT = Aws::Vector<ByteMatchTuple>>
  ByteMatchSet& WithByteMatchTuples(ByteMatchTuplesT&& value) {
    SetByteMatchTuples(std::forward<ByteMatchTuplesT>(value));
    return *this;
  }
  template <typename ByteMatchTuplesT = ByteMatchTuple>
  ByteMatchSet& AddByteMatchTuples(ByteMatchTuplesT&& value) {
    m_byteMatchTuplesHasBeenSet = true;
    m_byteMatchTuples.emplace_back(std::forward<ByteMatchTuplesT>(value));
    return *this;
  }
  ///@}
 private:
