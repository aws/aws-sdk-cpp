...
template <>
AWS_NOTIFICATIONS_API ConflictException NotificationsError::GetModeledError() {
  assert(this->GetErrorType() == NotificationsErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_NOTIFICATIONS_API ServiceQuotaExceededException NotificationsError::GetModeledError() {
  assert(this->GetErrorType() == NotificationsErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_NOTIFICATIONS_API ThrottlingException NotificationsError::GetModeledError() {
  assert(this->GetErrorType() == NotificationsErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_NOTIFICATIONS_API ResourceNotFoundException NotificationsError::GetModeledError() {
  assert(this->GetErrorType() == NotificationsErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_NOTIFICATIONS_API ValidationException NotificationsError::GetModeledError() {
