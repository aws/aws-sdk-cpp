...
AggregationDetail::AggregationDetail(JsonView jsonValue) { *this = jsonValue; }
...
AggregationDetail& AggregationDetail::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("summarizationDimensions")) {
    ...
    for (unsigned summarizationDimensionsIndex = 0; summarizationDimensionsIndex < summarizationDimensionsJsonList.GetLength();
         ++summarizationDimensionsIndex) {
...
JsonValue AggregationDetail::Jsonize() const {
  ...
  if (m_summarizationDimensionsHasBeenSet) {
    ...
    for (unsigned summarizationDimensionsIndex = 0; summarizationDimensionsIndex < summarizationDimensionsJsonList.GetLength();
         ++summarizationDimensionsIndex) {
      summarizationDimensionsJsonList[summarizationDimensionsIndex].AsObject(
          m_summarizationDimensions[summarizationDimensionsIndex].Jsonize());
    }
