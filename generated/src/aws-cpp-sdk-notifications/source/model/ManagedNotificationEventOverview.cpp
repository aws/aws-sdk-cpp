...
ManagedNotificationEventOverview::ManagedNotificationEventOverview(JsonView jsonValue) { *this = jsonValue; }
...
ManagedNotificationEventOverview& ManagedNotificationEventOverview::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("aggregatedNotificationRegions")) {
    ...
    for (unsigned aggregatedNotificationRegionsIndex = 0;
         aggregatedNotificationRegionsIndex < aggregatedNotificationRegionsJsonList.GetLength(); ++aggregatedNotificationRegionsIndex) {
...
JsonValue ManagedNotificationEventOverview::Jsonize() const {
  ...
  if (m_aggregatedNotificationRegionsHasBeenSet) {
    ...
    for (unsigned aggregatedNotificationRegionsIndex = 0;
         aggregatedNotificationRegionsIndex < aggregatedNotificationRegionsJsonList.GetLength(); ++aggregatedNotificationRegionsIndex) {
      aggregatedNotificationRegionsJsonList[aggregatedNotificationRegionsIndex].AsString(
          m_aggregatedNotificationRegions[aggregatedNotificationRegionsIndex]);
    }
