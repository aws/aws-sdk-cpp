...
AggregationSummary::AggregationSummary(JsonView jsonValue) { *this = jsonValue; }
...
AggregationSummary& AggregationSummary::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("additionalSummarizationDimensions")) {
    ...
    for (unsigned additionalSummarizationDimensionsIndex = 0;
         additionalSummarizationDimensionsIndex < additionalSummarizationDimensionsJsonList.GetLength();
         ++additionalSummarizationDimensionsIndex) {
      m_additionalSummarizationDimensions.push_back(
          additionalSummarizationDimensionsJsonList[additionalSummarizationDimensionsIndex].AsObject());
    }
...
JsonValue AggregationSummary::Jsonize() const {
  ...
  if (m_additionalSummarizationDimensionsHasBeenSet) {
    ...
    for (unsigned additionalSummarizationDimensionsIndex = 0;
         ...
         ++additionalSummarizationDimensionsIndex) {
      additionalSummarizationDimensionsJsonList[additionalSummarizationDimensionsIndex].AsObject(
          m_additionalSummarizationDimensions[additionalSummarizationDimensionsIndex].Jsonize());
    }
