...
class X12Envelope {
 public:
  ...
  void SetCommon(CommonT&& value) {
    ...
    m_common = std::forward<CommonT>(value);
  }
  template <typename CommonT = X12OutboundEdiHeaders>
  X12Envelope& WithCommon(CommonT&& value) {
    SetCommon(std::forward<CommonT>(value));
    return *this;
  }
  ///@}
  ...
  void SetWrapOptions(WrapOptionsT&& value) {
    ...
    m_wrapOptions = std::forward<WrapOptionsT>(value);
  }
  template <typename WrapOptionsT = WrapOptions>
  X12Envelope& WithWrapOptions(WrapOptionsT&& value) {
    SetWrapOptions(std::forward<WrapOptionsT>(value));
    return *this;
  }
  ///@}
 private:
