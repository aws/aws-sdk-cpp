...
class X12ElementRequirementValidationRule {
 public:
  ...
  void SetElementPosition(ElementPositionT&& value) {
    m_elementPositionHasBeenSet = true;
    m_elementPosition = std::forward<ElementPositionT>(value);
  }
  template <typename ElementPositionT = Aws::String>
  X12ElementRequirementValidationRule& WithElementPosition(ElementPositionT&& value) {
    SetElementPosition(std::forward<ElementPositionT>(value));
    return *this;
  }
  ///@}
  ...
  inline void SetRequirement(ElementRequirement value) {
    m_requirementHasBeenSet = true;
    m_requirement = value;
  }
  inline X12ElementRequirementValidationRule& WithRequirement(ElementRequirement value) {
