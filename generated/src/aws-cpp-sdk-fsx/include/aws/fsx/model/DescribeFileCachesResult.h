...
class DescribeFileCachesResult {
 public:
  ...
  void SetFileCaches(FileCachesT&& value) {
    m_fileCachesHasBeenSet = true;
    m_fileCaches = std::forward<FileCachesT>(value);
  }
  template <typename FileCachesT = Aws::Vector<FileCache>>
  DescribeFileCachesResult& WithFileCaches(FileCachesT&& value) {
    SetFileCaches(std::forward<FileCachesT>(value));
    return *this;
  }
  template <typename FileCachesT = FileCache>
  DescribeFileCachesResult& AddFileCaches(FileCachesT&& value) {
    m_fileCachesHasBeenSet = true;
    m_fileCaches.emplace_back(std::forward<FileCachesT>(value));
    return *this;
  }
  ///@}
