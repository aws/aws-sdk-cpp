...
class DisassociateFileSystemAliasesResult {
 public:
  ...
  template <typename AliasesT = Aws::Vector<Alias>>
  DisassociateFileSystemAliasesResult& WithAliases(AliasesT&& value) {
    SetAliases(std::forward<AliasesT>(value));
    return *this;
  }
  template <typename AliasesT = Alias>
  DisassociateFileSystemAliasesResult& AddAliases(AliasesT&& value) {
    m_aliasesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  DisassociateFileSystemAliasesResult& WithRequestId(RequestIdT&& value) {
