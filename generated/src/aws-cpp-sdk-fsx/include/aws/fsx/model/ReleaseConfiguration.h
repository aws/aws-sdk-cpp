...
namespace Model
{
   ...
   * <p>Only file data that has been archived can be released. Files that have not
   * yet been archived, such as new or changed files that have not been exported, are
   * not eligible for release. When files are released, their metadata stays on the
   * file system, so they can still be accessed later. Users and applications can
   * access a released file by reading the file again, which restores data from
   * Amazon S3 to the FSx for Lustre file system.</p>  <p>If a file meets the
   * last accessed time criteria, its file or directory path must also be specified
   * with the <code>Paths</code> parameter of the operation in order for the file to
   * be released.</p> <p><h3>See Also:</h3>   <a
  ...
  public:
    ...
    /**
     * <p>Defines the point-in-time since an archived file was last accessed, in order
