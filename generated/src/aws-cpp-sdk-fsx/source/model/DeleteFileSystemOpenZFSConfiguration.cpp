...
DeleteFileSystemOpenZFSConfiguration::DeleteFileSystemOpenZFSConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
DeleteFileSystemOpenZFSConfiguration& DeleteFileSystemOpenZFSConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("Options")) {
    ...
    for (unsigned optionsIndex = 0; optionsIndex < optionsJsonList.GetLength(); ++optionsIndex) {
      m_options.push_back(
          DeleteFileSystemOpenZFSOptionMapper::GetDeleteFileSystemOpenZFSOptionForName(optionsJsonList[optionsIndex].AsString()));
    }
  ...
  if (m_optionsHasBeenSet) {
    ...
    for (unsigned optionsIndex = 0; optionsIndex < optionsJsonList.GetLength(); ++optionsIndex) {
      optionsJsonList[optionsIndex].AsString(
          DeleteFileSystemOpenZFSOptionMapper::GetNameForDeleteFileSystemOpenZFSOption(m_options[optionsIndex]));
    }
