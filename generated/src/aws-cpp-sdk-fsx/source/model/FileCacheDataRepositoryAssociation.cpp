...
FileCacheDataRepositoryAssociation::FileCacheDataRepositoryAssociation(JsonView jsonValue) { *this = jsonValue; }
...
FileCacheDataRepositoryAssociation& FileCacheDataRepositoryAssociation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("DataRepositorySubdirectories")) {
    ...
    for (unsigned dataRepositorySubdirectoriesIndex = 0;
         dataRepositorySubdirectoriesIndex < dataRepositorySubdirectoriesJsonList.GetLength(); ++dataRepositorySubdirectoriesIndex) {
...
JsonValue FileCacheDataRepositoryAssociation::Jsonize() const {
  ...
  if (m_dataRepositorySubdirectoriesHasBeenSet) {
    ...
    for (unsigned dataRepositorySubdirectoriesIndex = 0;
         dataRepositorySubdirectoriesIndex < dataRepositorySubdirectoriesJsonList.GetLength(); ++dataRepositorySubdirectoriesIndex) {
      dataRepositorySubdirectoriesJsonList[dataRepositorySubdirectoriesIndex].AsString(
          m_dataRepositorySubdirectories[dataRepositorySubdirectoriesIndex]);
    }
