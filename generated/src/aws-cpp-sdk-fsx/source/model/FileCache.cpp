...
FileCache::FileCache(JsonView jsonValue) { *this = jsonValue; }

FileCache& FileCache::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("NetworkInterfaceIds")) {
    ...
    for (unsigned networkInterfaceIdsIndex = 0; networkInterfaceIdsIndex < networkInterfaceIdsJsonList.GetLength();
         ++networkInterfaceIdsIndex) {
  ...
  if (jsonValue.ValueExists("DataRepositoryAssociationIds")) {
    ...
    for (unsigned dataRepositoryAssociationIdsIndex = 0;
         dataRepositoryAssociationIdsIndex < dataRepositoryAssociationIdsJsonList.GetLength(); ++dataRepositoryAssociationIdsIndex) {
...
JsonValue FileCache::Jsonize() const {
  JsonValue payload;
  ...
  if (m_dataRepositoryAssociationIdsHasBeenSet) {
    ...
    for (unsigned dataRepositoryAssociationIdsIndex = 0;
         dataRepositoryAssociationIdsIndex < dataRepositoryAssociationIdsJsonList.GetLength(); ++dataRepositoryAssociationIdsIndex) {
      dataRepositoryAssociationIdsJsonList[dataRepositoryAssociationIdsIndex].AsString(
          m_dataRepositoryAssociationIds[dataRepositoryAssociationIdsIndex]);
    }
