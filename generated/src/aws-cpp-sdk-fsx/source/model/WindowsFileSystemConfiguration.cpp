...
WindowsFileSystemConfiguration::WindowsFileSystemConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
WindowsFileSystemConfiguration& WindowsFileSystemConfiguration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("MaintenanceOperationsInProgress")) {
    ...
    for (unsigned maintenanceOperationsInProgressIndex = 0;
         maintenanceOperationsInProgressIndex < maintenanceOperationsInProgressJsonList.GetLength();
         ++maintenanceOperationsInProgressIndex) {
      m_maintenanceOperationsInProgress.push_back(FileSystemMaintenanceOperationMapper::GetFileSystemMaintenanceOperationForName(
          maintenanceOperationsInProgressJsonList[maintenanceOperationsInProgressIndex].AsString()));
    }
...
JsonValue WindowsFileSystemConfiguration::Jsonize() const {
  ...
  if (m_maintenanceOperationsInProgressHasBeenSet) {
    ...
    for (unsigned maintenanceOperationsInProgressIndex = 0;
         ...
         ++maintenanceOperationsInProgressIndex) {
      maintenanceOperationsInProgressJsonList[maintenanceOperationsInProgressIndex].AsString(
          FileSystemMaintenanceOperationMapper::GetNameForFileSystemMaintenanceOperation(
              m_maintenanceOperationsInProgress[maintenanceOperationsInProgressIndex]));
    }
