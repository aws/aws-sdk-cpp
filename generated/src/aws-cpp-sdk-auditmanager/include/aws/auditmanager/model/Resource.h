...
class Resource {
 public:
   ...
   * as non-compliant if Security Hub CSPM reports a <i>Fail</i> result, or if Config
   * reports a <i>Non-compliant</i> result.</p> </li> <li> <p>Audit Manager classes
   * the resource as compliant if Security Hub CSPM reports a <i>Pass</i> result, or
   * if Config reports a <i>Compliant</i> result.</p> </li> <li> <p>If a compliance
  ...
  void SetComplianceCheck(ComplianceCheckT&& value) {
    ...
    m_complianceCheck = std::forward<ComplianceCheckT>(value);
  }
  template <typename ComplianceCheckT = Aws::String>
  Resource& WithComplianceCheck(ComplianceCheckT&& value) {
    SetComplianceCheck(std::forward<ComplianceCheckT>(value));
    return *this;
  }
  ///@}
 private:
