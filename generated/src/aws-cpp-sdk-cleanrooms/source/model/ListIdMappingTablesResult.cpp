...
ListIdMappingTablesResult::ListIdMappingTablesResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListIdMappingTablesResult& ListIdMappingTablesResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("idMappingTableSummaries")) {
    ...
    for (unsigned idMappingTableSummariesIndex = 0; idMappingTableSummariesIndex < idMappingTableSummariesJsonList.GetLength();
         ++idMappingTableSummariesIndex) {
      m_idMappingTableSummaries.push_back(idMappingTableSummariesJsonList[idMappingTableSummariesIndex].AsObject());
    }
    m_idMappingTableSummariesHasBeenSet = true;
  }
