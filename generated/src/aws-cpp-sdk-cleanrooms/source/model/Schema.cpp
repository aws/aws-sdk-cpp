...
Schema& Schema::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("analysisRuleTypes")) {
    ...
    for (unsigned analysisRuleTypesIndex = 0; analysisRuleTypesIndex < analysisRuleTypesJsonList.GetLength(); ++analysisRuleTypesIndex) {
      m_analysisRuleTypes.push_back(
          AnalysisRuleTypeMapper::GetAnalysisRuleTypeForName(analysisRuleTypesJsonList[analysisRuleTypesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("selectedAnalysisMethods")) {
    ...
    for (unsigned selectedAnalysisMethodsIndex = 0; selectedAnalysisMethodsIndex < selectedAnalysisMethodsJsonList.GetLength();
         ++selectedAnalysisMethodsIndex) {
      m_selectedAnalysisMethods.push_back(SelectedAnalysisMethodMapper::GetSelectedAnalysisMethodForName(
          selectedAnalysisMethodsJsonList[selectedAnalysisMethodsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("schemaStatusDetails")) {
    ...
    for (unsigned schemaStatusDetailsIndex = 0; schemaStatusDetailsIndex < schemaStatusDetailsJsonList.GetLength();
         ++schemaStatusDetailsIndex) {
...
JsonValue Schema::Jsonize() const {
  ...
  if (m_analysisRuleTypesHasBeenSet) {
    ...
    for (unsigned analysisRuleTypesIndex = 0; analysisRuleTypesIndex < analysisRuleTypesJsonList.GetLength(); ++analysisRuleTypesIndex) {
      analysisRuleTypesJsonList[analysisRuleTypesIndex].AsString(
          AnalysisRuleTypeMapper::GetNameForAnalysisRuleType(m_analysisRuleTypes[analysisRuleTypesIndex]));
    }
  ...
  if (m_selectedAnalysisMethodsHasBeenSet) {
    ...
    for (unsigned selectedAnalysisMethodsIndex = 0; selectedAnalysisMethodsIndex < selectedAnalysisMethodsJsonList.GetLength();
         ++selectedAnalysisMethodsIndex) {
      selectedAnalysisMethodsJsonList[selectedAnalysisMethodsIndex].AsString(
          SelectedAnalysisMethodMapper::GetNameForSelectedAnalysisMethod(m_selectedAnalysisMethods[selectedAnalysisMethodsIndex]));
    }
