...
ConfiguredTableSummary::ConfiguredTableSummary(JsonView jsonValue) { *this = jsonValue; }
...
ConfiguredTableSummary& ConfiguredTableSummary::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("analysisRuleTypes")) {
    ...
    for (unsigned analysisRuleTypesIndex = 0; analysisRuleTypesIndex < analysisRuleTypesJsonList.GetLength(); ++analysisRuleTypesIndex) {
      m_analysisRuleTypes.push_back(ConfiguredTableAnalysisRuleTypeMapper::GetConfiguredTableAnalysisRuleTypeForName(
...
JsonValue ConfiguredTableSummary::Jsonize() const {
  ...
  if (m_analysisRuleTypesHasBeenSet) {
    ...
    for (unsigned analysisRuleTypesIndex = 0; analysisRuleTypesIndex < analysisRuleTypesJsonList.GetLength(); ++analysisRuleTypesIndex) {
      analysisRuleTypesJsonList[analysisRuleTypesIndex].AsString(
          ConfiguredTableAnalysisRuleTypeMapper::GetNameForConfiguredTableAnalysisRuleType(m_analysisRuleTypes[analysisRuleTypesIndex]));
    }
