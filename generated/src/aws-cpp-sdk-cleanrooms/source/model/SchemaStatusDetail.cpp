...
SchemaStatusDetail::SchemaStatusDetail(JsonView jsonValue) { *this = jsonValue; }
  ...
  if (jsonValue.ValueExists("configurations")) {
    ...
    for (unsigned configurationsIndex = 0; configurationsIndex < configurationsJsonList.GetLength(); ++configurationsIndex) {
      m_configurations.push_back(
          SchemaConfigurationMapper::GetSchemaConfigurationForName(configurationsJsonList[configurationsIndex].AsString()));
    }
...
JsonValue SchemaStatusDetail::Jsonize() const {
  ...
  if (m_configurationsHasBeenSet) {
    ...
    for (unsigned configurationsIndex = 0; configurationsIndex < configurationsJsonList.GetLength(); ++configurationsIndex) {
      configurationsJsonList[configurationsIndex].AsString(
          SchemaConfigurationMapper::GetNameForSchemaConfiguration(m_configurations[configurationsIndex]));
    }
