...
class AnalysisSchema {
 public:
  ...
  void SetReferencedTables(ReferencedTablesT&& value) {
    m_referencedTablesHasBeenSet = true;
    m_referencedTables = std::forward<ReferencedTablesT>(value);
  }
  template <typename ReferencedTablesT = Aws::Vector<Aws::String>>
  AnalysisSchema& WithReferencedTables(ReferencedTablesT&& value) {
    SetReferencedTables(std::forward<ReferencedTablesT>(value));
    return *this;
  }
  template <typename ReferencedTablesT = Aws::String>
  AnalysisSchema& AddReferencedTables(ReferencedTablesT&& value) {
    m_referencedTablesHasBeenSet = true;
    m_referencedTables.emplace_back(std::forward<ReferencedTablesT>(value));
    return *this;
  }
  ///@}
 private:
