...
class ProtectedQueryDistributeOutput {
 public:
  ...
  template <typename S3T = ProtectedQueryS3Output>
  ProtectedQueryDistributeOutput& WithS3(S3T&& value) {
  ...
  void SetMemberList(MemberListT&& value) {
    m_memberListHasBeenSet = true;
    m_memberList = std::forward<MemberListT>(value);
  }
  template <typename MemberListT = Aws::Vector<ProtectedQuerySingleMemberOutput>>
  ProtectedQueryDistributeOutput& WithMemberList(MemberListT&& value) {
    SetMemberList(std::forward<MemberListT>(value));
    return *this;
  }
  ...
  ProtectedQueryDistributeOutput& AddMemberList(MemberListT&& value) {
    m_memberListHasBeenSet = true;
    m_memberList.emplace_back(std::forward<MemberListT>(value));
    return *this;
  }
  ///@}
 private:
