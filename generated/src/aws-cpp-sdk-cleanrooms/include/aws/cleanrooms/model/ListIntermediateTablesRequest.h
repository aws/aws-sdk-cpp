...
/**
 */
class ListIntermediateTablesRequest : public CleanRoomsRequest {
 public:
  AWS_CLEANROOMS_API ListIntermediateTablesRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListIntermediateTables"; }
  ...
  ///@{
  /**
   * <p>The unique identifier of the membership for which to list intermediate
   * tables.</p>
   */
  ...
  template <typename MembershipIdentifierT = Aws::String>
  ListIntermediateTablesRequest& WithMembershipIdentifier(MembershipIdentifierT&& value) {
  ...
  template <typename NextTokenT = Aws::String>
  ListIntermediateTablesRequest& WithNextToken(NextTokenT&& value) {
  ...
  inline ListIntermediateTablesRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
