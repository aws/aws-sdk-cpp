...
/**
 */
class GetIntermediateTableRequest : public CleanRoomsRequest {
 public:
  AWS_CLEANROOMS_API GetIntermediateTableRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetIntermediateTable"; }
  ...
  template <typename IntermediateTableIdentifierT = Aws::String>
  GetIntermediateTableRequest& WithIntermediateTableIdentifier(IntermediateTableIdentifierT&& value) {
  ...
  template <typename MembershipIdentifierT = Aws::String>
  GetIntermediateTableRequest& WithMembershipIdentifier(MembershipIdentifierT&& value) {
