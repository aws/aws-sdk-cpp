...
class DifferentialPrivacyParameters {
 public:
  ...
  void SetSensitivityParameters(SensitivityParametersT&& value) {
    m_sensitivityParametersHasBeenSet = true;
    m_sensitivityParameters = std::forward<SensitivityParametersT>(value);
  }
  template <typename SensitivityParametersT = Aws::Vector<DifferentialPrivacySensitivityParameters>>
  DifferentialPrivacyParameters& WithSensitivityParameters(SensitivityParametersT&& value) {
    SetSensitivityParameters(std::forward<SensitivityParametersT>(value));
    return *this;
  }
  template <typename SensitivityParametersT = DifferentialPrivacySensitivityParameters>
  DifferentialPrivacyParameters& AddSensitivityParameters(SensitivityParametersT&& value) {
    m_sensitivityParametersHasBeenSet = true;
    m_sensitivityParameters.emplace_back(std::forward<SensitivityParametersT>(value));
    return *this;
  }
  ///@}
 private:
