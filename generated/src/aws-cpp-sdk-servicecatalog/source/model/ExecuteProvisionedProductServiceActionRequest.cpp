...
Aws::String ExecuteProvisionedProductServiceActionRequest::SerializePayload() const {
  ...
  if (m_parametersHasBeenSet) {
    ...
    for (auto& parametersItem : m_parameters) {
      ...
      for (unsigned executionParameterValueListIndex = 0;
           executionParameterValueListIndex < executionParameterValueListJsonList.GetLength(); ++executionParameterValueListIndex) {
        executionParameterValueListJsonList[executionParameterValueListIndex].AsString(
            parametersItem.second[executionParameterValueListIndex]);
      }
