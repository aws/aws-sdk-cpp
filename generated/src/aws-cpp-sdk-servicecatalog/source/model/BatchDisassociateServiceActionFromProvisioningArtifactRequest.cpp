...
Aws::String BatchDisassociateServiceActionFromProvisioningArtifactRequest::SerializePayload() const {
  ...
  if (m_serviceActionAssociationsHasBeenSet) {
    ...
    for (unsigned serviceActionAssociationsIndex = 0; serviceActionAssociationsIndex < serviceActionAssociationsJsonList.GetLength();
         ++serviceActionAssociationsIndex) {
      serviceActionAssociationsJsonList[serviceActionAssociationsIndex].AsObject(
          m_serviceActionAssociations[serviceActionAssociationsIndex].Jsonize());
    }
...
Aws::Http::HeaderValueCollection BatchDisassociateServiceActionFromProvisioningArtifactRequest::GetRequestSpecificHeaders() const {
  ...
  headers.insert(
      Aws::Http::HeaderValuePair("X-Amz-Target", "AWS242ServiceCatalogService.BatchDisassociateServiceActionFromProvisioningArtifact"));
  return headers;
}
