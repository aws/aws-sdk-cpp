...
Aws::String SearchProvisionedProductsRequest::SerializePayload() const {
  ...
  if (m_filtersHasBeenSet) {
    ...
    for (auto& filtersItem : m_filters) {
      ...
      for (unsigned provisionedProductViewFilterValuesIndex = 0;
           provisionedProductViewFilterValuesIndex < provisionedProductViewFilterValuesJsonList.GetLength();
           ++provisionedProductViewFilterValuesIndex) {
        provisionedProductViewFilterValuesJsonList[provisionedProductViewFilterValuesIndex].AsString(
            filtersItem.second[provisionedProductViewFilterValuesIndex]);
      }
      filtersJsonMap.WithArray(ProvisionedProductViewFilterByMapper::GetNameForProvisionedProductViewFilterBy(filtersItem.first),
                               std::move(provisionedProductViewFilterValuesJsonList));
    }
