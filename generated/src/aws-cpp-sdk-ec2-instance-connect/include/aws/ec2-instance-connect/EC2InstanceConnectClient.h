...
class AWS_EC2INSTANCECONNECT_API EC2InstanceConnectClient : public Aws::Client::AWSJsonClient,
 ...
 public:
   ...
   */
  EC2InstanceConnectClient(const Aws::EC2InstanceConnect::EC2InstanceConnectClientConfiguration& clientConfiguration =
  ...
  EC2InstanceConnectClient(const Aws::Auth::AWSCredentials& credentials,
                           std::shared_ptr<EC2InstanceConnectEndpointProviderBase> endpointProvider = nullptr,
                           const Aws::EC2InstanceConnect::EC2InstanceConnectClientConfiguration& clientConfiguration =
                               Aws::EC2InstanceConnect::EC2InstanceConnectClientConfiguration());
  ...
  /**
   * A Callable wrapper for SendSSHPublicKey that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for SendSSHPublicKey that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename SendSSHPublicKeyRequestT = Model::SendSSHPublicKeyRequest>
  void SendSSHPublicKeyAsync(const SendSSHPublicKeyRequestT& request, const SendSSHPublicKeyResponseReceivedHandler& handler,
   ...
   * API Reference</a></p>
   */
  virtual Model::SendSerialConsoleSSHPublicKeyOutcome SendSerialConsoleSSHPublicKey(
      const Model::SendSerialConsoleSSHPublicKeyRequest& request) const;

  /**
   * A Callable wrapper for SendSerialConsoleSSHPublicKey that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename SendSerialConsoleSSHPublicKeyRequestT = Model::SendSerialConsoleSSHPublicKeyRequest>
  Model::SendSerialConsoleSSHPublicKeyOutcomeCallable SendSerialConsoleSSHPublicKeyCallable(
      const SendSerialConsoleSSHPublicKeyRequestT& request) const {
    return SubmitCallable(&EC2InstanceConnectClient::SendSerialConsoleSSHPublicKey, request);
  }

  /**
   * An Async wrapper for SendSerialConsoleSSHPublicKey that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename SendSerialConsoleSSHPublicKeyRequestT = Model::SendSerialConsoleSSHPublicKeyRequest>
  void SendSerialConsoleSSHPublicKeyAsync(const SendSerialConsoleSSHPublicKeyRequestT& request,
                                          const SendSerialConsoleSSHPublicKeyResponseReceivedHandler& handler,
                                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
