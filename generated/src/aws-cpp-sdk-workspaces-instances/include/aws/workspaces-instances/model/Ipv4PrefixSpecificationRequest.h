...
class Ipv4PrefixSpecificationRequest {
 public:
  ...
  AWS_WORKSPACESINSTANCES_API Ipv4PrefixSpecificationRequest(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_WORKSPACESINSTANCES_API Ipv4PrefixSpecificationRequest& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  void SetIpv4Prefix(Ipv4PrefixT&& value) {
    ...
    m_ipv4Prefix = std::forward<Ipv4PrefixT>(value);
  }
  template <typename Ipv4PrefixT = Aws::String>
  Ipv4PrefixSpecificationRequest& WithIpv4Prefix(Ipv4PrefixT&& value) {
    SetIpv4Prefix(std::forward<Ipv4PrefixT>(value));
    return *this;
  }
  ///@}
 private:
