...
WhenSentTo::WhenSentTo(JsonView jsonValue) { *this = jsonValue; }
...
WhenSentTo& WhenSentTo::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("WhenSentToSegmentsList")) {
    ...
    for (unsigned whenSentToSegmentsListIndex = 0; whenSentToSegmentsListIndex < whenSentToSegmentsListJsonList.GetLength();
         ++whenSentToSegmentsListIndex) {
