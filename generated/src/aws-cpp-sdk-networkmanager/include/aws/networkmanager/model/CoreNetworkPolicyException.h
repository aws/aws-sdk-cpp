...
class CoreNetworkPolicyException {
 public:
  ...
  template <typename MessageT = Aws::String>
  CoreNetworkPolicyException& WithMessage(MessageT&& value) {
  ...
  template <typename ErrorsT = Aws::Vector<CoreNetworkPolicyError>>
  CoreNetworkPolicyException& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = CoreNetworkPolicyError>
  CoreNetworkPolicyException& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
