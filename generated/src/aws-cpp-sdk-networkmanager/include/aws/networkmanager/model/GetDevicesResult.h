...
class GetDevicesResult {
 public:
  ...
  template <typename DevicesT = Aws::Vector<Device>>
  GetDevicesResult& WithDevices(DevicesT&& value) {
    SetDevices(std::forward<DevicesT>(value));
    return *this;
  }
  template <typename DevicesT = Device>
  GetDevicesResult& AddDevices(DevicesT&& value) {
    m_devicesHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  GetDevicesResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  GetDevicesResult& WithRequestId(RequestIdT&& value) {
