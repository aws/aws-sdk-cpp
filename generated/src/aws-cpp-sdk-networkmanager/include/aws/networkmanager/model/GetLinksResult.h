...
class GetLinksResult {
 public:
  ...
  template <typename LinksT = Aws::Vector<Link>>
  GetLinksResult& WithLinks(LinksT&& value) {
    SetLinks(std::forward<LinksT>(value));
    return *this;
  }
  template <typename LinksT = Link>
  GetLinksResult& AddLinks(LinksT&& value) {
    m_linksHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  GetLinksResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  GetLinksResult& WithRequestId(RequestIdT&& value) {
