...
class WhenSentTo {
 public:
  ...
  void SetWhenSentToSegmentsList(WhenSentToSegmentsListT&& value) {
    ...
    m_whenSentToSegmentsList = std::forward<WhenSentToSegmentsListT>(value);
  }
  template <typename WhenSentToSegmentsListT = Aws::Vector<Aws::String>>
  WhenSentTo& WithWhenSentToSegmentsList(WhenSentToSegmentsListT&& value) {
    SetWhenSentToSegmentsList(std::forward<WhenSentToSegmentsListT>(value));
    return *this;
  }
  template <typename WhenSentToSegmentsListT = Aws::String>
  WhenSentTo& AddWhenSentToSegmentsList(WhenSentToSegmentsListT&& value) {
    m_whenSentToSegmentsListHasBeenSet = true;
    m_whenSentToSegmentsList.emplace_back(std::forward<WhenSentToSegmentsListT>(value));
    return *this;
  }
  ///@}
 private:
