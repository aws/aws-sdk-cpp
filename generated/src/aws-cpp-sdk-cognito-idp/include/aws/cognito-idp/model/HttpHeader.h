...
class HttpHeader {
 public:
  ...
  template <typename HeaderNameT = Aws::String>
  HttpHeader& WithHeaderName(HeaderNameT&& value) {
  ...
  void SetHeaderValue(HeaderValueT&& value) {
    ...
    m_headerValue = std::forward<HeaderValueT>(value);
  }
  template <typename HeaderValueT = Aws::String>
  HttpHeader& WithHeaderValue(HeaderValueT&& value) {
    SetHeaderValue(std::forward<HeaderValueT>(value));
    return *this;
  }
  ///@}
 private:
