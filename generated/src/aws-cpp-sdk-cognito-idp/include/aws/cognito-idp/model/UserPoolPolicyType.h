...
class UserPoolPolicyType {
 public:
  ...
  template <typename PasswordPolicyT = PasswordPolicyType>
  UserPoolPolicyType& WithPasswordPolicy(PasswordPolicyT&& value) {
  ...
  void SetSignInPolicy(SignInPolicyT&& value) {
    m_signInPolicyHasBeenSet = true;
    m_signInPolicy = std::forward<SignInPolicyT>(value);
  }
  template <typename SignInPolicyT = SignInPolicyType>
  UserPoolPolicyType& WithSignInPolicy(SignInPolicyT&& value) {
    SetSignInPolicy(std::forward<SignInPolicyT>(value));
    return *this;
  }
  ///@}
 private:
