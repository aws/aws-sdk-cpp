...
class DeviceSecretVerifierConfigType {
 public:
  ...
  void SetPasswordVerifier(PasswordVerifierT&& value) {
    m_passwordVerifierHasBeenSet = true;
    m_passwordVerifier = std::forward<PasswordVerifierT>(value);
  }
  template <typename PasswordVerifierT = Aws::String>
  DeviceSecretVerifierConfigType& WithPasswordVerifier(PasswordVerifierT&& value) {
    SetPasswordVerifier(std::forward<PasswordVerifierT>(value));
    return *this;
  }
  ///@}
  ...
  void SetSalt(SaltT&& value) {
    m_saltHasBeenSet = true;
    m_salt = std::forward<SaltT>(value);
  }
  template <typename SaltT = Aws::String>
  DeviceSecretVerifierConfigType& WithSalt(SaltT&& value) {
    SetSalt(std::forward<SaltT>(value));
    return *this;
  }
  ///@}
 private:
