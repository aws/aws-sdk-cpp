...
class ListUserPoolsResult {
 public:
  ...
  void SetUserPools(UserPoolsT&& value) {
    m_userPoolsHasBeenSet = true;
    m_userPools = std::forward<UserPoolsT>(value);
  }
  template <typename UserPoolsT = Aws::Vector<UserPoolDescriptionType>>
  ListUserPoolsResult& WithUserPools(UserPoolsT&& value) {
    SetUserPools(std::forward<UserPoolsT>(value));
    return *this;
  }
  template <typename UserPoolsT = UserPoolDescriptionType>
  ListUserPoolsResult& AddUserPools(UserPoolsT&& value) {
    m_userPoolsHasBeenSet = true;
    m_userPools.emplace_back(std::forward<UserPoolsT>(value));
    return *this;
  }
  ///@}
