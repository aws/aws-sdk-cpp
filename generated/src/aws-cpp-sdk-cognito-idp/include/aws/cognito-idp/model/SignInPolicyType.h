...
class SignInPolicyType {
 public:
  ...
  void SetAllowedFirstAuthFactors(AllowedFirstAuthFactorsT&& value) {
    m_allowedFirstAuthFactorsHasBeenSet = true;
    m_allowedFirstAuthFactors = std::forward<AllowedFirstAuthFactorsT>(value);
  }
  template <typename AllowedFirstAuthFactorsT = Aws::Vector<AuthFactorType>>
  SignInPolicyType& WithAllowedFirstAuthFactors(AllowedFirstAuthFactorsT&& value) {
    SetAllowedFirstAuthFactors(std::forward<AllowedFirstAuthFactorsT>(value));
    return *this;
  }
  inline SignInPolicyType& AddAllowedFirstAuthFactors(AuthFactorType value) {
    m_allowedFirstAuthFactorsHasBeenSet = true;
    m_allowedFirstAuthFactors.push_back(value);
    return *this;
  }
  ///@}
 private:
