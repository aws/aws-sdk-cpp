...
/**
 * <p>The request to delete a specific client secret from a user pool app
 * client.</p><p><h3>See Also:</h3>   <a
 * href="http://docs.aws.amazon.com/goto/WebAPI/cognito-idp-2016-04-18/DeleteUserPoolClientSecretRequest">AWS
 * API Reference</a></p>
 */
class DeleteUserPoolClientSecretRequest : public CognitoIdentityProviderRequest {
 public:
  AWS_COGNITOIDENTITYPROVIDER_API DeleteUserPoolClientSecretRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteUserPoolClientSecret"; }
  ...
  template <typename UserPoolIdT = Aws::String>
  DeleteUserPoolClientSecretRequest& WithUserPoolId(UserPoolIdT&& value) {
  ...
  template <typename ClientIdT = Aws::String>
  DeleteUserPoolClientSecretRequest& WithClientId(ClientIdT&& value) {
  ...
  template <typename ClientSecretIdT = Aws::String>
  DeleteUserPoolClientSecretRequest& WithClientSecretId(ClientSecretIdT&& value) {
