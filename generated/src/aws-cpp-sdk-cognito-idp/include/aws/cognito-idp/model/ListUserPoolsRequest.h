...
class ListUserPoolsRequest : public CognitoIdentityProviderRequest {
 public:
  AWS_COGNITOIDENTITYPROVIDER_API ListUserPoolsRequest() = default;
  ...
  template <typename NextTokenT = Aws::String>
  ListUserPoolsRequest& WithNextToken(NextTokenT&& value) {
  ...
  ///@{
  /**
   * <p>The maximum number of user pools that you want Amazon Cognito to return in
   * the response.</p>
   */
  ...
  inline ListUserPoolsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
