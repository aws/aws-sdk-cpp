...
AuthEventType::AuthEventType(JsonView jsonValue) { *this = jsonValue; }
...
AuthEventType& AuthEventType::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ChallengeResponses")) {
    ...
    for (unsigned challengeResponsesIndex = 0; challengeResponsesIndex < challengeResponsesJsonList.GetLength();
         ++challengeResponsesIndex) {
