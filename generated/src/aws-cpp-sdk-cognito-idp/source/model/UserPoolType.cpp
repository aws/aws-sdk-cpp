...
UserPoolType::UserPoolType(JsonView jsonValue) { *this = jsonValue; }
...
UserPoolType& UserPoolType::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AutoVerifiedAttributes")) {
    ...
    for (unsigned autoVerifiedAttributesIndex = 0; autoVerifiedAttributesIndex < autoVerifiedAttributesJsonList.GetLength();
      ...
      m_autoVerifiedAttributes.push_back(VerifiedAttributeTypeMapper::GetVerifiedAttributeTypeForName(
  ...
  if (jsonValue.ValueExists("AliasAttributes")) {
    ...
    for (unsigned aliasAttributesIndex = 0; aliasAttributesIndex < aliasAttributesJsonList.GetLength(); ++aliasAttributesIndex) {
      m_aliasAttributes.push_back(
          AliasAttributeTypeMapper::GetAliasAttributeTypeForName(aliasAttributesJsonList[aliasAttributesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("UsernameAttributes")) {
    ...
    for (unsigned usernameAttributesIndex = 0; usernameAttributesIndex < usernameAttributesJsonList.GetLength();
         ++usernameAttributesIndex) {
      m_usernameAttributes.push_back(
          UsernameAttributeTypeMapper::GetUsernameAttributeTypeForName(usernameAttributesJsonList[usernameAttributesIndex].AsString()));
    }
...
JsonValue UserPoolType::Jsonize() const {
  ...
  if (m_aliasAttributesHasBeenSet) {
    ...
    for (unsigned aliasAttributesIndex = 0; aliasAttributesIndex < aliasAttributesJsonList.GetLength(); ++aliasAttributesIndex) {
      aliasAttributesJsonList[aliasAttributesIndex].AsString(
          AliasAttributeTypeMapper::GetNameForAliasAttributeType(m_aliasAttributes[aliasAttributesIndex]));
    }
  ...
  if (m_usernameAttributesHasBeenSet) {
    ...
    for (unsigned usernameAttributesIndex = 0; usernameAttributesIndex < usernameAttributesJsonList.GetLength();
         ++usernameAttributesIndex) {
      usernameAttributesJsonList[usernameAttributesIndex].AsString(
          UsernameAttributeTypeMapper::GetNameForUsernameAttributeType(m_usernameAttributes[usernameAttributesIndex]));
    }
