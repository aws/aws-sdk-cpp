...
Aws::String UpdateRoutingControlStatesRequest::SerializePayload() const {
  ...
  if (m_updateRoutingControlStateEntriesHasBeenSet) {
    ...
    for (unsigned updateRoutingControlStateEntriesIndex = 0;
         updateRoutingControlStateEntriesIndex < updateRoutingControlStateEntriesJsonList.GetLength();
         ++updateRoutingControlStateEntriesIndex) {
      updateRoutingControlStateEntriesJsonList[updateRoutingControlStateEntriesIndex].AsObject(
          m_updateRoutingControlStateEntries[updateRoutingControlStateEntriesIndex].Jsonize());
    }
  ...
  if (m_safetyRulesToOverrideHasBeenSet) {
    ...
    for (unsigned safetyRulesToOverrideIndex = 0; safetyRulesToOverrideIndex < safetyRulesToOverrideJsonList.GetLength();
         ++safetyRulesToOverrideIndex) {
