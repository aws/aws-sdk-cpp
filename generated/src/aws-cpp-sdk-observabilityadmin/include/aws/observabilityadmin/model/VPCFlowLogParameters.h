...
class VPCFlowLogParameters {
 public:
  ...
  template <typename LogFormatT = Aws::String>
  VPCFlowLogParameters& WithLogFormat(LogFormatT&& value) {
  ...
  void SetTrafficType(TrafficTypeT&& value) {
    m_trafficTypeHasBeenSet = true;
    m_trafficType = std::forward<TrafficTypeT>(value);
  }
  template <typename TrafficTypeT = Aws::String>
  VPCFlowLogParameters& WithTrafficType(TrafficTypeT&& value) {
    SetTrafficType(std::forward<TrafficTypeT>(value));
    return *this;
  }
  ///@}
  ...
  inline VPCFlowLogParameters& WithMaxAggregationInterval(int value) {
