...
DescribeInstanceHealthResult::DescribeInstanceHealthResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
DescribeInstanceHealthResult& DescribeInstanceHealthResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!instanceStatesNode.IsNull()) {
      ...
      m_instanceStatesHasBeenSet = !instanceStatesMember.IsNull();
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::ElasticLoadBalancing::Model::DescribeInstanceHealthResult",
