...
AccessLog::AccessLog(const XmlNode& xmlNode) { *this = xmlNode; }
...
AccessLog& AccessLog::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!emitIntervalNode.IsNull()) {
      m_emitInterval =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(emitIntervalNode.GetText()).c_str()).c_str());
