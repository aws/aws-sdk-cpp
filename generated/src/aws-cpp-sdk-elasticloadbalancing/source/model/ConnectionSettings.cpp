...
ConnectionSettings::ConnectionSettings(const XmlNode& xmlNode) { *this = xmlNode; }
...
ConnectionSettings& ConnectionSettings::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!idleTimeoutNode.IsNull()) {
      m_idleTimeout =
          StringUtils::ConvertToInt32(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(idleTimeoutNode.GetText()).c_str()).c_str());
