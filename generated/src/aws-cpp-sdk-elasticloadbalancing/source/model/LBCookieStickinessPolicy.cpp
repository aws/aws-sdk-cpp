...
LBCookieStickinessPolicy::LBCookieStickinessPolicy(const XmlNode& xmlNode) { *this = xmlNode; }
...
LBCookieStickinessPolicy& LBCookieStickinessPolicy::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!cookieExpirationPeriodNode.IsNull()) {
      m_cookieExpirationPeriod = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(cookieExpirationPeriodNode.GetText()).c_str()).c_str());
...
void LBCookieStickinessPolicy::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
