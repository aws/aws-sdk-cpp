...
IntOptions::IntOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
IntOptions& IntOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!defaultValueNode.IsNull()) {
      m_defaultValue =
          StringUtils::ConvertToInt64(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(defaultValueNode.GetText()).c_str()).c_str());
