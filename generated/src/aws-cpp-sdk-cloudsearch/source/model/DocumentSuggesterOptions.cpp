...
DocumentSuggesterOptions::DocumentSuggesterOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
DocumentSuggesterOptions& DocumentSuggesterOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!fuzzyMatchingNode.IsNull()) {
      m_fuzzyMatching = SuggesterFuzzyMatchingMapper::GetSuggesterFuzzyMatchingForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(fuzzyMatchingNode.GetText()).c_str()));
...
void DocumentSuggesterOptions::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_fuzzyMatchingHasBeenSet) {
    oStream << location << index << locationValue
            << ".FuzzyMatching=" << StringUtils::URLEncode(SuggesterFuzzyMatchingMapper::GetNameForSuggesterFuzzyMatching(m_fuzzyMatching))
            << "&";
  }
