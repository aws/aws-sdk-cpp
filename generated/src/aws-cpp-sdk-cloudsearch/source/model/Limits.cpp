...
Limits::Limits(const XmlNode& xmlNode) { *this = xmlNode; }
...
Limits& Limits::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!maximumReplicationCountNode.IsNull()) {
      m_maximumReplicationCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maximumReplicationCountNode.GetText()).c_str()).c_str());
    ...
    if (!maximumPartitionCountNode.IsNull()) {
      m_maximumPartitionCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maximumPartitionCountNode.GetText()).c_str()).c_str());
