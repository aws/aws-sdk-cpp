...
TextOptions::TextOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
TextOptions& TextOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!highlightEnabledNode.IsNull()) {
      m_highlightEnabled = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(highlightEnabledNode.GetText()).c_str()).c_str());
