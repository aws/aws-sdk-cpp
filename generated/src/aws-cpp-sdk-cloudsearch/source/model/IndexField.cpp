...
IndexField::IndexField(const XmlNode& xmlNode) { *this = xmlNode; }
...
IndexField& IndexField::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!indexFieldTypeNode.IsNull()) {
      m_indexFieldType = IndexFieldTypeMapper::GetIndexFieldTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(indexFieldTypeNode.GetText()).c_str()));
...
void IndexField::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_indexFieldTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".IndexFieldType=" << StringUtils::URLEncode(IndexFieldTypeMapper::GetNameForIndexFieldType(m_indexFieldType)) << "&";
  }
