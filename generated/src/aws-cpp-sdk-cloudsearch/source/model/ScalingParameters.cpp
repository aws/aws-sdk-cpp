...
ScalingParameters::ScalingParameters(const XmlNode& xmlNode) { *this = xmlNode; }
...
ScalingParameters& ScalingParameters::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!desiredInstanceTypeNode.IsNull()) {
      m_desiredInstanceType = PartitionInstanceTypeMapper::GetPartitionInstanceTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(desiredInstanceTypeNode.GetText()).c_str()));
    ...
    if (!desiredReplicationCountNode.IsNull()) {
      m_desiredReplicationCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(desiredReplicationCountNode.GetText()).c_str()).c_str());
    ...
    if (!desiredPartitionCountNode.IsNull()) {
      m_desiredPartitionCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(desiredPartitionCountNode.GetText()).c_str()).c_str());
...
void ScalingParameters::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_desiredInstanceTypeHasBeenSet) {
    oStream << location << index << locationValue << ".DesiredInstanceType="
            << StringUtils::URLEncode(PartitionInstanceTypeMapper::GetNameForPartitionInstanceType(m_desiredInstanceType)) << "&";
  }
...
void ScalingParameters::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_desiredInstanceTypeHasBeenSet) {
    oStream << location << ".DesiredInstanceType="
            << StringUtils::URLEncode(PartitionInstanceTypeMapper::GetNameForPartitionInstanceType(m_desiredInstanceType)) << "&";
  }
  if (m_desiredReplicationCountHasBeenSet) {
