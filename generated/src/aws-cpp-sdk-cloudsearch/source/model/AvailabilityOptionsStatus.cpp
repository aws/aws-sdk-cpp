...
AvailabilityOptionsStatus::AvailabilityOptionsStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
AvailabilityOptionsStatus& AvailabilityOptionsStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!optionsNode.IsNull()) {
      m_options =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(optionsNode.GetText()).c_str()).c_str());
...
void AvailabilityOptionsStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
