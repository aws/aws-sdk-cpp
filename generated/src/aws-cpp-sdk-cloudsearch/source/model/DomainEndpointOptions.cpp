...
DomainEndpointOptions::DomainEndpointOptions(const XmlNode& xmlNode) { *this = xmlNode; }
...
DomainEndpointOptions& DomainEndpointOptions::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!enforceHTTPSNode.IsNull()) {
      m_enforceHTTPS =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(enforceHTTPSNode.GetText()).c_str()).c_str());
    ...
    if (!tLSSecurityPolicyNode.IsNull()) {
      m_tLSSecurityPolicy = TLSSecurityPolicyMapper::GetTLSSecurityPolicyForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(tLSSecurityPolicyNode.GetText()).c_str()));
...
void DomainEndpointOptions::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_tLSSecurityPolicyHasBeenSet) {
    oStream << location << index << locationValue
            << ".TLSSecurityPolicy=" << StringUtils::URLEncode(TLSSecurityPolicyMapper::GetNameForTLSSecurityPolicy(m_tLSSecurityPolicy))
            << "&";
  }
}
