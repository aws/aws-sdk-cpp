...
class GetCredentialsResult {
 public:
  ...
  void SetDbPassword(DbPasswordT&& value) {
    ...
    m_dbPassword = std::forward<DbPasswordT>(value);
  }
  template <typename DbPasswordT = Aws::String>
  GetCredentialsResult& WithDbPassword(DbPasswordT&& value) {
    SetDbPassword(std::forward<DbPasswordT>(value));
    return *this;
  }
  ///@}
  ...
  void SetNextRefreshTime(NextRefreshTimeT&& value) {
    m_nextRefreshTimeHasBeenSet = true;
    m_nextRefreshTime = std::forward<NextRefreshTimeT>(value);
  }
  template <typename NextRefreshTimeT = Aws::Utils::DateTime>
  GetCredentialsResult& WithNextRefreshTime(NextRefreshTimeT&& value) {
    SetNextRefreshTime(std::forward<NextRefreshTimeT>(value));
    return *this;
  }
  ///@}
