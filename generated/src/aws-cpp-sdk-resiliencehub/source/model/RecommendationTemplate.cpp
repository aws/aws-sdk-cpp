...
RecommendationTemplate::RecommendationTemplate(JsonView jsonValue) { *this = jsonValue; }
...
RecommendationTemplate& RecommendationTemplate::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("recommendationTypes")) {
    ...
    for (unsigned recommendationTypesIndex = 0; recommendationTypesIndex < recommendationTypesJsonList.GetLength();
         ++recommendationTypesIndex) {
      m_recommendationTypes.push_back(RenderRecommendationTypeMapper::GetRenderRecommendationTypeForName(
          recommendationTypesJsonList[recommendationTypesIndex].AsString()));
    }
...
JsonValue RecommendationTemplate::Jsonize() const {
  ...
  if (m_recommendationTypesHasBeenSet) {
    ...
    for (unsigned recommendationTypesIndex = 0; recommendationTypesIndex < recommendationTypesJsonList.GetLength();
         ++recommendationTypesIndex) {
      recommendationTypesJsonList[recommendationTypesIndex].AsString(
          RenderRecommendationTypeMapper::GetNameForRenderRecommendationType(m_recommendationTypes[recommendationTypesIndex]));
    }
