...
ComponentRecommendation::ComponentRecommendation(JsonView jsonValue) { *this = jsonValue; }
...
ComponentRecommendation& ComponentRecommendation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("configRecommendations")) {
    ...
    for (unsigned configRecommendationsIndex = 0; configRecommendationsIndex < configRecommendationsJsonList.GetLength();
         ++configRecommendationsIndex) {
  ...
  if (jsonValue.ValueExists("recommendationStatus")) {
    m_recommendationStatus =
        RecommendationComplianceStatusMapper::GetRecommendationComplianceStatusForName(jsonValue.GetString("recommendationStatus"));
...
JsonValue ComponentRecommendation::Jsonize() const {
  ...
  if (m_recommendationStatusHasBeenSet) {
    payload.WithString("recommendationStatus",
                       RecommendationComplianceStatusMapper::GetNameForRecommendationComplianceStatus(m_recommendationStatus));
  }
