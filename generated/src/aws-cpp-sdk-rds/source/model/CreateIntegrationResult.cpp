...
CreateIntegrationResult& CreateIntegrationResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "CreateIntegrationResult")) {
    resultNode = rootNode.FirstChild("CreateIntegrationResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::RDS::Model::CreateIntegrationResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId());
  }
  return *this;
}
