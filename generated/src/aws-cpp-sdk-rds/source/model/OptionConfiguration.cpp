...
OptionConfiguration::OptionConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
...
void OptionConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_dBSecurityGroupMembershipsHasBeenSet) {
    ...
    for (auto& item : m_dBSecurityGroupMemberships) {
      oStream << location << index << locationValue << ".DBSecurityGroupMemberships.DBSecurityGroupName." << dBSecurityGroupMembershipsIdx++
  ...
  if (m_vpcSecurityGroupMembershipsHasBeenSet) {
    ...
    for (auto& item : m_vpcSecurityGroupMemberships) {
      oStream << location << index << locationValue << ".VpcSecurityGroupMemberships.VpcSecurityGroupId."
              << vpcSecurityGroupMembershipsIdx++ << "=" << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
...
void OptionConfiguration::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_dBSecurityGroupMembershipsHasBeenSet) {
    unsigned dBSecurityGroupMembershipsIdx = 1;
    for (auto& item : m_dBSecurityGroupMemberships) {
      oStream << location << ".DBSecurityGroupMemberships.DBSecurityGroupName." << dBSecurityGroupMembershipsIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
  if (m_vpcSecurityGroupMembershipsHasBeenSet) {
    unsigned vpcSecurityGroupMembershipsIdx = 1;
    for (auto& item : m_vpcSecurityGroupMemberships) {
      oStream << location << ".VpcSecurityGroupMemberships.VpcSecurityGroupId." << vpcSecurityGroupMembershipsIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
  if (m_optionSettingsHasBeenSet) {
