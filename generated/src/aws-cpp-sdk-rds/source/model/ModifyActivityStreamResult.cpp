...
ModifyActivityStreamResult::ModifyActivityStreamResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
ModifyActivityStreamResult& ModifyActivityStreamResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!policyStatusNode.IsNull()) {
      m_policyStatus = ActivityStreamPolicyStatusMapper::GetActivityStreamPolicyStatusForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(policyStatusNode.GetText()).c_str()));
