...
CreateDBShardGroupResult::CreateDBShardGroupResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }

CreateDBShardGroupResult& CreateDBShardGroupResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "CreateDBShardGroupResult")) {
    resultNode = rootNode.FirstChild("CreateDBShardGroupResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::RDS::Model::CreateDBShardGroupResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId());
  }
  return *this;
}
