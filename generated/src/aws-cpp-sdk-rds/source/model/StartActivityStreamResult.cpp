...
StartActivityStreamResult::StartActivityStreamResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
StartActivityStreamResult& StartActivityStreamResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = ActivityStreamStatusMapper::GetActivityStreamStatusForName(
    ...
    if (!modeNode.IsNull()) {
      m_mode = ActivityStreamModeMapper::GetActivityStreamModeForName(
    ...
    if (!engineNativeAuditFieldsIncludedNode.IsNull()) {
      m_engineNativeAuditFieldsIncluded = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(engineNativeAuditFieldsIncludedNode.GetText()).c_str()).c_str());
    ...
    if (!applyImmediatelyNode.IsNull()) {
      m_applyImmediately = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(applyImmediatelyNode.GetText()).c_str()).c_str());
