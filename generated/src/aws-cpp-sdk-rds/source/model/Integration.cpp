...
Integration::Integration(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!additionalEncryptionContextNode.IsNull()) {
      ...
      while (!additionalEncryptionContextEntry.IsNull()) {
        ...
        m_additionalEncryptionContext[keyNode.GetText()] = valueNode.GetText();
...
void Integration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_additionalEncryptionContextHasBeenSet) {
    ...
    for (auto& item : m_additionalEncryptionContext) {
      oStream << location << index << locationValue << ".AdditionalEncryptionContext.entry." << additionalEncryptionContextIdx
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(IntegrationStatusMapper::GetNameForIntegrationStatus(m_status)) << "&";
  }
...
void Integration::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_additionalEncryptionContextHasBeenSet) {
    unsigned additionalEncryptionContextIdx = 1;
    for (auto& item : m_additionalEncryptionContext) {
      oStream << location << ".AdditionalEncryptionContext.entry." << additionalEncryptionContextIdx
              << ".key=" << StringUtils::URLEncode(item.first.c_str()) << "&";
      oStream << location << ".AdditionalEncryptionContext.entry." << additionalEncryptionContextIdx
              << ".value=" << StringUtils::URLEncode(item.second.c_str()) << "&";
      additionalEncryptionContextIdx++;
