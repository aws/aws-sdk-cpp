...
ServerlessV2ScalingConfigurationInfo::ServerlessV2ScalingConfigurationInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
ServerlessV2ScalingConfigurationInfo& ServerlessV2ScalingConfigurationInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!secondsUntilAutoPauseNode.IsNull()) {
      m_secondsUntilAutoPause = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(secondsUntilAutoPauseNode.GetText()).c_str()).c_str());
...
void ServerlessV2ScalingConfigurationInfo::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
