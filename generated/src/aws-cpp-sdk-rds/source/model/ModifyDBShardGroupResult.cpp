...
ModifyDBShardGroupResult::ModifyDBShardGroupResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }

ModifyDBShardGroupResult& ModifyDBShardGroupResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "ModifyDBShardGroupResult")) {
    resultNode = rootNode.FirstChild("ModifyDBShardGroupResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::RDS::Model::ModifyDBShardGroupResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId());
  }
  return *this;
}
