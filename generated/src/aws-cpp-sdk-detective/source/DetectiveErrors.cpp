...
namespace Detective {
template <>
AWS_DETECTIVE_API ServiceQuotaExceededException DetectiveError::GetModeledError() {
  assert(this->GetErrorType() == DetectiveErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_DETECTIVE_API ValidationException DetectiveError::GetModeledError() {
  assert(this->GetErrorType() == DetectiveErrors::VALIDATION);
  return ValidationException(this->GetJsonPayload().View());
}

template <>
AWS_DETECTIVE_API AccessDeniedException DetectiveError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(DetectiveErrors::CONFLICT), RetryableType::NOT_RETRYABLE);
  } else if (hashCode == SERVICE_QUOTA_EXCEEDED_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(DetectiveErrors::SERVICE_QUOTA_EXCEEDED), RetryableType::NOT_RETRYABLE);
