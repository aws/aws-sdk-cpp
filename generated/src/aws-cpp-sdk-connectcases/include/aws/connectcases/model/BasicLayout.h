...
class BasicLayout {
 public:
  ...
  void SetTopPanel(TopPanelT&& value) {
    ...
    m_topPanel = std::forward<TopPanelT>(value);
  }
  template <typename TopPanelT = LayoutSections>
  BasicLayout& WithTopPanel(TopPanelT&& value) {
    SetTopPanel(std::forward<TopPanelT>(value));
    return *this;
  }
  ///@}
  ...
  void SetMoreInfo(MoreInfoT&& value) {
    ...
    m_moreInfo = std::forward<MoreInfoT>(value);
  }
  template <typename MoreInfoT = LayoutSections>
  BasicLayout& WithMoreInfo(MoreInfoT&& value) {
    SetMoreInfo(std::forward<MoreInfoT>(value));
    return *this;
  }
  ///@}
 private:
