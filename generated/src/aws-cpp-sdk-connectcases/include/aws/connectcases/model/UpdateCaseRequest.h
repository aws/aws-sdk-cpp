...
class UpdateCaseRequest : public ConnectCasesRequest {
 public:
  ...
  template <typename DomainIdT = Aws::String>
  UpdateCaseRequest& WithDomainId(DomainIdT&& value) {
  ...
  template <typename FieldsT = Aws::Vector<FieldValue>>
  UpdateCaseRequest& WithFields(FieldsT&& value) {
    SetFields(std::forward<FieldsT>(value));
    return *this;
  }
  template <typename FieldsT = FieldValue>
  UpdateCaseRequest& AddFields(FieldsT&& value) {
    m_fieldsHasBeenSet = true;
  ...
  template <typename PerformedByT = UserUnion>
  UpdateCaseRequest& WithPerformedBy(PerformedByT&& value) {
