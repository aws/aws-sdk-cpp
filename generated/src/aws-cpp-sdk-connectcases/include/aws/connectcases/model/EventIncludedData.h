...
class EventIncludedData {
 public:
  ...
  void SetCaseData(CaseDataT&& value) {
    ...
    m_caseData = std::forward<CaseDataT>(value);
  }
  template <typename CaseDataT = CaseEventIncludedData>
  EventIncludedData& WithCaseData(CaseDataT&& value) {
    SetCaseData(std::forward<CaseDataT>(value));
    return *this;
  }
  ///@}
  ...
  void SetRelatedItemData(RelatedItemDataT&& value) {
    ...
    m_relatedItemData = std::forward<RelatedItemDataT>(value);
  }
  template <typename RelatedItemDataT = RelatedItemEventIncludedData>
  EventIncludedData& WithRelatedItemData(RelatedItemDataT&& value) {
    SetRelatedItemData(std::forward<RelatedItemDataT>(value));
    return *this;
  }
  ///@}
 private:
