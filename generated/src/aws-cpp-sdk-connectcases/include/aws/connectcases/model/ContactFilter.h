...
class ContactFilter {
 public:
  ...
  template <typename ChannelT = Aws::Vector<Aws::String>>
  ContactFilter& WithChannel(ChannelT&& value) {
    SetChannel(std::forward<ChannelT>(value));
    return *this;
  }
  template <typename ChannelT = Aws::String>
  ContactFilter& AddChannel(ChannelT&& value) {
    m_channelHasBeenSet = true;
    m_channel.emplace_back(std::forward<ChannelT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename ContactArnT = Aws::String>
  ContactFilter& WithContactArn(ContactArnT&& value) {
