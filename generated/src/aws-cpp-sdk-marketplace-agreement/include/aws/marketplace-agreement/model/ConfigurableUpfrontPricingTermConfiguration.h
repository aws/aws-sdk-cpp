...
class ConfigurableUpfrontPricingTermConfiguration {
 public:
  ...
  template <typename DimensionsT = Aws::Vector<Dimension>>
  ConfigurableUpfrontPricingTermConfiguration& WithDimensions(DimensionsT&& value) {
    SetDimensions(std::forward<DimensionsT>(value));
    return *this;
  }
  template <typename DimensionsT = Dimension>
  ConfigurableUpfrontPricingTermConfiguration& AddDimensions(DimensionsT&& value) {
    m_dimensionsHasBeenSet = true;
