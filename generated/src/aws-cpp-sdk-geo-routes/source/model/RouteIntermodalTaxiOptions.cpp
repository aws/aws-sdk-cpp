...
RouteIntermodalTaxiOptions::RouteIntermodalTaxiOptions(JsonView jsonValue) { *this = jsonValue; }

RouteIntermodalTaxiOptions& RouteIntermodalTaxiOptions::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("AllowedModes")) {
    ...
    for (unsigned allowedModesIndex = 0; allowedModesIndex < allowedModesJsonList.GetLength(); ++allowedModesIndex) {
      m_allowedModes.push_back(RouteTaxiModeMapper::GetRouteTaxiModeForName(allowedModesJsonList[allowedModesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("ExcludedModes")) {
    ...
    for (unsigned excludedModesIndex = 0; excludedModesIndex < excludedModesJsonList.GetLength(); ++excludedModesIndex) {
      m_excludedModes.push_back(RouteTaxiModeMapper::GetRouteTaxiModeForName(excludedModesJsonList[excludedModesIndex].AsString()));
    }
...
JsonValue RouteIntermodalTaxiOptions::Jsonize() const {
  JsonValue payload;

  if (m_allowedModesHasBeenSet) {
    Aws::Utils::Array<JsonValue> allowedModesJsonList(m_allowedModes.size());
    for (unsigned allowedModesIndex = 0; allowedModesIndex < allowedModesJsonList.GetLength(); ++allowedModesIndex) {
      allowedModesJsonList[allowedModesIndex].AsString(RouteTaxiModeMapper::GetNameForRouteTaxiMode(m_allowedModes[allowedModesIndex]));
    }
  ...
  if (m_excludedModesHasBeenSet) {
    ...
    for (unsigned excludedModesIndex = 0; excludedModesIndex < excludedModesJsonList.GetLength(); ++excludedModesIndex) {
      excludedModesJsonList[excludedModesIndex].AsString(RouteTaxiModeMapper::GetNameForRouteTaxiMode(m_excludedModes[excludedModesIndex]));
    }
