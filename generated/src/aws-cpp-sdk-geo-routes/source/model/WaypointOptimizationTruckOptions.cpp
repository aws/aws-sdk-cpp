...
WaypointOptimizationTruckOptions::WaypointOptimizationTruckOptions(JsonView jsonValue) { *this = jsonValue; }
...
WaypointOptimizationTruckOptions& WaypointOptimizationTruckOptions::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("HazardousCargos")) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      m_hazardousCargos.push_back(WaypointOptimizationHazardousCargoTypeMapper::GetWaypointOptimizationHazardousCargoTypeForName(
...
JsonValue WaypointOptimizationTruckOptions::Jsonize() const {
  ...
  if (m_hazardousCargosHasBeenSet) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      hazardousCargosJsonList[hazardousCargosIndex].AsString(
          WaypointOptimizationHazardousCargoTypeMapper::GetNameForWaypointOptimizationHazardousCargoType(
              m_hazardousCargos[hazardousCargosIndex]));
    }
