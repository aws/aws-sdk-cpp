...
RoutePedestrianSpan::RoutePedestrianSpan(JsonView jsonValue) { *this = jsonValue; }
...
RoutePedestrianSpan& RoutePedestrianSpan::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("PedestrianAccess")) {
    ...
    for (unsigned pedestrianAccessIndex = 0; pedestrianAccessIndex < pedestrianAccessJsonList.GetLength(); ++pedestrianAccessIndex) {
      m_pedestrianAccess.push_back(RouteSpanPedestrianAccessAttributeMapper::GetRouteSpanPedestrianAccessAttributeForName(
          pedestrianAccessJsonList[pedestrianAccessIndex].AsString()));
    }
...
JsonValue RoutePedestrianSpan::Jsonize() const {
  ...
  if (m_pedestrianAccessHasBeenSet) {
    ...
    for (unsigned pedestrianAccessIndex = 0; pedestrianAccessIndex < pedestrianAccessJsonList.GetLength(); ++pedestrianAccessIndex) {
      pedestrianAccessJsonList[pedestrianAccessIndex].AsString(
          RouteSpanPedestrianAccessAttributeMapper::GetNameForRouteSpanPedestrianAccessAttribute(
              m_pedestrianAccess[pedestrianAccessIndex]));
    }
