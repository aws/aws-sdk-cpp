...
RoadSnapTruckOptions::RoadSnapTruckOptions(JsonView jsonValue) { *this = jsonValue; }
  ...
  if (jsonValue.ValueExists("HazardousCargos")) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      m_hazardousCargos.push_back(
          RoadSnapHazardousCargoTypeMapper::GetRoadSnapHazardousCargoTypeForName(hazardousCargosJsonList[hazardousCargosIndex].AsString()));
    }
  ...
  if (m_hazardousCargosHasBeenSet) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      hazardousCargosJsonList[hazardousCargosIndex].AsString(
          RoadSnapHazardousCargoTypeMapper::GetNameForRoadSnapHazardousCargoType(m_hazardousCargos[hazardousCargosIndex]));
    }
