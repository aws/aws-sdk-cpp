...
IsolineTruckOptions::IsolineTruckOptions(JsonView jsonValue) { *this = jsonValue; }

IsolineTruckOptions& IsolineTruckOptions::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("HazardousCargos")) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      m_hazardousCargos.push_back(
          IsolineHazardousCargoTypeMapper::GetIsolineHazardousCargoTypeForName(hazardousCargosJsonList[hazardousCargosIndex].AsString()));
    }
...
JsonValue IsolineTruckOptions::Jsonize() const {
  JsonValue payload;
  ...
  if (m_hazardousCargosHasBeenSet) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      hazardousCargosJsonList[hazardousCargosIndex].AsString(
          IsolineHazardousCargoTypeMapper::GetNameForIsolineHazardousCargoType(m_hazardousCargos[hazardousCargosIndex]));
    }
  ...
  if (m_truckTypeHasBeenSet) {
    payload.WithString("TruckType", IsolineTruckTypeMapper::GetNameForIsolineTruckType(m_truckType));
  }
