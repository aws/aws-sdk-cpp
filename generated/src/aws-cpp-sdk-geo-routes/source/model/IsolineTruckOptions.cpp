...
IsolineTruckOptions& IsolineTruckOptions::operator =(JsonView jsonValue)
{
  ...
  if(jsonValue.ValueExists("HazardousCargos"))
  {
    ...
    {
      m_hazardousCargos.push_back(IsolineHazardousCargoTypeMapper::GetIsolineHazardousCargoTypeForName(hazardousCargosJsonList[hazardousCargosIndex].AsString()));
    }
...
JsonValue IsolineTruckOptions::Jsonize() const
{
  ...
  if(m_hazardousCargosHasBeenSet)
  {
   ...
   for(unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex)
   {
     hazardousCargosJsonList[hazardousCargosIndex].AsString(IsolineHazardousCargoTypeMapper::GetNameForIsolineHazardousCargoType(m_hazardousCargos[hazardousCargosIndex]));
   }
