...
RouteIntermodalRentalOptions::RouteIntermodalRentalOptions(JsonView jsonValue) { *this = jsonValue; }

RouteIntermodalRentalOptions& RouteIntermodalRentalOptions::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("AllowedModes")) {
    ...
    for (unsigned allowedModesIndex = 0; allowedModesIndex < allowedModesJsonList.GetLength(); ++allowedModesIndex) {
      m_allowedModes.push_back(RouteRentalModeMapper::GetRouteRentalModeForName(allowedModesJsonList[allowedModesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("ExcludedModes")) {
    ...
    for (unsigned excludedModesIndex = 0; excludedModesIndex < excludedModesJsonList.GetLength(); ++excludedModesIndex) {
      m_excludedModes.push_back(RouteRentalModeMapper::GetRouteRentalModeForName(excludedModesJsonList[excludedModesIndex].AsString()));
    }
...
JsonValue RouteIntermodalRentalOptions::Jsonize() const {
  JsonValue payload;

  if (m_allowedModesHasBeenSet) {
    Aws::Utils::Array<JsonValue> allowedModesJsonList(m_allowedModes.size());
    for (unsigned allowedModesIndex = 0; allowedModesIndex < allowedModesJsonList.GetLength(); ++allowedModesIndex) {
      allowedModesJsonList[allowedModesIndex].AsString(RouteRentalModeMapper::GetNameForRouteRentalMode(m_allowedModes[allowedModesIndex]));
    }
  ...
  if (m_excludedModesHasBeenSet) {
    ...
    for (unsigned excludedModesIndex = 0; excludedModesIndex < excludedModesJsonList.GetLength(); ++excludedModesIndex) {
      excludedModesJsonList[excludedModesIndex].AsString(
          RouteRentalModeMapper::GetNameForRouteRentalMode(m_excludedModes[excludedModesIndex]));
    }
