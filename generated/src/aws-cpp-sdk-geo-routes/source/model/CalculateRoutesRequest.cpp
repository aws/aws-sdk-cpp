...
Aws::String CalculateRoutesRequest::SerializePayload() const {
  ...
  if (m_instructionsMeasurementSystemHasBeenSet) {
    payload.WithString("InstructionsMeasurementSystem",
                       MeasurementSystemMapper::GetNameForMeasurementSystem(m_instructionsMeasurementSystem));
  }
  ...
  if (m_legAdditionalFeaturesHasBeenSet) {
    ...
    for (unsigned legAdditionalFeaturesIndex = 0; legAdditionalFeaturesIndex < legAdditionalFeaturesJsonList.GetLength();
         ++legAdditionalFeaturesIndex) {
      legAdditionalFeaturesJsonList[legAdditionalFeaturesIndex].AsString(
          RouteLegAdditionalFeatureMapper::GetNameForRouteLegAdditionalFeature(m_legAdditionalFeatures[legAdditionalFeaturesIndex]));
    }
  ...
  if (m_spanAdditionalFeaturesHasBeenSet) {
    ...
    for (unsigned spanAdditionalFeaturesIndex = 0; spanAdditionalFeaturesIndex < spanAdditionalFeaturesJsonList.GetLength();
         ++spanAdditionalFeaturesIndex) {
      spanAdditionalFeaturesJsonList[spanAdditionalFeaturesIndex].AsString(
          RouteSpanAdditionalFeatureMapper::GetNameForRouteSpanAdditionalFeature(m_spanAdditionalFeatures[spanAdditionalFeaturesIndex]));
    }
