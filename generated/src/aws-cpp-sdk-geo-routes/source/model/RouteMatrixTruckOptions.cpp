...
RouteMatrixTruckOptions::RouteMatrixTruckOptions(JsonView jsonValue) { *this = jsonValue; }
...
RouteMatrixTruckOptions& RouteMatrixTruckOptions::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("HazardousCargos")) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      m_hazardousCargos.push_back(RouteMatrixHazardousCargoTypeMapper::GetRouteMatrixHazardousCargoTypeForName(
          hazardousCargosJsonList[hazardousCargosIndex].AsString()));
    }
...
JsonValue RouteMatrixTruckOptions::Jsonize() const {
  ...
  if (m_hazardousCargosHasBeenSet) {
    ...
    for (unsigned hazardousCargosIndex = 0; hazardousCargosIndex < hazardousCargosJsonList.GetLength(); ++hazardousCargosIndex) {
      hazardousCargosJsonList[hazardousCargosIndex].AsString(
          RouteMatrixHazardousCargoTypeMapper::GetNameForRouteMatrixHazardousCargoType(m_hazardousCargos[hazardousCargosIndex]));
    }
