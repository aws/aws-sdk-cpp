...
RouteTollRate::RouteTollRate(JsonView jsonValue) { *this = jsonValue; }
...
RouteTollRate& RouteTollRate::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("PaymentMethods")) {
    ...
    for (unsigned paymentMethodsIndex = 0; paymentMethodsIndex < paymentMethodsJsonList.GetLength(); ++paymentMethodsIndex) {
      m_paymentMethods.push_back(
          RouteTollPaymentMethodMapper::GetRouteTollPaymentMethodForName(paymentMethodsJsonList[paymentMethodsIndex].AsString()));
    }
...
JsonValue RouteTollRate::Jsonize() const {
  ...
  if (m_paymentMethodsHasBeenSet) {
    ...
    for (unsigned paymentMethodsIndex = 0; paymentMethodsIndex < paymentMethodsJsonList.GetLength(); ++paymentMethodsIndex) {
      paymentMethodsJsonList[paymentMethodsIndex].AsString(
          RouteTollPaymentMethodMapper::GetNameForRouteTollPaymentMethod(m_paymentMethods[paymentMethodsIndex]));
    }
