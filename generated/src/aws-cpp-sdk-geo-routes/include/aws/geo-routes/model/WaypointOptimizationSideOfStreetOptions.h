...
class WaypointOptimizationSideOfStreetOptions {
 public:
  ...
  template <typename PositionT = Aws::Vector<double>>
  WaypointOptimizationSideOfStreetOptions& WithPosition(PositionT&& value) {
    SetPosition(std::forward<PositionT>(value));
    return *this;
  }
  inline WaypointOptimizationSideOfStreetOptions& AddPosition(double value) {
    m_positionHasBeenSet = true;
  ...
  inline WaypointOptimizationSideOfStreetOptions& WithUseWith(SideOfStreetMatchingStrategy value) {
    SetUseWith(value);
    return *this;
  }
  ///@}
