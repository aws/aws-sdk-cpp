...
class RecommendationData {
 public:
  ...
  void SetRdsEngine(RdsEngineT&& value) {
    m_rdsEngineHasBeenSet = true;
    m_rdsEngine = std::forward<RdsEngineT>(value);
  }
  template <typename RdsEngineT = RdsRecommendation>
  RecommendationData& WithRdsEngine(RdsEngineT&& value) {
    SetRdsEngine(std::forward<RdsEngineT>(value));
    return *this;
  }
  ///@}
 private:
