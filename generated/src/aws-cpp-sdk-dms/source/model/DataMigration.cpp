...
DataMigration::DataMigration(JsonView jsonValue) { *this = jsonValue; }
...
DataMigration& DataMigration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("DataMigrationCidrBlocks")) {
    ...
    for (unsigned dataMigrationCidrBlocksIndex = 0; dataMigrationCidrBlocksIndex < dataMigrationCidrBlocksJsonList.GetLength();
         ++dataMigrationCidrBlocksIndex) {
