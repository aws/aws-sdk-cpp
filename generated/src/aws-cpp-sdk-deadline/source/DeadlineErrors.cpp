...
namespace deadline {
template <>
AWS_DEADLINE_API ConflictException DeadlineError::GetModeledError() {
  assert(this->GetErrorType() == DeadlineErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_DEADLINE_API ThrottlingException DeadlineError::GetModeledError() {
  assert(this->GetErrorType() == DeadlineErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_DEADLINE_API ServiceQuotaExceededException DeadlineError::GetModeledError() {
  assert(this->GetErrorType() == DeadlineErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_DEADLINE_API ResourceNotFoundException DeadlineError::GetModeledError() {
  assert(this->GetErrorType() == DeadlineErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_DEADLINE_API ValidationException DeadlineError::GetModeledError() {
  assert(this->GetErrorType() == DeadlineErrors::VALIDATION);
  return ValidationException(this->GetJsonPayload().View());
}

template <>
AWS_DEADLINE_API AccessDeniedException DeadlineError::GetModeledError() {
  assert(this->GetErrorType() == DeadlineErrors::ACCESS_DENIED);
  return AccessDeniedException(this->GetJsonPayload().View());
}

template <>
AWS_DEADLINE_API InternalServerErrorException DeadlineError::GetModeledError() {
