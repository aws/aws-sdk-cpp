...
BatchGetJobItem::BatchGetJobItem(JsonView jsonValue) { *this = jsonValue; }

BatchGetJobItem& BatchGetJobItem::operator=(JsonView jsonValue) {
...
JsonValue BatchGetJobItem::Jsonize() const {
  JsonValue payload;
  ...
  if (m_taskRunStatusCountsHasBeenSet) {
    ...
    for (auto& taskRunStatusCountsItem : m_taskRunStatusCounts) {
      taskRunStatusCountsJsonMap.WithInteger(TaskRunStatusMapper::GetNameForTaskRunStatus(taskRunStatusCountsItem.first),
                                             taskRunStatusCountsItem.second);
    }
