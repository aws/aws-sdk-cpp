...
Aws::String UpdateQueueRequest::SerializePayload() const {
  ...
  if (m_requiredFileSystemLocationNamesToAddHasBeenSet) {
    ...
    for (unsigned requiredFileSystemLocationNamesToAddIndex = 0;
         requiredFileSystemLocationNamesToAddIndex < requiredFileSystemLocationNamesToAddJsonList.GetLength();
         ++requiredFileSystemLocationNamesToAddIndex) {
      requiredFileSystemLocationNamesToAddJsonList[requiredFileSystemLocationNamesToAddIndex].AsString(
          m_requiredFileSystemLocationNamesToAdd[requiredFileSystemLocationNamesToAddIndex]);
    }
  ...
  if (m_requiredFileSystemLocationNamesToRemoveHasBeenSet) {
    ...
    for (unsigned requiredFileSystemLocationNamesToRemoveIndex = 0;
         requiredFileSystemLocationNamesToRemoveIndex < requiredFileSystemLocationNamesToRemoveJsonList.GetLength();
         ++requiredFileSystemLocationNamesToRemoveIndex) {
      requiredFileSystemLocationNamesToRemoveJsonList[requiredFileSystemLocationNamesToRemoveIndex].AsString(
          m_requiredFileSystemLocationNamesToRemove[requiredFileSystemLocationNamesToRemoveIndex]);
    }
  ...
  if (m_allowedStorageProfileIdsToAddHasBeenSet) {
    ...
    for (unsigned allowedStorageProfileIdsToAddIndex = 0;
         allowedStorageProfileIdsToAddIndex < allowedStorageProfileIdsToAddJsonList.GetLength(); ++allowedStorageProfileIdsToAddIndex) {
      allowedStorageProfileIdsToAddJsonList[allowedStorageProfileIdsToAddIndex].AsString(
          m_allowedStorageProfileIdsToAdd[allowedStorageProfileIdsToAddIndex]);
    }
  ...
  if (m_allowedStorageProfileIdsToRemoveHasBeenSet) {
    ...
    for (unsigned allowedStorageProfileIdsToRemoveIndex = 0;
         allowedStorageProfileIdsToRemoveIndex < allowedStorageProfileIdsToRemoveJsonList.GetLength();
         ++allowedStorageProfileIdsToRemoveIndex) {
      allowedStorageProfileIdsToRemoveJsonList[allowedStorageProfileIdsToRemoveIndex].AsString(
          m_allowedStorageProfileIdsToRemove[allowedStorageProfileIdsToRemoveIndex]);
    }
