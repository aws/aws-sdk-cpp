 ...
 * Also:</h3>   <a
 * href="http://docs.aws.amazon.com/goto/WebAPI/deadline-2023-10-12/ListQueueMembersResponse">AWS
 * API Reference</a></p>
 */
class ListQueueMembersResult {
 public:
  ...
  template <typename MembersT = Aws::Vector<QueueMember>>
  ListQueueMembersResult& WithMembers(MembersT&& value) {
    SetMembers(std::forward<MembersT>(value));
    return *this;
  }
  template <typename MembersT = QueueMember>
  ListQueueMembersResult& AddMembers(MembersT&& value) {
    m_membersHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListQueueMembersResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListQueueMembersResult& WithRequestId(RequestIdT&& value) {
