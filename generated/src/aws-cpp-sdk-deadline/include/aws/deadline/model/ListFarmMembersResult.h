 ...
 * Also:</h3>   <a
 * href="http://docs.aws.amazon.com/goto/WebAPI/deadline-2023-10-12/ListFarmMembersResponse">AWS
 * API Reference</a></p>
 */
class ListFarmMembersResult {
 public:
  ...
  template <typename MembersT = Aws::Vector<FarmMember>>
  ListFarmMembersResult& WithMembers(MembersT&& value) {
    SetMembers(std::forward<MembersT>(value));
    return *this;
  }
  template <typename MembersT = FarmMember>
  ListFarmMembersResult& AddMembers(MembersT&& value) {
    m_membersHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListFarmMembersResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListFarmMembersResult& WithRequestId(RequestIdT&& value) {
