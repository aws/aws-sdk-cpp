...
/**
 */
class GetVolumeRequest : public DeadlineRequest {
 public:
  AWS_DEADLINE_API GetVolumeRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetVolume"; }
  ...
  template <typename FarmIdT = Aws::String>
  GetVolumeRequest& WithFarmId(FarmIdT&& value) {
  ...
  template <typename FleetIdT = Aws::String>
  GetVolumeRequest& WithFleetId(FleetIdT&& value) {
  ...
  template <typename VolumeIdT = Aws::String>
  GetVolumeRequest& WithVolumeId(VolumeIdT&& value) {
