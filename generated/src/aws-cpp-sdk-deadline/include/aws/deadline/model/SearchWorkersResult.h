 ...
 * href="http://docs.aws.amazon.com/goto/WebAPI/deadline-2023-10-12/SearchWorkersResponse">AWS
 * API Reference</a></p>
 */
class SearchWorkersResult {
 public:
  ...
  template <typename WorkersT = Aws::Vector<WorkerSearchSummary>>
  SearchWorkersResult& WithWorkers(WorkersT&& value) {
    SetWorkers(std::forward<WorkersT>(value));
    return *this;
  }
  template <typename WorkersT = WorkerSearchSummary>
  SearchWorkersResult& AddWorkers(WorkersT&& value) {
    m_workersHasBeenSet = true;
  ...
  inline SearchWorkersResult& WithNextItemOffset(int value) {
  ...
  inline SearchWorkersResult& WithTotalResults(int value) {
  ...
  template <typename RequestIdT = Aws::String>
  SearchWorkersResult& WithRequestId(RequestIdT&& value) {
