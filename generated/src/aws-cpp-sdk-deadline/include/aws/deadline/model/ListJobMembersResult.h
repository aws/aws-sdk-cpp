 ...
 * Also:</h3>   <a
 * href="http://docs.aws.amazon.com/goto/WebAPI/deadline-2023-10-12/ListJobMembersResponse">AWS
 * API Reference</a></p>
 */
class ListJobMembersResult {
 public:
  ...
  template <typename MembersT = Aws::Vector<JobMember>>
  ListJobMembersResult& WithMembers(MembersT&& value) {
    SetMembers(std::forward<MembersT>(value));
    return *this;
  }
  template <typename MembersT = JobMember>
  ListJobMembersResult& AddMembers(MembersT&& value) {
    m_membersHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListJobMembersResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListJobMembersResult& WithRequestId(RequestIdT&& value) {
