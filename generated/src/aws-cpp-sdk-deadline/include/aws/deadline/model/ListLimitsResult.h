 ...
 * Also:</h3>   <a
 * href="http://docs.aws.amazon.com/goto/WebAPI/deadline-2023-10-12/ListLimitsResponse">AWS
 * API Reference</a></p>
 */
class ListLimitsResult {
 public:
  ...
  template <typename LimitsT = Aws::Vector<LimitSummary>>
  ListLimitsResult& WithLimits(LimitsT&& value) {
    SetLimits(std::forward<LimitsT>(value));
    return *this;
  }
  template <typename LimitsT = LimitSummary>
  ListLimitsResult& AddLimits(LimitsT&& value) {
    m_limitsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListLimitsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListLimitsResult& WithRequestId(RequestIdT&& value) {
