...
class BatchGetJobEntityResult {
 public:
  ...
  template <typename EntitiesT = Aws::Vector<JobEntity>>
  BatchGetJobEntityResult& WithEntities(EntitiesT&& value) {
    SetEntities(std::forward<EntitiesT>(value));
    return *this;
  }
  template <typename EntitiesT = JobEntity>
  BatchGetJobEntityResult& AddEntities(EntitiesT&& value) {
    m_entitiesHasBeenSet = true;
  ...
  template <typename ErrorsT = Aws::Vector<GetJobEntityError>>
  BatchGetJobEntityResult& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = GetJobEntityError>
  BatchGetJobEntityResult& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchGetJobEntityResult& WithRequestId(RequestIdT&& value) {
