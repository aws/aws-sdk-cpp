...
class CostAndUsageComparison {
 public:
  ...
  void SetCostAndUsageSelector(CostAndUsageSelectorT&& value) {
    m_costAndUsageSelectorHasBeenSet = true;
    m_costAndUsageSelector = std::forward<CostAndUsageSelectorT>(value);
  }
  template <typename CostAndUsageSelectorT = Expression>
  CostAndUsageComparison& WithCostAndUsageSelector(CostAndUsageSelectorT&& value) {
    SetCostAndUsageSelector(std::forward<CostAndUsageSelectorT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename MetricsT = Aws::Map<Aws::String, ComparisonMetricValue>>
  CostAndUsageComparison& WithMetrics(MetricsT&& value) {
