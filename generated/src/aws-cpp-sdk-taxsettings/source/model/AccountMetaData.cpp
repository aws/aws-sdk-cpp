...
AccountMetaData::AccountMetaData(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue AccountMetaData::Jsonize() const {
  ...
  if (m_addressRoleMapHasBeenSet) {
    ...
    for (auto& addressRoleMapItem : m_addressRoleMap) {
      addressRoleMapJsonMap.WithObject(AddressRoleTypeMapper::GetNameForAddressRoleType(addressRoleMapItem.first),
