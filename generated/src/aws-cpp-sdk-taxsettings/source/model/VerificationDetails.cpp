...
VerificationDetails& VerificationDetails::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("taxRegistrationDocuments")) {
    ...
    for (unsigned taxRegistrationDocumentsIndex = 0; taxRegistrationDocumentsIndex < taxRegistrationDocumentsJsonList.GetLength();
         ++taxRegistrationDocumentsIndex) {
...
JsonValue VerificationDetails::Jsonize() const {
  ...
  if (m_taxRegistrationDocumentsHasBeenSet) {
    ...
    for (unsigned taxRegistrationDocumentsIndex = 0; taxRegistrationDocumentsIndex < taxRegistrationDocumentsJsonList.GetLength();
         ++taxRegistrationDocumentsIndex) {
      taxRegistrationDocumentsJsonList[taxRegistrationDocumentsIndex].AsObject(
          m_taxRegistrationDocuments[taxRegistrationDocumentsIndex].Jsonize());
    }
