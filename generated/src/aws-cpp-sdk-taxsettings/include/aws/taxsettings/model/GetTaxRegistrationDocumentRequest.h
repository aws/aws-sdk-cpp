...
class GetTaxRegistrationDocumentRequest : public TaxSettingsRequest {
 public:
  ...
  void SetTaxDocumentMetadata(TaxDocumentMetadataT&& value) {
    m_taxDocumentMetadataHasBeenSet = true;
    m_taxDocumentMetadata = std::forward<TaxDocumentMetadataT>(value);
  }
  template <typename TaxDocumentMetadataT = TaxDocumentMetadata>
  GetTaxRegistrationDocumentRequest& WithTaxDocumentMetadata(TaxDocumentMetadataT&& value) {
    SetTaxDocumentMetadata(std::forward<TaxDocumentMetadataT>(value));
    return *this;
  }
  ///@}
 private:
