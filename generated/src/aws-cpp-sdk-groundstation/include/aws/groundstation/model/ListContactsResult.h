...
class ListContactsResult {
 public:
  ...
  void SetContactList(ContactListT&& value) {
    m_contactListHasBeenSet = true;
    m_contactList = std::forward<ContactListT>(value);
  }
  template <typename ContactListT = Aws::Vector<ContactData>>
  ListContactsResult& WithContactList(ContactListT&& value) {
    SetContactList(std::forward<ContactListT>(value));
    return *this;
  }
  template <typename ContactListT = ContactData>
  ListContactsResult& AddContactList(ContactListT&& value) {
    m_contactListHasBeenSet = true;
    m_contactList.emplace_back(std::forward<ContactListT>(value));
    return *this;
  }
  ///@}
