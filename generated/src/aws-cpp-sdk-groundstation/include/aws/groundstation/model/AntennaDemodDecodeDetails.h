...
class AntennaDemodDecodeDetails {
 public:
  ...
  void SetOutputNode(OutputNodeT&& value) {
    m_outputNodeHasBeenSet = true;
    m_outputNode = std::forward<OutputNodeT>(value);
  }
  template <typename OutputNodeT = Aws::String>
  AntennaDemodDecodeDetails& WithOutputNode(OutputNodeT&& value) {
    SetOutputNode(std::forward<OutputNodeT>(value));
    return *this;
  }
  ///@}
 private:
