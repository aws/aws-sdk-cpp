...
class DescribeDeviceEc2InstancesResult {
 public:
  ...
  AWS_SNOWDEVICEMANAGEMENT_API DescribeDeviceEc2InstancesResult& operator=(
  ...
  template <typename InstancesT = Aws::Vector<InstanceSummary>>
  DescribeDeviceEc2InstancesResult& WithInstances(InstancesT&& value) {
    SetInstances(std::forward<InstancesT>(value));
    return *this;
  }
  template <typename InstancesT = InstanceSummary>
  DescribeDeviceEc2InstancesResult& AddInstances(InstancesT&& value) {
    m_instancesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  DescribeDeviceEc2InstancesResult& WithRequestId(RequestIdT&& value) {
