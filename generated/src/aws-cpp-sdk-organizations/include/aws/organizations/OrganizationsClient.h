 ...
 * the Organizations API. For more information about using this service, see the <a
 * href="https://docs.aws.amazon.com/organizations/latest/userguide/orgs_introduction.html">Organizations
 * User Guide</a>.</p> <p> <b>API version</b> </p> <p>This version of the
 * Organizations API Reference documents the Organizations API version
 * 2016-11-28.</p>  <p>As an alternative to using the API directly, you can
 * use one of the Amazon Web Services SDKs, which consist of libraries and sample
 * code for various programming languages and platforms (Java, Ruby, .NET, iOS,
 * Android, and more). The SDKs provide a convenient way to create programmatic
 * access to Organizations. For example, the SDKs take care of cryptographically
 * signing requests, managing errors, and retrying requests automatically. For more
 ...
 * Services</a>.</p>  <p>We recommend that you use the Amazon Web Services
 * SDKs to make programmatic API calls to Organizations. However, you also can use
 * the Organizations Query API to make direct calls to the Organizations web
 * service. To learn more about the Organizations Query API, see <a
 * href="https://docs.aws.amazon.com/organizations/latest/userguide/orgs_query-requests.html">Calling
 * the API by making HTTP Query requests</a> in the <i>Organizations User
 * Guide</i>. Organizations supports GET and POST requests for all actions. That
 * is, the API doesn't require you to use GET for some actions and POST for others.
 * However, GET requests are subject to the limitation size of a URL. Therefore,
 * for operations that require larger sizes, use a POST request.</p> <p> <b>Signing
 * requests</b> </p> <p>When you send HTTP requests to Amazon Web Services, sign
 * the requests so that Amazon Web Services can identify who sent them. You sign
 * requests with your Amazon Web Services access key, which consists of an access
 * key ID and a secret access key. We strongly recommend that you don't create an
 * access key for your root account. Anyone who has the access key for your root
 * account has unrestricted access to all the resources in your account. Instead,
 * create an access key for an IAM user that has administrative permissions. As
 * another option, use Amazon Web Services Security Token Service (Amazon Web
 * Services STS) to generate temporary security credentials, and use those
 * credentials to sign requests.</p> <p>To sign requests, we recommend that you use
 * <a
 * href="https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html">Signature
 * Version 4</a>. If you have an existing application that uses Signature Version
 * 2, you don't have to update it to use Signature Version 4. However, some
 * operations now require Signature Version 4. The documentation for operations
 * that require version 4 indicate this requirement.</p> <p>When you use the
 * Command Line Interface (CLI) or one of the Amazon Web Services SDKs to make
 * requests to Amazon Web Services, these tools automatically sign the requests for
 * you with the access key that you specify when you configure the tools.</p> <p>In
 * this release, each organization can have only one root.</p> <p> <b>Support and
 * feedback for Organizations</b> </p> <p>We welcome your feedback. You can post
 * your feedback and questions in the <a
 ...
 * cn-northwest-1</code> <i>(from Amazon Web Services Regions in China)</i> </p>
 * </li> </ul> <p> <b>How examples are presented</b> </p> <p>The JSON returned by
 * the Organizations service as response to your requests arrives as a single long
 * string without line breaks or formatting whitespace. The examples in this guide
 * include both line breaks and whitespace to improve readability. When example
 * input parameters also would result in long strings that would extend beyond the
 * screen, we insert line breaks to enhance readability. Always submit the input as
 * a single JSON text string.</p> <p> <b>Recording API Requests</b> </p>
 * <p>Organizations supports CloudTrail, a service that records Amazon Web Services
...
class AWS_ORGANIZATIONS_API OrganizationsClient : public Aws::Client::AWSJsonClient,
 ...
 public:
   ...
   */
  OrganizationsClient(const Aws::Organizations::OrganizationsClientConfiguration& clientConfiguration =
  ...
  OrganizationsClient(const Aws::Auth::AWSCredentials& credentials,
                      std::shared_ptr<OrganizationsEndpointProviderBase> endpointProvider = nullptr,
                      const Aws::Organizations::OrganizationsClientConfiguration& clientConfiguration =
                          Aws::Organizations::OrganizationsClientConfiguration());
  ...
  /**
   * An Async wrapper for AcceptHandshake that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename AcceptHandshakeRequestT = Model::AcceptHandshakeRequest>
  void AcceptHandshakeAsync(const AcceptHandshakeRequestT& request, const AcceptHandshakeResponseReceivedHandler& handler,
  ...
  /**
   * An Async wrapper for CancelHandshake that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename CancelHandshakeRequestT = Model::CancelHandshakeRequest>
  void CancelHandshakeAsync(const CancelHandshakeRequestT& request, const CancelHandshakeResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * An Async wrapper for CloseAccount that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename CloseAccountRequestT = Model::CloseAccountRequest>
  void CloseAccountAsync(const CloseAccountRequestT& request, const CloseAccountResponseReceivedHandler& handler,
                         const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for CreateGovCloudAccount that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateGovCloudAccount that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateGovCloudAccountRequestT = Model::CreateGovCloudAccountRequest>
  void CreateGovCloudAccountAsync(const CreateGovCloudAccountRequestT& request, const CreateGovCloudAccountResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   */
  template <typename CreateOrganizationRequestT = Model::CreateOrganizationRequest>
  void CreateOrganizationAsync(const CreateOrganizationResponseReceivedHandler& handler,
                               const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                               const CreateOrganizationRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for CreateOrganizationalUnit that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for CreateOrganizationalUnit that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename CreateOrganizationalUnitRequestT = Model::CreateOrganizationalUnitRequest>
  void CreateOrganizationalUnitAsync(const CreateOrganizationalUnitRequestT& request,
                                     const CreateOrganizationalUnitResponseReceivedHandler& handler,
                                     const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for DeclineHandshake that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeclineHandshake that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DeclineHandshakeRequestT = Model::DeclineHandshakeRequest>
  void DeclineHandshakeAsync(const DeclineHandshakeRequestT& request, const DeclineHandshakeResponseReceivedHandler& handler,
                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   */
  template <typename DeleteOrganizationRequestT = Model::DeleteOrganizationRequest>
  void DeleteOrganizationAsync(const DeleteOrganizationResponseReceivedHandler& handler,
                               const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                               const DeleteOrganizationRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for DeleteOrganizationalUnit that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DeleteOrganizationalUnit that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DeleteOrganizationalUnitRequestT = Model::DeleteOrganizationalUnitRequest>
  void DeleteOrganizationalUnitAsync(const DeleteOrganizationalUnitRequestT& request,
                                     const DeleteOrganizationalUnitResponseReceivedHandler& handler,
                                     const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::DeregisterDelegatedAdministratorOutcome DeregisterDelegatedAdministrator(
      const Model::DeregisterDelegatedAdministratorRequest& request) const;

  /**
   * A Callable wrapper for DeregisterDelegatedAdministrator that returns a future to the operation so that it can be executed in parallel
   * to other requests.
   */
  template <typename DeregisterDelegatedAdministratorRequestT = Model::DeregisterDelegatedAdministratorRequest>
  Model::DeregisterDelegatedAdministratorOutcomeCallable DeregisterDelegatedAdministratorCallable(
      const DeregisterDelegatedAdministratorRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::DeregisterDelegatedAdministrator, request);
  }

  /**
   * An Async wrapper for DeregisterDelegatedAdministrator that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename DeregisterDelegatedAdministratorRequestT = Model::DeregisterDelegatedAdministratorRequest>
  void DeregisterDelegatedAdministratorAsync(const DeregisterDelegatedAdministratorRequestT& request,
                                             const DeregisterDelegatedAdministratorResponseReceivedHandler& handler,
                                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   */
  template <typename DescribeAccountRequestT = Model::DescribeAccountRequest>
  void DescribeAccountAsync(const DescribeAccountRequestT& request, const DescribeAccountResponseReceivedHandler& handler,
                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeCreateAccountStatusOutcome DescribeCreateAccountStatus(
      const Model::DescribeCreateAccountStatusRequest& request) const;

  /**
   * A Callable wrapper for DescribeCreateAccountStatus that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeCreateAccountStatusRequestT = Model::DescribeCreateAccountStatusRequest>
  Model::DescribeCreateAccountStatusOutcomeCallable DescribeCreateAccountStatusCallable(
      const DescribeCreateAccountStatusRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::DescribeCreateAccountStatus, request);
  }

  /**
   * An Async wrapper for DescribeCreateAccountStatus that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeCreateAccountStatusRequestT = Model::DescribeCreateAccountStatusRequest>
  void DescribeCreateAccountStatusAsync(const DescribeCreateAccountStatusRequestT& request,
                                        const DescribeCreateAccountStatusResponseReceivedHandler& handler,
                                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for DescribeEffectivePolicy that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeEffectivePolicy that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeEffectivePolicyRequestT = Model::DescribeEffectivePolicyRequest>
  void DescribeEffectivePolicyAsync(const DescribeEffectivePolicyRequestT& request,
                                    const DescribeEffectivePolicyResponseReceivedHandler& handler,
                                    const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for DescribeHandshake that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DescribeHandshake that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DescribeHandshakeRequestT = Model::DescribeHandshakeRequest>
  void DescribeHandshakeAsync(const DescribeHandshakeRequestT& request, const DescribeHandshakeResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   */
  template <typename DescribeOrganizationRequestT = Model::DescribeOrganizationRequest>
  void DescribeOrganizationAsync(const DescribeOrganizationResponseReceivedHandler& handler,
                                 const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                 const DescribeOrganizationRequestT& request = {}) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::DescribeOrganizationalUnitOutcome DescribeOrganizationalUnit(
      const Model::DescribeOrganizationalUnitRequest& request) const;

  /**
   * A Callable wrapper for DescribeOrganizationalUnit that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename DescribeOrganizationalUnitRequestT = Model::DescribeOrganizationalUnitRequest>
  Model::DescribeOrganizationalUnitOutcomeCallable DescribeOrganizationalUnitCallable(
      const DescribeOrganizationalUnitRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::DescribeOrganizationalUnit, request);
  }

  /**
   * An Async wrapper for DescribeOrganizationalUnit that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DescribeOrganizationalUnitRequestT = Model::DescribeOrganizationalUnitRequest>
  void DescribeOrganizationalUnitAsync(const DescribeOrganizationalUnitRequestT& request,
                                       const DescribeOrganizationalUnitResponseReceivedHandler& handler,
                                       const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * An Async wrapper for DescribePolicy that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename DescribePolicyRequestT = Model::DescribePolicyRequest>
  void DescribePolicyAsync(const DescribePolicyRequestT& request, const DescribePolicyResponseReceivedHandler& handler,
                           const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   */
  template <typename DescribeResourcePolicyRequestT = Model::DescribeResourcePolicyRequest>
  void DescribeResourcePolicyAsync(const DescribeResourcePolicyResponseReceivedHandler& handler,
                                   const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                   const DescribeResourcePolicyRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for DisableAWSServiceAccess that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DisableAWSServiceAccess that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename DisableAWSServiceAccessRequestT = Model::DisableAWSServiceAccessRequest>
  void DisableAWSServiceAccessAsync(const DisableAWSServiceAccessRequestT& request,
  ...
  /**
   * A Callable wrapper for DisablePolicyType that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for DisablePolicyType that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename DisablePolicyTypeRequestT = Model::DisablePolicyTypeRequest>
  void DisablePolicyTypeAsync(const DisablePolicyTypeRequestT& request, const DisablePolicyTypeResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for EnableAWSServiceAccess that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for EnableAWSServiceAccess that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename EnableAWSServiceAccessRequestT = Model::EnableAWSServiceAccessRequest>
  void EnableAWSServiceAccessAsync(const EnableAWSServiceAccessRequestT& request,
                                   const EnableAWSServiceAccessResponseReceivedHandler& handler,
                                   const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for EnableAllFeatures that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for EnableAllFeatures that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename EnableAllFeaturesRequestT = Model::EnableAllFeaturesRequest>
  void EnableAllFeaturesAsync(const EnableAllFeaturesResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                              const EnableAllFeaturesRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for EnablePolicyType that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for EnablePolicyType that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename EnablePolicyTypeRequestT = Model::EnablePolicyTypeRequest>
  void EnablePolicyTypeAsync(const EnablePolicyTypeRequestT& request, const EnablePolicyTypeResponseReceivedHandler& handler,
                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::InviteAccountToOrganizationOutcome InviteAccountToOrganization(
      const Model::InviteAccountToOrganizationRequest& request) const;

  /**
   * A Callable wrapper for InviteAccountToOrganization that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename InviteAccountToOrganizationRequestT = Model::InviteAccountToOrganizationRequest>
  Model::InviteAccountToOrganizationOutcomeCallable InviteAccountToOrganizationCallable(
      const InviteAccountToOrganizationRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::InviteAccountToOrganization, request);
  }

  /**
   * An Async wrapper for InviteAccountToOrganization that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename InviteAccountToOrganizationRequestT = Model::InviteAccountToOrganizationRequest>
  void InviteAccountToOrganizationAsync(const InviteAccountToOrganizationRequestT& request,
                                        const InviteAccountToOrganizationResponseReceivedHandler& handler,
                                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for LeaveOrganization that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for LeaveOrganization that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename LeaveOrganizationRequestT = Model::LeaveOrganizationRequest>
  void LeaveOrganizationAsync(const LeaveOrganizationResponseReceivedHandler& handler,
                              const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                              const LeaveOrganizationRequestT& request = {}) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListAWSServiceAccessForOrganizationOutcome ListAWSServiceAccessForOrganization(
      const Model::ListAWSServiceAccessForOrganizationRequest& request = {}) const;

  /**
   * A Callable wrapper for ListAWSServiceAccessForOrganization that returns a future to the operation so that it can be executed in
   * parallel to other requests.
   */
  template <typename ListAWSServiceAccessForOrganizationRequestT = Model::ListAWSServiceAccessForOrganizationRequest>
  Model::ListAWSServiceAccessForOrganizationOutcomeCallable ListAWSServiceAccessForOrganizationCallable(
      const ListAWSServiceAccessForOrganizationRequestT& request = {}) const {
    return SubmitCallable(&OrganizationsClient::ListAWSServiceAccessForOrganization, request);
  }

  /**
   * An Async wrapper for ListAWSServiceAccessForOrganization that queues the request into a thread executor and triggers associated
   * callback when operation has finished.
   */
  template <typename ListAWSServiceAccessForOrganizationRequestT = Model::ListAWSServiceAccessForOrganizationRequest>
  void ListAWSServiceAccessForOrganizationAsync(const ListAWSServiceAccessForOrganizationResponseReceivedHandler& handler,
                                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                                const ListAWSServiceAccessForOrganizationRequestT& request = {}) const {
  ...
  /**
   * A Callable wrapper for ListAccountsForParent that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ListAccountsForParent that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListAccountsForParentRequestT = Model::ListAccountsForParentRequest>
  void ListAccountsForParentAsync(const ListAccountsForParentRequestT& request, const ListAccountsForParentResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListAccountsWithInvalidEffectivePolicyOutcome ListAccountsWithInvalidEffectivePolicy(
      const Model::ListAccountsWithInvalidEffectivePolicyRequest& request) const;

  /**
   * A Callable wrapper for ListAccountsWithInvalidEffectivePolicy that returns a future to the operation so that it can be executed in
   * parallel to other requests.
   */
  template <typename ListAccountsWithInvalidEffectivePolicyRequestT = Model::ListAccountsWithInvalidEffectivePolicyRequest>
  Model::ListAccountsWithInvalidEffectivePolicyOutcomeCallable ListAccountsWithInvalidEffectivePolicyCallable(
      const ListAccountsWithInvalidEffectivePolicyRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::ListAccountsWithInvalidEffectivePolicy, request);
  }

  /**
   * An Async wrapper for ListAccountsWithInvalidEffectivePolicy that queues the request into a thread executor and triggers associated
   * callback when operation has finished.
   */
  template <typename ListAccountsWithInvalidEffectivePolicyRequestT = Model::ListAccountsWithInvalidEffectivePolicyRequest>
  void ListAccountsWithInvalidEffectivePolicyAsync(const ListAccountsWithInvalidEffectivePolicyRequestT& request,
                                                   const ListAccountsWithInvalidEffectivePolicyResponseReceivedHandler& handler,
                                                   const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * An Async wrapper for ListChildren that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListChildrenRequestT = Model::ListChildrenRequest>
  void ListChildrenAsync(const ListChildrenRequestT& request, const ListChildrenResponseReceivedHandler& handler,
                         const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for ListCreateAccountStatus that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ListCreateAccountStatus that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListCreateAccountStatusRequestT = Model::ListCreateAccountStatusRequest>
  void ListCreateAccountStatusAsync(const ListCreateAccountStatusResponseReceivedHandler& handler,
                                    const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                    const ListCreateAccountStatusRequestT& request = {}) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListDelegatedAdministratorsOutcome ListDelegatedAdministrators(
      const Model::ListDelegatedAdministratorsRequest& request = {}) const;

  /**
   * A Callable wrapper for ListDelegatedAdministrators that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename ListDelegatedAdministratorsRequestT = Model::ListDelegatedAdministratorsRequest>
  Model::ListDelegatedAdministratorsOutcomeCallable ListDelegatedAdministratorsCallable(
      const ListDelegatedAdministratorsRequestT& request = {}) const {
    return SubmitCallable(&OrganizationsClient::ListDelegatedAdministrators, request);
  }

  /**
   * An Async wrapper for ListDelegatedAdministrators that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListDelegatedAdministratorsRequestT = Model::ListDelegatedAdministratorsRequest>
  void ListDelegatedAdministratorsAsync(const ListDelegatedAdministratorsResponseReceivedHandler& handler,
                                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                        const ListDelegatedAdministratorsRequestT& request = {}) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListDelegatedServicesForAccountOutcome ListDelegatedServicesForAccount(
      const Model::ListDelegatedServicesForAccountRequest& request) const;

  /**
   * A Callable wrapper for ListDelegatedServicesForAccount that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename ListDelegatedServicesForAccountRequestT = Model::ListDelegatedServicesForAccountRequest>
  Model::ListDelegatedServicesForAccountOutcomeCallable ListDelegatedServicesForAccountCallable(
      const ListDelegatedServicesForAccountRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::ListDelegatedServicesForAccount, request);
  }

  /**
   * An Async wrapper for ListDelegatedServicesForAccount that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename ListDelegatedServicesForAccountRequestT = Model::ListDelegatedServicesForAccountRequest>
  void ListDelegatedServicesForAccountAsync(const ListDelegatedServicesForAccountRequestT& request,
                                            const ListDelegatedServicesForAccountResponseReceivedHandler& handler,
                                            const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListEffectivePolicyValidationErrorsOutcome ListEffectivePolicyValidationErrors(
      const Model::ListEffectivePolicyValidationErrorsRequest& request) const;

  /**
   * A Callable wrapper for ListEffectivePolicyValidationErrors that returns a future to the operation so that it can be executed in
   * parallel to other requests.
   */
  template <typename ListEffectivePolicyValidationErrorsRequestT = Model::ListEffectivePolicyValidationErrorsRequest>
  Model::ListEffectivePolicyValidationErrorsOutcomeCallable ListEffectivePolicyValidationErrorsCallable(
      const ListEffectivePolicyValidationErrorsRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::ListEffectivePolicyValidationErrors, request);
  }

  /**
   * An Async wrapper for ListEffectivePolicyValidationErrors that queues the request into a thread executor and triggers associated
   * callback when operation has finished.
   */
  template <typename ListEffectivePolicyValidationErrorsRequestT = Model::ListEffectivePolicyValidationErrorsRequest>
  void ListEffectivePolicyValidationErrorsAsync(const ListEffectivePolicyValidationErrorsRequestT& request,
                                                const ListEffectivePolicyValidationErrorsResponseReceivedHandler& handler,
                                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for ListHandshakesForAccount that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename ListHandshakesForAccountRequestT = Model::ListHandshakesForAccountRequest>
  Model::ListHandshakesForAccountOutcomeCallable ListHandshakesForAccountCallable(
      const ListHandshakesForAccountRequestT& request = {}) const {
    return SubmitCallable(&OrganizationsClient::ListHandshakesForAccount, request);
  }

  /**
   * An Async wrapper for ListHandshakesForAccount that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListHandshakesForAccountRequestT = Model::ListHandshakesForAccountRequest>
  void ListHandshakesForAccountAsync(const ListHandshakesForAccountResponseReceivedHandler& handler,
                                     const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                     const ListHandshakesForAccountRequestT& request = {}) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListHandshakesForOrganizationOutcome ListHandshakesForOrganization(
      const Model::ListHandshakesForOrganizationRequest& request = {}) const;

  /**
   * A Callable wrapper for ListHandshakesForOrganization that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename ListHandshakesForOrganizationRequestT = Model::ListHandshakesForOrganizationRequest>
  Model::ListHandshakesForOrganizationOutcomeCallable ListHandshakesForOrganizationCallable(
      const ListHandshakesForOrganizationRequestT& request = {}) const {
    return SubmitCallable(&OrganizationsClient::ListHandshakesForOrganization, request);
  }

  /**
   * An Async wrapper for ListHandshakesForOrganization that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListHandshakesForOrganizationRequestT = Model::ListHandshakesForOrganizationRequest>
  void ListHandshakesForOrganizationAsync(const ListHandshakesForOrganizationResponseReceivedHandler& handler,
                                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                                          const ListHandshakesForOrganizationRequestT& request = {}) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::ListOrganizationalUnitsForParentOutcome ListOrganizationalUnitsForParent(
      const Model::ListOrganizationalUnitsForParentRequest& request) const;

  /**
   * A Callable wrapper for ListOrganizationalUnitsForParent that returns a future to the operation so that it can be executed in parallel
   * to other requests.
   */
  template <typename ListOrganizationalUnitsForParentRequestT = Model::ListOrganizationalUnitsForParentRequest>
  Model::ListOrganizationalUnitsForParentOutcomeCallable ListOrganizationalUnitsForParentCallable(
      const ListOrganizationalUnitsForParentRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::ListOrganizationalUnitsForParent, request);
  }

  /**
   * An Async wrapper for ListOrganizationalUnitsForParent that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename ListOrganizationalUnitsForParentRequestT = Model::ListOrganizationalUnitsForParentRequest>
  void ListOrganizationalUnitsForParentAsync(const ListOrganizationalUnitsForParentRequestT& request,
                                             const ListOrganizationalUnitsForParentResponseReceivedHandler& handler,
                                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * An Async wrapper for ListParents that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListParentsRequestT = Model::ListParentsRequest>
  void ListParentsAsync(const ListParentsRequestT& request, const ListParentsResponseReceivedHandler& handler,
                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for ListPoliciesForTarget that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ListPoliciesForTarget that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename ListPoliciesForTargetRequestT = Model::ListPoliciesForTargetRequest>
  void ListPoliciesForTargetAsync(const ListPoliciesForTargetRequestT& request, const ListPoliciesForTargetResponseReceivedHandler& handler,
                                  const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * An Async wrapper for ListRoots that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename ListRootsRequestT = Model::ListRootsRequest>
  void ListRootsAsync(const ListRootsResponseReceivedHandler& handler,
                      const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr,
                      const ListRootsRequestT& request = {}) const {
  ...
  /**
   * An Async wrapper for MoveAccount that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename MoveAccountRequestT = Model::MoveAccountRequest>
  void MoveAccountAsync(const MoveAccountRequestT& request, const MoveAccountResponseReceivedHandler& handler,
                        const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::RegisterDelegatedAdministratorOutcome RegisterDelegatedAdministrator(
      const Model::RegisterDelegatedAdministratorRequest& request) const;

  /**
   * A Callable wrapper for RegisterDelegatedAdministrator that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename RegisterDelegatedAdministratorRequestT = Model::RegisterDelegatedAdministratorRequest>
  Model::RegisterDelegatedAdministratorOutcomeCallable RegisterDelegatedAdministratorCallable(
      const RegisterDelegatedAdministratorRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::RegisterDelegatedAdministrator, request);
  }

  /**
   * An Async wrapper for RegisterDelegatedAdministrator that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename RegisterDelegatedAdministratorRequestT = Model::RegisterDelegatedAdministratorRequest>
  void RegisterDelegatedAdministratorAsync(const RegisterDelegatedAdministratorRequestT& request,
                                           const RegisterDelegatedAdministratorResponseReceivedHandler& handler,
                                           const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::RemoveAccountFromOrganizationOutcome RemoveAccountFromOrganization(
      const Model::RemoveAccountFromOrganizationRequest& request) const;

  /**
   * A Callable wrapper for RemoveAccountFromOrganization that returns a future to the operation so that it can be executed in parallel to
   * other requests.
   */
  template <typename RemoveAccountFromOrganizationRequestT = Model::RemoveAccountFromOrganizationRequest>
  Model::RemoveAccountFromOrganizationOutcomeCallable RemoveAccountFromOrganizationCallable(
      const RemoveAccountFromOrganizationRequestT& request) const {
    return SubmitCallable(&OrganizationsClient::RemoveAccountFromOrganization, request);
  }

  /**
   * An Async wrapper for RemoveAccountFromOrganization that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename RemoveAccountFromOrganizationRequestT = Model::RemoveAccountFromOrganizationRequest>
  void RemoveAccountFromOrganizationAsync(const RemoveAccountFromOrganizationRequestT& request,
                                          const RemoveAccountFromOrganizationResponseReceivedHandler& handler,
                                          const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
  ...
  /**
   * A Callable wrapper for UpdateOrganizationalUnit that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for UpdateOrganizationalUnit that queues the request into a thread executor and triggers associated callback when
   * operation has finished.
   */
  template <typename UpdateOrganizationalUnitRequestT = Model::UpdateOrganizationalUnitRequest>
  void UpdateOrganizationalUnitAsync(const UpdateOrganizationalUnitRequestT& request,
                                     const UpdateOrganizationalUnitResponseReceivedHandler& handler,
                                     const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
