...
class Policy {
 public:
  ...
  void SetPolicySummary(PolicySummaryT&& value) {
    m_policySummaryHasBeenSet = true;
    m_policySummary = std::forward<PolicySummaryT>(value);
  }
  template <typename PolicySummaryT = PolicySummary>
  Policy& WithPolicySummary(PolicySummaryT&& value) {
    SetPolicySummary(std::forward<PolicySummaryT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename ContentT = Aws::String>
  Policy& WithContent(ContentT&& value) {
