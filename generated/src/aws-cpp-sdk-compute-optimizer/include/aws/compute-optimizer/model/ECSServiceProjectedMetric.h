...
class ECSServiceProjectedMetric {
 public:
  ...
  AWS_COMPUTEOPTIMIZER_API ECSServiceProjectedMetric(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_COMPUTEOPTIMIZER_API ECSServiceProjectedMetric& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  inline ECSServiceProjectedMetric& WithName(ECSServiceMetricName value) {
    SetName(value);
    return *this;
  }
  ///@}
  ...
  void SetUpperBoundValues(UpperBoundValuesT&& value) {
    ...
    m_upperBoundValues = std::forward<UpperBoundValuesT>(value);
  }
  template <typename UpperBoundValuesT = Aws::Vector<double>>
  ECSServiceProjectedMetric& WithUpperBoundValues(UpperBoundValuesT&& value) {
    SetUpperBoundValues(std::forward<UpperBoundValuesT>(value));
    return *this;
  }
  inline ECSServiceProjectedMetric& AddUpperBoundValues(double value) {
    m_upperBoundValuesHasBeenSet = true;
    m_upperBoundValues.push_back(value);
    return *this;
  }
  ///@}
  ...
  void SetLowerBoundValues(LowerBoundValuesT&& value) {
    ...
    m_lowerBoundValues = std::forward<LowerBoundValuesT>(value);
  }
  template <typename LowerBoundValuesT = Aws::Vector<double>>
  ECSServiceProjectedMetric& WithLowerBoundValues(LowerBoundValuesT&& value) {
    SetLowerBoundValues(std::forward<LowerBoundValuesT>(value));
    return *this;
  }
  inline ECSServiceProjectedMetric& AddLowerBoundValues(double value) {
    m_lowerBoundValuesHasBeenSet = true;
    m_lowerBoundValues.push_back(value);
    return *this;
  }
  ///@}
 private:
