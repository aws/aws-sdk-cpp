...
class MetricSource {
 public:
  ...
  AWS_COMPUTEOPTIMIZER_API MetricSource(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_COMPUTEOPTIMIZER_API MetricSource& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  inline void SetProvider(MetricSourceProvider value) {
    m_providerHasBeenSet = true;
    m_provider = value;
  }
  inline MetricSource& WithProvider(MetricSourceProvider value) {
    SetProvider(value);
    return *this;
  }
  ///@}
  ...
  template <typename ProviderArnT = Aws::String>
  MetricSource& WithProviderArn(ProviderArnT&& value) {
