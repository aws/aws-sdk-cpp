...
RecommendationSummary& RecommendationSummary::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("recommendationResourceType")) {
    m_recommendationResourceType =
        RecommendationSourceTypeMapper::GetRecommendationSourceTypeForName(jsonValue.GetString("recommendationResourceType"));
  ...
  if (jsonValue.ValueExists("inferredWorkloadSavings")) {
    ...
    for (unsigned inferredWorkloadSavingsIndex = 0; inferredWorkloadSavingsIndex < inferredWorkloadSavingsJsonList.GetLength();
         ++inferredWorkloadSavingsIndex) {
...
JsonValue RecommendationSummary::Jsonize() const {
  ...
  if (m_recommendationResourceTypeHasBeenSet) {
    payload.WithString("recommendationResourceType",
                       RecommendationSourceTypeMapper::GetNameForRecommendationSourceType(m_recommendationResourceType));
  }
  ...
  if (m_inferredWorkloadSavingsHasBeenSet) {
    ...
    for (unsigned inferredWorkloadSavingsIndex = 0; inferredWorkloadSavingsIndex < inferredWorkloadSavingsJsonList.GetLength();
         ++inferredWorkloadSavingsIndex) {
      inferredWorkloadSavingsJsonList[inferredWorkloadSavingsIndex].AsObject(
          m_inferredWorkloadSavings[inferredWorkloadSavingsIndex].Jsonize());
    }
