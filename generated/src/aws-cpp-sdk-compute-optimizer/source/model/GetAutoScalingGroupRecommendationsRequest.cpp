...
Aws::String GetAutoScalingGroupRecommendationsRequest::SerializePayload() const {
  ...
  if (m_autoScalingGroupArnsHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("autoScalingGroupArns"));
    encoder.WriteArrayStart(m_autoScalingGroupArns.size());
    for (const auto& item_0 : m_autoScalingGroupArns) {
      encoder.WriteText(Aws::Crt::ByteCursorFromCString(item_0.c_str()));
    }
  }
