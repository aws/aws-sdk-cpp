...
RDSStorageSavingsOpportunityAfterDiscounts::RDSStorageSavingsOpportunityAfterDiscounts(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  *this = decoder;
}

RDSStorageSavingsOpportunityAfterDiscounts& RDSStorageSavingsOpportunityAfterDiscounts::operator=(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "savingsOpportunityPercentage") {
                ...
                m_estimatedMonthlySavings = RDSStorageEstimatedMonthlySavings(decoder);
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("RDSStorageSavingsOpportunityAfterDiscounts", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void RDSStorageSavingsOpportunityAfterDiscounts::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
