...
RDSDatabaseRecommendedOptionProjectedMetric& RDSDatabaseRecommendedOptionProjectedMetric::operator=(
    const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "recommendedDBInstanceClass") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_recommendedDBInstanceClass = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                    ...
                    m_recommendedDBInstanceClass = ss.str();
                  }
                }
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_projectedMetrics.push_back(RDSDatabaseProjectedMetric(decoder));
                      }
                    }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("RDSDatabaseRecommendedOptionProjectedMetric", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void RDSDatabaseRecommendedOptionProjectedMetric::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_recommendedDBInstanceClassHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("recommendedDBInstanceClass"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_recommendedDBInstanceClass.c_str()));
  }
