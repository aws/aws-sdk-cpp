...
GetEnrollmentStatusResult::GetEnrollmentStatusResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result) {
  *this = result;
}

GetEnrollmentStatusResult& GetEnrollmentStatusResult::operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "memberAccountsEnrolled") {
                auto val = decoder->PopNextBooleanVal();
                if (val.has_value()) {
                  m_memberAccountsEnrolled = val.value();
                }
              ...
              else if (initialKeyStr == "numberOfMemberAccountsOptedIn") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::UInt) {
                    auto val = decoder->PopNextUnsignedIntVal();
                    if (val.has_value()) {
                      m_numberOfMemberAccountsOptedIn = static_cast<int64_t>(val.value());
                    }
                  } else {
                    auto val = decoder->PopNextNegativeIntVal();
                    if (val.has_value()) {
                      m_numberOfMemberAccountsOptedIn = static_cast<int64_t>(1 - val.value());
                    }
                  }
                }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("GetEnrollmentStatusResult", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
