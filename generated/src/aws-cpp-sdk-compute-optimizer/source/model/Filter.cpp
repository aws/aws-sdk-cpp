          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "name") {
                ...
                if (val.has_value()) {
                  m_name =
                      FilterNameMapper::GetFilterNameForName(Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                }
...
void Filter::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  ...
  if (m_nameHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(FilterNameMapper::GetNameForFilterName(m_name).c_str()));
  }
