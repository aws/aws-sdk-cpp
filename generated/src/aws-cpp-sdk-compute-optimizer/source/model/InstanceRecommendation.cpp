...
InstanceRecommendation::InstanceRecommendation(JsonView jsonValue) { *this = jsonValue; }
...
InstanceRecommendation& InstanceRecommendation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("findingReasonCodes")) {
    ...
    for (unsigned findingReasonCodesIndex = 0; findingReasonCodesIndex < findingReasonCodesJsonList.GetLength();
         ++findingReasonCodesIndex) {
      m_findingReasonCodes.push_back(InstanceRecommendationFindingReasonCodeMapper::GetInstanceRecommendationFindingReasonCodeForName(
          findingReasonCodesJsonList[findingReasonCodesIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("recommendationOptions")) {
    ...
    for (unsigned recommendationOptionsIndex = 0; recommendationOptionsIndex < recommendationOptionsJsonList.GetLength();
         ++recommendationOptionsIndex) {
  ...
  if (jsonValue.ValueExists("recommendationSources")) {
    ...
    for (unsigned recommendationSourcesIndex = 0; recommendationSourcesIndex < recommendationSourcesJsonList.GetLength();
         ++recommendationSourcesIndex) {
  ...
  if (jsonValue.ValueExists("currentPerformanceRisk")) {
    m_currentPerformanceRisk =
        CurrentPerformanceRiskMapper::GetCurrentPerformanceRiskForName(jsonValue.GetString("currentPerformanceRisk"));
...
JsonValue InstanceRecommendation::Jsonize() const {
  ...
  if (m_findingReasonCodesHasBeenSet) {
    ...
    for (unsigned findingReasonCodesIndex = 0; findingReasonCodesIndex < findingReasonCodesJsonList.GetLength();
         ++findingReasonCodesIndex) {
      findingReasonCodesJsonList[findingReasonCodesIndex].AsString(
          InstanceRecommendationFindingReasonCodeMapper::GetNameForInstanceRecommendationFindingReasonCode(
              m_findingReasonCodes[findingReasonCodesIndex]));
    }
