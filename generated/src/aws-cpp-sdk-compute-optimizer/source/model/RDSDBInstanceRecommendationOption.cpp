...
RDSDBInstanceRecommendationOption::RDSDBInstanceRecommendationOption(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  *this = decoder;
}

RDSDBInstanceRecommendationOption& RDSDBInstanceRecommendationOption::operator=(
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "dbInstanceClass") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_dbInstanceClass = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
                  ...
                  if (peekType_0.value() == CborType::ArrayStart) {
                    ...
                    if (listSize_0.has_value()) {
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        m_projectedUtilizationMetrics.push_back(RDSDBUtilizationMetric(decoder));
                      }
                    }
                ...
                m_savingsOpportunityAfterDiscounts = RDSInstanceSavingsOpportunityAfterDiscounts(decoder);
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("RDSDBInstanceRecommendationOption", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void RDSDBInstanceRecommendationOption::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_dbInstanceClassHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("dbInstanceClass"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_dbInstanceClass.c_str()));
  }
