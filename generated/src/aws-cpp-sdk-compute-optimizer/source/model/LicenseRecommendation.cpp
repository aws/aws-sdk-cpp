...
LicenseRecommendation::LicenseRecommendation(JsonView jsonValue) { *this = jsonValue; }
...
LicenseRecommendation& LicenseRecommendation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("licenseRecommendationOptions")) {
    ...
    for (unsigned licenseRecommendationOptionsIndex = 0;
         licenseRecommendationOptionsIndex < licenseRecommendationOptionsJsonList.GetLength(); ++licenseRecommendationOptionsIndex) {
...
JsonValue LicenseRecommendation::Jsonize() const {
  ...
  if (m_findingReasonCodesHasBeenSet) {
    ...
    for (unsigned findingReasonCodesIndex = 0; findingReasonCodesIndex < findingReasonCodesJsonList.GetLength();
         ++findingReasonCodesIndex) {
      findingReasonCodesJsonList[findingReasonCodesIndex].AsString(
          LicenseFindingReasonCodeMapper::GetNameForLicenseFindingReasonCode(m_findingReasonCodes[findingReasonCodesIndex]));
    }
  ...
  if (m_licenseRecommendationOptionsHasBeenSet) {
    ...
    for (unsigned licenseRecommendationOptionsIndex = 0;
         licenseRecommendationOptionsIndex < licenseRecommendationOptionsJsonList.GetLength(); ++licenseRecommendationOptionsIndex) {
      licenseRecommendationOptionsJsonList[licenseRecommendationOptionsIndex].AsObject(
          m_licenseRecommendationOptions[licenseRecommendationOptionsIndex].Jsonize());
    }
