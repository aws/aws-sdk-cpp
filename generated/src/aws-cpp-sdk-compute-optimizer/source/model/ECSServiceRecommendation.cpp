...
ECSServiceRecommendation::ECSServiceRecommendation(JsonView jsonValue) { *this = jsonValue; }
...
ECSServiceRecommendation& ECSServiceRecommendation::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("findingReasonCodes")) {
    ...
    for (unsigned findingReasonCodesIndex = 0; findingReasonCodesIndex < findingReasonCodesJsonList.GetLength();
      ...
      m_findingReasonCodes.push_back(ECSServiceRecommendationFindingReasonCodeMapper::GetECSServiceRecommendationFindingReasonCodeForName(
  ...
  if (jsonValue.ValueExists("serviceRecommendationOptions")) {
    ...
    for (unsigned serviceRecommendationOptionsIndex = 0;
         serviceRecommendationOptionsIndex < serviceRecommendationOptionsJsonList.GetLength(); ++serviceRecommendationOptionsIndex) {
...
JsonValue ECSServiceRecommendation::Jsonize() const {
  ...
  if (m_findingReasonCodesHasBeenSet) {
    ...
    for (unsigned findingReasonCodesIndex = 0; findingReasonCodesIndex < findingReasonCodesJsonList.GetLength();
         ++findingReasonCodesIndex) {
      findingReasonCodesJsonList[findingReasonCodesIndex].AsString(
          ECSServiceRecommendationFindingReasonCodeMapper::GetNameForECSServiceRecommendationFindingReasonCode(
  ...
  if (m_serviceRecommendationOptionsHasBeenSet) {
    ...
    for (unsigned serviceRecommendationOptionsIndex = 0;
         serviceRecommendationOptionsIndex < serviceRecommendationOptionsJsonList.GetLength(); ++serviceRecommendationOptionsIndex) {
      serviceRecommendationOptionsJsonList[serviceRecommendationOptionsIndex].AsObject(
          m_serviceRecommendationOptions[serviceRecommendationOptionsIndex].Jsonize());
    }
