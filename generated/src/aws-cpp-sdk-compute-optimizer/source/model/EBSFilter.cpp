...
EBSFilter::EBSFilter(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

EBSFilter& EBSFilter::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "name") {
                ...
                if (val.has_value()) {
                  m_name = EBSFilterNameMapper::GetEBSFilterNameForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("EBSFilter", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void EBSFilter::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_nameHasBeenSet) {
    mapSize++;
  ...
  if (m_nameHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(EBSFilterNameMapper::GetNameForEBSFilterName(m_name).c_str()));
  }
