...
class GetDICOMImportJobResult {
 public:
  ...
  void SetJobProperties(JobPropertiesT&& value) {
    m_jobPropertiesHasBeenSet = true;
    m_jobProperties = std::forward<JobPropertiesT>(value);
  }
  template <typename JobPropertiesT = DICOMImportJobProperties>
  GetDICOMImportJobResult& WithJobProperties(JobPropertiesT&& value) {
    SetJobProperties(std::forward<JobPropertiesT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetDICOMImportJobResult& WithRequestId(RequestIdT&& value) {
