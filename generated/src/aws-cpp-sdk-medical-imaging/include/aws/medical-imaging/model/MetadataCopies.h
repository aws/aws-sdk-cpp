...
class MetadataCopies {
 public:
  ...
  void SetCopiableAttributes(CopiableAttributesT&& value) {
    m_copiableAttributesHasBeenSet = true;
    m_copiableAttributes = std::forward<CopiableAttributesT>(value);
  }
  template <typename CopiableAttributesT = Aws::String>
  MetadataCopies& WithCopiableAttributes(CopiableAttributesT&& value) {
    SetCopiableAttributes(std::forward<CopiableAttributesT>(value));
    return *this;
  }
  ///@}
 private:
