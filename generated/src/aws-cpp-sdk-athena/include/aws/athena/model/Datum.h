...
class Datum {
 public:
  ...
  void SetVarCharValue(VarCharValueT&& value) {
    m_varCharValueHasBeenSet = true;
    m_varCharValue = std::forward<VarCharValueT>(value);
  }
  template <typename VarCharValueT = Aws::String>
  Datum& WithVarCharValue(VarCharValueT&& value) {
    SetVarCharValue(std::forward<VarCharValueT>(value));
    return *this;
  }
  ///@}
 private:
