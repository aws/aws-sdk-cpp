...
class AwsCloudFrontDistributionOriginSslProtocols {
 public:
  ...
  template <typename ItemsT = Aws::Vector<Aws::String>>
  AwsCloudFrontDistributionOriginSslProtocols& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  template <typename ItemsT = Aws::String>
  AwsCloudFrontDistributionOriginSslProtocols& AddItems(ItemsT&& value) {
    m_itemsHasBeenSet = true;
  ...
  inline AwsCloudFrontDistributionOriginSslProtocols& WithQuantity(int value) {
    SetQuantity(value);
    return *this;
  }
  ///@}
