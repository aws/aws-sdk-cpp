...
class AwsCloudFormationStackDriftInformationDetails {
 public:
  ...
  void SetStackDriftStatus(StackDriftStatusT&& value) {
    m_stackDriftStatusHasBeenSet = true;
    m_stackDriftStatus = std::forward<StackDriftStatusT>(value);
  }
  template <typename StackDriftStatusT = Aws::String>
  AwsCloudFormationStackDriftInformationDetails& WithStackDriftStatus(StackDriftStatusT&& value) {
    SetStackDriftStatus(std::forward<StackDriftStatusT>(value));
    return *this;
  }
  ///@}
 private:
