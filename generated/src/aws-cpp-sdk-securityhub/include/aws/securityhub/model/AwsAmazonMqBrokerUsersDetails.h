...
class AwsAmazonMqBrokerUsersDetails {
 public:
  ...
  void SetPendingChange(PendingChangeT&& value) {
    m_pendingChangeHasBeenSet = true;
    m_pendingChange = std::forward<PendingChangeT>(value);
  }
  template <typename PendingChangeT = Aws::String>
  AwsAmazonMqBrokerUsersDetails& WithPendingChange(PendingChangeT&& value) {
    SetPendingChange(std::forward<PendingChangeT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename UsernameT = Aws::String>
  AwsAmazonMqBrokerUsersDetails& WithUsername(UsernameT&& value) {
