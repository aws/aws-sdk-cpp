...
class RuleGroupVariablesPortSetsDetails {
 public:
  ...
  template <typename DefinitionT = Aws::Vector<Aws::String>>
  RuleGroupVariablesPortSetsDetails& WithDefinition(DefinitionT&& value) {
    SetDefinition(std::forward<DefinitionT>(value));
    return *this;
  }
  template <typename DefinitionT = Aws::String>
  RuleGroupVariablesPortSetsDetails& AddDefinition(DefinitionT&& value) {
    m_definitionHasBeenSet = true;
    m_definition.emplace_back(std::forward<DefinitionT>(value));
    return *this;
  }
  ///@}
 private:
