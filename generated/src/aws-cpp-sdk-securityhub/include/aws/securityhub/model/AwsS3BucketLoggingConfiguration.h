...
class AwsS3BucketLoggingConfiguration {
 public:
  ...
  void SetDestinationBucketName(DestinationBucketNameT&& value) {
    ...
    m_destinationBucketName = std::forward<DestinationBucketNameT>(value);
  }
  template <typename DestinationBucketNameT = Aws::String>
  AwsS3BucketLoggingConfiguration& WithDestinationBucketName(DestinationBucketNameT&& value) {
    SetDestinationBucketName(std::forward<DestinationBucketNameT>(value));
    return *this;
  }
  ///@}
  ...
  void SetLogFilePrefix(LogFilePrefixT&& value) {
    ...
    m_logFilePrefix = std::forward<LogFilePrefixT>(value);
  }
  template <typename LogFilePrefixT = Aws::String>
  AwsS3BucketLoggingConfiguration& WithLogFilePrefix(LogFilePrefixT&& value) {
    SetLogFilePrefix(std::forward<LogFilePrefixT>(value));
    return *this;
  }
  ///@}
 private:
