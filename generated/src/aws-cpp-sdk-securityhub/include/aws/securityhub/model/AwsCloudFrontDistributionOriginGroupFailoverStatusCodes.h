...
class AwsCloudFrontDistributionOriginGroupFailoverStatusCodes {
 public:
  ...
  template <typename ItemsT = Aws::Vector<int>>
  AwsCloudFrontDistributionOriginGroupFailoverStatusCodes& WithItems(ItemsT&& value) {
    SetItems(std::forward<ItemsT>(value));
    return *this;
  }
  inline AwsCloudFrontDistributionOriginGroupFailoverStatusCodes& AddItems(int value) {
    m_itemsHasBeenSet = true;
  ...
  inline AwsCloudFrontDistributionOriginGroupFailoverStatusCodes& WithQuantity(int value) {
    SetQuantity(value);
    return *this;
  }
  ///@}
