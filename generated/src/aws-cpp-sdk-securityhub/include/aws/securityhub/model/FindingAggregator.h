...
/**
 * <p>A finding aggregator is a Security Hub CSPM resource that specifies
...
class FindingAggregator {
 public:
  ...
  void SetFindingAggregatorArn(FindingAggregatorArnT&& value) {
    ...
    m_findingAggregatorArn = std::forward<FindingAggregatorArnT>(value);
  }
  template <typename FindingAggregatorArnT = Aws::String>
  FindingAggregator& WithFindingAggregatorArn(FindingAggregatorArnT&& value) {
    SetFindingAggregatorArn(std::forward<FindingAggregatorArnT>(value));
    return *this;
  }
  ///@}
 private:
