...
class AwsCertificateManagerCertificateExtendedKeyUsage {
 public:
  ...
  template <typename NameT = Aws::String>
  AwsCertificateManagerCertificateExtendedKeyUsage& WithName(NameT&& value) {
  ...
  void SetOId(OIdT&& value) {
    m_oIdHasBeenSet = true;
    m_oId = std::forward<OIdT>(value);
  }
  template <typename OIdT = Aws::String>
  AwsCertificateManagerCertificateExtendedKeyUsage& WithOId(OIdT&& value) {
    SetOId(std::forward<OIdT>(value));
    return *this;
  }
  ///@}
 private:
