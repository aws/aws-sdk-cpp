...
Product& Product::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("IntegrationTypes")) {
    ...
    for (unsigned integrationTypesIndex = 0; integrationTypesIndex < integrationTypesJsonList.GetLength(); ++integrationTypesIndex) {
      m_integrationTypes.push_back(
          IntegrationTypeMapper::GetIntegrationTypeForName(integrationTypesJsonList[integrationTypesIndex].AsString()));
    }
...
JsonValue Product::Jsonize() const {
  ...
  if (m_integrationTypesHasBeenSet) {
    ...
    for (unsigned integrationTypesIndex = 0; integrationTypesIndex < integrationTypesJsonList.GetLength(); ++integrationTypesIndex) {
      integrationTypesJsonList[integrationTypesIndex].AsString(
          IntegrationTypeMapper::GetNameForIntegrationType(m_integrationTypes[integrationTypesIndex]));
    }
