...
SecurityHubPolicy::SecurityHubPolicy(JsonView jsonValue) { *this = jsonValue; }
...
SecurityHubPolicy& SecurityHubPolicy::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("EnabledStandardIdentifiers")) {
    ...
    for (unsigned enabledStandardIdentifiersIndex = 0; enabledStandardIdentifiersIndex < enabledStandardIdentifiersJsonList.GetLength();
         ++enabledStandardIdentifiersIndex) {
...
JsonValue SecurityHubPolicy::Jsonize() const {
  ...
  if (m_enabledStandardIdentifiersHasBeenSet) {
    ...
    for (unsigned enabledStandardIdentifiersIndex = 0; enabledStandardIdentifiersIndex < enabledStandardIdentifiersJsonList.GetLength();
         ++enabledStandardIdentifiersIndex) {
      enabledStandardIdentifiersJsonList[enabledStandardIdentifiersIndex].AsString(
          m_enabledStandardIdentifiers[enabledStandardIdentifiersIndex]);
    }
