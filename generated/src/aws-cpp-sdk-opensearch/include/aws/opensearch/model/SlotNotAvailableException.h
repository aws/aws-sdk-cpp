...
class SlotNotAvailableException {
 public:
  ...
  void SetSlotSuggestions(SlotSuggestionsT&& value) {
    m_slotSuggestionsHasBeenSet = true;
    m_slotSuggestions = std::forward<SlotSuggestionsT>(value);
  }
  template <typename SlotSuggestionsT = Aws::Vector<long long>>
  SlotNotAvailableException& WithSlotSuggestions(SlotSuggestionsT&& value) {
    SetSlotSuggestions(std::forward<SlotSuggestionsT>(value));
    return *this;
  }
  inline SlotNotAvailableException& AddSlotSuggestions(long long value) {
    m_slotSuggestionsHasBeenSet = true;
    m_slotSuggestions.push_back(value);
    return *this;
  }
  ///@}
 private:
