...
class ClusterPendingUpdates {
 public:
  ...
  void SetResharding(ReshardingT&& value) {
    ...
    m_resharding = std::forward<ReshardingT>(value);
  }
  template <typename ReshardingT = ReshardingStatus>
  ClusterPendingUpdates& WithResharding(ReshardingT&& value) {
    SetResharding(std::forward<ReshardingT>(value));
    return *this;
  }
  ///@}
  ...
  void SetACLs(ACLsT&& value) {
    m_aCLsHasBeenSet = true;
    m_aCLs = std::forward<ACLsT>(value);
  }
  template <typename ACLsT = ACLsUpdateStatus>
  ClusterPendingUpdates& WithACLs(ACLsT&& value) {
    SetACLs(std::forward<ACLsT>(value));
    return *this;
  }
  ///@}
  ...
  void SetServiceUpdates(ServiceUpdatesT&& value) {
    m_serviceUpdatesHasBeenSet = true;
    m_serviceUpdates = std::forward<ServiceUpdatesT>(value);
  }
  template <typename ServiceUpdatesT = Aws::Vector<PendingModifiedServiceUpdate>>
  ClusterPendingUpdates& WithServiceUpdates(ServiceUpdatesT&& value) {
    SetServiceUpdates(std::forward<ServiceUpdatesT>(value));
    return *this;
  }
  template <typename ServiceUpdatesT = PendingModifiedServiceUpdate>
  ClusterPendingUpdates& AddServiceUpdates(ServiceUpdatesT&& value) {
    m_serviceUpdatesHasBeenSet = true;
    m_serviceUpdates.emplace_back(std::forward<ServiceUpdatesT>(value));
    return *this;
  }
  ///@}
 private:
