...
class GetMetricsRequest : public IoTWirelessRequest {
 public:
  ...
  void SetSummaryMetricQueries(SummaryMetricQueriesT&& value) {
    m_summaryMetricQueriesHasBeenSet = true;
    m_summaryMetricQueries = std::forward<SummaryMetricQueriesT>(value);
  }
  template <typename SummaryMetricQueriesT = Aws::Vector<SummaryMetricQuery>>
  GetMetricsRequest& WithSummaryMetricQueries(SummaryMetricQueriesT&& value) {
    SetSummaryMetricQueries(std::forward<SummaryMetricQueriesT>(value));
    return *this;
  }
  template <typename SummaryMetricQueriesT = SummaryMetricQuery>
  GetMetricsRequest& AddSummaryMetricQueries(SummaryMetricQueriesT&& value) {
    m_summaryMetricQueriesHasBeenSet = true;
    m_summaryMetricQueries.emplace_back(std::forward<SummaryMetricQueriesT>(value));
    return *this;
  }
  ///@}
 private:
