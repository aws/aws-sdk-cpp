...
TraceContent::TraceContent(JsonView jsonValue) { *this = jsonValue; }

TraceContent& TraceContent::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("WirelessDeviceFrameInfo")) {
    m_wirelessDeviceFrameInfo =
        WirelessDeviceFrameInfoMapper::GetWirelessDeviceFrameInfoForName(jsonValue.GetString("WirelessDeviceFrameInfo"));
...
JsonValue TraceContent::Jsonize() const {
  ...
  if (m_wirelessDeviceFrameInfoHasBeenSet) {
    payload.WithString("WirelessDeviceFrameInfo",
                       WirelessDeviceFrameInfoMapper::GetNameForWirelessDeviceFrameInfo(m_wirelessDeviceFrameInfo));
  }
