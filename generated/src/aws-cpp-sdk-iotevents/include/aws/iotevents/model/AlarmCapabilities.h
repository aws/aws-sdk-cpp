...
class AlarmCapabilities {
 public:
  ...
  void SetInitializationConfiguration(InitializationConfigurationT&& value) {
    ...
    m_initializationConfiguration = std::forward<InitializationConfigurationT>(value);
  }
  template <typename InitializationConfigurationT = InitializationConfiguration>
  AlarmCapabilities& WithInitializationConfiguration(InitializationConfigurationT&& value) {
    SetInitializationConfiguration(std::forward<InitializationConfigurationT>(value));
    return *this;
  }
  ///@}
  ...
  void SetAcknowledgeFlow(AcknowledgeFlowT&& value) {
    ...
    m_acknowledgeFlow = std::forward<AcknowledgeFlowT>(value);
  }
  template <typename AcknowledgeFlowT = AcknowledgeFlow>
  AlarmCapabilities& WithAcknowledgeFlow(AcknowledgeFlowT&& value) {
    SetAcknowledgeFlow(std::forward<AcknowledgeFlowT>(value));
    return *this;
  }
  ///@}
 private:
