...
DeliverToMailboxAction::DeliverToMailboxAction(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

DeliverToMailboxAction& DeliverToMailboxAction::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              else if (initialKeyStr == "MailboxArn") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Text) {
                    auto val = decoder->PopNextTextVal();
                    if (val.has_value()) {
                      m_mailboxArn = Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len);
                    }
                  } else {
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("DeliverToMailboxAction", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void DeliverToMailboxAction::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_mailboxArnHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("MailboxArn"));
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_mailboxArn.c_str()));
  }
