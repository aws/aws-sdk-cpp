...
RuleStringExpression::RuleStringExpression(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

RuleStringExpression& RuleStringExpression::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Evaluate") {
                m_evaluate = RuleStringToEvaluate(decoder);
                ...
                if (val.has_value()) {
                  m_operator = RuleStringOperatorMapper::GetRuleStringOperatorForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("RuleStringExpression", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void RuleStringExpression::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_operatorHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(RuleStringOperatorMapper::GetNameForRuleStringOperator(m_operator).c_str()));
  }
