...
IngressStringExpression::IngressStringExpression(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

IngressStringExpression& IngressStringExpression::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Evaluate") {
                m_evaluate = IngressStringToEvaluate(decoder);
                ...
                if (val.has_value()) {
                  m_operator = IngressStringOperatorMapper::GetIngressStringOperatorForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("IngressStringExpression", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void IngressStringExpression::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_operatorHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(IngressStringOperatorMapper::GetNameForIngressStringOperator(m_operator).c_str()));
  }
