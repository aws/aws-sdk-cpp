...
IngressIpv6Expression::IngressIpv6Expression(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

IngressIpv6Expression& IngressIpv6Expression::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "Evaluate") {
                m_evaluate = IngressIpv6ToEvaluate(decoder);
                ...
                if (val.has_value()) {
                  m_operator = IngressIpOperatorMapper::GetIngressIpOperatorForName(
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("IngressIpv6Expression", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void IngressIpv6Expression::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  ...
  if (m_operatorHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(IngressIpOperatorMapper::GetNameForIngressIpOperator(m_operator).c_str()));
  }
