...
class ArchiveFilterCondition {
 public:
  ...
  AWS_MAILMANAGER_API ArchiveFilterCondition(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_MAILMANAGER_API ArchiveFilterCondition& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  void SetStringExpression(StringExpressionT&& value) {
    ...
    m_stringExpression = std::forward<StringExpressionT>(value);
  }
  template <typename StringExpressionT = ArchiveStringExpression>
  ArchiveFilterCondition& WithStringExpression(StringExpressionT&& value) {
    SetStringExpression(std::forward<StringExpressionT>(value));
    return *this;
  }
  ///@}
  ...
  void SetBooleanExpression(BooleanExpressionT&& value) {
    ...
    m_booleanExpression = std::forward<BooleanExpressionT>(value);
  }
  template <typename BooleanExpressionT = ArchiveBooleanExpression>
  ArchiveFilterCondition& WithBooleanExpression(BooleanExpressionT&& value) {
    SetBooleanExpression(std::forward<BooleanExpressionT>(value));
    return *this;
  }
  ///@}
 private:
