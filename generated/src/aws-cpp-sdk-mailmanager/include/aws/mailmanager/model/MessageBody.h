...
class MessageBody {
 public:
  ...
  void SetHtml(HtmlT&& value) {
    ...
    m_html = std::forward<HtmlT>(value);
  }
  template <typename HtmlT = Aws::String>
  MessageBody& WithHtml(HtmlT&& value) {
    SetHtml(std::forward<HtmlT>(value));
    return *this;
  }
  ///@}
  ...
  inline void SetMessageMalformed(bool value) {
    m_messageMalformedHasBeenSet = true;
    m_messageMalformed = value;
  }
  inline MessageBody& WithMessageMalformed(bool value) {
    SetMessageMalformed(value);
    return *this;
  }
  ///@}
 private:
