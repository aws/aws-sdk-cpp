...
class ArchiveAction {
 public:
  ...
  AWS_MAILMANAGER_API ArchiveAction(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_MAILMANAGER_API ArchiveAction& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  inline ArchiveAction& WithActionFailurePolicy(ActionFailurePolicy value) {
  ...
  void SetTargetArchive(TargetArchiveT&& value) {
    m_targetArchiveHasBeenSet = true;
    m_targetArchive = std::forward<TargetArchiveT>(value);
  }
  template <typename TargetArchiveT = Aws::String>
  ArchiveAction& WithTargetArchive(TargetArchiveT&& value) {
    SetTargetArchive(std::forward<TargetArchiveT>(value));
    return *this;
  }
  ///@}
 private:
