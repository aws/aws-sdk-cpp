...
class RelayAction {
 public:
  ...
  AWS_MAILMANAGER_API RelayAction(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_MAILMANAGER_API RelayAction& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  inline RelayAction& WithActionFailurePolicy(ActionFailurePolicy value) {
  ...
  inline void SetMailFrom(MailFrom value) {
    m_mailFromHasBeenSet = true;
    m_mailFrom = value;
  }
  inline RelayAction& WithMailFrom(MailFrom value) {
    SetMailFrom(value);
    return *this;
  }
  ///@}
