...
class Envelope {
 public:
  ...
  AWS_MAILMANAGER_API Envelope(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_MAILMANAGER_API Envelope& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  void SetHelo(HeloT&& value) {
    ...
    m_helo = std::forward<HeloT>(value);
  }
  template <typename HeloT = Aws::String>
  Envelope& WithHelo(HeloT&& value) {
    SetHelo(std::forward<HeloT>(value));
    return *this;
  }
  ///@}
  ...
  Envelope& AddTo(ToT&& value) {
    m_toHasBeenSet = true;
    m_to.emplace_back(std::forward<ToT>(value));
    return *this;
  }
  ///@}
 private:
