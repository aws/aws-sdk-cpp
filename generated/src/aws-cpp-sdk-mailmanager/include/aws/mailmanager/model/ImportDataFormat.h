...
class ImportDataFormat {
 public:
  ...
  AWS_MAILMANAGER_API ImportDataFormat(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_MAILMANAGER_API ImportDataFormat& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  inline void SetImportDataType(ImportDataType value) {
    m_importDataTypeHasBeenSet = true;
    m_importDataType = value;
  }
  inline ImportDataFormat& WithImportDataType(ImportDataType value) {
    SetImportDataType(value);
    return *this;
  }
  ///@}
 private:
