...
class AddressFilter {
 public:
  ...
  AWS_MAILMANAGER_API AddressFilter(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_MAILMANAGER_API AddressFilter& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  void SetAddressPrefix(AddressPrefixT&& value) {
    m_addressPrefixHasBeenSet = true;
    m_addressPrefix = std::forward<AddressPrefixT>(value);
  }
  template <typename AddressPrefixT = Aws::String>
  AddressFilter& WithAddressPrefix(AddressPrefixT&& value) {
    SetAddressPrefix(std::forward<AddressPrefixT>(value));
    return *this;
  }
  ///@}
 private:
