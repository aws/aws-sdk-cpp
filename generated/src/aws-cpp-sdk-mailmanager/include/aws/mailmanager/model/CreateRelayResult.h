...
class CreateRelayResult {
 public:
  ...
  AWS_MAILMANAGER_API CreateRelayResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  AWS_MAILMANAGER_API CreateRelayResult& operator=(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result);
  ...
  void SetRelayId(RelayIdT&& value) {
    ...
    m_relayId = std::forward<RelayIdT>(value);
  }
  template <typename RelayIdT = Aws::String>
  CreateRelayResult& WithRelayId(RelayIdT&& value) {
    SetRelayId(std::forward<RelayIdT>(value));
    return *this;
  }
  ///@}
