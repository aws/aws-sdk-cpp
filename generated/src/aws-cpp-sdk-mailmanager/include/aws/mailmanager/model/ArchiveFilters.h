...
class ArchiveFilters {
 public:
  ...
  AWS_MAILMANAGER_API ArchiveFilters(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_MAILMANAGER_API ArchiveFilters& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  template <typename IncludeT = Aws::Vector<ArchiveFilterCondition>>
  ArchiveFilters& WithInclude(IncludeT&& value) {
    SetInclude(std::forward<IncludeT>(value));
    return *this;
  }
  template <typename IncludeT = ArchiveFilterCondition>
  ArchiveFilters& AddInclude(IncludeT&& value) {
    m_includeHasBeenSet = true;
    m_include.emplace_back(std::forward<IncludeT>(value));
    return *this;
  }
  ///@}
  ...
  void SetUnless(UnlessT&& value) {
    ...
    m_unless = std::forward<UnlessT>(value);
  }
  template <typename UnlessT = Aws::Vector<ArchiveFilterCondition>>
  ArchiveFilters& WithUnless(UnlessT&& value) {
    SetUnless(std::forward<UnlessT>(value));
    return *this;
  }
  template <typename UnlessT = ArchiveFilterCondition>
  ArchiveFilters& AddUnless(UnlessT&& value) {
    m_unlessHasBeenSet = true;
    m_unless.emplace_back(std::forward<UnlessT>(value));
    return *this;
  }
  ///@}
 private:
