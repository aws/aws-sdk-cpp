...
class DeleteDiskSnapshotResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  DeleteDiskSnapshotResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  DeleteDiskSnapshotResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  DeleteDiskSnapshotResult& WithRequestId(RequestIdT&& value) {
