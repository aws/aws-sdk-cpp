...
class CreateLoadBalancerResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  CreateLoadBalancerResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  CreateLoadBalancerResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  CreateLoadBalancerResult& WithRequestId(RequestIdT&& value) {
