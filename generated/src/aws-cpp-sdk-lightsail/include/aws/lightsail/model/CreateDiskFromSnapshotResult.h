...
class CreateDiskFromSnapshotResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  CreateDiskFromSnapshotResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  CreateDiskFromSnapshotResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  CreateDiskFromSnapshotResult& WithRequestId(RequestIdT&& value) {
