...
class ExportSnapshotResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  ExportSnapshotResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  ExportSnapshotResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ExportSnapshotResult& WithRequestId(RequestIdT&& value) {
