...
class UpdateDomainEntryResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  UpdateDomainEntryResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  UpdateDomainEntryResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  UpdateDomainEntryResult& WithRequestId(RequestIdT&& value) {
