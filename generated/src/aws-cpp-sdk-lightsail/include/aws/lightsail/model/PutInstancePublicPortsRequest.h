...
class PutInstancePublicPortsRequest : public LightsailRequest {
 public:
  ...
  void SetPortInfos(PortInfosT&& value) {
    m_portInfosHasBeenSet = true;
    m_portInfos = std::forward<PortInfosT>(value);
  }
  template <typename PortInfosT = Aws::Vector<PortInfo>>
  PutInstancePublicPortsRequest& WithPortInfos(PortInfosT&& value) {
    SetPortInfos(std::forward<PortInfosT>(value));
    return *this;
  }
  template <typename PortInfosT = PortInfo>
  PutInstancePublicPortsRequest& AddPortInfos(PortInfosT&& value) {
    m_portInfosHasBeenSet = true;
    m_portInfos.emplace_back(std::forward<PortInfosT>(value));
    return *this;
  }
  ///@}
