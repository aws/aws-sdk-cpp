...
class StartGUISessionResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  StartGUISessionResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  StartGUISessionResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  StartGUISessionResult& WithRequestId(RequestIdT&& value) {
