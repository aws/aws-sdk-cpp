...
class DetachStaticIpResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  DetachStaticIpResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  DetachStaticIpResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  DetachStaticIpResult& WithRequestId(RequestIdT&& value) {
