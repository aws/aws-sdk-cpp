...
class EnableAddOnRequest : public LightsailRequest {
 public:
  ...
  template <typename ResourceNameT = Aws::String>
  EnableAddOnRequest& WithResourceName(ResourceNameT&& value) {
  ...
  void SetAddOnRequest(AddOnRequestT&& value) {
    m_addOnRequestHasBeenSet = true;
    m_addOnRequest = std::forward<AddOnRequestT>(value);
  }
  template <typename AddOnRequestT = AddOnRequest>
  EnableAddOnRequest& WithAddOnRequest(AddOnRequestT&& value) {
    SetAddOnRequest(std::forward<AddOnRequestT>(value));
    return *this;
  }
  ///@}
 private:
