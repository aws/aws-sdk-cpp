...
class DeleteKeyPairRequest : public LightsailRequest {
 public:
  ...
  void SetExpectedFingerprint(ExpectedFingerprintT&& value) {
    m_expectedFingerprintHasBeenSet = true;
    m_expectedFingerprint = std::forward<ExpectedFingerprintT>(value);
  }
  template <typename ExpectedFingerprintT = Aws::String>
  DeleteKeyPairRequest& WithExpectedFingerprint(ExpectedFingerprintT&& value) {
    SetExpectedFingerprint(std::forward<ExpectedFingerprintT>(value));
    return *this;
  }
  ///@}
 private:
