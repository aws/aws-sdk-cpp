...
class SetIpAddressTypeResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  SetIpAddressTypeResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  SetIpAddressTypeResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  SetIpAddressTypeResult& WithRequestId(RequestIdT&& value) {
