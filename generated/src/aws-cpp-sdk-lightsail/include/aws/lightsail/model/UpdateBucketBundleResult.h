...
class UpdateBucketBundleResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  UpdateBucketBundleResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  UpdateBucketBundleResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  UpdateBucketBundleResult& WithRequestId(RequestIdT&& value) {
