...
class AttachStaticIpResult {
 public:
  ...
  template <typename OperationsT = Aws::Vector<Operation>>
  AttachStaticIpResult& WithOperations(OperationsT&& value) {
    SetOperations(std::forward<OperationsT>(value));
    return *this;
  }
  template <typename OperationsT = Operation>
  AttachStaticIpResult& AddOperations(OperationsT&& value) {
    m_operationsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  AttachStaticIpResult& WithRequestId(RequestIdT&& value) {
