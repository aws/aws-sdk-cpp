...
GetCostEstimateResult::GetCostEstimateResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
GetCostEstimateResult& GetCostEstimateResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("resourcesBudgetEstimate")) {
    ...
    for (unsigned resourcesBudgetEstimateIndex = 0; resourcesBudgetEstimateIndex < resourcesBudgetEstimateJsonList.GetLength();
         ++resourcesBudgetEstimateIndex) {
      m_resourcesBudgetEstimate.push_back(resourcesBudgetEstimateJsonList[resourcesBudgetEstimateIndex].AsObject());
    }
    m_resourcesBudgetEstimateHasBeenSet = true;
  }
