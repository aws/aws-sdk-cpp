...
Bundle::Bundle(JsonView jsonValue) { *this = jsonValue; }
...
Bundle& Bundle::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("supportedPlatforms")) {
    ...
    for (unsigned supportedPlatformsIndex = 0; supportedPlatformsIndex < supportedPlatformsJsonList.GetLength();
         ++supportedPlatformsIndex) {
      m_supportedPlatforms.push_back(
          InstancePlatformMapper::GetInstancePlatformForName(supportedPlatformsJsonList[supportedPlatformsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("supportedAppCategories")) {
    ...
    for (unsigned supportedAppCategoriesIndex = 0; supportedAppCategoriesIndex < supportedAppCategoriesJsonList.GetLength();
         ++supportedAppCategoriesIndex) {
      m_supportedAppCategories.push_back(
          AppCategoryMapper::GetAppCategoryForName(supportedAppCategoriesJsonList[supportedAppCategoriesIndex].AsString()));
    }
...
JsonValue Bundle::Jsonize() const {
  ...
  if (m_supportedPlatformsHasBeenSet) {
    ...
    for (unsigned supportedPlatformsIndex = 0; supportedPlatformsIndex < supportedPlatformsJsonList.GetLength();
         ++supportedPlatformsIndex) {
      supportedPlatformsJsonList[supportedPlatformsIndex].AsString(
          InstancePlatformMapper::GetNameForInstancePlatform(m_supportedPlatforms[supportedPlatformsIndex]));
    }
  ...
  if (m_supportedAppCategoriesHasBeenSet) {
    ...
    for (unsigned supportedAppCategoriesIndex = 0; supportedAppCategoriesIndex < supportedAppCategoriesJsonList.GetLength();
         ++supportedAppCategoriesIndex) {
      supportedAppCategoriesJsonList[supportedAppCategoriesIndex].AsString(
          AppCategoryMapper::GetNameForAppCategory(m_supportedAppCategories[supportedAppCategoriesIndex]));
    }
