...
QueryStringObject::QueryStringObject(JsonView jsonValue) { *this = jsonValue; }
...
QueryStringObject& QueryStringObject::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("queryStringsAllowList")) {
    ...
    for (unsigned queryStringsAllowListIndex = 0; queryStringsAllowListIndex < queryStringsAllowListJsonList.GetLength();
         ++queryStringsAllowListIndex) {
