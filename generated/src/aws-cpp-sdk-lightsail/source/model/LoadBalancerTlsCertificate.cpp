...
LoadBalancerTlsCertificate::LoadBalancerTlsCertificate(JsonView jsonValue) { *this = jsonValue; }
...
LoadBalancerTlsCertificate& LoadBalancerTlsCertificate::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("domainValidationRecords")) {
    ...
    for (unsigned domainValidationRecordsIndex = 0; domainValidationRecordsIndex < domainValidationRecordsJsonList.GetLength();
         ++domainValidationRecordsIndex) {
  ...
  if (jsonValue.ValueExists("failureReason")) {
    m_failureReason = LoadBalancerTlsCertificateFailureReasonMapper::GetLoadBalancerTlsCertificateFailureReasonForName(
        jsonValue.GetString("failureReason"));
  ...
  if (jsonValue.ValueExists("revocationReason")) {
    m_revocationReason = LoadBalancerTlsCertificateRevocationReasonMapper::GetLoadBalancerTlsCertificateRevocationReasonForName(
        jsonValue.GetString("revocationReason"));
...
JsonValue LoadBalancerTlsCertificate::Jsonize() const {
  ...
  if (m_domainValidationRecordsHasBeenSet) {
    ...
    for (unsigned domainValidationRecordsIndex = 0; domainValidationRecordsIndex < domainValidationRecordsJsonList.GetLength();
         ++domainValidationRecordsIndex) {
      domainValidationRecordsJsonList[domainValidationRecordsIndex].AsObject(
          m_domainValidationRecords[domainValidationRecordsIndex].Jsonize());
    }
    payload.WithArray("domainValidationRecords", std::move(domainValidationRecordsJsonList));
  }

  if (m_failureReasonHasBeenSet) {
    payload.WithString("failureReason",
                       LoadBalancerTlsCertificateFailureReasonMapper::GetNameForLoadBalancerTlsCertificateFailureReason(m_failureReason));
  }
  ...
  if (m_revocationReasonHasBeenSet) {
    payload.WithString(
        "revocationReason",
        LoadBalancerTlsCertificateRevocationReasonMapper::GetNameForLoadBalancerTlsCertificateRevocationReason(m_revocationReason));
  }
