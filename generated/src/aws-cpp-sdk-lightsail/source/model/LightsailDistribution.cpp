...
LightsailDistribution::LightsailDistribution(JsonView jsonValue) { *this = jsonValue; }
...
LightsailDistribution& LightsailDistribution::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("alternativeDomainNames")) {
    ...
    for (unsigned alternativeDomainNamesIndex = 0; alternativeDomainNamesIndex < alternativeDomainNamesJsonList.GetLength();
         ++alternativeDomainNamesIndex) {
