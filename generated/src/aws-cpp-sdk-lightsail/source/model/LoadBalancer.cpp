...
LoadBalancer::LoadBalancer(JsonView jsonValue) { *this = jsonValue; }
...
LoadBalancer& LoadBalancer::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("instanceHealthSummary")) {
    ...
    for (unsigned instanceHealthSummaryIndex = 0; instanceHealthSummaryIndex < instanceHealthSummaryJsonList.GetLength();
         ++instanceHealthSummaryIndex) {
  ...
  if (jsonValue.ValueExists("tlsCertificateSummaries")) {
    ...
    for (unsigned tlsCertificateSummariesIndex = 0; tlsCertificateSummariesIndex < tlsCertificateSummariesJsonList.GetLength();
         ++tlsCertificateSummariesIndex) {
  ...
  if (jsonValue.ValueExists("configurationOptions")) {
    ...
    for (auto& configurationOptionsItem : configurationOptionsJsonMap) {
      m_configurationOptions[LoadBalancerAttributeNameMapper::GetLoadBalancerAttributeNameForName(configurationOptionsItem.first)] =
          configurationOptionsItem.second.AsString();
    }
...
JsonValue LoadBalancer::Jsonize() const {
  ...
  if (m_tlsCertificateSummariesHasBeenSet) {
    ...
    for (unsigned tlsCertificateSummariesIndex = 0; tlsCertificateSummariesIndex < tlsCertificateSummariesJsonList.GetLength();
         ++tlsCertificateSummariesIndex) {
      tlsCertificateSummariesJsonList[tlsCertificateSummariesIndex].AsObject(
          m_tlsCertificateSummaries[tlsCertificateSummariesIndex].Jsonize());
    }
  ...
  if (m_configurationOptionsHasBeenSet) {
    ...
    for (auto& configurationOptionsItem : m_configurationOptions) {
      configurationOptionsJsonMap.WithString(
          LoadBalancerAttributeNameMapper::GetNameForLoadBalancerAttributeName(configurationOptionsItem.first),
          configurationOptionsItem.second);
    }
