...
class AWS_SAGEMAKERRUNTIME_API SageMakerRuntimeClient : public Aws::Client::AWSJsonClient,
 ...
 public:
   ...
   */
  SageMakerRuntimeClient(const Aws::SageMakerRuntime::SageMakerRuntimeClientConfiguration& clientConfiguration =
  ...
  SageMakerRuntimeClient(const Aws::Auth::AWSCredentials& credentials,
                         std::shared_ptr<SageMakerRuntimeEndpointProviderBase> endpointProvider = nullptr,
                         const Aws::SageMakerRuntime::SageMakerRuntimeClientConfiguration& clientConfiguration =
                             Aws::SageMakerRuntime::SageMakerRuntimeClientConfiguration());
   ...
   */
  template <typename InvokeEndpointRequestT = Model::InvokeEndpointRequest>
  ...
  /**
   * An Async wrapper for InvokeEndpoint that queues the request into a thread executor and triggers associated callback when operation has
   * finished.
   */
  template <typename InvokeEndpointRequestT = Model::InvokeEndpointRequest>
  void InvokeEndpointAsync(const InvokeEndpointRequestT& request, const InvokeEndpointResponseReceivedHandler& handler,
  ...
  /**
   * A Callable wrapper for InvokeEndpointAsync that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  template <typename InvokeEndpointAsyncRequestT = Model::InvokeEndpointAsyncRequest>
  ...
  /**
   * An Async wrapper for InvokeEndpointAsync that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename InvokeEndpointAsyncRequestT = Model::InvokeEndpointAsyncRequest>
  void InvokeEndpointAsyncAsync(const InvokeEndpointAsyncRequestT& request, const InvokeEndpointAsyncResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
   ...
   * API Reference</a></p>
   */
  virtual Model::InvokeEndpointWithResponseStreamOutcome InvokeEndpointWithResponseStream(
      Model::InvokeEndpointWithResponseStreamRequest& request) const;

  /**
   * A Callable wrapper for InvokeEndpointWithResponseStream that returns a future to the operation so that it can be executed in parallel
   * to other requests.
   */
  template <typename InvokeEndpointWithResponseStreamRequestT = Model::InvokeEndpointWithResponseStreamRequest>
  Model::InvokeEndpointWithResponseStreamOutcomeCallable InvokeEndpointWithResponseStreamCallable(
      InvokeEndpointWithResponseStreamRequestT& request) const {
    return SubmitCallable(&SageMakerRuntimeClient::InvokeEndpointWithResponseStream, request);
  }

  /**
   * An Async wrapper for InvokeEndpointWithResponseStream that queues the request into a thread executor and triggers associated callback
   * when operation has finished.
   */
  template <typename InvokeEndpointWithResponseStreamRequestT = Model::InvokeEndpointWithResponseStreamRequest>
  void InvokeEndpointWithResponseStreamAsync(InvokeEndpointWithResponseStreamRequestT& request,
                                             const InvokeEndpointWithResponseStreamResponseReceivedHandler& handler,
                                             const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
