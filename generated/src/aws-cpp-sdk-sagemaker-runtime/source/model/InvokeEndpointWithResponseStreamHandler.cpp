...
InvokeEndpointWithResponseStreamHandler::InvokeEndpointWithResponseStreamHandler() : EventStreamHandler() {
  m_onInitialResponse = [&](const InvokeEndpointWithResponseStreamInitialResponse&, const Utils::Event::InitialResponseType eventType) {
    AWS_LOGSTREAM_TRACE(INVOKEENDPOINTWITHRESPONSESTREAM_HANDLER_CLASS_TAG,
                        "InvokeEndpointWithResponseStream initial response received from "
...
void InvokeEndpointWithResponseStreamHandler::HandleEventInMessage() {
  ...
  switch (InvokeEndpointWithResponseStreamEventMapper::GetInvokeEndpointWithResponseStreamEventTypeForName(
      eventTypeHeaderIter->second.GetEventHeaderValueAsString())) {
    case InvokeEndpointWithResponseStreamEventType::INITIAL_RESPONSE: {
      InvokeEndpointWithResponseStreamInitialResponse event(GetEventHeadersAsHttpHeaders());
...
void InvokeEndpointWithResponseStreamHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(INVOKEENDPOINTWITHRESPONSESTREAM_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(INVOKEENDPOINTWITHRESPONSESTREAM_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
    ...
    if (!exceptionPayload.WasParseSuccessful()) {
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(INVOKEENDPOINTWITHRESPONSESTREAM_HANDLER_CLASS_TAG,
