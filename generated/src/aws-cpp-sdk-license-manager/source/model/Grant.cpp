...
Grant& Grant::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("GrantedOperations")) {
    ...
    for (unsigned grantedOperationsIndex = 0; grantedOperationsIndex < grantedOperationsJsonList.GetLength(); ++grantedOperationsIndex) {
      m_grantedOperations.push_back(
          AllowedOperationMapper::GetAllowedOperationForName(grantedOperationsJsonList[grantedOperationsIndex].AsString()));
    }
...
JsonValue Grant::Jsonize() const {
  ...
  if (m_grantedOperationsHasBeenSet) {
    ...
    for (unsigned grantedOperationsIndex = 0; grantedOperationsIndex < grantedOperationsJsonList.GetLength(); ++grantedOperationsIndex) {
      grantedOperationsJsonList[grantedOperationsIndex].AsString(
          AllowedOperationMapper::GetNameForAllowedOperation(m_grantedOperations[grantedOperationsIndex]));
    }
