...
template <>
AWS_LICENSEMANAGER_API RedirectException LicenseManagerError::GetModeledError() {
  assert(this->GetErrorType() == LicenseManagerErrors::REDIRECT);
  return RedirectException(this->GetJsonPayload().View());
}

template <>
AWS_LICENSEMANAGER_API FailedDependencyException LicenseManagerError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(LicenseManagerErrors::UNSUPPORTED_DIGITAL_SIGNATURE_METHOD),
