...
class ListRoutesResult {
 public:
  ...
  void SetRouteSummaryList(RouteSummaryListT&& value) {
    m_routeSummaryListHasBeenSet = true;
    m_routeSummaryList = std::forward<RouteSummaryListT>(value);
  }
  template <typename RouteSummaryListT = Aws::Vector<RouteSummary>>
  ListRoutesResult& WithRouteSummaryList(RouteSummaryListT&& value) {
    SetRouteSummaryList(std::forward<RouteSummaryListT>(value));
    return *this;
  }
  template <typename RouteSummaryListT = RouteSummary>
  ListRoutesResult& AddRouteSummaryList(RouteSummaryListT&& value) {
    m_routeSummaryListHasBeenSet = true;
    m_routeSummaryList.emplace_back(std::forward<RouteSummaryListT>(value));
    return *this;
  }
  ///@}
