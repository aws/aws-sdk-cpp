...
class UrlEndpointSummary {
 public:
  ...
  void SetHealthUrl(HealthUrlT&& value) {
    m_healthUrlHasBeenSet = true;
    m_healthUrl = std::forward<HealthUrlT>(value);
  }
  template <typename HealthUrlT = Aws::String>
  UrlEndpointSummary& WithHealthUrl(HealthUrlT&& value) {
    SetHealthUrl(std::forward<HealthUrlT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename UrlT = Aws::String>
  UrlEndpointSummary& WithUrl(UrlT&& value) {
