...
class AWS_MIGRATIONHUBREFACTORSPACES_API MigrationHubRefactorSpacesClient
 ...
 public:
  ...
  MigrationHubRefactorSpacesClient(
      const Aws::MigrationHubRefactorSpaces::MigrationHubRefactorSpacesClientConfiguration& clientConfiguration =
  ...
  /**
   * A Callable wrapper for ListEnvironmentVpcs that returns a future to the operation so that it can be executed in parallel to other
   * requests.
   */
  ...
  /**
   * An Async wrapper for ListEnvironmentVpcs that queues the request into a thread executor and triggers associated callback when operation
   * has finished.
   */
  template <typename ListEnvironmentVpcsRequestT = Model::ListEnvironmentVpcsRequest>
  void ListEnvironmentVpcsAsync(const ListEnvironmentVpcsRequestT& request, const ListEnvironmentVpcsResponseReceivedHandler& handler,
                                const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context = nullptr) const {
