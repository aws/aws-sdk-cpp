...
template <>
AWS_MIGRATIONHUBREFACTORSPACES_API ConflictException MigrationHubRefactorSpacesError::GetModeledError() {
  assert(this->GetErrorType() == MigrationHubRefactorSpacesErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_MIGRATIONHUBREFACTORSPACES_API ServiceQuotaExceededException MigrationHubRefactorSpacesError::GetModeledError() {
  assert(this->GetErrorType() == MigrationHubRefactorSpacesErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_MIGRATIONHUBREFACTORSPACES_API ThrottlingException MigrationHubRefactorSpacesError::GetModeledError() {
  assert(this->GetErrorType() == MigrationHubRefactorSpacesErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_MIGRATIONHUBREFACTORSPACES_API ResourceNotFoundException MigrationHubRefactorSpacesError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(MigrationHubRefactorSpacesErrors::SERVICE_QUOTA_EXCEEDED),
                                RetryableType::NOT_RETRYABLE);
  } else if (hashCode == INTERNAL_SERVER_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(MigrationHubRefactorSpacesErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
  } else if (hashCode == INVALID_RESOURCE_POLICY_HASH) {
    return AWSError<CoreErrors>(static_cast<CoreErrors>(MigrationHubRefactorSpacesErrors::INVALID_RESOURCE_POLICY),
                                RetryableType::NOT_RETRYABLE);
  }
