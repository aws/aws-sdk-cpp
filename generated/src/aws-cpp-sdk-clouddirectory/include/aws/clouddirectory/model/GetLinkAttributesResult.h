...
class GetLinkAttributesResult {
 public:
  ...
  template <typename AttributesT = Aws::Vector<AttributeKeyAndValue>>
  GetLinkAttributesResult& WithAttributes(AttributesT&& value) {
    SetAttributes(std::forward<AttributesT>(value));
    return *this;
  }
  template <typename AttributesT = AttributeKeyAndValue>
  GetLinkAttributesResult& AddAttributes(AttributesT&& value) {
    m_attributesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  GetLinkAttributesResult& WithRequestId(RequestIdT&& value) {
