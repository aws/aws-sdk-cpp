...
class GetFacetResult {
 public:
  ...
  void SetFacet(FacetT&& value) {
    m_facetHasBeenSet = true;
    m_facet = std::forward<FacetT>(value);
  }
  template <typename FacetT = Facet>
  GetFacetResult& WithFacet(FacetT&& value) {
    SetFacet(std::forward<FacetT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetFacetResult& WithRequestId(RequestIdT&& value) {
