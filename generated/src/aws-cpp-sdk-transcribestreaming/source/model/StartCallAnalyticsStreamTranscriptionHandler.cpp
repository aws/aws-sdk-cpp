...
void StartCallAnalyticsStreamTranscriptionHandler::HandleEventInMessage() {
  ...
  switch (StartCallAnalyticsStreamTranscriptionEventMapper::GetStartCallAnalyticsStreamTranscriptionEventTypeForName(
    ...
    case StartCallAnalyticsStreamTranscriptionEventType::UTTERANCEEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCALLANALYTICSSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper UtteranceEvent object from the response in JSON format.");
        break;
      }
    ...
    case StartCallAnalyticsStreamTranscriptionEventType::CATEGORYEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCALLANALYTICSSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper CategoryEvent object from the response in JSON format.");
        break;
      }
...
void StartCallAnalyticsStreamTranscriptionHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(STARTCALLANALYTICSSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(STARTCALLANALYTICSSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
    ...
    if (!exceptionPayload.WasParseSuccessful()) {
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(STARTCALLANALYTICSSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG,
