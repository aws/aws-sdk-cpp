...
void StartMedicalStreamTranscriptionHandler::HandleEventInMessage() {
  ...
  switch (StartMedicalStreamTranscriptionEventMapper::GetStartMedicalStreamTranscriptionEventTypeForName(
    ...
    case StartMedicalStreamTranscriptionEventType::TRANSCRIPTEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTMEDICALSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper MedicalTranscriptEvent object from the response in JSON format.");
        break;
      }
...
void StartMedicalStreamTranscriptionHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(STARTMEDICALSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(STARTMEDICALSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
    ...
    if (!exceptionPayload.WasParseSuccessful()) {
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(STARTMEDICALSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG,
...
StartMedicalStreamTranscriptionEventType GetStartMedicalStreamTranscriptionEventTypeForName(const Aws::String& name) {
  ...
  if (hashCode == INITIAL_RESPONSE_HASH) {
    return StartMedicalStreamTranscriptionEventType::INITIAL_RESPONSE;
