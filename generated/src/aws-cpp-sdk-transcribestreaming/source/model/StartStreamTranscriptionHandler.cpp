...
void StartStreamTranscriptionHandler::HandleEventInMessage() {
  ...
  switch (StartStreamTranscriptionEventMapper::GetStartStreamTranscriptionEventTypeForName(
...
void StartStreamTranscriptionHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(STARTSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(STARTSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
    ...
    if (!exceptionPayload.WasParseSuccessful()) {
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(STARTSTREAMTRANSCRIPTION_HANDLER_CLASS_TAG,
