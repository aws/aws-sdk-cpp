...
void StartMedicalScribeStreamHandler::HandleEventInMessage() {
  ...
  switch (StartMedicalScribeStreamEventMapper::GetStartMedicalScribeStreamEventTypeForName(
...
void StartMedicalScribeStreamHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(STARTMEDICALSCRIBESTREAM_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(STARTMEDICALSCRIBESTREAM_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
    ...
    if (!exceptionPayload.WasParseSuccessful()) {
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(STARTMEDICALSCRIBESTREAM_HANDLER_CLASS_TAG,
