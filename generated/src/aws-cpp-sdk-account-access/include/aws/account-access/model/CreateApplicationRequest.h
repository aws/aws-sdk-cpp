...
/**
 */
class CreateApplicationRequest : public AccountAccessRequest {
 public:
  ...
  ///@{
  /**
   * <p>Specifies the identity source for the application. The identity source
   * defines the IAM Identity Center instance that provides principals for
   * entitlements.</p>
   */
  inline const IdentitySource& GetIdentitySource() const { return m_identitySource; }
  inline bool IdentitySourceHasBeenSet() const { return m_identitySourceHasBeenSet; }
  template <typename IdentitySourceT = IdentitySource>
  ...
  ///@}
 private:
  IdentitySource m_identitySource;
