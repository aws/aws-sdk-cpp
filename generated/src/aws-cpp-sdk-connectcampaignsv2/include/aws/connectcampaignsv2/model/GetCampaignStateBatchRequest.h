...
class GetCampaignStateBatchRequest : public ConnectCampaignsV2Request {
 public:
  ...
  void SetCampaignIds(CampaignIdsT&& value) {
    m_campaignIdsHasBeenSet = true;
    m_campaignIds = std::forward<CampaignIdsT>(value);
  }
  template <typename CampaignIdsT = Aws::Vector<Aws::String>>
  GetCampaignStateBatchRequest& WithCampaignIds(CampaignIdsT&& value) {
    SetCampaignIds(std::forward<CampaignIdsT>(value));
    return *this;
  }
  template <typename CampaignIdsT = Aws::String>
  GetCampaignStateBatchRequest& AddCampaignIds(CampaignIdsT&& value) {
    m_campaignIdsHasBeenSet = true;
    m_campaignIds.emplace_back(std::forward<CampaignIdsT>(value));
    return *this;
  }
  ///@}
 private:
