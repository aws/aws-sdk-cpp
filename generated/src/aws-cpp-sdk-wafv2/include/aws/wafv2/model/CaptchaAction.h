...
class CaptchaAction {
 public:
  AWS_WAFV2_API CaptchaAction() = default;
  ...
  void SetCustomRequestHandling(CustomRequestHandlingT&& value) {
    ...
    m_customRequestHandling = std::forward<CustomRequestHandlingT>(value);
  }
  template <typename CustomRequestHandlingT = CustomRequestHandling>
  CaptchaAction& WithCustomRequestHandling(CustomRequestHandlingT&& value) {
    SetCustomRequestHandling(std::forward<CustomRequestHandlingT>(value));
    return *this;
  }
  ///@}
 private:
