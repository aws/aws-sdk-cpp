...
namespace WAFV2 {
template <>
AWS_WAFV2_API WAFLimitsExceededException WAFV2Error::GetModeledError() {
  assert(this->GetErrorType() == WAFV2Errors::W_A_F_LIMITS_EXCEEDED);
  return WAFLimitsExceededException(this->GetJsonPayload().View());
}

template <>
AWS_WAFV2_API WAFInvalidParameterException WAFV2Error::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == W_A_F_UNAVAILABLE_ENTITY_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(WAFV2Errors::W_A_F_EXPIRED_MANAGED_RULE_GROUP_VERSION),
