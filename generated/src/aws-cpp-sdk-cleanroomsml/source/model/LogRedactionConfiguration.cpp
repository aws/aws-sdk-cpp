...
LogRedactionConfiguration::LogRedactionConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
JsonValue LogRedactionConfiguration::Jsonize() const {
  ...
  if (m_entitiesToRedactHasBeenSet) {
    ...
    for (unsigned entitiesToRedactIndex = 0; entitiesToRedactIndex < entitiesToRedactJsonList.GetLength(); ++entitiesToRedactIndex) {
      entitiesToRedactJsonList[entitiesToRedactIndex].AsString(
          EntityTypeMapper::GetNameForEntityType(m_entitiesToRedact[entitiesToRedactIndex]));
    }
