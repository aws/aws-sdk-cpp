...
ListAudienceExportJobsResult::ListAudienceExportJobsResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
ListAudienceExportJobsResult& ListAudienceExportJobsResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("audienceExportJobs")) {
    ...
    for (unsigned audienceExportJobsIndex = 0; audienceExportJobsIndex < audienceExportJobsJsonList.GetLength();
         ++audienceExportJobsIndex) {
      m_audienceExportJobs.push_back(audienceExportJobsJsonList[audienceExportJobsIndex].AsObject());
    }
    m_audienceExportJobsHasBeenSet = true;
  }
