...
BatchStopJobRunResult::BatchStopJobRunResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
BatchStopJobRunResult& BatchStopJobRunResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("SuccessfulSubmissions")) {
    ...
    for (unsigned successfulSubmissionsIndex = 0; successfulSubmissionsIndex < successfulSubmissionsJsonList.GetLength();
         ++successfulSubmissionsIndex) {
      m_successfulSubmissions.push_back(successfulSubmissionsJsonList[successfulSubmissionsIndex].AsObject());
    }
    m_successfulSubmissionsHasBeenSet = true;
  }
