...
StatisticSummary::StatisticSummary(JsonView jsonValue) { *this = jsonValue; }
...
StatisticSummary& StatisticSummary::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ReferencedDatasets")) {
    ...
    for (unsigned referencedDatasetsIndex = 0; referencedDatasetsIndex < referencedDatasetsJsonList.GetLength();
         ++referencedDatasetsIndex) {
