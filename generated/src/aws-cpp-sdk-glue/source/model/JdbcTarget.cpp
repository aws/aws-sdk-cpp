...
JdbcTarget::JdbcTarget(JsonView jsonValue) { *this = jsonValue; }
...
JdbcTarget& JdbcTarget::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("EnableAdditionalMetadata")) {
    ...
    for (unsigned enableAdditionalMetadataIndex = 0; enableAdditionalMetadataIndex < enableAdditionalMetadataJsonList.GetLength();
         ++enableAdditionalMetadataIndex) {
      m_enableAdditionalMetadata.push_back(
          JdbcMetadataEntryMapper::GetJdbcMetadataEntryForName(enableAdditionalMetadataJsonList[enableAdditionalMetadataIndex].AsString()));
    }
...
JsonValue JdbcTarget::Jsonize() const {
  ...
  if (m_enableAdditionalMetadataHasBeenSet) {
    ...
    for (unsigned enableAdditionalMetadataIndex = 0; enableAdditionalMetadataIndex < enableAdditionalMetadataJsonList.GetLength();
         ++enableAdditionalMetadataIndex) {
      enableAdditionalMetadataJsonList[enableAdditionalMetadataIndex].AsString(
          JdbcMetadataEntryMapper::GetNameForJdbcMetadataEntry(m_enableAdditionalMetadata[enableAdditionalMetadataIndex]));
    }
