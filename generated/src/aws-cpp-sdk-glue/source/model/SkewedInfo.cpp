...
SkewedInfo::SkewedInfo(JsonView jsonValue) { *this = jsonValue; }
...
SkewedInfo& SkewedInfo::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("SkewedColumnValues")) {
    ...
    for (unsigned skewedColumnValuesIndex = 0; skewedColumnValuesIndex < skewedColumnValuesJsonList.GetLength();
         ++skewedColumnValuesIndex) {
  ...
  if (jsonValue.ValueExists("SkewedColumnValueLocationMaps")) {
    Aws::Map<Aws::String, JsonView> skewedColumnValueLocationMapsJsonMap =
        jsonValue.GetObject("SkewedColumnValueLocationMaps").GetAllObjects();
