...
ConnectionInput::ConnectionInput(JsonView jsonValue) { *this = jsonValue; }
...
ConnectionInput& ConnectionInput::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("ConnectionProperties")) {
    ...
    for (auto& connectionPropertiesItem : connectionPropertiesJsonMap) {
      m_connectionProperties[ConnectionPropertyKeyMapper::GetConnectionPropertyKeyForName(connectionPropertiesItem.first)] =
          connectionPropertiesItem.second.AsString();
    }
  ...
  if (jsonValue.ValueExists("ValidateForComputeEnvironments")) {
    ...
    for (unsigned validateForComputeEnvironmentsIndex = 0;
         validateForComputeEnvironmentsIndex < validateForComputeEnvironmentsJsonList.GetLength(); ++validateForComputeEnvironmentsIndex) {
      m_validateForComputeEnvironments.push_back(ComputeEnvironmentMapper::GetComputeEnvironmentForName(
          validateForComputeEnvironmentsJsonList[validateForComputeEnvironmentsIndex].AsString()));
    }
...
JsonValue ConnectionInput::Jsonize() const {
  ...
  if (m_connectionPropertiesHasBeenSet) {
    ...
    for (auto& connectionPropertiesItem : m_connectionProperties) {
      connectionPropertiesJsonMap.WithString(ConnectionPropertyKeyMapper::GetNameForConnectionPropertyKey(connectionPropertiesItem.first),
                                             connectionPropertiesItem.second);
    }
  ...
  if (m_validateForComputeEnvironmentsHasBeenSet) {
    ...
    for (unsigned validateForComputeEnvironmentsIndex = 0;
         validateForComputeEnvironmentsIndex < validateForComputeEnvironmentsJsonList.GetLength(); ++validateForComputeEnvironmentsIndex) {
      validateForComputeEnvironmentsJsonList[validateForComputeEnvironmentsIndex].AsString(
          ComputeEnvironmentMapper::GetNameForComputeEnvironment(m_validateForComputeEnvironments[validateForComputeEnvironmentsIndex]));
    }
