...
class OpenTableFormatInput {
 public:
  ...
  void SetIcebergInput(IcebergInputT&& value) {
    m_icebergInputHasBeenSet = true;
    m_icebergInput = std::forward<IcebergInputT>(value);
  }
  template <typename IcebergInputT = IcebergInput>
  OpenTableFormatInput& WithIcebergInput(IcebergInputT&& value) {
    SetIcebergInput(std::forward<IcebergInputT>(value));
    return *this;
  }
  ///@}
 private:
