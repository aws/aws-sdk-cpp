...
class GetSchemaVersionsDiffResult {
 public:
  ...
  void SetDiff(DiffT&& value) {
    m_diffHasBeenSet = true;
    m_diff = std::forward<DiffT>(value);
  }
  template <typename DiffT = Aws::String>
  GetSchemaVersionsDiffResult& WithDiff(DiffT&& value) {
    SetDiff(std::forward<DiffT>(value));
    return *this;
  }
  ///@}
  ...
  template <typename RequestIdT = Aws::String>
  GetSchemaVersionsDiffResult& WithRequestId(RequestIdT&& value) {
