...
class BatchStopJobRunRequest : public GlueRequest {
 public:
  ...
  void SetJobRunIds(JobRunIdsT&& value) {
    m_jobRunIdsHasBeenSet = true;
    m_jobRunIds = std::forward<JobRunIdsT>(value);
  }
  template <typename JobRunIdsT = Aws::Vector<Aws::String>>
  BatchStopJobRunRequest& WithJobRunIds(JobRunIdsT&& value) {
    SetJobRunIds(std::forward<JobRunIdsT>(value));
    return *this;
  }
  template <typename JobRunIdsT = Aws::String>
  BatchStopJobRunRequest& AddJobRunIds(JobRunIdsT&& value) {
    m_jobRunIdsHasBeenSet = true;
    m_jobRunIds.emplace_back(std::forward<JobRunIdsT>(value));
    return *this;
  }
  ///@}
 private:
