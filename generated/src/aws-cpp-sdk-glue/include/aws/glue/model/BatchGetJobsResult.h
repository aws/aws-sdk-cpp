...
class BatchGetJobsResult {
 public:
  ...
  void SetJobsNotFound(JobsNotFoundT&& value) {
    ...
    m_jobsNotFound = std::forward<JobsNotFoundT>(value);
  }
  template <typename JobsNotFoundT = Aws::Vector<Aws::String>>
  BatchGetJobsResult& WithJobsNotFound(JobsNotFoundT&& value) {
    SetJobsNotFound(std::forward<JobsNotFoundT>(value));
    return *this;
  }
  template <typename JobsNotFoundT = Aws::String>
  BatchGetJobsResult& AddJobsNotFound(JobsNotFoundT&& value) {
    m_jobsNotFoundHasBeenSet = true;
    m_jobsNotFound.emplace_back(std::forward<JobsNotFoundT>(value));
    return *this;
  }
  ///@}
