...
class StatementOutputData {
 public:
  ...
  void SetTextPlain(TextPlainT&& value) {
    m_textPlainHasBeenSet = true;
    m_textPlain = std::forward<TextPlainT>(value);
  }
  template <typename TextPlainT = Aws::String>
  StatementOutputData& WithTextPlain(TextPlainT&& value) {
    SetTextPlain(std::forward<TextPlainT>(value));
    return *this;
  }
  ///@}
 private:
