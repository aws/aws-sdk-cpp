...
class BatchGetPartitionRequest : public GlueRequest {
 public:
  ...
  void SetPartitionsToGet(PartitionsToGetT&& value) {
    m_partitionsToGetHasBeenSet = true;
    m_partitionsToGet = std::forward<PartitionsToGetT>(value);
  }
  template <typename PartitionsToGetT = Aws::Vector<PartitionValueList>>
  BatchGetPartitionRequest& WithPartitionsToGet(PartitionsToGetT&& value) {
    SetPartitionsToGet(std::forward<PartitionsToGetT>(value));
    return *this;
  }
  template <typename PartitionsToGetT = PartitionValueList>
  BatchGetPartitionRequest& AddPartitionsToGet(PartitionsToGetT&& value) {
    m_partitionsToGetHasBeenSet = true;
    m_partitionsToGet.emplace_back(std::forward<PartitionsToGetT>(value));
    return *this;
  }
  ///@}
 private:
