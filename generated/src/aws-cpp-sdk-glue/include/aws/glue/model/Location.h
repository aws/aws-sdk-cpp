...
class Location {
 public:
  ...
  void SetJdbc(JdbcT&& value) {
    ...
    m_jdbc = std::forward<JdbcT>(value);
  }
  template <typename JdbcT = Aws::Vector<CodeGenNodeArg>>
  Location& WithJdbc(JdbcT&& value) {
    SetJdbc(std::forward<JdbcT>(value));
    return *this;
  }
  template <typename JdbcT = CodeGenNodeArg>
  Location& AddJdbc(JdbcT&& value) {
    m_jdbcHasBeenSet = true;
    m_jdbc.emplace_back(std::forward<JdbcT>(value));
    return *this;
  }
  ///@}
  ...
  Location& AddDynamoDB(DynamoDBT&& value) {
    m_dynamoDBHasBeenSet = true;
    m_dynamoDB.emplace_back(std::forward<DynamoDBT>(value));
    return *this;
  }
  ///@}
 private:
