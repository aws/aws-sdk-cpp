...
class DataSource {
 public:
  ...
  void SetGlueTable(GlueTableT&& value) {
    ...
    m_glueTable = std::forward<GlueTableT>(value);
  }
  template <typename GlueTableT = GlueTable>
  DataSource& WithGlueTable(GlueTableT&& value) {
    SetGlueTable(std::forward<GlueTableT>(value));
    return *this;
  }
  ///@}
  ...
  void SetDataQualityGlueTable(DataQualityGlueTableT&& value) {
    ...
    m_dataQualityGlueTable = std::forward<DataQualityGlueTableT>(value);
  }
  template <typename DataQualityGlueTableT = DataQualityGlueTable>
  DataSource& WithDataQualityGlueTable(DataQualityGlueTableT&& value) {
    SetDataQualityGlueTable(std::forward<DataQualityGlueTableT>(value));
    return *this;
  }
  ///@}
 private:
