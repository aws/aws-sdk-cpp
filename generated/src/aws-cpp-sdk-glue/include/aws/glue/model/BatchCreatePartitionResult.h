...
class BatchCreatePartitionResult {
 public:
  ...
  template <typename ErrorsT = Aws::Vector<PartitionError>>
  BatchCreatePartitionResult& WithErrors(ErrorsT&& value) {
    SetErrors(std::forward<ErrorsT>(value));
    return *this;
  }
  template <typename ErrorsT = PartitionError>
  BatchCreatePartitionResult& AddErrors(ErrorsT&& value) {
    m_errorsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchCreatePartitionResult& WithRequestId(RequestIdT&& value) {
