...
class ListEntitiesResult {
 public:
  ...
  template <typename EntitiesT = Aws::Vector<Entity>>
  ListEntitiesResult& WithEntities(EntitiesT&& value) {
    SetEntities(std::forward<EntitiesT>(value));
    return *this;
  }
  template <typename EntitiesT = Entity>
  ListEntitiesResult& AddEntities(EntitiesT&& value) {
    m_entitiesHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  ListEntitiesResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  ListEntitiesResult& WithRequestId(RequestIdT&& value) {
