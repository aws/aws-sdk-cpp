...
class GetPartitionsResult {
 public:
  ...
  template <typename PartitionsT = Aws::Vector<Partition>>
  GetPartitionsResult& WithPartitions(PartitionsT&& value) {
    SetPartitions(std::forward<PartitionsT>(value));
    return *this;
  }
  template <typename PartitionsT = Partition>
  GetPartitionsResult& AddPartitions(PartitionsT&& value) {
    m_partitionsHasBeenSet = true;
  ...
  template <typename NextTokenT = Aws::String>
  GetPartitionsResult& WithNextToken(NextTokenT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  GetPartitionsResult& WithRequestId(RequestIdT&& value) {
