...
class ValidateSolFunctionPackageContentMetadata {
 public:
  AWS_TNB_API ValidateSolFunctionPackageContentMetadata() = default;
  ...
  void SetVnfd(VnfdT&& value) {
    m_vnfdHasBeenSet = true;
    m_vnfd = std::forward<VnfdT>(value);
  }
  template <typename VnfdT = FunctionArtifactMeta>
  ValidateSolFunctionPackageContentMetadata& WithVnfd(VnfdT&& value) {
    SetVnfd(std::forward<VnfdT>(value));
    return *this;
  }
  ///@}
 private:
