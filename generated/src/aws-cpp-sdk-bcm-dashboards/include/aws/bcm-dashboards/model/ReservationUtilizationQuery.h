...
class ReservationUtilizationQuery {
 public:
  ...
  template <typename TimeRangeT = DateTimeRange>
  ReservationUtilizationQuery& WithTimeRange(TimeRangeT&& value) {
  ...
  template <typename GroupByT = Aws::Vector<GroupDefinition>>
  ReservationUtilizationQuery& WithGroupBy(GroupByT&& value) {
    SetGroupBy(std::forward<GroupByT>(value));
    return *this;
  }
  template <typename GroupByT = GroupDefinition>
  ReservationUtilizationQuery& AddGroupBy(GroupByT&& value) {
    m_groupByHasBeenSet = true;
  ...
  inline ReservationUtilizationQuery& WithGranularity(Granularity value) {
  ...
  template <typename FilterT = Expression>
  ReservationUtilizationQuery& WithFilter(FilterT&& value) {
