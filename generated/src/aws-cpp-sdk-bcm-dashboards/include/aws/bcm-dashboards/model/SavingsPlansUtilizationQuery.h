...
class SavingsPlansUtilizationQuery {
 public:
  ...
  template <typename TimeRangeT = DateTimeRange>
  SavingsPlansUtilizationQuery& WithTimeRange(TimeRangeT&& value) {
  ...
  inline SavingsPlansUtilizationQuery& WithGranularity(Granularity value) {
  ...
  template <typename FilterT = Expression>
  SavingsPlansUtilizationQuery& WithFilter(FilterT&& value) {
