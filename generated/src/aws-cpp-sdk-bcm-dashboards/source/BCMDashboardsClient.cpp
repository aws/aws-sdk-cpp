...
BCMDashboardsClient::BCMDashboardsClient(const AWSCredentials& credentials, const Client::ClientConfiguration& clientConfiguration)
    : BASECLASS(clientConfiguration,
                Aws::MakeShared<AWSAuthV4Signer>(ALLOCATION_TAG, Aws::MakeShared<SimpleAWSCredentialsProvider>(ALLOCATION_TAG, credentials),
                                                 SERVICE_NAME, Aws::Region::ComputeSignerRegion(clientConfiguration.region)),
                Aws::MakeShared<BCMDashboardsErrorMarshaller>(ALLOCATION_TAG)),
...
void BCMDashboardsClient::init(const BCMDashboards::BCMDashboardsClientConfiguration& config) {
  ...
  if (!config.endpointOverride.empty()) {
    ...
    Aws::Config::EndpointResolver::EndpointSource("bcm-dashboards", config.profileName, *m_endpointProvider);
  }
}
