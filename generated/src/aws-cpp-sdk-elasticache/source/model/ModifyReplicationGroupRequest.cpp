  ...
  if (m_userGroupIdsToAddHasBeenSet) {
    if (m_userGroupIdsToAdd.empty()) {
      ...
      for (auto& item : m_userGroupIdsToAdd) {
        ss << "UserGroupIdsToAdd.member." << userGroupIdsToAddCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_userGroupIdsToRemoveHasBeenSet) {
    if (m_userGroupIdsToRemove.empty()) {
      ...
      for (auto& item : m_userGroupIdsToRemove) {
        ss << "UserGroupIdsToRemove.member." << userGroupIdsToRemoveCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void ModifyReplicationGroupRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
