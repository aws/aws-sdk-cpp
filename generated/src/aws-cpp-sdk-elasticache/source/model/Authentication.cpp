...
Authentication::Authentication(const XmlNode& xmlNode) { *this = xmlNode; }
...
Authentication& Authentication::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = AuthenticationTypeMapper::GetAuthenticationTypeForName(
    ...
    if (!passwordCountNode.IsNull()) {
      m_passwordCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(passwordCountNode.GetText()).c_str()).c_str());
...
void Authentication::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(AuthenticationTypeMapper::GetNameForAuthenticationType(m_type)) << "&";
  }
