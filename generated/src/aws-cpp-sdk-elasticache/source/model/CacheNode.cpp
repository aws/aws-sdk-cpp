...
CacheNode::CacheNode(const XmlNode& xmlNode) { *this = xmlNode; }
...
CacheNode& CacheNode::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!cacheNodeCreateTimeNode.IsNull()) {
      m_cacheNodeCreateTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(cacheNodeCreateTimeNode.GetText()).c_str()).c_str(),
...
void CacheNode::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_cacheNodeCreateTimeHasBeenSet) {
    oStream << location << index << locationValue << ".CacheNodeCreateTime="
            << StringUtils::URLEncode(m_cacheNodeCreateTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  ...
  if (m_customerAvailabilityZoneHasBeenSet) {
    oStream << location << index << locationValue
            << ".CustomerAvailabilityZone=" << StringUtils::URLEncode(m_customerAvailabilityZone.c_str()) << "&";
  }
...
void CacheNode::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_cacheNodeCreateTimeHasBeenSet) {
    oStream << location << ".CacheNodeCreateTime="
            << StringUtils::URLEncode(m_cacheNodeCreateTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  if (m_endpointHasBeenSet) {
