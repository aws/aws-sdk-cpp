...
ScaleConfig::ScaleConfig(const XmlNode& xmlNode) { *this = xmlNode; }
...
ScaleConfig& ScaleConfig::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!scalePercentageNode.IsNull()) {
      m_scalePercentage = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(scalePercentageNode.GetText()).c_str()).c_str());
    ...
    if (!scaleIntervalMinutesNode.IsNull()) {
      m_scaleIntervalMinutes = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(scaleIntervalMinutesNode.GetText()).c_str()).c_str());
