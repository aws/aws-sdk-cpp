  ...
  if (m_snapshotArnsToRestoreHasBeenSet) {
    if (m_snapshotArnsToRestore.empty()) {
      ...
      for (auto& item : m_snapshotArnsToRestore) {
        ss << "SnapshotArnsToRestore.SnapshotArn." << snapshotArnsToRestoreCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_subnetIdsHasBeenSet) {
    if (m_subnetIds.empty()) {
      ...
      for (auto& item : m_subnetIds) {
        ss << "SubnetIds.SubnetId." << subnetIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        subnetIdsCount++;
      }
    }
  }
...
void CreateServerlessCacheRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
