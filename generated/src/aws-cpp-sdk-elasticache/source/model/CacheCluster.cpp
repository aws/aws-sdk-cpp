...
CacheCluster::CacheCluster(const XmlNode& xmlNode) { *this = xmlNode; }
...
CacheCluster& CacheCluster::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!replicationGroupLogDeliveryEnabledNode.IsNull()) {
      m_replicationGroupLogDeliveryEnabled = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(replicationGroupLogDeliveryEnabledNode.GetText()).c_str()).c_str());
...
void CacheCluster::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_clientDownloadLandingPageHasBeenSet) {
    oStream << location << index << locationValue
            << ".ClientDownloadLandingPage=" << StringUtils::URLEncode(m_clientDownloadLandingPage.c_str()) << "&";
  }
  ...
  if (m_replicationGroupLogDeliveryEnabledHasBeenSet) {
    oStream << location << index << locationValue << ".ReplicationGroupLogDeliveryEnabled=" << std::boolalpha
            << m_replicationGroupLogDeliveryEnabled << "&";
  }
