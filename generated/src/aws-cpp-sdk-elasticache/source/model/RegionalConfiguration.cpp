...
RegionalConfiguration::RegionalConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
...
void RegionalConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_reshardingConfigurationHasBeenSet) {
    ...
    for (auto& item : m_reshardingConfiguration) {
      ...
      reshardingConfigurationSs << location << index << locationValue << ".ReshardingConfiguration.ReshardingConfiguration."
                                << reshardingConfigurationIdx++;
