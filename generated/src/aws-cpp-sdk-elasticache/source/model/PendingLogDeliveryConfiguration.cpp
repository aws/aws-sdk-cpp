...
PendingLogDeliveryConfiguration::PendingLogDeliveryConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!destinationTypeNode.IsNull()) {
      m_destinationType = DestinationTypeMapper::GetDestinationTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(destinationTypeNode.GetText()).c_str()));
    ...
    if (!logFormatNode.IsNull()) {
      m_logFormat =
          LogFormatMapper::GetLogFormatForName(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(logFormatNode.GetText()).c_str()));
...
void PendingLogDeliveryConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_destinationTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".DestinationType=" << StringUtils::URLEncode(DestinationTypeMapper::GetNameForDestinationType(m_destinationType)) << "&";
  }
  ...
  if (m_logFormatHasBeenSet) {
    oStream << location << index << locationValue
            << ".LogFormat=" << StringUtils::URLEncode(LogFormatMapper::GetNameForLogFormat(m_logFormat)) << "&";
  }
}
