...
Aws::String ModifyCacheClusterRequest::SerializePayload() const {
  ...
  if (m_cacheNodeIdsToRemoveHasBeenSet) {
    if (m_cacheNodeIdsToRemove.empty()) {
      ...
      for (auto& item : m_cacheNodeIdsToRemove) {
        ss << "CacheNodeIdsToRemove.CacheNodeId." << cacheNodeIdsToRemoveCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_newAvailabilityZonesHasBeenSet) {
    if (m_newAvailabilityZones.empty()) {
      ...
      for (auto& item : m_newAvailabilityZones) {
        ss << "NewAvailabilityZones.PreferredAvailabilityZone." << newAvailabilityZonesCount << "=" << StringUtils::URLEncode(item.c_str())
           << "&";
  ...
  if (m_authTokenUpdateStrategyHasBeenSet) {
    ss << "AuthTokenUpdateStrategy="
       << StringUtils::URLEncode(AuthTokenUpdateStrategyTypeMapper::GetNameForAuthTokenUpdateStrategyType(m_authTokenUpdateStrategy))
       << "&";
  }
...
void ModifyCacheClusterRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
