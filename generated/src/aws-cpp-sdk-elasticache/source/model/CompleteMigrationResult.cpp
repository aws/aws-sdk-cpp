...
CompleteMigrationResult::CompleteMigrationResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }

CompleteMigrationResult& CompleteMigrationResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "CompleteMigrationResult")) {
    resultNode = rootNode.FirstChild("CompleteMigrationResult");
  }
