...
ReservedCacheNode::ReservedCacheNode(const XmlNode& xmlNode) { *this = xmlNode; }
...
ReservedCacheNode& ReservedCacheNode::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!cacheNodeCountNode.IsNull()) {
      m_cacheNodeCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(cacheNodeCountNode.GetText()).c_str()).c_str());
...
void ReservedCacheNode::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_reservedCacheNodesOfferingIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".ReservedCacheNodesOfferingId=" << StringUtils::URLEncode(m_reservedCacheNodesOfferingId.c_str()) << "&";
  }
