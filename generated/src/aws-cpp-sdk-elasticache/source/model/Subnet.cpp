...
Subnet::Subnet(const XmlNode& xmlNode) { *this = xmlNode; }
    ...
    if (!supportedNetworkTypesNode.IsNull()) {
      ...
      while (!supportedNetworkTypesMember.IsNull()) {
        m_supportedNetworkTypes.push_back(
            NetworkTypeMapper::GetNetworkTypeForName(StringUtils::Trim(supportedNetworkTypesMember.GetText().c_str())));
  ...
  if (m_supportedNetworkTypesHasBeenSet) {
    ...
    for (auto& item : m_supportedNetworkTypes) {
      oStream << location << index << locationValue << ".SupportedNetworkTypes.member." << supportedNetworkTypesIdx++ << "="
              << StringUtils::URLEncode(NetworkTypeMapper::GetNameForNetworkType(item)) << "&";
    }
  }
}
...
void Subnet::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_supportedNetworkTypesHasBeenSet) {
    unsigned supportedNetworkTypesIdx = 1;
    for (auto& item : m_supportedNetworkTypes) {
      oStream << location << ".SupportedNetworkTypes.member." << supportedNetworkTypesIdx++ << "="
              << StringUtils::URLEncode(NetworkTypeMapper::GetNameForNetworkType(item)) << "&";
    }
  }
}
