...
ConfigureShard::ConfigureShard(const XmlNode& xmlNode) { *this = xmlNode; }
...
ConfigureShard& ConfigureShard::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!newReplicaCountNode.IsNull()) {
      m_newReplicaCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(newReplicaCountNode.GetText()).c_str()).c_str());
...
void ConfigureShard::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_preferredAvailabilityZonesHasBeenSet) {
    ...
    for (auto& item : m_preferredAvailabilityZones) {
      oStream << location << index << locationValue << ".PreferredAvailabilityZones.PreferredAvailabilityZone."
              << preferredAvailabilityZonesIdx++ << "=" << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
  ...
  if (m_preferredOutpostArnsHasBeenSet) {
    ...
    for (auto& item : m_preferredOutpostArns) {
      oStream << location << index << locationValue << ".PreferredOutpostArns.PreferredOutpostArn." << preferredOutpostArnsIdx++ << "="
...
void ConfigureShard::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_preferredAvailabilityZonesHasBeenSet) {
    unsigned preferredAvailabilityZonesIdx = 1;
    for (auto& item : m_preferredAvailabilityZones) {
      oStream << location << ".PreferredAvailabilityZones.PreferredAvailabilityZone." << preferredAvailabilityZonesIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
  if (m_preferredOutpostArnsHasBeenSet) {
    unsigned preferredOutpostArnsIdx = 1;
    for (auto& item : m_preferredOutpostArns) {
      oStream << location << ".PreferredOutpostArns.PreferredOutpostArn." << preferredOutpostArnsIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
}
