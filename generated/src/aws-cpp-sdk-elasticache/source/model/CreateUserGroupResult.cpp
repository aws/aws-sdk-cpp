...
CreateUserGroupResult::CreateUserGroupResult(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  *this = result;
}
...
CreateUserGroupResult& CreateUserGroupResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "CreateUserGroupResult"))
  {
    resultNode = rootNode.FirstChild("CreateUserGroupResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::ElastiCache::Model::CreateUserGroupResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
