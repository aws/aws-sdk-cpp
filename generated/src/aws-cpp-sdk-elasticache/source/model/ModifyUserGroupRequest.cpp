  ...
  if (m_userIdsToAddHasBeenSet) {
    if (m_userIdsToAdd.empty()) {
      ...
      for (auto& item : m_userIdsToAdd) {
        ss << "UserIdsToAdd.member." << userIdsToAddCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        userIdsToAddCount++;
      }
    }
  }
  ...
  if (m_userIdsToRemoveHasBeenSet) {
    if (m_userIdsToRemove.empty()) {
      ...
      for (auto& item : m_userIdsToRemove) {
        ss << "UserIdsToRemove.member." << userIdsToRemoveCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void ModifyUserGroupRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
