...
AuthenticationMode::AuthenticationMode(const XmlNode& xmlNode) { *this = xmlNode; }
...
AuthenticationMode& AuthenticationMode::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!typeNode.IsNull()) {
      m_type = InputAuthenticationTypeMapper::GetInputAuthenticationTypeForName(
...
void AuthenticationMode::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_typeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Type=" << StringUtils::URLEncode(InputAuthenticationTypeMapper::GetNameForInputAuthenticationType(m_type)) << "&";
  }
