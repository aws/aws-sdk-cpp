  ...
  if (m_replicationGroupIdsHasBeenSet) {
    if (m_replicationGroupIds.empty()) {
      ...
      for (auto& item : m_replicationGroupIds) {
        ss << "ReplicationGroupIds.member." << replicationGroupIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
  ...
  if (m_cacheClusterIdsHasBeenSet) {
    if (m_cacheClusterIds.empty()) {
      ...
      for (auto& item : m_cacheClusterIds) {
        ss << "CacheClusterIds.member." << cacheClusterIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void BatchApplyUpdateActionRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
