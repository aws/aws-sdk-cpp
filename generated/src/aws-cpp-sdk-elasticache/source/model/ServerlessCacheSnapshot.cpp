...
ServerlessCacheSnapshot::ServerlessCacheSnapshot(const XmlNode& xmlNode) { *this = xmlNode; }
...
ServerlessCacheSnapshot& ServerlessCacheSnapshot::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!expiryTimeNode.IsNull()) {
      m_expiryTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(expiryTimeNode.GetText()).c_str()).c_str(),
                              Aws::Utils::DateFormat::ISO_8601);
...
void ServerlessCacheSnapshot::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_serverlessCacheSnapshotNameHasBeenSet) {
    oStream << location << index << locationValue
            << ".ServerlessCacheSnapshotName=" << StringUtils::URLEncode(m_serverlessCacheSnapshotName.c_str()) << "&";
  }
  ...
  if (m_expiryTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".ExpiryTime=" << StringUtils::URLEncode(m_expiryTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
