...
GlobalReplicationGroup::GlobalReplicationGroup(const XmlNode& xmlNode) { *this = xmlNode; }
...
void GlobalReplicationGroup::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_globalReplicationGroupDescriptionHasBeenSet) {
    oStream << location << index << locationValue
            << ".GlobalReplicationGroupDescription=" << StringUtils::URLEncode(m_globalReplicationGroupDescription.c_str()) << "&";
  }
