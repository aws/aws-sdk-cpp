...
class DeleteTagsRequest : public MachineLearningRequest {
 public:
  ...
  template <typename TagKeysT = Aws::Vector<Aws::String>>
  DeleteTagsRequest& WithTagKeys(TagKeysT&& value) {
    SetTagKeys(std::forward<TagKeysT>(value));
    return *this;
  }
  template <typename TagKeysT = Aws::String>
  DeleteTagsRequest& AddTagKeys(TagKeysT&& value) {
    m_tagKeysHasBeenSet = true;
  ...
  template <typename ResourceIdT = Aws::String>
  DeleteTagsRequest& WithResourceId(ResourceIdT&& value) {
  ...
  inline DeleteTagsRequest& WithResourceType(TaggableResourceType value) {
