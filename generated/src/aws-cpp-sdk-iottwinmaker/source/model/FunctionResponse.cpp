...
FunctionResponse::FunctionResponse(JsonView jsonValue) { *this = jsonValue; }
...
FunctionResponse& FunctionResponse::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("requiredProperties")) {
    ...
    for (unsigned requiredPropertiesIndex = 0; requiredPropertiesIndex < requiredPropertiesJsonList.GetLength();
         ++requiredPropertiesIndex) {
