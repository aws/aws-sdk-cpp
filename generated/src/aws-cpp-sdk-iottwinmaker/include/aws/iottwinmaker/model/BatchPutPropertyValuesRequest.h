...
class BatchPutPropertyValuesRequest : public IoTTwinMakerRequest {
 public:
  ...
  template <typename WorkspaceIdT = Aws::String>
  BatchPutPropertyValuesRequest& WithWorkspaceId(WorkspaceIdT&& value) {
  ...
  template <typename EntriesT = Aws::Vector<PropertyValueEntry>>
  BatchPutPropertyValuesRequest& WithEntries(EntriesT&& value) {
    SetEntries(std::forward<EntriesT>(value));
    return *this;
  }
  template <typename EntriesT = PropertyValueEntry>
  BatchPutPropertyValuesRequest& AddEntries(EntriesT&& value) {
    m_entriesHasBeenSet = true;
