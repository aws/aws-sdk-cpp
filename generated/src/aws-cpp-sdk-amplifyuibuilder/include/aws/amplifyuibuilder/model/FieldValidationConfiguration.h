...
class FieldValidationConfiguration {
 public:
  ...
  void SetNumValues(NumValuesT&& value) {
    ...
    m_numValues = std::forward<NumValuesT>(value);
  }
  template <typename NumValuesT = Aws::Vector<int>>
  FieldValidationConfiguration& WithNumValues(NumValuesT&& value) {
    SetNumValues(std::forward<NumValuesT>(value));
    return *this;
  }
  inline FieldValidationConfiguration& AddNumValues(int value) {
    m_numValuesHasBeenSet = true;
