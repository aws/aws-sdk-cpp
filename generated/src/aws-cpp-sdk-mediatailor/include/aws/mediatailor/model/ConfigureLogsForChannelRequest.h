...
class ConfigureLogsForChannelRequest : public MediaTailorRequest {
 public:
  ...
  template <typename ChannelNameT = Aws::String>
  ConfigureLogsForChannelRequest& WithChannelName(ChannelNameT&& value) {
  ...
  template <typename LogTypesT = Aws::Vector<LogType>>
  ConfigureLogsForChannelRequest& WithLogTypes(LogTypesT&& value) {
    SetLogTypes(std::forward<LogTypesT>(value));
    return *this;
  }
  inline ConfigureLogsForChannelRequest& AddLogTypes(LogType value) {
    m_logTypesHasBeenSet = true;
