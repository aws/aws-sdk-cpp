...
AdsInteractionLog::AdsInteractionLog(JsonView jsonValue) { *this = jsonValue; }
...
AdsInteractionLog& AdsInteractionLog::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("PublishOptInEventTypes")) {
    ...
    for (unsigned publishOptInEventTypesIndex = 0; publishOptInEventTypesIndex < publishOptInEventTypesJsonList.GetLength();
      ...
      m_publishOptInEventTypes.push_back(AdsInteractionPublishOptInEventTypeMapper::GetAdsInteractionPublishOptInEventTypeForName(
  ...
  if (jsonValue.ValueExists("ExcludeEventTypes")) {
    ...
    for (unsigned excludeEventTypesIndex = 0; excludeEventTypesIndex < excludeEventTypesJsonList.GetLength(); ++excludeEventTypesIndex) {
      m_excludeEventTypes.push_back(AdsInteractionExcludeEventTypeMapper::GetAdsInteractionExcludeEventTypeForName(
...
JsonValue AdsInteractionLog::Jsonize() const {
  ...
  if (m_publishOptInEventTypesHasBeenSet) {
    ...
    for (unsigned publishOptInEventTypesIndex = 0; publishOptInEventTypesIndex < publishOptInEventTypesJsonList.GetLength();
         ++publishOptInEventTypesIndex) {
      publishOptInEventTypesJsonList[publishOptInEventTypesIndex].AsString(
          AdsInteractionPublishOptInEventTypeMapper::GetNameForAdsInteractionPublishOptInEventType(
  ...
  if (m_excludeEventTypesHasBeenSet) {
    ...
    for (unsigned excludeEventTypesIndex = 0; excludeEventTypesIndex < excludeEventTypesJsonList.GetLength(); ++excludeEventTypesIndex) {
      excludeEventTypesJsonList[excludeEventTypesIndex].AsString(
          AdsInteractionExcludeEventTypeMapper::GetNameForAdsInteractionExcludeEventType(m_excludeEventTypes[excludeEventTypesIndex]));
    }
