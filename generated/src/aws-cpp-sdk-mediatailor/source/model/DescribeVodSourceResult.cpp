...
DescribeVodSourceResult::DescribeVodSourceResult(const Aws::AmazonWebServiceResult<JsonValue>& result) { *this = result; }
...
DescribeVodSourceResult& DescribeVodSourceResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("AdBreakOpportunities")) {
    ...
    for (unsigned adBreakOpportunitiesIndex = 0; adBreakOpportunitiesIndex < adBreakOpportunitiesJsonList.GetLength();
         ++adBreakOpportunitiesIndex) {
      m_adBreakOpportunities.push_back(adBreakOpportunitiesJsonList[adBreakOpportunitiesIndex].AsObject());
    }
    m_adBreakOpportunitiesHasBeenSet = true;
  }
