...
ConfigureLogsForPlaybackConfigurationResult& ConfigureLogsForPlaybackConfigurationResult::operator=(
    const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("EnabledLoggingStrategies")) {
    ...
    for (unsigned enabledLoggingStrategiesIndex = 0; enabledLoggingStrategiesIndex < enabledLoggingStrategiesJsonList.GetLength();
         ++enabledLoggingStrategiesIndex) {
      m_enabledLoggingStrategies.push_back(
          LoggingStrategyMapper::GetLoggingStrategyForName(enabledLoggingStrategiesJsonList[enabledLoggingStrategiesIndex].AsString()));
    }
