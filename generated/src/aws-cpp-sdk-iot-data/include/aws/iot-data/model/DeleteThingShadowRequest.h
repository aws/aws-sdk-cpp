...
class DeleteThingShadowRequest : public IoTDataPlaneRequest {
 public:
  ...
  template <typename ThingNameT = Aws::String>
  DeleteThingShadowRequest& WithThingName(ThingNameT&& value) {
  ...
  void SetShadowName(ShadowNameT&& value) {
    m_shadowNameHasBeenSet = true;
    m_shadowName = std::forward<ShadowNameT>(value);
  }
  template <typename ShadowNameT = Aws::String>
  DeleteThingShadowRequest& WithShadowName(ShadowNameT&& value) {
    SetShadowName(std::forward<ShadowNameT>(value));
    return *this;
  }
  ///@}
 private:
