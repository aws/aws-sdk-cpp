...
class InspectorScoreDetails {
 public:
  ...
  void SetAdjustedCvss(AdjustedCvssT&& value) {
    m_adjustedCvssHasBeenSet = true;
    m_adjustedCvss = std::forward<AdjustedCvssT>(value);
  }
  template <typename AdjustedCvssT = CvssScoreDetails>
  InspectorScoreDetails& WithAdjustedCvss(AdjustedCvssT&& value) {
    SetAdjustedCvss(std::forward<AdjustedCvssT>(value));
    return *this;
  }
  ///@}
 private:
