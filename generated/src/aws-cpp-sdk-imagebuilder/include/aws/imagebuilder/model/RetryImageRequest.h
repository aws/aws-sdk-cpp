...
/**
 */
class RetryImageRequest : public ImagebuilderRequest {
 public:
  AWS_IMAGEBUILDER_API RetryImageRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "RetryImage"; }

  AWS_IMAGEBUILDER_API Aws::String SerializePayload() const override;

  ///@{
  /**
   * <p>The Amazon Resource Name (ARN) of the image build version that you want to
   * retry. The image must be in the <code>FAILED</code> or <code>CANCELLED</code>
   * state.</p>
   */
  ...
  template <typename ImageBuildVersionArnT = Aws::String>
  RetryImageRequest& WithImageBuildVersionArn(ImageBuildVersionArnT&& value) {
  ...
  template <typename ClientTokenT = Aws::String>
  RetryImageRequest& WithClientToken(ClientTokenT&& value) {
