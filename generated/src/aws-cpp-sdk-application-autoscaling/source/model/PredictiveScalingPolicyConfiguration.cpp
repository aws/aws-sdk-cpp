...
PredictiveScalingPolicyConfiguration::PredictiveScalingPolicyConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
PredictiveScalingPolicyConfiguration& PredictiveScalingPolicyConfiguration::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("MetricSpecifications")) {
    ...
    for (unsigned metricSpecificationsIndex = 0; metricSpecificationsIndex < metricSpecificationsJsonList.GetLength();
         ++metricSpecificationsIndex) {
  ...
  if (jsonValue.ValueExists("MaxCapacityBreachBehavior")) {
    m_maxCapacityBreachBehavior = PredictiveScalingMaxCapacityBreachBehaviorMapper::GetPredictiveScalingMaxCapacityBreachBehaviorForName(
        jsonValue.GetString("MaxCapacityBreachBehavior"));
...
JsonValue PredictiveScalingPolicyConfiguration::Jsonize() const {
  ...
  if (m_maxCapacityBreachBehaviorHasBeenSet) {
    payload.WithString("MaxCapacityBreachBehavior",
                       PredictiveScalingMaxCapacityBreachBehaviorMapper::GetNameForPredictiveScalingMaxCapacityBreachBehavior(
                           m_maxCapacityBreachBehavior));
  }
