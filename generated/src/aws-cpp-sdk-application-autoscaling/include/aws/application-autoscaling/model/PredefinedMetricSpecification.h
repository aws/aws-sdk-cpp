...
class PredefinedMetricSpecification {
 public:
   ...
   * <ul> <li> <p> <code>ALBRequestCountPerTarget</code> - This metric type applies
   * only to Spot Fleets and ECS services.</p> </li> <li> <p>
   * <code>ECSServiceAverageCPUUtilizationHighResolution</code> - The high-resolution
   * version of <code>ECSServiceAverageCPUUtilization</code> that uses 20-second
   * CloudWatch metrics. Use this metric for target tracking scaling policies that
   * evaluate metrics every 20 seconds. You must enable high-resolution metrics in
   * Amazon ECS before creating a scaling policy with this metric type.</p> </li>
   * <li> <p> <code>ECSServiceAverageMemoryUtilizationHighResolution</code> - The
   * high-resolution version of <code>ECSServiceAverageMemoryUtilization</code> that
   * uses 20-second CloudWatch metrics. Use this metric for target tracking scaling
   * policies that evaluate metrics every 20 seconds. You must enable high-resolution
   * metrics in Amazon ECS before creating a scaling policy with this metric
   * type.</p> </li> </ul>
   */
