...
class LoadForecast {
 public:
  ...
  inline LoadForecast& AddValues(double value) {
    m_valuesHasBeenSet = true;
  ...
  void SetMetricSpecification(MetricSpecificationT&& value) {
    ...
    m_metricSpecification = std::forward<MetricSpecificationT>(value);
  }
  template <typename MetricSpecificationT = PredictiveScalingMetricSpecification>
  LoadForecast& WithMetricSpecification(MetricSpecificationT&& value) {
    SetMetricSpecification(std::forward<MetricSpecificationT>(value));
    return *this;
  }
  ///@}
 private:
