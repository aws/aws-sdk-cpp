  ...
  if (m_snapshotIdentifierListHasBeenSet) {
    if (m_snapshotIdentifierList.empty()) {
      ...
      for (auto& item : m_snapshotIdentifierList) {
        ss << "SnapshotIdentifierList.String." << snapshotIdentifierListCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void BatchModifyClusterSnapshotsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
