...
SnapshotSortingEntity::SnapshotSortingEntity(const XmlNode& xmlNode) { *this = xmlNode; }
...
SnapshotSortingEntity& SnapshotSortingEntity::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!attributeNode.IsNull()) {
      m_attribute = SnapshotAttributeToSortByMapper::GetSnapshotAttributeToSortByForName(
    ...
    if (!sortOrderNode.IsNull()) {
      m_sortOrder = SortByOrderMapper::GetSortByOrderForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sortOrderNode.GetText()).c_str()));
...
void SnapshotSortingEntity::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_attributeHasBeenSet) {
    oStream << location << index << locationValue
            << ".Attribute=" << StringUtils::URLEncode(SnapshotAttributeToSortByMapper::GetNameForSnapshotAttributeToSortBy(m_attribute))
            << "&";
  }

  if (m_sortOrderHasBeenSet) {
    oStream << location << index << locationValue
            << ".SortOrder=" << StringUtils::URLEncode(SortByOrderMapper::GetNameForSortByOrder(m_sortOrder)) << "&";
  }
}
