...
EndpointAccess::EndpointAccess(const XmlNode& xmlNode) { *this = xmlNode; }
...
EndpointAccess& EndpointAccess::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!endpointCreateTimeNode.IsNull()) {
      m_endpointCreateTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(endpointCreateTimeNode.GetText()).c_str()).c_str(),
...
void EndpointAccess::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_endpointCreateTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".EndpointCreateTime=" << StringUtils::URLEncode(m_endpointCreateTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
