...
Recommendation::Recommendation(const XmlNode& xmlNode) { *this = xmlNode; }
...
Recommendation& Recommendation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!impactRankingNode.IsNull()) {
      m_impactRanking = ImpactRankingTypeMapper::GetImpactRankingTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(impactRankingNode.GetText()).c_str()));
...
void Recommendation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_impactRankingHasBeenSet) {
    oStream << location << index << locationValue
            << ".ImpactRanking=" << StringUtils::URLEncode(ImpactRankingTypeMapper::GetNameForImpactRankingType(m_impactRanking)) << "&";
  }
