...
RevisionTarget::RevisionTarget(const XmlNode& xmlNode) { *this = xmlNode; }
...
RevisionTarget& RevisionTarget::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!databaseRevisionReleaseDateNode.IsNull()) {
      m_databaseRevisionReleaseDate =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(databaseRevisionReleaseDateNode.GetText()).c_str()).c_str(),
...
void RevisionTarget::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_databaseRevisionReleaseDateHasBeenSet) {
    oStream << location << index << locationValue << ".DatabaseRevisionReleaseDate="
            << StringUtils::URLEncode(m_databaseRevisionReleaseDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
...
void RevisionTarget::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_databaseRevisionReleaseDateHasBeenSet) {
    oStream << location << ".DatabaseRevisionReleaseDate="
            << StringUtils::URLEncode(m_databaseRevisionReleaseDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
