  ...
  if (m_clusterSecurityGroupsHasBeenSet) {
    if (m_clusterSecurityGroups.empty()) {
      ...
      for (auto& item : m_clusterSecurityGroups) {
        ss << "ClusterSecurityGroups.ClusterSecurityGroupName." << clusterSecurityGroupsCount << "=" << StringUtils::URLEncode(item.c_str())
           << "&";
  ...
  if (m_iamRolesHasBeenSet) {
    if (m_iamRoles.empty()) {
      ...
      for (auto& item : m_iamRoles) {
        ss << "IamRoles.IamRoleArn." << iamRolesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        iamRolesCount++;
      }
    }
  }
...
void CreateClusterRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
