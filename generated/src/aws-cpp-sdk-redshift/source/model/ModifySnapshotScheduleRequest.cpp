  ...
  if (m_scheduleDefinitionsHasBeenSet) {
    if (m_scheduleDefinitions.empty()) {
      ...
      for (auto& item : m_scheduleDefinitions) {
        ss << "ScheduleDefinitions.ScheduleDefinition." << scheduleDefinitionsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void ModifySnapshotScheduleRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
