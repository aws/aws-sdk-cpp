...
ScheduledAction::ScheduledAction(const XmlNode& xmlNode) { *this = xmlNode; }
...
ScheduledAction& ScheduledAction::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!stateNode.IsNull()) {
      m_state = ScheduledActionStateMapper::GetScheduledActionStateForName(
    ...
    if (!nextInvocationsNode.IsNull()) {
      ...
      while (!nextInvocationsMember.IsNull()) {
        m_nextInvocations.push_back(
            DateTime(StringUtils::Trim(nextInvocationsMember.GetText().c_str()).c_str(), Aws::Utils::DateFormat::ISO_8601));
...
void ScheduledAction::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_scheduledActionDescriptionHasBeenSet) {
    oStream << location << index << locationValue
            << ".ScheduledActionDescription=" << StringUtils::URLEncode(m_scheduledActionDescription.c_str()) << "&";
  }

  if (m_stateHasBeenSet) {
    oStream << location << index << locationValue
            << ".State=" << StringUtils::URLEncode(ScheduledActionStateMapper::GetNameForScheduledActionState(m_state)) << "&";
  }

  if (m_nextInvocationsHasBeenSet) {
    unsigned nextInvocationsIdx = 1;
    for (auto& item : m_nextInvocations) {
      oStream << location << index << locationValue << ".NextInvocations.ScheduledActionTime." << nextInvocationsIdx++ << "="
...
void ScheduledAction::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_nextInvocationsHasBeenSet) {
    unsigned nextInvocationsIdx = 1;
    for (auto& item : m_nextInvocations) {
      oStream << location << ".NextInvocations.ScheduledActionTime." << nextInvocationsIdx++ << "="
              << StringUtils::URLEncode(item.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
    }
  }
  if (m_startTimeHasBeenSet) {
