...
ReservedNodeExchangeStatus::ReservedNodeExchangeStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
ReservedNodeExchangeStatus& ReservedNodeExchangeStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = ReservedNodeExchangeStatusTypeMapper::GetReservedNodeExchangeStatusTypeForName(
    ...
    if (!requestTimeNode.IsNull()) {
      m_requestTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(requestTimeNode.GetText()).c_str()).c_str(),
    ...
    if (!sourceReservedNodeCountNode.IsNull()) {
      m_sourceReservedNodeCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sourceReservedNodeCountNode.GetText()).c_str()).c_str());
...
void ReservedNodeExchangeStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                const char* locationValue) const {
  if (m_reservedNodeExchangeRequestIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".ReservedNodeExchangeRequestId=" << StringUtils::URLEncode(m_reservedNodeExchangeRequestId.c_str()) << "&";
  }

  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue << ".Status="
            << StringUtils::URLEncode(ReservedNodeExchangeStatusTypeMapper::GetNameForReservedNodeExchangeStatusType(m_status)) << "&";
  }

  if (m_requestTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".RequestTime=" << StringUtils::URLEncode(m_requestTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  ...
  if (m_targetReservedNodeOfferingIdHasBeenSet) {
    oStream << location << index << locationValue
            << ".TargetReservedNodeOfferingId=" << StringUtils::URLEncode(m_targetReservedNodeOfferingId.c_str()) << "&";
  }
