...
ResizeInfo::ResizeInfo(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResizeInfo& ResizeInfo::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!allowCancelResizeNode.IsNull()) {
      m_allowCancelResize = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(allowCancelResizeNode.GetText()).c_str()).c_str());
