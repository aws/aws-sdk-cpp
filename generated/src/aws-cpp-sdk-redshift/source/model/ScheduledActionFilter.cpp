...
ScheduledActionFilter::ScheduledActionFilter(const XmlNode& xmlNode) { *this = xmlNode; }
...
ScheduledActionFilter& ScheduledActionFilter::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!nameNode.IsNull()) {
      m_name = ScheduledActionFilterNameMapper::GetScheduledActionFilterNameForName(
...
void ScheduledActionFilter::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_nameHasBeenSet) {
    oStream << location << index << locationValue
            << ".Name=" << StringUtils::URLEncode(ScheduledActionFilterNameMapper::GetNameForScheduledActionFilterName(m_name)) << "&";
  }
