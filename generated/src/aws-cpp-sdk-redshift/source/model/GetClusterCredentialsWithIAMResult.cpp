...
GetClusterCredentialsWithIAMResult& GetClusterCredentialsWithIAMResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!nextRefreshTimeNode.IsNull()) {
      m_nextRefreshTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(nextRefreshTimeNode.GetText()).c_str()).c_str(),
                                   Aws::Utils::DateFormat::ISO_8601);
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::Redshift::Model::GetClusterCredentialsWithIAMResult",
