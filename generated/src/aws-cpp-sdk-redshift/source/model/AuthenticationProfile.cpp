...
AuthenticationProfile::AuthenticationProfile(const XmlNode& xmlNode) { *this = xmlNode; }
...
void AuthenticationProfile::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_authenticationProfileNameHasBeenSet) {
    oStream << location << index << locationValue
            << ".AuthenticationProfileName=" << StringUtils::URLEncode(m_authenticationProfileName.c_str()) << "&";
  }

  if (m_authenticationProfileContentHasBeenSet) {
    oStream << location << index << locationValue
            << ".AuthenticationProfileContent=" << StringUtils::URLEncode(m_authenticationProfileContent.c_str()) << "&";
  }
}
