...
TableRestoreStatus::TableRestoreStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
TableRestoreStatus& TableRestoreStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = TableRestoreStatusTypeMapper::GetTableRestoreStatusTypeForName(
...
void TableRestoreStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(TableRestoreStatusTypeMapper::GetNameForTableRestoreStatusType(m_status)) << "&";
  }
