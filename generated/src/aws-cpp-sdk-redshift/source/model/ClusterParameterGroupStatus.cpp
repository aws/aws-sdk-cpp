...
ClusterParameterGroupStatus::ClusterParameterGroupStatus(const XmlNode& xmlNode) { *this = xmlNode; }
...
void ClusterParameterGroupStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_clusterParameterStatusListHasBeenSet) {
    ...
    for (auto& item : m_clusterParameterStatusList) {
      ...
      clusterParameterStatusListSs << location << index << locationValue << ".ClusterParameterStatusList.member."
                                   << clusterParameterStatusListIdx++;
