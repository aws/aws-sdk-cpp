...
NodeConfigurationOptionsFilter::NodeConfigurationOptionsFilter(const XmlNode& xmlNode) { *this = xmlNode; }
...
NodeConfigurationOptionsFilter& NodeConfigurationOptionsFilter::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!nameNode.IsNull()) {
      m_name = NodeConfigurationOptionsFilterNameMapper::GetNodeConfigurationOptionsFilterNameForName(
    ...
    if (!operatorNode.IsNull()) {
      m_operator = OperatorTypeMapper::GetOperatorTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(operatorNode.GetText()).c_str()));
...
void NodeConfigurationOptionsFilter::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                    const char* locationValue) const {
  if (m_nameHasBeenSet) {
    oStream << location << index << locationValue << ".Name="
            << StringUtils::URLEncode(NodeConfigurationOptionsFilterNameMapper::GetNameForNodeConfigurationOptionsFilterName(m_name))
            << "&";
  }

  if (m_operatorHasBeenSet) {
    oStream << location << index << locationValue
            << ".Operator=" << StringUtils::URLEncode(OperatorTypeMapper::GetNameForOperatorType(m_operator)) << "&";
  }
