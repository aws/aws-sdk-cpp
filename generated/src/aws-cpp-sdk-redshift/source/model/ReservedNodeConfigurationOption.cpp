...
ReservedNodeConfigurationOption::ReservedNodeConfigurationOption(const XmlNode& xmlNode) { *this = xmlNode; }
...
ReservedNodeConfigurationOption& ReservedNodeConfigurationOption::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!targetReservedNodeCountNode.IsNull()) {
      m_targetReservedNodeCount = StringUtils::ConvertToInt32(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(targetReservedNodeCountNode.GetText()).c_str()).c_str());
...
void ReservedNodeConfigurationOption::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
