...
DescribeLoggingStatusResult::DescribeLoggingStatusResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
DescribeLoggingStatusResult& DescribeLoggingStatusResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!loggingEnabledNode.IsNull()) {
      m_loggingEnabled = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(loggingEnabledNode.GetText()).c_str()).c_str());
    ...
    if (!lastSuccessfulDeliveryTimeNode.IsNull()) {
      m_lastSuccessfulDeliveryTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastSuccessfulDeliveryTimeNode.GetText()).c_str()).c_str(),
    ...
    if (!lastFailureTimeNode.IsNull()) {
      m_lastFailureTime = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastFailureTimeNode.GetText()).c_str()).c_str(),
                                   Aws::Utils::DateFormat::ISO_8601);
