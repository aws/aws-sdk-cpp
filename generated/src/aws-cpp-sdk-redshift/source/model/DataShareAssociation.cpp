...
DataShareAssociation::DataShareAssociation(const XmlNode& xmlNode) { *this = xmlNode; }
...
DataShareAssociation& DataShareAssociation::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!statusNode.IsNull()) {
      m_status = DataShareStatusMapper::GetDataShareStatusForName(
    ...
    if (!statusChangeDateNode.IsNull()) {
      m_statusChangeDate =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(statusChangeDateNode.GetText()).c_str()).c_str(),
                   Aws::Utils::DateFormat::ISO_8601);
    ...
    if (!producerAllowedWritesNode.IsNull()) {
      m_producerAllowedWrites = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(producerAllowedWritesNode.GetText()).c_str()).c_str());
    ...
    if (!consumerAcceptedWritesNode.IsNull()) {
      m_consumerAcceptedWrites = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(consumerAcceptedWritesNode.GetText()).c_str()).c_str());
...
void DataShareAssociation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(DataShareStatusMapper::GetNameForDataShareStatus(m_status)) << "&";
  }
  ...
  if (m_statusChangeDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".StatusChangeDate=" << StringUtils::URLEncode(m_statusChangeDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
            << "&";
  }
