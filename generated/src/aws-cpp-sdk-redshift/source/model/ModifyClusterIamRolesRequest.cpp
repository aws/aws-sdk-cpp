  ...
  if (m_addIamRolesHasBeenSet) {
    if (m_addIamRoles.empty()) {
      ...
      for (auto& item : m_addIamRoles) {
        ss << "AddIamRoles.IamRoleArn." << addIamRolesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        addIamRolesCount++;
      }
    }
  }
  ...
  if (m_removeIamRolesHasBeenSet) {
    if (m_removeIamRoles.empty()) {
      ...
      for (auto& item : m_removeIamRoles) {
        ss << "RemoveIamRoles.IamRoleArn." << removeIamRolesCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        removeIamRolesCount++;
      }
    }
  }
...
void ModifyClusterIamRolesRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
