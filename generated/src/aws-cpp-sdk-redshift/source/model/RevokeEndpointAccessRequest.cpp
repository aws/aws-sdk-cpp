  ...
  if (m_vpcIdsHasBeenSet) {
    if (m_vpcIds.empty()) {
      ...
      for (auto& item : m_vpcIds) {
        ss << "VpcIds.VpcIdentifier." << vpcIdsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        vpcIdsCount++;
      }
    }
  }
...
void RevokeEndpointAccessRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
