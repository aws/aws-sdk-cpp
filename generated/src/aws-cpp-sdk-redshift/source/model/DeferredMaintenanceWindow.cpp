...
DeferredMaintenanceWindow::DeferredMaintenanceWindow(const XmlNode& xmlNode) { *this = xmlNode; }
...
DeferredMaintenanceWindow& DeferredMaintenanceWindow::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!deferMaintenanceStartTimeNode.IsNull()) {
      m_deferMaintenanceStartTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deferMaintenanceStartTimeNode.GetText()).c_str()).c_str(),
    ...
    if (!deferMaintenanceEndTimeNode.IsNull()) {
      m_deferMaintenanceEndTime =
          DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(deferMaintenanceEndTimeNode.GetText()).c_str()).c_str(),
                   Aws::Utils::DateFormat::ISO_8601);
...
void DeferredMaintenanceWindow::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                               const char* locationValue) const {
  if (m_deferMaintenanceIdentifierHasBeenSet) {
    oStream << location << index << locationValue
            << ".DeferMaintenanceIdentifier=" << StringUtils::URLEncode(m_deferMaintenanceIdentifier.c_str()) << "&";
  }

  if (m_deferMaintenanceStartTimeHasBeenSet) {
    oStream << location << index << locationValue << ".DeferMaintenanceStartTime="
  ...
  if (m_deferMaintenanceEndTimeHasBeenSet) {
    oStream << location << index << locationValue << ".DeferMaintenanceEndTime="
...
void DeferredMaintenanceWindow::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_deferMaintenanceStartTimeHasBeenSet) {
    oStream << location << ".DeferMaintenanceStartTime="
            << StringUtils::URLEncode(m_deferMaintenanceStartTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  if (m_deferMaintenanceEndTimeHasBeenSet) {
    oStream << location << ".DeferMaintenanceEndTime="
            << StringUtils::URLEncode(m_deferMaintenanceEndTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
}
