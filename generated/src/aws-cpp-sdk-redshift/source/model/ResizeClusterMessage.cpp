...
ResizeClusterMessage::ResizeClusterMessage(const XmlNode& xmlNode) { *this = xmlNode; }
...
ResizeClusterMessage& ResizeClusterMessage::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!classicNode.IsNull()) {
      m_classic =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(classicNode.GetText()).c_str()).c_str());
