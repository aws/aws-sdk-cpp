...
Parameter& Parameter::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!applyTypeNode.IsNull()) {
      m_applyType = ParameterApplyTypeMapper::GetParameterApplyTypeForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(applyTypeNode.GetText()).c_str()));
...
void Parameter::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_applyTypeHasBeenSet) {
    oStream << location << index << locationValue
            << ".ApplyType=" << StringUtils::URLEncode(ParameterApplyTypeMapper::GetNameForParameterApplyType(m_applyType)) << "&";
  }
