...
DataTransferProgress::DataTransferProgress(const XmlNode& xmlNode) { *this = xmlNode; }
...
DataTransferProgress& DataTransferProgress::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!currentRateInMegaBytesPerSecondNode.IsNull()) {
      m_currentRateInMegaBytesPerSecond = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(currentRateInMegaBytesPerSecondNode.GetText()).c_str()).c_str());
    ...
    if (!totalDataInMegaBytesNode.IsNull()) {
      m_totalDataInMegaBytes = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(totalDataInMegaBytesNode.GetText()).c_str()).c_str());
    ...
    if (!dataTransferredInMegaBytesNode.IsNull()) {
      m_dataTransferredInMegaBytes = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(dataTransferredInMegaBytesNode.GetText()).c_str()).c_str());
...
void DataTransferProgress::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_currentRateInMegaBytesPerSecondHasBeenSet) {
    oStream << location << index << locationValue
            << ".CurrentRateInMegaBytesPerSecond=" << StringUtils::URLEncode(m_currentRateInMegaBytesPerSecond) << "&";
  }
