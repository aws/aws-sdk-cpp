...
LakeFormationQuery::LakeFormationQuery(const XmlNode& xmlNode) { *this = xmlNode; }
...
LakeFormationQuery& LakeFormationQuery::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!authorizationNode.IsNull()) {
      m_authorization = ServiceAuthorizationMapper::GetServiceAuthorizationForName(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(authorizationNode.GetText()).c_str()));
...
void LakeFormationQuery::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_authorizationHasBeenSet) {
    oStream << location << index << locationValue
            << ".Authorization=" << StringUtils::URLEncode(ServiceAuthorizationMapper::GetNameForServiceAuthorization(m_authorization))
            << "&";
  }
}
