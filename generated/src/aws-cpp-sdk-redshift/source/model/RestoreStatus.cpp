...
RestoreStatus& RestoreStatus::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!currentRestoreRateInMegaBytesPerSecondNode.IsNull()) {
      m_currentRestoreRateInMegaBytesPerSecond = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(currentRestoreRateInMegaBytesPerSecondNode.GetText()).c_str()).c_str());
    ...
    if (!snapshotSizeInMegaBytesNode.IsNull()) {
      m_snapshotSizeInMegaBytes = StringUtils::ConvertToInt64(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(snapshotSizeInMegaBytesNode.GetText()).c_str()).c_str());
...
void RestoreStatus::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_currentRestoreRateInMegaBytesPerSecondHasBeenSet) {
    oStream << location << index << locationValue
            << ".CurrentRestoreRateInMegaBytesPerSecond=" << StringUtils::URLEncode(m_currentRestoreRateInMegaBytesPerSecond) << "&";
  }
