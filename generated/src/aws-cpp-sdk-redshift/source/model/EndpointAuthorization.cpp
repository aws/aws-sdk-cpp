...
EndpointAuthorization::EndpointAuthorization(const XmlNode& xmlNode) { *this = xmlNode; }
...
void EndpointAuthorization::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_authorizeTimeHasBeenSet) {
    oStream << location << index << locationValue
            << ".AuthorizeTime=" << StringUtils::URLEncode(m_authorizeTime.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  ...
  if (m_statusHasBeenSet) {
    oStream << location << index << locationValue
            << ".Status=" << StringUtils::URLEncode(AuthorizationStatusMapper::GetNameForAuthorizationStatus(m_status)) << "&";
  }
  ...
  if (m_allowedVPCsHasBeenSet) {
    ...
    for (auto& item : m_allowedVPCs) {
      oStream << location << index << locationValue << ".AllowedVPCs.VpcIdentifier." << allowedVPCsIdx++ << "="
