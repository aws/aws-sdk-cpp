  ...
  if (m_dbGroupsHasBeenSet) {
    if (m_dbGroups.empty()) {
      ...
      for (auto& item : m_dbGroups) {
        ss << "DbGroups.DbGroup." << dbGroupsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
        dbGroupsCount++;
      }
    }
  }
...
void GetClusterCredentialsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
