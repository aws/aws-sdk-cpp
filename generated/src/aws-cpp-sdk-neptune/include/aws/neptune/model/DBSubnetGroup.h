...
class DBSubnetGroup {
 public:
  ...
  void SetDBSubnetGroupArn(DBSubnetGroupArnT&& value) {
    ...
    m_dBSubnetGroupArn = std::forward<DBSubnetGroupArnT>(value);
  }
  template <typename DBSubnetGroupArnT = Aws::String>
  DBSubnetGroup& WithDBSubnetGroupArn(DBSubnetGroupArnT&& value) {
    SetDBSubnetGroupArn(std::forward<DBSubnetGroupArnT>(value));
    return *this;
  }
  ///@}
  ...
  ///@{
  /**
   * <p>The network types supported by the DB subnet group.</p> <p>Valid network
   * types include <code>IPV4</code> and <code>DUAL</code>. A DB subnet group
   * supports <code>DUAL</code> if all subnets in the group have both IPv4 and IPv6
   * CIDRs.</p>
   */
