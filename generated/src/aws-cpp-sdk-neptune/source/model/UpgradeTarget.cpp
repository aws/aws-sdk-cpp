...
UpgradeTarget::UpgradeTarget(const XmlNode& xmlNode) { *this = xmlNode; }
...
UpgradeTarget& UpgradeTarget::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!autoUpgradeNode.IsNull()) {
      m_autoUpgrade =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(autoUpgradeNode.GetText()).c_str()).c_str());
    ...
    if (!isMajorVersionUpgradeNode.IsNull()) {
      m_isMajorVersionUpgrade = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isMajorVersionUpgradeNode.GetText()).c_str()).c_str());
