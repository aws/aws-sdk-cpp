...
CloudwatchLogsExportConfiguration::CloudwatchLogsExportConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
...
void CloudwatchLogsExportConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                       const char* locationValue) const {
  if (m_enableLogTypesHasBeenSet) {
    unsigned enableLogTypesIdx = 1;
    for (auto& item : m_enableLogTypes) {
      oStream << location << index << locationValue << ".EnableLogTypes.member." << enableLogTypesIdx++ << "="
  ...
  if (m_disableLogTypesHasBeenSet) {
    ...
    for (auto& item : m_disableLogTypes) {
      oStream << location << index << locationValue << ".DisableLogTypes.member." << disableLogTypesIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
}
