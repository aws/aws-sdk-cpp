...
class HandlerConfigs {
 public:
  ...
  void SetOnPublish(OnPublishT&& value) {
    ...
    m_onPublish = std::forward<OnPublishT>(value);
  }
  template <typename OnPublishT = HandlerConfig>
  HandlerConfigs& WithOnPublish(OnPublishT&& value) {
    SetOnPublish(std::forward<OnPublishT>(value));
    return *this;
  }
  ///@}
  ...
  void SetOnSubscribe(OnSubscribeT&& value) {
    ...
    m_onSubscribe = std::forward<OnSubscribeT>(value);
  }
  template <typename OnSubscribeT = HandlerConfig>
  HandlerConfigs& WithOnSubscribe(OnSubscribeT&& value) {
    SetOnSubscribe(std::forward<OnSubscribeT>(value));
    return *this;
  }
  ///@}
 private:
