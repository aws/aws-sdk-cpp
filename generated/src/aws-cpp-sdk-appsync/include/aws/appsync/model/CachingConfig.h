...
class CachingConfig {
 public:
  ...
  inline CachingConfig& WithTtl(long long value) {
    SetTtl(value);
    return *this;
  }
  ///@}
  ...
  void SetCachingKeys(CachingKeysT&& value) {
    ...
    m_cachingKeys = std::forward<CachingKeysT>(value);
  }
  template <typename CachingKeysT = Aws::Vector<Aws::String>>
  CachingConfig& WithCachingKeys(CachingKeysT&& value) {
    SetCachingKeys(std::forward<CachingKeysT>(value));
    return *this;
  }
  template <typename CachingKeysT = Aws::String>
  CachingConfig& AddCachingKeys(CachingKeysT&& value) {
    m_cachingKeysHasBeenSet = true;
    m_cachingKeys.emplace_back(std::forward<CachingKeysT>(value));
    return *this;
  }
  ///@}
 private:
