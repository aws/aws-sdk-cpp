...
class EvaluateCodeErrorDetail {
 public:
  ...
  template <typename MessageT = Aws::String>
  EvaluateCodeErrorDetail& WithMessage(MessageT&& value) {
  ...
  template <typename CodeErrorsT = Aws::Vector<CodeError>>
  EvaluateCodeErrorDetail& WithCodeErrors(CodeErrorsT&& value) {
    SetCodeErrors(std::forward<CodeErrorsT>(value));
    return *this;
  }
  template <typename CodeErrorsT = CodeError>
  EvaluateCodeErrorDetail& AddCodeErrors(CodeErrorsT&& value) {
    m_codeErrorsHasBeenSet = true;
