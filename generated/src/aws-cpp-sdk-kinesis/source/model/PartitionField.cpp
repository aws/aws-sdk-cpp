...
PartitionField::PartitionField(JsonView jsonValue) { *this = jsonValue; }

PartitionField& PartitionField::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("Transform")) {
    m_transform = PartitionTransformMapper::GetPartitionTransformForName(jsonValue.GetString("Transform"));
...
JsonValue PartitionField::Jsonize() const {
  ...
  if (m_transformHasBeenSet) {
    payload.WithString("Transform", PartitionTransformMapper::GetNameForPartitionTransform(m_transform));
  }
