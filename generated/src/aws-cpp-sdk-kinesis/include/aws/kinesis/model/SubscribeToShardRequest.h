...
class SubscribeToShardRequest : public KinesisRequest {
 public:
   ...
   * Underlying Event Stream Handler which is used to define callback functions.
   */
  inline void SetEventStreamHandler(const SubscribeToShardHandler& value) {
    m_handler = value;
   ...
   * Underlying Event Stream Handler which is used to define callback functions.
   */
  inline SubscribeToShardRequest& WithEventStreamHandler(const SubscribeToShardHandler& value) {
  ...
  void SetStartingPosition(StartingPositionT&& value) {
    ...
    m_startingPosition = std::forward<StartingPositionT>(value);
  }
  template <typename StartingPositionT = StartingPosition>
  SubscribeToShardRequest& WithStartingPosition(StartingPositionT&& value) {
    SetStartingPosition(std::forward<StartingPositionT>(value));
    return *this;
  }
  ///@}
 private:
