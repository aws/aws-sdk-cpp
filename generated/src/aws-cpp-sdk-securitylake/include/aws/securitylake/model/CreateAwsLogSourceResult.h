...
class CreateAwsLogSourceResult {
 public:
  ...
  template <typename FailedT = Aws::Vector<Aws::String>>
  CreateAwsLogSourceResult& WithFailed(FailedT&& value) {
    SetFailed(std::forward<FailedT>(value));
    return *this;
  }
  template <typename FailedT = Aws::String>
  CreateAwsLogSourceResult& AddFailed(FailedT&& value) {
    m_failedHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  CreateAwsLogSourceResult& WithRequestId(RequestIdT&& value) {
