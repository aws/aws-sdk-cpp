...
class ListDataLakesResult {
 public:
  AWS_SECURITYLAKE_API ListDataLakesResult() = default;
  ...
  template <typename DataLakesT = Aws::Vector<DataLakeResource>>
  ListDataLakesResult& WithDataLakes(DataLakesT&& value) {
    SetDataLakes(std::forward<DataLakesT>(value));
    return *this;
  }
  template <typename DataLakesT = DataLakeResource>
  ListDataLakesResult& AddDataLakes(DataLakesT&& value) {
    m_dataLakesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  ListDataLakesResult& WithRequestId(RequestIdT&& value) {
