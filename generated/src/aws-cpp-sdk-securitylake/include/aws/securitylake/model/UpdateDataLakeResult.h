...
class UpdateDataLakeResult {
 public:
  ...
  template <typename DataLakesT = Aws::Vector<DataLakeResource>>
  UpdateDataLakeResult& WithDataLakes(DataLakesT&& value) {
    SetDataLakes(std::forward<DataLakesT>(value));
    return *this;
  }
  template <typename DataLakesT = DataLakeResource>
  UpdateDataLakeResult& AddDataLakes(DataLakesT&& value) {
    m_dataLakesHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  UpdateDataLakeResult& WithRequestId(RequestIdT&& value) {
