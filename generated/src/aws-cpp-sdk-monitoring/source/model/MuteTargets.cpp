...
MuteTargets::MuteTargets(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

MuteTargets& MuteTargets::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
                      ...
                      for (size_t j_0 = 0; j_0 < listSize_0.value(); j_0++) {
                        ...
                        if (peekType_1.has_value()) {
                          if (peekType_1.value() == Aws::Crt::Cbor::CborType::Text) {
                            ...
                            if (val.has_value()) {
                              m_alarmNames.push_back(Aws::String(reinterpret_cast<const char*>(val.value().ptr), val.value().len));
                            }
                          } else {
                            ...
                            m_alarmNames.push_back(ss_1.str());
                            ss_1.clear();
                          }
                        }
                      }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("MuteTargets", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void MuteTargets::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
