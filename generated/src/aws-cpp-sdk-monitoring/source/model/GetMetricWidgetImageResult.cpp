...
GetMetricWidgetImageResult::GetMetricWidgetImageResult(const Aws::AmazonWebServiceResult<Aws::Utils::Cbor::CborValue>& result) {
  *this = result;
}
        ...
        if (mapSize.has_value()) {
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if (initialKeyStr == "MetricWidgetImage") {
                auto peekType = decoder->PeekType();
                if (peekType.has_value()) {
                  if (peekType.value() == Aws::Crt::Cbor::CborType::Bytes) {
                    auto val = decoder->PopNextBytesVal();
                    if (val.has_value()) {
                      m_metricWidgetImage = Aws::Utils::ByteBuffer(val.value().ptr, val.value().len);
                    }
                  } else {
                    ...
                    m_metricWidgetImage = Aws::Utils::ByteBuffer(ss.str());
                    ss.clear();
                  }
                }
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("GetMetricWidgetImageResult", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
