...
DashboardEntry::DashboardEntry(const XmlNode& xmlNode) { *this = xmlNode; }
...
void DashboardEntry::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastModifiedHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastModified=" << StringUtils::URLEncode(m_lastModified.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
