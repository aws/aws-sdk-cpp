...
MetricStreamEntry::MetricStreamEntry(const XmlNode& xmlNode) { *this = xmlNode; }
...
MetricStreamEntry& MetricStreamEntry::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!lastUpdateDateNode.IsNull()) {
      m_lastUpdateDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(lastUpdateDateNode.GetText()).c_str()).c_str(),
                                  Aws::Utils::DateFormat::ISO_8601);
...
void MetricStreamEntry::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_lastUpdateDateHasBeenSet) {
    oStream << location << index << locationValue
            << ".LastUpdateDate=" << StringUtils::URLEncode(m_lastUpdateDate.ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str()) << "&";
  }
  ...
  if (m_outputFormatHasBeenSet) {
    oStream << location << index << locationValue << ".OutputFormat="
            << StringUtils::URLEncode(MetricStreamOutputFormatMapper::GetNameForMetricStreamOutputFormat(m_outputFormat)) << "&";
  }
}
