...
MetricStreamEntry::MetricStreamEntry(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) { *this = decoder; }

MetricStreamEntry& MetricStreamEntry::operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder) {
  if (decoder != nullptr) {
          ...
          for (size_t i = 0; i < mapSize.value(); ++i) {
            ...
            if (initialKey.has_value()) {
              ...
              if ((decoder->LastError() != AWS_ERROR_UNKNOWN)) {
                AWS_LOG_ERROR("MetricStreamEntry", "Invalid data received for %s", initialKeyStr.c_str());
                break;
              }
            }
          }
...
void MetricStreamEntry::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const {
  // Calculate map size
  size_t mapSize = 0;
  if (m_arnHasBeenSet) {
    mapSize++;
  ...
  if (m_lastUpdateDateHasBeenSet) {
    encoder.WriteText(Aws::Crt::ByteCursorFromCString("LastUpdateDate"));
    encoder.WriteTag(1);  // 1 represents Epoch-based date/time. See https://www.rfc-editor.org/rfc/rfc8949.html#tags
    encoder.WriteUInt(m_lastUpdateDate.Seconds());
  }
  ...
  if (m_stateHasBeenSet) {
    ...
    encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_state.c_str()));
  }
