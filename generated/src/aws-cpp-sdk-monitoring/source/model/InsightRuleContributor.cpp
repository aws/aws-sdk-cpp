...
InsightRuleContributor::InsightRuleContributor(const XmlNode& xmlNode) { *this = xmlNode; }
...
InsightRuleContributor& InsightRuleContributor::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!approximateAggregateValueNode.IsNull()) {
      m_approximateAggregateValue = StringUtils::ConvertToDouble(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(approximateAggregateValueNode.GetText()).c_str()).c_str());
