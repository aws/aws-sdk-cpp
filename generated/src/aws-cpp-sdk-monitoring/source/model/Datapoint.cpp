...
Datapoint::Datapoint(const XmlNode& xmlNode) { *this = xmlNode; }
...
Datapoint& Datapoint::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!sampleCountNode.IsNull()) {
      m_sampleCount =
          StringUtils::ConvertToDouble(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(sampleCountNode.GetText()).c_str()).c_str());
      m_sampleCountHasBeenSet = true;
    }
    XmlNode averageNode = resultNode.FirstChild("Average");
    if (!averageNode.IsNull()) {
      m_average =
          StringUtils::ConvertToDouble(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(averageNode.GetText()).c_str()).c_str());
    ...
    if (!minimumNode.IsNull()) {
      m_minimum =
          StringUtils::ConvertToDouble(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(minimumNode.GetText()).c_str()).c_str());
      m_minimumHasBeenSet = true;
    }
    XmlNode maximumNode = resultNode.FirstChild("Maximum");
    if (!maximumNode.IsNull()) {
      m_maximum =
          StringUtils::ConvertToDouble(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(maximumNode.GetText()).c_str()).c_str());
    ...
    if (!extendedStatisticsNode.IsNull()) {
      ...
      while (!extendedStatisticsEntry.IsNull()) {
        ...
        m_extendedStatistics[keyNode.GetText()] = StringUtils::ConvertToDouble(StringUtils::Trim(valueNode.GetText().c_str()).c_str());
...
void Datapoint::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_extendedStatisticsHasBeenSet) {
    ...
    for (auto& item : m_extendedStatistics) {
      oStream << location << index << locationValue << ".ExtendedStatistics.entry." << extendedStatisticsIdx
              << ".key=" << StringUtils::URLEncode(item.first.c_str()) << "&";
      oStream << location << index << locationValue << ".ExtendedStatistics.entry." << extendedStatisticsIdx
              << ".value=" << StringUtils::URLEncode(item.second) << "&";
...
void Datapoint::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_extendedStatisticsHasBeenSet) {
    unsigned extendedStatisticsIdx = 1;
    for (auto& item : m_extendedStatistics) {
      oStream << location << ".ExtendedStatistics.entry." << extendedStatisticsIdx << ".key=" << StringUtils::URLEncode(item.first.c_str())
              << "&";
      oStream << location << ".ExtendedStatistics.entry." << extendedStatisticsIdx << ".value=" << StringUtils::URLEncode(item.second)
              << "&";
      extendedStatisticsIdx++;
    }
  }
