...
MetricStreamStatisticsConfiguration::MetricStreamStatisticsConfiguration(const XmlNode& xmlNode) { *this = xmlNode; }
...
void MetricStreamStatisticsConfiguration::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_additionalStatisticsHasBeenSet) {
    ...
    for (auto& item : m_additionalStatistics) {
      oStream << location << index << locationValue << ".AdditionalStatistics.member." << additionalStatisticsIdx++ << "="
