  ...
  if (m_statisticsHasBeenSet) {
    if (m_statistics.empty()) {
      ...
      for (auto& item : m_statistics) {
        ss << "Statistics.member." << statisticsCount << "=" << StringUtils::URLEncode(StatisticMapper::GetNameForStatistic(item)) << "&";
        statisticsCount++;
      }
    }
  }
  ...
  if (m_extendedStatisticsHasBeenSet) {
    if (m_extendedStatistics.empty()) {
      ...
      for (auto& item : m_extendedStatistics) {
        ss << "ExtendedStatistics.member." << extendedStatisticsCount << "=" << StringUtils::URLEncode(item.c_str()) << "&";
...
void GetMetricStatisticsRequest::DumpBodyToUrl(Aws::Http::URI& uri) const { uri.SetQueryString(SerializePayload()); }
