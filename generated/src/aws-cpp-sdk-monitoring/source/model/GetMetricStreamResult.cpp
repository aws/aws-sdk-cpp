...
GetMetricStreamResult::GetMetricStreamResult(const Aws::AmazonWebServiceResult<XmlDocument>& result) { *this = result; }
...
GetMetricStreamResult& GetMetricStreamResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!includeLinkedAccountsMetricsNode.IsNull()) {
      m_includeLinkedAccountsMetrics = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(includeLinkedAccountsMetricsNode.GetText()).c_str()).c_str());
