...
Entity::Entity(const XmlNode& xmlNode) { *this = xmlNode; }
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!keyAttributesNode.IsNull()) {
      ...
      while (!keyAttributesEntry.IsNull()) {
        ...
        m_keyAttributes[keyNode.GetText()] = valueNode.GetText();
...
void Entity::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  if (m_keyAttributesHasBeenSet) {
    ...
    for (auto& item : m_keyAttributes) {
      oStream << location << index << locationValue << ".KeyAttributes.entry." << keyAttributesIdx
  ...
  if (m_attributesHasBeenSet) {
    ...
    for (auto& item : m_attributes) {
      oStream << location << index << locationValue << ".Attributes.entry." << attributesIdx
              << ".key=" << StringUtils::URLEncode(item.first.c_str()) << "&";
      oStream << location << index << locationValue << ".Attributes.entry." << attributesIdx
              << ".value=" << StringUtils::URLEncode(item.second.c_str()) << "&";
      attributesIdx++;
...
void Entity::OutputToStream(Aws::OStream& oStream, const char* location) const {
  if (m_keyAttributesHasBeenSet) {
    unsigned keyAttributesIdx = 1;
    for (auto& item : m_keyAttributes) {
      oStream << location << ".KeyAttributes.entry." << keyAttributesIdx << ".key=" << StringUtils::URLEncode(item.first.c_str()) << "&";
      oStream << location << ".KeyAttributes.entry." << keyAttributesIdx << ".value=" << StringUtils::URLEncode(item.second.c_str()) << "&";
      keyAttributesIdx++;
    }
  }
  if (m_attributesHasBeenSet) {
    unsigned attributesIdx = 1;
    for (auto& item : m_attributes) {
      oStream << location << ".Attributes.entry." << attributesIdx << ".key=" << StringUtils::URLEncode(item.first.c_str()) << "&";
      oStream << location << ".Attributes.entry." << attributesIdx << ".value=" << StringUtils::URLEncode(item.second.c_str()) << "&";
      attributesIdx++;
    }
  }
}
