...
InsightRule::InsightRule(const XmlNode& xmlNode) { *this = xmlNode; }
...
InsightRule& InsightRule::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!managedRuleNode.IsNull()) {
      m_managedRule =
          StringUtils::ConvertToBool(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(managedRuleNode.GetText()).c_str()).c_str());
    ...
    if (!applyOnTransformedLogsNode.IsNull()) {
      m_applyOnTransformedLogs = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(applyOnTransformedLogsNode.GetText()).c_str()).c_str());
