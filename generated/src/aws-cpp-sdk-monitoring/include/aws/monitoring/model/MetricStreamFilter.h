...
class MetricStreamFilter {
 public:
  ...
  AWS_CLOUDWATCH_API MetricStreamFilter(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  AWS_CLOUDWATCH_API MetricStreamFilter& operator=(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder);
  ...
  template <typename NamespaceT = Aws::String>
  MetricStreamFilter& WithNamespace(NamespaceT&& value) {
  ...
  template <typename MetricNamesT = Aws::Vector<Aws::String>>
  MetricStreamFilter& WithMetricNames(MetricNamesT&& value) {
    SetMetricNames(std::forward<MetricNamesT>(value));
    return *this;
  }
  template <typename MetricNamesT = Aws::String>
  MetricStreamFilter& AddMetricNames(MetricNamesT&& value) {
    m_metricNamesHasBeenSet = true;
