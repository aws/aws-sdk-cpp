...
class BatchEvaluateFeatureResult {
 public:
  ...
  template <typename ResultsT = Aws::Vector<EvaluationResult>>
  BatchEvaluateFeatureResult& WithResults(ResultsT&& value) {
    SetResults(std::forward<ResultsT>(value));
    return *this;
  }
  template <typename ResultsT = EvaluationResult>
  BatchEvaluateFeatureResult& AddResults(ResultsT&& value) {
    m_resultsHasBeenSet = true;
  ...
  template <typename RequestIdT = Aws::String>
  BatchEvaluateFeatureResult& WithRequestId(RequestIdT&& value) {
