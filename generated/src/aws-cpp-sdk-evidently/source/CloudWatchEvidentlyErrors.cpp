...
template <>
AWS_CLOUDWATCHEVIDENTLY_API ConflictException CloudWatchEvidentlyError::GetModeledError() {
  assert(this->GetErrorType() == CloudWatchEvidentlyErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_CLOUDWATCHEVIDENTLY_API ThrottlingException CloudWatchEvidentlyError::GetModeledError() {
  assert(this->GetErrorType() == CloudWatchEvidentlyErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_CLOUDWATCHEVIDENTLY_API ServiceQuotaExceededException CloudWatchEvidentlyError::GetModeledError() {
  assert(this->GetErrorType() == CloudWatchEvidentlyErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_CLOUDWATCHEVIDENTLY_API ResourceNotFoundException CloudWatchEvidentlyError::GetModeledError() {
  assert(this->GetErrorType() == CloudWatchEvidentlyErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_CLOUDWATCHEVIDENTLY_API ValidationException CloudWatchEvidentlyError::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(CloudWatchEvidentlyErrors::INTERNAL_SERVER), RetryableType::RETRYABLE);
  }
