...
namespace DSQL {
template <>
AWS_DSQL_API ConflictException DSQLError::GetModeledError() {
  assert(this->GetErrorType() == DSQLErrors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_DSQL_API ServiceQuotaExceededException DSQLError::GetModeledError() {
  assert(this->GetErrorType() == DSQLErrors::SERVICE_QUOTA_EXCEEDED);
  return ServiceQuotaExceededException(this->GetJsonPayload().View());
}

template <>
AWS_DSQL_API ThrottlingException DSQLError::GetModeledError() {
  assert(this->GetErrorType() == DSQLErrors::THROTTLING);
  return ThrottlingException(this->GetJsonPayload().View());
}

template <>
AWS_DSQL_API InternalServerException DSQLError::GetModeledError() {
  assert(this->GetErrorType() == DSQLErrors::INTERNAL_SERVER);
  return InternalServerException(this->GetJsonPayload().View());
}

template <>
AWS_DSQL_API ResourceNotFoundException DSQLError::GetModeledError() {
  assert(this->GetErrorType() == DSQLErrors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_DSQL_API ValidationException DSQLError::GetModeledError() {
