...
class VisibleRangeOptions {
 public:
  ...
  void SetPercentRange(PercentRangeT&& value) {
    m_percentRangeHasBeenSet = true;
    m_percentRange = std::forward<PercentRangeT>(value);
  }
  template <typename PercentRangeT = PercentVisibleRange>
  VisibleRangeOptions& WithPercentRange(PercentRangeT&& value) {
    SetPercentRange(std::forward<PercentRangeT>(value));
    return *this;
  }
  ///@}
 private:
