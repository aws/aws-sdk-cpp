...
class SnapshotS3DestinationConfiguration {
 public:
  ...
  void SetBucketConfiguration(BucketConfigurationT&& value) {
    m_bucketConfigurationHasBeenSet = true;
    m_bucketConfiguration = std::forward<BucketConfigurationT>(value);
  }
  template <typename BucketConfigurationT = S3BucketConfiguration>
  SnapshotS3DestinationConfiguration& WithBucketConfiguration(BucketConfigurationT&& value) {
    SetBucketConfiguration(std::forward<BucketConfigurationT>(value));
    return *this;
  }
  ///@}
 private:
