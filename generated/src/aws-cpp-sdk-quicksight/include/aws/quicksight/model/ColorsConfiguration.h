...
class ColorsConfiguration {
 public:
  ...
  template <typename CustomColorsT = Aws::Vector<CustomColor>>
  ColorsConfiguration& WithCustomColors(CustomColorsT&& value) {
    SetCustomColors(std::forward<CustomColorsT>(value));
    return *this;
  }
  template <typename CustomColorsT = CustomColor>
  ColorsConfiguration& AddCustomColors(CustomColorsT&& value) {
    m_customColorsHasBeenSet = true;
    m_customColors.emplace_back(std::forward<CustomColorsT>(value));
    return *this;
  }
  ///@}
 private:
