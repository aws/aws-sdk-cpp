...
class GradientColor {
 public:
  ...
  void SetStops(StopsT&& value) {
    m_stopsHasBeenSet = true;
    m_stops = std::forward<StopsT>(value);
  }
  template <typename StopsT = Aws::Vector<GradientStop>>
  GradientColor& WithStops(StopsT&& value) {
    SetStops(std::forward<StopsT>(value));
    return *this;
  }
  template <typename StopsT = GradientStop>
  GradientColor& AddStops(StopsT&& value) {
    m_stopsHasBeenSet = true;
    m_stops.emplace_back(std::forward<StopsT>(value));
    return *this;
  }
  ///@}
 private:
