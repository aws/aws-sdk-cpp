...
class GaugeChartPrimaryValueConditionalFormatting {
 public:
  ...
  template <typename TextColorT = ConditionalFormattingColor>
  GaugeChartPrimaryValueConditionalFormatting& WithTextColor(TextColorT&& value) {
  ...
  void SetIcon(IconT&& value) {
    ...
    m_icon = std::forward<IconT>(value);
  }
  template <typename IconT = ConditionalFormattingIcon>
  GaugeChartPrimaryValueConditionalFormatting& WithIcon(IconT&& value) {
    SetIcon(std::forward<IconT>(value));
    return *this;
  }
  ///@}
 private:
