...
class KPIFieldWells {
 public:
  ...
  void SetTargetValues(TargetValuesT&& value) {
    ...
    m_targetValues = std::forward<TargetValuesT>(value);
  }
  template <typename TargetValuesT = Aws::Vector<MeasureField>>
  KPIFieldWells& WithTargetValues(TargetValuesT&& value) {
    SetTargetValues(std::forward<TargetValuesT>(value));
    return *this;
  }
  template <typename TargetValuesT = MeasureField>
  KPIFieldWells& AddTargetValues(TargetValuesT&& value) {
    m_targetValuesHasBeenSet = true;
    m_targetValues.emplace_back(std::forward<TargetValuesT>(value));
    return *this;
  }
  ///@}
  ...
  void SetTrendGroups(TrendGroupsT&& value) {
    ...
    m_trendGroups = std::forward<TrendGroupsT>(value);
  }
  template <typename TrendGroupsT = Aws::Vector<DimensionField>>
  KPIFieldWells& WithTrendGroups(TrendGroupsT&& value) {
    SetTrendGroups(std::forward<TrendGroupsT>(value));
    return *this;
  }
  template <typename TrendGroupsT = DimensionField>
  KPIFieldWells& AddTrendGroups(TrendGroupsT&& value) {
    m_trendGroupsHasBeenSet = true;
    m_trendGroups.emplace_back(std::forward<TrendGroupsT>(value));
    return *this;
  }
  ///@}
 private:
