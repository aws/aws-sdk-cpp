...
class NullValueFormatConfiguration {
 public:
  ...
  void SetNullString(NullStringT&& value) {
    m_nullStringHasBeenSet = true;
    m_nullString = std::forward<NullStringT>(value);
  }
  template <typename NullStringT = Aws::String>
  NullValueFormatConfiguration& WithNullString(NullStringT&& value) {
    SetNullString(std::forward<NullStringT>(value));
    return *this;
  }
  ///@}
 private:
