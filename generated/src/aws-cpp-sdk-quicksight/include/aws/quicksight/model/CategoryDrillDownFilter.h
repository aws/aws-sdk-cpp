...
class CategoryDrillDownFilter {
 public:
  ...
  template <typename ColumnT = ColumnIdentifier>
  CategoryDrillDownFilter& WithColumn(ColumnT&& value) {
  ...
  template <typename CategoryValuesT = Aws::Vector<Aws::String>>
  CategoryDrillDownFilter& WithCategoryValues(CategoryValuesT&& value) {
    SetCategoryValues(std::forward<CategoryValuesT>(value));
    return *this;
  }
  template <typename CategoryValuesT = Aws::String>
  CategoryDrillDownFilter& AddCategoryValues(CategoryValuesT&& value) {
    m_categoryValuesHasBeenSet = true;
