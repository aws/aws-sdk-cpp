...
class UntagColumnOperation {
 public:
  ...
  template <typename ColumnNameT = Aws::String>
  UntagColumnOperation& WithColumnName(ColumnNameT&& value) {
  ...
  void SetTagNames(TagNamesT&& value) {
    m_tagNamesHasBeenSet = true;
    m_tagNames = std::forward<TagNamesT>(value);
  }
  template <typename TagNamesT = Aws::Vector<ColumnTagName>>
  UntagColumnOperation& WithTagNames(TagNamesT&& value) {
    SetTagNames(std::forward<TagNamesT>(value));
    return *this;
  }
  inline UntagColumnOperation& AddTagNames(ColumnTagName value) {
    m_tagNamesHasBeenSet = true;
    m_tagNames.push_back(value);
    return *this;
  }
  ///@}
 private:
