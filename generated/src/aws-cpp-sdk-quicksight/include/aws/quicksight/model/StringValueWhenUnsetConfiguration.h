...
class StringValueWhenUnsetConfiguration {
 public:
  ...
  inline void SetValueWhenUnsetOption(ValueWhenUnsetOption value) {
    m_valueWhenUnsetOptionHasBeenSet = true;
    m_valueWhenUnsetOption = value;
  }
  inline StringValueWhenUnsetConfiguration& WithValueWhenUnsetOption(ValueWhenUnsetOption value) {
    SetValueWhenUnsetOption(value);
    return *this;
  }
  ///@}
  ...
  void SetCustomValue(CustomValueT&& value) {
    ...
    m_customValue = std::forward<CustomValueT>(value);
  }
  template <typename CustomValueT = Aws::String>
  StringValueWhenUnsetConfiguration& WithCustomValue(CustomValueT&& value) {
    SetCustomValue(std::forward<CustomValueT>(value));
    return *this;
  }
  ///@}
 private:
