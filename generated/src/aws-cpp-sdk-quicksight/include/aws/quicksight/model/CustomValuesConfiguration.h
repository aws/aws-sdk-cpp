...
class CustomValuesConfiguration {
 public:
  ...
  inline void SetIncludeNullValue(bool value) {
    m_includeNullValueHasBeenSet = true;
    m_includeNullValue = value;
  }
  inline CustomValuesConfiguration& WithIncludeNullValue(bool value) {
    SetIncludeNullValue(value);
    return *this;
  }
  ///@}
  ...
  void SetCustomValues(CustomValuesT&& value) {
    ...
    m_customValues = std::forward<CustomValuesT>(value);
  }
  template <typename CustomValuesT = CustomParameterValues>
  CustomValuesConfiguration& WithCustomValues(CustomValuesT&& value) {
    SetCustomValues(std::forward<CustomValuesT>(value));
    return *this;
  }
  ///@}
 private:
