...
class QueueInfo {
 public:
  ...
  void SetWaitingOnIngestion(WaitingOnIngestionT&& value) {
    ...
    m_waitingOnIngestion = std::forward<WaitingOnIngestionT>(value);
  }
  template <typename WaitingOnIngestionT = Aws::String>
  QueueInfo& WithWaitingOnIngestion(WaitingOnIngestionT&& value) {
    SetWaitingOnIngestion(std::forward<WaitingOnIngestionT>(value));
    return *this;
  }
  ///@}
  ...
  void SetQueuedIngestion(QueuedIngestionT&& value) {
    ...
    m_queuedIngestion = std::forward<QueuedIngestionT>(value);
  }
  template <typename QueuedIngestionT = Aws::String>
  QueueInfo& WithQueuedIngestion(QueuedIngestionT&& value) {
    SetQueuedIngestion(std::forward<QueuedIngestionT>(value));
    return *this;
  }
  ///@}
 private:
