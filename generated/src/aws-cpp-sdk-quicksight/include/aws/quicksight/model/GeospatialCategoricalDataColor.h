...
class GeospatialCategoricalDataColor {
 public:
  ...
  template <typename ColorT = Aws::String>
  GeospatialCategoricalDataColor& WithColor(ColorT&& value) {
  ...
  void SetDataValue(DataValueT&& value) {
    m_dataValueHasBeenSet = true;
    m_dataValue = std::forward<DataValueT>(value);
  }
  template <typename DataValueT = Aws::String>
  GeospatialCategoricalDataColor& WithDataValue(DataValueT&& value) {
    SetDataValue(std::forward<DataValueT>(value));
    return *this;
  }
  ///@}
 private:
