...
class ForecastConfiguration {
 public:
  ...
  void SetForecastProperties(ForecastPropertiesT&& value) {
    ...
    m_forecastProperties = std::forward<ForecastPropertiesT>(value);
  }
  template <typename ForecastPropertiesT = TimeBasedForecastProperties>
  ForecastConfiguration& WithForecastProperties(ForecastPropertiesT&& value) {
    SetForecastProperties(std::forward<ForecastPropertiesT>(value));
    return *this;
  }
  ///@}
  ...
  void SetScenario(ScenarioT&& value) {
    ...
    m_scenario = std::forward<ScenarioT>(value);
  }
  template <typename ScenarioT = ForecastScenario>
  ForecastConfiguration& WithScenario(ScenarioT&& value) {
    SetScenario(std::forward<ScenarioT>(value));
    return *this;
  }
  ///@}
 private:
