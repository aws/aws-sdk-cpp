...
class WordCloudSortConfiguration {
 public:
  ...
  template <typename CategoryItemsLimitT = ItemsLimitConfiguration>
  WordCloudSortConfiguration& WithCategoryItemsLimit(CategoryItemsLimitT&& value) {
  ...
  template <typename CategorySortT = Aws::Vector<FieldSortOptions>>
  WordCloudSortConfiguration& WithCategorySort(CategorySortT&& value) {
    SetCategorySort(std::forward<CategorySortT>(value));
    return *this;
  }
  template <typename CategorySortT = FieldSortOptions>
  WordCloudSortConfiguration& AddCategorySort(CategorySortT&& value) {
    m_categorySortHasBeenSet = true;
