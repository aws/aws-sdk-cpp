...
class ClusterMarkerConfiguration {
 public:
  ...
  void SetClusterMarker(ClusterMarkerT&& value) {
    m_clusterMarkerHasBeenSet = true;
    m_clusterMarker = std::forward<ClusterMarkerT>(value);
  }
  template <typename ClusterMarkerT = ClusterMarker>
  ClusterMarkerConfiguration& WithClusterMarker(ClusterMarkerT&& value) {
    SetClusterMarker(std::forward<ClusterMarkerT>(value));
    return *this;
  }
  ///@}
 private:
