...
class DataSetRefreshProperties {
 public:
  ...
  void SetRefreshConfiguration(RefreshConfigurationT&& value) {
    ...
    m_refreshConfiguration = std::forward<RefreshConfigurationT>(value);
  }
  template <typename RefreshConfigurationT = RefreshConfiguration>
  DataSetRefreshProperties& WithRefreshConfiguration(RefreshConfigurationT&& value) {
    SetRefreshConfiguration(std::forward<RefreshConfigurationT>(value));
    return *this;
  }
  ///@}
  ...
  void SetFailureConfiguration(FailureConfigurationT&& value) {
    ...
    m_failureConfiguration = std::forward<FailureConfigurationT>(value);
  }
  template <typename FailureConfigurationT = RefreshFailureConfiguration>
  DataSetRefreshProperties& WithFailureConfiguration(FailureConfigurationT&& value) {
    SetFailureConfiguration(std::forward<FailureConfigurationT>(value));
    return *this;
  }
  ///@}
 private:
