...
class TableAggregatedFieldWells {
 public:
  AWS_QUICKSIGHT_API TableAggregatedFieldWells() = default;
  ...
  template <typename GroupByT = Aws::Vector<DimensionField>>
  TableAggregatedFieldWells& WithGroupBy(GroupByT&& value) {
    SetGroupBy(std::forward<GroupByT>(value));
    return *this;
  }
  template <typename GroupByT = DimensionField>
  TableAggregatedFieldWells& AddGroupBy(GroupByT&& value) {
    m_groupByHasBeenSet = true;
  ...
  template <typename ValuesT = Aws::Vector<MeasureField>>
  TableAggregatedFieldWells& WithValues(ValuesT&& value) {
    SetValues(std::forward<ValuesT>(value));
    return *this;
  }
  template <typename ValuesT = MeasureField>
  TableAggregatedFieldWells& AddValues(ValuesT&& value) {
    m_valuesHasBeenSet = true;
