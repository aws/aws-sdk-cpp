...
class FieldSortOptions {
 public:
  ...
  void SetFieldSort(FieldSortT&& value) {
    ...
    m_fieldSort = std::forward<FieldSortT>(value);
  }
  template <typename FieldSortT = FieldSort>
  FieldSortOptions& WithFieldSort(FieldSortT&& value) {
    SetFieldSort(std::forward<FieldSortT>(value));
    return *this;
  }
  ///@}
  ...
  void SetColumnSort(ColumnSortT&& value) {
    m_columnSortHasBeenSet = true;
    m_columnSort = std::forward<ColumnSortT>(value);
  }
  template <typename ColumnSortT = ColumnSort>
  FieldSortOptions& WithColumnSort(ColumnSortT&& value) {
    SetColumnSort(std::forward<ColumnSortT>(value));
    return *this;
  }
  ///@}
 private:
