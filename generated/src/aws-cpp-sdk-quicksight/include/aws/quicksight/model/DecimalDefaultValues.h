...
class DecimalDefaultValues {
 public:
  ...
  template <typename DynamicValueT = DynamicDefaultValue>
  DecimalDefaultValues& WithDynamicValue(DynamicValueT&& value) {
  ...
  template <typename StaticValuesT = Aws::Vector<double>>
  DecimalDefaultValues& WithStaticValues(StaticValuesT&& value) {
    SetStaticValues(std::forward<StaticValuesT>(value));
    return *this;
  }
  inline DecimalDefaultValues& AddStaticValues(double value) {
    m_staticValuesHasBeenSet = true;
