...
class PredefinedHierarchy {
 public:
  ...
  template <typename HierarchyIdT = Aws::String>
  PredefinedHierarchy& WithHierarchyId(HierarchyIdT&& value) {
  ...
  template <typename ColumnsT = Aws::Vector<ColumnIdentifier>>
  PredefinedHierarchy& WithColumns(ColumnsT&& value) {
    SetColumns(std::forward<ColumnsT>(value));
    return *this;
  }
  template <typename ColumnsT = ColumnIdentifier>
  PredefinedHierarchy& AddColumns(ColumnsT&& value) {
    m_columnsHasBeenSet = true;
  ...
  void SetDrillDownFilters(DrillDownFiltersT&& value) {
    m_drillDownFiltersHasBeenSet = true;
    m_drillDownFilters = std::forward<DrillDownFiltersT>(value);
  }
  template <typename DrillDownFiltersT = Aws::Vector<DrillDownFilter>>
  PredefinedHierarchy& WithDrillDownFilters(DrillDownFiltersT&& value) {
    SetDrillDownFilters(std::forward<DrillDownFiltersT>(value));
    return *this;
  }
  template <typename DrillDownFiltersT = DrillDownFilter>
  PredefinedHierarchy& AddDrillDownFilters(DrillDownFiltersT&& value) {
    m_drillDownFiltersHasBeenSet = true;
    m_drillDownFilters.emplace_back(std::forward<DrillDownFiltersT>(value));
    return *this;
  }
  ///@}
 private:
