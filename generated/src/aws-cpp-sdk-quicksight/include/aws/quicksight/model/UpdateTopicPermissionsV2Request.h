...
/**
 */
class UpdateTopicPermissionsV2Request : public QuickSightRequest {
 public:
  AWS_QUICKSIGHT_API UpdateTopicPermissionsV2Request() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "UpdateTopicPermissionsV2"; }
  ...
  template <typename AwsAccountIdT = Aws::String>
  UpdateTopicPermissionsV2Request& WithAwsAccountId(AwsAccountIdT&& value) {
  ...
  template <typename TopicIdT = Aws::String>
  UpdateTopicPermissionsV2Request& WithTopicId(TopicIdT&& value) {
  ...
  template <typename GrantPermissionsT = Aws::Vector<ResourcePermission>>
  UpdateTopicPermissionsV2Request& WithGrantPermissions(GrantPermissionsT&& value) {
    SetGrantPermissions(std::forward<GrantPermissionsT>(value));
    return *this;
  }
  template <typename GrantPermissionsT = ResourcePermission>
  UpdateTopicPermissionsV2Request& AddGrantPermissions(GrantPermissionsT&& value) {
    m_grantPermissionsHasBeenSet = true;
  ...
  template <typename RevokePermissionsT = Aws::Vector<ResourcePermission>>
  UpdateTopicPermissionsV2Request& WithRevokePermissions(RevokePermissionsT&& value) {
    SetRevokePermissions(std::forward<RevokePermissionsT>(value));
    return *this;
  }
  template <typename RevokePermissionsT = ResourcePermission>
  UpdateTopicPermissionsV2Request& AddRevokePermissions(RevokePermissionsT&& value) {
    m_revokePermissionsHasBeenSet = true;
