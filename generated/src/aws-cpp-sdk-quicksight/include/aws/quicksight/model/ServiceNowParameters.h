...
class ServiceNowParameters {
 public:
  ...
  void SetSiteBaseUrl(SiteBaseUrlT&& value) {
    m_siteBaseUrlHasBeenSet = true;
    m_siteBaseUrl = std::forward<SiteBaseUrlT>(value);
  }
  template <typename SiteBaseUrlT = Aws::String>
  ServiceNowParameters& WithSiteBaseUrl(SiteBaseUrlT&& value) {
    SetSiteBaseUrl(std::forward<SiteBaseUrlT>(value));
    return *this;
  }
  ///@}
 private:
