...
class UpdateBrandAssignmentRequest : public QuickSightRequest {
 public:
  ...
  template <typename AwsAccountIdT = Aws::String>
  UpdateBrandAssignmentRequest& WithAwsAccountId(AwsAccountIdT&& value) {
  ...
  void SetBrandArn(BrandArnT&& value) {
    m_brandArnHasBeenSet = true;
    m_brandArn = std::forward<BrandArnT>(value);
  }
  template <typename BrandArnT = Aws::String>
  UpdateBrandAssignmentRequest& WithBrandArn(BrandArnT&& value) {
    SetBrandArn(std::forward<BrandArnT>(value));
    return *this;
  }
  ///@}
 private:
