...
class GeospatialWindowOptions {
 public:
  ...
  void SetBounds(BoundsT&& value) {
    ...
    m_bounds = std::forward<BoundsT>(value);
  }
  template <typename BoundsT = GeospatialCoordinateBounds>
  GeospatialWindowOptions& WithBounds(BoundsT&& value) {
    SetBounds(std::forward<BoundsT>(value));
    return *this;
  }
  ///@}
  ...
  inline void SetMapZoomMode(MapZoomMode value) {
    m_mapZoomModeHasBeenSet = true;
    m_mapZoomMode = value;
  }
  inline GeospatialWindowOptions& WithMapZoomMode(MapZoomMode value) {
    SetMapZoomMode(value);
    return *this;
  }
  ///@}
 private:
