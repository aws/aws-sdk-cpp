...
class BrandElementStyle {
 public:
  ...
  void SetNavbarStyle(NavbarStyleT&& value) {
    m_navbarStyleHasBeenSet = true;
    m_navbarStyle = std::forward<NavbarStyleT>(value);
  }
  template <typename NavbarStyleT = NavbarStyle>
  BrandElementStyle& WithNavbarStyle(NavbarStyleT&& value) {
    SetNavbarStyle(std::forward<NavbarStyleT>(value));
    return *this;
  }
  ///@}
 private:
