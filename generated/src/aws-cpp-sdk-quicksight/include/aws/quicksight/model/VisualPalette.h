...
class VisualPalette {
 public:
  ...
  void SetChartColor(ChartColorT&& value) {
    ...
    m_chartColor = std::forward<ChartColorT>(value);
  }
  template <typename ChartColorT = Aws::String>
  VisualPalette& WithChartColor(ChartColorT&& value) {
    SetChartColor(std::forward<ChartColorT>(value));
    return *this;
  }
  ///@}
  ...
  void SetColorMap(ColorMapT&& value) {
    ...
    m_colorMap = std::forward<ColorMapT>(value);
  }
  template <typename ColorMapT = Aws::Vector<DataPathColor>>
  VisualPalette& WithColorMap(ColorMapT&& value) {
    SetColorMap(std::forward<ColorMapT>(value));
    return *this;
  }
  template <typename ColorMapT = DataPathColor>
  VisualPalette& AddColorMap(ColorMapT&& value) {
    m_colorMapHasBeenSet = true;
    m_colorMap.emplace_back(std::forward<ColorMapT>(value));
    return *this;
  }
  ///@}
 private:
