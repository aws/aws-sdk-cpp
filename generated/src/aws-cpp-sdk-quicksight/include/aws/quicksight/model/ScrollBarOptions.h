...
class ScrollBarOptions {
 public:
  ...
  inline ScrollBarOptions& WithVisibility(Visibility value) {
    SetVisibility(value);
    return *this;
  }
  ///@}
  ...
  void SetVisibleRange(VisibleRangeT&& value) {
    m_visibleRangeHasBeenSet = true;
    m_visibleRange = std::forward<VisibleRangeT>(value);
  }
  template <typename VisibleRangeT = VisibleRangeOptions>
  ScrollBarOptions& WithVisibleRange(VisibleRangeT&& value) {
    SetVisibleRange(std::forward<VisibleRangeT>(value));
    return *this;
  }
  ///@}
 private:
