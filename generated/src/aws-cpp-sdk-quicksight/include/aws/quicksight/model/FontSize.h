...
class FontSize {
 public:
  ...
  inline void SetRelative(RelativeFontSize value) {
    m_relativeHasBeenSet = true;
    m_relative = value;
  }
  inline FontSize& WithRelative(RelativeFontSize value) {
    SetRelative(value);
    return *this;
  }
  ///@}
  ...
  void SetAbsolute(AbsoluteT&& value) {
    ...
    m_absolute = std::forward<AbsoluteT>(value);
  }
  template <typename AbsoluteT = Aws::String>
  FontSize& WithAbsolute(AbsoluteT&& value) {
    SetAbsolute(std::forward<AbsoluteT>(value));
    return *this;
  }
  ///@}
 private:
