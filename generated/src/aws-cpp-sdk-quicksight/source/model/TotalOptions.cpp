...
TotalOptions::TotalOptions(JsonView jsonValue) { *this = jsonValue; }
...
TotalOptions& TotalOptions::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("TotalAggregationOptions")) {
    ...
    for (unsigned totalAggregationOptionsIndex = 0; totalAggregationOptionsIndex < totalAggregationOptionsJsonList.GetLength();
         ++totalAggregationOptionsIndex) {
...
JsonValue TotalOptions::Jsonize() const {
  ...
  if (m_totalAggregationOptionsHasBeenSet) {
    ...
    for (unsigned totalAggregationOptionsIndex = 0; totalAggregationOptionsIndex < totalAggregationOptionsJsonList.GetLength();
         ++totalAggregationOptionsIndex) {
      totalAggregationOptionsJsonList[totalAggregationOptionsIndex].AsObject(
          m_totalAggregationOptions[totalAggregationOptionsIndex].Jsonize());
    }
