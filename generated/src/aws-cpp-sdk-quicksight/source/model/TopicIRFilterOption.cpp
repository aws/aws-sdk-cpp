...
TopicIRFilterOption::TopicIRFilterOption(JsonView jsonValue) { *this = jsonValue; }
...
TopicIRFilterOption& TopicIRFilterOption::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AggregationPartitionBy")) {
    ...
    for (unsigned aggregationPartitionByIndex = 0; aggregationPartitionByIndex < aggregationPartitionByJsonList.GetLength();
         ++aggregationPartitionByIndex) {
