...
DataAggregation::DataAggregation(JsonView jsonValue) { *this = jsonValue; }

DataAggregation& DataAggregation::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("DatasetRowDateGranularity")) {
    m_datasetRowDateGranularity =
        TopicTimeGranularityMapper::GetTopicTimeGranularityForName(jsonValue.GetString("DatasetRowDateGranularity"));
...
JsonValue DataAggregation::Jsonize() const {
  ...
  if (m_datasetRowDateGranularityHasBeenSet) {
    payload.WithString("DatasetRowDateGranularity",
                       TopicTimeGranularityMapper::GetNameForTopicTimeGranularity(m_datasetRowDateGranularity));
  }
