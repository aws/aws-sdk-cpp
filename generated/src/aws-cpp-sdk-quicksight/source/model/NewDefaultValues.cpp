...
NewDefaultValues::NewDefaultValues(JsonView jsonValue) { *this = jsonValue; }
...
NewDefaultValues& NewDefaultValues::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("StringStaticValues")) {
    ...
    for (unsigned stringStaticValuesIndex = 0; stringStaticValuesIndex < stringStaticValuesJsonList.GetLength();
         ++stringStaticValuesIndex) {
  ...
  if (jsonValue.ValueExists("DecimalStaticValues")) {
    ...
    for (unsigned decimalStaticValuesIndex = 0; decimalStaticValuesIndex < decimalStaticValuesJsonList.GetLength();
         ++decimalStaticValuesIndex) {
  ...
  if (jsonValue.ValueExists("DateTimeStaticValues")) {
    ...
    for (unsigned dateTimeStaticValuesIndex = 0; dateTimeStaticValuesIndex < dateTimeStaticValuesJsonList.GetLength();
         ++dateTimeStaticValuesIndex) {
  ...
  if (jsonValue.ValueExists("IntegerStaticValues")) {
    ...
    for (unsigned integerStaticValuesIndex = 0; integerStaticValuesIndex < integerStaticValuesJsonList.GetLength();
         ++integerStaticValuesIndex) {
...
JsonValue NewDefaultValues::Jsonize() const {
  ...
  if (m_dateTimeStaticValuesHasBeenSet) {
    ...
    for (unsigned dateTimeStaticValuesIndex = 0; dateTimeStaticValuesIndex < dateTimeStaticValuesJsonList.GetLength();
         ++dateTimeStaticValuesIndex) {
      dateTimeStaticValuesJsonList[dateTimeStaticValuesIndex].AsDouble(
          m_dateTimeStaticValues[dateTimeStaticValuesIndex].SecondsWithMSPrecision());
    }
