...
StringParameterDeclaration::StringParameterDeclaration(JsonView jsonValue) { *this = jsonValue; }
...
StringParameterDeclaration& StringParameterDeclaration::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("MappedDataSetParameters")) {
    ...
    for (unsigned mappedDataSetParametersIndex = 0; mappedDataSetParametersIndex < mappedDataSetParametersJsonList.GetLength();
         ++mappedDataSetParametersIndex) {
...
JsonValue StringParameterDeclaration::Jsonize() const {
  ...
  if (m_mappedDataSetParametersHasBeenSet) {
    ...
    for (unsigned mappedDataSetParametersIndex = 0; mappedDataSetParametersIndex < mappedDataSetParametersJsonList.GetLength();
         ++mappedDataSetParametersIndex) {
      mappedDataSetParametersJsonList[mappedDataSetParametersIndex].AsObject(
          m_mappedDataSetParameters[mappedDataSetParametersIndex].Jsonize());
    }
