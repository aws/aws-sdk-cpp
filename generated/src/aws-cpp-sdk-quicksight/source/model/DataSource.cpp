...
DataSource& DataSource::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("AlternateDataSourceParameters")) {
    ...
    for (unsigned alternateDataSourceParametersIndex = 0;
         alternateDataSourceParametersIndex < alternateDataSourceParametersJsonList.GetLength(); ++alternateDataSourceParametersIndex) {
...
JsonValue DataSource::Jsonize() const {
  ...
  if (m_alternateDataSourceParametersHasBeenSet) {
    ...
    for (unsigned alternateDataSourceParametersIndex = 0;
         alternateDataSourceParametersIndex < alternateDataSourceParametersJsonList.GetLength(); ++alternateDataSourceParametersIndex) {
      alternateDataSourceParametersJsonList[alternateDataSourceParametersIndex].AsObject(
          m_alternateDataSourceParameters[alternateDataSourceParametersIndex].Jsonize());
    }
