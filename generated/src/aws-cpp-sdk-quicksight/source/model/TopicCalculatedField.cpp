...
TopicCalculatedField::TopicCalculatedField(JsonView jsonValue) { *this = jsonValue; }
...
TopicCalculatedField& TopicCalculatedField::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("CalculatedFieldSynonyms")) {
    ...
    for (unsigned calculatedFieldSynonymsIndex = 0; calculatedFieldSynonymsIndex < calculatedFieldSynonymsJsonList.GetLength();
         ++calculatedFieldSynonymsIndex) {
  ...
  if (jsonValue.ValueExists("AllowedAggregations")) {
    ...
    for (unsigned allowedAggregationsIndex = 0; allowedAggregationsIndex < allowedAggregationsJsonList.GetLength();
         ++allowedAggregationsIndex) {
      m_allowedAggregations.push_back(AuthorSpecifiedAggregationMapper::GetAuthorSpecifiedAggregationForName(
          allowedAggregationsJsonList[allowedAggregationsIndex].AsString()));
    }
  ...
  if (jsonValue.ValueExists("NotAllowedAggregations")) {
    ...
    for (unsigned notAllowedAggregationsIndex = 0; notAllowedAggregationsIndex < notAllowedAggregationsJsonList.GetLength();
         ++notAllowedAggregationsIndex) {
      m_notAllowedAggregations.push_back(AuthorSpecifiedAggregationMapper::GetAuthorSpecifiedAggregationForName(
          notAllowedAggregationsJsonList[notAllowedAggregationsIndex].AsString()));
    }
...
JsonValue TopicCalculatedField::Jsonize() const {
  ...
  if (m_allowedAggregationsHasBeenSet) {
    ...
    for (unsigned allowedAggregationsIndex = 0; allowedAggregationsIndex < allowedAggregationsJsonList.GetLength();
         ++allowedAggregationsIndex) {
      allowedAggregationsJsonList[allowedAggregationsIndex].AsString(
          AuthorSpecifiedAggregationMapper::GetNameForAuthorSpecifiedAggregation(m_allowedAggregations[allowedAggregationsIndex]));
    }
  ...
  if (m_notAllowedAggregationsHasBeenSet) {
    ...
    for (unsigned notAllowedAggregationsIndex = 0; notAllowedAggregationsIndex < notAllowedAggregationsJsonList.GetLength();
         ++notAllowedAggregationsIndex) {
      notAllowedAggregationsJsonList[notAllowedAggregationsIndex].AsString(
          AuthorSpecifiedAggregationMapper::GetNameForAuthorSpecifiedAggregation(m_notAllowedAggregations[notAllowedAggregationsIndex]));
    }
