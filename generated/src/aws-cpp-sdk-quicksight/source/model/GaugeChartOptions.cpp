...
GaugeChartOptions::GaugeChartOptions(JsonView jsonValue) { *this = jsonValue; }

GaugeChartOptions& GaugeChartOptions::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("PrimaryValueDisplayType")) {
    m_primaryValueDisplayType =
        PrimaryValueDisplayTypeMapper::GetPrimaryValueDisplayTypeForName(jsonValue.GetString("PrimaryValueDisplayType"));
...
JsonValue GaugeChartOptions::Jsonize() const {
  ...
  if (m_primaryValueDisplayTypeHasBeenSet) {
    payload.WithString("PrimaryValueDisplayType",
                       PrimaryValueDisplayTypeMapper::GetNameForPrimaryValueDisplayType(m_primaryValueDisplayType));
  }
