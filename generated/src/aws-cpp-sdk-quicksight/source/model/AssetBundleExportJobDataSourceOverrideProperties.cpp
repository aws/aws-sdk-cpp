  ...
  if (jsonValue.ValueExists("Properties")) {
    ...
    for (unsigned propertiesIndex = 0; propertiesIndex < propertiesJsonList.GetLength(); ++propertiesIndex) {
      m_properties.push_back(
          AssetBundleExportJobDataSourcePropertyToOverrideMapper::GetAssetBundleExportJobDataSourcePropertyToOverrideForName(
...
JsonValue AssetBundleExportJobDataSourceOverrideProperties::Jsonize() const {
  ...
  if (m_propertiesHasBeenSet) {
    ...
    for (unsigned propertiesIndex = 0; propertiesIndex < propertiesJsonList.GetLength(); ++propertiesIndex) {
      propertiesJsonList[propertiesIndex].AsString(
          AssetBundleExportJobDataSourcePropertyToOverrideMapper::GetNameForAssetBundleExportJobDataSourcePropertyToOverride(
