...
ResourcePendingMaintenanceActions::ResourcePendingMaintenanceActions(const XmlNode& xmlNode) { *this = xmlNode; }
...
void ResourcePendingMaintenanceActions::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
  ...
  if (m_pendingMaintenanceActionDetailsHasBeenSet) {
    ...
    for (auto& item : m_pendingMaintenanceActionDetails) {
      ...
      pendingMaintenanceActionDetailsSs << location << index << locationValue
                                        << ".PendingMaintenanceActionDetails.PendingMaintenanceAction."
                                        << pendingMaintenanceActionDetailsIdx++;
...
void ResourcePendingMaintenanceActions::OutputToStream(Aws::OStream& oStream, const char* location) const {
  ...
  if (m_pendingMaintenanceActionDetailsHasBeenSet) {
    ...
    for (auto& item : m_pendingMaintenanceActionDetails) {
      Aws::StringStream pendingMaintenanceActionDetailsSs;
      pendingMaintenanceActionDetailsSs << location << ".PendingMaintenanceActionDetails.PendingMaintenanceAction."
                                        << pendingMaintenanceActionDetailsIdx++;
      item.OutputToStream(oStream, pendingMaintenanceActionDetailsSs.str().c_str());
    }
  }
