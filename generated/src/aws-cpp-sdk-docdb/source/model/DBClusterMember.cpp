...
DBClusterMember::DBClusterMember(const XmlNode& xmlNode) { *this = xmlNode; }
...
DBClusterMember& DBClusterMember::operator=(const XmlNode& xmlNode) {
  ...
  if (!resultNode.IsNull()) {
    ...
    if (!isClusterWriterNode.IsNull()) {
      m_isClusterWriter = StringUtils::ConvertToBool(
          StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(isClusterWriterNode.GetText()).c_str()).c_str());
...
void DBClusterMember::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const {
  ...
  if (m_dBClusterParameterGroupStatusHasBeenSet) {
    oStream << location << index << locationValue
            << ".DBClusterParameterGroupStatus=" << StringUtils::URLEncode(m_dBClusterParameterGroupStatus.c_str()) << "&";
  }
