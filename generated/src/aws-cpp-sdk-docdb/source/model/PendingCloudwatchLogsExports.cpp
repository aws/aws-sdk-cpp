...
PendingCloudwatchLogsExports::PendingCloudwatchLogsExports(const XmlNode& xmlNode) { *this = xmlNode; }
...
void PendingCloudwatchLogsExports::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index,
                                                  const char* locationValue) const {
  if (m_logTypesToEnableHasBeenSet) {
    unsigned logTypesToEnableIdx = 1;
    for (auto& item : m_logTypesToEnable) {
      oStream << location << index << locationValue << ".LogTypesToEnable.member." << logTypesToEnableIdx++ << "="
  ...
  if (m_logTypesToDisableHasBeenSet) {
    ...
    for (auto& item : m_logTypesToDisable) {
      oStream << location << index << locationValue << ".LogTypesToDisable.member." << logTypesToDisableIdx++ << "="
              << StringUtils::URLEncode(item.c_str()) << "&";
    }
  }
}
