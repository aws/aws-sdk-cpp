...
NotifyWorkersResult& NotifyWorkersResult::operator=(const Aws::AmazonWebServiceResult<JsonValue>& result) {
  ...
  if (jsonValue.ValueExists("NotifyWorkersFailureStatuses")) {
    ...
    for (unsigned notifyWorkersFailureStatusesIndex = 0;
         notifyWorkersFailureStatusesIndex < notifyWorkersFailureStatusesJsonList.GetLength(); ++notifyWorkersFailureStatusesIndex) {
