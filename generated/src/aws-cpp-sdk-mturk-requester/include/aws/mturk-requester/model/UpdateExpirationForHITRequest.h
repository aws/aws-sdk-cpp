...
class UpdateExpirationForHITRequest : public MTurkRequest {
 public:
  ...
  template <typename HITIdT = Aws::String>
  UpdateExpirationForHITRequest& WithHITId(HITIdT&& value) {
  ...
  void SetExpireAt(ExpireAtT&& value) {
    m_expireAtHasBeenSet = true;
    m_expireAt = std::forward<ExpireAtT>(value);
  }
  template <typename ExpireAtT = Aws::Utils::DateTime>
  UpdateExpirationForHITRequest& WithExpireAt(ExpireAtT&& value) {
    SetExpireAt(std::forward<ExpireAtT>(value));
    return *this;
  }
  ///@}
 private:
