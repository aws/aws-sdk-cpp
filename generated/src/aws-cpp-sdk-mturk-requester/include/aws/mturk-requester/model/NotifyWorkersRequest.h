...
class NotifyWorkersRequest : public MTurkRequest {
 public:
  ...
  void SetWorkerIds(WorkerIdsT&& value) {
    m_workerIdsHasBeenSet = true;
    m_workerIds = std::forward<WorkerIdsT>(value);
  }
  template <typename WorkerIdsT = Aws::Vector<Aws::String>>
  NotifyWorkersRequest& WithWorkerIds(WorkerIdsT&& value) {
    SetWorkerIds(std::forward<WorkerIdsT>(value));
    return *this;
  }
  template <typename WorkerIdsT = Aws::String>
  NotifyWorkersRequest& AddWorkerIds(WorkerIdsT&& value) {
    m_workerIdsHasBeenSet = true;
    m_workerIds.emplace_back(std::forward<WorkerIdsT>(value));
    return *this;
  }
  ///@}
 private:
