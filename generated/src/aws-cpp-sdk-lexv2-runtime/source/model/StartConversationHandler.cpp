...
StartConversationHandler::StartConversationHandler() : EventStreamHandler() {
  ...
  m_onHeartbeatEvent = [&](const HeartbeatEvent&) { AWS_LOGSTREAM_TRACE(STARTCONVERSATION_HANDLER_CLASS_TAG, "HeartbeatEvent received."); };
  ...
  switch (StartConversationEventMapper::GetStartConversationEventTypeForName(eventTypeHeaderIter->second.GetEventHeaderValueAsString())) {
    ...
    case StartConversationEventType::PLAYBACKINTERRUPTIONEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCONVERSATION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper PlaybackInterruptionEvent object from the response in JSON format.");
        break;
      }
    ...
    case StartConversationEventType::TRANSCRIPTEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCONVERSATION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper TranscriptEvent object from the response in JSON format.");
        break;
      }
    ...
    case StartConversationEventType::INTENTRESULTEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCONVERSATION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper IntentResultEvent object from the response in JSON format.");
        break;
      }
    ...
    case StartConversationEventType::TEXTRESPONSEEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCONVERSATION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper TextResponseEvent object from the response in JSON format.");
        break;
      }
    ...
    case StartConversationEventType::AUDIORESPONSEEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCONVERSATION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper AudioResponseEvent object from the response in JSON format.");
        break;
      }
    ...
    case StartConversationEventType::HEARTBEATEVENT: {
      ...
      if (!json.WasParseSuccessful()) {
        AWS_LOGSTREAM_WARN(STARTCONVERSATION_HANDLER_CLASS_TAG,
                           "Unable to generate a proper HeartbeatEvent object from the response in JSON format.");
        break;
      }
...
void StartConversationHandler::HandleErrorInMessage() {
  ...
  if (errorHeaderIter == headers.end()) {
    ...
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_WARN(STARTCONVERSATION_HANDLER_CLASS_TAG, "Error type was not found in the event message.");
      return;
    }
  }
  ...
  if (errorHeaderIter == headers.end()) {
    errorHeaderIter = headers.find(EXCEPTION_TYPE_HEADER);
    if (errorHeaderIter == headers.end()) {
      AWS_LOGSTREAM_ERROR(STARTCONVERSATION_HANDLER_CLASS_TAG, "Error description was not found in the event message.");
      return;
    }
    ...
    if (!exceptionPayload.WasParseSuccessful()) {
      ...
      if (contentTypeIter != headers.end()) {
        AWS_LOGSTREAM_DEBUG(STARTCONVERSATION_HANDLER_CLASS_TAG,
