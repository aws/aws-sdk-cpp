...
class RuntimeHints {
 public:
  ...
  void SetSlotHints(SlotHintsT&& value) {
    ...
    m_slotHints = std::forward<SlotHintsT>(value);
  }
  template <typename SlotHintsT = Aws::Map<Aws::String, Aws::Map<Aws::String, RuntimeHintDetails>>>
  RuntimeHints& WithSlotHints(SlotHintsT&& value) {
    SetSlotHints(std::forward<SlotHintsT>(value));
    return *this;
  }
  ...
  RuntimeHints& AddSlotHints(SlotHintsKeyT&& key, SlotHintsValueT&& value) {
    m_slotHintsHasBeenSet = true;
    m_slotHints.emplace(std::forward<SlotHintsKeyT>(key), std::forward<SlotHintsValueT>(value));
    return *this;
  }
  ///@}
 private:
