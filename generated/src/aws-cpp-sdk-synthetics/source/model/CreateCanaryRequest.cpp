...
Aws::String CreateCanaryRequest::SerializePayload() const {
  ...
  if (m_resourcesToReplicateTagsHasBeenSet) {
    ...
    for (unsigned resourcesToReplicateTagsIndex = 0; resourcesToReplicateTagsIndex < resourcesToReplicateTagsJsonList.GetLength();
         ++resourcesToReplicateTagsIndex) {
      resourcesToReplicateTagsJsonList[resourcesToReplicateTagsIndex].AsString(
          ResourceToTagMapper::GetNameForResourceToTag(m_resourcesToReplicateTags[resourcesToReplicateTagsIndex]));
    }
  ...
  if (m_provisionedResourceCleanupHasBeenSet) {
    payload.WithString("ProvisionedResourceCleanup",
                       ProvisionedResourceCleanupSettingMapper::GetNameForProvisionedResourceCleanupSetting(m_provisionedResourceCleanup));
  }
