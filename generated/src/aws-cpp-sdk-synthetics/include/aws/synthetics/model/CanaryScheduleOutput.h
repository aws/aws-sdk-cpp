...
class CanaryScheduleOutput {
 public:
  ...
  inline CanaryScheduleOutput& WithDurationInSeconds(long long value) {
  ...
  void SetRetryConfig(RetryConfigT&& value) {
    ...
    m_retryConfig = std::forward<RetryConfigT>(value);
  }
  template <typename RetryConfigT = RetryConfigOutput>
  CanaryScheduleOutput& WithRetryConfig(RetryConfigT&& value) {
    SetRetryConfig(std::forward<RetryConfigT>(value));
    return *this;
  }
  ///@}
 private:
