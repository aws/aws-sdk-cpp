...
template <>
AWS_MEDIAPACKAGEV2_API ConflictException Mediapackagev2Error::GetModeledError() {
  assert(this->GetErrorType() == Mediapackagev2Errors::CONFLICT);
  return ConflictException(this->GetJsonPayload().View());
}

template <>
AWS_MEDIAPACKAGEV2_API ResourceNotFoundException Mediapackagev2Error::GetModeledError() {
  assert(this->GetErrorType() == Mediapackagev2Errors::RESOURCE_NOT_FOUND);
  return ResourceNotFoundException(this->GetJsonPayload().View());
}

template <>
AWS_MEDIAPACKAGEV2_API ValidationException Mediapackagev2Error::GetModeledError() {
...
AWSError<CoreErrors> GetErrorForName(const char* errorName) {
  ...
  if (hashCode == CONFLICT_HASH) {
    ...
    return AWSError<CoreErrors>(static_cast<CoreErrors>(Mediapackagev2Errors::INTERNAL_SERVER), RetryableType::RETRYABLE);
  }
