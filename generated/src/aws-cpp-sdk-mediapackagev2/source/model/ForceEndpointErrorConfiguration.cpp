...
ForceEndpointErrorConfiguration::ForceEndpointErrorConfiguration(JsonView jsonValue) { *this = jsonValue; }
...
ForceEndpointErrorConfiguration& ForceEndpointErrorConfiguration::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("EndpointErrorConditions")) {
    ...
    for (unsigned endpointErrorConditionsIndex = 0; endpointErrorConditionsIndex < endpointErrorConditionsJsonList.GetLength();
         ++endpointErrorConditionsIndex) {
      m_endpointErrorConditions.push_back(EndpointErrorConditionMapper::GetEndpointErrorConditionForName(
          endpointErrorConditionsJsonList[endpointErrorConditionsIndex].AsString()));
    }
...
JsonValue ForceEndpointErrorConfiguration::Jsonize() const {
  ...
  if (m_endpointErrorConditionsHasBeenSet) {
    ...
    for (unsigned endpointErrorConditionsIndex = 0; endpointErrorConditionsIndex < endpointErrorConditionsJsonList.GetLength();
         ++endpointErrorConditionsIndex) {
      endpointErrorConditionsJsonList[endpointErrorConditionsIndex].AsString(
          EndpointErrorConditionMapper::GetNameForEndpointErrorCondition(m_endpointErrorConditions[endpointErrorConditionsIndex]));
    }
