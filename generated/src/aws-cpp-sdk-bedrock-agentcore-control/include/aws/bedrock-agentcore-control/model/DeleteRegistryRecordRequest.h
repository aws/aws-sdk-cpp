...
/**
 */
class DeleteRegistryRecordRequest : public BedrockAgentCoreControlRequest {
 public:
  AWS_BEDROCKAGENTCORECONTROL_API DeleteRegistryRecordRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteRegistryRecord"; }
  ...
  template <typename RegistryIdT = Aws::String>
  DeleteRegistryRecordRequest& WithRegistryId(RegistryIdT&& value) {
  ...
  ///@{
  /**
   * <p>The identifier of the registry record to delete. You can specify either the
  ...
  template <typename RecordIdT = Aws::String>
  DeleteRegistryRecordRequest& WithRecordId(RecordIdT&& value) {
