...
class McpLambdaTargetConfiguration {
 public:
  ...
  template <typename LambdaArnT = Aws::String>
  McpLambdaTargetConfiguration& WithLambdaArn(LambdaArnT&& value) {
  ...
  void SetToolSchema(ToolSchemaT&& value) {
    m_toolSchemaHasBeenSet = true;
    m_toolSchema = std::forward<ToolSchemaT>(value);
  }
  template <typename ToolSchemaT = ToolSchema>
  McpLambdaTargetConfiguration& WithToolSchema(ToolSchemaT&& value) {
    SetToolSchema(std::forward<ToolSchemaT>(value));
    return *this;
  }
  ///@}
 private:
