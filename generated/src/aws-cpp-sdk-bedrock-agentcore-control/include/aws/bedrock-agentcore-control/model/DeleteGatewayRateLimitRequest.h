...
/**
 */
class DeleteGatewayRateLimitRequest : public BedrockAgentCoreControlRequest {
 public:
  AWS_BEDROCKAGENTCORECONTROL_API DeleteGatewayRateLimitRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "DeleteGatewayRateLimit"; }
  ...
  template <typename GatewayIdentifierT = Aws::String>
  DeleteGatewayRateLimitRequest& WithGatewayIdentifier(GatewayIdentifierT&& value) {
  ...
  template <typename RateLimitIdT = Aws::String>
  DeleteGatewayRateLimitRequest& WithRateLimitId(RateLimitIdT&& value) {
