...
/**
 */
class GetPolicyGenerationSummaryRequest : public BedrockAgentCoreControlRequest {
 public:
  AWS_BEDROCKAGENTCORECONTROL_API GetPolicyGenerationSummaryRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "GetPolicyGenerationSummary"; }

  AWS_BEDROCKAGENTCORECONTROL_API Aws::String SerializePayload() const override;

  ///@{
  /**
   * <p>The unique identifier of the policy generation request to retrieve the
   * summary for.</p>
   */
  ...
  template <typename PolicyGenerationIdT = Aws::String>
  GetPolicyGenerationSummaryRequest& WithPolicyGenerationId(PolicyGenerationIdT&& value) {
  ...
  template <typename PolicyEngineIdT = Aws::String>
  GetPolicyGenerationSummaryRequest& WithPolicyEngineId(PolicyEngineIdT&& value) {
