...
/**
 */
class CreatePaymentCredentialProviderRequest : public BedrockAgentCoreControlRequest {
 public:
  AWS_BEDROCKAGENTCORECONTROL_API CreatePaymentCredentialProviderRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "CreatePaymentCredentialProvider"; }
  ...
  template <typename NameT = Aws::String>
  CreatePaymentCredentialProviderRequest& WithName(NameT&& value) {
  ...
  inline CreatePaymentCredentialProviderRequest& WithCredentialProviderVendor(PaymentCredentialProviderVendorType value) {
  ...
  template <typename ProviderConfigurationInputT = PaymentProviderConfigurationInput>
  CreatePaymentCredentialProviderRequest& WithProviderConfigurationInput(ProviderConfigurationInputT&& value) {
  ...
  template <typename TagsT = Aws::Map<Aws::String, Aws::String>>
  CreatePaymentCredentialProviderRequest& WithTags(TagsT&& value) {
    SetTags(std::forward<TagsT>(value));
    return *this;
  }
  template <typename TagsKeyT = Aws::String, typename TagsValueT = Aws::String>
  CreatePaymentCredentialProviderRequest& AddTags(TagsKeyT&& key, TagsValueT&& value) {
    m_tagsHasBeenSet = true;
