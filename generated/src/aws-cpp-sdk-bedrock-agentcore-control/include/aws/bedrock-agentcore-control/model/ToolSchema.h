...
class ToolSchema {
 public:
  ...
  template <typename InlinePayloadT = Aws::Vector<ToolDefinition>>
  ToolSchema& WithInlinePayload(InlinePayloadT&& value) {
    SetInlinePayload(std::forward<InlinePayloadT>(value));
    return *this;
  }
  template <typename InlinePayloadT = ToolDefinition>
  ToolSchema& AddInlinePayload(InlinePayloadT&& value) {
    m_inlinePayloadHasBeenSet = true;
    m_inlinePayload.emplace_back(std::forward<InlinePayloadT>(value));
    return *this;
  }
  ///@}
 private:
