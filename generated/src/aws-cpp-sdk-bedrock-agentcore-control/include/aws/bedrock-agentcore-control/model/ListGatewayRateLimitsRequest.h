...
/**
 */
class ListGatewayRateLimitsRequest : public BedrockAgentCoreControlRequest {
 public:
  AWS_BEDROCKAGENTCORECONTROL_API ListGatewayRateLimitsRequest() = default;

  // Service request name is the Operation name which will send this request out,
  // each operation should has unique request name, so that we can get operation's name from this request.
  // Note: this is not true for response, multiple operations may have the same response name,
  // so we can not get operation's name from response.
  inline virtual const char* GetServiceRequestName() const override { return "ListGatewayRateLimits"; }
  ...
  template <typename GatewayIdentifierT = Aws::String>
  ListGatewayRateLimitsRequest& WithGatewayIdentifier(GatewayIdentifierT&& value) {
  ...
  inline ListGatewayRateLimitsRequest& WithMaxResults(int value) {
    SetMaxResults(value);
    return *this;
  }
  ///@}
  ...
  ///@{
  /**
   * <p>The token to use to retrieve the next page of results. Use the value returned
   * in a previous <code>ListGatewayRateLimits</code> response.</p>
   */
  ...
  template <typename NextTokenT = Aws::String>
  ListGatewayRateLimitsRequest& WithNextToken(NextTokenT&& value) {
