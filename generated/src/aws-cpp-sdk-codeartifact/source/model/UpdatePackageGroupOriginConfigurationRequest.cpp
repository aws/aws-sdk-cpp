...
Aws::String UpdatePackageGroupOriginConfigurationRequest::SerializePayload() const {
  ...
  if (m_restrictionsHasBeenSet) {
    ...
    for (auto& restrictionsItem : m_restrictions) {
      restrictionsJsonMap.WithString(
          PackageGroupOriginRestrictionTypeMapper::GetNameForPackageGroupOriginRestrictionType(restrictionsItem.first),
          PackageGroupOriginRestrictionModeMapper::GetNameForPackageGroupOriginRestrictionMode(restrictionsItem.second));
    }
  ...
  if (m_addAllowedRepositoriesHasBeenSet) {
    ...
    for (unsigned addAllowedRepositoriesIndex = 0; addAllowedRepositoriesIndex < addAllowedRepositoriesJsonList.GetLength();
         ++addAllowedRepositoriesIndex) {
  ...
  if (m_removeAllowedRepositoriesHasBeenSet) {
    ...
    for (unsigned removeAllowedRepositoriesIndex = 0; removeAllowedRepositoriesIndex < removeAllowedRepositoriesJsonList.GetLength();
         ++removeAllowedRepositoriesIndex) {
      removeAllowedRepositoriesJsonList[removeAllowedRepositoriesIndex].AsObject(
          m_removeAllowedRepositories[removeAllowedRepositoriesIndex].Jsonize());
    }
