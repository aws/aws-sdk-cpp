...
VpcOutputSettings::VpcOutputSettings(JsonView jsonValue) { *this = jsonValue; }
...
VpcOutputSettings& VpcOutputSettings::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("publicAddressAllocationIds")) {
    ...
    for (unsigned publicAddressAllocationIdsIndex = 0; publicAddressAllocationIdsIndex < publicAddressAllocationIdsJsonList.GetLength();
         ++publicAddressAllocationIdsIndex) {
...
JsonValue VpcOutputSettings::Jsonize() const {
  ...
  if (m_publicAddressAllocationIdsHasBeenSet) {
    ...
    for (unsigned publicAddressAllocationIdsIndex = 0; publicAddressAllocationIdsIndex < publicAddressAllocationIdsJsonList.GetLength();
         ++publicAddressAllocationIdsIndex) {
      publicAddressAllocationIdsJsonList[publicAddressAllocationIdsIndex].AsString(
          m_publicAddressAllocationIds[publicAddressAllocationIdsIndex]);
    }
