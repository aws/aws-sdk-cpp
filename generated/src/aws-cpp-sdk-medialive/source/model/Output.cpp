...
Output::Output(JsonView jsonValue) { *this = jsonValue; }
...
Output& Output::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("audioDescriptionNames")) {
    ...
    for (unsigned audioDescriptionNamesIndex = 0; audioDescriptionNamesIndex < audioDescriptionNamesJsonList.GetLength();
         ++audioDescriptionNamesIndex) {
  ...
  if (jsonValue.ValueExists("captionDescriptionNames")) {
    ...
    for (unsigned captionDescriptionNamesIndex = 0; captionDescriptionNamesIndex < captionDescriptionNamesJsonList.GetLength();
         ++captionDescriptionNamesIndex) {
