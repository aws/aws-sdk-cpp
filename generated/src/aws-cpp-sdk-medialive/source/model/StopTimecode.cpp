...
StopTimecode::StopTimecode(JsonView jsonValue) { *this = jsonValue; }

StopTimecode& StopTimecode::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("lastFrameClippingBehavior")) {
    m_lastFrameClippingBehavior =
        LastFrameClippingBehaviorMapper::GetLastFrameClippingBehaviorForName(jsonValue.GetString("lastFrameClippingBehavior"));
...
JsonValue StopTimecode::Jsonize() const {
  ...
  if (m_lastFrameClippingBehaviorHasBeenSet) {
    payload.WithString("lastFrameClippingBehavior",
                       LastFrameClippingBehaviorMapper::GetNameForLastFrameClippingBehavior(m_lastFrameClippingBehavior));
  }
