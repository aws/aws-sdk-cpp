...
ChannelAlert::ChannelAlert(JsonView jsonValue) { *this = jsonValue; }

ChannelAlert& ChannelAlert::operator=(JsonView jsonValue) {
  ...
  if (jsonValue.ValueExists("state")) {
    m_state = ChannelAlertStateMapper::GetChannelAlertStateForName(jsonValue.GetString("state"));
    m_stateHasBeenSet = true;
  }
  return *this;
}

JsonValue ChannelAlert::Jsonize() const {
  JsonValue payload;
  ...
  if (m_stateHasBeenSet) {
    payload.WithString("state", ChannelAlertStateMapper::GetNameForChannelAlertState(m_state));
  }
