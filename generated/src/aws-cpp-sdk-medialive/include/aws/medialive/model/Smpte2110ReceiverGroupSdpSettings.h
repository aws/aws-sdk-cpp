...
Each stream encapsulates one captions
  ...
  void SetVideoSdp(VideoSdpT&& value) {
    m_videoSdpHasBeenSet = true;
    m_videoSdp = std::forward<VideoSdpT>(value);
  }
  template <typename VideoSdpT = InputSdpLocation>
  Smpte2110ReceiverGroupSdpSettings& WithVideoSdp(VideoSdpT&& value) {
    SetVideoSdp(std::forward<VideoSdpT>(value));
    return *this;
  }
  ///@}
 private:
