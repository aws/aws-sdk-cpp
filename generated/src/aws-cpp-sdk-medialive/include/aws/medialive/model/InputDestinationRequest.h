...
is going to be on the customer local network.
  ...
  void SetNetworkRoutes(NetworkRoutesT&& value) {
    ...
    m_networkRoutes = std::forward<NetworkRoutesT>(value);
  }
  template <typename NetworkRoutesT = Aws::Vector<InputRequestDestinationRoute>>
  InputDestinationRequest& WithNetworkRoutes(NetworkRoutesT&& value) {
    SetNetworkRoutes(std::forward<NetworkRoutesT>(value));
    return *this;
  }
  ...
  InputDestinationRequest& AddNetworkRoutes(NetworkRoutesT&& value) {
    m_networkRoutesHasBeenSet = true;
    m_networkRoutes.emplace_back(std::forward<NetworkRoutesT>(value));
    return *this;
  }
  ///@}
...
of the input is going to be on the customer local network.
  ...
  void SetStaticIpAddress(StaticIpAddressT&& value) {
    m_staticIpAddressHasBeenSet = true;
    m_staticIpAddress = std::forward<StaticIpAddressT>(value);
  }
  template <typename StaticIpAddressT = Aws::String>
  InputDestinationRequest& WithStaticIpAddress(StaticIpAddressT&& value) {
    SetStaticIpAddress(std::forward<StaticIpAddressT>(value));
    return *this;
  }
  ///@}
 private:
