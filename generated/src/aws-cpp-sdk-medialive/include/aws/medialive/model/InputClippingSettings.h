...
class InputClippingSettings {
 public:
  ...
  inline void SetInputTimecodeSource(InputTimecodeSource value) {
    m_inputTimecodeSourceHasBeenSet = true;
    m_inputTimecodeSource = value;
  }
  inline InputClippingSettings& WithInputTimecodeSource(InputTimecodeSource value) {
    SetInputTimecodeSource(value);
    return *this;
  }
  ///@}
  ...
  void SetStopTimecode(StopTimecodeT&& value) {
    m_stopTimecodeHasBeenSet = true;
    m_stopTimecode = std::forward<StopTimecodeT>(value);
  }
  template <typename StopTimecodeT = StopTimecode>
  InputClippingSettings& WithStopTimecode(StopTimecodeT&& value) {
    SetStopTimecode(std::forward<StopTimecodeT>(value));
    return *this;
  }
  ///@}
 private:
