...
class StandardHlsSettings {
 public:
  ...
  void SetAudioRenditionSets(AudioRenditionSetsT&& value) {
    ...
    m_audioRenditionSets = std::forward<AudioRenditionSetsT>(value);
  }
  template <typename AudioRenditionSetsT = Aws::String>
  StandardHlsSettings& WithAudioRenditionSets(AudioRenditionSetsT&& value) {
    SetAudioRenditionSets(std::forward<AudioRenditionSetsT>(value));
    return *this;
  }
  ///@}
  ...
  void SetM3u8Settings(M3u8SettingsT&& value) {
    m_m3u8SettingsHasBeenSet = true;
    m_m3u8Settings = std::forward<M3u8SettingsT>(value);
  }
  template <typename M3u8SettingsT = M3u8Settings>
  StandardHlsSettings& WithM3u8Settings(M3u8SettingsT&& value) {
    SetM3u8Settings(std::forward<M3u8SettingsT>(value));
    return *this;
  }
  ///@}
 private:
