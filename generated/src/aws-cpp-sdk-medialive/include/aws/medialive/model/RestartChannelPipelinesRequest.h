...
class RestartChannelPipelinesRequest : public MediaLiveRequest {
 public:
  ...
  void SetPipelineIds(PipelineIdsT&& value) {
    m_pipelineIdsHasBeenSet = true;
    m_pipelineIds = std::forward<PipelineIdsT>(value);
  }
  template <typename PipelineIdsT = Aws::Vector<ChannelPipelineIdToRestart>>
  RestartChannelPipelinesRequest& WithPipelineIds(PipelineIdsT&& value) {
    SetPipelineIds(std::forward<PipelineIdsT>(value));
    return *this;
  }
  inline RestartChannelPipelinesRequest& AddPipelineIds(ChannelPipelineIdToRestart value) {
    m_pipelineIdsHasBeenSet = true;
    m_pipelineIds.push_back(value);
    return *this;
  }
  ///@}
 private:
