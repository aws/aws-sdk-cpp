...
class Rule {
 public:
  ...
  void SetNonTalkTimeFilter(NonTalkTimeFilterT&& value) {
    ...
    m_nonTalkTimeFilter = std::forward<NonTalkTimeFilterT>(value);
  }
  template <typename NonTalkTimeFilterT = NonTalkTimeFilter>
  Rule& WithNonTalkTimeFilter(NonTalkTimeFilterT&& value) {
    SetNonTalkTimeFilter(std::forward<NonTalkTimeFilterT>(value));
    return *this;
  }
  ///@}
  ...
  void SetInterruptionFilter(InterruptionFilterT&& value) {
    ...
    m_interruptionFilter = std::forward<InterruptionFilterT>(value);
  }
  template <typename InterruptionFilterT = InterruptionFilter>
  Rule& WithInterruptionFilter(InterruptionFilterT&& value) {
    SetInterruptionFilter(std::forward<InterruptionFilterT>(value));
    return *this;
  }
  ///@}
  ...
  void SetSentimentFilter(SentimentFilterT&& value) {
    ...
    m_sentimentFilter = std::forward<SentimentFilterT>(value);
  }
  template <typename SentimentFilterT = SentimentFilter>
  Rule& WithSentimentFilter(SentimentFilterT&& value) {
    SetSentimentFilter(std::forward<SentimentFilterT>(value));
    return *this;
  }
  ///@}
 private:
