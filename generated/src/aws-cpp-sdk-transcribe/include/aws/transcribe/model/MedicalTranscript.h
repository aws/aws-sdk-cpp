...
class MedicalTranscript {
 public:
  ...
  void SetTranscriptFileUri(TranscriptFileUriT&& value) {
    ...
    m_transcriptFileUri = std::forward<TranscriptFileUriT>(value);
  }
  template <typename TranscriptFileUriT = Aws::String>
  MedicalTranscript& WithTranscriptFileUri(TranscriptFileUriT&& value) {
    SetTranscriptFileUri(std::forward<TranscriptFileUriT>(value));
    return *this;
  }
  ///@}
 private:
