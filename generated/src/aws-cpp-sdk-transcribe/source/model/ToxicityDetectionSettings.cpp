...
ToxicityDetectionSettings::ToxicityDetectionSettings(JsonView jsonValue) { *this = jsonValue; }
...
ToxicityDetectionSettings& ToxicityDetectionSettings::operator=(JsonView jsonValue) {
  if (jsonValue.ValueExists("ToxicityCategories")) {
    ...
    for (unsigned toxicityCategoriesIndex = 0; toxicityCategoriesIndex < toxicityCategoriesJsonList.GetLength();
         ++toxicityCategoriesIndex) {
      m_toxicityCategories.push_back(
          ToxicityCategoryMapper::GetToxicityCategoryForName(toxicityCategoriesJsonList[toxicityCategoriesIndex].AsString()));
    }
...
JsonValue ToxicityDetectionSettings::Jsonize() const {
  ...
  if (m_toxicityCategoriesHasBeenSet) {
    ...
    for (unsigned toxicityCategoriesIndex = 0; toxicityCategoriesIndex < toxicityCategoriesJsonList.GetLength();
         ++toxicityCategoriesIndex) {
      toxicityCategoriesJsonList[toxicityCategoriesIndex].AsString(
          ToxicityCategoryMapper::GetNameForToxicityCategory(m_toxicityCategories[toxicityCategoriesIndex]));
    }
