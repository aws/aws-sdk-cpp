...
GreetingStruct::GreetingStruct(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder)
{
*this = decoder;
}

GreetingStruct& GreetingStruct::operator =(const std::shared_ptr<Aws::Crt::Cbor::CborDecoder>& decoder)
{
    if (decoder != nullptr)
{
...
void GreetingStruct::CborEncode(Aws::Crt::Cbor::CborEncoder& encoder) const
{

    // Calculate map size
    size_t mapSize = 0;
                                        if(m_hiHasBeenSet) mapSize++;
  ...
  encoder.WriteMapStart(mapSize);
    ...
    if(m_hiHasBeenSet)
    {
        encoder.WriteText(Aws::Crt::ByteCursorFromCString("hi"));
        encoder.WriteText(Aws::Crt::ByteCursorFromCString(m_hi.c_str()));
     }
}
