...
{
template<> AWS_JSONPROTOCOL_API ErrorWithMembers JsonProtocolError::GetModeledError()
{
  assert(this->GetErrorType() == JsonProtocolErrors::ERROR_WITH_MEMBERS);
  return ErrorWithMembers(this->GetJsonPayload().View());
}

template<> AWS_JSONPROTOCOL_API ComplexError JsonProtocolError::GetModeledError()
{
