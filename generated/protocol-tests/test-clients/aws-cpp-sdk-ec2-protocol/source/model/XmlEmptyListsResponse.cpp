...
XmlEmptyListsResponse::XmlEmptyListsResponse()
{
}

XmlEmptyListsResponse::XmlEmptyListsResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  *this = result;
}

XmlEmptyListsResponse& XmlEmptyListsResponse::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "XmlEmptyListsResponse"))
  {
    resultNode = rootNode.FirstChild("XmlEmptyListsResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2Protocol::Model::XmlEmptyListsResponse", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
