...
SimpleScalarXmlPropertiesResponse& SimpleScalarXmlPropertiesResponse::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "SimpleScalarXmlPropertiesResponse"))
  {
    resultNode = rootNode.FirstChild("SimpleScalarXmlPropertiesResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2Protocol::Model::SimpleScalarXmlPropertiesResponse", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
