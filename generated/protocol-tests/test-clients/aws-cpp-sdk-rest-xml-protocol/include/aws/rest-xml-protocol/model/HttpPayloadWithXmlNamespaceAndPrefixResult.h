...
class HttpPayloadWithXmlNamespaceAndPrefixResult {
 public:
  ...
  AWS_RESTXMLPROTOCOL_API HttpPayloadWithXmlNamespaceAndPrefixResult& operator=(
      const Aws::AmazonWebServiceResult<Aws::Utils::Xml::XmlDocument>& result);
  ...
  template <typename NestedT = PayloadWithXmlNamespaceAndPrefix>
  HttpPayloadWithXmlNamespaceAndPrefixResult& WithNested(NestedT&& value) {
  ...
  template <typename RequestIdT = Aws::String>
  HttpPayloadWithXmlNamespaceAndPrefixResult& WithRequestId(RequestIdT&& value) {
