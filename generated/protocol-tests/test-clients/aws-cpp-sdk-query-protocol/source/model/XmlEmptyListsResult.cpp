...
XmlEmptyListsResult& XmlEmptyListsResult::operator=(const Aws::AmazonWebServiceResult<XmlDocument>& result) {
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "XmlEmptyListsResult")) {
    resultNode = rootNode.FirstChild("XmlEmptyListsResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::QueryProtocol::Model::XmlEmptyListsResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId());
  }
  return *this;
}
