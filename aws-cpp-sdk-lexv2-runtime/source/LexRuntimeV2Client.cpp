...
{
  std::shared_ptr<RecognizeTextRequest> pRequest = request.Clone();
  auto task = Aws::MakeShared< std::packaged_task< RecognizeTextOutcome() > >(ALLOCATION_TAG, [this, pRequest](){ return this->RecognizeText(*pRequest); } );
  auto packagedFunction = [task]() { (*task)(); };
  m_executor->Submit(packagedFunction);
  return task->get_future();
}
void LexRuntimeV2Client::RecognizeTextAsync(const RecognizeTextRequest& request, const RecognizeTextResponseReceivedHandler& handler, const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context) const
{
  ...
  m_executor->Submit( [this, pRequest, handler, context]()
    {
      handler(this, *pRequest, RecognizeText(*pRequest), context);
    } );
}
...
{
  std::shared_ptr<RecognizeUtteranceRequest> pRequest = request.Clone();
  auto task = Aws::MakeShared< std::packaged_task< RecognizeUtteranceOutcome() > >(ALLOCATION_TAG, [this, pRequest](){ return this->RecognizeUtterance(*pRequest); } );
  auto packagedFunction = [task]() { (*task)(); };
  m_executor->Submit(packagedFunction);
  return task->get_future();
}
void LexRuntimeV2Client::RecognizeUtteranceAsync(const RecognizeUtteranceRequest& request, const RecognizeUtteranceResponseReceivedHandler& handler, const std::shared_ptr<const Aws::Client::AsyncCallerContext>& context) const
{
  ...
  m_executor->Submit( [this, pRequest, handler, context]()
    {
      handler(this, *pRequest, RecognizeUtterance(*pRequest), context);
    } );
}
