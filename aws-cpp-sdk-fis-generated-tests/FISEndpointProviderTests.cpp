...
static const char* ALLOCATION_TAG = "AWSFISEndpointProviderTests";
using FISEndpointProvider = Aws::FIS::Endpoint::FISEndpointProvider;
using EndpointParameters = Aws::Vector<Aws::Endpoint::EndpointParameter>;
using FISResolveEndpointOutcome = Aws::FIS::Endpoint::FISEndpointProvider::FISResolveEndpointOutcome;
...
class FISEndpointProviderTests :public ::testing::TestWithParam<size_t>
{
public:
...
struct FISEndpointProviderEndpointTestCase
{
...
static const Aws::Vector<FISEndpointProviderEndpointTestCase> TEST_CASES = {
  /*TEST CASE 0*/
...
void ValidateOutcome(const FISResolveEndpointOutcome& outcome, const FISEndpointProviderEndpointTestCase::Expect& expect)
{
...
TEST_P(FISEndpointProviderTests, EndpointProviderTest)
{
    ...
    const FISEndpointProviderEndpointTestCase& TEST_CASE = TEST_CASES.at(TEST_CASE_IDX);
    SCOPED_TRACE(Aws::String("\nTEST CASE # ") + Aws::Utils::StringUtils::to_string(TEST_CASE_IDX) + ": " + TEST_CASE.documentation);

    std::shared_ptr<FISEndpointProvider> endpointProvider = Aws::MakeShared<FISEndpointProvider>(ALLOCATION_TAG);
    ASSERT_TRUE(endpointProvider) << "Failed to allocate/initialize FISEndpointProvider";
    ...
    for(const auto& operation : TEST_CASE.operationInput)
    {
         ...
         */
        const Aws::Vector<std::reference_wrapper<const FISEndpointProviderEndpointTestCase::OperationParamsFromTest>>
