...
static const char* ALLOCATION_TAG = "AWSShieldEndpointProviderTests";
using ShieldEndpointProvider = Aws::Shield::Endpoint::ShieldEndpointProvider;
using EndpointParameters = Aws::Vector<Aws::Endpoint::EndpointParameter>;
using ShieldResolveEndpointOutcome = Aws::Shield::Endpoint::ShieldEndpointProvider::ShieldResolveEndpointOutcome;
...
class ShieldEndpointProviderTests :public ::testing::TestWithParam<size_t>
{
public:
...
struct ShieldEndpointProviderEndpointTestCase
{
...
static const Aws::Vector<ShieldEndpointProviderEndpointTestCase> TEST_CASES = {
  /*TEST CASE 0*/
  {"For region aws-global with FIPS disabled and DualStack disabled", // documentation
    ...
    {}, // tags
    {{/*epUrl*/"https://shield.us-east-1.amazonaws.com",
       {/*properties*/{"authSchemes", {EpProp("signingRegion", "us-east-1"), EpProp("name", "sigv4"), EpProp("signingName", "shield")}}},
...
void ValidateOutcome(const ShieldResolveEndpointOutcome& outcome, const ShieldEndpointProviderEndpointTestCase::Expect& expect)
{
...
TEST_P(ShieldEndpointProviderTests, EndpointProviderTest)
{
    ...
    const ShieldEndpointProviderEndpointTestCase& TEST_CASE = TEST_CASES.at(TEST_CASE_IDX);
    SCOPED_TRACE(Aws::String("\nTEST CASE # ") + Aws::Utils::StringUtils::to_string(TEST_CASE_IDX) + ": " + TEST_CASE.documentation);

    std::shared_ptr<ShieldEndpointProvider> endpointProvider = Aws::MakeShared<ShieldEndpointProvider>(ALLOCATION_TAG);
    ASSERT_TRUE(endpointProvider) << "Failed to allocate/initialize ShieldEndpointProvider";
    ...
    for(const auto& operation : TEST_CASE.operationInput)
    {
         ...
         */
        const Aws::Vector<std::reference_wrapper<const ShieldEndpointProviderEndpointTestCase::OperationParamsFromTest>>
