...
namespace Model
{
  ...
  public:
    ...
    /**
     * The address of a recipient that bounced from the original message. Will be used
     * as the Original-Recipient (and also the Final-Recipient if a BounceType is given
     * or if FinalRecipient not specified in the RecipientDsnFields). If the
     * FinalRecipient is not specified, this must be a recipient of the original
     * inbound message. Should be just an email address (i.e. do not prepend with "rfc
     * 822;").
     */
    ...
    /**
     * The ARN of the identity for the address of the recipient that bounced from the
     * original message (i.e. the Recipient member of this structure). This is only
     * necessary if the caller doesn't own the identity being bounced and is using
     * sending authorization policies to bounce messages sent to another account.
     */
    ...
    /**
     * The reason for the bounce, which is used to auto populate the recipient-level
     * DSN fields. This or RecipientDsnFields is required.
     */
    ...
    /**
     * Recipient-level DSN fields, most of which would normally be filled in
     * automatically given a BounceType. This or BounceType is required.
     */
