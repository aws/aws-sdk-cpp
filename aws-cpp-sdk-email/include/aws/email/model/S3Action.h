...
namespace Model
{
   ...
   * SES Developer Guide</a>.</p> <note>When you save your emails to an Amazon S3
   * bucket, the maximum email size (including headers) is 30 MB. Emails larger than
   * that will bounce.</note> <p>For information about specifying Amazon S3 actions
  ...
  public:
     ...
     * or a custom master key you created in AWS KMS as follows:</p> <ul> <li>To use
     * the default master key, provide an ARN in the form of
     ...
     * Amazon SES permission to use the key.</li> <li>To use a custom master key you
     * created in AWS KMS, provide the ARN of the master key and ensure that you add a
     * statement to your key's policy to give Amazon SES permission to use it. For more
     * information about giving permissions, see the <a
     * href="http://docs.aws.amazon.com/ses/latest/DeveloperGuide/receiving-email-permissions.html">Amazon
     * SES Developer Guide</a>.</li> </ul> <p>For more information about key policies,
     * see the <a
     ...
     * encrypt your emails.</p> <important>Your mail is encrypted by Amazon SES using
     * the Amazon S3 encryption client before the mail is submitted to Amazon S3 for
     * storage. It is not encrypted using Amazon S3 server-side encryption. This means
     * that you must use the Amazon S3 encryption client to decrypt the email after
     * retrieving it from Amazon S3, as the service has no access to use your AWS KMS
     * keys for decryption. This encryption client is currently available with the <a
     * href="https://aws.amazon.com/sdk-for-java/">AWS Java SDK</a> and <a
     * href="https://aws.amazon.com/sdk-for-ruby/">AWS Ruby SDK</a> only. For more
