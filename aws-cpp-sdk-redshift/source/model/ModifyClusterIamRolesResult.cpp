...
ModifyClusterIamRolesResult& ModifyClusterIamRolesResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "ModifyClusterIamRolesResult"))
  {
    resultNode = rootNode.FirstChild("ModifyClusterIamRolesResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::Redshift::Model::ModifyClusterIamRolesResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
