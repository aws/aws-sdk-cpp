...
DataShareAssociation& DataShareAssociation::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    {
      m_statusChangeDate = DateTime(StringUtils::Trim(Aws::Utils::Xml::DecodeEscapedXmlText(statusChangeDateNode.GetText()).c_str()).c_str(), DateFormat::ISO_8601);
...
void DataShareAssociation::OutputToStream(Aws::OStream& oStream, const char* location, unsigned index, const char* locationValue) const
{
  ...
  {
      oStream << location << index << locationValue << ".StatusChangeDate=" << StringUtils::URLEncode(m_statusChangeDate.ToGmtString(DateFormat::ISO_8601).c_str()) << "&";
  }
...
void DataShareAssociation::OutputToStream(Aws::OStream& oStream, const char* location) const
{
  ...
  if(m_statusChangeDateHasBeenSet)
  {
      oStream << location << ".StatusChangeDate=" << StringUtils::URLEncode(m_statusChangeDate.ToGmtString(DateFormat::ISO_8601).c_str()) << "&";
  }
}
