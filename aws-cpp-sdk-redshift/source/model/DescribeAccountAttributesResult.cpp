...
DescribeAccountAttributesResult& DescribeAccountAttributesResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    {
      XmlNode accountAttributesMember = accountAttributesNode.FirstChild("AccountAttribute");
      while(!accountAttributesMember.IsNull())
      {
        m_accountAttributes.push_back(accountAttributesMember);
        accountAttributesMember = accountAttributesMember.NextNode("AccountAttribute");
      }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::Redshift::Model::DescribeAccountAttributesResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
