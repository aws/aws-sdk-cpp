...
namespace Model
{
   ...
   * specify the mode that Elastic Transcoder will use to decrypt your file,
   * otherwise you must specify the mode you want Elastic Transcoder to use to
   * encrypt your output files.</p>
   */
  ...
  public:
     ...
     * Transcoder supports the following options:</p> <ul> <li><p><b>S3:</b> Amazon S3
     * creates and manages the keys used for encrypting your files.</p></li>
     * <li><p><b>S3-AWS-KMS:</b> Amazon S3 calls the Amazon Key Management Service,
     * which creates and manages the keys that are used for encrypting your files. If
     * you specify <code>S3-AWS-KMS</code> and you don't want to use the default key,
     * you must add the AWS-KMS key that you want to use to your pipeline.</p></li>
     * <li><p><b>AES-CBC-PKCS7:</b> A padded cipher-block mode of operation originally
     * used for HLS files.</p></li> <li><p><b>AES-CTR:</b> AES Counter Mode.</p></li>
     * <li><p><b>AES-GCM:</b> AES Galois Counter Mode, a mode of operation that is an
     * authenticated encryption format, meaning that a file, key, or initialization
     * vector that has been tampered with will fail the decryption process.</p></li>
     * </ul> <p>For all three AES options, you must provide the following settings,
     * which must be base64-encoded:</p> <ul> <li><p><b>Key</b></p></li> <li><p><b>Key
     * MD5</b></p></li> <li><p><b>Initialization Vector</b></p></li> </ul>
     * <important><p>For the AES modes, your private encryption keys and your
     * unencrypted data are never stored by AWS; therefore, it is important that you
     * safely manage your encryption keys. If you lose them, you won't be able to
     * unencrypt your data.</p></important>
     */
