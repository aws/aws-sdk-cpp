...
GetServerCertificateResult& GetServerCertificateResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "GetServerCertificateResult"))
  {
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::IAM::Model::GetServerCertificateResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
