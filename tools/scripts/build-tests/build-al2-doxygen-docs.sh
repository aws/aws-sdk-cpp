...
export VERSION_NUM=$(grep AWS_SDK_VERSION_STRING "${PREFIX_DIR}/aws-sdk-cpp/src/aws-cpp-sdk-core/include/aws/core/VersionConfig.h" | cut -d '\"' -f2)
echo "Setting config to build docs for version: ${VERSION_NUM}"
sed -i "s/PROJECT_NUMBER .*/PROJECT_NUMBER         = $VERSION_NUM/" "${PREFIX_DIR}/aws-sdk-cpp/tools/doxygen/modules/static/root.config"
sed -i "s/PROJECT_NUMBER .*/PROJECT_NUMBER         = $VERSION_NUM/" "${PREFIX_DIR}/aws-sdk-cpp/tools/doxygen/modules/template/module-template.config"
echo "Running doc generation"
cd "${PREFIX_DIR}/aws-sdk-cpp"
python3 "${PREFIX_DIR}/aws-sdk-cpp/tools/doxygen/modules/scripts/run-doc-generation.py"
echo "Staging static files"
cp "${PREFIX_DIR}/aws-sdk-cpp/LICENSE" "${PREFIX_DIR}/aws-sdk-cpp/tools/doxygen/output/root/html"
mkdir -p "${OUTPUT_DIR}/${VERSION_NUM}"
mkdir -p "${OUTPUT_DIR}/LATEST"
rsync -av "${PREFIX_DIR}"/aws-sdk-cpp/tools/doxygen/output/* "${OUTPUT_DIR}/${VERSION_NUM}"
rsync -av "${PREFIX_DIR}"/aws-sdk-cpp/tools/doxygen/output/* "${OUTPUT_DIR}/LATEST"
...
cp "${PREFIX_DIR}/aws-sdk-cpp/tools/doxygen/modules/static/index.html" -p "${OUTPUT_DIR}/"
cp "${PREFIX_DIR}/aws-sdk-cpp/tools/doxygen/modules/static/local-index.html" "${OUTPUT_DIR}/LATEST/index.html"
cp "${PREFIX_DIR}/aws-sdk-cpp/tools/doxygen/modules/static/local-index.html" "${OUTPUT_DIR}/${VERSION_NUM}/index.html"
