 ...
 * {@code GeneratedPolicyResults}. C2J preserves the JSON wire key ({@code generatedPolicyResult})
 * via {@code locationName}; this transform mirrors that with a {@code @jsonName} trait so the model
 * stays serde-correct even though serde is currently stubbed.
 *
...
public final class AccessAnalyzerTransforms {
    ...
    private static Model apply(Model model, ServiceShape service) {
        ...
        Optional<StructureShape> updated = TransformSupport.renameMember(
            resp.get(), "generatedPolicyResult", "generatedPolicyResults",
            new JsonNameTrait("generatedPolicyResult"));
