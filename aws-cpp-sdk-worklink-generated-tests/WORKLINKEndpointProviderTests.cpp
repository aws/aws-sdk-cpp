...
static const char* ALLOCATION_TAG = "AWSWorkLinkEndpointProviderTests";
using WorkLinkEndpointProvider = Aws::WorkLink::Endpoint::WorkLinkEndpointProvider;
using EndpointParameters = Aws::Vector<Aws::Endpoint::EndpointParameter>;
using WorkLinkResolveEndpointOutcome = Aws::WorkLink::Endpoint::WorkLinkEndpointProvider::WorkLinkResolveEndpointOutcome;
...
class WorkLinkEndpointProviderTests :public ::testing::TestWithParam<size_t>
{
public:
...
struct WorkLinkEndpointProviderEndpointTestCase
{
...
static const Aws::Vector<WorkLinkEndpointProviderEndpointTestCase> TEST_CASES = {
  /*TEST CASE 0*/
...
void ValidateOutcome(const WorkLinkResolveEndpointOutcome& outcome, const WorkLinkEndpointProviderEndpointTestCase::Expect& expect)
{
...
TEST_P(WorkLinkEndpointProviderTests, EndpointProviderTest)
{
    ...
    const WorkLinkEndpointProviderEndpointTestCase& TEST_CASE = TEST_CASES.at(TEST_CASE_IDX);
    SCOPED_TRACE(Aws::String("\nTEST CASE # ") + Aws::Utils::StringUtils::to_string(TEST_CASE_IDX) + ": " + TEST_CASE.documentation);

    std::shared_ptr<WorkLinkEndpointProvider> endpointProvider = Aws::MakeShared<WorkLinkEndpointProvider>(ALLOCATION_TAG);
    ASSERT_TRUE(endpointProvider) << "Failed to allocate/initialize WorkLinkEndpointProvider";
    ...
    for(const auto& operation : TEST_CASE.operationInput)
    {
         ...
         */
        const Aws::Vector<std::reference_wrapper<const WorkLinkEndpointProviderEndpointTestCase::OperationParamsFromTest>>
