...
Progress& Progress::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    {
      m_bytesScanned = StringUtils::ConvertToInt64(StringUtils::Trim(bytesScannedNode.GetText().c_str()).c_str());
      m_bytesScannedHasBeenSet = true;
    }
    XmlNode bytesProcessedNode = resultNode.FirstChild("BytesProcessed");
    if(!bytesProcessedNode.IsNull())
    {
      m_bytesProcessed = StringUtils::ConvertToInt64(StringUtils::Trim(bytesProcessedNode.GetText().c_str()).c_str());
      m_bytesProcessedHasBeenSet = true;
    }
    XmlNode bytesReturnedNode = resultNode.FirstChild("BytesReturned");
    if(!bytesReturnedNode.IsNull())
    {
      m_bytesReturned = StringUtils::ConvertToInt64(StringUtils::Trim(bytesReturnedNode.GetText().c_str()).c_str());
