...
CopyFpgaImageResponse::CopyFpgaImageResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  *this = result;
}

CopyFpgaImageResponse& CopyFpgaImageResponse::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "CopyFpgaImageResponse"))
  {
    resultNode = rootNode.FirstChild("CopyFpgaImageResponse");
  }
