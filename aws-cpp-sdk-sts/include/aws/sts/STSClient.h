    ...
    public:
      typedef Aws::Client::AWSXMLClient BASECLASS;
        ...
        */
         ...
         * token. Typically, you use <code>AssumeRole</code> for cross-account access or
         * federation. For a comparison of <code>AssumeRole</code> with other API
         ...
         * your network, you don't have to recreate identities in AWS in order to grant
         * them access to AWS. Instead, after a user has been authenticated, you call
         * <code>AssumeRole</code> (and specify the role with the appropriate permissions)
         ...
         * service's <code>GetFederationToken</code> or <code>GetSessionToken</code> API
         * operations.</p> <p>(Optional) You can pass an IAM permissions policy to this
         * operation. If you pass a policy to this operation, the resulting temporary
         * credentials have the permissions of the assumed role <i>and</i> the policy that
         ...
         * grant permissions that are in excess of those allowed by the permissions policy
         ...
         * </a> in the <i>IAM User Guide</i>.</p> <p>To assume a role, your AWS account
         ...
         * are allowed to delegate access to this account's role. </p> <p>The user who
         * wants to access the role must also have permissions delegated from the role's
         * administrator. If the user and the role are in a different account, then the
         * user's administrator must attach a policy. That attached policy must allow the
         * user to call <code>AssumeRole</code> for the ARN of the role in the other
         * account. If the user is in the same account as the role, then you can do either
         * of the following:</p> <ul> <li> <p>Attach a policy to the user (identical to the
         ...
         * the trust policy acts as the only resource-based policy in IAM. Users in the
         * same account as the role do not need explicit permission to assume the role. For
         ...
         * AssumeRole</b> </p> <p>(Optional) You can include multi-factor authentication
         ...
         * temporary security credentials to sign calls to AWS services.</p> <p>By default,
         * the temporary security credentials created by <code>AssumeRoleWithSAML</code>
         * last for one hour. However, you can use the optional
         * <code>DurationSeconds</code> parameter to specify the duration of your session.
         * Your role session lasts for the duration that you specify, or until the time
         * specified in the SAML authentication response's <code>SessionNotOnOrAfter</code>
         * value, whichever is shorter. You can provide a <code>DurationSeconds</code>
         * value from 900 seconds (15 minutes) up to the maximum session duration setting
         * for the role. This setting can have a value from 1 hour to 12 hours. To learn
         * how to view the maximum value for your role, see <a
         * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use.html#id_roles_use_view-role-max-session">View
         * the Maximum Session Duration Setting for a Role</a> in the <i>IAM User
         * Guide</i>. The maximum session duration limit applies when you use the
         * <code>AssumeRole*</code> API operations or the <code>assume-role*</code> CLI
         ...
         * service's <code>GetFederationToken</code> or <code>GetSessionToken</code> API
         * operations.</p> <p>Optionally, you can pass an IAM permissions policy to this
         * operation. If you pass a policy to this operation, the resulting temporary
         * credentials have the permissions of the assumed role <i>and</i> the policy that
         * you pass. This gives you a way to further restrict the permissions for the
         * resulting temporary security credentials. You cannot use the passed policy to
         ...
         * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_control-access_assumerole.html">
         * Permissions for AssumeRole, AssumeRoleWithSAML, and AssumeRoleWithWebIdentity
         * </a> in the <i>IAM User Guide</i>.</p> <p>Before your application can call
         ...
         * role that specifies this SAML provider in its trust policy. </p> <p>Calling
         ...
         * security credentials to sign calls to AWS service API operations.</p> <p>By
         * default, the temporary security credentials created by
         * <code>AssumeRoleWithWebIdentity</code> last for one hour. However, you can use
         * the optional <code>DurationSeconds</code> parameter to specify the duration of
         * your session. You can provide a value from 900 seconds (15 minutes) up to the
         ...
         * <code>GetSessionToken</code> API operations.</p> <p>(Optional) You can pass an
         * IAM permissions policy to this operation. If you pass a policy to this
         * operation, the resulting temporary credentials have the permissions of the
         * assumed role <i>and</i> the policy that you pass. This gives you a way to
         ...
         * excess of those allowed by the permissions policy of the role that is being
         ...
         * how to invoke the identity providers, and then how to use the information from
         * these providers to get and use temporary security credentials. </p> </li> <li>
         * <p> <a
         ...
         * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_request.html#api_assumerolewithwebidentity">Federation
         * Through a Web-based Identity Provider</a>.</p> </note> <p>You can also call
         * <code>GetFederationToken</code> using the security credentials of an AWS account
         * root user, but we do not recommend it. Instead, we recommend that you create an
         * IAM user for the purpose of the proxy application. Then attach a policy to the
         * IAM user that limits federated users to only the actions and resources that they
         ...
         * credentials are valid for the specified duration, from 900 seconds (15 minutes)
         * up to a maximum of 129,600 seconds (36 hours). The default is 43,200 seconds (12
         * hours). Temporary credentials that are obtained by using AWS account root user
         * credentials have a maximum duration of 3,600 seconds (1 hour).</p> <p>The
         * temporary security credentials created by <code>GetFederationToken</code> can be
         * used to make API calls to any AWS service with the following exceptions:</p>
         * <ul> <li> <p>You cannot use these credentials to call any IAM API
         * operations.</p> </li> <li> <p>You cannot call any STS API operations except
         * <code>GetCallerIdentity</code>.</p> </li> </ul> <p> <b>Permissions</b> </p>
         * <p>You must pass an IAM permissions policy to <code>GetFederationToken</code>.
         * When you pass a policy to this operation, the resulting temporary credentials
         * are defined by the intersection of your IAM user policies and the passed policy
         * . The passed policy defines the permissions of the <i>federated user</i>. AWS
         * allows the federated user's request only when both the attached policy and the
         ...
         * defined in the IAM user policy. For more information about how permissions work,
         * see <a
         ...
         * href="https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_request.html#stsapi_comparison">Comparing
         * the AWS STS API operations</a> in the <i>IAM User Guide</i>.</p> <p>The
         * <code>GetSessionToken</code> operation must be called by using the long-term AWS
         ...
         * created by using account credentials can range from 900 seconds (15 minutes) up
         * to a maximum of 3,600 seconds (1 hour), with a default of 1 hour. </p> <p>The
         ...
         * information is included in the request.</p> </li> <li> <p>You cannot call any
         * STS API <i>except</i> <code>AssumeRole</code> or
         * <code>GetCallerIdentity</code>.</p> </li> </ul> <note> <p>We recommend that you
         * do not call <code>GetSessionToken</code> with AWS account root user credentials.
         ...
         * account or IAM user whose credentials are used to call the operation. If
         * <code>GetSessionToken</code> is called using AWS account root user credentials,
         * the temporary credentials have root user permissions. Similarly, if
