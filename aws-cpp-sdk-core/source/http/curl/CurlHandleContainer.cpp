...
CurlHandleContainer::CurlHandleContainer(unsigned maxSize, long lowSpeedTimeout, long requestTimeout, long connectTimeout, bool enableTcpKeepAlive, unsigned long tcpKeepAliveIntervalMs, unsigned long lowSpeedLimit) :
                m_maxPoolSize(maxSize), m_lowSpeedTimeout(lowSpeedTimeout), m_requestTimeout(requestTimeout), m_connectTimeout(connectTimeout),
...
void CurlHandleContainer::SetDefaultOptionsOnHandle(CURL* handle)
{
    ...
    curl_easy_setopt(handle, CURLOPT_LOW_SPEED_TIME, m_lowSpeedTimeout < 1000 ? (m_lowSpeedTimeout == 0 ? 0 : 1) : m_lowSpeedTimeout / 1000);
