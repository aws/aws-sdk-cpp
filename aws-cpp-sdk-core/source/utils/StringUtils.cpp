...
double StringUtils::ConvertHeaderToDoubleDate(const char* source)
{
    if(!source)
    {
        return 0.0;
    ...
    //Fri, 29 Jan 2016 22:32:31 GMT
#ifdef _WIN32
    if(sscanf_s(source, "%*3s, %u %3s %u %u:%u:%u GMT", &day, mabbr, (rsize_t) sizeof(mabbr), &year, &hour, &minute, &second) < 6)
#else
    if(sscanf(source, "%*3s, %u %3s %u %u:%u:%u GMT", &year, mabbr, (rsize_t) sizeof(mabbr), &day, &hour, &minute, &second) < 6)
#endif
    {
        return 0.0;
    }
...
double StringUtils::ConvertXmlToDoubleDate(const char* source)
{
    if(!source)
    {
        return 0.0;
