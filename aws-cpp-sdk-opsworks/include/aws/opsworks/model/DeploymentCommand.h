...
namespace Model
{
  ...
  public:
     ...
     * the following commands are available:</p> <ul> <li>
     ...
     * <code>{"recipes":["phpapp::appsetup"]}</code>.</li> <li>
     * <code>install_dependencies</code>: Install the stack's dependencies.</li> <li>
     ...
     * </ul> <note>The update_dependencies and install_dependencies commands are
     * supported only for Linux instances. You can run the commands successfully on
     * Windows instances, but they do nothing.</note> <p>For apps, the following
     * commands are available:</p> <ul> <li> <code>deploy</code>: Deploy an app. Ruby
     * on Rails apps have an optional <code>Args</code> parameter named
     * <code>migrate</code>. Set <code>Args</code> to {"migrate":["true"]} to migrate
     * the database. The default setting is {"migrate":["false"]}.</li> <li>
     ...
     * <code>update_dependencies</code> command takes two arguments:</p> <ul> <li>
