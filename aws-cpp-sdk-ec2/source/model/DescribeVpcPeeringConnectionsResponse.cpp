...
DescribeVpcPeeringConnectionsResponse& DescribeVpcPeeringConnectionsResponse::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "DescribeVpcPeeringConnectionsResponse"))
  {
    resultNode = rootNode.FirstChild("DescribeVpcPeeringConnectionsResponse");
  }
  ...
  if(!resultNode.IsNull())
  {
    ...
    if(!vpcPeeringConnectionsNode.IsNull())
    {
      ...
      while(!vpcPeeringConnectionsMember.IsNull())
      {
        m_vpcPeeringConnections.push_back(vpcPeeringConnectionsMember);
        vpcPeeringConnectionsMember = vpcPeeringConnectionsMember.NextNode("item");
      }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::DescribeVpcPeeringConnectionsResponse", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
