...
DescribeTransitGatewayMulticastDomainsResponse& DescribeTransitGatewayMulticastDomainsResponse::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "DescribeTransitGatewayMulticastDomainsResponse"))
  {
    resultNode = rootNode.FirstChild("DescribeTransitGatewayMulticastDomainsResponse");
  }

  if(!resultNode.IsNull())
  {
    XmlNode transitGatewayMulticastDomainsNode = resultNode.FirstChild("transitGatewayMulticastDomains");
