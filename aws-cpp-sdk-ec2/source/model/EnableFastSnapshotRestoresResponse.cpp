...
EnableFastSnapshotRestoresResponse& EnableFastSnapshotRestoresResponse::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "EnableFastSnapshotRestoresResponse"))
  {
    resultNode = rootNode.FirstChild("EnableFastSnapshotRestoresResponse");
  }
