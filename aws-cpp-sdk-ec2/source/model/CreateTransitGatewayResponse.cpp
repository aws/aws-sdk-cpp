...
CreateTransitGatewayResponse::CreateTransitGatewayResponse(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  *this = result;
}
...
CreateTransitGatewayResponse& CreateTransitGatewayResponse::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "CreateTransitGatewayResponse"))
  {
    resultNode = rootNode.FirstChild("CreateTransitGatewayResponse");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::EC2::Model::CreateTransitGatewayResponse", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
