...
namespace Model
{
  ...
  public:
     ...
     * use. If you purchased your certificate from a third-party certificate authority
     * and uploaded it to the IAM certificate store, specify the certificate ID that
     * you want to use for this distribution.</p> <p>If you specify a certificate ID,
     * you must also specify an SSL support method (<code>sni-only</code> or
     * <code>vip</code>).</p>
     */
     ...
     * <p>If you want viewers to use HTTPS to request your objects and you're using an
     * alternate domain name, you must choose the type of certificate that you want to
     * use. If ACM provided your certificate, specify the Amazon Resource Name (ARN)
     * for the ACM certificate that you want to use for this distribution. CloudFront
     * only supports ACM certificates in the US East (N. Virginia) Region
     * (us-east-1).</p> <p>If you specify an ACM certificate ARN, you must also specify
     * an SSL support method (<code>sni-only</code> or <code>vip</code>).</p>
     */
     ...
     * method that works for browsers and clients released after 2010, or one that
     * works for all clients.</p> <ul> <li> <p> <code>sni-only</code>: CloudFront can
     * respond to HTTPS requests from viewers that support Server Name Indication
     * (SNI). All modern browsers support SNI, but there are a few that don't. For a
     * current list of the browsers that support SNI, see the <a
     ...
     * viewers with browsers that don't include SNI support, see <a
     ...
     * specify a value here if you specified <code>CloudFrontDefaultCertificate</code>
     * as <code>true</code>.</p> <p>For more information, see <a
     * href="https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/cnames-https-dedicated-ip-or-sni.html">Choosing
     * How CloudFront Serves HTTPS Requests</a> in the <i>Amazon CloudFront Developer
     * Guide</i>.</p>
     */
     ...
     * specify <code>TLSv1.1_2016</code> unless your viewers are using browsers or
     ...
     * </p> <ul> <li> <p>You're using a custom certificate; that is, you specified a
     * value for <code>ACMCertificateArn</code> or for
     * <code>IAMCertificateId</code>.</p> </li> <li> <p>You're using SNI; that is, you
     * specified <code>sni-only</code> for <code>SSLSupportMethod</code>.</p> </li>
     * </ul> <p>If you specify <code>true</code> for
     * <code>CloudFrontDefaultCertificate</code>, CloudFront automatically sets the
     * security policy to <code>TLSv1</code> regardless of the value that you specify
     * here.</p> <p>For information about the relationship between the security policy
     * that you choose and the protocols and ciphers that CloudFront uses to
     * communicate with viewers, see <a
