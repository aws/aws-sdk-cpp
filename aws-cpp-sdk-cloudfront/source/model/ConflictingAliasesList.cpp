...
ConflictingAliasesList& ConflictingAliasesList::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    if(!itemsNode.IsNull())
    {
      ...
      while(!itemsMember.IsNull())
      {
        ...
        itemsMember = itemsMember.NextNode("ConflictingAlias");
      }
...
void ConflictingAliasesList::AddToNode(XmlNode& parentNode) const
{
  Aws::StringStream ss;
  ...
  if(m_itemsHasBeenSet)
  {
   ...
   {
     XmlNode itemsNode = itemsParentNode.CreateChildElement("ConflictingAlias");
