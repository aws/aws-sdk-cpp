...
DistributionList& DistributionList::operator =(const XmlNode& xmlNode)
{
  ...
  if(!resultNode.IsNull())
  {
    ...
    XmlNode distributionSummaryNode = resultNode.FirstChild("DistributionSummary");
    if(!distributionSummaryNode.IsNull())
    {
      while(!distributionSummaryNode.IsNull())
      {
        m_items.push_back(distributionSummaryNode);
        distributionSummaryNode = distributionSummaryNode.NextNode("DistributionSummary");
      }
...
void DistributionList::AddToNode(XmlNode& parentNode) const
{
  ...
  if(m_itemsHasBeenSet)
  {
   ...
   {
     XmlNode itemsNode = parentNode.CreateChildElement("DistributionSummary");
