...
namespace Model
{
  ...
  public:
     ...
     * data in the bucket. Possible values are:</p> <ul><li><p>TRUE - One or more jobs
     * is configured to analyze data in the bucket, and at least one of those jobs has
     * a status other than CANCELLED.</p></li> <li><p>FALSE - No jobs are configured to
     * analyze data in the bucket, or all the jobs that are configured to analyze data
     * in the bucket have a status of CANCELLED.</p></li> <li><p>UNKNOWN - An exception
     * occurred when Amazon Macie attempted to retrieve job data for the
     * bucket.</p></li></ul> <p></p>
     */
     ...
     * bucket. Possible values are:</p> <ul><li><p>TRUE - One or more recurring jobs is
     * configured to analyze data in the bucket, and at least one of those jobs has a
     * status other than CANCELLED.</p></li> <li><p>FALSE - No recurring jobs are
     * configured to analyze data in the bucket, or all the recurring jobs that are
     * configured to analyze data in the bucket have a status of CANCELLED.</p></li>
    ...
    /**
     * <p>The unique identifier for the job that ran most recently (either the latest
     * run of a recurring job or the only run of a one-time job) and is configured to
     * analyze data in the bucket.</p> <p>This value is null if the value for the
     ...
     * the most recent run started.</p> <p>This value is null if the value for the
     * isDefinedInJob property is FALSE or UNKNOWN.</p>
     */
