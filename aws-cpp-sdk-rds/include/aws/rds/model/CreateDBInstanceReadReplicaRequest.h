...
namespace Model
{
  ...
  public:
     ...
     * <p>Constraints:</p> <ul> <li>Must be the identifier of an existing MySQL,
     * MariaDB, or PostgreSQL DB instance.</li> <li>Can specify a DB instance that is a
     * MySQL Read Replica only if the source is running MySQL 5.6.</li> <li>Can specify
     * a DB instance that is a PostgreSQL Read Replica only if the source is running
     * PostgreSQL 9.3.5.</li> <li>The specified DB instance must have automatic backups
     * enabled, its backup retention period must be greater than 0.</li> <li>If the
     * source DB instance is in the same region as the Read Replica, specify a valid DB
     * instance identifier.</li> <li>If the source DB instance is in a different region
     * than the Read Replica, specify a valid DB instance ARN. For more information, go
     * to <a
     ...
     * <p>Constraints:</p> <ul> <li>Can only be specified if the source DB instance
     * identifier specifies a DB instance in another region.</li> <li>The specified DB
     * subnet group must be in the same region in which the operation is running.</li>
     * <li> All Read Replicas in one region that are created from the same source DB
