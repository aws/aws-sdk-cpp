...
DeleteCustomDBEngineVersionResult::DeleteCustomDBEngineVersionResult() : 
...
DeleteCustomDBEngineVersionResult& DeleteCustomDBEngineVersionResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  const XmlDocument& xmlDocument = result.GetPayload();
  XmlNode rootNode = xmlDocument.GetRootElement();
  XmlNode resultNode = rootNode;
  if (!rootNode.IsNull() && (rootNode.GetName() != "DeleteCustomDBEngineVersionResult"))
  {
    resultNode = rootNode.FirstChild("DeleteCustomDBEngineVersionResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::RDS::Model::DeleteCustomDBEngineVersionResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
