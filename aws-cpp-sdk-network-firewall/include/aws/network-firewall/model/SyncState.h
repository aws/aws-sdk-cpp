...
namespace Model
{
   ...
   * firewall, AWS Network Firewall does the following: </p> <ul> <li>
   * <p>Instantiates a firewall endpoint in the subnet, ready to take traffic.</p>
   * </li> <li> <p>Configures the endpoint with the current firewall policy settings,
   * to provide the filtering behavior for the endpoint.</p> </li> </ul> <p>When you
   * update a firewall, for example to add a subnet association or change a rule
   * group in the firewall policy, the affected sync states reflect out-of-sync or
   * not ready status until the changes are complete. </p><p><h3>See Also:</h3>   <a
