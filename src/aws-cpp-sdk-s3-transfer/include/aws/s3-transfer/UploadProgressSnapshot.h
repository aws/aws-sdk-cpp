...
class AWS_S3_TRANSFER_API UploadProgressSnapshot final {
 public:
  UploadProgressSnapshot(uint64_t transferredBytes,
                         uint64_t totalBytes,
                         std::shared_ptr<UploadResponse> response,
  ...
  inline const std::shared_ptr<UploadResponse>& GetResponse() const { return m_response; }
  inline bool ResponseHasBeenSet() const { return m_response != nullptr; }

 private:
  uint64_t m_transferredBytes = 0;
  uint64_t m_totalBytes = 0;
  std::shared_ptr<UploadResponse> m_response;
