                                           ...
                                           std::shared_ptr<Aws::Utils::Threading::Executor> pExecutor) const
{
    ...
    pRequestCtx->m_authOption = this->SelectAuthOption(*pRequestCtx);
    Aws::Endpoint::EndpointParameters epParams = request ? request->GetEndpointContextParams() : Aws::Endpoint::EndpointParameters();
    epParams.insert(epParams.end(), pRequestCtx->m_authOption.endpointParameters.begin(), pRequestCtx->m_authOption.endpointParameters.end());
    pRequestCtx->m_endpoint = this->ResolveEndpoint(std::move(epParams), std::move(endpointCallback));
...
void AwsSmithyClientBase::AttemptOneRequestAsync(std::shared_ptr<AwsSmithyClientAsyncRequestContext> pRequestCtx) const
{
    ...
    SigningOutcome signingOutcome = TracingUtils::MakeCallWithTiming<SigningOutcome>([&]() -> SigningOutcome {
            return this->SignRequest(*pRequestCtx->m_httpRequest, pRequestCtx->m_authOption);
        },
    ...
    auto httpResponseHandler = [this, pRequestCtx](std::shared_ptr<Aws::Http::HttpResponse> pResponse)
    {
        HandleAsyncReply(pRequestCtx, pResponse);
    };
                                           ...
                                           std::shared_ptr<Aws::Http::HttpResponse> httpResponse) const
{
    ...
    do // goto in a form of "do { break; } while(0);" // TODO: refactor
    {
        ...
        if (m_clientConfig.enableClockSkewAdjustment)
        {
            ...
            shouldSleep |= !this->AdjustClockSkew(outcome, pRequestCtx->m_authOption);
        }
