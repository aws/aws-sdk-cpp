...
class CRTClientStream : public Aws::Http::ClientStream {
 public:
  ...
  int WriteData(std::shared_ptr<Aws::IOStream> stream, bool endStream) override {
    auto crtStream = std::make_shared<Aws::Crt::Io::StdIOStreamInputStream>(stream, Aws::Crt::ApiAllocator());
    AsyncWaiter waiter;
    auto writeDataResult = m_stream->WriteData(crtStream,
        [&waiter](std::shared_ptr<Aws::Crt::Http::HttpStream>& stream, int errorCode) -> void {
...
class CRTConnection : public Aws::Http::Connection {
 public:
  explicit CRTConnection(std::shared_ptr<Aws::Crt::Http::HttpClientConnection> m_connection) : m_connection(std::move(m_connection)) {}
  ...
  std::shared_ptr<Aws::Http::ClientStream> NewClientStream(const std::shared_ptr<Aws::Http::HttpRequest>& request,
                                                           std::function<void()> onStreamComplete) override {
...
namespace Aws
{
    namespace Http
    {
        ...
        Aws::Http::HttpClient::AcquireConnectionOutcome CRTHttpClient::AcquireConnection(const std::shared_ptr<HttpRequest>& request) {
          ...
          AcquireConnectionOutcome outcome{};
          AsyncWaiter waiter;

          connectionManager->AcquireConnection(
              [&outcome, &waiter](
                  std::shared_ptr<Crt::Http::HttpClientConnection> acquiredConnection, int errorCode) -> void {
                if (errorCode != AWS_OP_SUCCESS) {
                  ...
                  outcome = AcquireConnectionOutcome{Aws::MakeShared<CRTConnection>(CRT_HTTP_CLIENT_TAG, std::move(acquiredConnection))};
                }
                waiter.Wakeup();
              });
