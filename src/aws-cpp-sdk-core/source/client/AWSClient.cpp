...
#include <aws/core/client/AWSClientAsyncRequestContext.h>
...
bool AWSClient::DoesResponseGenerateError(const std::shared_ptr<Aws::Http::HttpResponse>& response)
{
...
using RequestInfo = AwsClientAsyncRequestCtx::RequestInfo;
...
void AWSClient::StartAsyncAttempt(const Aws::Endpoint::AWSEndpoint& endpoint,
                                  ...
                                  std::shared_ptr<Aws::Utils::Threading::Executor> pExecutor) const
{
    if(!responseHandler)
    {
        ...
        AWS_LOGSTREAM_FATAL(AWS_CLIENT_LOG_TAG, "Unable to continue AWSClient request: response handler is missing!");
        return;
    }

    if (!Aws::Utils::IsValidHost(endpoint.GetURI().GetAuthority()))
    {
        AWS_LOGSTREAM_ERROR(AWS_CLIENT_LOG_TAG, "Invalid DNS Label found in URI host");
        auto outcome = HttpResponseOutcome(AWSError<CoreErrors>(CoreErrors::VALIDATION, "", "Invalid DNS Label found in URI host", false/*retryable*/));
        pExecutor->Submit([outcome, responseHandler]()
          {
              responseHandler(outcome);
          } );
        return;
    }

    std::shared_ptr<Aws::Client::AwsClientAsyncRequestCtx> pRequestCtx =
        Aws::MakeShared<Aws::Client::AwsClientAsyncRequestCtx>(AWS_CLIENT_LOG_TAG);
    if (!pRequestCtx)
    {
        AWS_LOGSTREAM_ERROR(AWS_CLIENT_LOG_TAG, "Failed to allocate an AwsClientAsyncRequestCtx under a shared ptr");
        auto outcome = HttpResponseOutcome(AWSError<CoreErrors>(CoreErrors::MEMORY_ALLOCATION, "", "Failed to allocate async request context", false/*retryable*/));
        pExecutor->Submit([outcome, responseHandler]()
          {
              responseHandler(outcome);
          } );
    ...
    pRequestCtx->m_endpoint = std::move(endpoint);
    if(pRequestCtx->m_pRequest) {
      pRequestCtx->m_httpRequest = CreateHttpRequest(pRequestCtx->m_endpoint.GetURI(), method, request->GetResponseStreamFactory());
    } else {
      pRequestCtx->m_httpRequest = CreateHttpRequest(pRequestCtx->m_endpoint.GetURI(), method, Aws::Utils::Stream::DefaultResponseStreamFactoryMethod);
    }
    if (!pRequestCtx->m_httpRequest)
    {
        AWS_LOGSTREAM_ERROR(AWS_CLIENT_LOG_TAG, "Failed to CreateHttpRequest");
        auto outcome = HttpResponseOutcome(AWSError<CoreErrors>(CoreErrors::VALIDATION, "", "Unable to create HttpRequest object", false/*retryable*/));
        pExecutor->Submit([outcome, responseHandler]()
          {
              responseHandler(outcome);
          } );
    ...
    if(request)
      pRequestCtx->m_httpRequest->SetEventStreamRequest(request->IsEventStreamRequest());
    ...
    pRequestCtx->m_httpRequest->SetHeaderValue(Http::SDK_INVOCATION_ID_HEADER, pRequestCtx->m_invocationId);
    pRequestCtx->m_httpRequest->SetHeaderValue(Http::SDK_REQUEST_HEADER, pRequestCtx->m_requestInfo);
    AppendRecursionDetectionHeader(pRequestCtx->m_httpRequest);

    if(m_retryStrategy && !m_retryStrategy->HasSendToken())
    {
        auto errOutcome = HttpResponseOutcome(AWSError<CoreErrors>(CoreErrors::SLOW_DOWN,
                                                                "",
                                                                "Unable to acquire enough send tokens to execute request.",
                                                                false/*retryable*/));
        pExecutor->Submit([errOutcome, responseHandler]()
          {
              responseHandler(errOutcome);
          } );
        return;
    };
...
/*HttpResponseOutcome*/
void AWSClient::AttemptOneRequestAsync(std::shared_ptr<Aws::Client::AwsClientAsyncRequestCtx> pRequestCtx) const
{
    if(!pRequestCtx)
    {
        assert(!"Missing pRequestCtx");
        AWS_LOGSTREAM_FATAL(AWS_CLIENT_LOG_TAG, "Missing request context!");
    }
    if(!pRequestCtx->m_httpRequest)
    {
        assert(!"Missing m_httpRequest");
        AWS_LOGSTREAM_FATAL(AWS_CLIENT_LOG_TAG, "Missing http request context!");
    }
    ...
    if(pRequestCtx->m_pRequest)
    {
      TracingUtils::MakeCallWithTiming(
          [&]() -> void {
              BuildHttpRequest(*pRequestCtx->m_pRequest, pRequestCtx->m_httpRequest);
          },
    ...
    assert(pRequestCtx->m_endpoint.GetAttributes());
    auto signer = GetSignerByName(pRequestCtx->m_endpoint.GetAttributes()->authScheme.GetName().c_str());
    auto signedRequest = TracingUtils::MakeCallWithTiming<bool>([&]() -> bool {
            return signer->SignRequest(*pRequestCtx->m_httpRequest,
                                       pRequestCtx->m_endpoint.GetAttributes()->authScheme.GetSigningRegion()->c_str(),
                                       pRequestCtx->m_endpoint.GetAttributes()->authScheme.GetSigningName()->c_str(),
                                       true);
        },
    ...
    if (!signedRequest)
    {
        ...
        auto errOutcome = HttpResponseOutcome(AWSError<CoreErrors>(CoreErrors::CLIENT_SIGNING_FAILURE, "", "SDK failed to sign the request", false/*retryable*/));
        pRequestCtx->m_pExecutor->Submit([errOutcome, pRequestCtx]()
          {
              pRequestCtx->m_responseHandler(errOutcome);
          } );
        return;
    }
    ...
    {
        pRequestCtx->m_pRequest->GetRequestSignedHandler()(*pRequestCtx->m_httpRequest);
    }

    auto responseHandler = [this, pRequestCtx](std::shared_ptr<Aws::Http::HttpResponse> pResponse)
    {
        HandleExhaustiveAsyncReply(pRequestCtx, pResponse);
    };
...
void AWSClient::HandleExhaustiveAsyncReply(std::shared_ptr<Aws::Client::AwsClientAsyncRequestCtx> pRequestCtx,
                                           std::shared_ptr<Aws::Http::HttpResponse> httpResponse) const
{
        ...
        for (const auto& hashIterator : pRequestCtx->m_httpRequest->GetResponseValidationHashes())
        {
            ...
            if (httpResponse->HasHeader(checksumHeaderKey.c_str()))
            {
                ...
                if (HashingUtils::Base64Encode(hashIterator.second->GetHash().GetResult()) != checksumHeaderValue)
                {
                    ...
                    return pRequestCtx->m_responseHandler(HttpResponseOutcome(error));
                }
    ...
    if (DoesResponseGenerateError(httpResponse) || hasEmbeddedError)
    {
    ...
    do // goto in a form of "do { break; } while(0);" // TODO: refactor, this is just least intrusive attempt to make it async
    {
        ...
        else
        {
            ...
            m_retryStrategy->RequestBookkeeping(outcome, pRequestCtx->m_lastError.value());
        }
        ...
        bool retryWithCorrectRegion = [&]() {
                ...
                httpResponseCode == HttpResponseCode::FORBIDDEN)            // 403
            {
              ...
              if (m_region == Aws::Region::AWS_GLOBAL && !regionFromResponse.empty() &&
                  regionFromResponse != signerRegion) {
                pRequestCtx->m_endpoint.AccessAttributes()->authScheme.SetSigningRegion(regionFromResponse);
                AWS_LOGSTREAM_DEBUG(AWS_CLIENT_LOG_TAG, "Need to retry with a correct region");
                return true;
              }
            }
            return false;
        ...
        long sleepMillis = TracingUtils::MakeCallWithTiming<long>(
            [&]() -> long {
                return m_retryStrategy->CalculateDelayBeforeNextRetry(outcome.GetError(), pRequestCtx->m_retryCount);
            },
            TracingUtils::SMITHY_CLIENT_SERVICE_BACKOFF_DELAY_METRIC,
            *m_telemetryProvider->getMeter(this->GetServiceClientName(), {}),
            {{TracingUtils::SMITHY_METHOD_DIMENSION, pRequestCtx->m_requestName},
        ...
        const Aws::String& signerName = pRequestCtx->m_endpoint.GetAttributes()->authScheme.GetName();
        bool shouldSleep = !AdjustClockSkew(outcome, signerName.c_str()) && !retryWithCorrectRegion;

        if (!retryWithCorrectRegion && !m_retryStrategy->ShouldRetry(outcome.GetError(), pRequestCtx->m_retryCount))
        {
            break;
        }
        ...
        if (retryWithCorrectRegion)
        {
            ...
            if (newEndpoint.empty()) {
              ...
              AWS_LOGSTREAM_DEBUG(AWS_CLIENT_LOG_TAG, "Endpoint has been updated to " << newUri.GetURIString());
            }
        }
        if(pRequestCtx->m_pRequest) {
          pRequestCtx->m_httpRequest = CreateHttpRequest(pRequestCtx->m_endpoint.GetURI(), pRequestCtx->m_method, pRequestCtx->m_pRequest->GetResponseStreamFactory());
        } else {
          pRequestCtx->m_httpRequest = CreateHttpRequest(pRequestCtx->m_endpoint.GetURI(), pRequestCtx->m_method, Aws::Utils::Stream::DefaultResponseStreamFactoryMethod);
        }
        ...
        if (serverTime.WasParseSuccessful() && serverTime != DateTime())
        {
            pRequestCtx->m_requestInfo.ttl = DateTime::Now() + clockSkew + std::chrono::milliseconds(m_requestTimeoutMs);
        }
        pRequestCtx->m_requestInfo.attempt ++;
        pRequestCtx->m_requestInfo.maxAttempts = m_retryStrategy->GetMaxAttempts();
        pRequestCtx->m_httpRequest->SetHeaderValue(Http::SDK_REQUEST_HEADER, pRequestCtx->m_requestInfo);
