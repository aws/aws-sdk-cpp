...
namespace Aws
{
    namespace Internal
    {
        ...
        constexpr std::chrono::milliseconds CREDENTIAL_REFRESH_BACKOFF_MIN = std::chrono::minutes(5);
        constexpr std::chrono::milliseconds CREDENTIAL_REFRESH_BACKOFF_MAX = std::chrono::minutes(10);
        ...
        enum class CredentialRefreshPhase
        {
        ...
        inline CredentialRefreshPhase ClassifyRefreshPhase(bool hasCredentials,
                                                       ...
                                                       std::chrono::milliseconds advisoryWindow)
        {
            if (!hasCredentials)
            {
                return CredentialRefreshPhase::NoCredentials;
            }
            ...
            {
                return CredentialRefreshPhase::Expired;
            }
            ...
            {
                return CredentialRefreshPhase::Mandatory;
            }
            if (nowMs >= advisoryAtMs)
            {
                return CredentialRefreshPhase::Advisory;
            }
            return CredentialRefreshPhase::Valid;
        }
        ...
        inline RefreshAction DecideRefreshAction(CredentialRefreshPhase state, bool refreshRateLimited)
        {
            // Initial fetch always attempts, regardless of backoff: there is nothing cached to serve yet.
            if (state == CredentialRefreshPhase::NoCredentials)
            {
                return RefreshAction::FetchBlocking;
            }
            if (state == CredentialRefreshPhase::Valid)
            {
            ...
            if (state == CredentialRefreshPhase::Advisory)
            {
        ...
        {
            return ScaleJitter(CREDENTIAL_REFRESH_BACKOFF_MIN, CREDENTIAL_REFRESH_BACKOFF_MAX, jitter01);
        }
        ...
        inline bool IsNewCredentialRefreshEnabled()
        {
        ...
        class AWS_CORE_LOCAL CredentialRefreshState
        {
        public:
            ...
            explicit CredentialRefreshState(FetchFunction fetch,
                                            ClockFunction clock = &Aws::Utils::DateTime::Now,
                                            JitterFunction jitter = &CredentialRefreshState::DefaultJitter)
            ...
            Aws::Utils::Outcome<CredentialsT, Aws::String> GetCredentials(ResolveObservation* observation = nullptr)
            {
                ...
                CredentialsT cachedCredentials{};
                {
                    ...
                    state = m_cached
                        ...
                        : CredentialRefreshPhase::NoCredentials;
                ...
                const bool needsRefresh = state == CredentialRefreshPhase::Advisory ||
                                          state == CredentialRefreshPhase::Mandatory ||
                                          state == CredentialRefreshPhase::Expired;
            ...
            {
                {
                    ...
                    if (m_cached)
                    {
                        const CredentialRefreshPhase state = ClassifyRefreshPhase(true, now, m_cached->expiration, m_cached->advisoryWindow);
                        if (state == CredentialRefreshPhase::Valid)
                        {
                            obs.advisoryWindow = m_cached->advisoryWindow;
                            return Aws::Utils::Outcome<CredentialsT, Aws::String>(m_cached->credentials);
