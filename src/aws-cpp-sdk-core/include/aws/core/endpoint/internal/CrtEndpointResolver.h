...
namespace Aws
{
    namespace Internal
    {
        namespace Endpoint
        {
            static const char CRT_ENDPOINT_RESOLVER_TAG[] = "Aws::Endpoint::CRTEndpointResolver";

            inline char CharToDec(const char c)
            {
                if(c >= '0' && c <= '9')
                    return c - '0';
                if(c >= 'A' && c <= 'F')
            ...
            inline Aws::String PercentDecode(Aws::String inputString)
            {
            ...
            /**
             * Resolve an endpoint using any CRT engine (RuleEngine or BddEngine).
             */
            template<typename EngineT>
            Aws::Endpoint::ResolveEndpointOutcome ResolveEndpointImpl(
                const EngineT& engine,
                const Aws::Endpoint::EndpointParameters& builtInParameters,
                const Aws::Endpoint::EndpointParameters& clientContextParameters,
                const Aws::Endpoint::EndpointParameters& endpointParameters)
            {
                using namespace Aws::Endpoint;

                if(!engine) {
                    AWS_LOGSTREAM_FATAL(CRT_ENDPOINT_RESOLVER_TAG, "Invalid CRT endpoint engine state");
                    return ResolveEndpointOutcome(
                            Aws::Client::AWSError<Aws::Client::CoreErrors>(
                                    Aws::Client::CoreErrors::INTERNAL_FAILURE,
                                    "",
                                    "CRT endpoint engine is not initialized",
                                    false/*retryable*/));
                }
                ...
                for (const auto& parameterClass : allParameters)
                {
                    for(const auto& parameter : parameterClass.get())
                    {
                        ...
                        {
                            AWS_LOGSTREAM_TRACE(CRT_ENDPOINT_RESOLVER_TAG, "Endpoint bool eval parameter: " << parameter.GetName() << " = " << parameter.GetBoolValueNoCheck());
                        ...
                        {
                            AWS_LOGSTREAM_TRACE(CRT_ENDPOINT_RESOLVER_TAG, "Endpoint str eval parameter: " << parameter.GetName() << " = " << parameter.GetStrValueNoCheck());
                        ...
                        else if(EndpointParameter::ParameterType::STRING_ARRAY == parameter.GetStoredType())
                        {
                            ...
                            AWS_LOGSTREAM_TRACE(CRT_ENDPOINT_RESOLVER_TAG,
                ...
                auto resolved = engine.Resolve(crtRequestCtx);
                ...
                if(resolved.has_value())
                {
                    if(resolved->IsError())
                    {
                        ...
                        Aws::String sdkCrtError = crtError ? Aws::String(crtError->begin(), crtError->end()) :
                                "CRT endpoint engine resolution resulted in an unknown error";
                        return ResolveEndpointOutcome(
                                Aws::Client::AWSError<Aws::Client::CoreErrors>(
                    ...
                    else if(resolved->IsEndpoint() && resolved->GetUrl())
                    {
                        ...
                        AWS_LOGSTREAM_DEBUG(CRT_ENDPOINT_RESOLVER_TAG, "Endpoint engine evaluated the endpoint: " << sdkCrtUrl);
                        ...
                        if (crtProps && crtProps->size() > 2) {
                            ...
                            AWS_LOGSTREAM_TRACE(CRT_ENDPOINT_RESOLVER_TAG, "Endpoint engine evaluated props: " << sdkCrtProps);
                    ...
                    else
                    {
                        return ResolveEndpointOutcome(
                                Aws::Client::AWSError<Aws::Client::CoreErrors>(
                                        Aws::Client::CoreErrors::INVALID_QUERY_PARAMETER,
                                        "",
                                        "Invalid AWS CRT endpoint engine state",
                                        false/*retryable*/));
                    }
                }

                auto errCode = Aws::Crt::LastError();
                AWS_LOGSTREAM_DEBUG(CRT_ENDPOINT_RESOLVER_TAG, "ERROR: Endpoint engine has failed to evaluate the endpoint: " << errCode << " " << Aws::Crt::ErrorDebugString(errCode));

                return ResolveEndpointOutcome(
                        Aws::Client::AWSError<Aws::Client::CoreErrors>(
                                Aws::Client::CoreErrors::INVALID_QUERY_PARAMETER,
                                "",
                                "Failed to evaluate the endpoint: null output from AWS CRT endpoint engine",
                                false/*retryable*/));
            }
        } // namespace Endpoint
    } // namespace Internal
