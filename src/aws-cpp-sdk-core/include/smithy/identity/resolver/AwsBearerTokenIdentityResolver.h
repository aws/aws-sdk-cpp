...
namespace smithy {
    ...
    class AwsBearerTokenIdentityResolver : public IdentityResolverBase<AwsBearerTokenIdentityBase> {
    public:
        ...
        ResolveIdentityFutureOutcome getIdentity(const IdentityProperties& identityProperties, const AdditionalParameters& additionalParameters) override
        {
            ...
            for (auto& bearerTokenProvider : m_providerChainLegacy)
            {
                if(!bearerTokenProvider) {
                    ...
                    return Aws::Client::AWSError<Aws::Client::CoreErrors>(Aws::Client::CoreErrors::INVALID_PARAMETER_VALUE, "", "Unexpected nullptr in BearerTokenProviderChain::m_providerChain", false);
                }
                auto bearerToken = bearerTokenProvider->GetAWSBearerToken();
                if(!bearerToken.IsExpiredOrEmpty())
                {
                    auto outcomePtr = Aws::MakeUnique<AwsBearerTokenIdentity>( BEARER_TOKEN_PROVIDER_CHAIN_LOG_TAG);
            ...
            return Aws::Client::AWSError<Aws::Client::CoreErrors>(Aws::Client::CoreErrors::NOT_INITIALIZED, "", "No bearer token provider in chain found", false);

        }

        void AddBearerTokenProvider(std::shared_ptr<Aws::Auth::AWSBearerTokenProviderBase> provider){
