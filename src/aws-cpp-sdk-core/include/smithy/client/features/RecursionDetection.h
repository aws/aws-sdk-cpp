    ...
    namespace client
    {
        ...
        public:
            static void AppendRecursionDetectionHeader(const std::shared_ptr<Aws::Http::HttpRequest>& ioRequest)
            {
                ...
                const Aws::String& awsLambdaFunctionName = Aws::Environment::GetEnv(AWS_LAMBDA_FUNCTION_NAME);
                if (awsLambdaFunctionName.empty())
                {
                    return;
                }
                const Aws::String& xAmznTraceIdVal = Aws::Environment::GetEnv(X_AMZN_TRACE_ID);
