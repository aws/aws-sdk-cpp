    ...
    public:
      typedef Aws::Client::AWSJsonClient BASECLASS;
        ...
        */
         ...
         * query.</p> <p>If the original query execution ran using an
         * <a>ResultConfiguration$ExpectedBucketOwner</a> setting, the setting also applies
         * to Amazon S3 read operations when <code>GetQueryResults</code> is called. If an
         * expected bucket owner has been specified and the query results are in an Amazon
         * S3 bucket whose owner account ID is different from the expected bucket owner,
         * the <code>GetQueryResults</code> call fails with an Amazon S3 permissions
         * error.</p> <p>To stream query results successfully, the IAM principal with
