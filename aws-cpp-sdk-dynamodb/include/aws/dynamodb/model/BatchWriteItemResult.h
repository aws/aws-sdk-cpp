...
namespace Model
{
  ...
  public:
     ...
     * (<i>DeleteRequest</i> or <i>PutRequest</i>).</p> <ul> <li>
     * <p><i>DeleteRequest</i> - Perform a <i>DeleteItem</i> operation on the specified
     ...
     * by an <i>Item</i> subelement:</p> <ul> <li> <p><i>Item</i> - A map of attributes
     * and their values. Each entry in this map consists of an attribute name and an
     * attribute value. Attribute values must not be null; string and binary type
     * attributes must have lengths greater than zero; and set type attributes must not
     * be empty. Requests that contain empty values will be rejected with a
     * <i>ValidationException</i> exception.</p> <p>If you specify any attributes that
     * are part of an index key, then the data types for those attributes must match
     * those of the schema in the table's attribute definition.</p> </li> </ul> </li>
     * </ul> <p>If there are no unprocessed items remaining, the response contains an
     * empty <i>UnprocessedItems</i> map.</p>
     */
