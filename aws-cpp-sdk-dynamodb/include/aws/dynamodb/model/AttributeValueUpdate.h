...
namespace Model
{
  ...
  public:
     ...
     * item with the specified <i>Key</i> is found in the table:</b> </p> <ul> <li>
     ...
     * <p><code>DELETE</code> - If no value is specified, the attribute and its value
     * are removed from the item. The data type of the specified value must match the
     ...
     * Specifying an empty set is an error.</p> </li> <li> <p><code>ADD</code> - If the
     * attribute does not already exist, then the attribute and its values are added to
     * the item. If the attribute does exist, then the behavior of <code>ADD</code>
     * depends on the data type of the attribute:</p> <ul> <li> <p>If the existing
     * attribute is a number, and if <i>Value</i> is also a number, then the
     ...
     * value.</p> <p>In addition, if you use <code>ADD</code> to update an existing
     * item, and intend to increment or decrement an attribute value which does not yet
     * exist, DynamoDB uses <code>0</code> as the initial value. For example, suppose
     ...
     * attribute in the item, with a value of <code>3</code>.</p> </note> </li> <li>
     * <p>If the existing data type is a set, and if the <i>Value</i> is also a set,
     * then the <i>Value</i> is added to the existing set. (This is a <i>set</i>
     ...
     * is a set of strings, the <i>Value</i> must also be a set of strings. The same
     * holds true for number sets and binary sets.</p> </li> </ul> <p>This action is
     * only valid for an existing attribute whose data type is number or is a set. Do
     * not use <code>ADD</code> for any other data types.</p> </li> </ul> <p> <b>If no
     * item with the specified <i>Key</i> is found:</b> </p> <ul> <li>
     * <p><code>PUT</code> - DynamoDB creates a new item with the specified primary
     * key, and then adds the attribute. </p> </li> <li> <p><code>DELETE</code> -
     * Nothing happens; there is no attribute to delete.</p> </li> <li>
     * <p><code>ADD</code> - DynamoDB creates an item with the supplied primary key and
     * number (or set of numbers) for the attribute value. The only data types allowed
     * are number and number set; no other data types can be specified.</p> </li> </ul>
     */
