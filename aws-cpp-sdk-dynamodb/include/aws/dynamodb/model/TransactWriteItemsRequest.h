...
namespace Model
{
  ...
  public:
    ...
    /**
     * <p>An ordered array of up to 10 <code>TransactWriteItem</code> objects, each of
     ...
     * the server (no side effects), the responses to the calls may not be the same. If
     * the <code>ReturnConsumedCapacity&gt;</code> parameter is set, then the initial
     * <code>TransactWriteItems</code> call returns the amount of write capacity units
     * consumed in making the changes, and subsequent <code>TransactWriteItems</code>
     * calls with the same client token return the amount of read capacity units
     * consumed in reading the item.</p> <p>A client request token is valid for 10
     * minutes after the first request that uses it completes. After 10 minutes, any
     * request with the same client token is treated as a new request. Do not resubmit
     * the same request with the same client token for more than 10 minutes or the
     * result may not be idempotent.</p> <p>If you submit a request with the same
     * client token but a change in other parameters within the 10 minute idempotency
