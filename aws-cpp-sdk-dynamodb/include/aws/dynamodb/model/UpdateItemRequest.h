...
namespace Model
{
  ...
  public:
     ...
     * the table, the following values perform the following actions:</p> <ul> <li>
     * <p><code>PUT</code> - Adds the specified attribute to the item. If the attribute
     * already exists, it is replaced by the new value. </p> </li> <li>
     * <p><code>DELETE</code> - Removes the attribute and its value, if no value is
     ...
     * <code>[b]</code>. Specifying an empty set is an error.</p> </li> <li>
     * <p><code>ADD</code> - Adds the specified value to the item, if the attribute
     ...
     * found in the table, the following values perform the following actions: </p>
     * <ul> <li> <p><code>PUT</code> - Causes DynamoDB to create a new item with the
     * specified primary key, and then adds the attribute. </p> </li> <li>
     * <p><code>DELETE</code> - Nothing happens, because attributes cannot be deleted
     * from a nonexistent item. The operation succeeds, but DynamoDB does not create a
     * new item.</p> </li> <li> <p><code>ADD</code> - Causes DynamoDB to create an item
     ...
     * valid values are:</p> <ul> <li> <p><code>NONE</code> - If <i>ReturnValues</i> is
     * not specified, or if its value is <code>NONE</code>, then nothing is returned.
     * (This setting is the default for <i>ReturnValues</i>.)</p> </li> <li>
     * <p><code>ALL_OLD</code> - If <i>UpdateItem</i> overwrote an attribute name-value
     * pair, then the content of the old item is returned.</p> </li> <li>
     ...
     * values are available for <i>UpdateExpression</i>.</p> <ul> <li>
     * <p><code>SET</code> - Adds one or more attributes and values to an item. If any
     * of these attribute already exist, they are replaced by the new values. You can
     * also use <code>SET</code> to add or subtract from an attribute that is of type
     * Number. For example: <code>SET myNum = myNum + :val</code></p>
     * <p><code>SET</code> supports the following functions:</p> <ul>
     * <li><p><code>if_not_exists (path, operand)</code> - if the item does not contain
     * an attribute at the specified path, then <code>if_not_exists</code> evaluates to
     * operand; otherwise, it evaluates to path. You can use this function to avoid
     * overwriting an attribute that may already be present in the item.</p></li>
     * <li><p><code>list_append (operand, operand)</code> - evaluates to a list with a
     * new element added to it. You can append the new element to the start or the end
     * of the list by reversing the order of the operands.</p></li> </ul> <p>These
