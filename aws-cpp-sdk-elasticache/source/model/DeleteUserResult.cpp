...
DeleteUserResult::DeleteUserResult(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  *this = result;
}
...
DeleteUserResult& DeleteUserResult::operator =(const Aws::AmazonWebServiceResult<XmlDocument>& result)
{
  ...
  if (!rootNode.IsNull() && (rootNode.GetName() != "DeleteUserResult"))
  {
    resultNode = rootNode.FirstChild("DeleteUserResult");
  }
  ...
  if (!rootNode.IsNull()) {
    ...
    AWS_LOGSTREAM_DEBUG("Aws::ElastiCache::Model::DeleteUserResult", "x-amzn-request-id: " << m_responseMetadata.GetRequestId() );
  }
  return *this;
}
