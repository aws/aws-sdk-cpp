...
static const char* ALLOCATION_TAG = "AWSEBSEndpointProviderTests";
using EBSEndpointProvider = Aws::EBS::Endpoint::EBSEndpointProvider;
using EndpointParameters = Aws::Vector<Aws::Endpoint::EndpointParameter>;
using EBSResolveEndpointOutcome = Aws::EBS::Endpoint::EBSEndpointProvider::EBSResolveEndpointOutcome;
...
class EBSEndpointProviderTests :public ::testing::TestWithParam<size_t>
{
public:
...
struct EBSEndpointProviderEndpointTestCase
{
...
static const Aws::Vector<EBSEndpointProviderEndpointTestCase> TEST_CASES = {
  /*TEST CASE 0*/
...
void ValidateOutcome(const EBSResolveEndpointOutcome& outcome, const EBSEndpointProviderEndpointTestCase::Expect& expect)
{
...
TEST_P(EBSEndpointProviderTests, EndpointProviderTest)
{
    ...
    const EBSEndpointProviderEndpointTestCase& TEST_CASE = TEST_CASES.at(TEST_CASE_IDX);
    SCOPED_TRACE(Aws::String("\nTEST CASE # ") + Aws::Utils::StringUtils::to_string(TEST_CASE_IDX) + ": " + TEST_CASE.documentation);

    std::shared_ptr<EBSEndpointProvider> endpointProvider = Aws::MakeShared<EBSEndpointProvider>(ALLOCATION_TAG);
    ASSERT_TRUE(endpointProvider) << "Failed to allocate/initialize EBSEndpointProvider";
    ...
    for(const auto& operation : TEST_CASE.operationInput)
    {
         ...
         */
        const Aws::Vector<std::reference_wrapper<const EBSEndpointProviderEndpointTestCase::OperationParamsFromTest>>
