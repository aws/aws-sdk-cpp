...
void TimestreamQueryClient::LoadTimestreamQuerySpecificConfig(const Aws::Client::ClientConfiguration& clientConfiguration)
{
  ...
  else
  {
    ...
    {
      AWS_LOGSTREAM_WARN("TimestreamQueryClient", "\"AWS_ENABLE_ENDPOINT_DISCOVERY\" in environment variables or \"endpoint_discovery_enabled\" in config file"
                                  << "should either be true of false if specified, otherwise follow the default behavior.");
    }
  }
}
