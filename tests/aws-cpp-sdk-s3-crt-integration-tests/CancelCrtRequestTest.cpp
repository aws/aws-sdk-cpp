...
protected:
  void SetUp() override {
    ...
    ASSERT_TRUE(WaitForBucketToPropagate(m_bucketName));
  ...
  void DeleteBucket(const Aws::String& bucketName) {
    ...
    if (bucketOutcome.IsSuccess()) {
      ...
      auto deleteBucketOutcome = CallOperationWithUnconditionalRetry(m_client.get(), &Aws::S3Crt::S3CrtClient::DeleteBucket,
                                                                     deleteBucketRequest);
...
TEST_F(CancelCrtRequestTest, ShouldCancelCrtRequest) {
  ...
  // Try to put something else but abort the operation
  {
     ...
     */
    class SlowTestStream : public Aws::Utils::Stream::SimpleStreamBuf {
    public:
      std::function<void()> m_onReadCallback;

      explicit SlowTestStream(const Aws::String& value, std::function<void()>&& onReadCallback)
    ...
    putObjectRequest2.SetContinueRequestHandler([&shouldContinueAtomic](const HttpRequest*) {
      return shouldContinueAtomic.load();
    });
    ...
    SlowTestStream slowStream(largePayloadToBeSplitIntoMultiPart, [&shouldContinueAtomic]() {
      shouldContinueAtomic.store(false);
    });
    std::shared_ptr<Aws::IOStream> objectStream = Aws::MakeShared<Aws::IOStream>("CancelCrtRequestTest", &slowStream);
    ...
    auto asyncHandler = [&putObjectOutcome2, &mtx, &cv, &handlerCalled](const S3CrtClient*, const Model::PutObjectRequest&,
      const Model::PutObjectOutcome& outcome, const std::shared_ptr<const Aws::Client::AsyncCallerContext>&) {
    ...
    //AWS_ASSERT_SUCCESS(putObjectOutcome2);
  }
}
}
