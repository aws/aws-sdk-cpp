...
class AwsStreamBufTest : public Aws::Testing::AwsCppSdkGTestSuite
{
};

//Write to an empty buffer via a stream interface, and make sure the buffer
//contains the data.
TEST_F(AwsStreamBufTest, TestStreamWriteToPrefilledBuffer)
{
    AwsStreamBuf streamBuf;
...
TEST_F(AwsStreamBufTest, TestStreamReadSeekBeg)
{
    AwsStreamBuf streamBuf;
    Aws::IOStream ioStream(&streamBuf);
    ioStream.write(bufferStr, sizeof(bufferStr));
...
TEST_F(AwsStreamBufTest, TestStreamReadSeekCur)
{
    AwsStreamBuf streamBuf;
    Aws::IOStream ioStream(&streamBuf);
    ioStream.write(bufferStr, sizeof(bufferStr));
...
TEST_F(AwsStreamBufTest, TestStreamReadSeekEnd)
{
    AwsStreamBuf streamBuf;
    Aws::IOStream ioStream(&streamBuf);
    ioStream.write(bufferStr, sizeof(bufferStr));
...
TEST_F(AwsStreamBufTest, TestStreamWriteSeekBeg)
{
    AwsStreamBuf streamBuf;
    Aws::IOStream ioStream(&streamBuf);
    ioStream.write(bufferStr, sizeof(bufferStr));
...
TEST_F(AwsStreamBufTest, TestStreamWriteSeekEnd)
{
    AwsStreamBuf streamBuf;
    Aws::IOStream ioStream(&streamBuf);
    ioStream.write(bufferStr, sizeof(bufferStr));
...
TEST_F(AwsStreamBufTest, TestZeroLengthSeekFromEnd)
{
    AwsStreamBuf streamBuf;
    Aws::IOStream ioStream(&streamBuf);
    ioStream.write(bufferStr, sizeof(bufferStr));
...
TEST_F(AwsStreamBufTest, SetStr)
{
    AwsStreamBuf streamBuf;
...
TEST_F(AwsStreamBufTest, StringConstructor)
{
    AwsStreamBuf streamBuf(bufferStr);
    ASSERT_STREQ(bufferStr, streamBuf.str().c_str());
}
