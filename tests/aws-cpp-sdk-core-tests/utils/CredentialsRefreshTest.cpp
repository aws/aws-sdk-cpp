...
#include <aws/core/internal/CredentialsRefresh.h>
...
namespace
{
    ...
    using Cache = CredentialsRefreshState<Creds>;
...
class CredentialsRefreshTest : public Aws::Testing::AwsCppSdkGTestSuite {};

TEST_F(CredentialsRefreshTest, RunsSepTestCases)
{
...
namespace
{
    // Scripted source the decorator wraps; each fetch returns whatever was programmed.
    class ScriptedSource : public Aws::Auth::CredentialsSource
    {
    public:
        void SetNext(RefreshResult<AWSCredentials> next) { m_next = std::move(next); }
        int FetchCount() const { return m_fetchCount; }

        RefreshResult<AWSCredentials> FetchCredentials() override
        {
            ++m_fetchCount;
            return m_next;
        }
    ...
    // Decorator with an injected clock so the pinned window durations are exact.
    class ClockedRefreshProvider : public Aws::Auth::CredentialsRefreshProvider
    {
    public:
        ClockedRefreshProvider(std::shared_ptr<Aws::Auth::CredentialsSource> source, std::shared_ptr<DateTime> clock)
            : CredentialsRefreshProvider(std::move(source)), m_now(std::move(clock))
        {
        }
...
TEST_F(CredentialsRefreshTest, RunsLifecycleAndInvalidate)
{
    auto now = std::make_shared<DateTime>(DateTime::Now());
    auto source = Aws::MakeShared<ScriptedSource>("CredentialsRefreshTest");
    ClockedRefreshProvider provider(source, now);

    // Initial fetch: 60-minute credentials.
    source->SetNext(FreshCreds("AKID-1", *now + std::chrono::minutes(60)));
    ...
    *now = *now + std::chrono::minutes(50);
    source->SetNext(FreshCreds("AKID-2", *now + std::chrono::minutes(60)));
    ...
    source->SetNext(FreshCreds("AKID-3", *now + std::chrono::minutes(60)));
    provider.Invalidate();
    EXPECT_EQ("AKID-3", provider.GetAWSCredentials().GetAWSAccessKeyId());
    EXPECT_EQ(3, source->FetchCount());
}
