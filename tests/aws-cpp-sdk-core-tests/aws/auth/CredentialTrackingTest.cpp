...
protected:
    ...
    void RunTestWithCredentialsProvicer(const std::shared_ptr<AWSCredentialsProvider>& credentialsProvider, std::string letter) {
      // Setup mock response
      ...
      auto businessMetrics = std::find_if(userAgentParsed.begin(), userAgentParsed.end(),
          [&letter](const Aws::String& value) { return value.find("m/") != Aws::String::npos && value.find(letter) != Aws::String::npos; });
...
TEST_F(CredentialTrackingTest, TestEnvironmentCredentialsTracking)
{
    ...
    RunTestWithCredentialsProvicer(std::move(credsProvider), "g");
}
...
TEST_F(CredentialTrackingTest, TestInstanceProfileCredentialsTracking)
{
    ...
    auto imdsProvider = Aws::MakeShared<InstanceProfileCredentialsProvider>(TEST_LOG_TAG,
        Aws::MakeShared<Aws::Config::EC2InstanceProfileConfigLoader>(TEST_LOG_TAG, mockClient), 1000 * 60 * 15);
