...
int main(int argc, char** argv) {
  ...
  {
    for (const auto& testConfig : PerformanceTest::Services::SecretsManager::TestConfig::TestMatrix) {
      const size_t metricCount = PerformanceTest::Services::SecretsManager::TestConfig::GetSecretSizeValue(testConfig.secretSize);
      const Aws::String outputFile = "SecretsManager-" + std::to_string(metricCount) + "-metrics-performance-test-results.json";
      ...
      sdkOptions.monitoringOptions.customizedMonitoringFactory_create_fn = {[&, metricCount, outputFile]() -> Aws::UniquePtr<Aws::Monitoring::MonitoringFactory> {
        ...
        for (const auto& operation : PerformanceTest::Services::SecretsManager::TestConfig::TestOperations) {
      ...
      PerformanceTest::Services::SecretsManager::SecretsManagerPerformanceTest performanceTest(clientConfig, testConfig, iterations);
