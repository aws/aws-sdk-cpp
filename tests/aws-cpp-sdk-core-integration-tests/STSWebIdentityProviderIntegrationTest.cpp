...
namespace {
const char* TEST_LOG_TAG = "STSWebIdentityProviderIntegrationTest";
...
class CongnitoIdentitySetup {
 public:
  CongnitoIdentitySetup(const Aws::String& testIdentifier)
  ...
  void Init() {
    if (m_state != STATE::CREATED) {
      AWS_LOGSTREAM_ERROR(TEST_LOG_TAG, "Can only call init once after creation");
      return;
    }
  ...
  void Cleanup() {
    if (m_state != STATE::INIT) {
      AWS_LOGSTREAM_ERROR(TEST_LOG_TAG, "Can only call cleanup once after init");
      return;
    }
...
class STSWebIdentityProviderIntegrationTest : public testing::Test {
 protected:
  STSWebIdentityProviderIntegrationTest() {
    ...
    InitAPI(m_options);
    m_cognitoIdentitySetup = Aws::MakeUnique<CongnitoIdentitySetup>(TEST_LOG_TAG, UUID::RandomUUID());
    m_cognitoIdentitySetup->Init();
  }

  ~STSWebIdentityProviderIntegrationTest() {
    m_cognitoIdentitySetup->Cleanup();
    ShutdownAPI(m_options);
  }

  SDKOptions m_options;
  Aws::UniquePtr<CongnitoIdentitySetup> m_cognitoIdentitySetup{nullptr};
};

TEST_F(STSWebIdentityProviderIntegrationTest, ShouldWork) {
  ClientConfiguration config;
  config.credentialProviderConfig.region = config.region;
  config.credentialProviderConfig.stsCredentialsProviderConfig.roleArn = m_cognitoIdentitySetup->GetRoleArn();
  config.credentialProviderConfig.stsCredentialsProviderConfig.tokenFilePath = m_cognitoIdentitySetup->AccessTokenFile().GetFileName();
