...
namespace Model
{
  ...
  /**
   * <p>Describes an API key.</p> <p>Customers invoke AWS AppSync GraphQL APIs with
   * API keys as an identity mechanism. There are two key versions:</p> <p>
   ...
   * date.</p> <ul> <li> <p> <code>ListApiKeys</code> returns the expiration time in
   ...
   * action, we will delete these keys from the table after Feb 21, 2018.</p> </li>
   * </ul> <p> <b>da2</b>: This version was introduced in February 2018 when AppSync
   * added support to extend key expiration.</p> <ul> <li> <p>
   * <code>ListApiKeys</code> returns the expiration time in seconds.</p> </li> <li>
   * <p> <code>CreateApiKey</code> returns the expiration time in seconds and accepts
   * a user-provided expiration time in seconds.</p> </li> <li> <p>
   ...
   * while the key has not expired.</p> </li> <li> <p> <code>DeleteApiKey</code>
   * deletes the item from the table.</p> </li> <li> <p>Expiration is stored in
   * DynamoDB as seconds.</p> </li> </ul><p><h3>See Also:</h3>   <a
