...
namespace Model
{
  ...
  public:
     ...
     * include the following:</p> <p> <b>AWS_ALIAS_DNS_NAME</b> </p> <p>If you want AWS
     * Cloud Map to create an Amazon Route 53 alias record that routes traffic to an
     ...
     * Cloud Map will create the Route 53 health check, but it won't associate the
     * health check with the alias record.</p> </li> <li> <p>Auto naming currently
     * doesn't support creating alias records that route traffic to AWS resources other
     * than Elastic Load Balancing load balancers.</p> </li> <li> <p>If you specify a
     * value for <code>AWS_ALIAS_DNS_NAME</code>, don't specify values for any of the
     * <code>AWS_INSTANCE</code> attributes.</p> </li> </ul> <p>
     * <b>AWS_EC2_INSTANCE_ID</b> </p> <p> <i>HTTP namespaces only.</i> The Amazon EC2
     * instance ID for the instance. If the <code>AWS_EC2_INSTANCE_ID</code> attribute
     ...
     * be filled out with the primary private IPv4 address.</p> <p>
     ...
     * configuration includes a <code>CNAME</code> record, the domain name that you
     ...
     * includes an <code>A</code> record, the IPv4 address that you want Route 53 to
     ...
     * includes settings for an <code>A</code> record. If the service includes settings
     * for an <code>SRV</code> record, you must specify a value for
     * <code>AWS_INSTANCE_IPV4</code>, <code>AWS_INSTANCE_IPV6</code>, or both.</p> <p>
     * <b>AWS_INSTANCE_IPV6</b> </p> <p>If the service configuration includes an
     * <code>AAAA</code> record, the IPv6 address that you want Route 53 to return in
     ...
     * <code>AWS_INSTANCE_IPV4</code>, <code>AWS_INSTANCE_IPV6</code>, or both.</p> <p>
     * <b>AWS_INSTANCE_PORT</b> </p> <p>If the service includes an <code>SRV</code>
     * record, the value that you want Route 53 to return for the port.</p> <p>If the
     * service includes <code>HealthCheckConfig</code>, the port on the endpoint that
     * you want Route 53 to send requests to. </p> <p>This value is required if you
     * specified settings for an <code>SRV</code> record or a Route 53 health check
     * when you created the service.</p> <p> <b>Custom attributes</b> </p> <p>You can
     * add up to 30 custom attributes. For each key-value pair, the maximum length of
     * the attribute name is 255 characters, and the maximum length of the attribute
     * value is 1,024 characters. The total size of all provided attributes (sum of all
     * keys and values) must not exceed 5,000 characters.</p>
     */
