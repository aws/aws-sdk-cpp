...
static const char* ALLOCATION_TAG = "AWSMobileEndpointProviderTests";
using MobileEndpointProvider = Aws::Mobile::Endpoint::MobileEndpointProvider;
using EndpointParameters = Aws::Vector<Aws::Endpoint::EndpointParameter>;
using MobileResolveEndpointOutcome = Aws::Mobile::Endpoint::MobileEndpointProvider::MobileResolveEndpointOutcome;
...
class MobileEndpointProviderTests :public ::testing::TestWithParam<size_t>
{
public:
...
struct MobileEndpointProviderEndpointTestCase
{
...
static const Aws::Vector<MobileEndpointProviderEndpointTestCase> TEST_CASES = {
  /*TEST CASE 0*/
...
void ValidateOutcome(const MobileResolveEndpointOutcome& outcome, const MobileEndpointProviderEndpointTestCase::Expect& expect)
{
...
TEST_P(MobileEndpointProviderTests, EndpointProviderTest)
{
    ...
    const MobileEndpointProviderEndpointTestCase& TEST_CASE = TEST_CASES.at(TEST_CASE_IDX);
    SCOPED_TRACE(Aws::String("\nTEST CASE # ") + Aws::Utils::StringUtils::to_string(TEST_CASE_IDX) + ": " + TEST_CASE.documentation);

    std::shared_ptr<MobileEndpointProvider> endpointProvider = Aws::MakeShared<MobileEndpointProvider>(ALLOCATION_TAG);
    ASSERT_TRUE(endpointProvider) << "Failed to allocate/initialize MobileEndpointProvider";
    ...
    for(const auto& operation : TEST_CASE.operationInput)
    {
         ...
         */
        const Aws::Vector<std::reference_wrapper<const MobileEndpointProviderEndpointTestCase::OperationParamsFromTest>>
